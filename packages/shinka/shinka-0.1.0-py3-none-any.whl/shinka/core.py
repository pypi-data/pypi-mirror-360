...
def _edge_sharpen(
    ...
    detail = image - gaussian_filter(image, sigma)
    mask = sobel(image) > thr
...
def _to_pil_and_save(arr: np.ndarray, save_path: Optional[str] = None) -> Image.Image:
    arr255 = (arr * 255).clip(0, 255).astype("uint8")
    if arr255.ndim == 2:
        out_img = Image.fromarray(arr255, mode="L")
    else:
        mode = (
...
def upscale(
    ...
    if arr.ndim == 2:
        ...
        out_arr = _edge_sharpen(arr, sigma=0.676, amount=1.571, thr=0.0113)
    elif arr.ndim == 3:
        out_channels = []
