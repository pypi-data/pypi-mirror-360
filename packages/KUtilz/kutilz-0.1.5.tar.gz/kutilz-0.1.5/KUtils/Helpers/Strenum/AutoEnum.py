...
class AutoEnum(str, Enum):
    ...
    @classmethod
    def Builder(cls, name: str) -> '_AutoEnumBuilder':
        return _AutoEnumBuilder(name)


class _AutoEnumBuilder:
    def __init__(self, name: str):
        self.name = name
        self.items = {}
        self.current = None

    def __update_cur(self, **kwargs):
        assert self.current is not None
        self.items.setdefault(self.current, {})
        self.items[self.current].update(kwargs)
    ...
    def Alias(self, *alias) -> Self:
        self.__update_cur(aliases=alias)
        return self

    def Aliased(self, name: str, *alias) -> Self:
        return self.Add(name).Alias(*alias)

    def Finished(self) -> Union[Any, AutoEnum]:
        _dict = {}
        for name, info in self.items.items():
            _dict[name] = alias(*info['aliases'])

        return AutoEnum(self.name, _dict)
        # return types.new_class(
        #     self.name,
        #     (AutoEnum),
        #     exec_body=lambda ns: ns.update(_dict)
        # )

if __name__ == '__main__':

    e = AutoEnum.Builder('test').Aliased(
        'one', '1', '2'
    ).Aliased(
        'two', '3', '4'
    ).Finished()
