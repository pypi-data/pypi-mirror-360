...
def catch_invalid_geohash_id(f: Callable) -> Callable:
    """Wrapper that catches potential invalid geohash ID.
    ...
    def safe_f(*args, **kwargs):
        try:
            ...
            message = "geohash method raised an error. Is the geohash ID correct?"
...
def doc_standard(column_name: str, description: str) -> Callable:
    """Wrapper to provide a standard apply-to-geohash-id docstring"""
    ...
    def doc_decorator(f):
        ...
        doc = f"""Adds the column `{column_name}` {description}. Assumes geohash ID.
        {parameters}
        Returns
        -------
        ...
        ------
        ValueError
            When an invalid geohash address is encountered
        """
