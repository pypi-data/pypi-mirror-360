...
def catch_invalid_tilecode_id(f: Callable) -> Callable:
    """Wrapper that catches potential invalid Tilecode ID.
    ...
    def safe_f(*args, **kwargs):
        try:
            ...
            message = "Tilecode method raised an error. Is the Tilecode ID correct?"
...
def doc_standard(column_name: str, description: str) -> Callable:
    """Wrapper to provide a standard apply-to-Tilecode-id docstring"""
    ...
    def doc_decorator(f):
        ...
        doc = f"""Adds the column `{column_name}` {description}. Assumes Tilecode ID.
        {parameters}
        Returns
        -------
        ...
        ------
        ValueError
            When an invalid Tilecode address is encountered
        """
