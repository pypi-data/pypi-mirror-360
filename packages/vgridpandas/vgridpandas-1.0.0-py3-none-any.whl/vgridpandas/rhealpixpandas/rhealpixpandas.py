...
from .util.decorator import catch_invalid_rhealpix_id
...
class rHEALPixPandas:
    ...
    def latlon2rhealpix(
        ...
        Returns
        -------
        (Geo)DataFrame with RHEALPIX IDs added     
        """
        ...
        rhealpix_ids = [
            latlon2rhealpix(lat, lon, resolution) for lat, lon in zip(lats, lons)
        ]

        colname = self._format_resolution(resolution)
        assign_arg = {colname: rhealpix_ids}
        df = self._df.assign(**assign_arg)
        if set_index:
            return df.set_index(colname)
        return df

    def rhealpix2geo(self) -> GeoDataFrame:
    ...
    def _apply_index_assign(
        ...
        """
        func = catch_invalid_rhealpix_id(func)
    ...
    def _apply_index_explode(
        ...
        result = (
            pd.DataFrame.from_dict(
                {rhealpix_id: processor(func(rhealpix_id)) for rhealpix_id in self._df.index},
                orient="index",
            )
            .stack()
