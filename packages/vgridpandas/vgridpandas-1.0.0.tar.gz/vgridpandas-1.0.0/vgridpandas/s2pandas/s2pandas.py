...
class S2Pandas:
    ...
    def latlon2s2(
        ...
        See Also
        --------
        geo_to_h2_aggregate : Extended API method that aggregates points by S2 ID
        ...
        >>> df.s2.latlon2s2(8)
                        lat                   lon
        s2_08
                        10.775299665325505   106.70647975040788
                        10.775299665325505   -171.58256682775985

        >>> df.s2.latlon2s2(8, set_index=False)
            lat                  lon                    s2_08
        0   10.775299665325505   106.70647975040788
        1   10.775299665325505   -171.58256682775985
        ...
        >>> gdf.s2.latlon2s2(8)
                        val                   geometry
        s2_08
                       5  POINT (10.775299665325505 106.70647975040788)
                       1  POINT (10.775299665325505 -171.58256682775985)

        """
        if not isinstance(resolution, int) or resolution not in range(0, 31):
            raise ValueError("Resolution must be an integer in range [0, 30]")
    ...
    #     """Add `geometry` with centroid of each S2 Token to the DataFrame.
    #     Assumes H3 index.
    ...
    # @doc_standard("h3_resolution", "containing the resolution of each S2 Token")
    ...
    #     >>> df = pd.DataFrame({'val': [5, 1]},
    #     >>>                   index=['881e309739fffff', '881e2659c3fffff'])
    #     >>> df.s2.h3_get_resolution()
    #                      val  h3_resolution
    #     881e309739fffff    5              8
    #     881e2659c3fffff    1              8
    #     """
    #     return self._apply_index_assign(s2.get_resolution, "h3_resolution")

    # @doc_standard("h3_base_cell", "containing the base cell of each S2 Token")
    ...
    #     >>> df = pd.DataFrame({'val': [5, 1]},
    #     >>>                   index=['881e309739fffff', '881e2659c3fffff'])
    #     >>> df.s2.h3_get_base_cell()
    #                      val  h3_base_cell
    #     881e309739fffff    5            15
    #     881e2659c3fffff    1            15
    #     """
    #     return self._apply_index_assign(s2.get_base_cell_number, "h3_base_cell")

    # @doc_standard("h3_is_valid", "containing the validity of each S2 Token")
    ...
    #     INVALID            1        False
    #     """
    #     return self._apply_index_assign(s2.is_valid_cell, "h3_is_valid")
    ...
    #     ----------
    #     k : int
    #         the distance from the origin S2 Token. Default k = 1
    #     explode : bool
    ...
    #     """
    #     func = wrapped_partial(s2.grid_disk, k=k)
    ...
    #     881e309739fffff    5  881e309731fffff
    #     """
    #     func = wrapped_partial(s2.grid_ring, k=k)
    #     column_name = "h3_hex_ring"
    #     if explode:
    #         return self._apply_index_explode(func, column_name, list)
    #     return self._apply_index_assign(func, column_name, list)

    # @doc_standard("h3_{resolution}", "containing the parent of each S2 Token")
    ...
    #     )
    #     return self._apply_index_assign(
    #         wrapped_partial(s2.cell_to_parent, res=resolution), column
    #     )

    # @doc_standard("h3_center_child", "containing the center child of each S2 Token")
    ...
    #     >>> df = pd.DataFrame({'val': [5, 1]},
    #     >>>                    index=['881e309739fffff', '881e2659c3fffff'])
    #     >>> df.s2.h3_to_center_child()
    #                      val  h3_center_child
    #     881e309739fffff    5  891e3097383ffff
    #     881e2659c3fffff    1  891e2659c23ffff
    #     """
    #     return self._apply_index_assign(
    #         wrapped_partial(s2.cell_to_center_child, res=resolution), "h3_center_child"
    #     )
    ...
    #     >>> gdf.s2.polyfill(4, explode=True)
    #                                                 geometry      h3_polyfill
    ...
    # @doc_standard("h3_cell_area", "containing the area of each S2 Token")
    # def cell_area(
    ...
    #     """
    #     return self._apply_index_assign(
    #         wrapped_partial(s2.cell_area, unit=unit), "h3_cell_area"
    #     )
    ...
    #     Returns
    #     -------
    #     (Geo)DataFrame aggregated by S2 Token into which each row's point falls
    ...
    #         .agg(operation)
    #     )
    #     return grouped.s2.h3_to_geo_boundary() if return_geometry else grouped
    ...
    #     Returns
    #     -------
    #     (Geo)DataFrame aggregated by the parent of each S2 Token
    ...
    #     >>> df = pd.DataFrame({'val': [5, 1]},
    #     >>>                   index=['881e309739fffff', '881e2659c3fffff'])
    #     >>> df.s2.h3_to_parent(1)
    #                      val            h3_01
    #     881e309739fffff    5  811e3ffffffffff
    #     881e2659c3fffff    1  811e3ffffffffff
    #     >>> df.s2.h3_to_parent_aggregate(1)
    #                      val                                           geometry
    #     h3_01
    #     811e3ffffffffff    6  POLYGON ((12.34575 50.55428, 12.67732 46.40696...
    #     >>> df.s2.h3_to_parent_aggregate(1, operation='mean')
    #                      val                                           geometry
    #     h3_01
    #     811e3ffffffffff    3  POLYGON ((12.34575 50.55428, 12.67732 46.40696...
    #     >>> df.s2.h3_to_parent_aggregate(1, return_geometry=False)
    #                      val
    #     h3_01
    #     811e3ffffffffff    6
    #     """
    #     parent_h3addresses = [
    #         catch_invalid_h3_address(s2.cell_to_parent)(h3address, resolution)
    ...
    #     ----------
    #     k : int
    #         The distance from the origin S2 Token
    ...
    #     >>> df.s2.k_ring_smoothing(weights=[2, 1])
    #                        val                                           geometry
    #     h3_hex_ring
    ...
    #     >>> df.s2.k_ring_smoothing(1, return_geometry=False)
    #                           val
    #     h3_k_ring
    ...
    #     if weights is None:
    #         result = pd.DataFrame(
    #             df.s2.k_ring(k, explode=True)
    #             .groupby("h3_k_ring")
    #             .sum()
    #             .divide((1 + 3 * k * (k + 1)))
    #         )

    #         return result.s2.h3_to_geo_boundary() if return_geometry else result
    ...
    #         return df.s2.hex_ring(k, explode=True).s2._multiply_numeric(
    #             normalized_weight
    #         )
    ...
    #     """
    #     result = self._df.s2.polyfill(resolution, explode=True)
    ...
    #     >>> gdf.s2.linetrace(4, explode=True)
    #                                                 geometry     h3_linetrace
