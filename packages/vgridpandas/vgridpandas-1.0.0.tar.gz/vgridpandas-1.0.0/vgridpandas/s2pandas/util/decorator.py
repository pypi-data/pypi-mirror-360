...
def catch_invalid_s2_token(f: Callable) -> Callable:
    """Wrapper that catches potential invalid s2 Token.
...
def doc_standard(column_name: str, description: str) -> Callable:
    """Wrapper to provide a standard apply-to-s2-id docstring"""
    ...
    def doc_decorator(f):
        ...
        doc = f"""Adds the column `{column_name}` {description}. Assumes S2 Token.
        {parameters}
        Returns
        -------
