...
def catch_invalid_mgrs_id(f: Callable) -> Callable:
    """Wrapper that catches potential invalid MGRS ID.
    ...
    def safe_f(*args, **kwargs):
        try:
            ...
            message = "MGRS method raised an error. Is the MGRS ID correct?"
...
def doc_standard(column_name: str, description: str) -> Callable:
    """Wrapper to provide a standard apply-to-mgrs-id docstring"""
    ...
    def doc_decorator(f):
        ...
        doc = f"""Adds the column `{column_name}` {description}. Assumes MGRS ID.
        {parameters}
        Returns
        -------
        ...
        ------
        ValueError
            When an invalid MGRS address is encountered
        """
