...
def catch_invalid_quadkey_id(f: Callable) -> Callable:
    """Wrapper that catches potential invalid Quadkey ID.
    ...
    def safe_f(*args, **kwargs):
        try:
            ...
            message = "Quadkey method raised an error. Is the Quadkey ID correct?"
...
def doc_standard(column_name: str, description: str) -> Callable:
    """Wrapper to provide a standard apply-to-Quadkey-id docstring"""
    ...
    def doc_decorator(f):
        ...
        doc = f"""Adds the column `{column_name}` {description}. Assumes Quadkey ID.
        {parameters}
        Returns
        -------
        ...
        ------
        ValueError
            When an invalid Quadkey address is encountered
        """
