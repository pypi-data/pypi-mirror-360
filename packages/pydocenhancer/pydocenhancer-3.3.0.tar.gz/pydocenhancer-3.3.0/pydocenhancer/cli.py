...
@cli.command()
@click.option("--module", required=True, help="Path to Python module")
@click.option("--output", default="README.generated.md", help="Output path for generated README.md")
@click.option("--provider", required=True, help="AI provider (openai, local [ctransformers, ollama])")
@click.option("--model", required=True, help="Model name (e.g., llama3.2, ollama/llama3.2:latest)")
@click.option("--api-key", default=None, help="API key for cloud providers")
@click.option("--language", default="en", help="Language code for documentation (e.g., en, fr, es, zh)")
def generate_readme(module, output, provider, model, api_key, language):
    """Generate a project-level README.md summarizing the module, its classes, and functions."""
    enhancer = DocEnhancer(provider=provider, model=model, api_key=api_key, language=language)
    enhancer.generate_readme(module_path=module, output_path=output, language=language)
    click.echo(f"README generated at {output} (language: {language})")
