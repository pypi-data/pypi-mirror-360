...
from tuir.objects import codecs_open
...
def test_terminal_open_editor(terminal, use_ascii):
    ...
    def side_effect(args):
        ...
        with codecs_open(data['filename'], 'r+', encoding='utf-8') as fp:
...
def test_terminal_load_mailcaps(stdscr, config):
    with mock.patch('os.path.exists', return_value=True):
        env_mailcaps = os.environ.get('MAILCAPS')
        try:
            ...
            assert term._mailcap_dict is not None
            assert os.environ['MAILCAPS'] == 'NON_NULL'
        finally:
            if env_mailcaps is None:
                del os.environ['MAILCAPS']
            else:
                os.environ['MAILCAPS'] = env_mailcaps
...
def test_terminal_open_urlview(terminal, stdscr):
    ...
    with mock.patch('subprocess.Popen') as Popen, \
        ...
        Popen.side_effect = side_effect
        terminal.open_urlview(data)
        message = 'Failed to open urlview'.encode('utf-8')
        print(stdscr.method_calls)
        stdscr.subwin.addstr.assert_called_with(1, 1, message)
