...
from .objects import LoadScreen, codecs_open
...
try:
    ...
    unescape = html_parser.HTMLParser().unescape  # pylint: disable=E1101
...
def import_mailcap():
    ...
    try:
        ...
        pass
    from .packages.mailcap import mailcap
    return mailcap


def open_url_silent(url, force_new_browser_window):
...
class Terminal(object):
    ...
    def _load_mailcaps(self):
        if self._mailcap_dict is not None:
            return
        mailcap = import_mailcap()
    ...
    def open_browser(self, url):
        ...
        if self.display:
            with self.loader('Opening page in a new window'):
                p = Process(target=open_url_silent, args=(url, self.config['force_new_browser_window']))
                p.start()
    ...
    def open_editor(self, data=''):
        ...
        with codecs_open(filepath, 'w', encoding='utf-8') as fp:
            fp.write(data)
        ...
        with codecs_open(filepath, 'r', encoding='utf-8') as fp:
            text = fp.read()
