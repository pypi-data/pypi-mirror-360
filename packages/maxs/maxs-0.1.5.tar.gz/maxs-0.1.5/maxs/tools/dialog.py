...
TOOL_SPEC = {
    "name": "dialog",
    "description": (
        "Create interactive dialog interfaces with advanced features including validation, forms, rich text, and more"
    ),
    "inputSchema": {
        "json": {
            "type": "object",
            "properties": {
                ...
                "text": {
                    "type": "string",
                    "description": "Main text or question to display (supports HTML formatting for rich_message)",
                },
                "options": {
                    "type": "array",
                    "description": "Options for radio, checkbox, buttons, or autocomplete dialogs as [value, label] pairs",
                },
                "style": {
                    "type": "string",
                    "enum": ["default", "blue", "green", "purple", "dark"],
                ...
                "form_fields": {
                    "type": "array",
                    "description": "Fields for multi-input form dialogs",
                    "items": {
                        "type": "object",
                        "properties": {
                ...
                "progress_steps": {
                    "type": "integer",
                    "description": "Number of steps for progress bar dialog",
                },
                "step_delay": {
                    "type": "number",
                    "description": "Delay in seconds between progress steps",
                },
                "key_bindings": {
                    "type": "object",
                    "description": "Custom key bindings for dialogs",
                    "additionalProperties": {"type": "string"},
                },
                "path_filter": {
                    "type": "string",
                    "description": "Filter for file selection dialog (e.g., '*.py', '*.txt')",
                },
                "multiline": {
                    "type": "boolean",
                    "description": "Whether input should allow multiple lines",
                },
            },
            "required": ["dialog_type", "text"],
        }
    },
}
...
def dialog(tool: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    """
    Create interactive dialogs with advanced features.

    Args:
        tool: The tool parameters with input containing dialog configuration.
        **kwargs: Additional keyword arguments that might be passed by the system,
                 including system_prompt, model_id, etc. We don't use these
                 directly but need to accept them.
    ...
    try:
        ...
        dialog_type = tool_input["dialog_type"]
        text = tool_input["text"]
        title = tool_input.get("title", "Dialog")

        # Get and parse options if needed
        ...
        # Get other parameters
        progress_steps = tool_input.get("progress_steps")
        step_delay = tool_input.get("step_delay")
        key_bindings = tool_input.get("key_bindings")
