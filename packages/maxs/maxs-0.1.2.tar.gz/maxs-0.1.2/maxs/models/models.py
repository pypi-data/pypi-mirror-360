...
# Default model configuration for Ollama
...
if ADDITIONAL_REQUEST_FIELDS != "{}":
    try:
        DEFAULT_MODEL_CONFIG["additional_request_fields"] = json.loads(
...
if len(ANTHROPIC_BETA_FEATURES) > 0:
    DEFAULT_MODEL_CONFIG["additional_request_fields"]["anthropic_beta"] = (
        ANTHROPIC_BETA_FEATURES.split(",")
    )
...
def create_model(provider: str = None, config: dict[str, Any] = None) -> Model:
    ...
    Args:
        provider: Model provider name. If None, uses STRANDS_PROVIDER env var or defaults to 'ollama'.
    ...
    """
    if provider is None:
        provider = os.getenv("STRANDS_PROVIDER", "ollama")
...
def get_provider_config(provider: str) -> dict[str, Any]:
    ...
    if provider == "bedrock":
        return {
            ...
            "additional_request_fields": DEFAULT_MODEL_CONFIG[
                "additional_request_fields"
            ],
        ...
        return {
            ...
            "model_id": os.getenv(
                "STRANDS_MODEL_ID", "anthropic/claude-sonnet-4-20250514"
            ),
            "params": {
                "max_tokens": int(os.getenv("STRANDS_MAX_TOKENS", "10000")),
                "temperature": float(os.getenv("STRANDS_TEMPERATURE", "1")),
        ...
        return {
            ...
            "model_id": os.getenv(
                "STRANDS_MODEL_ID", "Llama-4-Maverick-17B-128E-Instruct-FP8"
            ),
            "params": {
        ...
        return {
            ...
            "params": {
                "max_completion_tokens": int(os.getenv("STRANDS_MAX_TOKENS", "10000"))
            },
        }
        ...
        return {
            ...
            "model_id": os.getenv("STRANDS_MODEL_ID", "mistral-large-latest"),
            "max_tokens": int(os.getenv("STRANDS_MAX_TOKENS", "4096")),
            "temperature": float(os.getenv("STRANDS_TEMPERATURE", "1")),
            "top_p": float(os.getenv("STRANDS_TOP_P", "0.9")),
            "stream": os.getenv("STRANDS_STREAM", "false").lower() == "true",
        }
...
def get_available_providers() -> list[str]:
    ...
    """
    return ["bedrock", "anthropic", "litellm", "llamaapi", "ollama", "openai", "github", "mistral"]
...
def get_provider_info(provider: str) -> dict[str, Any]:
    ...
    provider_info = {
        ...
        "mistral": {
            "name": "Mistral AI",
            "description": "Mistral's open source and premier language models",
    ...
    return provider_info.get(
        provider, {"name": provider, "description": "Custom provider"}
    )
