...
class Merger(BaseMerger):
    ...
    def get_rule_merge_edits(self, orig, corr, group_edits, align_edits):
        ...
        for start, end in combos:
            ...
            if start == 0 and ops[start] == "D" and o[0].text.lower() in language_resources.filler_words:
            ...
            if o[-1].lower == c[-1].lower:
                ...
                if start == 0 and (
                    (len(o) == 1 and c[0].text[0].isupper()) or (len(c) == 1 and o[0].text[0].isupper())
                ):
                ...
                if (len(o) > 1 and self.is_punct(o[-2])) or (len(c) > 1 and self.is_punct(c[-2])):
                    return (
                        self.get_rule_merge_edits(
                            orig,
                            corr,
            ...
            if end - start < 2:
                ...
                if end == len(group_edits) - 1 and (
                    (ops[-1] in {"D", "S"} and o[-1].pos == POS.DET) or (ops[-1] in {"I", "S"} and c[-1].pos == POS.DET)
                ):
                    return self.get_rule_merge_edits(
                        orig,
                        corr,
