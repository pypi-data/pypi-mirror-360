...
def check_attribute_predicate(
    request: processes.AttributePredicate, offer: processes.AttributePredicate
) -> bool:
    """
    ...
    """
    if not (
        request.attribute_carrier == offer.attribute_carrier
        and request.general_attribute == offer.general_attribute
        and request.predicate_type == offer.predicate_type
    ):
        return False
    if request.predicate_type == "equals":
        ...
        return (
            request.attribute_value[0]
            <= offer.attribute_value
            <= request.attribute_value[1]
        )
...
def check_process_attributes(
    request: processes.ProcessAttributes, offer: processes.ProcessAttributes
) -> bool:
    """
    ...
    for request_attribute in request.process_attributes:
        if not any(
            [
                check_attribute_predicate(request_attribute, offer_attribute)
...
def is_procedure_capable_of_process(
    procedure: procedure.Procedure, process: processes.Process
) -> bool:
    """
...
def check_origin_setup_process_attributes(
    request: procedure.ProcessAttributes, offer: procedure.ProcessAttributes
) -> bool:
    """
    Checks if the process attributes of a request are compatible with the process attributes of an offer.
    ...
    for request_attribute in request.process_attributes:
        if not any(
            [
                check_attribute_predicate(request_attribute, offer_attribute)
                for offer_attribute in offer_attributes
            ]
        ):
            return False
    return True
...
def check_target_setup_process_attributes(
    request: procedure.ProcessAttributes, offer: procedure.ProcessAttributes
) -> bool:
    """
    Checks if the process attributes of a request are compatible with the process attributes of an offer.
...
def is_procedure_origin_of_setup(
    procedure: procedure.Procedure, setup_procedure: procedure.Procedure
) -> bool:
    return check_origin_setup_process_attributes(
        request=procedure.process_attributes, offer=setup_procedure.process_attributes
    )


def is_procedure_target_of_setup(
    procedure: procedure.Procedure, setup_procedure: procedure.Procedure
) -> bool:
...
class Matcher:
    ...
    def __init__(
        self, processes: processes.Process, procedures: procedure.Procedure
    ) -> None:
        """
    ...
    def from_reference_model(
        cls, reference_model: reference_model.ReferenceModel
    ) -> "Matcher":
        """
    ...
    def get_matching_procedures(
        self, request: processes.Process
    ...
    def get_matching_processes(
        self, offer: procedure.Procedure
