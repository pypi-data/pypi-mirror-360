...
class EmbeddedTargetManager(TargetManager):
    ...
    def _listen_to_rtt(self, data_channel: "RTTUpChannel", log_channel: "RTTUpChannel"):
        ...
        json_schema = self._load_telemetry_schema()
        postcard_schema = JsonSchema(json_schema)
            ...
            try:
                ...
                with probe:
                    ...
                    if data:
                        ...
                        try:
                            json_data = self._deserialize_postcard_bytestream(postcard_schema, data)
                            ...
                            for telem in json_data:
                                ...
                                time = start_time + timedelta(microseconds=app_time)
                                self._config.add_telem_cb(
                                    TelemData(
                                        utc_time_ms=utc_timestamp_ms(time),
