...
class BrassCLI:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        global_config_file = Path.home() / ".config" / "coppersun-brass" / "config.json"
    ...
    def _save_config(self, scope: str = "local"):
        ...
        if scope == "global":
            config_file = Path.home() / ".config" / "coppersun-brass" / "config.json"
    ...
    def config_list(self):
        ...
        print(f"\n📍 Config resolution order: env vars > ~/.config/coppersun-brass/config.json > ./.brass/config.json > defaults")
    ...
    def config_audit(self):
        ...
        global_encrypted = global_config_file.exists() and 'encrypted' in str(global_config_file)
        env_secure = bool(os.getenv('ANTHROPIC_API_KEY'))
        legacy_files = len(env_files_found)
        
        if env_secure:
            print("  ✅ Using environment variables (most secure)")
        elif global_encrypted:
            print("  ✅ Using encrypted global config (recommended)")
        elif global_config_file.exists():
            print("  ⚠️  Using global config but encryption needs verification")
        else:
            print("  ❌ No secure API key storage detected")
    ...
    def config_show(self):
        ...
        print("  2. Global Config (~/.config/coppersun-brass/config.json)")
    ...
    def migrate_configurations(self, dry_run: bool = False):
        ...
        print(f"\n4. 🔧 Ready to migrate")
        print(f"💡 This will:")
        print(f"   • Create secure encrypted storage in ~/.config/coppersun-brass/")
    ...
    def init(self, mode: str = "claude-companion", integration_mode: Optional[str] = None):
        ...
        if migration_status['migration_needed']:
            ...
            print(f"💡 This will:")
            print(f"   • Encrypt API keys with machine-specific encryption")
            print(f"   • Move config to standard location (~/.config/coppersun-brass/)")
            ...
            while True:
                ...
                if api_key:
                    ...
                    if is_valid:
                        ...
                        print("🔒 API key encrypted and stored securely in ~/.config/coppersun-brass/config.json")
    ...
    def uninstall(self, credentials_only: bool = False, remove_all: bool = False, dry_run: bool = False):
        ...
        global_config = Path.home() / ".config" / "coppersun-brass" / "config.json"
        ...
        global_config_dir = Path.home() / ".config" / "coppersun-brass"
        if remove_all and global_config_dir.exists():
            files_to_remove.append(("Global config directory", global_config_dir))
            
        global_brass_dir = Path.home() / ".brass"
        if remove_all and global_brass_dir.exists():
            files_to_remove.append(("Legacy global .brass directory", global_brass_dir))
    ...
    def _start_automatic_monitoring(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if monitoring_started:
                # Run initial analysis to populate .brass files immediately
                try:
                    print("   🔍 Running initial analysis...")
                    result = brass.analyze_project()
                    if result['success']:
                        observations = result.get('summary', {}).get('total_observations', 0)
                        return True, f"✅ Automatic monitoring started with {observations} initial findings"
                    else:
                        return True, "✅ Automatic monitoring started (initial analysis pending)"
                except Exception as e:
                    return True, f"✅ Automatic monitoring started (initial analysis error: {e})"
            else:
...
def main():
    ...
    scout_scan_parser.add_argument('--detail', choices=['summary', 'normal', 'verbose'], default='normal', 
                                 help='Detail level for output (summary=current behavior, normal=enhanced, verbose=full details)')
    scout_scan_parser.add_argument('--limit', type=int, default=20, help='Maximum number of items to display per category')
    scout_scan_parser.add_argument('--format', choices=['table', 'json', 'markdown'], default='table', 
                                 help='Output format')
    scout_scan_parser.add_argument('--file', help='Focus analysis on specific file')
    ...
    scout_analyze_parser.add_argument('--detail', choices=['summary', 'normal', 'verbose'], default='normal',
                                    help='Detail level for output (summary=current behavior, normal=enhanced, verbose=full details)')
    scout_analyze_parser.add_argument('--limit', type=int, default=20, help='Maximum number of items to display per category')
    scout_analyze_parser.add_argument('--format', choices=['table', 'json', 'markdown'], default='table',
                                    help='Output format')
    scout_analyze_parser.add_argument('--file', help='Focus analysis on specific file')
