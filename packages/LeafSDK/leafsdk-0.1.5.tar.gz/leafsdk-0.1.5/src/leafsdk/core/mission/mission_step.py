    ...
    def execute_step(self, mav_proxy: MavLinkExternalProxy = None):
        if not hasattr(self, 'trajectory_sampler'):
            ...
            if (self.__offset_pos_recved or self.__offset_yaw_recved):
                try:
                    self.yaw_offset = self.info["yaw_offset"]
            ...
            if self._exec_count == self._start_count:
                ...
                try:
                    ...
                    # Update the offset values in the info by summing offset and target values together
                    self.waypoint_offset = self.target_waypoint
                    self.yaw_offset = self.target_yaw
                    self.info.update({
                        "waypoint_offset": self.waypoint_offset,
                        "yaw_offset": self.yaw_offset
                    })
                    logger.debug(f"Updated waypoint offset: {self.waypoint_offset} and yaw offset: {self.yaw_offset}")
                    self.completed = True
...
class Takeoff(_MissionStep):
    def __init__(self, mav_proxy: MavLinkExternalProxy = None, alt: float = 1.0):
        super().__init__()
