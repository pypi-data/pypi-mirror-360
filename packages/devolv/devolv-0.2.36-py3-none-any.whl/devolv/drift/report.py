...
def generate_diff_lines(local_doc: dict, aws_doc: dict):
    ...
    """
    local_clean = clean_policy(local_doc)
    aws_clean = clean_policy(aws_doc)

    # Normalize resources before dumping for diff
    local_clean["Statement"] = [
        normalize_statement(s) for s in local_clean.get("Statement", [])
    ]
    aws_clean["Statement"] = [
        normalize_statement(s) for s in aws_clean.get("Statement", [])
    ]

    local_str = json.dumps(local_clean, indent=2, sort_keys=True)
    aws_str = json.dumps(aws_clean, indent=2, sort_keys=True)
