...
class LeapLoader(LeapLoaderBase):
    ...
    def _check_handlers(self) -> List[DatasetTestResultPayload]:
        ...
        assert sum(isinstance(x, MetadataHandler) for x in dataset_base_handlers) < 100, "Metadata handles must be less than 100"
        metadata_length = 0
        for dataset_base_handler in dataset_base_handlers:
            ...
            if isinstance(dataset_base_handler, MetadataHandler):
                metadata_length += len(test_result)
                assert metadata_length < 800, "Total number of metadata columns handles must be less than 800"
