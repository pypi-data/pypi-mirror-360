# -*- coding: utf-8 -*-
"""
QtGui module provides a compatibility layer for the QtGui module of Qt.
It is used to provide backwards compatibility with older versions of Qt.
"""
...
__removed__ = ['QMatrix', 'QMimeSource', 'QProxyModel', 'QPyTextObject', 'QWorkspace', 'qSwap']  # type: list[str]
...
try:
    ...
    __all__.extend(['QMacPasteboardMime', 'QMacCocoaViewContainer', 'qRegisterDraggedTypes'])
except ImportError:
    pass
...
from PyQt5.QtCore import QStandardPaths as _QStandardPaths, QObject as _QObject
...
class QDesktopServices(QDesktopServices):
    ...
    def storageLocation(location):
        """
        :param location: QStandardPaths.StandardLocation | int
...
class QFileDialog(_QFileDialog):
    ...
    def getOpenFileNameAndFilter(*args, **kwargs):
        ...
        :param args: any
        :param kwargs: any
        :return: tuple(str | unicode | QString, str | unicode | QString)
        """
    ...
    def getOpenFileNamesAndFilter(*args, **kwargs):
        ...
        :param args: any
        :param kwargs: any
        :return: tuple(list[str | unicode | QString], str | unicode | QString)
        """
...
def _QApplication_setGraphicsSystem(system):
    ...
    :return:
    """
    warn("QApplication.setGraphicsSystem() is not available in Qt5", )
...
def _QLayout_setMargins(self, margin):
    ...
    warn(
        ...
        "Use setContentsMargins instead.", DeprecationWarning, stacklevel=2)
...
def _QGraphicsItem_rotate(self, angle):
    """
    :param angle: float | int
    :return:
    """
    warn("QGraphicsItem.rotate(angle) is obsolete and removed in PyQt5. "
         "Use setTransform(QTransform().rotate(angle), True)", DeprecationWarning, stacklevel=2)
...
def _QPainter_drawPixmapFragments(self, a1, a2, *args, **kwargs):
    """
    :param a1: union[list[QRectF], list[QPainter.PixmapFragment]]
    :param a2: union[list[QRectF], QPixmap]
    :param args: any
    :param kwargs: any
    :return:
    """
...
try:
    from .QtCore import Qt as __Qt
except:
    from QtCore import Qt as __Qt


# PyQt5 does not expose the obsolete QWheelEvent.orientation and QWheelEvent.delta
...
def __QWheelEvent_delta(self):
    ...
    :return: int
    """
    warn("QWheelEvent.delta is obsolete and removed in PyQt5. use angleDelta instead.",
         DeprecationWarning, stacklevel=2)
...
def __QWheelEvent_orientation(self):
    ...
    """
    warn("QWheelEvent.orientation is obsolete and removed in PyQt5. Use angleDelta instead.",
         DeprecationWarning, stacklevel=2)
    delta = self.angleDelta()  # type QPoint
    if delta.x() == 0 and delta.y() != 0:
