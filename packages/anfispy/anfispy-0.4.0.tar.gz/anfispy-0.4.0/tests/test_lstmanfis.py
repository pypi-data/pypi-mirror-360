...
import torch
import numpy as np
from ANFISpy.anfis import LSTMANFIS
...
def test_initialization_regression(mf_type):
    ...
    assert lstmanfis.output_var_names == 'd'
    assert lstmanfis.output_n_classes == 1
...
def test_initialization_classification(mf_type):
    ...
    assert len(lstmanfis.input_var_names) == 3
    assert lstmanfis.output_var_names == 'd'
    assert lstmanfis.output_n_classes == n_classes
...
def test_training_regression(mf_type):
    ...
    for name, param in lstmanfis.named_parameters():
