...
class TestPerformance:
    ...
    def test_loading_performance(self):
        ...
        try:
            ...
            print(
                f"✓ Performance: {len(df)} records in {loading_time:.2f}s, {memory_usage:,} bytes"
            )
...
class TestIntegration:
    ...
    def test_real_data_integration(self):
        ...
        try:
            ...
            if os.getenv("FRED_API_KEY"):
                ...
                print(
                    f"✓ Full integration: CM({len(df_cm)}) + FRED({len(df_fred)}) = Merged({len(merged_df)})"
                )
            else:
    ...
    def test_timestamp_alignment(self, mock_get, mock_fred_response, mock_coinmetrics_response):
        ...
        try:
            ...
            for source_name, df in [("CoinMetrics", df_cm), ("FRED", df_fred)]:
