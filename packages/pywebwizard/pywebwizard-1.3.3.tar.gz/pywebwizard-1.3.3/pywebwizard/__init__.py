...
class PyWebWizardBase:
    ...
    def __init__(self, spell_file: str, headers: dict = None, destruct_on_finish: bool = True,
                 external_functions_mapping: dict = None, keepass_file: str = None, keepass_password: str = None,
        ...
        self.undetectable = self.config.get('undetectable')  # Añadido para controlar el modo
    ...
    def _driver_setup(self):
        ...
        _undetectable = self.config.get('undetectable')
        _exec_path = _browser_options.get('exec_path')
        _remote = self.config.get('remote')

        if _undetectable:
            # Utilizar el driver indetectable
            ...
            if _proxy := _browser_options.get('proxy', {}):
                ...
                if _webdriver == webdriver.Firefox:
                    ...
                    # Configuración adicional para Firefox si es necesario
...
class PyWebWizard(PyWebWizardBase):
    ...
    def __init__(self, spell_file: str, spell_book: list = None, headers: dict = None,
        ...
        super().__init__(spell_file, headers, destruct_on_finish, external_functions_mapping,
                         keepass_file, keepass_password)
        
        # Inicializar el controlador de IA si se proporciona una clave de API
        ...
        if self.ai_enabled:
            try:
                ...
                logging.info("IA habilitada con éxito")
            except ImportError as e:
                self.ai_enabled = False
                logging.warning(f"No se pudo cargar el módulo de IA: {str(e)}")
                logging.warning("La funcionalidad de IA estará deshabilitada")
    ...
    async def execute_ai_task(self, task_description: str) -> bool:
        """
        ...
        try:
            ...
            # Analyze the task and get actions
    ...
    def _get_current_page_info(self) -> Dict[str, Any]:
        """
        Gets information about the current page state.
        ...
        try:
            return {
                ...
                "screenshot": self._take_screenshot_as_base64(),
                "elements": self._get_page_elements_info()
            }
        except Exception as e:
            logging.warning(f"Error getting page info: {str(e)}")
            return {}
            
    def _get_page_elements_info(self) -> List[Dict[str, Any]]:
        """
        Gets information about interactive elements on the page.
        
        :return: List of dictionaries with element information
        """
        elements = []
        try:
            # Get buttons
            buttons = self.driver.find_elements_by_xpath("//button | //input[@type='button'] | //input[@type='submit']")
            for btn in buttons[:10]:  # Limit to first 10 buttons for performance
                try:
                    elements.append({
                        "type": "button",
            ...
            # Get input fields
            inputs = self.driver.find_elements_by_xpath("//input[not(@type='hidden')] | //textarea | //select")
            for inp in inputs[:10]:  # Limit to first 10 inputs for performance
                try:
                    elements.append({
                        "type": "input",
            ...
            logging.warning(f"Error getting page elements: {str(e)}")
            return []
            
    def _get_available_actions(self, current_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Returns a list of available actions based on the current page state.
        
        :param current_state: Current page state
        :return: List of available actions
        """
        actions = []
        
        # Basic actions that are always available
        actions.append({"action": "navigate", "description": "Navigate to a specific URL"})
        
        # Add actions based on page elements
        for element in current_state.get("elements", []):
            if element["type"] == "button":
                actions.append({
                    ...
                    "interface": "xpath",
                    "query": element["xpath"],
                    "description": f"Click on button: {element.get('text', 'no text')}"
                })
                ...
                actions.append({
                    ...
                    "interface": "xpath",
                    "query": element["xpath"],
                    "description": f"Fill in field: {element.get('name', element.get('placeholder', 'field'))}"
                })
                
        return actions
        
    def _is_goal_achieved(self, goal: str, current_state: Dict[str, Any]) -> bool:
        """
        Checks if a goal has been achieved based on the current page state.
        
        :param goal: Description of the goal
        :param current_state: Current page state
        :return: True if the goal has been achieved, False otherwise
        """
        # This is a basic implementation that checks if the title or URL contains goal keywords
        goal_keywords = set(goal.lower().split())
        
        # Check title
        title = current_state.get("title", "").lower()
        if any(keyword in title for keyword in goal_keywords):
            return True
            
        # Check URL
        url = current_state.get("url", "").lower()
        if any(keyword in url for keyword in goal_keywords):
            return True
            
        return False
        
    async def _execute_action_async(self, action: Dict[str, Any]) -> bool:
        """
        ...
        """
        try:
            # Convert action to a format compatible with _execute_action
            if "interface" in action and "query" in action:
                # It's an interface action (click, fill, etc.)
                ...
                # Add content if it's a fill action
                if action_type == "fill" and "content" in action:
                    action_data[action_type]["content"] = action["content"]
                ...
                self._navigate({"navigate": {"url": action["url"]}})
                
            elif "sleep" in action:
                # It's a sleep action
                time.sleep(int(action.get("sleep", 1)))
                
            return True
            
        except Exception as e:
            logging.error(f"Error executing action {action.get('action', 'unknown')}: {str(e)}")
            return False
            
    def _take_screenshot_as_base64(self) -> Optional[str]:
        """
        Takes a screenshot and returns it as a base64 string.
        
        :return: Base64-encoded image or None if there was an error
        """
        try:
            screenshot = self.driver.get_screenshot_as_base64()
            return f"data:image/png;base64,{screenshot}"
        except Exception as e:
            logging.warning(f"Could not take screenshot: {str(e)}")
            return None
