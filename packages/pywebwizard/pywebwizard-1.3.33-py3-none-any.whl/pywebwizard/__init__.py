...
class PyWebWizardBase:
    ...
    def invoke_ai(self, task_description: str, max_cycles: int = 50, per_action_retries: int = 2) -> list:
        """
        Executes a complex task using AI to determine and iteratively refine the actions until complete.

        :param task_description: High-level description of the task to perform
        :param max_cycles: Maximum reasoning/interaction cycles before aborting
        :param per_action_retries: Number of retries per action
        :return: True if task completed successfully, False if aborted or failed
        """
        ...
        while cycle <= max_cycles:
            ...
            try:
                current_page_info = self._html({'action': 'html'})
                consecutive_failures = 0
            except Exception as e:
                consecutive_failures += 1
...
class PyWebWizard(PyWebWizardBase):
    ...
    def _click(self, action: dict):
        ...
        try:
            ...
            print("[CLICK] Intercepted, trying scroll and ActionChains...")
        except StaleElementReferenceException:
            print("[CLICK] Element went stale, waiting and refreshing reference...")
        ...
        try:
            ...
            print(f"[CLICK] ActionChains fallback failed: {e}")

        try:
            self.driver.execute_script("arguments[0].click();", _element)
            return
        except Exception as e:
            logging.error(f"[CLICK] JavaScript click also failed: {e}")
            raise e
