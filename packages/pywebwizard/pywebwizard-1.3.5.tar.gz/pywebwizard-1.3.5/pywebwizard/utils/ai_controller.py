...
class AIController:
    ...
    async def decide_next_action(self, current_state: Dict[str, Any], available_actions: List[str]) -> Dict[str, Any]:
        ...
        :param current_state: Current state of the application/web page
        :param available_actions: List of available actions
        :return: Action to perform
        """
        try:
            system_prompt = """
            You are a web automation assistant. Your task is to analyze the current 
            state of the page and decide the best action to take next.
            
            Return ONLY a JSON with the action to perform.
            Example response:
            {"action": "click", "interface": "xpath", "query": "//button[contains(text(),'Next')]"}
            """
            ...
            What is the best action to take? Respond ONLY with the action JSON.
            """
            ...
            if not isinstance(action, dict) or 'action' not in action:
                raise ValueError("The returned action does not have the expected format")
                
            return action
            
        except Exception as e:
            logging.error(f"Error deciding next action: {str(e)}")
            raise

    @staticmethod
    def _take_screenshot_as_base64(driver) -> Optional[str]:
        """
        Takes a screenshot and returns it in base64 format.
