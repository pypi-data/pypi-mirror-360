...
class ModelAPI:  # noqa: D101
    ...
    def upload_version(
        ...
        storage_version, should_create_new_version = self.validated_storage_version(mrt, base_version)

        if not dry_run and should_create_new_version:
    ...
    def validated_storage_version(
        self, mrt: ModelRegistryTarget, base_version: Optional[str] = None
    ) -> Tuple[Literal["V1", "V2"], bool]:
        ...
        if storage_version == "V1":
            ...
            return storage_version, False

        if base_version:
            self.validate_base_version(mrt.org, mrt.team, mrt.name, base_version)
        return "V2", False

    def _get_license_terms(self, org, team, name):
        resp = self.get(org, team, name)
...
class GuestModelAPI(ModelAPI):  # noqa: D101
    ...
    def _license_check(self, target, agree_license=False):
        mrt = ModelRegistryTarget(target, org_required=True, name_required=True, version_required=False)
        license_terms = self._get_license_terms(mrt.org, mrt.team, mrt.name)
    ...
    def download_version(
        ...
        """
        ret = super().download_version(target, destination, file_patterns, exclude_patterns, progress_callback_func)
        self._license_check(target, agree_license)
        return ret
