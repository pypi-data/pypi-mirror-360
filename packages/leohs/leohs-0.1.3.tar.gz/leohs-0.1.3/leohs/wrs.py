...
def chunk_list(lst,n):
    ...
    while last<len(lst):
        ...
        last+=avg
    return out
def process_strip_chunk(LS7,LS8,months,years,maxCloudCover,strips_chunk,chunk_index):
    all_matching_pairs=[]
    ee.Initialize()
...
def create_overlap_df(Aoi_shp_path, wrs_shp_path, Save_folder_path,SR_or_TOA,LS7,LS8,
    ...
    t0 = datetime.now()
    logs.append(f"LEOHS start time: {t0.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if strips is None:
        ...
        results=Parallel(n_jobs=num_cores)(
            delayed(process_strip_chunk)(LS7,LS8,months,years,maxCloudCover, chunk, idx) for idx, chunk in enumerate(chunks))
    ...
    if overlap_df.empty:
        ...
        logs.append(f"Overlap dataframe empty")
        return None, None
    ...
    if frequency_gdf.empty:
        print("No matches available")
        logs.append(f"Frequency dataframe empty")
        return None, None
    ...
    overlap_df.to_csv(f'{Save_folder_path}\\Landsatoverlaps.csv')
