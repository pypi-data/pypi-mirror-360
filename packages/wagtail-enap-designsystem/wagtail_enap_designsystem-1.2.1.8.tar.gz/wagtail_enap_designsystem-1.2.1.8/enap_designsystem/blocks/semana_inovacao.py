...
class ConcursoInovacao(Page):
    ...
    parent_page_types = ['enap_designsystem.ConcursoInovacao']
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            try:
                ...
                print(f"Erro ao criar páginas filhas: {e}")

    def create_child_pages(self):
        """Criar as páginas filhas automaticamente"""
        try:
                # Verificar se as páginas filhas já existem
                edital_exists = EditalConcursoInovacao.objects.child_of(self).exists()
                avaliador_exists = SejaAvaliador.objects.child_of(self).exists()
                
                # Criar página Edital se não existir
                if not edital_exists:
                    edital = EditalConcursoInovacao(
                        title=f"{self.title} - Edital",
                        slug=f"{self.slug}-edital",
                    )
                    self.add_child(instance=edital)
                    edital.save_revision().publish()
            ...
            print(f"Erro específico na criação: {e}")
    class Meta:
...
class EditalConcursoInovacao(Page):
    ...
    subpage_types = [
        'enap_designsystem.EditalConcursoInovacao',
...
class SejaAvaliador(Page):
    ...
    class Meta:
        ...
        verbose_name_plural = "Concursos de inovação - Seja Avaliador"