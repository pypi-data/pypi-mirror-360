...
class MarkdownInclude(Extension):
    def __init__(self, configs={}):
        self.config = {
            ...
            "annotated_comments": [
                False,
                "When true, add a comment with the filename and line number before each included line when using lines=.",
            ],
        }
...
class IncludePreprocessor(Preprocessor):
    ...
    def __init__(self, md, config):
        ...
        self.annotated_comments = config.get("annotated_comments", False)
                    ...
                    if m.group(2) is None:
                        ...
                        for block in lines_blocks:
                            if "-" in block:
                                ...
                                # Add a single comment for the range if annotated_comments is True
                                if self.annotated_comments:
                                    annotated_lines.append(f"// {os.path.basename(filename)}:{current_start}-{current_end}")
                                for i in range(current_start, current_end + 1):
                                    annotated_lines.append(original_text[i - 1])
                            else:
                                ...
                                if self.annotated_comments:
                                    annotated_lines.append(f"// {os.path.basename(filename)}:{current_line}")
                                annotated_lines.append(original_text[current_line - 1])
                        text = annotated_lines
