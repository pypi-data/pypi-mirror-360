...
- **HISevent**: Structural Entropy-Based Social Event Detection is an unsupervised tool that explores message correlations without labeling or predetermining event numbers. HISevent combines GNN-based methods' advantages with efficient performance, achieving new state-of-the-art results in closed- and open-set settings.
- **ADPSEMEvent**: Adaptive Differential Privacy Social Event Message Event Detection is an unsupervised framework that prioritizes privacy while detecting social events. It uses a two-stage approach: constructing a private message graph using adaptive differential privacy, then applying a novel 2-dimensional structural entropy minimization algorithm for event detection. This method effectively balances privacy protection with data utility.
- **HyperSED**: Hyperbolic Space-Based Social Event Detection is an unsupervised framework that models social messages using semantic-based anchor graphs in hyperbolic space. It efficiently detects events by leveraging structural and geometric properties, achieving state-of-the-art performance with significant improvements in efficiency and effectiveness.
...
-   **HumAID**: An extensive dataset of 76,484 manually annotated tweets documenting 21 major natural disasters between 2016-2019. Provides broad coverage of various disaster types across different geographical and temporal contexts.
