...
class Base():
  ...
  def geom_opt_universal(self, atoms: ase.Atoms,
    ...
    """
    尽管janus-core只直接支持入门中列出的MLIP计算器，但是任何有效的ASE计算器都可以附加到一个结构，包括当前不受支持的MLIP计算器。然后可以将此结构传递给janus-core计算，该计算可以像往常一样运行。
      给出 带计算器的原子 atoms.calc = xxx 就可以进行计算
      kwargs
      1. file_prefix='xxx/tmp', 保存优化结果, write_traj=True, write_results=True
      2. 优化晶格, filter_func="FrechetCellFilter",  # 默认就是这个可以不设置
      3. 设置优化方向, filter_kwargs={'mask': [1, 1, 0, 0, 0, 0]} : 只优化x,y方向 
      4. 只优化位置: filter_func=None 或 filter_kwargs={"constant_volume": True}
    """
    if file_prefix is not None:
