import pypdf
import numpy as np
import os
import reportlab.lib.pagesizes
import reportlab.pdfgen
import reportlab.pdfgen.canvas
import reportlab
import io


class PypdfLearn():
  def __init__(self) -> None:
    """ * 如果pdf 读取时出错, 可以使用 pdfjam 出错的.pdf -o ok.pdf  生成没有问题的pdf
    * pymupdf 也可以
    - 优点:
    易于使用: PyPDF 的 API 相对简单，适合处理 PDF 文件的基本操作，如合并、拆分、旋转、加密、解密等。
    活跃的社区: PyPDF 是一个流行的库，有大量的资源和社区支持。
    文档和示例丰富: 文档详细且有许多示例，便于新手上手。
    - 缺点:
    功能有限: 虽然 PyPDF 可以处理常见的 PDF 操作，但在一些高级操作（如图像提取、详细页面操作）上不如 PyMuPDF 强大。
    性能稍弱: 在处理大文件或需要更复杂操作时，性能可能不如 PyMuPDF。
    """

  def get_numPages(self, fname_pdf,):
    """获取总页码

    Returns:
        _type_: _description_
    """
    reader = pypdf.PdfReader(stream=fname_pdf)
    numPages = len(reader.pages)
    return numPages

  def page_operation(self, fname_pdf, fname_out):

    with pypdf.PdfWriter() as writer:
      # 读取文件的两种方法
      # 从文件对象读取文件
      reader = pypdf.PdfReader(open(fname_pdf, 'rb'),)
      # 从文件路径读取文件
      reader = pypdf.PdfReader(stream=fname_pdf)
      # 获取页面对象
      page0 = reader.pages[0]
      page3 = reader.pages[3].rotate(90)
      writer = pypdf.PdfWriter()
      writer.add_page(page=page3)
      writer.add_page(page=page0)
      writer.write(fname_out)

      page0.merge_page(page3)  # 两页的内容合并到一页
      writer.add_page(page0)
      writer.write(fname_out)
    pass

  def write_operation(self, fname_pdf, fname_out='t.pdf'):
    with pypdf.PdfMerger() as merger:
      # 写入 1和3 页
      merger.append(fileobj=fname_pdf,
                    pages=[1, 3])
      # 写入 0-3 页
      merger.append(fileobj=fname_pdf,
                    pages=(0, 3))
      # 写入 1-end 页
      numPages = pypdf.PdfReader(stream=fname_pdf).pages.__len__()
      merger.append(fileobj=fname_pdf,
                    pages=(1, numPages))
      # 使用页码范围
      pagerange = pypdf.pagerange.PageRange('-2:')
      merger.append(fileobj=fname_pdf,
                    pages=pagerange)
      # 指定位置写入
      merger.merge(position=1,
                   fileobj=fname_pdf,
                   pages=[1, 2])

      # 写入
      merger.write(fname_out)
    pass

  def get_page_obj(self, fname_pdf, page=0):
    reader = pypdf.PdfReader(fname_pdf)
    page = reader.pages[page]
    return page

  def replace_one_page(self,
                       original_pdf,
                       replaced_1page_pdf,
                       replaced_pageNum=-2,  # 可以为负值
                       fname_out='t.pdf',):
    """取代一页pdf

    Args:
        original_pdf (_type_): _description_
        replaced_1page_pdf (_type_): _description_
        replaced_pageNum (int, optional): _description_. Defaults to -2.
        fname_out (str, optional): _description_. Defaults to 't.pdf'.
    """
    reader = pypdf.PdfReader(original_pdf)
    pageNum = reader.pages.__len__()
    replaced_page = pypdf.PdfReader(replaced_1page_pdf).pages[0]

    writer = pypdf.PdfWriter()
    if replaced_pageNum < 0:
      replaced_pageNum = pageNum + replaced_pageNum

    for idx in range(pageNum):
      if idx != replaced_pageNum:
        writer.add_page(page=reader.pages[idx])
      else:
        writer.add_page(page=replaced_page)

    writer.write(fname_out)

  def get_page_range_tuple(self, fname, pages='1:-2',):
    """以数组的方式选取页码范围

    Args:
        fname (_type_): _description_
        pages (str, optional): _description_. Defaults to '1:-2'.

    Returns:
        _type_: _description_
    """
    reader = pypdf.PdfReader(stream=fname)
    numPages = reader.pages.__len__()
    p1, p2 = pages.split(':')
    if p1 == '':
      p1 = 0
    elif int(p1) < 0:
      p1 = int(p1) + numPages
    if p2 == '':
      p2 = numPages
    elif int(p2) < 0:
      p2 = int(p2) + numPages
    pages = np.array([p1, p2]).astype(int)
    pages = tuple(pages)
    return pages

  # Featurs
  def save(self, writer: pypdf.PdfWriter,
           fname_out='/Users/wangjinlong/Desktop/tmp.pdf'):
    writer.write(fname_out)
    writer.close()
    print(f'保存文件-> {os.path.abspath(fname_out)}')
    return None

  def merge_pdf(self, fname_list, pages_list=None,
                writer=None,
                is_save=False,
                fname_out='/Users/wangjinlong/Desktop/tmp.pdf'):
    """pages_list = [[2,3],(0,-1)] 使用元组表示页面范围, 使用列表表示取其中的哪些页

    写入文件的两种方法:
    with open('merged.pdf', 'wb') as output:  
        pdf_writer.write(output)  # 根据文件对象写入文件
    writer.write('tmp.pdf')  # 根据文件路径写入文件

    Args:
        fname_list (_type_): _description_
        pages_list (_type_, optional): _description_. Defaults to None.
        writer (_type_, optional): _description_. Defaults to None.
        is_save (bool, optional): _description_. Defaults to False.
        fname_out (str, optional): _description_. Defaults to '/Users/wangjinlong/Desktop/tmp.pdf'.

    Returns:
        _type_: _description_
    """

    if pages_list is None:
      pages_list = [None]*len(fname_list)

    if writer is None:
      writer = pypdf.PdfWriter()
    for fname, pages in zip(fname_list, pages_list):
      reader = pypdf.PdfReader(fname)
      if isinstance(pages, tuple):
        start_page, end_page = pages
        # -1 就表示最后一页了
        if end_page < 0:
          end_page = reader.get_num_pages() + end_page + 1
        if start_page < 0:
          start_page = reader.get_num_pages() + start_page + 1
        pages = (start_page, end_page)
      writer.append(fileobj=fname, pages=pages)

    if is_save:
      self.save(writer=writer, fname_out=fname_out)
    else:
      return writer

  def split_pdf(self,
                fname_input='/Users/wangjinlong/Desktop/wang2020molecular.pdf',
                page_range='1:',
                fname_output='x.pdf'):
    """把 pdf 中的首页去掉 首页是图片 故而pdf较大 去掉后可以减小pdf

    Args:
        fname_input (str, optional): _description_. Defaults to '/Users/wangjinlong/Desktop/wang2020molecular.pdf'.
        page_range (str, optional): _description_. Defaults to '1:'.
        fname_output (str, optional): _description_. Defaults to 'x.pdf'.
    """

    reader = pypdf.PdfReader(stream=fname_input)
    writer = pypdf.PdfWriter()
    page_range_tuple = self.get_page_range_tuple(
        fname=fname_input, pages=page_range)
    for page in reader.pages[page_range_tuple[0]:page_range_tuple[1]]:
      writer.add_page(page=page)

    writer.write(stream=fname_output)
    pass

  def get_info(self, fname):
    size = os.path.getsize(filename=fname)
    size_mb = round(size/1024/1024, 2)
    abspath = os.path.abspath(fname)
    data = {'abspath': abspath, 'size': size_mb}
    return data

  def compress_pdf_old(self, directory,
                       input_pdf='xx.pdf',
                       output_pdf=None,
                       level='printer'):
    """level 可以是
    /screen: 适合屏幕显示，低分辨率，文件最小化。
    /ebook: 适合电子书或在线发布，中等分辨率和压缩。
    /printer: 适合打印输出，高分辨率和质量。

    Args:
        input_pdf (_type_): _description_
        output_pdf (_type_): _description_
        level (str, optional): _description_. Defaults to 'ebook'.
    """
    if output_pdf is None:
      output_pdf = 'compressed_'+input_pdf

    from py_package_learn.subprocess_learn import subprocessLearn
    sl = subprocessLearn.SubprocessLearn()
    sl.CLI_popen(directory=directory,
                 args=['gs', '-sDEVICE=pdfwrite', '-dCompatibilityLevel=1.4', f'-dPDFSETTINGS=/{level}',
                       '-dNOPAUSE', '-dQUIET', '-dBATCH', '-sOutputFile=' + output_pdf, input_pdf])

    data1 = self.get_info(fname=os.path.join(directory, input_pdf))
    data2 = self.get_info(fname=os.path.join(directory, output_pdf))
    print(
        f"{data1['abspath']} ({data1['size']} MB) -> {data2['abspath']} ({data2['size']} MB)")
    return None

  def compress_pdf(self, fname='zm.pdf',
                   pdf_page=0,
                   pdf_page_image=0,
                   devide=2,
                   fname_out='zm_small.pdf'):
    """压缩pdf
    1. 首先抽取图片
    2. 压缩图片 
    3. ...
    """
    # 1. 首先抽取图片
    from py_package_learn.pymupdf_learn import pymupdfLearn
    image = pymupdfLearn.PymupdfLearn().extract_image_from_pdf(fname=fname,
                                                               pdf_page=pdf_page,
                                                               pdf_page_image=pdf_page_image)
    # 2. 压缩图片
    from py_package_learn.pillow_learn import pillowLearn
    pf = pillowLearn.PillowLearn()
    image_small = pf.resize_from_image(img=image, devide=devide)

    # 3. 保存pdf
    image_small.save(fname_out)

    # 4. 描述
    # data1 = self.get_info(fname=fname)
    # print(f"{data1['abspath']} ({data1['size']} MB)")

    data2 = self.get_info(fname=fname_out)
    print(f"{data2['abspath']} ({data2['size']} MB)")
    pass

  def add_page_numbers(self, pdf_path,
                       pagenumber_location_x=500,
                       pagenumber_location_y=40,
                       output_path='output.pdf'):
    """为一个没有页码的pdf添加页码
    *示例使用
    - self.add_page_numbers("jtbg_安徽省博士后研究人员科研项目结题报告_final.pdf", "output.pdf")

    Args:
        pdf_path (_type_): _description_
        output_path (_type_): _description_
    """

    # 读取现有的 PDF
    reader = pypdf.PdfReader(pdf_path)
    writer = pypdf.PdfWriter()

    # 遍历每一页，添加页码
    for i in range(len(reader.pages)):
      # 创建一个新的 PDF 页面，尺寸与现有 PDF 页面相同
      packet = io.BytesIO()
      can = reportlab.pdfgen.canvas.Canvas(
          packet, pagesize=reportlab.lib.pagesizes.letter)
      # 页码位置（500, 10）可以根据需要调整
      can.drawString(x=pagenumber_location_x,
                     y=pagenumber_location_y,
                     text=f'- {i+1} -',)
      can.save()

      # 移动到起始位置并读取新的 PDF 内容
      packet.seek(0)
      new_pdf = pypdf.PdfReader(packet)

      # 获取现有的页面并将新的内容合并到页面上
      page = reader.pages[i]
      page.merge_page(new_pdf.pages[0])
      writer.add_page(page)

    # 保存修改后的 PDF
    with open(output_path, 'wb') as output_pdf:
      writer.write(output_pdf)

  def extract_text(self, fname='xx/yy.pdf'):
    """抽取pdf 的文本
    """
    with open(file=fname, mode="rb", ) as file:
      reader = pypdf.PdfReader(file)
      text = "\n".join([page.extract_text() for page in reader.pages])
    return text
