...
class HashBatchSync:
    ...
    def is_active(self) -> bool:
        ...
        """
        return not self.root is None and self.root.is_dir() and os.access(self.root, os.W_OK)
    ...
    def get_batch(
        self, *, prune: bool = True, sleep: float | None = None
    ...
    def _discover_batches(self) -> List[Path]:
        """Return all batch directories on disk (sorted by mtime)."""
        paths: List[Path] = []
        for path in self.root.rglob("*"):

            if len(path.parts[-1]) == 64 and path.is_dir():
                # Heuristic: 64‑char hex == SHA‑256 digest
                paths.append(path)
    ...
    def _save_seen(self) -> None:
        if self._persist_seen:
            tmp = self._seen_path.with_suffix(".tmp")
            tmp.write_text(json.dumps(self._seen, indent=2))
            tmp.rename(self._seen_path)

    # ---------------- House‑keeping helpers ---------------------
    def _prune_old(self) -> None:
        """Delete old batch folders to keep disk usage bounded."""

        if self._max_retained is None:
            return

        all_batches = self._discover_batches()
        excess = len(all_batches) - self._max_retained
        
        if excess < 1:
            return True

        for path in all_batches[: excess]:
            try:
                shutil.rmtree(path, ignore_errors=True)
                self._seen.pop(path.name, None)

                # Remove the shard directory if it became empty
                parent = path.parent
                if parent != self.root:
                    try:
                        next(parent.iterdir())  # raises StopIteration if empty
