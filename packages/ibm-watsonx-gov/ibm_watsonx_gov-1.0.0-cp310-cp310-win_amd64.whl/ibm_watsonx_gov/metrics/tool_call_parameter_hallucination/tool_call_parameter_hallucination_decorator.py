...
from ibm_watsonx_gov.metrics.tool_call_parameter_hallucination.tool_call_parameter_hallucination_metric import \
...
class ToolCallParameterHallucinationDecorator(BaseMetricDecorator):
    def evaluate_tool_call_parameter_hallucination(self,
        ...
        """
        An evaluation decorator for computing tool call parameter hallucination metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_parameter_hallucination, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallParameterHallucinationMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call parameter hallucination metric on {func.__name__},") from ex
