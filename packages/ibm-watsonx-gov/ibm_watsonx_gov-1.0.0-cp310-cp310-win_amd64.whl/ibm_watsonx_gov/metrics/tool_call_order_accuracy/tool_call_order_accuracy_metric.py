...
TOOL_CALLING_ORDER_ACCURACY = "tool_call_order_accuracy"


class ToolCallOrderAccuracyMetric(GenAIMetric):
    """
    Implementation class for ToolCallOrderAccuracyMetric.
    ToolCallOrderAccuracyMetric will check that required 
    upstream steps (auth, data fetch, prior calls) have 
    already done ie., the tool calls are called in the correct order.

    The following methods are supported:
    1. llm_as_judge
    """

    name: Annotated[Literal["tool_call_order_accuracy"], Field(title="Metric Name",
                                                               description="The name of metric.",
                                                               default=TOOL_CALLING_ORDER_ACCURACY)]
    ...
    metric_mapping_name: Annotated[Literal["prerequisite_satisfaction"], Field(title="Metric Mapping Name",
                                                                               description="The mapping name of metric with llmevalkit.",
                                                                               default="prerequisite_satisfaction")]

    def evaluate(self, data: pd.DataFrame | dict,
                 configuration: GenAIConfiguration | AgenticAIConfiguration,
                 **kwargs) -> AggregateMetricResult:
        """
        Evaluate the data for ToolCallOrderAccuracyMetric
        Args:
    ...
    def _compute_metrics(self, data: pd.DataFrame, configuration: GenAIConfiguration | AgenticAIConfiguration, provider: ToolCallMetricProvider):
        """
        Compute the ToolCallOrderAccuracyMetric metrics for the given data
