...
TOOL_CALLING_PARAMETER_ALIGNMENT = "tool_call_parameter_alignment"


class ToolCallParameterAlignmentMetric(GenAIMetric):
    """
    Implementation class for ToolCallParameterAlignmentMetric.
    ToolCallParameterAlignmentMetric confirm the value’s type, 
    format, and units match the spec exactly.

    The following methods are supported:
    1. llm_as_judge
    """

    name: Annotated[Literal["tool_call_parameter_alignment"], Field(title="Metric Name",
                                                                    description="The name of metric.",
                                                                    default=TOOL_CALLING_PARAMETER_ALIGNMENT)]
    ...
    method: Annotated[Literal["semantic_evaluator"], Field(title="Computation Method",
    ...
    metric_mapping_name: Annotated[Literal["value_format_alignment"], Field(title="Metric Mapping Name",
                                                                            description="The mapping name of metric with llmevalkit.",
                                                                            default="value_format_alignment")]

    def evaluate(self, data: pd.DataFrame | dict,
                 configuration: GenAIConfiguration | AgenticAIConfiguration,
                 **kwargs) -> AggregateMetricResult:
        """
        Evaluate the data for ToolCallParameterAlignmentMetric
        Args:
    ...
    def _compute_metrics(self, data: pd.DataFrame, configuration: GenAIConfiguration | AgenticAIConfiguration, provider: ToolCallMetricProvider):
        """
        Compute the ToolCallParameterAlignmentMetric metrics for the given data
