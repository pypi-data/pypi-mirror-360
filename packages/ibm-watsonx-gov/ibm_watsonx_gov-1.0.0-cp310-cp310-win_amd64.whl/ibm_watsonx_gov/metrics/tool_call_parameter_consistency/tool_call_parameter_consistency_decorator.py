...
from ibm_watsonx_gov.metrics.tool_call_parameter_consistency.tool_call_parameter_consistency_metric import \
...
class ToolCallParameterConsistencyDecorator(BaseMetricDecorator):
    def evaluate_tool_call_parameter_consistency(self,
        ...
        """
        An evaluation decorator for computing tool call parameter consistency metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_parameter_consistency, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallParameterConsistencyMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call parameter consistency metric on {func.__name__},") from ex
