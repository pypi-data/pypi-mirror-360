...
from ibm_watsonx_gov.metrics.tool_call_parameter_groundedness.tool_call_parameter_groundedness_metric import \
...
class ToolCallParameterGroundednessDecorator(BaseMetricDecorator):
    def evaluate_tool_call_parameter_groundedness(self,
        ...
        """
        An evaluation decorator for computing tool call parameter groundedness metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_parameter_groundedness, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallParameterGroundednessMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call parameter groundedness metric on {func.__name__},") from ex
