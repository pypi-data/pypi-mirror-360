...
from ibm_watsonx_gov.metrics import ToolCallIntentAccuracyMetric
...
class ToolCallIntentAccuracyDecorator(BaseMetricDecorator):
    def evaluate_tool_call_intent_accuracy(self,
        ...
        """
        An evaluation decorator for computing tool call intent accuracy metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_intent_accuracy, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallIntentAccuracyMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call intent accuracy metric on {func.__name__},") from ex
