...
from ibm_watsonx_gov.metrics.tool_call_order_accuracy.tool_call_order_accuracy_metric import \
...
class ToolCallOrderAccuracyDecorator(BaseMetricDecorator):
    def evaluate_tool_call_order_accuracy(self,
        ...
        """
        An evaluation decorator for computing tool call order accuracy metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_order_accuracy, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallOrderAccuracyMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call order accuracy metric on {func.__name__},") from ex
