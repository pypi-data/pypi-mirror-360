...
TOOL_CALLING_PARAMETER_HALLUCINATION = "tool_call_parameter_hallucination"


class ToolCallParameterHallucinationMetric(GenAIMetric):
    """
    Implementation class for ToolCallParameterHallucinationMetric.

    The following methods are supported:
    1. llm_as_judge
    """

    name: Annotated[Literal["tool_call_parameter_hallucination"], Field(title="Metric Name",
                                                                        description="The name of metric.",
                                                                        default=TOOL_CALLING_PARAMETER_HALLUCINATION)]
    ...
    metric_mapping_name: Annotated[Literal["parameter_hallucination_check"], Field(title="Metric Mapping Name",
                                                                                   description="The mapping name of metric with llmevalkit.",
                                                                                   default="parameter_hallucination_check")]

    def evaluate(self, data: pd.DataFrame | dict,
                 configuration: GenAIConfiguration | AgenticAIConfiguration,
                 **kwargs) -> AggregateMetricResult:
        """
        Evaluate the data for ToolCallParameterHallucinationMetric
        Args:
    ...
    def _compute_metrics(self, data: pd.DataFrame, configuration: GenAIConfiguration | AgenticAIConfiguration, provider: ToolCallMetricProvider):
        """
        Compute the ToolCallParameterHallucinationMetric metrics for the given data
