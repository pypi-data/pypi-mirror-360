...
class ToolSelectionAccuracyMetric(GenAIMetric):
    """
    Implementation class for ToolSelectionAccuracyMetric.
    ...
    def _compute_metrics(self, data: pd.DataFrame, configuration: GenAIConfiguration | AgenticAIConfiguration, provider: ToolCallMetricProvider):
        """
        Compute the ToolSelectionAccuracyMetric metrics for the given data
            ...
            for _, row in data.iterrows():
                ...
                for call in tool_calls:
                    pipeline = ReflectionPipeline(
                        ...
                        function_metrics=[FunctionCallMetric(**prompt_json)],
                    ...
                    explanations = get(
                        result.model_dump(), f"function_selection.metrics.{self.metric_mapping_name}")
