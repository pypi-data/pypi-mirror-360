...
TOOL_CALLING_PARAMETER_GROUNDEDNESS = "tool_call_parameter_groundedness"


class ToolCallParameterGroundednessMetric(GenAIMetric):
    """
    Implementation class for ToolCallParameterGroundednessMetric.
    Ensure every parameter value is directly drawn from user text, 
    assistant messages, previous outputs of api calls, or API spec defaults

    The following methods are supported:
    1. llm_as_judge
    """

    name: Annotated[Literal["tool_call_parameter_groundedness"], Field(title="Metric Name",
                                                                       description="The name of metric.",
                                                                       default=TOOL_CALLING_PARAMETER_GROUNDEDNESS)]
    ...
    metric_mapping_name: Annotated[Literal["parameter_value_grounding"], Field(title="Metric Mapping Name",
                                                                               description="The mapping name of metric with llmevalkit.",
                                                                               default="parameter_value_grounding")]

    def evaluate(self, data: pd.DataFrame | dict,
                 configuration: GenAIConfiguration | AgenticAIConfiguration,
                 **kwargs) -> AggregateMetricResult:
        """
        Evaluate the data for ToolCallParameterGroundednessMetric
        Args:
    ...
    def _compute_metrics(self, data: pd.DataFrame, configuration: GenAIConfiguration | AgenticAIConfiguration, provider: ToolCallMetricProvider):
        """
        Compute the ToolCallParameterGroundednessMetric metrics for the given data
                ...
                for call in tool_calls:
                    pipeline = ReflectionPipeline(
                        metrics_client=provider.get_llm_metric_client(
                            self.llm_judge),
                        general_metrics=[FunctionCallMetric(**prompt_json)],
                    ...
                    explanations = get(
                        result.model_dump(), f"general.metrics.{self.metric_mapping_name}")
