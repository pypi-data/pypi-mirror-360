...
from ibm_watsonx_gov.metrics.tool_call_parameter_alignment.tool_call_parameter_alignment_metric import \
...
class ToolCallParameterAlignmentDecorator(BaseMetricDecorator):
    def evaluate_tool_call_parameter_alignment(self,
        ...
        """
        An evaluation decorator for computing tool call parameter alignment metric on an agentic node.
        """
        if func is None:
            return partial(self.evaluate_tool_call_parameter_alignment, configuration=configuration, metrics=metrics)

        if not metrics:
            metrics = [ToolCallParameterAlignmentMetric()]
        ...
        def wrapper(func, instance, args, kwargs):
            ...
            try:
                ...
                raise Exception(
                    f"There was an error while evaluating tool call parameter alignment metric on {func.__name__},") from ex
