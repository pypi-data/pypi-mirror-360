...
class UVVirtualEnvironmentPlugin(EnvironmentInterface):
    PLUGIN_NAME = "uvenv"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.virtual_env = UVVirtualEnv(self.virtual_env_path, self.platform, self.verbosity)
        self.build_virtual_env = UVVirtualEnv(
            app_virtual_env_path.parent / f"{app_virtual_env_path.name}-build",
            self.platform,
            self.verbosity,
        )
...
"""
        ...
        with self.expose_uv():
            self.virtual_env.create(
                self.parent_python,
                allow_system_packages=self.config.get("system-packages", False),
            )

    def add_group_flags(self, command):
        command.extend([f"--group={group}" for group in self.dependency_groups])
