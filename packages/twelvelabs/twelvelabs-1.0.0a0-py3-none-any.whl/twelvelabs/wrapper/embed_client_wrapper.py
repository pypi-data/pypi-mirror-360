        ...
        *,
        ...
        for video_params in videos:
            try:
                req = {}
                video_file = (
                    getattr(video_params, "video_file", None)
                    if hasattr(video_params, "video_file")
                    else video_params.get("video_file")
                )
                if video_file:
                    req["video_file"] = video_file

                video_url = (
                    getattr(video_params, "video_url", None)
                    if hasattr(video_params, "video_url")
                    else video_params.get("video_url")
                )
                if video_url:
                    req["video_url"] = video_url

                video_start_offset_sec = (
                    getattr(video_params, "video_start_offset_sec", OMIT)
                    if hasattr(video_params, "video_start_offset_sec")
                    else video_params.get("video_start_offset_sec")
                )
                if video_start_offset_sec not in [None, OMIT]:
                    req["video_start_offset_sec"] = video_start_offset_sec

                video_end_offset_sec = (
                    getattr(video_params, "video_end_offset_sec", OMIT)
                    if hasattr(video_params, "video_end_offset_sec")
                    else video_params.get("video_end_offset_sec")
                )
                if video_end_offset_sec not in [None, OMIT]:
                    req["video_end_offset_sec"] = video_end_offset_sec

                video_clip_length = (
                    getattr(video_params, "video_clip_length", OMIT)
                    if hasattr(video_params, "video_clip_length")
                    else video_params.get("video_clip_length")
                )
                if video_clip_length not in [None, OMIT]:
                    req["video_clip_length"] = video_clip_length

                video_embedding_scope = (
                    getattr(video_params, "video_embedding_scope", OMIT)
                    if hasattr(video_params, "video_embedding_scope")
                    else video_params.get("video_embedding_scope")
                )
                if video_embedding_scope:
                    req["video_embedding_scope"] = video_embedding_scope

                task = self.create(
                    model_name=model_name,
                    video_file=typing.cast(
                        typing.Optional[core.File], req.get("video_file")
                    ),
                    video_url=typing.cast(typing.Optional[str], req.get("video_url")),
                    video_start_offset_sec=typing.cast(
                        typing.Optional[float], req.get("video_start_offset_sec", OMIT)
                    ),
                    video_end_offset_sec=typing.cast(
                        typing.Optional[float], req.get("video_end_offset_sec", OMIT)
                    ),
                    video_clip_length=typing.cast(
                        typing.Optional[float], req.get("video_clip_length", OMIT)
                    ),
                    video_embedding_scope=typing.cast(
                        typing.Optional[
                            typing.List[TasksCreateRequestVideoEmbeddingScopeItem]
                        ],
                        req.get("video_embedding_scope", OMIT),
                    ),
