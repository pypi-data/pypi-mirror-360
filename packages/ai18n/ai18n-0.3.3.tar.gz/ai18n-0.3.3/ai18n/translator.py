...
from ai18n.openai_translator import OpenAIMessageTranslator
...
class Translator:
    def __init__(
        ...
        self.yaml = YAML(typ="rt")  # Round-trip for preserving formatting/comments
    ...
    def from_yaml(self, yaml_file: Optional[str] = None) -> None:
        ...
        try:
            with open(yaml_location, "r", encoding="utf-8") as file:
    ...
    def translate(
        ...
        for i, msg in enumerate(messages_to_translate):
            ...
            self.openai_translator.translate_message(
                msg, dry_run=dry_run, force=force, verbose=verbose
            )
            if checkpoint:
                self.to_yaml()
    ...
    def to_yaml(self, yaml_file: Optional[str] = None) -> None:
        ...
        try:
            ...
            export_yaml()
            print(f"Export completed to '{yaml_location}', exiting now...")
            sys.exit(1)
    ...
    def push_po_file(
        ...
        for message in self.messages.values():
            ...
            if not entry or not message.ai_translations:
                continue

            if occurrence_regex and not any(
                re.match(occurrence_regex, occ[0]) for occ in entry.occurrences
            ):
                continue
            ...
            flags: Any = message.flags.get(lang) or []
            if prefer_ai:
                if "ai18n-force" in flags:
            ...
            # Avoid assigning non-str (e.g., CommentedMap) to msgstr
            entry.msgstr = str(translation)
            msg_count += 1
