...
def main(args):
    """
    Convert image(s) to AVIF format.
    Usage:
        faiz avif <input_image> [output_image]
        faiz avif *.png
    """
    if not args:
        print("Usage: faiz avif <input_image_path_or_pattern> [output_image_path]")
        return

    # If only one argument is given and it includes *, treat it as a pattern
    if len(args) == 1 and "*" in args[0]:
        ...
        process_images(image_files)
    # If multiple files are passed (e.g., CMD expands), treat as batch
    elif len(args) > 1 and all(os.path.isfile(a) for a in args):
        print(f"🔄 Processing {len(args)} images in parallel...")
        process_images(args)
    else:
        # Treat as single conversion
        input_path = args[0]
