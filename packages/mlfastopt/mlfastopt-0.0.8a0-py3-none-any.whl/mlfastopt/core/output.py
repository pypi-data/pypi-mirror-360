...
class OutputManager:
    ...
    def _save_qualifying_trials(
        ...
        """Save multiple qualifying trials based on threshold or top-K selection"""
        try:
            ...
            # Save each qualifying trial
            for i, trial in enumerate(trials_to_save):
                trial_file = f"qualifying_trial_{trial['trial']:02d}_{selection_reason}.json"
                trial_path = os.path.join(self.output_dir, trial_file)
                
                # Calculate timing stats for this trial
                trial_data = {
                ...
                # Add to summary
                summary["qualifying_trials"].append({
                    "trial_number": trial['trial'],
                    "recall": self._safe_extract_float(trial['results']['soft_recall']),
                    "f1_score": self._safe_extract_float(trial['results']['soft_f1_score']),
                    "precision": self._safe_extract_float(trial['results']['soft_precision']),
                    "qualifies_for_saving": trial.get('qualifies_for_saving', False),
                    "file": trial_file
                })
            
            # Save summary file
            summary_file = os.path.join(self.output_dir, "qualifying_trials_summary.json")
            ...
            self.logger.info(f"Saved {len(trials_to_save)} qualifying trials to {self.output_dir}")
