...
# ✅ DB and models (only used if API key is provided)
from openmailer.db import get_db
from openmailer.models import ApiKey, EmailLog, SMTPCredential
...
class OpenMailerClient:
    def __init__(self, config=None, dry_run=False):
        ...
        self.api_key = os.getenv("OPENMAILER_API_KEY")  # Optional
    ...
    def send_email(
        ...
        # ✅ Optional API key verification and logging
        if self.api_key:
            try:
                db = next(get_db())
        
                # Step 1: Validate API key
                key_obj = db.query(ApiKey).filter_by(key=self.api_key, is_active=True).first()
                if not key_obj:
                    raise Exception("Invalid or expired API key")
                if key_obj.expires_at and key_obj.expires_at < datetime.utcnow():
                    raise Exception("API key has expired")
                if "send:email" not in (key_obj.scopes or ""):
                    raise Exception("API key lacks permission to send email")
        
                # Step 2: Validate SMTP credential matches API key's metadata
                smtp_cred = db.query(SMTPCredential).filter_by(
                    user_id=key_obj.user_id,
                    team_id=key_obj.team_id,
                    workspace_id=key_obj.workspace_id,
                    smtp_username=self.backends[0]["username"]  # Ensure correct mapping
                ).first()
        
                if not smtp_cred:
                    raise Exception("No valid SMTP credentials match the API key metadata")
        
            except Exception as e:
                raise Exception(f"❌ API key or SMTP credential validation failed: {e}")
        ...
        for backend in self.backends:
            try:
                ...
                # ✅ Log to platform if API key is used
                if self.api_key:
                    db.add(EmailLog(
                        user_id=key_obj.user_id,
                        team_id=key_obj.team_id,
