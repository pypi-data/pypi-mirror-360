...
class Recipient:
    def __init__(self, contact_id: int = None, person_id: int = None, user_id: int = None, profile_id: int = None,
        ...
        # To improve performance, we will get it only when needed.
        self.__variables_local: VariablesLocal | None = None
    ...
    def get_first_name_in_lang_code(self, lang_code: LangCode = None) -> str:
        ...
        name = self.get_first_name() or self.__title_per_lang_code_str.get(lang_code.value)
        return name

    # TODO region or country_id?
    def get_phone_number_full_normalized(self, region: str = "IL") -> str | None:
    ...
    @staticmethod
    def recipients_from_dicts(recipients: list[dict]) -> list['Recipient'] | list | None:
        """get recipients"""
        if not recipients:
    ...
    @staticmethod
    def recipients_to_dicts(recipients: list['Recipient']) -> list[dict] | list | None:
        """recipients to json"""
        if not recipients:
            return recipients
    ...
    def get_user_dict_by_system_id(self, system_id: int) -> dict:
        ...
        if where == f"system_id = {system_id}":
            ...
            user_dict = {"username": list, "user_external_id": list,
