...
def annotate_current_structure(graph, use_coarse_names=True, DATA_DIR=None):
    """
    Annotate nodes based on the current structure
    """
    id_to_roi, id_to_acronym, id_to_parent = clustering.get_roi_map(DATA_DIR)
    for node in graph.nodes():
...
def annotate_color_on_current_structure(graph):
    """
