...
class AutoWaterQualityModeler:
        ...
        try:
            ...
            try:
                result.models = self._format_result(
                    result.models,
                    result.model_type,
                    merged_data if merged_data is not None else filter_metric_data,
                )
            except Exception as e:
    ...
    def _format_result(
        ...
        if not columns:
            columns = [f"STZ{i}" for i in range(1, 26)]
        ...
        for index in Range_coefficients.index:
            if index in merged_data.columns:
                ...
                Range_coefficients.loc[index, "m"] = max(
                    0, min_value - merged_data[index].std()
                )
                Range_coefficients.loc[index, "n"] = (
                    max_value + merged_data[index].std()
                )
