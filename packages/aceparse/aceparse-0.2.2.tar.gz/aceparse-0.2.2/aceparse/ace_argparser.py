...
import sys
import types
from argparse import ArgumentDefaultsHelpFormatter, ArgumentParser, ArgumentTypeError, _ArgumentGroup
...
def make_choice_type_function(choices: list[Any]) -> Callable[[str], Any]:
    """
    ...
    return lambda arg: str_to_choice[arg] if arg in str_to_choice else arg
...
class AceParser(ArgumentParser, Generic[DataClassType]):
    ...
    def _parse_dataclass_field(
        parser: ArgumentParser | _ArgumentGroup, field: dataclasses.Field, *, add_hyphenated_options: bool
    ) -> None:
    ...
    def parse_yaml_file(self, yaml_file: str | PathLike, *, allow_extra_keys: bool = False) -> ReturnType:
        ...
        """
        outputs: ReturnType = self.parse_dict(
            yaml.safe_load(Path(yaml_file).read_text()), allow_extra_keys=allow_extra_keys
        )
        return outputs
