    ...
    def __init__(
        ...
        **kwargs,
        ...
        if platform.lower() not in self.PLATFORM_VALUES:
            raise ValueError(
                "'platform' argument value must be one of: {}".format(
                    " | ".join(self.PLATFORM_VALUES)
                )
            )
        else:
            if platform.lower() == "custom":
                if not isinstance(isogeo_urls, dict):
                    raise TypeError(
                        "'isogeo_urls' argument must be a dict, not {}".format(
                            type(isogeo_urls)
                        )
                    )
                elif isogeo_urls == {}:
                    raise ValueError(
                        "'isogeo_urls' argument value cannot be empty if 'platform' argument value is 'custom'"
                    )
                else:
                    pass
            else:
