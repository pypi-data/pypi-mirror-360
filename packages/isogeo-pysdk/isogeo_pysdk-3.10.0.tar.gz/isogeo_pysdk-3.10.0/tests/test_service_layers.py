...
class TestServiceLayers(unittest.TestCase):
    ...
    def test_layers_create_sophisticated(self):
        ...
        layer_targetDataset = {"name": metadata_no_geo_dataset.name}
        layer_type = "table"
        ...
        self.assertTrue(
            any(
                dtng_md.get("_id") == metadata_no_geo_dataset._id
                for dtng_md in layer_new.noGeoDatasets
            )
        )
    ...
    def test_layers_association_vector_dataset(self):
        ...
        self.assertTrue(
            any(
                dataset_md.get("_id") == metadata_dataset._id
                for dataset_md in li_associated_layers[0].get("datasets")
            )
        )
    ...
    def test_layers_association_no_geo_dataset(self):
        ...
        self.isogeo.metadata.layers.associate_metadata(
            ...
            layer=layer_created,
            dataset=metadata_no_geo_dataset,
        )
        ...
        self.assertTrue(
            any(
                dataset_md.get("_id") == metadata_no_geo_dataset._id
                for dataset_md in li_associated_layers[0].get("noGeoDatasets")
            )
        )
        ServiceLayer(**li_associated_layers[0])
