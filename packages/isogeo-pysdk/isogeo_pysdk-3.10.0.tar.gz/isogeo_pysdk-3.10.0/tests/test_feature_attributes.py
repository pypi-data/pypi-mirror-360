...
class TestFeatureAttributes(unittest.TestCase):
    ...
    def test_featureAttributes_import_create_listing_source_featureAttributes(self):
        ...
        vector_metadata_fixture_existing_attributes = (
            self.isogeo.metadata.attributes.listing(
                self.vector_metadata_fixture_existing
            )
        )
        ...
        vector_metadata_fixture_created_attributes = (
            self.isogeo.metadata.attributes.listing(
        ...
        for fAttr_created in vector_metadata_fixture_created_attributes:
            for key in fAttr_created:
                if key != "_id":
                    self.assertTrue(
                        any(
                            fAttr_existing.get(key) == fAttr_created.get(key)
                            for fAttr_existing in vector_metadata_fixture_existing_attributes
                        )
                    )
                else:
                    pass
        dtng_metadata_fixture_existing_attributes = (
            self.isogeo.metadata.attributes.listing(self.dtng_metadata_fixture_existing)
        )
        self.isogeo.metadata.attributes.import_from_dataset(
        ...
        dtng_metadata_fixture_created_attributes = (
            self.isogeo.metadata.attributes.listing(self.dtng_metadata_fixture_created)
        )
            ...
            for key in fAttr_created:
                if key != "_id":
                    self.assertTrue(
                        any(
                            fAttr_existing.get(key) == fAttr_created.get(key)
                            for fAttr_existing in dtng_metadata_fixture_existing_attributes
                        )
                    )
                else:
                    pass
    ...
    def test_featureAttributes_import_create_using_source_featureAttributes(self):
        ...
        self.vector_metadata_fixture_existing.featureAttributes = (
            vector_metadata_fixture_existing_attributes
        )
        self.isogeo.metadata.attributes.import_from_dataset(
            metadata_source=self.vector_metadata_fixture_existing,
        ...
        self.dtng_metadata_fixture_existing.featureAttributes = (
            dtng_metadata_fixture_existing_attributes
        )
        self.isogeo.metadata.attributes.import_from_dataset(
            metadata_source=self.dtng_metadata_fixture_existing,
