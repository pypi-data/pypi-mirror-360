...
class TestMetadatasVector(unittest.TestCase):
    ...
    def test_metadatas_get_detailed_multilingual(self):
        ...
        self.assertTrue(
            all(
                all(key in trans for key in ["languageCode", "title", "abstract"])
                and all(
    ...
    def test_metadatas_create_basic(self):
        ...
        with self.assertRaises(ValueError):
            self.isogeo.metadata.create(
                workgroup_id=workgroup_id, metadata=metadata_toCreate
            )
    ...
    def test_metadatas_create_complete(self):
        ...
        self.assertEqual(
            created_metadata.groupName,
            created_metadata._creator.get("contact").get("name"),
        )
    ...
    def test_metadatas_create_basic_multilingual(self):
        ...
        with self.assertRaises(ValueError):
            self.isogeo.metadata.create(
                workgroup_id=workgroup_id, metadata=metadata_toCreate, lang=lang_code
            )
        ...
        created_metadata_with_translations = self.isogeo.metadata.get(
            metadata_id=created_metadata._id, lang=lang_code, include=("translations",)
        )
        ...
        self.assertTrue(
            any(
                trans.get("languageCode") == lang_code
                and trans.get("title") == metadata_title
                and trans.get("abstract") == metadata_abstract
    ...
    def test_metadatas_update_multilingual_put(self):
        """PUT :resources/{metadata_uuid}"""
        # retrieve fixture metadata
        ...
        # update metadata and check return
        self.assertIsInstance(
            self.isogeo.metadata.update(
                metadata=metadata_toUpdate, _http_method="PUT", lang="fr"
            ),
            Metadata,
        )
        # get the metadata updated
        ...
        french_translations = [
            trans
            for trans in updated_metadata.translations
            if trans.get("languageCode") == "fr"
        ]
        self.assertEqual(len(french_translations), 1)
        french_translation = french_translations[0]
        ...
        self.assertIsInstance(
            self.isogeo.metadata.update(
                metadata=metadata_toUpdate, _http_method="PUT", lang="es"
            ),
            Metadata,
        )
        # get the metadata updated
        ...
        self.assertIsInstance(
            self.isogeo.metadata.update(
                metadata=metadata_toUpdate, _http_method="PUT", lang="en"
            ),
            Metadata,
        )
        # get the metadata updated
    ...
    def test_metadatas_update_multilingual_patch(self):
        """PUT :resources/{metadata_uuid}"""
        # retrieve fixture metadata
        metadata_toUpdate = self.isogeo.metadata.get(
            self.fixture_metadata_ml._id, ("translations",), "fr"
        ...
        # update metadata and check return
        self.assertIsInstance(
            self.isogeo.metadata.update(metadata=metadata_toUpdate, lang="fr"), Metadata
        )
        # get the metadata updated
        updated_metadata = self.isogeo.metadata.get(
            metadata_toUpdate._id, include=("translations",), lang="fr"
        ...
        # update metadata and check return
        self.assertIsInstance(
            self.isogeo.metadata.update(metadata=metadata_toUpdate, lang="es"), Metadata
        )
        # get the metadata updated
        updated_metadata = self.isogeo.metadata.get(
            metadata_toUpdate._id, include=("translations",), lang="es"
        ...
        # update metadata and check return
        self.assertIsInstance(
            self.isogeo.metadata.update(metadata=metadata_toUpdate, lang="en"), Metadata
        )
        # get the metadata updated
        updated_metadata = self.isogeo.metadata.get(
            metadata_toUpdate._id, include=("translations",), lang="en"
