...
GROUP_THEME_TEST_FIXTURE_TRANSLATED_UUID = environ.get(
    "ISOGEO_FIXTURE_GROUP_THEME_TRANSLATED"
)
...
class TestKeywordsComplete(unittest.TestCase):
    ...
    def fetch_specific_thesaurus_kw(
        self, keywords: list, thesaurus_code: str = "isogeo"
    ):
    ...
    def make_dict_hashable(self, dictionary: dict, excluded_keys: list = []):
        ...
        filtered_dict = dict(
            (k, dictionary.get(k)) for k in dictionary if k not in excluded_keys
        )
    ...
    def tearDownClass(cls):
        ...
        if len(cls.li_fixtures_to_delete):
            for kw, th_id in cls.li_fixtures_to_delete:
                cls.isogeo.keyword.delete(keyword=kw, thesaurus_id=th_id)
        # close sessions
        cls.isogeo.close()
    ...
    def test_keywords_create_basic_unchecking_exists(self):
        ...
        for thesaurus_id in [ISOGEO_THESAURUS_ID, GROUPTHEME_THESAURUS_ID]:
            ...
            keyword_new = self.isogeo.keyword.create(
                keyword=keyword_new, thesaurus_id=thesaurus_id, check_exists=0
            )
            ...
            self.li_fixtures_to_delete.append((keyword_new, thesaurus_id))

        keyword_new = Keyword(text=keyword_text)
        with self.assertRaises(ValueError):
            keyword_new = self.isogeo.keyword.create(
                keyword=keyword_new, thesaurus_id=INSPIRE_THESAURUS_ID, check_exists=0
            )
    ...
    def test_keywords_create_basic_checking_exists(self):
        ...
        for thesaurus_id in [ISOGEO_THESAURUS_ID, GROUPTHEME_THESAURUS_ID]:
            ...
            # create it online
            keyword_new = self.isogeo.keyword.create(
                keyword=keyword_new, thesaurus_id=thesaurus_id, check_exists=1
            )
        ...
        with self.assertRaises(ValueError):
            keyword_new = self.isogeo.keyword.create(
                keyword=keyword_new, thesaurus_id=INSPIRE_THESAURUS_ID, check_exists=1
            )
        if keyword_new._id is not None:
            self.li_fixtures_to_delete.append((keyword_new, thesaurus_id))
    ...
    def test_keywords_create_similar_unchecking_exists(self):
        ...
        for thesaurus_id in [ISOGEO_THESAURUS_ID, GROUPTHEME_THESAURUS_ID]:
            ...
            # add created keyword to deletion
            self.li_fixtures_to_delete.append((keyword_new_1_created, thesaurus_id))
    ...
    def test_keywords_search_workgroup_whole_results_multilingual(self):
        ...
        wg_keywords_searches = [
            self.isogeo.keyword.workgroup(
                workgroup_id=WORKGROUP_TEST_FIXTURE_UUID_ML, whole_results=1, lang="fr"
            ),
            self.isogeo.keyword.workgroup(
                workgroup_id=WORKGROUP_TEST_FIXTURE_UUID_ML, whole_results=1, lang="en"
            ),
            self.isogeo.keyword.workgroup(
                workgroup_id=WORKGROUP_TEST_FIXTURE_UUID_ML, whole_results=1, lang="es"
            ),
        ]
    ...
    def test_keywords_search_workgroup_groupTheme_whole_results_multilingual(self):
        ...
        self.assertNotEqual(
            Counter(wg_groupThemes_en_raw), Counter(wg_groupThemes_fr_raw)
        )
        self.assertNotEqual(
            Counter(wg_groupThemes_es_raw), Counter(wg_groupThemes_fr_raw)
        )

        self.assertEqual(
            Counter(wg_groupThemes_en_raw_noText), Counter(wg_groupThemes_fr_raw_noText)
        )
        self.assertEqual(
            Counter(wg_groupThemes_es_raw_noText), Counter(wg_groupThemes_fr_raw_noText)
        )
    ...
    def test_keywords_search_thesaurus_whole_results_multilingual(self):
        ...
        # parse results to make theme easier to compare
        groupThemes_fr_raw = [self.make_dict_hashable(kw) for kw in groupThemes_fr]
        groupThemes_en_raw = [self.make_dict_hashable(kw) for kw in groupThemes_en]
        groupThemes_es_raw = [self.make_dict_hashable(kw) for kw in groupThemes_es]
        groupThemes_pt_raw = [self.make_dict_hashable(kw) for kw in groupThemes_pt]
        ...
        self.assertEqual(
            Counter(groupThemes_en_raw_noText), Counter(groupThemes_fr_raw_noText)
        )
        self.assertEqual(
            Counter(groupThemes_es_raw_noText), Counter(groupThemes_fr_raw_noText)
        )
        self.assertEqual(
            Counter(groupThemes_pt_raw_noText), Counter(groupThemes_fr_raw_noText)
        )
    ...
    def test_keyword_get_from_text_groupTheme_multilingual(self):
        ...
        groupTheme_fr = Keyword(
            **self.isogeo.keyword.get_from_text(
                text="PitouFR", thesaurus_id=GROUPTHEME_THESAURUS_ID, lang="fr"
            )[0]
        )
        groupTheme_en = Keyword(
            **self.isogeo.keyword.get_from_text(
                text="PitouEN", thesaurus_id=GROUPTHEME_THESAURUS_ID, lang="en"
            )[0]
        )
        groupTheme_es = Keyword(
            **self.isogeo.keyword.get_from_text(
                text="PitouES", thesaurus_id=GROUPTHEME_THESAURUS_ID, lang="es"
            )[0]
        )
        groupTheme_pt = Keyword(
            **self.isogeo.keyword.get_from_text(
                text="PitouPT", thesaurus_id=GROUPTHEME_THESAURUS_ID, lang="pt"
            )[0]
        )
        # parse and test object loader
        for keyword in [groupTheme_fr, groupTheme_en, groupTheme_es, groupTheme_pt]:
    ...
    def test_keyword_detailed(self):
        ...
        for thesaurus_id in [ISOGEO_THESAURUS_ID, GROUPTHEME_THESAURUS_ID]:
            thesaurus_keywords = self.isogeo.keyword.thesaurus(
                ...
                page_size=50,
                include=("_abilities", "count", "thesaurus"),
            )
            # pick one randomly
            ...
            self.assertEqual(
                random_keyword_dict.get("_abilities"), random_keyword._abilities
            )
            ...
            self.assertEqual(
                random_keyword_dict.get("description"), random_keyword.description
            )
            self.assertEqual(random_keyword_dict.get("text"), random_keyword.text)
            self.assertEqual(
                random_keyword_dict.get("thesaurus"), random_keyword.thesaurus
            )
    ...
    def test_associate_and_dissociate_workgroup_groupTheme(self):
        ...
        groupTheme_workgroup_keywords = self.isogeo.keyword.workgroup(
            workgroup_id=workgroup_test_fixture._id,
            thesaurus_id=GROUPTHEME_THESAURUS_ID,
            whole_results=True,
        )
        if groupTheme_workgroup_keywords.results and len(
            groupTheme_workgroup_keywords.results
        ):
            groupTheme_workgroup_tags = [
                kw.get("_tag") for kw in groupTheme_workgroup_keywords.results
            ]
            random_keyword_dict = sample(
                [
                    kw_dict
                    for kw_dict in groupTheme_thesaurus_keywords.results
                    if kw_dict.get("_tag") not in groupTheme_workgroup_tags
                ],
                1,
            )[0]
        else:
        ...
        self.assertTrue(
            any(
                kw_dict.get("_tag") == random_keyword._tag
                for kw_dict in groupTheme_workgroup_keywords.results
            )
        )
        self.assertTrue(
            any(
                kw_dict.get("text") == random_keyword.text
                for kw_dict in groupTheme_workgroup_keywords.results
            )
        )
        self.assertTrue(
            any(
                kw_dict.get("_id") == random_keyword._id
                for kw_dict in groupTheme_workgroup_keywords.results
            )
        )
        ...
        self.assertEqual(
            self.isogeo.keyword.associate_workgroup(
                workgroup=workgroup_test_fixture,
    ...
    def test_associate_and_dissociate_workgroup_isogeo(self):
        ...
        isogeo_workgroup_keywords = self.isogeo.keyword.workgroup(
            workgroup_id=workgroup_test_fixture._id,
            thesaurus_id=ISOGEO_THESAURUS_ID,
            whole_results=True,
        )
        if isogeo_workgroup_keywords.results and len(isogeo_workgroup_keywords.results):
            isogeo_workgroup_tags = [
                kw.get("_tag") for kw in isogeo_workgroup_keywords.results
            ]
            random_keyword_dict = sample(
                [
                    kw_dict
                    for kw_dict in isogeo_thesaurus_keywords.results
                    if kw_dict.get("_tag") not in isogeo_workgroup_tags
                ],
                1,
            )[0]
        else:
        ...
        self.assertTrue(
            any(
                kw_dict.get("_tag") == random_keyword._tag
                for kw_dict in isogeo_workgroup_keywords.results
            )
        )
        self.assertTrue(
            any(
