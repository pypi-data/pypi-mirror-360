...
class TestWorkgroups(unittest.TestCase):
    ...
    def test_workgroups_create_checking_multilingualism(self):
        ...
        workgroup = Workgroup(
            contact=contact_owner, isMultilingual=True, metadataLanguage="fr"
        )
        with self.assertRaises(ValueError):
            self.isogeo.workgroup.create(workgroup=workgroup, check_multilingualism=1)
    ...
    def test_workgroup_detailed_multilingual(self):
        ...
        multilingual_workgroup = self.isogeo.workgroup.get(
            WORKGROUP_TEST_FIXTURE_UUID_ML
        )
    ...
    def test_workgroups_update_multilingual(self):
        ...
        with self.assertRaises(ValueError):
            self.isogeo.workgroup.update(
                workgroup=multilingual_workgroup, check_multilingualism=1
            )
        ...
        multilingual_workgroup_updated = self.isogeo.workgroup.get(
            multilingual_workgroup._id
        )

        self.assertEqual(
            multilingual_workgroup_updated.metadataLanguage, new_metadataLanguage
        )
        ...
        multilingual_workgroup_reset = self.isogeo.workgroup.get(
            multilingual_workgroup._id
        )

        self.assertEqual(
            multilingual_workgroup_reset.metadataLanguage, initial_metadataLanguage
        )
    ...
    def test_workgroups_update_checking_multilingualism(self):
        ...
        with self.assertRaises(ValueError):
            self.isogeo.workgroup.update(
                workgroup=workgroup_fixture, check_multilingualism=1
            )
        # second test, specifying no metadataLanguage but availableLanguages
