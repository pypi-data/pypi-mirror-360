...
class ApiWorkgroup:
    ...
    def check_multilingualism_attr(self, workgroup: Workgroup):
        ...
        if workgroup.isMultilingual:
            if not workgroup.availableLanguages:
                ...
                availableLanguages_codes = [
                    lang.get("code") for lang in workgroup.availableLanguages
                ]
    ...
    def create(
        ...
        self.api_client._workgroups_names[new_workgroup.contact.get("name")] = (
            new_workgroup._id
        )
    ...
    @ApiDecorators._check_bearer_validity
    def update(
        self, workgroup: Workgroup, caching: bool = 1, check_multilingualism: int = 0
    ) -> Workgroup:
        ...
        if caching:
            self.api_client._workgroups_names[workgroup_updated.contact.get("name")] = (
