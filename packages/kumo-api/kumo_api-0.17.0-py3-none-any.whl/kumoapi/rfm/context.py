...
class Context:
    ...
    def serialize(self) -> bytes:
        ...
        for table_name, table in self.subgraph.tables.items():
            table_msg = msg.subgraph.tables[table_name]
            ...
            for column_name, stype in table.stypes.items():
                table_msg.stypes[column_name] = getattr(
                    context_pb2.Stype, stype.upper())
            if table.pkey_name is not None:
                table_msg.pkey_name = table.pkey_name
            if table.time_col_name is not None:
                table_msg.time_col_name = table.time_col_name

        for edge_type, link in self.subgraph.links.items():
            link_msg = msg.subgraph.links[EDGE_TYPE_SEP.join(edge_type)]
    ...
    def from_bytes(cls, data: bytes) -> Self:
        ...
        for table_name, table_msg in msg.subgraph.tables.items():
            tables[table_name] = Table(
                ...
                stypes={
                    col_name: Stype(context_pb2.Stype.Name(stype_msg).lower())
                    for col_name, stype_msg in table_msg.stypes.items()
                },
                pkey_name=table_msg.pkey_name or None,
                time_col_name=table_msg.time_col_name or None,
            )

        links: dict[tuple[str, str, str], Link] = {}
        for edge_type, link_msg in msg.subgraph.links.items():
            links[tuple(edge_type.split(EDGE_TYPE_SEP))] = Link(
