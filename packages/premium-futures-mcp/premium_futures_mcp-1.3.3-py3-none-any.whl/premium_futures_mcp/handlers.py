...
class ToolHandler:
    ...
    def _initialize_market_intelligence(self):
        """Initialize MarketIntelligence service"""
        try:
            # Initialize with no Redis client initially (will be set up async)
            ...
            self.logger.info("MarketIntelligence service initialized (Redis pending)")
        except Exception as e:
            self.logger.error(f"Failed to initialize MarketIntelligence: {e}")
            self.market_intelligence = None
    
    async def _setup_redis_connection(self):
        """Setup Redis connection with timeout - called once per service lifetime"""
        try:
            if not self.market_intelligence:
                self.logger.error("❌ MarketIntelligence service not initialized")
                return False
                
            # Skip if already setup
            if self.market_intelligence.redis:
                self.logger.info("✅ Redis already connected for MarketIntelligence")
                return True
            
            # Try different Redis configurations with timeout
            redis_urls = [
                "redis://:ilikeMyself1100@localhost:6379",
                "redis://:ilikeMyself1100@premium-redis:6379", 
                "redis://localhost:6379"
            ]
            
            for redis_url in redis_urls:
                try:
                    self.logger.info(f"🔗 Attempting Redis connection: {redis_url}")
                    ...
                    # Set the Redis client for the intelligence service
                    self.market_intelligence.redis = redis_client
                    self._redis_setup_pending = False
                    self.logger.info(f"✅ Redis connected for MarketIntelligence: {redis_url}")
                    
                    # Test getting cached data to ensure it works
                    try:
                        test_data = await redis_client.get("market_opportunities")
                        if test_data:
                            self.logger.info("✅ Redis cache test successful - data available")
                        else:
                            self.logger.warning("⚠️  Redis connected but no cache data found")
                    except Exception as e:
                        self.logger.warning(f"⚠️  Redis connected but cache test failed: {e}")
                    
                    return True
                    
                except asyncio.TimeoutError:
                    self.logger.debug(f"⏰ Timeout connecting to {redis_url}")
                    continue
                except Exception as e:
                    self.logger.debug(f"❌ Failed to connect to {redis_url}: {e}")
                    continue
            
            self.logger.warning("❌ No Redis connection successful - will use fallback data")
            return False
            
        except Exception as e:
            self.logger.error(f"❌ Error setting up Redis connection: {e}")
            return False
    ...
    async def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with BinanceClient(self.config) as client:
            ...
            # Market Intelligence Tools - Initialize service if needed
                ...
                # Initialize market intelligence service once with lazy Redis connection
                if not self.market_intelligence:
                    self.market_intelligence = MarketIntelligenceService()
                
                # Setup Redis connection with timeout if not already done
                if not self.market_intelligence.redis:
                    redis_success = await self._setup_redis_connection()
                    if not redis_success:
                        self.logger.warning("⚠️  Redis connection failed - intelligent tools will use fallback data")
    ...
    async def _handle_intelligent_tools(self, name: str, arguments: Dict[str, Any], client: BinanceClient) -> Dict[str, Any]:
        ...
        # Check if MarketIntelligence service is available
        if not self.market_intelligence:
            self.logger.error("MarketIntelligence service not available")
            return {
                "error": "MarketIntelligence service not available",
                "suggestion": "Please ensure Redis is running and market monitor is active",
        ...
        try:
            if name == "get_market_opportunities":
                ...
                redis_connected = self.market_intelligence.redis is not None
                self.logger.info(f"🔍 DEBUG: Redis connection status: {redis_connected}")
                
                # Get cached opportunities - NO nested timeout, let market_intelligence handle it
                try:
                    self.logger.info("🔍 DEBUG: Calling get_cached_opportunities()...")
                    opportunities = await self.market_intelligence.get_cached_opportunities()
                    self.logger.info(f"🔍 DEBUG: Got {len(opportunities)} opportunities from Redis")
                    
                    if opportunities:
                        first_symbol = opportunities[0].get('symbol', 'Unknown')
                        self.logger.info(f"🔍 DEBUG: First opportunity symbol: {first_symbol}")
                    
                except Exception as e:
                    self.logger.error(f"❌ FAILED to get cached opportunities: {e}")
                    ...
                    self.logger.warning("🔄 Using fallback data due to Redis failure")
                
                # Ensure we have some data
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("🔄 Using fallback data - empty opportunities list")
                ...
                try:
                    from .fallback_data import get_fallback_opportunities
                    fallback_data = get_fallback_opportunities()
                    if len(opportunities) == len(fallback_data):
                        # Double-check by comparing first symbol
                        if opportunities and fallback_data:
                            if opportunities[0].get('symbol') == fallback_data[0].get('symbol'):
                                is_fallback = True
                                self.logger.warning("⚠️ DETECTED: Using fallback data instead of Redis!")
                            else:
                                self.logger.info("✅ CONFIRMED: Using real Redis data")
                except Exception:
                    pass
                ...
                result = {
                    ...
                    "source": "fallback_data" if is_fallback else "redis_cached_data",
                    "processing_mode": "fallback_mode" if is_fallback else "vps_direct_redis",
                    "cache_status": "fallback" if is_fallback else "hit",
                    "note": "Using sample data for demonstration - Redis not available" if is_fallback else None
                }
                ...
                # Get cached opportunities - NO nested timeout
                try:
                    opportunities = await self.market_intelligence.get_cached_opportunities()
                except Exception as e:
                    self.logger.warning(f"Failed to get token analysis data: {e}")
                    opportunities = []
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.info("Using fallback data for token analysis")
                ...
                token_analysis["source"] = "redis_cached_data"
                token_analysis["processing_mode"] = "cache_only_no_api_calls"
                ...
                # Add source information
                dashboard["source"] = "redis_cached_data"
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to get funding extremes data: {e}")
                    # Use fallback data when Redis fails
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.info("Using fallback data for funding extremes")
