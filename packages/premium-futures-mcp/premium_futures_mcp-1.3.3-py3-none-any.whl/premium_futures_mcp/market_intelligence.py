...
class MarketIntelligenceService:
    ...
    async def _calculate_oi_score(self, symbol: str, market_data: Dict) -> int:
        ...
        try:
            ...
            # Cache current OI for future comparison (15 minutes cache)
            await self._cache_oi(symbol, current_oi)
            
            # Try to get cached OI from previous scan for additional comparison
            cached_oi = await self._get_cached_oi(symbol)
            ...
            # 2. Additional: Compare with cached OI from previous scan (if available)
            cached_change_pct = 0
            if cached_oi and cached_oi > 0:
                cached_change_pct = ((current_oi - cached_oi) / cached_oi) * 100
            
            # 3. Short-term trend (last 30min average vs previous 30min average)
            ...
            # 4. Volatility of OI (how much OI is fluctuating)
            ...
            # Bonus scoring for cached comparison (if significant change detected)
            if abs(cached_change_pct) > 3:
                score += 1  # Bonus for recent significant change
            ...
            self.logger.debug(f"OI Score for {symbol}: {score}/10 (1h: {hour_change_pct:.2f}%, "
                            f"cached: {cached_change_pct:.2f}%, trend: {trend_change_pct:.2f}%, "
                            f"vol: {oi_volatility:.2f}%)")
    ...
    async def _cache_market_opportunities(self, opportunities: List[Dict]):
        ...
        try:
            ...
            # Cache for 8 minutes (longer than 3min scan interval + scan duration)
            # This ensures cache doesn't expire during normal operations
            await self.redis.setex("market_opportunities", 785, json.dumps(cache_data))
            
            # Also cache summary statistics
            summary = self._create_market_summary(opportunities)
            await self.redis.setex("market_summary", 785, json.dumps(summary))
    ...
    async def _get_cached_oi(self, symbol: str) -> Optional[float]:
        """Get cached OI value for cross-scan comparison (15 min cache)"""
        if not self.redis:
            return None
        try:
    ...
    async def _cache_oi(self, symbol: str, oi_value: float):
        """Cache OI value for future comparison - REDUCED to 15 minutes for crypto volatility"""
        if self.redis:
            try:
                # Cache for 15 minutes only (900 seconds) - crypto OI can change rapidly
                # This reduces API calls while ensuring fresh data during volatile periods
                await self.redis.setex(f"oi:{symbol}", 785, str(oi_value))
            except:
                pass
