...
class MarketIntelligenceService:
    ...
    async def _cache_market_opportunities(self, opportunities: List[Dict]):
        ...
        try:
            ...
            await self.redis.setex("market_opportunities", 780, json.dumps(cache_data))
            
            # Also cache summary statistics
            summary = self._create_market_summary(opportunities)
            await self.redis.setex("market_summary", 780, json.dumps(summary))
    ...
    async def _cache_oi(self, symbol: str, oi_value: float):
        ...
        if self.redis:
            try:
                # Cache for 3 hours
                await self.redis.setex(f"oi:{symbol}", 1000, str(oi_value))
            except:
                pass
