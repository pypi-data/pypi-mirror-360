...
class ToolHandler:
    ...
    def __init__(self, config: BinanceConfig, ticker_cache: TickerCache):
        ...
        # Note: MarketIntelligence is no longer used as we read from JSON files directly
        # Initialize with basic setup
        self.logger.info("📁 ToolHandler initialized - using JSON file data source")
        
        # Suppress excessive logging for better performance
        self.logger.setLevel(logging.WARNING)
    
    def _initialize_market_intelligence(self):
        """Initialize MarketIntelligence service - DEPRECATED: Now using JSON files"""
        self.logger.info("📁 MarketIntelligence initialization skipped - using JSON file data source")
        pass
    
    async def _setup_redis_connection(self):
        """Setup Redis connection - DEPRECATED: Now using JSON files"""
        self.logger.info("📁 Redis connection setup skipped - using JSON file data source")
        pass
    
    async def _setup_redis_for_intelligence(self):
        """Setup Redis client for MarketIntelligence service - DEPRECATED: Now using JSON files"""
        self.logger.info("📁 Redis setup for intelligence skipped - using JSON file data source")
        pass
    
    def _read_json_data(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self.logger.info(f"✅ Successfully loaded {filename} ({len(json.dumps(data))} chars)")
                return data
    ...
    def _get_opportunities_from_json(self) -> list:
        """Get opportunities data from JSON file"""
        try:
            data = self._read_json_data('market_opportunities.json')
            
            if not data:
                return []
                
            # Extract the opportunities array from the data structure
            if 'data' in data and 'opportunities' in data['data']:
                opportunities = data['data']['opportunities']
                # Filter out entries that only have timestamp (incomplete data)
                complete_opportunities = [
                    opp for opp in opportunities 
                    if isinstance(opp, dict) and 'symbol' in opp and 'total_score' in opp
                ]
                self.logger.info(f"📊 Loaded {len(complete_opportunities)} complete opportunities from JSON")
                return complete_opportunities
            else:
                self.logger.warning("⚠️ JSON structure not as expected - missing data.opportunities")
                return []
                
        except Exception as e:
            self.logger.error(f"❌ Error processing opportunities JSON: {e}")
            return []
    
    def _get_market_summary_from_json(self) -> Dict[str, Any]:
        """Get market summary data from JSON file"""
        try:
            data = self._read_json_data('market_summary.json')
            
            if not data:
                return {}
                
            self.logger.info("📈 Loaded market summary from JSON")
            return data
            
        except Exception as e:
            self.logger.error(f"❌ Error processing market summary JSON: {e}")
            return {}
    ...
    async def _handle_intelligent_tools(self, name: str, arguments: Dict[str, Any], client: BinanceClient) -> Dict[str, Any]:
        """Handle intelligent market analysis tools - now using JSON data instead of Redis"""
        ...
        try:
            if name == "get_market_opportunities":
                ...
                # Get opportunities from JSON file instead of Redis
                self.logger.info("� Loading opportunities from JSON file...")
                opportunities = self._get_opportunities_from_json()
                
                # Fallback to dummy data if JSON is empty
                if not opportunities:
                    self.logger.warning("⚠️ No opportunities found in JSON, using fallback data")
                ...
                # Check data source
                data_source = "json_file_data"
                if not self._read_json_data('market_opportunities.json'):
                ...
                result = {
                    ...
                    "source": data_source,
                    "processing_mode": "json_file_based",
                    "cache_status": "json_file_read",
                    "note": "Data loaded from local JSON file updated by background bot" if data_source == "json_file_data" else "Using sample data - JSON file not available"
                }
                ...
                # Get data from JSON files instead of Redis
                self.logger.info("📊 Loading market dashboard from JSON files...")
                opportunities = self._get_opportunities_from_json()
                market_summary = self._get_market_summary_from_json()
                ...
                dashboard["source"] = "json_file_data"
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading funding extremes from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for funding extremes")
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading volume leaders from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for volume leaders")
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading volatility squeeze data from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for volatility squeeze")
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading whale activity data from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for whale activity")
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading narrative plays from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for narrative plays")
                ...
                # Get opportunities from JSON file
                self.logger.info("📁 Loading quick scan from JSON file...")
                opportunities = self._get_opportunities_from_json()
                ...
                if not opportunities:
                    from .fallback_data import get_fallback_opportunities
                    opportunities = get_fallback_opportunities()
                    self.logger.warning("⚠️ Using fallback data for quick scan")
