...
class ToolHandler:
    ...
    def _initialize_market_intelligence(self):
        ...
        try:
            ...
            # Suppress logging to avoid parsing issues
    ...
    async def initialize_services(self):
        """Initialize async services like Redis connection at startup."""
        self.logger.info("Initializing services and attempting Redis connection at startup...")
    ...
    async def _setup_redis_connection(self):
        ...
        try:
            ...
            # Try different Redis configurations with timeout, prioritizing local passwordless
            redis_urls = [
            ...
            for redis_url in redis_urls:
                try:
                    # Create connection with timeout (increased from 2s to 30s)
                    redis_client = await asyncio.wait_for(
                        redis.from_url(redis_url, decode_responses=True), 
                        timeout=30.0
                    )
                    
                    # Test connection with timeout (increased from 2s to 30s)
            ...
            self.logger.warning("Redis not available - will use fallback data")
    ...
    async def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with BinanceClient(self.config) as client:
            ...
            # Market Intelligence Tools - Initialize service if needed
                ...
                # Setup Redis connection with timeout if not already done (lazy loading)
                if not self.market_intelligence.redis:
                    self.logger.warning("Redis connection not found or failed at startup. Attempting lazy connect...")
                    await self._setup_redis_connection()
