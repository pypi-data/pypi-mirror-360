...
class MarketIntelligenceService:
    ...
    async def get_cached_opportunities(self) -> List[Dict[str, Any]]:
        ...
        if not self.redis:
            self.logger.warning("No Redis available - cannot retrieve cached opportunities")
            return []
        
        try:
            self.logger.info("Attempting to retrieve cached opportunities from Redis...")
            
            # Add timeout to prevent hanging
            import asyncio
            try:
                cached_data = await asyncio.wait_for(self.redis.get("market_opportunities"), timeout=5.0)
            except asyncio.TimeoutError:
                self.logger.error("Timeout retrieving data from Redis")
                return []
            
            if cached_data:
                self.logger.info(f"Raw cached data length: {len(cached_data)} characters")
                ...
                if opportunities:
                    ...
                    self.logger.info(f"First opportunity: {first_opp.get('symbol', 'Unknown')} - Score: {first_opp.get('total_score', 0)}")
                
                return opportunities
            else:
                self.logger.warning("No cached opportunities found in Redis - market monitor may not be running")
                return []
                
        except json.JSONDecodeError as e:
            self.logger.error(f"Error parsing cached JSON data: {e}")
            return []
        except Exception as e:
