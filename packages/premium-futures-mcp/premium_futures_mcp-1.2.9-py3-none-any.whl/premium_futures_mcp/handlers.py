...
class ToolHandler:
        ...
        async with BinanceClient(self.config) as client:
            ...
            # Market Intelligence Tools - Initialize service if needed
                ...
                if not self.market_intelligence:
                    ...
                    # Try localhost first (host), then Docker network fallback  
                    redis_urls = [
                        "redis://:ilikeMyself1100@localhost:6379",      # Host Redis (primary) - DATA IS HERE!
                        "redis://:ilikeMyself1100@premium-redis:6379"   # Docker network (fallback)
                    ]
    ...
    async def _handle_intelligent_tools(self, name: str, arguments: Dict[str, Any], client: BinanceClient) -> Dict[str, Any]:
        ...
        # Ensure Redis client is set up
        if self.market_intelligence and (not self.market_intelligence.redis or self._redis_setup_pending):
            await self._setup_redis_for_intelligence()
            self._redis_setup_pending = False
        ...
        try:
            if name == "get_market_opportunities":
                ...
                # Get cached opportunities with timeout
                try:
                    import asyncio
                    opportunities = await asyncio.wait_for(
                        self.market_intelligence.get_cached_opportunities(),
                        timeout=5.0
                    )
                except asyncio.TimeoutError:
                    return {
                        "error": "Timeout getting cached opportunities",
                        "suggestion": "Redis may be slow or unavailable",
                        "opportunities": [],
                        "total_found": 0
                    }
                except Exception as e:
                ...
                try:
                    ...
                    if len(opportunities) == len(get_fallback_opportunities()):
                        is_fallback = True
                except:
                    pass
                ...
                try:
                    ...
                    return {
                        "error": f"Timeout getting data for {symbol}",
                        "suggestion": "Redis may be slow or unavailable"
                    }
                except Exception as e:
                    # Use fallback data when Redis fails
                ...
                try:
                    ...
                    return {
                        "error": "Timeout getting funding extremes data",
                        "suggestion": "Redis may be slow or unavailable",
                        "funding_extremes": []
                    }
                except Exception as e:
