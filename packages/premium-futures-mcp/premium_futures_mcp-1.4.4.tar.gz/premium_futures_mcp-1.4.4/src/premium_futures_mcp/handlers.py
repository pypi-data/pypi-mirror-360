...
class ToolHandler:
    ...
    async def _setup_redis_connection(self):
        ...
        try:
            ...
            # Direct Redis connection to localhost:6380 with password
            ...
            # Set the Redis client for the intelligence service and direct use
            if self.market_intelligence:
                self.market_intelligence.redis = redis_client
            
            # Store Redis client for direct access in intelligent tools
            self.redis_client = redis_client
            
            self.logger.info("✅ Redis connected successfully: localhost:6380")
            return
            
        except redis.ConnectionError:
            self.logger.error("❌ Could not connect to Redis at localhost:6380")
            self.logger.error("   Make sure Redis server is running on port 6380")
        except redis.AuthenticationError:
            self.logger.error("❌ Redis authentication failed")
            self.logger.error("   Check if password 'ilikeMyself1100' is correct")
        except Exception as e:
        ...
        if self.market_intelligence:
            self.market_intelligence.redis = None
    ...
    async def _get_redis_data(self, key: str) -> Any:
        ...
        try:
            ...
            if self.redis_client is None:
                self.logger.warning("Redis client not available for direct access")
                return None
            ...
            self.logger.error(f"Error getting Redis data for key '{key}': {e}")
            return None
    
    async def _get_market_opportunities_from_redis(self) -> list:
        """Get market opportunities directly from Redis cache"""
        try:
            # Get cached opportunities from Redis
            cache_data = await self._get_redis_data("market_opportunities")
            
            if cache_data and isinstance(cache_data, dict):
                opportunities = cache_data.get("opportunities", [])
                timestamp = cache_data.get("timestamp", "Unknown")
                
                self.logger.info(f"✅ Retrieved {len(opportunities)} opportunities from Redis (updated: {timestamp})")
                return opportunities
            else:
                self.logger.warning("❌ No market opportunities found in Redis cache")
                return []
                
        except Exception as e:
            self.logger.error(f"Error retrieving market opportunities from Redis: {e}")
            return []
    ...
    async def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with BinanceClient(self.config) as client:
            ...
            # Market Intelligence Tools - Use direct Redis access
                ...
                # Ensure Redis connection is established for direct access
                if not hasattr(self, 'redis_client') or self.redis_client is None:
                    await self._setup_redis_connection()
