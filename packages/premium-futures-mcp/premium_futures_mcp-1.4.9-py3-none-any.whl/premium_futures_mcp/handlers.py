...
class ToolHandler:
    ...
    async def _setup_redis_connection(self):
        ...
        try:
            ...
            self.redis_client = redis.Redis(
                ...
                socket_connect_timeout=15,  # Increased from 0.5 to 5 seconds
                socket_timeout=15,          # Increased from 0.5 to 5 seconds
    ...
    async def _get_redis_data(self, key: str) -> Any:
        ...
        try:
            ...
            if not hasattr(self, '_redis_tested'):
                try:
                    await asyncio.wait_for(self.redis_client.ping(), timeout=13.0)  # Increased from 0.5 to 3 seconds
            ...
            # Get data from Redis with longer timeout
            data = await asyncio.wait_for(self.redis_client.get(key), timeout=13.0)  # Increased from 0.5 to 3 seconds
            if data:
                import json
    ...
    async def _get_market_opportunities_from_redis(self) -> list:
        ...
        try:
            ...
            # Get cached opportunities from Redis with longer timeout
            cache_data = await asyncio.wait_for(
                self._get_redis_data("market_opportunities"), 
                timeout=15.0  # Increased from 1.0 to 5.0 seconds
            )
