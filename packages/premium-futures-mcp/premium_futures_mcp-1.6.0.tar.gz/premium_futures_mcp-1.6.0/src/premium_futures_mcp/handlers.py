...
class ToolHandler:
    ...
    def __init__(self, config: BinanceConfig, ticker_cache: TickerCache):
        ...
        # Public API URL for intelligent tools (external port)
        # Use environment variable or default to external IP for remote access
        self.public_api_url = os.getenv("PREMIUM_PUBLIC_API_URL", "http://188.166.184.242:19426")
        self.premium_member_key = config.premium_member_key
        
        # Debug logging for configuration
        self.logger.info(f"Initialized ToolHandler with public API URL: {self.public_api_url}")
        self.logger.info(f"Premium member key configured: {'Yes' if self.premium_member_key else 'No'}")
        if self.premium_member_key:
            self.logger.info(f"Member key prefix: {self.premium_member_key[:16]}...")
    ...
    async def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with BinanceClient(self.config) as client:
            ...
            elif name in ["get_market_opportunities", 
    ...
    async def _call_public_api(self, endpoint: str, params: dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Debug logging
            self.logger.info(f"Calling public API: {self.public_api_url}{endpoint}")
            self.logger.info(f"Headers: Authorization=Bearer {self.premium_member_key[:16]}...")
            ...
            self.logger.error(f"General error in _call_public_api: {str(e)}")
    ...
    async def _handle_intelligent_tools(self, name: str, arguments: Dict[str, Any], client: BinanceClient) -> Dict[str, Any]:
        ...
        try:
            if name == "get_market_opportunities":
                ...
                min_score = arguments.get("min_score", 27)  # Updated for 55-point system
                ...
                # Get opportunities from public API with appropriate filters (updated for 55-point system)
                if urgency == "IMMEDIATE":
                    min_score = 30  # Updated from 28 (55% threshold)
                    limit = 5
                elif urgency == "URGENT":
                    min_score = 27  # Updated from 25 (50% threshold)
                    limit = 8
                else:  # NORMAL
                    min_score = 24  # Updated from 22 (45% threshold)
                    limit = 10
