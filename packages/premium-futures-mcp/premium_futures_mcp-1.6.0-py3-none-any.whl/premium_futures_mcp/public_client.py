...
class PublicBinanceClient:
    ...
    async def get_basis(self, pair: str, contract_type: str = "PERPETUAL", period: str = "1h", limit: int = 30, **kwargs) -> List[Dict[str, Any]]:
        """
        Get basis data for futures contracts
        
        Args:
            pair: Trading pair (e.g., BTCUSDT)
            contract_type: Contract type (CURRENT_QUARTER, NEXT_QUARTER, PERPETUAL)
            period: Period for basis data (5m, 15m, 30m, 1h, 2h, 4h, 6h, 12h, 1d)
            limit: Number of records to return (max 500, default 30)
            **kwargs: Additional parameters like startTime, endTime
        
        Returns:
            List of basis data with indexPrice, futuresPrice, basis, basisRate, etc.
        """
        params = {
            "pair": pair,
        ...
        params.update(kwargs)
        return await self._make_request("GET", "/futures/data/basis", params)
