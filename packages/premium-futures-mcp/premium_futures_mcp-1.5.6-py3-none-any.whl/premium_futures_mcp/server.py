...
class BinanceMCPServer:
    ...
    def __init__(self, api_key: str = "", secret_key: str = "", member_key: str = ""):
        ...
        self.config = BinanceConfig(api_key, secret_key, member_key)
    ...
    async def _validate_member_key(self, member_key: str):
        """Validate the premium member key using remote validation"""
        if not member_key:
            print("[WARNING] No premium member key provided. Premium features will be disabled.")
            return
            
        try:
            print("[INFO] Validating premium member key...")
            is_valid = await validate_premium_key(member_key)
            if is_valid:
                print(f"[OK] Premium member key validated successfully")
                self._member_key = True  # Mark as valid
            else:
            ...
            print("[WARNING] Premium features will be disabled.")
