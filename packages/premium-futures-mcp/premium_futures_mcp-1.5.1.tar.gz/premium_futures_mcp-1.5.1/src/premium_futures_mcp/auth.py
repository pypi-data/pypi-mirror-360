    ...
    def _ensure_db_directory(self):
        ...
        if db_dir and not os.path.exists(db_dir):
            try:
                ...
                # Fallback to current directory if we can't write to the preferred location
                self.db_path = os.path.join(os.getcwd(), "premium_keys.db")
        
        # Test if we can write to the database path
        try:
            # Try to create/open the database file to test write permissions
            test_path = self.db_path
            with open(test_path, 'a'):
                pass  # Just test if we can open for writing
        except PermissionError:
            # If we can't write to the intended path, use current directory
            self.db_path = os.path.join(os.getcwd(), "premium_keys.db")
