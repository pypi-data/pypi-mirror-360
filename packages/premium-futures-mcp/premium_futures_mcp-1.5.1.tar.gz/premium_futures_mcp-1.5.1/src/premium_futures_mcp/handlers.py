...
class ToolHandler:
    ...
    def __init__(self, config: BinanceConfig, ticker_cache: TickerCache):
        ...
        self.public_api_url = "http://localhost:8081"
        self.premium_member_key = getattr(config, 'premium_member_key', None)
    ...
    async def handle_tool_call(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with BinanceClient(self.config) as client:
            ...
            # Market Intelligence Tools - Use public API
    ...
    async def _call_public_api(self, endpoint: str, params: dict = None) -> Dict[str, Any]:
        """Call public API endpoint with premium member key authentication"""
        if not self.premium_member_key:
            raise Exception("Premium member key not configured")
        
        try:
            headers = {
                "Authorization": f"Bearer {self.premium_member_key}",
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception("Invalid or expired premium member key")
            ...
            raise Exception(f"Public API connection error: {str(e)}")
        except Exception as e:
            raise Exception(f"Public API call failed: {str(e)}")

    async def _get_market_opportunities_from_api(self, **kwargs) -> Dict[str, Any]:
        """Get market opportunities from public API"""
        params = {}
        if kwargs.get('limit'):
            params['limit'] = kwargs['limit']
        if kwargs.get('min_score') is not None:
            params['min_score'] = kwargs['min_score']
        if kwargs.get('direction') and kwargs['direction'] != 'ALL':
            # Map direction names for API compatibility
            direction_map = {
                'LONG': 'LONG_PRIORITY',
                'SHORT': 'SHORT_PRIORITY',
                'WATCH': 'WATCH'
            }
            params['direction'] = direction_map.get(kwargs['direction'], kwargs['direction'])
        
        return await self._call_public_api("/market-opportunities", params)
    
    async def _get_market_summary_from_api(self) -> Dict[str, Any]:
        """Get market summary from public API"""
        return await self._call_public_api("/market-summary")
    
    async def _get_symbol_analysis_from_api(self, symbol: str) -> Dict[str, Any]:
        """Get specific symbol analysis from public API"""
        return await self._call_public_api(f"/market-opportunities/{symbol}")
    ...
    async def _handle_intelligent_tools(self, name: str, arguments: Dict[str, Any], client: BinanceClient) -> Dict[str, Any]:
        """Handle intelligent market analysis tools using public API"""
        
        # Log tool execution
        self.logger.info(f"Executing intelligent tool: {name} via public API")
        ...
        try:
            if name == "get_market_opportunities":
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting market opportunities from public API...")
                api_response = await self._get_market_opportunities_from_api(
                    limit=limit,
                    min_score=min_score,
                    direction=direction_filter
                )
                
                opportunities = api_response.get("opportunities", [])
                ...
                result = {
                    ...
                    "total_analyzed": api_response.get("total_analyzed", len(opportunities)),
                    "filters_applied": {
                        "min_score": min_score,
                        "direction": direction_filter,
                    ...
                    "source": "public_api_endpoint",
                    "cache_version": api_response.get("cache_version"),
                ...
                # Get specific symbol analysis from public API
                self.logger.info(f"🔍 Getting token analysis for {symbol} from public API...")
                try:
                    api_response = await self._get_symbol_analysis_from_api(symbol)
                    
                    if api_response.get("success"):
                        token_analysis = api_response.get("data", {})
                        
                        if include_context:
                            # Get market summary for context
                            summary_response = await self._get_market_summary_from_api()
                            total_analyzed = summary_response.get("total_opportunities", 0)
                            
                            # Estimate rank based on score
                            if total_analyzed > 0:
                                symbol_score = token_analysis.get("total_score", 0)
                                estimated_rank = max(1, int((50 - symbol_score) / 50 * total_analyzed))
                                
                                token_analysis["market_context"] = {
                                    "estimated_rank": estimated_rank,
                                    "total_analyzed": total_analyzed,
                                    "score": symbol_score,
                                    "percentile": round((1 - (estimated_rank - 1) / total_analyzed) * 100, 1)
                                }
                        
                        token_analysis["source"] = "public_api_endpoint"
                    ...
                    return {
                        "error": f"Symbol {symbol} not found or API error: {str(e)}",
                        "suggestion": "Check symbol format (e.g., BTCUSDT) or try get_market_opportunities to see available symbols"
                    }
                ...
                # Get market summary from public API
                self.logger.info("🔍 Getting market dashboard data from public API...")
                summary_response = await self._get_market_summary_from_api()
                
                # Get full opportunities list for categorization
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                
                # Create comprehensive dashboard
                dashboard = {
                    "market_overview": {
                        "total_tokens_analyzed": summary_response.get("total_opportunities", 0),
                        "high_confidence_count": summary_response.get("high_confidence", 0),
                        "long_signals": summary_response.get("long_signals", 0),
                        "short_signals": summary_response.get("short_signals", 0),
                        "watch_signals": summary_response.get("watch_signals", 0),
                        "avg_score": summary_response.get("avg_score", 0),
                        "last_updated": summary_response.get("timestamp")
                    },
                    "top_opportunities": summary_response.get("top_5", []),
                    "categories": {
                        "funding_extremes": [o for o in opportunities 
                                           if o.get('score_breakdown', {}).get('funding_rate', 0) >= 4][:category_limit],
                        "volume_leaders": [o for o in opportunities 
                                         if o.get('score_breakdown', {}).get('volume_spike', 0) >= 6][:category_limit],
                        "volatility_squeeze": [o for o in opportunities 
                                             if o.get('score_breakdown', {}).get('volatility_squeeze', 0) >= 3][:category_limit],
                        "whale_activity": [o for o in opportunities 
                                         if o.get('score_breakdown', {}).get('whale_activity', 0) >= 4][:category_limit]
                    }
                }
                ...
                dashboard["source"] = "public_api_endpoint"
                dashboard["api_metadata"] = summary_response.get("metadata", {})
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting funding extremes data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                ...
                for opp in opportunities:
                    ...
                    if funding_score >= 4:
                        ...
                        elif include_direction == "LONG_SQUEEZE" and "LONG" in direction:
                            funding_extremes.append(opp)
                        elif include_direction == "SHORT_SQUEEZE" and "SHORT" in direction:
                            funding_extremes.append(opp)
                ...
                return {
                    ...
                    "squeeze_analysis": {
                        "long_squeeze_candidates": len([o for o in funding_extremes if "LONG" in o.get('direction', '')]),
                        "short_squeeze_candidates": len([o for o in funding_extremes if "SHORT" in o.get('direction', '')]),
                        "trading_signals": {
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting volume leaders data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                ...
                return {
                    ...
                    "timeframe": timeframe,
                    "source": "public_api_endpoint",
                    "note": f"Tokens with volume ≥ {multiplier}x average AND volume spike score ≥ 4"
                }
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting volatility squeeze data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting whale activity data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                ...
                for opp in opportunities:
                    ...
                    if whale_score >= min_whale_score:
                        ...
                        elif whale_direction in direction:
                            whale_tokens.append(opp)
                ...
                # Get opportunities from public API
                self.logger.info("🔍 Getting narrative plays data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(limit=50)
                opportunities = opportunities_response.get("opportunities", [])
                ...
                for opp in opportunities:
                    ...
                    # Filter based on narrative and sentiment
                    if narrative == "ALL":
                        ...
                        if category_scores.get(narrative, 0) > 0 and sentiment_score >= min_sentiment_score:
                            narrative_tokens.append(opp)
                        elif opp_narrative == narrative and sentiment_score >= min_sentiment_score:
                            narrative_tokens.append(opp)
                
                # Sort by category score, then sentiment, then total score
                if narrative == "ALL":
                ...
                return {
                    ...
                    "source": "public_api_endpoint",
                    "note": "Narrative-driven tokens with sustained momentum potential"
                }
            
            elif name == "get_quick_scan":
                urgency = arguments.get("urgency", "NORMAL")
                
                # Get opportunities from public API with appropriate filters
                ...
                self.logger.info("🔍 Getting quick scan data from public API...")
                opportunities_response = await self._get_market_opportunities_from_api(
                    limit=limit,
                    min_score=min_score
                )
                opportunities = opportunities_response.get("opportunities", [])
                
                # Additional filtering for urgency level
                if urgency == "IMMEDIATE":
                    quick_opportunities = [o for o in opportunities if o.get('confidence') == 'HIGH']
                elif urgency == "URGENT":
                    quick_opportunities = [o for o in opportunities if o.get('confidence') in ['HIGH', 'MEDIUM']]
                else:  # NORMAL
                    quick_opportunities = opportunities
                
                return {
                    "quick_opportunities": quick_opportunities,
                    ...
                    "source": "public_api_endpoint",
                    "note": "Quick scan for time-sensitive opportunities via secure API"
                }
            ...
            error_msg = str(e)
            self.logger.error(f"Error in intelligent tool {name}: {error_msg}")
            
            return {
                "error": f"Failed to execute {name}: {error_msg}",
                "tool_name": name,
                "arguments": arguments,
                "suggestion": "Check premium member key configuration and public API connectivity"
            }
