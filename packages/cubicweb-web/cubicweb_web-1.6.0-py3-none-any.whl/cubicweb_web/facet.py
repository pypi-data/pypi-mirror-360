...
"""
The :mod:`cubicweb_web.facet` module contains a set of abstract classes to use
...
--------------------------
.. autoclass:: cubicweb_web.facet.RelationFacet
.. autoclass:: cubicweb_web.facet.RelationAttributeFacet
.. autoclass:: cubicweb_web.facet.HasRelationFacet
.. autoclass:: cubicweb_web.facet.AttributeFacet
.. autoclass:: cubicweb_web.facet.RQLPathFacet
.. autoclass:: cubicweb_web.facet.RangeFacet
.. autoclass:: cubicweb_web.facet.DateRangeFacet
.. autoclass:: cubicweb_web.facet.BitFieldFacet
.. autoclass:: cubicweb_web.facet.AbstractRangeRQLPathFacet
.. autoclass:: cubicweb_web.facet.RangeRQLPathFacet
.. autoclass:: cubicweb_web.facet.DateRangeRQLPathFacet
...
.. autoclass:: cubicweb_web.facet.AbstractFacet
.. autoclass:: cubicweb_web.facet.VocabularyFacet
...
from cubicweb_web import RequestError, htmlwidgets
...
def cleanup_select(select, filtered_variable):
    ...
    * mandatory relations linked to the main variable
    ...
    while toremove:
        ...
        if rel is not None and rel not in removed:
            select.remove_node(rel)
            removed.add(rel)
...
class AbstractFacet(AppObject):
    ...
    * `context`, telling if a facet should be displayed in the table form filter
    ...
    .. automethod:: cubicweb_web.facet.AbstractFacet.get_widget
    .. automethod:: cubicweb_web.facet.AbstractFacet.add_rql_restrictions
    ...
    * `filtered_variable`, the variable node in this rql syntax tree that we're
    ...
    .. autoattribute:: cubicweb_web.facet.AbstractFacet.operator
    .. automethod:: cubicweb_web.facet.AbstractFacet.rqlexec
    """
    ...
    cw_property_defs = {
        _("visible"): dict(
            type="Boolean", default=True, help=_("display the facet or not")
        ),
        _("order"): dict(type="Int", default=99, help=_("display order of the facet")),
        _("context"): dict(
            type="String",
            default="",
            # None <-> both
            ...
            help=_(
                "context where this facet should be displayed, " "leave empty for both"
            ),
        ),
    }
    visible = True
    ...
    def __init__(self, req, select=None, filtered_variable=None, **kwargs):
        super().__init__(req, **kwargs)
...
class VocabularyFacet(AbstractFacet):
    ...
    .. automethod:: cubicweb_web.facet.VocabularyFacet.vocabulary
    .. automethod:: cubicweb_web.facet.VocabularyFacet.possible_values
    """
...
class RelationFacet(VocabularyFacet):
    ...
    * `sortasc`: boolean flag to control ascendant/descendant sorting
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__} on ({self.rtype}-{self.role})>"
    ...
    def vocabulary(self):
        ...
        try:
            ...
            try:
                ...
                self.exception(
                    "error while getting vocabulary for %s, rql: %s",
                    self,
                    select.as_string(),
                )
                return ()
    ...
    def add_rql_restrictions(self):
        ...
        restrvar, rel = _add_rtype_relation(
            self.select, filtered_variable, self.rtype, self.role
        )
    ...
    def i18nable(self):
        ...
        if self.target_type:
            ...
            eschema = self._cw.vreg.schema.relation_schema_for(self.rtype).objects()[0]
        else:
            eschema = self._cw.vreg.schema.rschema(self.rtype).subjects()[0]
        return getattr(
            eschema.relation_definition(self.target_attr), "internationalizable", False
        )

    @property
    def no_relation(self):
        return not self._cw.vreg.schema.relation_schema_for(
            self.rtype
        ).final and self._search_card("?*")
    ...
    def rql_sort(self):
        ...
        """
        return self.sortfunc is not None or (
            self.label_vid is None and not self.i18nable
        )
    ...
    def rset_vocabulary(self, rset):
        ...
        if self.rql_sort:
            ...
            if self.label_vid is None:
                ...
                values = [
                    (entity.view(self.label_vid), entity.eid)
        ...
        if isinstance(value, str):
            ...
            if value:
                self.select.add_constant_restriction(
                    restrvar, self.restr_attr, value, self.restr_attr_type
                )
            else:
            ...
            while value:
                restrvar, rtrel = _make_relation(
                    self.select, self.filtered_variable, self.rtype, self.role
                )
                if rel is None:
    ...
    def _and_restriction(self, rel, restrvar, value):
        if rel is None:
            ...
            rrel = nodes.make_constant_restriction(
                restrvar, self.restr_attr, value, self.restr_attr_type
            )
    ...
    def _include_no_relation(self):
        if not self.no_relation:
            return False
        if self._cw.vreg.schema.relation_schema_for(self.rtype).final:
            return False
        if self.role == "object":
            subj = next(
                utils.rqlvar_maker(
                    defined=self.select.defined_vars, aliases=self.select.aliases
                )
            )
        ...
        if self.select.with_:
            restrictions.append(
                "WITH " + ",".join(term.as_string() for term in self.select.with_)
            )
        if restrictions:
            restrictions = "," + ",".join(restrictions)
        else:
            restrictions = ""
        rql = "Any {} LIMIT 1 WHERE NOT {} {} {}{}".format(
            self.filtered_variable.name,
            subj,
            self.rtype,
            obj,
            restrictions,
    ...
    def _add_not_rel_restr(self, rel):
        nrrel = nodes.Not(
            _make_relation(self.select, self.filtered_variable, self.rtype, self.role)[
                1
            ]
        )
    ...
    * `role` neither, it's systematically 'subject'
    ...
    def vocabulary(self):
        ...
        try:
            filtered_variable = self.filtered_variable
            cleanup_select(select, filtered_variable)
            newvar = prepare_vocabulary_select(
                select, filtered_variable, self.rtype, self.role
            )
    ...
    def add_rql_restrictions(self):
        ...
        filtered_variable = self.filtered_variable
        self.select.add_constant_restriction(
            filtered_variable, self.rtype, value, self.target_attr_type, self.comparator
        )
...
class RQLPathFacet(RelationFacet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for part in self.path:
            if isinstance(part, str):
                part = part.split()
            assert len(part) == 3, "path should be a list of 3-uples, not %s" % part
    ...
    def add_rql_restrictions(self):
        ...
        if value is None:
            return
        varmap, restrvar = self.add_path_to_select(skiplabel=True, skipattrfilter=True)
    ...
    def add_path_to_select(self, skiplabel=False, skipattrfilter=False):
        ...
        for part in self.path:
            ...
            if object == self.filter_variable:
                rschema = self._cw.vreg.schema.relation_schema_for(rtype)
                if rschema.final:
                    # filter variable is an attribute variable
                    if self.restr_attr is None:
                        self.restr_attr = rtype
                    if self.restr_attr_type is None:
                        attrtypes = {obj for subj, obj in rschema.relation_definitions}
                        if len(attrtypes) > 1:
                            raise Exception(
                                "ambigous attribute %s, specify attrtype on %s"
            ...
            rel = nodes.make_relation(subjvar, rtype, (objvar,), nodes.VariableRef)
    ...
    * user can specify lower/upper boundaries, not only the lower one
    ...
    def _range_rset(self):
        ...
        try:
            ...
            if filtered_variable.stinfo["typerel"] is None:
                etypes = frozenset(
                    sol[filtered_variable.name] for sol in select.solutions
                )
                select.add_type_restriction(filtered_variable, etypes)
            try:
    ...
    def get_widget(self):
        """return the widget instance to use to display this facet"""
        values = {value for _, value in self.vocabulary() if value is not None}
...
class AbstractRangeRQLPathFacet(RQLPathFacet):
    ...
    def _add_restriction(self, value, operator, restrvar, restrel):
        self.select.add_constant_restriction(
            restrvar, restrel, self.formatvalue(value), self.target_attr_type, operator
        )
...
class BitFieldFacet(AttributeFacet):
    ...
    See also :class:`~cubicweb_web.formwidgets.BitSelect`.
    """
    ...
    def add_rql_restrictions(self):
        ...
        if value == 0:
            ...
            comp.append(
                nodes.MathExpression(
                    "&", nodes.variable_ref(attr_var), nodes.Constant(value, "Int")
                )
            )
...
class FacetVocabularyWidget(htmlwidgets.HTMLWidget):
    ...
    def css_overflow_limit(self):
        ...
        """
        return css_em_num_value(
            self.facet._cw.vreg, "facet_vocabMaxHeight", _DEFAULT_VOCAB_WIDGET_HEIGHT
        )

    @cachedproperty
    def height(self):
        """title, optional and/or dropdown, len(items) or upper limit"""
        return 1.5 + int(  # title + small magic constant
            self.facet.support_and + min(len(self.items), self.css_overflow_limit)
        )
...
"""
    ...
    def _render(self):
        ...
        facet._cw.html_headers.add_onload(
            self.onload
            % {
                "sliderid": sliderid,
                "facetid": facetid,
        ...
        self.w('<div id="%s"></div>', sliderid, escape=False)
        self.w("</div>\n")
        self.w("</div>\n")
...
class DateFacetRangeWidget(FacetRangeWidget):
    formatter = (
        "function (value) {return (new Date(parseFloat(value))).strftime(DATE_FMT);}"
    )
    ...
    def __init__(self, facet, minvalue, maxvalue):
        ...
        super().__init__(facet, datetime2ticks(minvalue), datetime2ticks(maxvalue))
