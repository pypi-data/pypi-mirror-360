...
class DataLoader(BaseEstimator):
    ...
    def fetch(self,
        ...
        concat_size: int
            合并LazyFrame的数据量大小，当惰性计算过大时，应当降低concat_size的大小。否则，会超出机器处理阈值
        eager: bool
        show_progress
        ...
        ranger = range(0, len(iterables), batch_size)
        ranger = tqdm(ranger, desc=f"{self.__class__.__name__}.fetching") if show_progress else ranger
        for i in ranger:
            batch = iterables[i: i + batch_size]
            batch_ranger = range(0, len(batch))
            batch_ranger = tqdm(batch_ranger, desc="Tranforming batch lazyframe") if show_progress else batch_ranger
            lfList = [self.pipe.fit_transform(batch[i]) for i in batch_ranger]
            if concat_size > 0:
                lfList = [lfList[i:i + concat_size] for i in range(0, len(lfList), concat_size)]

                lf_cache = list()

                with ygo.pool(n_jobs=0) as go:
                    for lfs in lfList:
                        go.submit(lambda lfs: concat(lfs).collect().lazy(),
                                  job_name="Concating batch lazyframe")(lfs=lfs)
                    for res in go.do():
                        lf_cache.append(res)
