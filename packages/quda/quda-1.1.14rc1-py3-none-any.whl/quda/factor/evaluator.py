...
class Evaluator:
    ...
    @staticmethod
    def read_data(fac: 'Factor', date_cond: str, time_cond: str):
        """读取本地数据"""
        query = f"select * from {fac.tb_name} where {date_cond} and {time_cond}"
        data = database.sql(query, lazy=False).drop(FIELD_VERSION).with_columns(pl.col(FIELD_DATE).cast(pl.Utf8))
        return data
    ...
    def get_value(fac: 'Factor', ctx: 'FactorContext') -> pl.DataFrame | None:
        ...
        data = Evaluator.read_data(fac, f"date='{val_date}'", f"time='{val_time}'")
