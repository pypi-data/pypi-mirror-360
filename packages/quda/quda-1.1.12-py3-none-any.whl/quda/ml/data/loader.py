...
class DataLoader(BaseEstimator):
    ...
    def fetch(self,
        ...
        pbar = tqdm(total=len(batch_list), desc=f"{self.__class__.__name__}.fetching", leave=False)
        for batch in batch_list:
            lfList = [self.pipe.fit_transform(batch[i]) for i in tqdm(range(len(batch)), desc="Tranforming batch lazyframe", leave=False)]
            lf = concat(lfList)
