...
IContainerManagement_VERSION = "5.3.2760"
...
class IContainerManagement(BaseEndpoint):
    ...
    def canAccessVerbs(self, verbsOrRoles):
        postUri = "/_canAccessVerbs"
        __data__ = None
        __encoded__ = None
        __data__ = serder.ListSerDer(serder.STRING).encode(verbsOrRoles)
        __encoded__ = json.dumps(__data__)
        ...
        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IContainerManagement_VERSION}, data=__encoded__)
        from netbluemind.core.container.api.PermittedVerbs import PermittedVerbs
        from netbluemind.core.container.api.PermittedVerbs import __PermittedVerbsSerDer__
        return self.handleResult__(__PermittedVerbsSerDer__(), response)
