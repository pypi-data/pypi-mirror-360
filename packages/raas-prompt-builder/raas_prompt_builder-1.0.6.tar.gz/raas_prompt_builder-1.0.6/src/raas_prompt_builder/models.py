...
class RuleValidationInput(BaseModel):
    """룰 검증 입력 데이터"""
    rule_json: Dict[str, Any] = Field(..., description="검증할 룰 JSON 데이터")
    validation_type: str = Field(..., description="검증 유형 (syntax, logic, performance)")
    context: Optional[str] = Field(default=None, description="검증 컨텍스트")
    options: Optional[Dict[str, Any]] = Field(default=None, description="검증 옵션")


class HtmlReportInput(BaseModel):
    """HTML 리포트 생성 입력 데이터"""
    analysis_results: Dict[str, Any] = Field(..., description="분석 결과 데이터")
    ...
    theme: str = Field(default="modern", description="테마 (modern, classic, minimal)")


class IssueDetectionInput(BaseModel):
    """이슈 검출 입력 데이터"""
    rule_data: Dict[str, Any] = Field(..., description="룰 데이터")
    detection_scope: str = Field(default="all", description="검출 범위 (syntax, logic, performance, all)")
    severity_threshold: str = Field(default="medium", description="심각도 임계값 (low, medium, high)")
