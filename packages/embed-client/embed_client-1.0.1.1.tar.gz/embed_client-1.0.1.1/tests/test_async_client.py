...
class MockAiohttpResponse:
    def __init__(self, json_data=None, status=200, raise_http=None, text_data=None, raise_json=None):
...
async def test_health(client):
    ...
    with patch.object(client._session, 'get', return_value=MockAiohttpResponse({"status": "ok"})) as mock_get:
        ...
        mock_get.assert_called_with(make_url("/health"), timeout=client.timeout)
...
async def test_get_openapi_schema(client):
    ...
    with patch.object(client._session, 'get', return_value=MockAiohttpResponse({"openapi": "3.0.2"})) as mock_get:
        ...
        mock_get.assert_called_with(make_url("/openapi.json"), timeout=client.timeout)
    await client.__aexit__(None, None, None)
...
async def test_get_commands(client):
    ...
    with patch.object(client._session, 'get', return_value=MockAiohttpResponse({"commands": ["embed", "models"]})) as mock_get:
        ...
        mock_get.assert_called_with(make_url("/api/commands"), timeout=client.timeout)
    await client.__aexit__(None, None, None)
...
async def test_cmd(client):
    ...
    with patch.object(client._session, 'post', return_value=MockAiohttpResponse({"result": "ok"})) as mock_post:
        ...
        mock_post.assert_called_with(make_url("/cmd"), json={"command": "embed", "params": {"texts": ["abc"]}}, timeout=client.timeout)
    await client.__aexit__(None, None, None)
...
async def test_cmd_server_timeout_error(client):
    with patch.object(client._session, 'post', side_effect=aiohttp.ServerTimeoutError("Server timeout")):
        with pytest.raises(EmbeddingServiceTimeoutError) as excinfo:
            await client.cmd("embed", params={"texts": ["abc"]})
...
async def test_cmd_ssl_error(client):
    ...
    with patch.object(client._session, 'post', side_effect=aiohttp.ClientConnectionError("SSL connection error")):
        with pytest.raises(EmbeddingServiceConnectionError) as excinfo:
            await client.cmd("embed", params={"texts": ["abc"]})
        assert "Connection error" in str(excinfo.value)
...
async def test_cmd_json_decode_error(client):
    mock_response = MockAiohttpResponse(raise_json=json.JSONDecodeError("Invalid JSON", "doc", 0), text_data="<html>error</html>")
...
async def test_cmd_api_error(client):
    mock_response = MockAiohttpResponse(json_data={"result": {"success": False, "error": {"code": 123, "message": "Invalid command"}}})
    with patch.object(client._session, 'post', return_value=mock_response):
        with pytest.raises(EmbeddingServiceAPIError) as excinfo:
...
async def test_cmd_with_lang_and_text(client):
    mock_response = MockAiohttpResponse(json_data={"result": {"success": False, "error": {"code": 123, "message": "Invalid text"}}})
    with patch.object(client._session, 'post', return_value=mock_response):
        with pytest.raises(EmbeddingServiceAPIError) as excinfo:
...
async def test_embed_invalid_params_type(client):
    mock_response = MockAiohttpResponse(json_data={"result": {"success": False, "error": {"code": 422, "message": "Invalid params"}}})
    with patch.object(client._session, 'post', return_value=mock_response):
...
async def test_embed_no_params(client):
    mock_response = MockAiohttpResponse(json_data={"result": {"success": False, "error": {"code": 422, "message": "Missing params"}}})
    with patch.object(client._session, 'post', return_value=mock_response):
        with pytest.raises(EmbeddingServiceAPIError):
...
async def test_health_ssl_error(client):
    # Use a simple connection error to simulate SSL issues
    with patch.object(client._session, 'get', side_effect=aiohttp.ClientConnectionError("SSL connection error")):
        with pytest.raises(EmbeddingServiceConnectionError):
            await client.health()
...
async def test_init_edge_cases():
    ...
    # Test None port (this won't trigger since port has a default from env)
    # We need to test invalid port type instead
    with pytest.raises(EmbeddingServiceConfigError, match="port must be a valid integer"):
        EmbeddingServiceAsyncClient(base_url="http://test", port=0)
...
async def test_cmd_nested_error_response():
    ...
    client = EmbeddingServiceAsyncClient(base_url="http://test", port=8001)
    mock_response = MockAiohttpResponse(json_data={
        "result": {
            "success": False,
...
async def test_specific_aiohttp_exceptions():
    ...
    with patch.object(client._session, 'get', side_effect=aiohttp.ClientResponseError(
...
async def test_port_none_edge_case(monkeypatch):
    ...
    with patch('os.getenv', side_effect=lambda key, default=None: None if key == "EMBEDDING_SERVICE_PORT" else default):
        with pytest.raises(EmbeddingServiceConfigError, match="Invalid port configuration"):
            EmbeddingServiceAsyncClient(base_url="http://test")
...
async def test_format_error_response():
    ...
    assert error_resp == {"error": "Embedding service error: Test error", "text": "sample text"}
    
    # Test error with both lang and text
    error_resp = client._format_error_response("Test error", lang="en", text="sample text")
    assert error_resp == {"error": "Embedding service error: Test error", "lang": "en", "text": "sample text"}
...
async def test_extract_embedding_data_invalid():
    ...
    result = {
        "result": {
            "data": [
                {"embedding": [0.1, 0.2], "chunks": ["chunk1"]}  # Missing body
            ]
        }
    }
    ...
    result = {
        "result": {
            "data": [
                {"body": "text1", "chunks": ["chunk1"]}  # Missing embedding
            ]
        }
    }
    ...
    result = {
        "result": {
            "data": [
                {"body": "text1", "embedding": [0.1, 0.2]}  # Missing chunks
            ]
        }
    }
    with pytest.raises(ValueError, match="missing 'chunks' field"):
        client.extract_embedding_data(result)
...
async def test_cmd_unexpected_exception(monkeypatch):
    ...
    class DummyResp:
        ...
        async def raise_for_status(self): return None
    async def bad_post(*a, **kw): raise RuntimeError("fail-cmd")
...
async def test__parse_json_response_unexpected(monkeypatch):
    client = EmbeddingServiceAsyncClient()
    class FakeResp:
        async def json(self): raise Exception("unexpected")
    with pytest.raises(EmbeddingServiceJSONError):
...
async def test_cmd_all_except_branches(monkeypatch):
    ...
    class DummyResp:
        ...
        async def __aexit__(self, *a): return None
        async def json(self): return {"result": "ok"}
        def raise_for_status(self): raise aiohttp.ClientResponseError(request_info=None, history=None, status=400, message="fail")
    def post(*a, **kw): return DummyResp()
    ...
    class DummyConnKey:
        ...
        port = 8001
        is_ssl = True
    def raise_ssl(*a, **kw): raise aiohttp.ClientSSLError(connection_key=DummyConnKey(), os_error=OSError("ssl"))
    client._session.post = raise_ssl
    with pytest.raises(EmbeddingServiceConnectionError):
        await client.cmd("embed", params={"texts": ["abc"]})
    # aiohttp.ClientOSError
    def raise_os(*a, **kw): raise aiohttp.ClientOSError(OSError("os"))
