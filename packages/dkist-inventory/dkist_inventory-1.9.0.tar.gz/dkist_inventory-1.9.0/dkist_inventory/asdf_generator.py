...
def asdf_tree_from_filenames(
    ...
    if len(datasets) == 1:
        ...
        # This next block can be removed once all old VBI datasets have been reprocessed
        instrument = datasets[0].meta["inventory"]["instrumentName"]
        if instrument == "VBI":
            try:
                # This try/except is here mostly for tests. Gross, but this is all going away soon* anyway
                workflow_version = Version(datasets[0].meta["inventory"]["workflowVersion"])
                old_vbi_version = workflow_version < Version("1.19.0")
            except InvalidVersion:
                old_vbi_version = False

            if old_vbi_version:
                return _wrong_vbi_tiled_dataset(header_parser, datasets)
        ...
        tree = {
            "dataset": TiledDataset(
                datasets_arr,
                meta={"inventory": datasets[0].meta["inventory"]},
                mask=mask,
            )
        }
...
def _wrong_vbi_tiled_dataset(
    header_parser: HeaderParser, datasets: list[Dataset]
) -> Mapping[str, Any]:
    """
    Produce correct-looking inventory for `dkist-processing-vbi` < 1.19.0 datasets.

    All VBI datasets made with `dkist-processing-vbi` < 1.19.0 have incorrect MINDEX keys. An update to the python-tools
    will soon break the interaction with these datasets in a way that is Very Terrible. Until we can reprocess these
    VBI datasets this function can be used to re-generate those datasets' inventory in a way that will play nice with the
    new `dkist` version.

    Note that this function does NOT fix the header values, just how the `TiledDataset` object is stored.

    Once there are no datasets generated with `dkist-processing-vbi` < 1.19.0 this function should be removed.
    """
    # Extract dataset shape
    mosaic_shape = tuple(
    ...
    # Don't transpose here
    datasets_arr = np.array(datasets, dtype=object).reshape(mosaic_shape, order="F")

    # Flip rows to produce a correctly oriented mosaic
    datasets_arr = datasets_arr[:, ::-1]

    tree = {
        "dataset": TiledDataset(datasets_arr, meta={"inventory": datasets[0].meta["inventory"]})
    }
