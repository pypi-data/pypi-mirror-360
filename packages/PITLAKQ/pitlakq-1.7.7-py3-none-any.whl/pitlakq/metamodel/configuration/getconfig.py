...
def is_frozen():
    """Check if the current Python interpreter is frozen (e.g., in an executable)."""
    spec = importlib.util.find_spec('__main__')
    if spec is not None:
        return getattr(spec.loader, 'is_frozen', False)
    else:
        return False


def main_is_frozen():
    """Find out if running as exe made with py2exe."""
    return (hasattr(sys, "frozen") or # new py2exe
            hasattr(sys, "importers") # old py2exe
            or is_frozen) # tools/freeze
