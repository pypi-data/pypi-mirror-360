...
# ── keyword helper (child → keyword construct) ─────────────────
def keyword(child):
    return st.one_of(
...
# ── recursive node respecting max_depth ─────────────────────────
def _node(max_depth: int, depth: int):
    if depth >= max_depth:
        return leaf_scalar
    ...
    return st.one_of(
        leaf_scalar,
        str_wrap,  # strings wrapped exactly once
        attrset_wrap,
        container,
        keyword(child),
    )
...
def ynx_nodes(max_depth: int = 8, max_leaves: int = 60, kw_ratio: float = 0.8):
    ...
    • max_leaves  – total node cap
    • kw_ratio    – fraction of roots that are keyword constructs
    """

    def build(md):
        deep_child = _node(md, 1)
        root_kw = keyword(deep_child)
        container = st.one_of(
            st.dictionaries(ident | dotted, deep_child, min_size=1, max_size=3),
            st.lists(deep_child, min_size=0, max_size=3),
        )
        scalar_root = leaf_scalar
        return st.one_of(
            *([root_kw] * int(kw_ratio * 10)),
            *([container] * int((1 - kw_ratio) * 6)),
            *([scalar_root] * int((1 - kw_ratio) * 4)),
        )
