...
class Convert:
    ...
    @staticmethod
    def torch_to_onnx(model, input_shape, onnx_output_path='model.onnx', input_names=["images"], output_names=["output0"], opset_version=14, dynamic=False, dtype=torch.float32):
        """  
    ...
    def onnx_to_engine(input_path, input_shape, dynamic_batch=16, dynamic=False, type = 'fp16', calibration_data_dir=None):
        ...
        if dynamic:
            ...
            max_shape = (dynamic_batch, input_shape[1], input_shape[2]*2, input_shape[3]*2)  # 設定輸入最大可以接受的尺寸
            for inp in inputs:
    ...
    @staticmethod
    def torch_to_engine(model, input_shape, dynamic_batch=16, onnx_output_path='model.onnx', dynamic=False, type='fp16', calibration_image_dir=None):
        """
        ...
        Convert.convert_onnx2engine(
            onnx_output_path, 
            input_shape, dynamic_batch=16, 
            dynamic=dynamic, 
            type=type, 
