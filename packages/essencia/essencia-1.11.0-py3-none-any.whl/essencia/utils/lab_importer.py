...
from ..models.laboratory import LabTest, LabTestBatch, LabTestType, LabTestCategory, LabOrder
...
class LabCSVImporter:
    ...
    def import_csv(self, file_path: Path, laboratory: Optional[str] = None, 
                   clinical_reason: Optional[str] = None) -> Tuple[int, List[str]]:
        """Import laboratory tests from CSV file with automatic order creation.
        
        Args:
            file_path: Path to CSV file
            laboratory: Laboratory name (optional)
            clinical_reason: Clinical indication for tests (optional)
        ...
        success_count = 0
        errors = []
        date_orders = {}  # Track orders by date
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                # First pass: create orders for each unique date
                for _, test_date in dates:
                    if test_date not in date_orders:
                        order = LabOrder(
                        ...
                        order = order.save_self()
                        date_orders[test_date] = order
                        logger.info(f"Created order for date {test_date}")
                ...
                for row_idx, row in enumerate(rows[1:], 1):
                    ...
                    for col_idx, test_date in dates:
                        ...
                        try:
                            # Get order for this date
                            order = date_orders[test_date]
                            
                            # Create lab test record with order reference
                            lab_test = LabTest(
                                doctor_key=self.doctor_key,
                                patient_key=self.patient_key,
                                ...
                                date=test_date,
                                laboratory=laboratory,
                                order_key=order.key,  # Link to order
                                batch_key=batch.key
                            )
                            ...
                            tests_by_date[test_date].append(lab_test)
                            success_count += 1
                            
                        except Exception as e:
                            errors.append(f"Row {row_idx}, Date {test_date}: {str(e)}")
                
                # Update order test counts
                for test_date, tests in tests_by_date.items():
                    order = date_orders[test_date]
                    order.test_count = len(tests)
                    order.update_self()
                    logger.info(f"Order {order.key} has {order.test_count} tests")
                
                # Update batch
                batch.test_count = success_count
                batch.status = LabTestBatch.Status.COMPLETED if not errors else LabTestBatch.Status.PARTIAL
                batch.notes = f"Imported {success_count} tests in {len(date_orders)} orders" + (f", {len(errors)} errors" if errors else "")
                batch.update_self()
