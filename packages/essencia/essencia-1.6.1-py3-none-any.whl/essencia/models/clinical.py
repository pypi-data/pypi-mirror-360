...
class DiagnosisNote(PatientRelatedMixin):
    """
    Registro de diagnóstico CID-11 para um paciente.
    
    Permite registrar diagnósticos independentemente de consultas,
    mantendo histórico completo de diagnósticos do paciente.
    
    Attributes:
        icd_code: Código CID-11 do diagnóstico
        diagnosis_title: Título/descrição do diagnóstico
        notes: Observações adicionais sobre o diagnóstico
        date: Data do diagnóstico
        creator: Profissional que registrou o diagnóstico
        is_primary: Se é um diagnóstico principal
        specifiers: Especificadores do diagnóstico
    """
    COLLECTION_NAME = 'diagnosis_notes'
    
    icd_code: str = Field(..., description="Código CID-11 do diagnóstico")
    diagnosis_title: str = Field(..., description="Título do diagnóstico")
    notes: Optional[str] = Field(None, description="Observações sobre o diagnóstico")
    date: fd.DefaultDate = Field(default_factory=datetime.date.today)
    creator: str = Field(default='doctor.admin')
    is_primary: bool = Field(default=False, description="Se é diagnóstico principal")
    specifiers: List[str] = Field(default_factory=list, description="Especificadores aplicados")
    
    def __str__(self):
        prefix = "★" if self.is_primary else "•"
        return f"{prefix} {self.icd_code} - {self.diagnosis_title}"
