...
def get_text_loader(
    ...
    Args:
        file_path (Union[str, Path]): 文本文件的路径，支持字符串路径或 Path 对象
        encoding (Optional[str], optional): 文本文件的字符编码格式，如 "utf-8"、"gbk" 等。默认为 None，使用系统默认编码
...
def get_pdf_loader(
    ...
    Args:
        ...
        password (Optional[str], optional): PDF 文件密码，用于解密加密的 PDF 文件。默认为 None
        ...
        extract_tables (Union[Literal["csv", "markdown", "html"], None], optional): 表格提取格式，支持 csv、markdown、html 格式或 None。默认为 None
        images_parser (Optional[BaseImageBlobParser], optional): 自定义图像解析器。默认为 None
...
def get_web_loader(
    ...
    Args:
        web_path (Union[str, Sequence[str]]): 要加载的网页URL，可以是单个URL字符串或URL列表
...
def get_csv_loader(
    ...
    Args:
        file_path (Union[str, Path]): CSV 文件的路径，支持字符串或 Path 对象
        source_column (Optional[str], optional): 指定作为文档来源标识的列名。默认为 None，使用文件路径作为来源
        csv_args (Optional[Dict], optional): 传递给 csv.DictReader 的参数字典，如分隔符、引号字符等。默认为 None
        encoding (Optional[str], optional): CSV 文件的字符编码格式，如 "utf-8"、"gbk" 等。默认为 None，使用系统默认编码
...
def get_local_html_loader(
    file_path: Union[str, Path], open_encoding: Union[str, None] = None
) -> BaseLoader:
    """
    创建本地 HTML 文件加载器实例

    Args:
        file_path (Union[str, Path]): 本地 HTML 文件的路径，支持字符串路径或 Path 对象
        open_encoding (Union[str, None], optional): 打开文件时使用的字符编码格式，如 "utf-8"、"gbk" 等。默认为 None，使用系统默认编码
...
def get_json_loader(
    ...
    Args:
        ...
        content_key (Optional[str], optional): 从 jq_schema 结果中提取内容的键名。默认为 None
        is_content_key_jq_parsable (Optional[bool], optional): 标识 content_key 是否可以被 jq 解析。默认为 False
...
def get_word_loader(
    file_path: Union[str, Path], mode: Literal["page", "single"] = "page"
) -> BaseLoader:
    """
    创建 Word 文档加载器实例

    Args:
        file_path (Union[str, Path]): Word 文档(.docx)的文件路径
...
def get_excel_loader(
    ...
    Args:
        file_path (Union[str, Path]): Excel 文档(.xlsx)的文件路径
