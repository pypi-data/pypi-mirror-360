...
def get_document_by_ids(
    vector_store: VectorStore, ids: Optional[List[str]]
) -> List[Document]:
    """
    根据 ID 从向量存储中获取文档
    ...
    Args:
        ...
        ids (Optional[List[str]]): 文档的唯一标识符列表。如果未提供，将获取所有文档
...
def add_context(
    ...
    Args:
        ...
        ids (Optional[List[str]]): 上下文的唯一标识符列表。如果未提供，将自动生成
...
def add_documents(
    ...
    Args:
        ...
        ids (Optional[List[str]]): 文档的唯一标识符列表。如果未提供，将自动生成。
...
def similarity_search(
    ...
    Args:
        ...
        metadata_filter (Optional[Dict[str, str]]): 可选的元数据过滤器，用于限制搜索结果
