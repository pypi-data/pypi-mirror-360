...
def get_chroma_vectorstore(
    ...
    Args:
        ...
        embedding (Embeddings): 用于向量化的嵌入模型
        persist_directory (Optional[str]): 持久化目录的路径如果为 None，则在内存中创建
        client_settings (Optional[chromadb.config.Settings]): Chroma 客户端的配置
        collection_metadata (Optional[dict]): 集合的元数据
        client (Optional[chromadb.ClientAPI]): 一个已有的 Chroma 客户端实例
        create_collection_if_not_exists (Optional[bool]): 如果集合不存在，是否创建它默认为 True
