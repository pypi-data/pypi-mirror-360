...
def _get_alternate_titles_sync(imdb_id: str) -> list[tuple[str, str]]:
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                all_buttons.nth(1).wait_for(timeout=6500)
            except PlaywrightTimeoutError:
            ...
            try:
                page.locator("li.ipc-metadata-list__item").first.wait_for(timeout=6500)
            except PlaywrightTimeoutError:
                raise IMDbStructureChangedError(
            ...
            for i in range(items.count()):
                item = items.nth(i)
                try:
                    label = item.locator("span.ipc-metadata-list-item__label").inner_text(timeout=200)
                    title = item.locator("span.ipc-metadata-list-item__list-content-item").inner_text(timeout=200)
