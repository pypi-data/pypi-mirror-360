...
class ModelTrainer:
    ...
    def fix_model_embedding_layer(self, nli_model):
        ...
        old_token_type_embeddings = nli_model.bert.embeddings.token_type_embeddings
        ...
        nli_model.bert.embeddings.token_type_embeddings = new_token_type_embeddings
    ...
    def _train_nli(self, model, tokenizer, dataset : Union[str, DatasetDict, dict], val_dataset, label2_id, num_samples_train, num_samples_validation, num_percentage_validation, shuffle_dataset, batch_size, num_epochs, learning_rate,save_checkpoint , checkpoint_path, epochs_to_save, training_model_max_tokens, head_to_save, freeze_base):
        ...
        def preprocess_function(dataset):
            return tokenizer(dataset['premise'], dataset['hypothesis'], padding="max_length", truncation=True, max_length=training_model_max_tokens, return_tensors="pt")  
        ...
        if isinstance(dataset, str):
            ...
            pass
        #     if val_dataset:
        #         raise ValueError("Include the validation set in the DatasetDict object!")
