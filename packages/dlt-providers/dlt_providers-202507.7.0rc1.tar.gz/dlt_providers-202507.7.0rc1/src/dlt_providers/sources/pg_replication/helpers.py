...
class MessageConsumer:
    ...
    def _should_process_table(self, schema_name: str, table_name: str) -> bool:
        """Determine if a table should be processed based on schema and table filters.
        ...
        Returns:
            bool: True if the table should be processed, False otherwise
        """
        # If no filtered tables, process all tables in the schema
        if self.filtered_tables is None:
            return schema_name == self.schema_name

        # Check if the table is in the filtered tables list
        return table_name in self.filtered_tables and schema_name == self.schema_name
    ...
    def process_relation(self, decoded_msg: Relation) -> None:
        ...
        # Skip if this table is not in the filtered list
        if not self._should_process_table(schema_name, table_name):
            logger.debug(
                f"Skipping table {schema_name}.{table_name} as it's not in the filtered tables list"
            )
            return
