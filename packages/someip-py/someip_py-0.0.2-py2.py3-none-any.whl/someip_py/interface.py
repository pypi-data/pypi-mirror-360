...
"""
        ...
        def generate_class(name, rename=None, first_kls=False):
            ...
            include_struct_len = self.platform == "V30"
        ...
        def parse_xlsx(path, cfg):
            ...
            for _, row in interfaces_df.iterrows():
                ...
                if output:
                    output_dir = os.path.join(output, self.platform, service_name)
    ...
    def _get_interface(
        ...
        if not self.services.get(service_id):
            raise ServiceNotFoundError(
                f"service {service_id} not found, available services: {self.service.keys()}"
            )
    ...
    def register_service(
        self, server: SOMEIPServer, service: str, interfaces: List[str]
    ):
        if not self.origin.get(service):
            raise ServiceNotFoundError(
                f"service {service} not found, available services: {self.origin.keys()}"
    ...
    def as_server(
        ...
        if service and interfaces:
            self.register_service(server, service, interfaces)
        return server
