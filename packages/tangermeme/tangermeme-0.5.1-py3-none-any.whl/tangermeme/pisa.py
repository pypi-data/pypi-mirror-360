...
def pisa(model, X, args=None, batch_size=32, references=dinucleotide_shuffle, 
    ...
    n_outputs = predict(model, X[:1]).shape[-1]
    ...
    for i in trange(len(X), disable=not verbose):
        ...
        if return_references:
            references_.append(references_)
        ...
        for j in range(n_shuffles):
            ...
            with torch.autograd.set_grad_enabled(True):
                ...
                for k in range(0, n_outputs, batch_size):
                    ...
                    _multipliers.append(multipliers_)                  
        ...
        attr = torch.stack(multipliers, dim=0).mean(dim=0)
    ...
    if return_references:
        return attributions, torch.stack(references)
    return attributions
