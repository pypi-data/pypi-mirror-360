...
from libc.math cimport log, isfinite, pow, fmax, exp, INFINITY
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef floating_float_double[:] nan_inf_to_0_cython(floating_float_double[:] x):
    """Replace NaN/inf with zero."""
    cdef int i
    cdef int n = x.shape[0]
    cdef floating_float_double[:] result = x.copy()
