...
def is_syftbox_mode():
    ...
    try:
        from .auto_install import is_app_installed, is_syftbox_installed
        return is_syftbox_installed() and is_app_installed()
    except:
        return False
...
def start_server():
    ...
    if is_syftbox_mode():
        ...
        # Just check if it's running and warn if not
        if not ensure_server_healthy(max_retries=5):
            warnings.warn(
                "SyftQueue server is not running. In SyftBox mode, the server "
                "should be started automatically by SyftBox. Please check your "
                "SyftBox apps dashboard."
            )
            return False
        return True
