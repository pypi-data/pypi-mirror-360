...
class BaseQueue(ABC):
    ...
    def __init__(self, folder_path: Union[str, Path], queue_name: str = "default-queue", owner_email: str = None, **kwargs):
        ...
        # Initialize queue structure and syft-object atomically
        self._create_queue_atomically()
    
    def _create_queue_atomically(self):
        """Create queue structure and syft-object atomically - either both succeed or both fail."""
        temp_dir = None
        syft_object = None
        
        try:
            # Step 1: Try to create syft-object FIRST (this can fail)
            syft_object = self._create_syft_object()
            
            # Step 2: Only if syft-object succeeds, create directories
            self._initialize_queue_structure()
            
            # Step 3: Store reference to syft-object
            ...
            try:
                ...
                # Remove syft-object if it was created
                if syft_object:
                    try:
                        # Try to remove from syft-objects system
                        # Note: This may not work with current syft-objects API
                        pass
                    except:
                        pass
            except:
            ...
            raise RuntimeError(f"Failed to create queue '{self.queue_name}' atomically: {e}") from e
    
    def _create_syft_object(self):
        """Create or load the syft-object for this queue."""
        # First check if a syft-object already exists for this queue
        object_name = self.queue_name
        
        try:
            # Check if syft-object already exists
            for obj in syo.objects:
                if obj.name == object_name:
                    # Found existing syft-object, reuse it instead of creating duplicate
                    return obj
        except Exception:
            pass  # If syft-objects not available, continue to create new one
...
def get_queue(name: str) -> Optional[Queue]:
    ...
    try:
        queue_instance = Queue(folder_path, name)
        queue_instance.refresh_stats()
...
def queue(name: str = "default-queue", queue_type: str = "code", owner_email: str = None, force: bool = False, **kwargs) -> BaseQueue:
    ...
    if force:
        # Remove filesystem directory if it exists
