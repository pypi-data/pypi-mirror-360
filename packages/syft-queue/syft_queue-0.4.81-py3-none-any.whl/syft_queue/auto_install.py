...
def is_syftbox_app_installed() -> bool:
    """Check if syft-queue app is installed in SyftBox.
    
    Returns:
        True if syft-queue app directory exists in SyftBox/apps
    """
    apps_path = get_syftbox_apps_path()
    if not apps_path:
        return False
        
    syft_queue_app_path = apps_path / "syft-queue"
    return syft_queue_app_path.exists() and syft_queue_app_path.is_dir()


def clone_syftbox_app() -> bool:
    """Clone the syft-queue repository into SyftBox/apps.
    ...
    apps_path = get_syftbox_apps_path()
    if not apps_path:
        print("Warning: SyftBox directory not found. Cannot auto-install syft-queue app.", file=sys.stderr)
        return False
    ...
    # Repository URL
    repo_url = "https://github.com/OpenMined/syft-queue.git"
    target_path = apps_path / "syft-queue"
    ...
    try:
        ...
        print("❌ Git is not available. Cannot auto-install syft-queue app.", file=sys.stderr)
        return False
    except FileNotFoundError:
        print("❌ Git is not installed. Cannot auto-install syft-queue app.", file=sys.stderr)
        return False
...
def ensure_server_healthy(timeout_minutes: int = 5) -> bool:
    """Ensure syft-queue server is healthy by checking /health endpoint.
    
    This is the main function that implements the robust startup flow:
    1. Always check /health endpoint first
    2. If /health fails, check if syft-queue is installed in SyftBox
    ...
    if not get_syftbox_apps_path():
        print("❌ SyftBox not found - syft-queue requires SyftBox to run")
        return False
    ...
    if not is_syftbox_app_installed():
        ...
        if not reinstall_syftbox_app(silent=True):
            print("❌ Failed to install syft-queue app")
            return False
        print("✅ Syft-queue app installed")
...
def _get_server_port() -> Optional[int]:
    ...
    try:
        config_file = Path.home() / ".syftbox" / "syft_queue.config"
...
def wait_for_syft_queue_server(timeout_minutes: int = 5) -> bool:
    """Wait for syft-queue server to be available.
...
def start_syftbox_app(app_path: Path) -> bool:
    """Start the syft-queue app in SyftBox.
    
    Args:
        app_path: Path to the syft-queue app directory
    ...
    try:
        print("🚀 Starting syft-queue server...")
        ...
        print(f"❌ Failed to start syft-queue app: {e}")
        return False
...
def is_syftbox_running() -> bool:
    ...
    """
    try:
        # Try to import syftbox to check if it's available
        try:
            import syftbox
            ...
            # Check if we can connect to the SyftBox client
            if requests:
                try:
                    response = requests.get(str(client_config.client_url), timeout=2)
                    return response.status_code == 200 and "go1." in response.text
                except Exception:
                    return False
            
            # If requests not available, assume it's running if config exists
            return True
            
        except ImportError:
            # syftbox not installed, check if the daemon process is running
...
def reinstall_syftbox_app(silent=False) -> bool:
    """Reinstall syft-queue app in SyftBox by removing and re-cloning.
    ...
    if not apps_path:
        if not silent:
            print("❌ SyftBox directory not found. Cannot reinstall syft-queue app.")
        return False
    ...
    try:
        ...
        if syft_queue_app_path.exists():
            if not silent:
                print(f"🗑️  Removing existing syft-queue app from {syft_queue_app_path}")
            shutil.rmtree(syft_queue_app_path)
        
        # Clone fresh copy
        if not silent:
            print("🔄 Reinstalling syft-queue app...")
        
        success = clone_syftbox_app()
        
        if success and not silent:
            print("✅ Syft-queue app reinstalled successfully")
...
def ensure_syftbox_app_installed(silent=True) -> bool:
    """Ensure syft-queue app is installed in SyftBox (but don't auto-start server).
    ...
    if not is_syftbox_running():
        if not silent:
            print("❌ SyftBox is not running. Please start SyftBox before using syft-queue.")
    ...
    if not app_installed:
        if not silent:
            print("SyftBox detected but syft-queue app not found. Attempting auto-installation...")
        if not clone_syftbox_app():
            return False
        
        if not silent:
            print("✅ Syft-queue app installed successfully")
...
if __name__ == "__main__":  # pragma: no cover
    ...
    if ensure_syftbox_app_installed():  # pragma: no cover
        print("syft-queue app is available in SyftBox")  # pragma: no cover
    else:  # pragma: no cover
        print("syft-queue app is not available")  # pragma: no cover