...
class ArrowETC:
    ...
    def __init__(
        ...
        if self.n_path < 2:
            raise ValueError(f"The `path` parameter must have at least 2 points, not {self.n_path}")
        if arrow_width <= 0:
            raise ValueError(f"The `arrow_width` parameter must be greater than 0, not {arrow_width}")
    ...
    def _get_bezier_samples(self) -> np.ndarray:
        """
    ...
    def _get_curve_vertices(self, samples: np.ndarray) -> np.ndarray:
        """
