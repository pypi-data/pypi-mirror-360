...
| *Adaptive image averaging application. PNM image file open/save and filtering before/after display are based on PyPNM.* |
...
>
> If you have only new versions of Python and Tkinter, you may consider downloading [Main version of PyPNM](https://github.com/Dnyarri/PyPNM), which doesn't have any backward compatibility fixes, and therefore doesn't waste CPU time on it.
...
With a fistful of code for widgets and events this simplistic code may be easily turned into to a rather functional application.
...
| *Example of Tkinter-based viewer displaying ASCII PPM. Note that ASCII PNM files per se are not directly supported by Tkinter; in this example ASCII PPM is opened as 3D list using PyPNM, then, using PyPNM again, converted on the fly to PNM-like binary data in memory, which Tkinter can handle successfully.* |
