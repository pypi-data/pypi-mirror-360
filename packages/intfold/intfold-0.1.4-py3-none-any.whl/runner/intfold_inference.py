...
from intfold.openfold.config import model_config
from intfold.openfold.inference_config import get_model_config
from intfold.openfold.model.model import IntFold
from intfold.openfold.utils.atom_token_conversion import aggregate_fn_advanced as aggregate_fn
from intfold.openfold.model.confidences import get_summary_confidence, get_full_confidence

#### Data Processing
from intfold.data.module.inference import get_inference_dataloader, construct_empty_template_features
from intfold.data.types import MSA, Manifest, Record
from intfold.data.write.writer import write_cif
from intfold.data.inference.params import BoltzProcessedInput
from intfold.data.inference.utils import download, get_cache_path
from intfold.data.inference.data_tools import check_inputs, compute_msa, process_inputs, check_outputs, compute_similar_sequence
...
def main(args):
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    model = IntFold(config,generator = generator)
    checkpoint_path = cache / "intfold_v0.1.0.pt"
...
@click.option(
    "--cache",
    type=str,
    help="The directory where to download the data and model. Default is ~/.intfold, or $INFOLD_CACHE if set.",
...
intfold_cli.add_command(predict)
