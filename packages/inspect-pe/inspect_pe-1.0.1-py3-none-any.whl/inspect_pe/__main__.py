...
def echo_pretty(rep: Dict[str, Any]):
    cprint(f"[bold cyan]=== {rep['file']} ===[/]" if rich else f"=== {rep['file']} ===")
    ...
    cprint("[bold green]Sections:[/]" if rich else "Sections:")
    ...
    # optional blocks
    if rep.get("hashes"):
        cprint("[bold yellow]Hashes:[/]") if rich else cprint("Hashes:")
        for h, v in rep['hashes'].items():
            cprint(f"{h.upper():6}: {v}")

    def dump_list(title, key):
        if rep.get(key):
            cprint(f"[bold magenta]{title}:[/]") if rich else cprint(f"{title}:")
            for item in rep[key]:
                cprint(str(item))
