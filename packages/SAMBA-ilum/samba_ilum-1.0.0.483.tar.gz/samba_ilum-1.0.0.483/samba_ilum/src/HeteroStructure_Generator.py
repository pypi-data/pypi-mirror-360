       ...
       for k in range(n_Lattice):
           ...
           for j in range(n_structures):
               ...
               try:
                   ...
                   param = float(VTemp)
                   #---------------------------------------------
                   VTemp = poscar1.readline().split(); A = [float(VTemp[0])*param, float(VTemp[1])*param, float(VTemp[2])*param]
                   VTemp = poscar1.readline().split(); B = [float(VTemp[0])*param, float(VTemp[1])*param, float(VTemp[2])*param]
                   VTemp = poscar1.readline().split(); C = [float(VTemp[0])*param, float(VTemp[1])*param, float(VTemp[2])*param]  
                   #----------------------------------------------------------------------
                   # Obtenção das coordenadas da nova origem do sistema de coordenadas ---
                   #----------------------------------------------------------------------
                   if (orig == 0):
                   ...
                   for i in range(passo):
                       ...
                       x = float(VTemp[0]) + center_x
                       y = float(VTemp[1]) + center_y
                       z = float(VTemp[2])
                       l = str(VTemp[4])
