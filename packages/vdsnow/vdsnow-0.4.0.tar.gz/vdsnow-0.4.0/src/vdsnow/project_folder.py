...
def _update_object_type_sql_manifests(setup_schema_dir: Path) -> None:
    """
    REFACTORED: Uses the generic helper to update object-type manifests
    ...
    # Instead of looping over the static FOLDERS list, we discover the
    # directories that actually exist in the snowflake_structure.
...
def add_database() -> None:
    ...
    # SIMPLIFIED: Just create the folders. Refresh will handle the rest.
    create_schema_folders(db_name, schemas)
    # Create the empty setup directories
    for schema in schemas:
        (Path(SETUP_BASE_DIR_NAME) / db_name / schema).mkdir(parents=True, exist_ok=True)
