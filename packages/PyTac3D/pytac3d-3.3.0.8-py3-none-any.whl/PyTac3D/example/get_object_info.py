'''
Example: get_object_info.py

This example demonstrates how to estimate shape information of contacted objects from Tac3D tactile sensor data frames.

Main contents include:
1. Using PyTac3D.DataLoader to load locally recorded data
2. Estimating 3D geometry of contact areas with PyTac3D.Analyzer.detectObjects() function
3. Obtaining contact region and object shape information from frames using the `ContactRegions` data label
4. Visualizing tactile data with PyTac3D.Sensor and PyTac3D.Displayer
...
本示例主要展示如何从Tac3D触觉传感器的数据帧中估计接触物体的形状信息
