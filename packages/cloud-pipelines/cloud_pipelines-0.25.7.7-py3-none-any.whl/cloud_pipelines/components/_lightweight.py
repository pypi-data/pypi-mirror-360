...
from .._components.components._naming import _make_name_unique_by_adding_index
from .._components.components import _structures as structures
...
def _extract_component_interface(func: Callable) -> structures.ComponentSpec:
    ...
    for parameter in parameters:
        ...
        if isinstance(
            parameter.annotation, (OutputPath, OutputTextFile, OutputBinaryFile)
        ):
            ...
            if parameter.default is not inspect.Parameter.empty:
                ...
                if parameter.default is not None:
                    ...
                    try:
                        ...
                        warnings.warn(
                            f'Could not serialize the default value of the parameter "{parameter.name}". {ex}'
                        )
...
def _create_component_spec_from_func(
    func,
    extra_code="",
    ...
    Returns:
        A :py:class:`cloud_pipelines.components.structures.ComponentSpec` instance.
    """
    ...
    arguments = []
    arguments.extend(
        structures.InputValuePlaceholder(input.name) for input in component_inputs
    )
    arguments.extend(
        structures.OutputPathPlaceholder(output.name) for output in component_outputs
    )
    ...
    def get_argparse_type_for_input_file(passing_style):
        ...
        raise NotImplementedError(f'Unexpected data passing style: "{passing_style}".')

    def get_serializer_and_register_definitions(type_name) -> str:
        serializer_func: Callable = get_serializer_func_for_type_struct(type_name)
        if serializer_func:
    ...
    for input in component_inputs + file_outputs_passed_using_func_parameters:
        ...
        is_required = isinstance(input, structures.OutputSpec) or not input.optional
        ...
        if input._passing_style in [InputPath, InputTextFile, InputBinaryFile]:
            arguments_for_input = [
                param_flag,
                structures.InputPathPlaceholder(input.name),
            ]
        elif input._passing_style in [OutputPath, OutputTextFile, OutputBinaryFile]:
            arguments_for_input = [
                param_flag,
                structures.OutputPathPlaceholder(input.name),
            ]
        else:
            arguments_for_input = [
                param_flag,
                structures.InputValuePlaceholder(input.name),
            ]
            ...
            arguments.append(
                structures.IfPlaceholder(
                    structures.IfPlaceholderStructure(
                        condition=structures.IsPresentPlaceholder(input.name),
    ...
    if outputs_passed_through_func_return_tuple:
        ...
        line = f'_parser.add_argument("{param_flag}", dest="{output_param_var}", type=str, nargs={len(outputs_passed_through_func_return_tuple)})'
        arg_parse_code_lines.append(line)
        arguments.append(param_flag)
        arguments.extend(
            structures.OutputPathPlaceholder(output.name)
    ...
    # The return type is singular, not sequence
    if (
...
def create_component_from_func(
    ...
    *,
    ...
    Examples:
        ...
        Use the :py:class:`cloud_pipelines.components.InputPath` parameter annotation to tell the system that the function wants to consume the corresponding input data as a file. The system will download the data, write it to a local file and then pass the **path** of that file to the function.
        Use the :py:class:`cloud_pipelines.components.OutputPath` parameter annotation to tell the system that the function wants to produce the corresponding output data as a file. The system will prepare and pass the **path** of a file where the function should write the output data. After the function exits, the system will upload the data to the storage system so that it can be passed to downstream components.

        You can specify the type of the consumed/produced data by specifying the type argument to :py:class:`cloud_pipelines.components.InputPath` and :py:class:`cloud_pipelines.components.OutputPath`. The type can be a python type or an arbitrary type name string. :code:`OutputPath("CatBoostModel")` means that the function states that the data it has written to a file has type :code:`CatBoostModel`. :code:`InputPath("CatBoostModel")` means that the function states that it expect the data it reads from a file to have type "CatBoostModel". When the pipeline author connects inputs to outputs the system checks whether the types match.
    ...
    component_spec = _create_component_spec_from_func(
        func=func,
        base_image=base_image,
    ...
    return _internal_components_module._create_task_factory_from_component_spec(
        component_spec
    )
