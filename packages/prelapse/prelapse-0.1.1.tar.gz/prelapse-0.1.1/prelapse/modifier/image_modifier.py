...
from ..common import parse_group_args, gen_list_file, write_list_file, \
  call_shell_command, build_ff_cmd
...
def run_image_stab(self, args):
  ...
  for group in self.groups:
    if "1" in args.modstabphase:
      call_shell_command(build_phase1_cmd(self, args, group))
      self.logger.info("parsed stab 1")
    if "2" in args.modstabphase:
      call_shell_command(build_phase2_cmd(self, args, group))
...
def build_phase1_cmd(self, args, group):
  ...
  options_string = "movie={}:f=ffconcat:si=0:format_opts='safe=0\\:auto_convert=0',".format(list_file)
