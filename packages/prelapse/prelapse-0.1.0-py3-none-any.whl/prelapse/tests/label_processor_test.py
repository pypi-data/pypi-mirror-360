...
def test_process_valid_labels(monkeypatch, capsys, temp_files_fixture): #pylint: disable=redefined-outer-name
  ...
  monkeypatch.setattr("sys.argv", [
    "prelapse",
    "play",
    "--ignore-files-dont-exist",
    "-n",
    "-f", ffconcat_file,
    "-fd", str(ffconcat_file_fd),
    "-c", config_file,
    "-l", labels_file,
    "-v",
  ])
  prelapse.prelapse_main()
  ...
  assert "movie={}".format(ffconcat_file) in captured.out
