...
def process_command_output(process):
  output = ""
  while True:
    char = process.stderr.read(1).decode(getpreferredencoding(False), errors='replace')
...
3. Normalize pixel formats:
   a. Modify files in-place:
        mogrify -sampling-factor 4:2:0 "image.jpg"
   b. Or, to preserve originals:
        mkdir resampled
        mogrify -path resampled -sampling-factor 4:2:0 "image.jpg"
