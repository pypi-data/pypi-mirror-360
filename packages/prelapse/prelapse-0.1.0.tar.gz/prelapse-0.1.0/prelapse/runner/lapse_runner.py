...
class LapseRunner: # pylint: disable=no-member
  ...
  def parse_labels_and_config(self):
    ...
    """
    with open(self.labels, "r", encoding=getpreferredencoding(False)) as f:
      content = f.readlines()
  ...
  def run_prelapse(self, args, run=False, out=False):
    ...
    try:
      call_shell_command(runcmd, dry_run=args.dry_run)
    finally:
  ...
  def build_video_filter(self, setpts):
    ...
    cmd = "movie={}:f=ffconcat:si=0:format_opts='".format(self.ffconcatfile)
  ...
  def build_audio_filter(self, setpts):
    ...
    cmd = ";amovie={}".format(self.audiofile)
