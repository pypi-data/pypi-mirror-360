    ...
    if self.files:
      ...
      if self.labels is not None:
        ...
        for group in self.files:
          ...
          self.logger.debug("Group '{}' with {} images, {} sec at {} fps"
                            .format(group.group, len(group_data), ttime, self.labels_time))
          labels_output += "{0:.6f}\t{1}\n".format(time_cnt, group.group)
          time_cnt += ttime
        labels_output += "{0:.6f}\tend\n".format(time_cnt)
        if self.dry_run:
          print("{}\nDry run {}".format(labels_output, self.labels_outpath))
        else:
          with open(self.labels_outpath, "w", encoding=getpreferredencoding(False)) as f:
            f.write(labels_output)
