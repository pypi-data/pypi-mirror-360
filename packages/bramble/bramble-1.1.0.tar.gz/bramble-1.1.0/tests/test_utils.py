...
def test_validate_log_call_str_message_and_default_type():
    msg, msg_type, meta = validate_log_call("hello")
    assert msg == "hello"
...
def test_validate_log_call_exception_message():
    try:
        ...
        msg, msg_type, meta = validate_log_call(e)
...
def test_validate_log_call_with_string_message_type():
    msg, msg_type, meta = validate_log_call("info", "error")
    assert msg == "info"
...
def test_validate_log_call_invalid_message_type_raises():
    with pytest.raises(ValueError):
        validate_log_call("msg", object())


def test_validate_log_call_invalid_metadata_type_raises():
    with pytest.raises(ValueError):
        validate_log_call("msg", entry_metadata="not a dict")


def test_validate_log_call_metadata_invalid_key_type():
    with pytest.raises(ValueError):
        validate_log_call("msg", entry_metadata={1: "val"})


def test_validate_log_call_metadata_invalid_value_type():
    with pytest.raises(ValueError):
        validate_log_call("msg", entry_metadata={"key": object()})
...
def test_validate_tags_and_metadata_empty_returns_none():
    tags, metadata = validate_tags_and_metadata([], {}, tags=None, metadata=None)
    assert tags is None
...
def test_validate_tags_and_metadata_invalid_tag_list():
    with pytest.raises(ValueError):
        validate_tags_and_metadata([123], tags=None, metadata=None)


def test_validate_tags_and_metadata_invalid_metadata_key():
    with pytest.raises(ValueError):
        validate_tags_and_metadata({1: "val"}, tags=None, metadata=None)


def test_validate_tags_and_metadata_invalid_metadata_value():
    with pytest.raises(ValueError):
        validate_tags_and_metadata({"key": object()}, tags=None, metadata=None)


def test_validate_tags_and_metadata_invalid_arg_type():
    with pytest.raises(ValueError):
        validate_tags_and_metadata("not a list or dict", tags=None, metadata=None)
...
def test_stringify_function_call_handles_args_and_kwargs():
    ...
    s = stringify_function_call(dummy, [1], {"b": 2})
    assert "dummy(" in s
    assert "1" in s
    assert "b=2" in s
...
def test_stringify_function_call_handles_unprintable():
    ...
    s = stringify_function_call(dummy, [Unprintable()], {"x": Unprintable()})
    assert "`ERROR`" in s
