...
class Worker(Generic[T]):
    ...
    """
    def __init__(self, pipe: Connection, event: T | None, name: str, *args, **kwargs) -> None:
        """
        ...
        self.args = args
        self.kwargs = kwargs
        if self.event and event._stream:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if self.event:
                self.event._timing.stamp(self.__class__.__name__, "Finished")
        finally:
