    ...
    for participant in participants:
        ...
        if puuid not in stored_puuids:
            ...
            player_names = [
                ...
                if get_team_id(p) == get_team_id(participant)
            ]
            ...
            session.add(
                create_account(puuid, game_name, get_tagline(participant), db_player.id)
            )
