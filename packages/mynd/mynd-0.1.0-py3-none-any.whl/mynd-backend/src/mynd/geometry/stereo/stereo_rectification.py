...
from mynd.models.schema import CalibrationSchema, SensorSchema
from mynd.models.schema import StereoSensorPairSchema, StereoRigSchema
...
class StereoRectificationResult:
    """Class representing a rectification results, including original camera
    calibrations, rectified camera calibrations, pixel maps, inverse pixel
    maps, and rectifying transforms."""

    sensors_unrectified: StereoSensorPairSchema
    sensors_rectified: StereoSensorPairSchema
    pixel_maps: Pair[PixelMap]
    inverse_pixel_maps: Pair[PixelMap] | None = None


def compute_stereo_rectification(
    sensor_pair: StereoSensorPairSchema,
    ...
    # Compute the updated sensors (location, rotation, calibration) for the
    # rectified stereo rig
    rectified_sensor_pair: StereoSensorPairSchema = compute_rectified_sensors(
        sensor_pair
    )

    # Compute pixel maps between the original calibrations to the rectified
    # calibrations
    pixel_maps: PixelMapPair
    inverse_pixel_maps: PixelMapPair
    pixel_maps, inverse_pixel_maps = compute_rectified_pixel_maps(
        sensor_pair,
        rectified_sensor_pair,
    )

    return StereoRectificationResult(
        sensors_unrectified=sensor_pair,
        sensors_rectified=rectified_sensor_pair,
...
def rectify_image_pair(
    ...
    """Rectifies an image pair with a pair of pixel maps."""
    return Pair[Image](
        first=remap_image_pixels(images.first, pixel_maps.first),
        second=remap_image_pixels(images.second, pixel_maps.second),
    )


def compute_rectified_sensors(
    stereo_sensor: StereoSensorPairSchema,
) -> StereoSensorPairSchema:
    """Computes the camera calibrations for a pair of rectified cameras."""

    assert isinstance(
        stereo_sensor, StereoSensorPairSchema
    ), "invalid stereo sensor type"

    transforms: Transforms = _compute_rectifying_camera_transforms(stereo_sensor)

    resolution_master: tuple[int, int] = (
        stereo_sensor.master.calibration.width,
        stereo_sensor.master.calibration.height,
    )
    resolution_slave: tuple[int, int] = (
        stereo_sensor.slave.calibration.width,
        stereo_sensor.slave.calibration.height,
    )

    desired_resolution: tuple[int, int] = resolution_master

    affine_transform: np.ndarray = _compute_common_affine_transform(
        stereo_sensor.master.calibration.projection_matrix,  # intrinsic 1
        stereo_sensor.slave.calibration.projection_matrix,  # intrinsic 2
        ...
        stereo_sensor.master.calibration.distortion_vector,  # distortion 1
        stereo_sensor.slave.calibration.distortion_vector,  # distortion 2
    ...
    new_first_camera: np.ndarray = (
        ...
        .dot(stereo_sensor.master.calibration.projection_matrix)
        .dot(stereo_sensor.master.rotation_matrix)
        .dot(transforms.rotation.T)
    )

    new_second_camera: np.ndarray = (
        affine_transform.dot(transforms.homographies.second)
        .dot(stereo_sensor.slave.calibration.projection_matrix)
        .dot(stereo_sensor.slave.rotation_matrix)
        .dot(transforms.rotation.T)
    )
    ...
    master_calibration: CalibrationSchema = _create_camera_calibration(
        width=desired_resolution[0],
        height=desired_resolution[1],
        projection_matrix=updated_camera_matrices.first,
    )

    slave_calibration: CalibrationSchema = _create_camera_calibration(
        width=desired_resolution[0],
        height=desired_resolution[1],
        projection_matrix=updated_camera_matrices.second,
    )

    master_sensor: SensorSchema = SensorSchema(
        label=f"{stereo_sensor.master.label}_rectified",
        width=desired_resolution[0],
        height=desired_resolution[1],
    ...
    slave_sensor: SensorSchema = SensorSchema(
        label=f"{stereo_sensor.slave.label}_rectified",
        width=desired_resolution[0],
        height=desired_resolution[1],
        location=np.array([stereo_sensor.baseline, 0.0, 0.0]).tolist(),
        rotation=transforms.rotation.dot(
            stereo_sensor.slave.rotation_matrix.T
        ).tolist(),
        calibration=slave_calibration,
    )

    return StereoSensorPairSchema(master=master_sensor, slave=slave_sensor)


def compute_rectified_pixel_maps(
    sensors: StereoSensorPairSchema,
    sensors_rectified: StereoSensorPairSchema,
) -> tuple[PixelMapPair, PixelMapPair]:
    """Computes updated camera matrices and pixel maps based on the given stereo calibration
    and rectifying transforms."""

    assert isinstance(sensors, StereoSensorPairSchema), "invalid stereo sensor type"
    assert isinstance(
        sensors_rectified, StereoSensorPairSchema
    ), "invalid rectified stereo sensor type"

    # Recompute final maps considering fitting transformations too
    pixel_maps: Pair[PixelMap] = Pair(
        first=compute_pixel_map(
            sensors.master.calibration.projection_matrix,
            sensors.master.calibration.distortion_vector,
            sensors_rectified.master.rotation_matrix,
            sensors_rectified.master.calibration.projection_matrix,
            (
                sensors_rectified.master.calibration.width,
                sensors_rectified.master.calibration.height,
            ),
        ),
        second=compute_pixel_map(
            sensors.slave.calibration.projection_matrix,
            sensors.slave.calibration.distortion_vector,
            sensors_rectified.slave.rotation_matrix,
            sensors_rectified.slave.calibration.projection_matrix,
            (
                sensors_rectified.slave.calibration.width,
                sensors_rectified.slave.calibration.height,
            ),
        ),
    )
    ...
    return pixel_maps, inverse_pixel_maps
...
def _compute_rectifying_camera_transforms(
    stereo_sensor: StereoSensorPairSchema,
) -> Transforms:
    """
    ...
    first_rotation, second_rotation, _, _, _, _, _ = cv2.stereoRectify(
        stereo_sensor.master.calibration.projection_matrix,  # 3x3 master camera matrix
        stereo_sensor.master.calibration.distortion_vector,  # 5x1 master camera distortion
        stereo_sensor.slave.calibration.projection_matrix,  # 3x3 slave camera matrix
        stereo_sensor.slave.calibration.distortion_vector,  # 5x1 slave camera distortion
        resolution,  # resolution (width, height)
        stereo_sensor.slave.rotation_matrix,  # 3x3 rotation matrix from camera 1 to camera 2
        stereo_sensor.slave.location_vector,  # 3x1 translation vector from camera 1 to camera 2
        flags=0,
    )
    ...
    first_homography: np.ndarray = first_rotation.dot(
        np.linalg.inv(stereo_sensor.master.calibration.projection_matrix)
    )
    second_homography: np.ndarray = second_rotation.dot(
        np.linalg.inv(stereo_sensor.slave.calibration.projection_matrix)
    )
