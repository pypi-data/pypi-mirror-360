...
import fbnconfig
import tests.integration.transaction_configuration as side
...
def test_teardown_side(setup_deployment):
    # create first
    fbnconfig.deploy(side.configure(setup_deployment), lusid_env, token)
...
def test_nochange_side(setup_deployment):
    fbnconfig.deploy(side.configure(setup_deployment), lusid_env, token)
    update = fbnconfig.deploy(side.configure(setup_deployment), lusid_env, token)
