...
async def main() -> None:
    ...
    cert = await upload_tls_certificate_and_poll(client=client)
    await get_secret_by_id(client=client, secret_id=cert.id)
    await list_all_secrets(client=client)
    await delete_secret(client=client, secret_id=cert.id)


async def upload_tls_certificate_and_poll(*, client: AsyncGcore) -> Secret:
    ...
    secret = await client.cloud.secrets.upload_tls_certificate_and_poll(name="gcore-python-example", payload=payload)
    print(f"Secret ID: {secret.id}, name: {secret.name}")
    print("========================")
    return secret


async def get_secret_by_id(*, client: AsyncGcore, secret_id: str) -> Secret:
    print("\n=== GET SECRET BY ID ===")
    secret = await client.cloud.secrets.get(secret_id=secret_id)
    print(f"Secret ID: {secret.id}, name: {secret.name}")
    print("========================")
    return secret
...
async def delete_secret(*, client: AsyncGcore, secret_id: str) -> None:
    print("\n=== DELETE SECRET ===")
    await client.cloud.secrets.delete(secret_id=secret_id)
