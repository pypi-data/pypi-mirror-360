...
class Syncer:
    ...
    def autosave_hashes(self):
        if not self.use_hashes:
            return
        if time.time() - self.last_autosave_time < 2:
            return
...
def sync(src, dest, verbose=True, do_delete=False, check_metadata=True,
    ...
    (Syncer(Path(src), Path(dest), verbose, do_delete, check_metadata, advanced_output_features, use_hash)
     .visit(Path(".")))
