...
class Syncer:
    def __init__(self, src: Path, dst: Path, verbose, do_delete, check_metadata,
        ...
        if use_hash:
            ...
            print(self.dst_hashes.serialise())
    ...
    def check_hashes(self, path: Path) -> bool:
        eq = "==" if (self.src_hashes.get_hash(path)
                      == self.dst_hashes.get_hash(path)) else "!="
        terminal_formatting.hide_temp()
        print(f"{path} {self.src_hashes.get_hash(path)} {eq} {self.dst_hashes.get_hash(path)}")
