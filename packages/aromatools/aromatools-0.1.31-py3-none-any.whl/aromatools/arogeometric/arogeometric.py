...
def compute_index(G, atoms, index_type, filename):
    ...
    df = pd.DataFrame(results, columns=["Ring", "Bonds", "Index"])
    try:
        from IPython.display import display
        display(df)
    except ImportError:
...
def main():
    ...
    for file_xyz in xyz_files:
        ...
        try:
            ...
            pio.renderers.default = 'colab'  # o 'notebook' si es Jupyter local
            fig.show()
        except Exception as e:
            print("Plotly figure could not be displayed:", e)
