...
from ase.formula import molecular_mass
...
def display_molecular_descriptors(ase_atoms):
    formula = ase_atoms.get_chemical_formula()
    mass = molecular_mass(formula)
    natoms = len(ase_atoms)
    elements = list(set(atom.symbol for atom in ase_atoms))

    descriptors = {
        "Formula": formula,
        "Molecular mass (amu)": round(mass, 2),
