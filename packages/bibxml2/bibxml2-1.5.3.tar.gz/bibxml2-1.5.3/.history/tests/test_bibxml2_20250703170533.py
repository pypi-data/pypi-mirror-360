import pytest
import lxml.etree
from bibxml2.bibxml2 import convert_pica_record, convert_marc_record
from unicodedata import normalize

# Helper to build XML elements
def make_elem(tag, attrib=None, text=None, children=None) -> lxml.etree._ElementIterator:
    attrib = attrib or {}
    ...
    if children:
        for child in children:
            elem.append(child)
    parent_element = lxml.etree.Element("parent", None, None)
    parent_element.append(elem)
...
def test_convert_pica_record_basic():
    ...
    record = [field]
    result = list(convert_pica_record(record))
...
def test_convert_marc_record_leader_and_controlfield():
    ...
    record = [leader, control]
    result = list(convert_marc_record(record))
    assert result == [
        (1, 1, 'LDR', '', 'leadertext'),
        (2, 1, '001', '', 'ctrl')
    ]
