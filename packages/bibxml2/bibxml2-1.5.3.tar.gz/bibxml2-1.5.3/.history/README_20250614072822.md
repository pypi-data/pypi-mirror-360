...
For MARC data fields, `ind1` and `ind2` values are reported as separate rows with the `subfield_code` being `i_1` or `ind2`, but only when non-empty. The also have an empty `subfield_number`.
