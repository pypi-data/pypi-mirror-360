...
def convert(tag: str, convert_record: Callable[[lxml.etree._ElementIterator], Iterator[tuple[int, int, str, str, str]]], input: list[str], output: str) -> None:
    ...
    with cast(OpenFile, fsspec.open(output, 'wt' if not writing_parquet else 'wb', compression="infer")) as of, pq.ParquetWriter(of, schema=schema, compression='zstd', use_dictionary=True) if writing_parquet else nullcontext(csv.writer(of, delimiter='\t' if '.tsv' in output else ',')) as ow:
        if not writing_parquet:
            pw = pq.ParquetWriter(of, schema=schema, compression='zstd', use_dictionary=True)
            batch = []
            cw = None
        else:
            pw = None
                        ...
                        for row in convert_record(elem):
                            if writing_parquet:
                                ...
                                if len(batch) == 1024*1024:
                                    cast(pq.ParquetWriter, pw).write_batch(pa.record_batch(list(zip(*batch)),
                                        schema=schema))
                                    batch = []
                            else:
