...
def convert(input: list[str], output: str) -> None:
    ...
    with cast(OpenFile, fsspec.open(output, 'wt', compression="infer")) as of:
        if output.endswith(".parquet"):
            ...
            writer = pq.ParquetWriter(of, schema=pa.schema([
                pa.field('record_number', pa.uint32()),
                pa.field('field_number', pa.uint16()),
                pa.field('subfield_number', pa.uint8()),
                pa.field('field_code', pa.string()),
                pa.field('subfield_code', pa.string()),
                pa.field('value', pa.string()),
            ]))
            for input_file in fsspec.open_files(input, 'rb'):
                with input_file as inf:
                    compression = infer_compression(input_file.path)
                    if compression is not None:
                        inf = compr[compression](inf, mode='rb') # type: ignore
                    context = lxml.etree.iterparse(inf, events=('end', ), tag='{XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}record')
                    for _, elem in context:
                        for row in convert_record(1, elem):
