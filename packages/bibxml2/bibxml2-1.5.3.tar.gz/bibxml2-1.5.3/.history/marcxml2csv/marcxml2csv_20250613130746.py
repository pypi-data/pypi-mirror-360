...
def convert_recordrecord) -> Generator[tuple[int, int, str, str, str]]:
    f = 1
    for field in record:
        if field.tag == '{http://www.loc.gov/MARC21/slim}leader':
            ...
            yield n, f, 1, field.attrib['tag'], '', field.text
        elif field.tag == '{http://www.loc.gov/MARC21/slim}datafield':
            tag = field.attrib['tag']
            if field.attrib['ind1'] != ' ':
                yield n, f, 1, tag, 'ind1', field.attrib['ind1']
            if field.attrib['ind2'] != ' ':
                yield n, f, 1, tag, 'ind2', field.attrib['ind2']
            sf = 1
