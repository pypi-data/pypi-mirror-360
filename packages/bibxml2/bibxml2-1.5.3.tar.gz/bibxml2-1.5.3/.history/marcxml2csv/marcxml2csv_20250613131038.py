...
from typing import Generator, Literal, Sequence, cast
...
def convert_record(record: lxml.etree._ElementIterator) -> Generator[tuple[int, int, str, str, str]]:
    f = 1
    for field in record:
...
def convert_marcxml(input: list[str], output: str):
    convert(
        ...
        input=list(input),
        output=cast(str, fsspec.core.url_to_fs(output)[0].open(output, 'wt', compression="infer"))
    )
