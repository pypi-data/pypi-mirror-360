...
def convert(convert_record: Callable[[lxml.etree._ElementIterator], Generator[tuple[int, int, int, str, str, str]]], input: list[str], output: str) -> None:
                    ...
                    for _, elem in context:
                        for row in convert_record(elem):
                            if cw is not None:
                                ...
                                batch.append(row)
                                if len(batch) == 100000:
                                    cast(pq.ParquetWriter, pw).write_batch(pa.record_batch(zip(*batch),
                                        schema=schema))
                        n += 1
                        elem.clear()
