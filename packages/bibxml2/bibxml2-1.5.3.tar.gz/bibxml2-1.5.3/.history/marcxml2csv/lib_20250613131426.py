...
def convert(convert_record: Callable[[lxml.etree._ElementIterator], Generator[tuple[int, int, str, str, str]]], input: list[str], output: str) -> None:
        ...
        for input_file in input_files:
            ...
            with input_file as oinf:
                    ...
                    if compression is not None:
                        ...
                        inf = oinf
                    context = lxml.etree.iterparse(inf, events=('end',), tag=tag'{http://www.loc.gov/MARC21/slim}record')
                    for _, elem in context:
                        for row in convert_record(elem):
                            if pw is not None:
                                batch.append((n, *row))
                                if len(batch) == 100000:
                                    pw.write_batch(pa.record_batch(zip(*batch),
                                        schema=schema))
                                    batch = []
                            else:
