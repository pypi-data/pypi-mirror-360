...
def convert(tag: str, convert_record: Callable[[lxml.etree._ElementIterator], Iterator[tuple[int, int, str, str, str]]], input: list[str], output: str) -> None:
    ...
    with cast(OpenFile, fsspec.open(output, 'wt' if not writing_parquet else 'wb', compression="infer")) as of, pq.ParquetWriter(of, schema=schema, compression='zstd', use_dictionary=True) if writing_parquet else None as ow:
        if writing_parquet:
            import pyarrow as pa
