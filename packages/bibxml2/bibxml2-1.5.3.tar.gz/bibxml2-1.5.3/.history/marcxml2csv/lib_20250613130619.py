...
def convert(convert_record: Callable[[lxml.etree._Element], Generator[tuple[int, int, int, str, str, str]]], input: list[str], output: str) -> None:
                    ...
                    for _, elem in context:
                        for row in convert_record(elem):
                            if cw is not None:
                                ...
                                batch.append(row)
                                if len(batch) == 100000:
                                    cast(pq.ParquetWriter, pw).write_batch(pa.record_batch).write_table(pa.RecordBatch.from_arrays(
                                        [pa.array([r[0] for r in batch]),
                                         pa.array([r[1] for r in batch]),
                                         pa.array([r[2] for r in batch], type=pa.uint8()),
                                         pa.array([r[3] for r in batch]),
                                         pa.array([r[4] for r in batch]),
                                         pa.array([r[5] for r in batch])],
                                        schema=schema))
                        n += 1
                        elem.clear()
