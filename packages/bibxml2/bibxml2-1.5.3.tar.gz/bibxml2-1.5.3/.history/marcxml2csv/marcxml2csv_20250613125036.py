...
def convert_record(n: int, record, co: _csv.Writer) -> tuple[int, int, int | Literal[''], str, str, str]:
    f = 1
    for field in record:
        if field.tag == '{http://www.loc.gov/MARC21/slim}leader':
            return n, f, '', 'leader', '', field.text
        elif field.tag == '{http://www.loc.gov/MARC21/slim}controlfield':
            return n, f, '', field.attrib['tag'], '', field.text
        elif field.tag == '{http://www.loc.gov/MARC21/slim}datafield':
            tag = field.attrib['tag']
            if field.attrib['ind1'] != ' ':
                return n, f, '', tag, 'ind1', field.attrib['ind1']
            if field.attrib['ind2'] != ' ':
                return n, f, '', tag, 'ind2', field.attrib['ind2']
            sf = 1
            for subfield in field:
                return n, f, sf, tag, subfield.attrib['code'], subfield.text
                sf += 1
        else:
