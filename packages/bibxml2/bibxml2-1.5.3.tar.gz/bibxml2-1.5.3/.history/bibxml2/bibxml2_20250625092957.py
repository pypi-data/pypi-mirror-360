...
from typing import Callable, Iterator, Literal, cast, Sequence
...
@click.command
@click.option("-f", "--format", help="Input format (marcxml or picaxml)", type=click.Choice(['marcxml', 'picaxml'], case_sensitive=False), default='marcxml')
@click.option("-o", "--output", help="Output CSV/TSV (gz) / parquet file", required=True)
@click.option("-c", "--parquet-compression", help="Parquet compression codec", type=click.Choice(['snappy', 'gzip', 'zstd', 'brotli', 'lz4', 'lz4_raw', 'none'], case_sensitive=False), default='zstd')
@click.option("-l", "--parquet-compression-level", help="Parquet compression level (0-9)", type=int, default=22)
@click.argument('input', nargs=-1)
def convert(input: list[str], format: Literal['marcxml','picaxml'], output: str, parquet_compression: str = 'zstd', parquet_compression_level = 22, parquet_batch_size=1024*1024*64) -> None:
    if format == 'marcxml':
        tags = ('{http://www.loc.gov/MARC21/slim}record', 'record')
