...
ef convert_record(record: lxml.etree._ElementIterator) -> Iterator[tuple[int, int, str, str, str]]:
    for field_number, field in enumerate(record, start=1):
        tag = field.attrib['tag']
...
def convert(input: list[str], format: Literal['marcxml','picaxml'], output: str, compression: str = 'zstd', compression_level = 22, batch_size=1024*1024*64) -> None:
    if format == 'marcxml':
