...
@click.command
@click.option("-f", "--format", help="Input format (marc, marcxml or picaxml)", type=click.Choice(['marc', 'marcxml', 'picaxml'], case_sensitive=False), default='marcxml')
...
def convert(input: list[str], format: Literal['marc', 'marcxml','picaxml'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int) -> None:
    if format == 'marcxml' or format == 'marc':
