...
def convert_marc_record(record: Iterable[lxml.etree._ElementIterator]) -> Iterator[tuple[int, int, str, str, str]]:
...
def convert_pica_record(record: Iterable[lxml.etree._Element]) -> Iterator[tuple[int, int, str, str, str]]:
    for field_number, field in enumerate(record, start=1):
        tag = field.attrib['tag']
