...
def parquet_writer(outptu: str, parquet_compression: str, parquet_compression_level: int) -> pq.ParquetWriter:
    return pq.ParquetWriter(outptu, 
            schema=schema, 
...
def convert(input: list[str], format: Literal['marc','pica'], output: str, parquet_compression: str, parquet_compression_level: int, parquet_batch_size: int, max_rows: int) -> None:
    ...
    if writing_parquet:
        ow = parquet_writer(output, parquet_compression, parquet_compression_level)
        pq_rows_written = 0
        batch = []
    else:
        of = fsspec.open(output, 'wt' if not writing_parquet else 'wb', compression="infer")
        ow = csv.writer(of, delimiter='\t' if '.tsv' in output else ',')
        ow.writerow(['record_number', 'field_number', 'subfield_number', 'field_code', 'subfield_code', 'value'])
    n = 1
        ...
        with input_file as oinf:
                ...
                for _, elem in context:
                    ...
                    if writing_parquet and pq_rows_written >= max_rows:
                        cast(pq.ParquetWriter, ow).close()
                        ow = parquet_writer(of, parquet_compression, parquet_compression_level)
