...
def convert_record(record) -> Iterator[tuple[int, int, int, str, str, str]]:
    n = int(record.attrib['id'])
    for field_number, field in enumerate(record, start=1):
        ...
        for subfield_number, subfield in enumerate(field, start=1):
            yield n, field_number, subfield_number, tag, subfield.attrib['code'], subfield.text
