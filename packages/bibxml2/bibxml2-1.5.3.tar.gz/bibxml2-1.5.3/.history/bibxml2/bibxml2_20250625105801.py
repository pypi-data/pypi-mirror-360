    ...
    for field_number, field in enumerate(record, start = 1):
        if field.tag == '{http://www.loc.gov/MARC21/slim}leader' or field.tag == 'leader':
            ...
            if field.attrib['ind2'] != ' ':
                ...
                sf += 1
            for subfield_number, subfield in enumerate(filter(lambda subfield: not (subfield.text is None and print(f"No text in subfield {subfield.attrib['code']} of field {tag} in record {}") is None), field), start=sf):
