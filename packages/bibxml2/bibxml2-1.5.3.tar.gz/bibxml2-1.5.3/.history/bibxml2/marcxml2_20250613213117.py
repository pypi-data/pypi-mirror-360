...
def convert_record(record: lxml.etree._ElementIterator) -> Iterator[tuple[int, int, str, str, str]]:
    for field_number, field in enumerate(record, start = 1):
        if field.tag == '{http://www.loc.gov/MARC21/slim}leader':
            yield field_number, 1, 'leader', '', field.text
            ...
            if field.attrib['ind1'] != ' ':
                yield field_number, 1, tag, 'ind1', field.attrib['ind1']
            if field.attrib['ind2'] != ' ':
                yield field_number, 1, tag, 'ind2', field.attrib['ind2']
            for subfield_number, subfield_code in enumerate(field, start=1):
                yield field_number, subfield_number, tag, subfield_code.attrib['code'], subfield_code.text
        else:
