    ...
    for field_number, field in enumerate(record, start = 1):
        if field.tag == '{http://www.loc.gov/MARC21/slim}leader' or field.tag == 'leader':
            ...
            if field.attrib['ind2'] != ' ':
                ...
                sf += 1
            for subfield_number, subfield in enumerate(filter(lambda subfield: subfield.text is not None, field), start=sf):
...
def convert_pica_record(record: lxml.etree._ElementIterator) -> Iterator[tuple[int, int, str, str, str]]:
    for field_number, field in enumerate(record, start=1):
        tag = field.attrib['tag']
        for subfield_number, subfield in enumerate(filter(lambda subfield: subfield.text is not None and print(f'Processing subfield {subfield.attrib["code"]} of field {tag} in record {field_number}'), field), start=1):
            yield field_number, subfield_number, tag, subfield.attrib['code'], normalize('NFC', subfield.text)
