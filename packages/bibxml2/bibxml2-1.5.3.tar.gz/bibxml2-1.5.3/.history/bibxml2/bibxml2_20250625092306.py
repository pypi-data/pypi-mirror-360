...
def convert(tags: Sequence[str], convert_record: Callable[[lxml.etree._ElementIterator], Iterator[tuple[int, int, str, str, str]]], input: list[str], output: str, compression: str = 'zstd', compression_level = 22, batch_size=1024*1024*64) -> None:
    if format == 'marc'('{http://www.loc.gov/MARC21/slim}record', 'record')
