...
If the output file extension is `.parquet`, the output will be in parquet format, which is a columnar storage format that is efficient for large datasets and supports complex data types. This is useful for further data processing or analysis.
Otherwise, compressed files will be read/written if the filename ends with an identifier recognised by fsspec. TSV format will be used if the output filename contains `.tsv`, otherwise CSV will be used.
