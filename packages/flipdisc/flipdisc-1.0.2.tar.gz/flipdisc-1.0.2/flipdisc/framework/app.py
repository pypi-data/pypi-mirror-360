...
class Application(object):
    def __init__(
        ...
        * if setup_output is True a PUB socket for the output stream will be
        ...
        if "input_stream" in self.config:
            ...
            self._sub_channels.append(REDIS_KEYS.SYS_INPUT_CHANNEL)
        else:
            setup_input = False
        ...
        if "hdmi_stream" in self.config:
            ...
            self._sub_channels.append(REDIS_KEYS.SYS_HDMI_CHANNEL)
        else:
        ...
        if "output_stream" in self.config:
            ...
            self._sub_channels.append(REDIS_KEYS.SYS_OUTPUT_CHANNEL)
        else:
            setup_output = False
            subtopic = None
        ...
        if self.name:
            ...
            self._sub_channels.append(REDIS_KEYS.APP_CHANNEL + self._app_rkey)
    ...
    async def cleanup(self) -> None:
        ...
        if self.name and self._red is not None:
            ...
            await self._red.hdel(REDIS_KEYS.APPS, self._app_rkey) # type: ignore
    ...
    async def _app_heartbeat(self, app: "Application") -> None:
        ...
        try:
            await self._red.hset( # type: ignore
                REDIS_KEYS.APPS, self._app_rkey, json.dumps(self.config) # type: ignore
            )
        except:
