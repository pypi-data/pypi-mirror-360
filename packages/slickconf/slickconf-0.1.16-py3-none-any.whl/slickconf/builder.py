...
def auto_config_call_handler(fn_or_cls: callable, *args, **kwargs):
    if (
        ...
        or isinstance(fn_or_cls, (EagerCallContainer, ExternalContainer))
...
def config_fn(fn):
    ...
    if _is_lambda(fn):
        from slickconf.lambda_finder import getsource_for_lambda

        source = getsource_for_lambda(fn)
...
def get_object_file(obj):
    ...
    raise ValueError("No __code__ attribute found in object to determine file path")
...
class ExternalContainer:
    ...
    def build_obj(self, fn=False, key=None, *args, **kwargs):
        init_obj = Init(self.obj, self.name, fn=fn, key=key)(*args, **kwargs)
        init_obj["__meta"] = self.meta

        return init_obj

    def __call__(self, *args, **kwargs):
        key = f"{self.name}#{SingleCounter.counter}"
        SingleCounter.increase()

        return self.build_obj(fn=False, key=key, *args, **kwargs)

    def fn_call(self, *args, **kwargs):
        return self.build_obj(fn=True, *args, **kwargs)
...
class External:
    def __getitem__(self, obj):
        ...
        return ExternalContainer(obj, name, meta)
...
class LazyFn:
    def __getitem__(self, obj):
        if isinstance(obj, ExternalContainer):
            return obj.fn_call
