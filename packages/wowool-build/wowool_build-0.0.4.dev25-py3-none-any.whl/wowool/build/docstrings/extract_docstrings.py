...
def build_markdown(dist_folder, markdown_folder):
...
def extract_docstrings(fp_root: str | Path, markdown: bool = False, upload: bool = False):
    ...
    build_folder = wowool_folder / "doc-build"
    ...
    print(doc_config)
    build_json(doc_config, wowool_folder, build_json_folder)
    ...
    tar_fn = wowool_folder / "docstrings-json.tar.gz"
    with tarfile.open(tar_fn, "w:gz") as tar:
        for file in dist_folder.glob("**/*.json"):
            tar.add(file, arcname=file.relative_to(dist_folder))
    print(f"Docstrings extracted to {wowool_folder / 'docstrings-json.tar.gz'}")
    if upload:
