...
class MarkdownVisitor(DocstringVisitor):
    ...
    def __init__(self, include_toc: bool = True, include_examples: bool = True, include_notes: bool = True):
        self.markdown_lines = []
    ...
    def get_markdown(self) -> str:
        ...
        result.extend(self.markdown_lines)
    ...
    def on_module(self, module_data: Dict[str, Any]) -> Any:
        ...
        self._add_heading(f"Module: {module_name}", 1)
    ...
    def on_class(self, class_data: Dict[str, Any], module_name: str) -> Any:
        ...
        self._add_heading(f"Class: {class_name}", 2)
    ...
    def on_property(self, property_data: Dict[str, Any], class_name: str, module_name: str) -> Any:
        ...
        self._add_heading(f"Property: {self._format_inline_code(property_name)}", 3)
...
def to_markdown(filename: Path, root: Path, output: Path):
    md_fn = Path(filename).with_suffix(".md")
    md_fn = md_fn.relative_to(root) if root else md_fn
    md_fn = output / md_fn
    print(f"Converting JSON to Markdown: {md_fn}")
    markdown_visitor = MarkdownVisitor(include_toc=True, include_examples=True, include_notes=True)
