...
class TeamsClient(UtilityBase):
    ...
    async def send_formatted_message(self, channel: str, payload: Dict[str, Any]) -> bool:
        """
    ...
    async def _post_payload(self, webhook_url: str, payload: Dict[str, Any]) -> bool:
        """
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            self._log_error(
                                f"Failed to send Teams notification: {resp.status}, {text} (attempt {attempt})"
                            )
            except Exception:
                self._log_exception(f"Exception during Teams notification (attempt {attempt})")
