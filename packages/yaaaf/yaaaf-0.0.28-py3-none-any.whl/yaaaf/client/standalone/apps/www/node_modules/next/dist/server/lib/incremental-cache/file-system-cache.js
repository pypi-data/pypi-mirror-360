...
class FileSystemCache {
        ...
        if (!data && process.env.NEXT_RUNTIME !== 'edge') {
            ...
            try {
                ...
                if (kind === 'fetch' && this.flushToDisk) {
                    ...
                    const pageData = isAppPath ? await this.fs.readFile(this.getFilePath(`${key}${this.isAppPPREnabled ? _constants.RSC_PREFETCH_SUFFIX : _constants.RSC_SUFFIX}`, 'app'), 'utf8') : JSON.parse(await this.fs.readFile(this.getFilePath(`${key}${_constants.NEXT_DATA_SUFFIX}`, 'pages'), 'utf8'));
                    let meta;
                    if (isAppPath) {
                        try {
    ...
    async set(...args) {
        ...
        if ((data == null ? void 0 : data.kind) === 'PAGE') {
            ...
            await this.fs.writeFile(this.getFilePath(`${key}${isAppPath ? this.isAppPPREnabled ? _constants.RSC_PREFETCH_SUFFIX : _constants.RSC_SUFFIX : _constants.NEXT_DATA_SUFFIX}`, isAppPath ? 'app' : 'pages'), isAppPath ? data.pageData : JSON.stringify(data.pageData));
