...
# Test 1: Check that generate_lowrank_discrete() returns the correct shape for valid parameters
...
# Test 2: Check that generate_lowrank_discrete() returns the expected number of samples
...
# Test 3: Ensure that a warning is raised for large n_samples > 50000
...
# Test 4: Check that generated results are consistent when using the same random seed
...
# Test 5: Ensure ValueError is raised for logically incorrect parameters (negative samples, too many classes, etc.)
...
# Test 6: Ensure Exception is raised for invalid parameter types (strings, negative values, zero, etc.)
...
# Test 7: Check that generate_lowrank_continuous() returns correct shape (n_samples, 2)
...
# Test 8: Check that warning is raised in generate_lowrank_discrete() when K is too large for small sample size
...
# Test 9: Ensure that generate_lowrank_continuous() returns consistent samples for same seed
...
# Test 10: Ensure Exception is raised for invalid parameters in generate_lowrank_continuous()
