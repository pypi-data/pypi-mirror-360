...
class DomService:
	...
	target_frame: Optional[Frame] = None, highlight_index: int = 0, initial_root_node: Optional[JSHandle] = None) -> tuple[DOMElementNode, SelectorMap]:
		...
		args = {
			...
			'debugMode': debug_mode,'initialRootNode': initial_root_node, 'highlightIndex': highlight_index
		}
	  ...
	  remove_highlights: Optional[Callable[..., Awaitable[None]]] = None,
		...
		for frame, closed_shadow_roots in frames_descriptor_dict.items():
			...
			if frame == self.page.main_frame or iframe_element:
				...
				dom_element_node, selector_map =         await self._build_dom_tree(highlight_elements, focus_element, viewport_expansion, frame, highlight_index)
				...
				for closed_shadow_root in closed_shadow_roots:
					...
					host_elements: list[DOMElementNode] =           await DomUtils.traverse_and_filter(final_dom_element_node,
