...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		response = await asyncio.wait_for(self.llm.ainvoke(input_messages, output_format=self.AgentOutput), timeout=LLM_TIMEOUT_SECONDS)
	...
	async def _run_planner(self) -> str | None:
		...
		# Get planner output
		try:
			response = await asyncio.wait_for(self.settings.planner_llm.ainvoke(planner_messages), timeout=LLM_TIMEOUT_SECONDS)
		except Exception as e:
