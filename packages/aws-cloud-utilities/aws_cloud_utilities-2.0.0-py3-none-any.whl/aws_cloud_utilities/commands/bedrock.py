...
@click.option(
    "--region",
    help="Specific region to list models from (default: all regions)"
)
@click.option(
    "--output-file",
    help="Save results to file (supports .json, .csv, .yaml)"
)
@click.option(
    "--model-type",
                        ...
                        for page in paginator.paginate():
                            for model in page.get("modelSummaries", []):
                                model_info = {
                                    ...
                                    "Created": model.get("creationTime", "").strftime("%Y-%m-%d") if model.get("creationTime") else ""
                                }
...
@click.option(
    "--region",
    help="Region where the model is available (default: current region)"
)
@click.pass_context
            ...
            for page in paginator.paginate(**params):
                for model in page.get("modelSummaries", []):
                    model_info = {
                        ...
                        "Base Model": model.get("baseModelArn", "").split("/")[-1] if model.get("baseModelArn") else "",
                        "Status": model.get("status", ""),
                        "Created": model.get("creationTime", "").strftime("%Y-%m-%d %H:%M") if model.get("creationTime") else "",
            ...
            for page in paginator.paginate(**params):
                for job in page.get("modelCustomizationJobSummaries", []):
                    job_info = {
                        ...
                        "Created": job.get("creationTime", "").strftime("%Y-%m-%d %H:%M") if job.get("creationTime") else "",
