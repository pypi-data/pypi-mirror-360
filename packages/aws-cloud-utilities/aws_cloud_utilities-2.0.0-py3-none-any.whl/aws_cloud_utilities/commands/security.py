...
@click.option(
    "--region",
    help="AWS region to collect metrics from (default: current region)"
)
@click.option(
    "--time-range",
    type=int,
...
@click.option(
    "--all-regions",
    is_flag=True,
    help="Collect metrics from all regions"
)
@click.pass_context
def metrics(
    ctx: click.Context,
    ...
    try:
        ...
        console.print(f"[blue]Collecting security metrics for {time_range} hours across {len(target_regions)} regions[/blue]")
...
@click.option(
    "--alt-names",
    help="Comma-separated list of alternative domain names"
)
@click.option(
    "--hosted-zone-id",
...
@click.option(
    "--region",
    help="AWS region to list certificates from (default: current region)"
)
@click.option(
    "--status",
    type=click.Choice(["PENDING_VALIDATION", "ISSUED", "INACTIVE", "EXPIRED", "VALIDATION_TIMED_OUT", "REVOKED", "FAILED"]),
    help="Filter certificates by status"
)
@click.option(
    "--all-regions",
    is_flag=True,
    help="List certificates from all regions"
)
@click.pass_context
                    ...
                    for cert in page.get("CertificateSummaryList", []):
                        ...
                        try:
                            ...
                            all_certificates.append({
                                ...
                                "Created": cert_details.get("CreatedAt", "").strftime("%Y-%m-%d") if cert_details.get("CreatedAt") else "",
                                "Expires": cert_details.get("NotAfter", "").strftime("%Y-%m-%d") if cert_details.get("NotAfter") else "",
...
def _get_waf_metrics(aws_auth: AWSAuth, region: str, time_range: int) -> Dict[str, Any]:
    ...
    for acl in web_acls:
        ...
        allowed = _get_cloudwatch_metric_sum(
            cloudwatch_client, "AWS/WAFV2", "AllowedRequests",
            dimensions, start_time, end_time, period
        )
        
        blocked = _get_cloudwatch_metric_sum(
            cloudwatch_client, "AWS/WAFV2", "BlockedRequests",
            dimensions, start_time, end_time, period
        )
        ...
        try:
            ...
            for rule in rules:
                ...
                rule_blocked = _get_cloudwatch_metric_sum(
                    cloudwatch_client, "AWS/WAFV2", "BlockedRequests",
                    rule_dimensions, start_time, end_time, period
                )
...
def _get_cloudwatch_metric_sum(
    ...
    dimensions: List[Dict], start_time: datetime, end_time: datetime, period: int
) -> float:
...
def _display_security_metrics(
    ...
    for region, region_metrics in all_metrics.items():
        ...
        for service, service_metrics in region_metrics.items():
            if isinstance(service_metrics, dict) and "error" not in service_metrics and "message" not in service_metrics:
