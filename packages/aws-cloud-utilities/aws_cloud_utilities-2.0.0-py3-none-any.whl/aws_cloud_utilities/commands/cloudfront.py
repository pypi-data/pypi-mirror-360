...
@click.option(
    "--log-bucket",
    help="S3 bucket for CloudFront logs (required for logging configuration)"
)
@click.option(
    "--log-prefix",
    default="cf-logs",
    help="Default log prefix when no alternate domain names found (default: cf-logs)"
)
@click.option(
    "--setup-alarms",
    is_flag=True,
    help="Setup CloudWatch alarms for CloudFront distributions"
)
@click.option(
    "--remove-alarms",
    is_flag=True,
    help="Remove CloudWatch alarms for CloudFront distributions"
)
@click.option(
    "--sns-topic",
    help="SNS topic name for alarm notifications (required for alarm setup)"
)
@click.option(
    "--region",
    help="AWS region for SNS topic and CloudWatch alarms (default: current region)"
)
@click.option(
    "--dry-run",
    is_flag=True,
...
@click.option(
    "--output-file",
    help="Output file for results (supports .json, .yaml, .csv)"
)
@click.pass_context
def update_logging(
    ctx: click.Context,
    ...
    try:
        ...
        if not log_bucket and not setup_alarms and not remove_alarms:
            console.print("[yellow]⚠️  Warning: No --log-bucket specified. Logging configuration will be skipped.[/yellow]")
            console.print("[dim]Use --log-bucket to enable CloudFront logging configuration[/dim]")
        
        if setup_alarms and not sns_topic:
            console.print("[yellow]⚠️  Warning: No --sns-topic specified. Alarms will be created without notification actions.[/yellow]")
        ...
        if setup_alarms:
            console.print(f"[dim]Setting up CloudWatch alarms{' with SNS topic: ' + sns_topic if sns_topic else ' (no notifications)'}[/dim]")
        elif remove_alarms:
        ...
        update_results = _update_cloudfront_distributions(
            aws_auth, cf_client, target_region, log_bucket, log_prefix,
            setup_alarms, remove_alarms, sns_topic_arn, dry_run, config.workers
        )
...
@click.option(
    "--include-disabled",
    is_flag=True,
    help="Include disabled distributions in the list"
)
@click.option(
    "--show-logging-status",
    is_flag=True,
    help="Show logging configuration status for each distribution"
)
@click.option(
    "--output-file",
    help="Output file for distributions list (supports .json, .yaml, .csv)"
)
@click.pass_context
def list_distributions(
    ctx: click.Context,
    include_disabled: bool,
    show_logging_status: bool,
    output_file: Optional[str]
) -> None:
...
@click.option(
    "--show-config",
    is_flag=True,
    help="Show detailed distribution configuration"
)
@click.option(
    "--output-file",
    help="Output file for distribution details (supports .json, .yaml)"
)
@click.pass_context
def distribution_details(
    ...
    try:
        ...
        basic_info = {
            ...
            "Last Modified": distribution.get("LastModifiedTime", "").strftime("%Y-%m-%d %H:%M:%S") if distribution.get("LastModifiedTime") else ""
        }
        ...
        for page in paginator.paginate():
            ...
            for dist in page["DistributionList"]["Items"]:
                ...
                dist_data = {
                    ...
                    "State": "Enabled" if dist_config.get("Enabled", False) else "Disabled",
                    "Price Class": dist_config.get("PriceClass", ""),
                    "Comment": dist_config.get("Comment", "")[:50] + "..." if len(dist_config.get("Comment", "")) > 50 else dist_config.get("Comment", ""),
                    "Last Modified": dist.get("LastModifiedTime", "").strftime("%Y-%m-%d %H:%M") if dist.get("LastModifiedTime") else ""
                }
...
def _update_cloudfront_distributions(
    ...
    def process_distribution(dist: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return _update_single_distribution(
            aws_auth, dist, region, log_bucket, log_prefix,
            setup_alarms, remove_alarms, sns_topic_arn, dry_run
        )
...
def _update_single_distribution(
    ...
    try:
        ...
        if log_bucket:
            needs_update = (
                not result["logging_enabled"] or 
                not result["current_bucket"].startswith(log_bucket)
            )
        ...
        if setup_alarms or remove_alarms:
            if setup_alarms:
                ...
                alarm_result = _remove_cloudfront_alarms(
                    aws_auth, dist_id, result["domain"], region, dry_run
                )
...
def _display_update_results(config: Config, update_results: Dict[str, Any], dry_run: bool) -> None:
    ...
    summary_display = {
        ...
        "Updated" if not dry_run else "Would Update": update_results["updated_count"] if not dry_run else update_results["would_update_count"],
    ...
    if cf_managed_to_update:
        ...
        for dist in cf_managed_to_update:
            cf_data.append({
                ...
                "Aliases": ", ".join(dist.get("aliases", [])[:2]) + ("..." if len(dist.get("aliases", [])) > 2 else "")
            })
