...
@click.option(
    "--regions",
    help="Comma-separated list of regions to backup (default: all regions)"
)
@click.option(
    "--output-dir",
...
def backup(
    ...
    try:
        ...
        _execute_cloudformation_backup(
            aws_auth, target_regions, output_path, stack_status,
            max_workers, parallel_stacks, backup_summary, format
        )
...
@click.option(
    "--region",
    help="AWS region to list stacks from (default: current region)"
)
@click.option(
    "--stack-status",
    multiple=True,
...
@click.option(
    "--all-regions",
    is_flag=True,
    help="List stacks from all regions"
)
@click.pass_context
def list_stacks(
    ctx: click.Context,
                ...
                for page in paginator.paginate():
                    for stack in page.get("Stacks", []):
                        ...
                        all_stacks.append({
                            ...
                            "Created": stack.get("CreationTime", "").strftime("%Y-%m-%d %H:%M") if stack.get("CreationTime") else "",
                            "Updated": stack.get("LastUpdatedTime", "").strftime("%Y-%m-%d %H:%M") if stack.get("LastUpdatedTime") else "",
                            "Description": (stack.get("Description", "")[:50] + "...") if len(stack.get("Description", "")) > 50 else stack.get("Description", ""),
...
@click.option(
    "--region",
    help="AWS region where the stack is located (default: current region)"
)
@click.option(
    "--show-template",
    is_flag=True,
    help="Show the stack template"
)
@click.option(
    "--show-parameters",
    is_flag=True,
    help="Show stack parameters"
)
@click.option(
    "--show-outputs",
    is_flag=True,
    help="Show stack outputs"
)
@click.pass_context
def stack_details(
    ctx: click.Context,
    ...
    try:
        ...
        stack_info = {
            ...
            "Updated": stack.get("LastUpdatedTime", "").strftime("%Y-%m-%d %H:%M:%S") if stack.get("LastUpdatedTime") else "",
