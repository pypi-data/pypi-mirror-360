...
@click.option(
    "--tag",
    default="latest",
    help="Tag to use for the image in ECR (default: latest)"
)
@click.option(
    "--region",
    help="AWS region for ECR repository (default: current region)"
)
@click.option(
    "--create-repo",
    is_flag=True,
    help="Create ECR repository if it doesn't exist"
)
@click.option(
    "--force",
    is_flag=True,
    help="Force overwrite if image already exists"
)
@click.pass_context
def copy_image(
    ctx: click.Context,
    ...
    try:
        ...
        if not force:
            if _image_exists_in_ecr(ecr_client, ecr_repository.split('/')[-1], tag):
                console.print(f"[yellow]Image with tag '{tag}' already exists in repository. Use --force to overwrite.[/yellow]")
                raise click.Abort()
        ...
        try:
            ...
            if image_details:
                details_display = {
                    ...
                    "Pushed At": image_details.get("imagePushedAt", "").strftime("%Y-%m-%d %H:%M:%S") if image_details.get("imagePushedAt") else "",
...
@click.option(
    "--region",
    help="AWS region to list repositories from (default: current region)"
)
@click.option(
    "--all-regions",
    is_flag=True,
    help="List repositories from all regions"
)
@click.option(
    "--output-file",
                ...
                for page in paginator.paginate():
                    for repo in page.get("repositories", []):
                        all_repositories.append({
                            ...
                            "Created": repo.get("createdAt", "").strftime("%Y-%m-%d %H:%M") if repo.get("createdAt") else "",
                            "Image Count": repo.get("imageTagMutability", ""),
                            "Scan on Push": "Enabled" if repo.get("imageScanningConfiguration", {}).get("scanOnPush") else "Disabled",
...
@click.option(
    "--output-file",
    help="Output file for images list (supports .json, .yaml, .csv)"
)
@click.pass_context
def list_images(
    ctx: click.Context,
                ...
                for image in page.get("imageDetails", []):
                    ...
                    for tag in tags:
                        all_images.append({
                            ...
                            "Pushed": image.get("imagePushedAt", "").strftime("%Y-%m-%d %H:%M") if image.get("imagePushedAt") else "",
                            "Scan Status": image.get("imageScanFindingsSummary", {}).get("findingCounts", {}).get("CRITICAL", 0) if image.get("imageScanFindingsSummary") else "Not Scanned",
...
@click.option(
    "--region",
    help="AWS region to create repository in (default: current region)"
)
@click.option(
    "--scan-on-push",
    is_flag=True,
    help="Enable image scanning on push"
)
@click.option(
    ...
    try:
        ...
        try:
            ...
            repo_details = {
                ...
                "Region": target_region,
                "Created": repository.get("createdAt", "").strftime("%Y-%m-%d %H:%M:%S") if repository.get("createdAt") else "",
...
@click.option(
    "--force",
    is_flag=True,
    help="Force delete repository even if it contains images"
)
@click.option(
    "--confirm",
    is_flag=True,
...
def delete_repository(
    ...
    try:
        ...
        console.print(f"  Created: {repository.get('createdAt', '').strftime('%Y-%m-%d %H:%M:%S') if repository.get('createdAt') else ''}")
...
@click.option(
    "--region",
    help="AWS region for ECR login (default: current region)"
)
@click.option(
    "--print-command",
    is_flag=True,
