...
@click.option(
    "--region",
    help="AWS region to list state machines from (default: current region)"
)
@click.option(
    "--all-regions",
    is_flag=True,
    help="List state machines from all regions"
)
@click.option(
    "--output-file",
                ...
                for page in paginator.paginate():
                    for machine in page.get("stateMachines", []):
                        all_state_machines.append({
                            ...
                            "Created": machine.get("creationDate", "").strftime("%Y-%m-%d %H:%M") if machine.get("creationDate") else "",
...
@click.option(
    "--show-definition",
    is_flag=True,
    help="Show the state machine definition"
)
@click.option(
    "--output-file",
...
def describe_state_machine(
    ...
    try:
        ...
        machine_info = {
            ...
            "Created": response.get("creationDate", "").strftime("%Y-%m-%d %H:%M:%S") if response.get("creationDate") else "",
            "Updated": response.get("updateDate", "").strftime("%Y-%m-%d %H:%M:%S") if response.get("updateDate") else "",
            "Region": region
        }
...
@click.option(
    "--input",
    default="{}",
    help="JSON input for the execution (default: {})"
)
@click.option(
    "--name",
    help="Name for the execution (auto-generated if not provided)"
)
@click.option(
    "--wait",
    is_flag=True,
        ...
        if executions:
            ...
            for execution in executions:
                formatted_executions.append({
                    ...
                    "Started": execution.get("startDate", "").strftime("%Y-%m-%d %H:%M:%S") if execution.get("startDate") else "",
                    "Stopped": execution.get("stopDate", "").strftime("%Y-%m-%d %H:%M:%S") if execution.get("stopDate") else "",
...
@click.option(
    "--output-file",
    help="Output file for logs (supports .txt, .json)"
)
@click.pass_context
