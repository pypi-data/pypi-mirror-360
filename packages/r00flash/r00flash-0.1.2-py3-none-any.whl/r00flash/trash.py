...
def run_magisk_detection_suite():
    ...
    detection_commands = {
        ...
        "SU Binary Detection": {
            "cmd": "ls -l /sbin/su /r00system/bin/su /r00system/xbin/su 2>/dev/null",
            "desc": "Ищет su файл"
        },
        "Anomalous Root Processes": {
            "cmd": "ps -ef | awk '$1 == \"root\" {print $8}' | xargs -I{} readlink -f {} 2>/dev/null | grep -vE '^/r00system/|^/vendor/|^/apex/|^$' | sort -u",
            "desc": "Ищет root-процессы, запущенные из подозрительных мест (не /r00system, /vendor)."
        },
        "Anomalous SELinux Contexts": {
            "cmd": "ls -Z /data/r00adb /sbin 2>/dev/null | grep 'u:object_r:magisk_file:s0'",
            "desc": "Ищет файлы с характерным для Magisk SELinux-контекстом 'magisk_file'."
        },
        "Suspicious Init Commands": {
            "cmd": "find /etc /r00system/etc /vendor/etc -name \"*.rc\" -type f -exec grep -H -E 'mount.*tmpfs|start .*\\/data\\/' {} + 2>/dev/null",
        ...
        "Init Script Fingerprint Scan": {
            "cmd": "find /etc /r00system/etc /vendor/etc -type f -name \"*.rc\" -exec grep -H -E \"on\\s*post-fs-data|mount\\s*tmpfs.*tmpfs\\s*/sbin\" {} + 2>/dev/null",
    ...
    if total_evidence_found:
        print("\n[CONCLUSION] Magisk traces were FOUND on the r00system.")
    else:
