    ...
    def llm_invoke(self, request: LLMRequest):
        ...
        for url in all_urls:
            ...
            try:
                ...
                logger.info(f"llm_invoke response, rid: {rid}, url = {url} , filtered_params: {filtered_params} , message: {message_str} ,  response: {response}")
                if response.choices:
                    ...
                    if is_valid:
                        ...
                        res_len = len(res_str)
                        statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "success", elapsed_time_ms, url, rid, model, filtered_params, f"[{message_len}]{message_str}", f"[{res_len}]{res_str}")
                        return res
                    else:
                        statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "fail", "response.choices response invalid", url, rid, model, filtered_params, f"[{message_len}]{message_str}")
                else:
                    statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "fail", "response.choices empty", url, rid, model, filtered_params, f"[{message_len}]{message_str}")
            except Exception as e:
                statis_log("normal", "common_api", "default", "llm_client", "invoke_" + url, "exception", e, url, rid, model, filtered_params, f"[{message_len}]{message_str}")
        ...
        for url in all_urls:
            try:
                ...
                async def process_stream():
                    ...
                    async for _item in llm_util.handle_stream_output(rid, stream_response, request_increase_mode):
                        yield _item
                ...
                try:
                    ...
                    if first_valid_chunk_received:
                        ...
                        statis_log("normal", "stream_api", "default", "llm_client", "stream_" + url, "success",
                                   elapsed_time_ms, first_token_time, url, rid, model, filtered_params, message_str, message_len)
                        return
                except Exception as e:
                    if first_valid_chunk_received:
                        statis_log("normal", "stream_api", "default", "llm_client", "stream_" + url, "exception", e, "first valid chunk already received,and received exception, will not use backup llm service",
                                   url, all_urls, rid, model, filtered_params, message_str, message_len)
                        return
                    else:
                ...
                statis_log("normal", "stream_api", "default", "llm_client", "stream_" + url, "exception", e, url, rid, message_str, message_len)
                error_msg = f"服务URL {url} 调用失败: {str(e)}"
                logger.error(f"call llm exception, rid={rid}, url={url}, {error_msg}", exc_info=True)
                errors.append(error_msg)
