...
static int warn_invalid_escape_sequence(Parser *p,
  ...
  if ((t->type == FSTRING_MIDDLE || t->type == FSTRING_END) &&
      (c == '{' ||
       c == '}')) { // in this case the tokenizer has already emitted a warning,
  ...
  int octal = ('4' <= c && c <= '7');
  PyObject *msg =
      octal ? PyUnicode_FromFormat("invalid octal escape sequence '\\%.3s'",
  ...
  if (PyErr_WarnExplicitObject(category, msg, p->tok->filename, t->lineno, NULL,
                               NULL) < 0) {
...
static PyObject *decode_utf8(const char **sPtr, const char *end) {
  const char *s;
  const char *t;
  t = s = *sPtr;
  while (s < end && (*s & 0x80)) {
...
static PyObject *decode_unicode_with_escapes(Parser *parser, const char *s,
                                             size_t len, Token *t) {
  PyObject *v;
  PyObject *u;
  char *buf;
  ...
  // tokens when we are decoding the string but we preserve the line numbers.
...
static PyObject *decode_bytes_with_escapes(Parser *p, const char *s,
...
static PyObject *_PyPegen_decode_string(Parser *p, int raw, const char *s,
                                        size_t len, Token *t) {
  if (raw) {
...
/* s must include the bracketing quote characters, and r, b &/or f prefixes
    (if any), and embedded escape sequences (if any). (f-strings are handled by
   the parser) _PyPegen_parse_string parses it, and returns the decoded Python
   string object. */
static PyObject *_PyPegen_parse_string(Parser *p, Token *t) {
  ...
  if (bytesmode) {
    ...
    for (ch = s; *ch; ch++) {
      if (Py_CHARMASK(*ch) >= 0x80) {
        RAISE_SYNTAX_ERROR_KNOWN_LOCATION(t, "bytes can only contain ASCII "
                                             "literal characters");
        return NULL;
      }
    }
