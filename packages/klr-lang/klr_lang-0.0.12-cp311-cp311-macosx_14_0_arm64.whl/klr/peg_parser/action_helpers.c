...
static void *_PyPegen_dummy_name(Parser *p, ...) {
  return &p->dummy_name;
}
...
#define EXTRA_EXPR(head, tail)                                                 \
  head->lineno, (head)->col_offset, (tail)->end_lineno,                        \
      (tail)->end_col_offset, p->arena
#define EXTRA                                                                  \
  _start_lineno, _start_col_offset, _end_lineno, _end_col_offset, p->arena

// action_helpers.c
static PyObject *_PyPegen_new_type_comment(Parser *, const char *);

static inline PyObject *NEW_TYPE_COMMENT(Parser *p, Token *tc) {
  if (tc == NULL) {
    return NULL;
  }
...
static inline void *INVALID_VERSION_CHECK(Parser *p, int version, char *msg,
                                          void *node) {
  if (node == NULL) {
    p->error_indicator = 1; // Inline CHECK_CALL
    return NULL;
...
static asdl_seq *_PyPegen_singleton_seq(Parser *p, void *a) {
  assert(a != NULL);
...
static asdl_seq *_PyPegen_seq_insert_in_front(Parser *p, void *a,
                                              asdl_seq *seq) {
  assert(a != NULL);
  if (!seq) {
...
static asdl_seq *_PyPegen_seq_flatten(Parser *p, asdl_seq *seqs) {
...
static void *_PyPegen_seq_last_item(asdl_seq *seq) {
...
static void *_PyPegen_seq_first_item(asdl_seq *seq) {
...
static expr_ty _PyPegen_join_names_with_dot(Parser *p, expr_ty first_name,
                                            expr_ty second_name) {
  ...
  *s = '\0';
  ...
  // PyUnicode_InternMortal(interp, &uni);
...
static int _PyPegen_seq_count_dots(asdl_seq *seq) {
...
static alias_ty _PyPegen_alias_for_star(Parser *p, int lineno, int col_offset,
...
static asdl_identifier_seq *_PyPegen_map_names_to_ids(Parser *p,
                                                      asdl_expr_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  assert(len > 0);
...
static CmpopExprPair *_PyPegen_cmpop_expr_pair(Parser *p, cmpop_ty cmpop,
                                               expr_ty expr) {
  assert(expr != NULL);
...
static asdl_int_seq *_PyPegen_get_cmpops(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  assert(len > 0);
...
static asdl_expr_seq *_PyPegen_get_exprs(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  assert(len > 0);
...
static expr_ty _PyPegen_set_expr_context(Parser *p, expr_ty expr,
                                         expr_context_ty ctx);
 ...
 */
static KeyValuePair *_PyPegen_key_value_pair(Parser *p, expr_ty key,
                                             expr_ty value) {
...
static asdl_expr_seq *_PyPegen_get_keys(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  asdl_expr_seq *new_seq = _Py_asdl_expr_seq_new(len, p->arena);
  if (!new_seq) {
    return NULL;
...
static asdl_expr_seq *_PyPegen_get_values(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  asdl_expr_seq *new_seq = _Py_asdl_expr_seq_new(len, p->arena);
  if (!new_seq) {
    return NULL;
 ...
 * patterns */
static KeyPatternPair *_PyPegen_key_pattern_pair(Parser *p, expr_ty key,
                                                 pattern_ty pattern) {
...
static asdl_expr_seq *_PyPegen_get_pattern_keys(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
  asdl_expr_seq *new_seq = _Py_asdl_expr_seq_new(len, p->arena);
  if (!new_seq) {
    return NULL;
...
static asdl_pattern_seq *_PyPegen_get_patterns(Parser *p, asdl_seq *seq) {
  Py_ssize_t len = asdl_seq_LEN(seq);
...
static arg_ty _PyPegen_add_type_comment_to_arg(Parser *p, arg_ty a, Token *tc);

/* Constructs a NameDefaultPair */
static NameDefaultPair *_PyPegen_name_default_pair(Parser *p, arg_ty arg,
...
_PyPegen_slash_with_default(Parser *p, asdl_arg_seq *plain_names,
...
static StarEtc *_PyPegen_star_etc(Parser *p, arg_ty vararg,
                                  asdl_seq *kwonlyargs, arg_ty kwarg) {
...
static asdl_seq *_PyPegen_join_sequences(Parser *p, asdl_seq *a, asdl_seq *b) {
...
_PyPegen_make_arguments(Parser *p, asdl_arg_seq *slash_without_default,
                        SlashWithDefault *slash_with_default,
                        asdl_arg_seq *plain_names, asdl_seq *names_with_default,
                        StarEtc *star_etc) {
  asdl_arg_seq *posonlyargs;
...
static arguments_ty _PyPegen_empty_arguments(Parser *p) {
...
static AugOperator *_PyPegen_augoperator(Parser *p, operator_ty kind) {
...
static stmt_ty _PyPegen_function_def_decorators(Parser *p,
...
static stmt_ty _PyPegen_class_def_decorators(Parser *p,
                                             asdl_expr_seq *decorators,
                                             stmt_ty class_def) {
...
static KeywordOrStarred *_PyPegen_keyword_or_starred(Parser *p, void *element,
                                                     int is_keyword) {
...
static asdl_expr_seq *_PyPegen_seq_extract_starred_exprs(Parser *p,
                                                         asdl_seq *kwargs) {
...
static asdl_keyword_seq *_PyPegen_seq_delete_starred_exprs(Parser *p,
                                                           asdl_seq *kwargs) {
...
static expr_ty _PyPegen_ensure_imaginary(Parser *p, expr_ty exp) {
...
static expr_ty _PyPegen_ensure_real(Parser *p, expr_ty exp) {
  if (exp->kind != Constant_kind ||
      PyComplex_CheckExact(exp->v.Constant.value)) {
    RAISE_SYNTAX_ERROR_KNOWN_LOCATION(
...
static PyObject *_PyPegen_new_type_comment(Parser *p, const char *s) {
...
static int _PyPegen_check_barry_as_flufl(Parser *p, Token *t) {
  assert(t->bytes != NULL);
...
static int _PyPegen_check_legacy_stmt(Parser *p, expr_ty name) {
  (void)p;
...
static expr_ty _PyPegen_collect_call_seqs(Parser *p, asdl_expr_seq *a,
                                          asdl_seq *b, int lineno,
...
static void *_PyPegen_arguments_parsing_error(Parser *p, expr_ty e) {
...
static expr_ty _PyPegen_joined_str(Parser *p, Token *a, asdl_expr_seq *expr,
                                   Token *b) {
  ...
  for (Py_ssize_t i = 0; i < n_items; i++) {
    ...
    // created _PyPegen_formatted_value. This situation can only be the result
    // of a f-string debug expression where the first element is a constant with
    // the text and the second a formatted value with the expression.
    if (item->kind == JoinedStr_kind) {
      asdl_expr_seq *values = item->v.JoinedStr.values;
      if (asdl_seq_LEN(values) != 2) {
        PyErr_Format(PyExc_SystemError,
                     "unexpected JoinedStr node without debug data in f-string "
                     "at line %d",
                     item->lineno);
        return NULL;
      }
...
static expr_ty _PyPegen_decoded_constant_from_token(Parser *p, Token *tok) {
  Py_ssize_t bsize;
  char *bstr;
...
static expr_ty _PyPegen_constant_from_token(Parser *p, Token *tok) {
...
static expr_ty _PyPegen_constant_from_string(Parser *p, Token *tok) {
...
static expr_ty _PyPegen_formatted_value(Parser *p, expr_ty expression,
                                        Token *debug,
...
static expr_ty _PyPegen_concatenate_strings(Parser *p, asdl_expr_seq *strings,
...
static stmt_ty _PyPegen_checked_future_import(
    Parser *p, identifier module, asdl_alias_seq *names, int level, int lineno,
    int col_offset, int end_lineno, int end_col_offset, PyArena *arena) {
  if (level == 0 &&
      PyUnicode_CompareWithASCIIString(module, "__future__") == 0) {
    for (Py_ssize_t i = 0; i < asdl_seq_LEN(names); i++) {
      alias_ty alias = asdl_seq_GET(names, i);
      if (PyUnicode_CompareWithASCIIString(alias->name, "barry_as_FLUFL") ==
          0) {
