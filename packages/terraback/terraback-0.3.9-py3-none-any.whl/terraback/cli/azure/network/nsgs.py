...
def scan_nsgs(
    ...
    if with_deps:
        ...
        output_file = output_dir / "azure_network_security_group.tf"
        generate_tf(nsg_data, "azure_network_security_group", output_file, provider="azure")
        typer.echo(f"Generated Terraform for {len(nsg_data)} NSGs -> {output_file}")
