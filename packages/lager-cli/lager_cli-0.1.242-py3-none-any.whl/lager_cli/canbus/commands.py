...
@click.group(name='canbus', hidden=True)
def canbus():
    """
...
def up(ctx, interfaces, gateway, dut, bitrate):
    """
...
@click.option('--gateway', required=False, help='ID of gateway to which DUT is connected', hidden=True)
@click.option('--dut', required=False, help='ID of DUT')
def down(ctx, interfaces, gateway, dut):
    """
...
def send(ctx, gateway, interface, can_frames, dut):
    """
...
def dump(ctx, gateway, dut, interface, filters):
    """
