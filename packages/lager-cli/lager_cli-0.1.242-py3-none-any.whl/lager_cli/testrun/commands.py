...
def testrun(ctx, gateway, serial_device, baudrate, bytesize, parity, stopbits, xonxoff, rtscts,
            dsrdtr, test_runner, interactive, message_timeout, overall_timeout, hexfile, binfile, elffile,
            preverify, verify, display_job_id, success_regex, failure_regex, mcu):
    """
    ...
    resp = do_reset(session, gateway, halt=True, mcu=mcu)
    stream_output(resp)
    ...
    resp = do_flash(session, gateway, hexfile, binfile, elffile, preverify, verify, run=False, mcu=mcu)
    stream_output(resp)

    resp = do_reset(ctx.obj.session, gateway, halt=False, mcu=mcu)
    stream_output(resp)
