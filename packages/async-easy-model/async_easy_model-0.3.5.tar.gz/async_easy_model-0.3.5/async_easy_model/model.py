    ...
    async def refresh_metadata(self):
        ...
        try:
            ...
            async with engine.begin() as conn:
                ...
                # Force refresh of all registered models
                for model_cls in SQLModel.registry.mappers:
                    if hasattr(model_cls.class_, '__table__'):
                        # Refresh table metadata
                        table = model_cls.class_.__table__
                        await conn.run_sync(lambda sync_conn: table.metadata.reflect(sync_conn, only=[table.name]))
                        
                logging.info("Metadata refreshed successfully")
                return True
    ...
    async def refresh_junction_table_metadata(self, table_name: str):
        ...
        try:
            ...
            async with engine.begin() as conn:
                ...
                # Reflect the specific table to refresh its metadata
                from sqlalchemy import Table, MetaData
                metadata = MetaData()
                table = Table(table_name, metadata, autoload_with=engine.sync_engine)
                
                # Force refresh by clearing and reflecting
                if table_name in SQLModel.metadata.tables:
                    SQLModel.metadata.tables[table_name] = table
                    
                logging.info(f"Refreshed metadata for junction table: {table_name}")
                return True
...
class EasyModel(SQLModel):
    ...
    async def get_session(cls):
        ...
        - Validating connection before creating session
        ...
        """
        # Validate connection and refresh metadata if needed
