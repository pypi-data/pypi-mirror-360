...
def generate_xml(maze_array: list) -> (dict, str):
    ...
    # Modify the position of the global camera, that is supposed to view the entire maze.
    # Its position then depends on the width of the environment.
    camera = world_body_node.find(".//camera[@name='env_view']")  # Find camera by name
    if camera is not None:
        new_z_position = max(maze_array.shape[:2])
        camera.set('pos', '0 0 ' + str(new_z_position))  # Update position as an example
        # camera.set('xyaxes', '1 0 0 0 0 -1')  # Flip the camera orientation to make it right-side up

        # # 4. Save the modified XML back to a new file
        # tree.write("modified_model.xml", pretty_print=True)
    else:
        print("Camera not found in the XML.")
