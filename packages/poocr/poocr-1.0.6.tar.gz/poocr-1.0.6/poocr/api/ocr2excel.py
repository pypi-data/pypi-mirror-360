...
from poocr.lib.CommonUtils import extract_all_fields


##### 调用poocr的原生接口 #####

def CustomsOCR2Excel(input_path, output_path=None, output_excel='RecognizeGeneralInvoiceOCR2Excel.xlsx',
    ...
    if img_paths is None or len(img_paths) == 0:
        raise BaseException(f'{input_path}目录下,无报关单,请检查后重新运行.')
        ...
        for item in simple_progress(img_paths):
            ...
            for invoice_item in invoice_items:
                ...
                if invoice_item['Code'] == 'OK':
                    ...
                    extract_all_fields(single_invoice_info[invoice_item['SubType']], invoice_info)
    ...
    for vat_img in simple_progress(vat_img_files):
        ...
        if isinstance(api_res, list):
            ...
            for pdf_page, page_res in enumerate(api_res):
                one_file_results = VatInvoiceOCR2(page_res, vat_img, file_name, trans, pdf_page + 1)
                all_file_results.extend(copy.deepcopy(one_file_results))
        else:
            one_file_results = VatInvoiceOCR2(api_res, vat_img, file_name, trans)
            all_file_results.extend(copy.deepcopy(one_file_results))
    # 整理全部识别结果
    if len(all_file_results) > 0:
...
def VatInvoiceOCR2(api_res, vat_img, file_name, trans, pdf_page=1):
    '''
    处理单个结果
    Args:
        api_res: 识别结果
    ...
    try:
        ...
        if 'VatInvoiceInfos' in api_res_json:
            ...
            logger.info(f"成功处理文件: {vat_img} 第 {pdf_page} 页,res_df:{len(res_df)}")
            return res_df
