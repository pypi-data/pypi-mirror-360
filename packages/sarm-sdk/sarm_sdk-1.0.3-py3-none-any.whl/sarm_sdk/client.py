...
class SARMClient:
    ...
    def _init_apis(self):
        """初始化所有API模块"""
        # 原有API
        self.organizations = OrganizationAPI(self.base_url, self.token)
        self.carriers = CarrierAPI(self.base_url, self.token)
        self.security_capabilities = SecurityCapabilityAPI(self.base_url, self.token)
        self.issue_data_import = IssueDataImportAPI(self.base_url, self.token)
        
        # 恢复的API
        self.vulnerabilities = VulnerabilityAPI(self.base_url, self.token)
        self.security_issues = SecurityIssueAPI(self.base_url, self.token)
        self.components = ComponentAPI(self.base_url, self.token)
        
        # 新增的API
        self.business_systems = BusinessSystemAPI(self.base_url, self.token)
        self.applications = ApplicationAPI(self.base_url, self.token)
        self.organize_users = OrganizeUserAPI(self.base_url, self.token)
