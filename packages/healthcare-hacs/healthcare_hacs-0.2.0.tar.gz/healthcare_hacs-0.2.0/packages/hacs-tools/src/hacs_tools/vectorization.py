...
class VectorMetadata(BaseModel):
    ...
    actor_id: Optional[str] = Field(
        None, description="ID of the actor who created this"
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))

    # Content metadata
    content_length: Optional[int] = Field(
        None, description="Length of original content"
    )
    importance_score: Optional[float] = Field(
        None, description="Importance score (0-1)"
    )
...
class VectorStore(ABC):
    ...
    def get_vector(
        self, vector_id: str
    ) -> Optional[Tuple[List[float], VectorMetadata]]:
...
class HACSVectorizer:
    ...
    def search_memories(
        self, query: str, limit: int = 10, actor: Optional[Actor] = None
    ) -> List[Tuple[str, float, VectorMetadata]]:
