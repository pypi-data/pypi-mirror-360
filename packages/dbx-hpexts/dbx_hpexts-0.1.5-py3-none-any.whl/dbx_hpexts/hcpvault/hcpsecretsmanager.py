...
class HCPSecretsMngr:
    ...
    @classmethod
    def __get_spark_conf(cls, conf_key, default:str = None)->str:
    ...
    @classmethod
    def __resolve_scope_name(cls, scope_name:str= None, dbutils:DBUtils=None)->str:
        _scope_name = cls.__get_spark_conf(cls.secrets_scope_key)
        if _scope_name is None and dbutils is not None:
            _scope_name = dbutils.widgets.get(cls.secrets_scope_key)
        scope_name = scope_name if _scope_name is None else _scope_name
        return scope_name
    
    
    @classmethod
    def __resolve_secrets_map(cls, secrets_map:str|dict=None, dbutils:DBUtils=None)->dict:
        required_keys:list = [cls.client_id_key, cls.client_secret_key, cls.org_id_key, cls.project_id_key, cls.app_id_key]
        f_secrets_map:dict = None
        _secrets_map = cls.__get_spark_conf(cls.secrets_map_key)
        if _secrets_map is None and dbutils is not None:
            _secrets_map = dbutils.widgets.get(cls.secrets_map_key)
        ...
        if isinstance(secrets_map, dict):
            if set(required_keys) not in set([str(key).lower() for key in secrets_map.keys()]):
                raise Exception(f"U are not providing all the required keys for {cls.secrets_map_key}. required_keys: {set(required_keys)}")
            f_secrets_map = { value: str(key).lower() for key, value in secrets_map.items() if key in required_keys }
        return f_secrets_map
    
    
    @classmethod
    def get_from_secrets(cls, scope_name:str=None, secrets_map:str|dict=None, secret_uri:str=None, refresh_timeout_min:int=None, clear_refresh:bool=False, dbutils:DBUtils=None)->dict:
        """
        scope_name: what secrets is scope contains the secrets when U have hcp configuration. 
                    if U set secrets_scope from base_parameters or configuration in your bundle yaml that value has more relevancy.
        secrets_map: mapping for hcp conf e.g. CLIENT_ID:HCP_CLIENT_ID;CLIENT_SECRET:HCP_CLIENT_SECRET;APP_ID:HCP_APP_ID;ORG_ID:HCP_ORG_ID;PROJECT_ID:HCP_PROJECT_ID
                    if U set hpc_secrets_map from base_parameters or configuration in your bundle yaml that value has more relevancy.
        """
        scope_name = cls.__resolve_scope_name(scope_name, dbutils)
        if scope_name is None:
            raise Exception(f"{cls.secrets_scope_key} can't be resolved, please provide secrets scope.")
        secrets_map = cls.__resolve_secrets_map(secrets_map, dbutils)
        if secrets_map is None:
            raise Exception(f"{cls.secrets_map_key} can't be resolved, please provide hcp secrets map.")

        params = {cls.clear_refresh_key: clear_refresh, cls.secret_uri_key: secret_uri, cls.refresh_timeout_min_key: refresh_timeout_min}
        for secret_key, hcp_key in secrets_map.items():
            params[hcp_key] = dbutils.secrets.get(scope=scope_name, key=secret_key)
