...
from synth_env.examples.sokoban.engine_helpers.vendored.envs.room_utils import generate_room
import numpy as np
...
class SokobanEngine(StatefulEngine, IReproducibleEngine):
    ...
    def __init__(self, task_instance: TaskInstance):
        ...
        if init_snap:
            ...
            # Using defaults that are more likely to succeed with gym_sokoban's room generation
            default_dim_room = (10, 10)  # Even larger room to avoid generation issues
            default_max_steps = 120
            default_num_boxes = 1  # Fewer boxes for easier generation
            ...
            # Try to create the environment with retry logic for room generation failures
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    self.package_sokoban_env = GymSokobanEnv(
                        ...
                        reset=True,  # Let it try to reset during creation
                    )
                    break  # Success!
                except (RecursionError, RuntimeError, RuntimeWarning) as e:
                    if attempt == max_retries - 1:
                        # Last attempt failed, raise a more informative error
                        raise RuntimeError(
                            f"Failed to create Sokoban environment after {max_retries} attempts. "
                            f"Room generation failed with parameters: dim_room={dim_param}, "
                            f"num_boxes={num_boxes_param}. Original error: {e}"
                        ) from e
                    # Try with different parameters on retry
                    if attempt == 0:
                        # First retry: try smaller box count
                        num_boxes_param = max(1, num_boxes_param - 1)
                    elif attempt == 1:
                        # Second retry: try larger room
                        dim_param = (dim_param[0] + 2, dim_param[1] + 2)
    ...
    async def _reset_engine(
        ...
        if init_snap:
            ...
            # Reset the existing environment with the seed
            # Since we already have a package_sokoban_env from __init__, just reset it
            try:
                ...
                # If reset fails, recreate the environment with retry logic
                # Use the same parameters as the constructor
                default_dim_room = (7, 7)
                default_max_steps = 120
                default_num_boxes = 2
                ...
                # Try to create a new environment with retry logic
                max_retries = 3
                for attempt in range(max_retries):
                    try:
                        self.package_sokoban_env = GymSokobanEnv(
                            ...
                            max_steps=max_steps_param,
                            num_boxes=num_boxes_param,
                            reset=False,  # Use manual reset to avoid recursion bug
                        )
                        # Manually reset after creation
                        self.package_sokoban_env.reset()
                        # Update the boxes_on_target counter
                        self.package_sokoban_env.boxes_on_target = _count_boxes_on_target(
                            self.package_sokoban_env.room_state
                        )
                        break  # Success!
                    except (RecursionError, RuntimeError, RuntimeWarning) as retry_e:
                        if attempt == max_retries - 1:
                            # Last attempt failed, raise a more informative error
                            raise RuntimeError(
                                f"Failed to create Sokoban environment after {max_retries} attempts during reset. "
                                f"Room generation failed with parameters: dim_room={dim_param}, "
                                f"num_boxes={num_boxes_param}. Original error: {retry_e}"
                            ) from retry_e
                        # Try with different parameters on retry
                        if attempt == 0:
                            # First retry: try smaller box count
                ...
                # Try to create the environment with retry logic
                max_retries = 3
                for attempt in range(max_retries):
                    try:
                        self.package_sokoban_env = GymSokobanEnv(
