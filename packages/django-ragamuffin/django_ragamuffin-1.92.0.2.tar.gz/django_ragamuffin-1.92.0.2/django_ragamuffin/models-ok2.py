...
class OpenAIClient( OpenAI ):
    ...
    def get_or_create_remote_vector_store_info(self, vs , old_checksum=None,old_file_ids=[]):
        ...
        if remote_vector_stores :
            ...
            if remote_vector_stores :
                print(f"OLD CHECKSUM VS {old_checksum} EXISTS")
                remote_vector_store = remote_vector_stores[0]
                print(f"REMOTE_VECTOR_STORE = {remote_vector_store}")
...
def handle_files_changed(sender, instance, action, reverse, model, pk_set, **kwargs):
    ...
    if action in {"pre_add", "pre_remove", "pre_clear"}:
        ...
        instance._updating_from_m2m = True
        instance.vsid = client.get_or_create_remote_vector_store_info( instance , old_checksum,old_file_ids)
