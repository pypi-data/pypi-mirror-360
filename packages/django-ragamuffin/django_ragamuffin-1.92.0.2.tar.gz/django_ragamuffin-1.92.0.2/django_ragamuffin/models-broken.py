...
class OpenAIClient( OpenAI ):
    ...
    def remote_update_vector_store(self, vs , old_checksum=None,old_file_ids=[]):
        ...
        if remote_vector_stores :
            print(f"NEW_CHECKSUM STORE WITH {new_checksum} EXISTS")
            ...
            if remote_vector_stores :
                ...
                self.vector_stores.file_batches.create( vector_store_id=vector_store_id, file_ids=new_file_ids)
                ...
                vs.save();
        print(f"FINISEHD VS CHECKSUM = {vs.checksum}")
    ...
    if hasattr( instance, "file_ids") :
        ...
        for file_id in file_ids :
            ...
            for vs in vst :
                ...
                vsid =  client.remote_update_vector_store( vs )
                vs.vsid = vsid
                vs.save()
...
class VectorStore( models.Model ):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            try:
                vector_store_id = client.remote_update_vector_store(self)
                self.vsid = vector_store_id
            except openai.OpenAIError as e:
                logger.error(f"OpenAIError in remote_update_vector_store: {e}")
                raise
            except Exception as e:
                logger.error(f"Unexpected error in remote_update_vector_store: {e}")
                raise
        super().save(*args, **kwargs)
...
def handle_files_changed(sender, instance, action, reverse, model, pk_set, **kwargs):
    ...
    if action in {"pre_add", "pre_remove", "pre_clear"}:
        ...
        instance._updating_from_m2m = True
        instance.vsid = client.remote_update_vector_store( instance , old_checksum,old_file_ids)
