...
class ModelTrainer:
    def __init__(self, model = None, columns_to_log: Optional[List[str]] = None):
        """Инициализация тренера модели.
        
        Args:
            model: Модель для обучения (по умолчанию RandomForest)
            columns_to_log: Список признаков для логарифмирования
        """
        self.model = model if model else build_model()
        self.columns_to_log = columns_to_log if columns_to_log else [
            'shape_index', 'mbr_area', 'mbr_aspect_ratio', 'solidity',
            'asymmetry_x', 'asymmetry_y'
        ]
        self.feature_cols = None
        self.target_col = None

    def train(self, data: pd.DataFrame, target_col: str = 'land_use_code') -> None:
        """Обучение модели.
        ...
        excluded_columns = ['geometry', 'land_use', 'land_use_code', 'city', 'city_center', target_col]
        excluded_columns += self.columns_to_log  
        self.feature_cols = [c for c in data.columns if c not in excluded_columns]
        
        logger.info(f"Используется {len(self.feature_cols)} признаков: {self.feature_cols}")
    ...
    def predict(self, X_new: pd.DataFrame) -> np.ndarray:
        ...
        Args:
            X_new: Новые данные для предсказания
        ...
        logger.info(f"Предсказание для {len(X_new)} объектов")
    ...
    def predict_proba(self, X_new: pd.DataFrame) -> np.ndarray:
        ...
        logger.info(f"Предсказание для {len(X_new)} объектов")
        return self.model.predict_proba(X_new[self.feature_cols])
...
class SpatialClassifier:
    ...
    def fit(self, train_gdf: gpd.GeoDataFrame, target_col: str = 'land_use_code') -> None:
        ...
        self.model_trainer.train(processed_train, target_col=target_col)
        self.is_fitted = True
        self.class_names_ = self.model_trainer.model.classes_
    ...
    def predict(self, new_gdf: gpd.GeoDataFrame) -> np.ndarray:
        ...
        return self.model_trainer.predict(processed_new)

    def predict_proba(self, new_gdf: gpd.GeoDataFrame) -> np.ndarray:
        """Вероятности классов для новых данных."""
        processed_new = self._prepare_for_prediction(new_gdf)
        return self.model_trainer.predict_proba(processed_new)
