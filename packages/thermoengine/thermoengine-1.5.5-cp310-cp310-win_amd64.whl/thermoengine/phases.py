...
__all__ = ['Rxn','Assemblage','Phase','PurePhase','SolutionPhase',
           'PhaseCalculator','get_phaselist']
...
class Phase(metaclass=ABCMeta):
    ...
    * The pure water phase, "H2O", is very complex and thus not available for
    ...
    # _MINVAL = np.finfo(float).eps

    def __init__(self, abbrev:str, phs_calc:'PhaseCalculator'):
        if abbrev=='H2O':
            calib=False
        
        self._calculator = phs_calc

        # Detect if calib is true or false from phs_calc
        calib = True

        self._calib = calib
        self.deriv_keys = {'dT','dP','dV','dS','dmol'}

        phase_classnm = phs_calc._method_base_name
    ...
    def _init_phase_props(self, phase_classnm, abbrev):
        # NOTE: props dictionary to be phased out
        ...
        props = OrderedDict()
        props['abbrev'] = abbrev
        # props['phase_name'] = phase_classnm
        props['phase_name'] = self.phase_name
    ...
    def _init_param_props(self):
        # TODO: add this back when we have calib detection
        # if 'calib_' in self._phase_obj:
        #     supports_calib = True
        #     method_stub = self._phase_obj.replace('calib_', '')
        #     method = getattr(self.calculator, method_stub+'get_param_number')
        #     param_num = method()
        #     method = getattr(self.calculator, method_stub+'get_param_names')
        #     param_names = method()
        #     method = getattr(self.calculator, method_stub+'get_param_units')
        #     param_units = method()
        #     method = getattr(self.calculator, method_stub+'get_param_values')
        #     param0 = method()
        # else:
        supports_calib = False
        param_num = 0
        param_names = np.array(self.calculator.get_param_names())
    ...
    def phase_name(self) -> str:
        ...
        return self._calculator.phase_name
    ...
    @property
    def formula(self) -> NDArray[np.str_]:
        """
        ...
        return self._calculator.endmember_formulas
    
    @property
    def endmember_formulas(self) -> NDArray[np.str_]:
        """
        Array of formulas for each endmember of phase
    ...
    @property
    def endmember_molwts(self) -> NDArray[np.float_]:
        """
        ...
        return self._calculator.endmember_molwts

    @property
    def endmember_element_comps(self) -> NDArray[np.int_]:
        """
        ...
        return self._calculator.endmember_element_comps

    @property
    def endmember_number_of_atoms(self) -> NDArray[np.int_]:
        """
        ...
        return self._calculator.endmember_number_of_atoms

    @property
    def endmember_mol_oxide_comps(self) -> NDArray[np.float_]:
        """
        ...
        return self._calculator.endmember_mol_oxide_comps

    @property
    def oxide_space(self) -> NDArray[np.bool_]:
        """
        Array of bools indicating if phase can contain
        each oxide
        ...
        """
        return self._calculator.oxide_space
    ...
    @property
    def endmember_names(self) -> ArrayLike:
        """
        ...
        return self.calculator.endmember_names

    @property
    def endmember_num(self) -> int:
        """
        ...
        return self.calculator.endmember_num
    
    @property
    def species_num(self) -> int:
        """
        ...
        """
        return self.calculator.species_num
    
    @property
    def species_names(self) -> NDArray[np.str_]:
        """
        ...
        """
        return self.calculator.species_names

    @property
    def species_element_comps(self) -> NDArray[np.float_]:
        """
        ...
        """
        return self.calculator.species_element_comps
    ...
    def set_ref_state(self, Tr=298.15, Pr=1.0, Trl=298.15):
        ...
        # pass
        assert False, 'set_ref_state not implemented for coder phases?'
    ...
    def enable_gibbs_energy_reference_state(self):
        ...
        # pass
        assert False, 'enable_gibbs_energy_reference_state not implemented for coder phases?'
    ...
    def disable_gibbs_energy_reference_state(self):
        ...
        # pass
        assert False, 'disable_gibbs_energy_reference_state not implemented for coder phases?'
    ...
    def get_param_values(self, param_names=[], all_params=False):
        ...
        if self.calib:
            # method_stub = self._phase_obj.replace('calib_', '')
            method = getattr(self.calculator, 'get_param_value')
            for ind,key in enumerate(param_names):
                lookup_ind = list(self.param_names).index(key)
                value = method(lookup_ind)
                values_a[ind] = value
        else:
            values_a = None
    ...
    def set_param_values(self, param_names=[], param_values=[]):
        ...
        if self.calib:
            # method_stub = self._phase_obj.replace('calib_', '')
            method = getattr(self.calculator, 'set_param_value')
            for name, value in zip(param_names, param_values):
                lookup_ind = list(self.param_names).index(name)
                method(lookup_ind, value)
    ...
    def affinity_and_comp(self, t, p, mu, **kwargs):
        ...
        # Detect if method defined in kwargs
        # return exch_equil.affinity_and_comp(t, p, mu, method='special', **kwargs)
        if 'method' not in kwargs:
            kwargs['method'] = 'special'
    ...
    def gibbs_energy(self, T, P, mol=None, V=None, deriv=None,
        ...
        if (not deriv) and (not deriv_param):
            ...
            method = getattr(self.calculator, method_sig)
    ...
    def volume(self, T, P, mol=None, V=None, deriv=None,
        ...
        if method:
            ...
            if mol is None:
                ...
                for ind,(iT,iP,imol) in enumerate(zip(T,P,mol)):
                    result.append(method(iT, iP, imol))#, V=V))
            result = np.array(result)
        else:
            result = None
    ...
    def entropy(self, T, P, mol=None, V=None, deriv=None,
        ...
        if (not deriv) and (not deriv_param):
            ...
            nT   = deriv['dT'] + 1 if ('dT' in deriv) else 1
            nP   = deriv['dP'] if ('dP' in deriv) else 0
            nMol = deriv['dmol'] if ('dmol' in deriv) else 0
            nW   = 0 if not deriv_param else 1
            nTot = nT + nP + nMol + nW
            method_sig = f"d{nTot}G" if nTot > 1 else "dG"
            method_sig += f"_dT{nT}" if nT>1 else "_dT"
            method_sig += f"dm{nMol}" if nMol>1 else "dm"
            if nP > 0 or nW > 0:
                assert False, "This block of code needs reimplementing"
            # method_sig = '_calc_'
        ...
        # NOTE the minus signs are for S = - dG/dT
        if method:
            result = []
            if mol is None:
                for ind,(iT,iP) in enumerate(zip(T,P)):
                    result.append(-method(iT, iP))
            else:
                for ind,(iT,iP,imol) in enumerate(zip(T,P,mol)):
                    result.append(-method(iT, iP, imol))
            result = np.array(result)
        else:
            result = None
    ...
    def chem_potential(self, T, P, mol=None, V=None, deriv=None,
        ...
        """
        assert species is False, 'species input not currently implemented'

        if not deriv:
            result = []
            if mol is None:
                for ind,(iT,iP) in enumerate(zip(T,P)):
                    mu = self.calculator.dG_dm(iT, iP)
                    if (endmember is not None):
                        mu = mu[endmember]
                    result.append(mu)
            else:
                for ind,(iT,iP,imol) in enumerate(zip(T,P,mol)):
                    mu = self.calculator.dG_dm(iT, iP, imol)
                    if (endmember > -1):
                        mu = mu[endmember]
                    result.append(mu)
            result = np.array(result)
    ...
    def heat_capacity(self, T, P, mol=None, V=None, const='P', deriv=None,
        ...
        if const=='P':
            if not deriv:
                method = self.calculator.d2G_dT2
                # method = getattr(self, '_calc_Cp')
            elif deriv=={'dT':1}:
                dCpdT = True
                result = None
                # method = self.calculator.d3G_dT3
                # method = getattr(self, '_calc_dCp_dT')
            elif deriv=={'dmol':1}:
                method = self.calculator.d3G_dT2dm
        ...
        if method:
            ...
            if mol is None:
                ...
                for ind,(iT,iP,imol) in enumerate(zip(T,P,mol)):
                    result.append(method(iT, iP, imol))#, V=V))
            result = - T * np.array(result) # minus T to account for calling Gibbs derivs directly
            ...
            if mol is None:
                for ind,(iT,iP) in enumerate(zip(T,P)):
                    result.append(
                        - self.calculator.d2G_dT2(iT, iP)
                        - T * self.calculator.d3G_dT3(iT,iP)
                    )
            else:
                for ind,(iT,iP,imol) in enumerate(zip(T,P,mol)):
                    result.append(
                        - self.calculator.d2G_dT2(iT, iP, imol)
                        - T * self.calculator.d3G_dT3(iT, iP, imol)
                    )
            result = np.array(result)
...
class PurePhase(Phase):
    ...
    * In addition to the attributes listed, this class inherits the Phase class
    ...
    # def __init__(self, phase_classnm, abbrev, calib=True, source='coder',
    #     coder_module=None):
    def __init__(self, abbrev, phs_calc):
        # super().__init__(phase_classnm, abbrev, calib=calib, source=source,
        #     coder_module=coder_module)
        super().__init__(abbrev, phs_calc)
        self._init_pure_phase_props()
        self._phase_type = 'pure'

        self._exchange_equil = ExchangeEquil(self)

    def _init_pure_phase_props(self):
        # phase_obj = self.phase_obj

        # if self.source == 'coder':
        #     method = getattr(self.calculator, phase_classnm+'elements')
        #     element_comp = method()
        ...
        # if self.source == 'coder':
        #     method = getattr(self.calculator, phase_classnm+'formula')
        #     props['formula'] = np.array([method()])
        #     method = getattr(self.calculator, phase_classnm+'mw')
        ...
        endmember_num = self.calculator.endmember_num
        species_num = self.calculator.species_num


        # endmember_props = OrderedDict()
        props['endmember_name'] = self.calculator.endmember_names
        props['endmember_num'] = endmember_num
        props['endmember_id'] = np.arange(endmember_num)
        props['species_num'] = species_num
        props['formula'] = self.calculator.endmember_formulas
        props['atom_num'] = self.calculator.endmember_number_of_atoms
        props['molwt'] = self.calculator.endmember_molwts
        props['elemental_entropy'] = [None]
        props['element_comp'] = self.calculator.endmember_element_comps
        props['mol_oxide_comp'] = self.calculator.endmember_mol_oxide_comps
        props['oxide_space'] = self.calculator.oxide_space
        props['species_name'] = self.calculator.species_names
        props['species_elms'] = self.calculator.species_element_comps
...
class SolutionPhase(Phase):
    ...
    * The pure water phase, "H2O", is very complex and thus not available for
    ...
    """
    def __init__(self, abbrev, phs_calc, XTOL=1e-12):
        super().__init__(abbrev, phs_calc)
        self._XTOL = XTOL
    ...
    def _init_species(self):
        ...
        elem_comp_spec = self.species_element_comps 
        elem_comp_endmem = self.endmember_element_comps
    ...
    def calc_endmember_comp(self, mol_oxide_comp, method='least_squares',
        ...
        * Eventually, we may want the ability to calculate endmember comp. using
        ...
        if method == 'least_squares':
            ...
            endmember_comp = self.calculator.convert_elements_to_moles(moles_elm)
    ...
    def test_endmember_comp(self, mol_comp):
        ...
        """
        result = self.calculator.test_moles(mol_comp)
    ...
    def compute_formula(self, T, P, mol_comp):
        ...
        """
        result = self.calculator.compute_formula(T, P, mol_comp)
        return result
    ...
    def convert_endmember_comp(self, mol_comp, output='total_moles'):
        ...
        if output == 'total_moles':
            return self.calculator.convert_moles_to_total_moles(mol_comp)
        elif output == 'moles_elements':
            return self.calculator.convert_moles_to_elements(mol_comp)
        elif output == 'mole_fraction':
            return self.calculator.convert_moles_to_mole_fractions(mol_comp)
        else:
    ...
    def convert_elements(self, mol_elm, output='moles_end'):
        ...
        phase_classnm = self._phase_cls
        if output == 'moles_end':
            return self.calculator.convert_elements_to_moles(mol_elm)
        elif output == 'total_moles':
            return self.calculator.convert_elements_to_total_moles(mol_elm)
        elif output == 'total_grams':
            return self.calculator.convert_elements_to_total_grams(mol_elm)
        else:
...
class PhaseCalculator():
    def __init__(self, coder_module):
        self._module_name = coder_module
        self._module = import_module(coder_module)

        name_parts = str.split(self._module_name, '.')
        # self._method_base_name = f"cy_{name_parts[0]}.{name_parts[1]}.Liquid_{name_parts[2]}_calib"
        coder_base_name = name_parts[3]
        phs_name = str.split(coder_base_name, '_')[0]
        self._method_base_name = f"cy_{phs_name}_{coder_base_name}_calib"
        self._method_base_name_non_calib = f"cy_{phs_name}_{coder_base_name}"

        self._init_phase_properties()

    def eval_method(self, func_name:str, args):
        method_name = f"{self._method_base_name}_{func_name}"
        method = getattr(self._module, method_name)
        return method(*args)

    def eval_method_non_calib(self, func_name:str, args):
        method_name = f"{self._method_base_name_non_calib}_{func_name}"
        method = getattr(self._module, method_name)
        return method(*args)
    
    def _init_phase_properties(self):
        self._phase_name    = getattr(self._module, f"{self._method_base_name}_name")()
        self._endmember_num = getattr(self._module, self._method_base_name+'_endmember_number')()
        self._endmember_formulas      = np.array([
            getattr(self._module, f"{self._method_base_name}_endmember_formula")(i) 
            for i in range(self._endmember_num)])
        self._identifier    = getattr(self._module, f"{self._method_base_name}_identifier")()
        self._species_num   = getattr(self._module, f"{self._method_base_name}_species_number")()
        self._endmember_names = np.array([
            getattr(self._module, f"{self._method_base_name}_endmember_name")(i)
            for i in range(self._endmember_num)])
        self._endmember_molwts = np.array([
            getattr(self._module, f"{self._method_base_name}_endmember_mw")(i)
            for i in range(self._endmember_num)])
        self._endmember_element_comps = np.array([
            getattr(self._module, f"{self._method_base_name}_endmember_elements")(i)
            for i in range(self._endmember_num)])
        self._endmember_number_of_atoms = np.sum(self._endmember_element_comps, axis=1)
        self._endmember_mol_oxide_comps = np.array(
            [chem.calc_mol_oxide_comp(self._endmember_element_comps[i])
             for i in range(self._endmember_num)])
        self._oxide_space = np.any(self._endmember_mol_oxide_comps, axis=0)
        self._species_names = np.array([
            getattr(self._module, f"{self._method_base_name}_species_name")(i) 
            for i in range(self._species_num)])
        self._species_element_comps = np.array([
            getattr(self._module, f"{self._method_base_name}_species_elements")(i)
            for i in range(self._species_num)])
    ...
    @property
    def endmember_formulas(self):
        return self._endmember_formulas
    ...
    def endmember_num(self):
        return self._endmember_num

    @property
    def species_num(self):
        return self._species_num
    ...
    @property
    def endmember_molwts(self):
        return self._endmember_molwts

    @property
    def endmember_element_comps(self):
        return self._endmember_element_comps
    
    @property
    def endmember_number_of_atoms(self):
        return self._endmember_number_of_atoms

    @property
    def endmember_mol_oxide_comps(self) -> NDArray[np.float_]:
        return self._endmember_mol_oxide_comps

    @property
    def oxide_space(self) -> NDArray[np.bool_]:
        return self._oxide_space
    ...
    @property
    def species_element_comps(self) -> NDArray[np.float_]:
        return self._species_element_comps

    def G(self, *args):
        return self.eval_method('g', args)

    def dG_dT(self, *args):
        return self.eval_method('dgdt', args)

    def dG_dP(self, *args):
        return self.eval_method('dgdp', args)

    def dG_dm(self, *args):
        return self.eval_method('dgdn', args)

    def d2G_dT2(self, *args):
        return self.eval_method('d2gdt2', args)

    def d2G_dTdP(self, *args):
        return self.eval_method('d2gdtdp', args)

    def d2G_dTdm(self, *args):
        return self.eval_method('d2gdndt', args)

    def d2G_dP2(self, *args):
        return self.eval_method('d2gdp2', args)

    def d2G_dPdm(self, *args):
        return self.eval_method('d2gdndp', args)

    def d2G_dm2(self, *args):
        result = self.eval_method('d2gdn2', args)
        endmem_num = getattr(self._module, self._method_base_name+'_endmember_number')()
        return _sym_to_dense(endmem_num, result)

    def d3G_dT3(self, *args):
        return self.eval_method('d3gdt3', args)

    def d3G_dT2dP(self, *args):
        return self.eval_method('d3gdt2dp', args)

    def d3G_dT2dm(self, *args):
        return self.eval_method('d3gdndt2', args)

    def d3G_dTdP2(self, *args):
        return self.eval_method('d3gdtdp2', args)

    def d3G_dTdPdm(self, *args):
        return self.eval_method('d3gdndtdp', args)

    def d3G_dTdm2(self, *args):
        result = self.eval_method('d3gdn2dt', args)
        endmem_num = getattr(self._module, self._method_base_name+'_endmember_number')()
        return _sym_to_dense(endmem_num, result)

    def d3G_dP3(self, *args):
        return self.eval_method('d3gdp3', args)

    def d3G_dP2dm(self, *args):
        return self.eval_method('d3gdndp2', args)

    def d3G_dPdm2(self, *args):
        result = self.eval_method('d3gdn2dp', args)
        endmem_num = getattr(self._module, self._method_base_name+'_endmember_number')()
        return _sym_to_dense(endmem_num, result)


    def d3G_dm3(self, *args):
        result = self.eval_method('d3gdn3', args)
        endmem_num = getattr(self._module, self._method_base_name+'_endmember_number')()
        return _sym_ten_to_dense(endmem_num, result)

    def affinity_and_composition(self, *args):
        return self.eval_method_non_calib('getAffnComp', args)
    ...
    # (order=1)
    def H(self, *args):
        T, P, mol = self.parse_args(args)
        return self.eval_method('g', args) + T*self.eval_method('s', args)


    def dS_dT(self, *args):
        return -self.d2G_dT2(args)


    def Cv(self, *args):
        T, P, mol = self.parse_args(args)
        return T*self.dS_dT(args)
    ...
    # TODO: remove dependence on non-Gibbs methods

    def S(self, *args):
        # return -self.dG_dT(args)
        return self.eval_method('s', args)

    def V(self, *args):
        #return self.dG_dP(args)
        return self.eval_method('v', args)

    def Cp(self, *args):
        return self.eval_method('cp', args)

    def dCp_dT(self, *args):
        return self.eval_method('dcpdt', args)

    def dCp_dm(self, *args):
        # Likely only required for adiabatic (isentropic) simulations
        assert False, "dcpdm not coded in coder module."
        return self.eval_method('dcpdm', args)

    def convert_elements_to_moles(self, *args) -> NDArray[np.float_]:
        return self.eval_method('conv_elm_to_moles', args)
    
    def convert_elements_to_total_moles(self, *args) -> float:
        return self.eval_method('conv_elm_to_tot_moles', args)
    
    def convert_elements_to_total_grams(self, *args) -> float:
        return self.eval_method('conv_elm_to_tot_grams', args)
    
    def convert_moles_to_total_moles(self, *args) -> float:
        return self.eval_method('conv_moles_to_tot_moles', args)

    def convert_moles_to_elements(self, *args) -> NDArray[np.float_]:
        return self.eval_method('conv_moles_to_elm', args)
    
    def convert_moles_to_mole_fractions(self, *args) -> NDArray[np.float_]:
        return self.eval_method('conv_moles_to_mole_frac', args)
    
    def test_moles(self, *args) -> bool:
        return self.eval_method('test_moles', args)

    def compute_formula(self, *args) -> str:
        return self.eval_method('formula', args)
    
    def get_param_names(self) -> list[str]:
        return self.eval_method_non_calib('get_param_names',{})

    def get_param_value(self, *args) -> float:
        return self.eval_method_non_calib('get_param_value', args)

    def set_param_value(self, *args) -> float:
        return self.eval_method_non_calib('set_param_value', args)



    
    # calc_
    # def density, alpha, K, compressibility
    # dV_dm
    ...
    # def dG_dw(self, T, P, mol=None, param=None):
    #     pass

    # def d2G_dTdw(self, T, P, mol=None, param=None):
    #     pass

    # def d2G_dPdw(self, T, P, mol=None, param=None):
    #     pass

    # def d2G_dmdw(self, T, P, mol=None, param=None):
    #     pass
    
    # def d3G_dT2dw(self, T, P, mol=None, param=None):
    #     pass

    # def d3G_dTdPdw(self, T, P, mol=None, param=None):
    #     pass

    # def d3G_dTdmdw(self, T, P, mol=None, param=None):
    #     pass
    
    # def d3G_dP2dw(self, T, P, mol=None, param=None):
    #     pass
    
    # def d3G_dPdmdw(self, T, P, mol=None, param=None):
    #     pass

    # def d3G_dm2dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dT2dmdw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dTdPdmdw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dP2dmdw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dm3dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dTdm2dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dPdm2dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dT3dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dT2dPdw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dTdP2dw(self, T, P, mol=None, param=None):
    #     pass

    # def d4G_dP3dw(self, T, P, mol=None, param=None):
    #     pass




class LegacyPhaseCalculator():
    """
    Wraps phase built using non-coder generated C code, ported from xMELTS, for example.
    """
    def __init__(self, coder_module):
        self._module_name = coder_module
        self._module = import_module(coder_module)
        ...
        name_parts = str.split(self._module_name, '.')
        base_name = name_parts[3]
        phs_name = str.split(base_name, '_')[0]

        # Change this- Phase shouldn't need this
        self._method_base_name = phs_name

        self._init_phase_properties()

    def eval_method(self, func_name:str, args):
        assert False, "Remove calls to eval_method in LegacyPhaseCalculator."
    
    def _init_phase_properties(self):
        self._phase_name    = self._module.phase_name
        self._endmember_num = self._module.endmember_num
        self._endmember_formulas = self._module.endmember_formula
        self._identifier    = self._module.identifier
        self._species_num   = self._module.species_num
        self._endmember_names = self._module.endmember_names
        self._endmember_molwts = self._module.endmember_molwts
        self._endmember_element_comps = self._module.endmember_element_comps
        self._endmember_number_of_atoms = np.sum(self._endmember_element_comps, axis=1)
        self._endmember_mol_oxide_comps = np.array(
            [chem.calc_mol_oxide_comp(self._endmember_element_comps[i])
             for i in range(self._endmember_num)])
        self._oxide_space = np.any(self._endmember_mol_oxide_comps, axis=0)
        self._species_names = self._module.species_names
        self._species_element_comps = self._module.species_element_comps
    ...
    def G(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_g(T, P, mol)

    def dG_dT(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_dgdt(T, P, mol)

    def dG_dP(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_dgdp(T, P, mol)

    def dG_dm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_dgdm(T, P, mol)

    def d2G_dT2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdt2(T, P, mol)

    def d2G_dTdP(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdtdp(T, P, mol)

    def d2G_dTdm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdtdm(T, P, mol)

    def d2G_dP2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdp2(T, P, mol)

    def d2G_dPdm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdpdm(T, P, mol)

    def d2G_dm2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d2gdm2(T, P, mol)

    def d3G_dT3(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdt3(T, P, mol)

    def d3G_dT2dP(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdt2dp(T, P, mol)

    def d3G_dT2dm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdt2dm(T, P, mol)

    def d3G_dTdP2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdtdp2(T, P, mol)

    def d3G_dTdPdm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdpdtdm(T, P, mol)

    def d3G_dTdm2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdtdm2(T, P, mol)

    def d3G_dP3(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdp3(T, P, mol)

    def d3G_dP2dm(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdp2dm(T, P, mol)

    def d3G_dPdm2(self, T: float, P: float, mol: NDArray[np.float_]):
        return self._module.cy_eval_d3gdpdm2(T, P, mol)

    def d3G_dm3(self, T: float, P: float, mol: NDArray[np.float_]):
        result = self._module.cy_eval_d3gdm3(T, P, mol)
        return result

    def affinity_and_composition(self, T: float, P: float, mu: NDArray[np.float_]):
        result = self._module.cy_affinity_and_comp(T, P, mu)
    ...
    def H(self, *args):
        T, P, mol = self.parse_args(args)
        return self.G(args) + T*self.S(args)
    ...
    def convert_elements_to_moles(self, elms: NDArray[np.float_]) -> NDArray[np.float_]:
        return self._module.cy_convert_elms_to_mol(elms)
    
    def convert_elements_to_total_moles(self, elms: NDArray[np.float_]) -> float:
        return np.sum(self._module.cy_convert_elms_to_mol(elms))
    
    def convert_elements_to_total_grams(self, elms: NDArray[np.float_]) -> float:
        mol_endms = self.convert_elements_to_moles(elms)
        return np.sum(mol_endms.dot(self.endmember_molwts))
    
    def convert_moles_to_total_moles(self, mols: NDArray[np.float_]) -> float:
        return np.sum(mols)

    def convert_moles_to_elements(self, mols: NDArray[np.float_]) -> NDArray[np.float_]:
        return self.endmember_element_comps.T.dot(mols)
    
    def convert_moles_to_mole_fractions(self, mols: NDArray[np.float_]) -> NDArray[np.float_]:
        return mols / np.sum(mols)
    
    def test_moles(self, mols: NDArray[np.float_]) -> bool:
        return self._module.cy_test_comp(mols)

    def compute_formula(self, T: float, P: float, mols: NDArray[np.float_]) -> str:
        return self._module.cy_compute_formula(T, P, mols)

    def get_param_names(self) -> list[str]:
        return []


class PurePhaseCalculator():
    def __init__(self, coder_module, phase_name):
        self._module_name = coder_module
        phs_name = phase_name
        self._module = import_module(coder_module)
    ...
    def _init_phase_properties(self):
        self._phase_name    = getattr(self._module, f"{self._method_base_name}_name")()
        self._endmember_num = 1 # getattr(self._module, self._method_base_name+'_endmember_number')()
        self._endmember_formulas      = np.array([
            getattr(self._module, f"{self._method_base_name}_formula")(),])
        self._identifier    = getattr(self._module, f"{self._method_base_name}_identifier")()
        self._species_num   = 1 # getattr(self._module, f"{self._method_base_name}_species_number")()
        self._endmember_names = np.array([
            getattr(self._module, f"{self._method_base_name}_name")(),])
        self._endmember_molwts = np.array([
            getattr(self._module, f"{self._method_base_name}_mw")(),])
        self._endmember_element_comps = np.array([
            getattr(self._module, f"{self._method_base_name}_elements")(),])
        self._endmember_number_of_atoms = np.sum(self._endmember_element_comps, axis=1)
        self._endmember_mol_oxide_comps = np.array(
            [chem.calc_mol_oxide_comp(self._endmember_element_comps[i])
             for i in range(self._endmember_num)])
    ...
    def dG_dm(self, *args):
        pure_phase_mu = np.array([self.eval_method('g', args),])
        return pure_phase_mu
...
class ExchangeEquil():
    ...
    def _tailored_affinity_and_comp(self, t, p, mu, debug=False):
        ...
        """
        # A module coded by hand or generated by coder

        method = self.phase.calculator.affinity_and_composition
        if method:
            if debug > 0:
            ...
            # Attempt to protect against numerical errors giving -ve element moles:
            elem_comp = self.phase.convert_endmember_comp(X, output='moles_elements')
            update_comp = False
            for i in range(len(elem_comp)):
                if elem_comp[i] < 1e-14:
                    elem_comp[i] = 0
                    update_comp = True
            if update_comp:
                X = self.phase.convert_elements(elem_comp)
    ...
    def _update_mu_X_A(self, t, p, mu, X, chempot_update,
        ...
        if np.any(np.isnan(muM)) or np.any(np.isnan(phi)):
            print('mu-phi isnan')
            assert False, "NaN values in mu or phi"
            # from IPython import embed; embed()
    ...
    def affinity_and_comp(self, t, p, mu, X_init=None, site_m=None,
        ...
        method : str, def 'generic'
            ...
            'special' - algorthmic specifically coded in the phase instance,
                currently, only Objective-C and generated coder methods are utilized,
    ...
    #     assert False, 'entered determine_phase_stability in phases.py'
