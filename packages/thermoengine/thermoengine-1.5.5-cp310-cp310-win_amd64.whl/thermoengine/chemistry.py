...
from thermoengine_utils.core import UnorderedList
...
WT_OXIDES = pd.Series({
    'SiO2': 60.0848, 'TiO2': 79.8988,
    'Al2O3':101.96128,'Fe2O3': 159.6922,'Cr2O3': 151.9902,
    'FeO': 71.8464, 'MnO': 70.9374,'MgO': 40.3044,
    'NiO': 74.7094,'CoO': 74.9326,'CaO': 56.0794,
    'Na2O': 61.97894,'K2O': 94.1954,'P2O5': 141.94452,
    'H2O': 18.0152,'CO2': 44.0098})

def broadcast_exp_conditions(T, P, X):
    ...
    exp_num = np.max((T.size, P.size, X.shape[0]))
    
    def _isempty(xarr):
        return not xarr.shape
    
    def tile_array_inputs(xarr, arr_len):
        if (xarr.size<arr_len) or _isempty(xarr):
            xarr = np.tile(xarr, arr_len)
        
        return xarr

    
    T = tile_array_inputs(T, exp_num)
    P = tile_array_inputs(P, exp_num)

    if X.shape[0] < exp_num:
        X = pd.concat([X]*exp_num).reset_index(drop=True)

    return T, P, X
    

def normalize_comp(comp:pd.DataFrame) -> pd.DataFrame:
    comp = (comp.T/comp.sum(axis=1)).T
    return comp
