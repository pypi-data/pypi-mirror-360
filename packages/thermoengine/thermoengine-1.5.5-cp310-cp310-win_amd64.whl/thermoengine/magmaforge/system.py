...
class System:
    ...
    def __init__(self, comp:Optional[dict]=None, P:float=1, 
        ...
        calc = EquilCalculator(self.ELEMS, min_potential=min_potential, model_name=model_name, 
                               Liq=Liq, solid_phases=solid_phases)
    ...
    def _init_comp(self, input_comp:dict):
        ...
        self._elem_comp = self._ensure_tiny_amt_of_everything(self._elem_comp)
        self._residue_elem_comp = pd.Series(0, index=self.ELEMS)
    
    def _ensure_tiny_amt_of_everything(self, elem_comp):
        MINVAL = np.sqrt(np.finfo(float).eps)
        elem_comp_adj = elem_comp.copy()

        for i in range(len(elem_comp_adj)):
            if elem_comp_adj[i] < MINVAL:
                elem_comp_adj[i] = MINVAL

        # elem_comp_adj = elem_comp_adj/elem_comp_adj.sum()
        return elem_comp_adj
...
class StateHistory:
    ...
    def get_mineral_comp_table(self, phase_name, units='wt') -> pd.DataFrame:
        assert units=='wt', 'mol units not yet implemented'
        return pd.DataFrame(self.get_mineral_comps(phase_name), index=self.get_temps())
    ...
    def get_mineral_comps(self, phase_name) -> list[dict]:
        return [state.minerals[phase_name].comp for state in self._states]
