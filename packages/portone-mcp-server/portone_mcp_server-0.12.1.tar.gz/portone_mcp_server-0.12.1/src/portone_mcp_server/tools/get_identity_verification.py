...
from portone_mcp_server.tools.utils.portone_rest import mask_identity_verification


def initialize(portone_client: Client):
    def get_identity_verification(identity_verification_id: str, store_id: Optional[str]) -> str | dict:
        """고객사 본인인증 거래번호로 포트원 서버에서 결제 내역을 검색합니다.
        고객사 본인인증 거래번호는 포트원 V1에서는 merchant_uid이며, V2에서는 identityVerificationId에 해당합니다.

        Args:
            identity_verification_id: 고객사에서 발급한 본인인증 거래번호입니다.
        ...
        """
        response = portone_client.get(f"/identity-verifications/{identity_verification_id}", params=filter_out_none({"storeId": store_id}))
        ...
        return mask_identity_verification(data)

    return get_identity_verification
