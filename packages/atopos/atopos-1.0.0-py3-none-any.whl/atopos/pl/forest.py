...
def check_data(data):
    assert isinstance(
        data, pd.DataFrame), 'Please supply data as Pandas DataFrame'
...
def annotate_columns(data, to_annotate, auc_col, se_col, ax, anot_base=0.15, annot_offset=13, annot_scaler=0.01,
    ...
    fargs = {'fontsize': 12, 'fontfamily': 'sans-serif',
             'clip_on': False} if fargs is None else fargs
    to_annotate = [to_annotate] if isinstance(
        to_annotate, str) else to_annotate
    assert isinstance(
        to_annotate, list), 'supply annotation column names as a list of strings'
    ...
    for i, txt in enumerate(np.arange(data.shape[0])):
        ...
        for j, column in enumerate(to_annotate):
            max_len = max(data[column].astype(str).str.len().max(), len(
                column))  # gets width of a column in characters
            col_shift = anot_base + ((max_len-annot_offset)*annot_scaler)
            if j != 0:
                # distance away from previous shifted proportional to number of characters
                xiter -= col_shift
            s = data.iloc[txt, :][column]
            if isinstance(s, (float, np.float64)) and np.isnan(s):
                continue
            s = parse_auc(data, txt, s, se_col,
                          add_ci) if column == auc_col else s
    ...
    if hbar_lim is None:
        ...
        #hbar_xmax = 1.22
        if add_ci:
            max_bar = np.max(annotations) if auc_label is None else np.max(
                annotations) + 0.009 * len(auc_label + ' [95%  CI]')
        else:
            max_bar = np.max(annotations) if auc_label is None else np.max(
                annotations) + 0.01 * len(auc_label)
    ...
    for hbar_ypos, line_width in zip([data.shape[0], data.shape[0]+2, -2.8], [1, 2, 2]):
        bars.append(ax.axhline(y=hbar_ypos, xmin=hbar_xmin,
                    xmax=hbar_xmax, linewidth=line_width, **bargs))
...
def plot_auc(data, ax, auc_col, se_col, subset_col=None, colour='#2E5A86', add_vline=1, add_legend=True,
             legend_loc='upper right', xlim=None, auc_label=None):
    if subset_col is not None:
        subsets = data[subset_col].unique().tolist(
        ) if subset_col in data.columns else None
    else:
        subsets = None
    ...
    if subsets is not None:
        ...
        assert len(markers) == len(
            subsets), 'More subsets than markers - overwrite marker list'
    ...
    if add_vline:
        ax.axvline(x=add_vline, c='#474747', linestyle='--', alpha=0.8, ymin=0,
