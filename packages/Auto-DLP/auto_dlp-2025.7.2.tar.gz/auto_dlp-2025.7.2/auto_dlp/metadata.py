...
import auto_dlp.image_metadata


def write_for_song(config, artist, name: str, song_id, path: Path):
    mut = EasyID3()
    mut["title"] = name.encode("utf-8")
    mut["artist"] = artist.encode("utf-8")
    mut["albumartist"] = artist.encode("utf-8")
    mut["album"] = f"{artist} songs".encode("utf-8")
    mut["copyright"] = "none"
    # mut["date"] = "0"
...
def write_for_playlist_item(config, artist, playlist, name, playlist_id, song_id, index, item_count, path: Path):
    ...
    mut["artist"] = artist.encode("utf-8")
    mut["albumartist"] = artist.encode("utf-8")
    mut["album"] = playlist.encode("utf-8")
    mut["copyright"] = "none"
