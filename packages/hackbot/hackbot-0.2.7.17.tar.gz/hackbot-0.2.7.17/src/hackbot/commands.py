...
def price_run(args: argparse.Namespace) -> tuple[int, int]:
    ...
    if not args.api_key:
        log.warning(f"No HACKBOT_API_KEY set. If you want to run hackbot, please sign up and setup an API key at {GREEN}https://hackbot.co{END}")
...
def hackbot_run(args: argparse.Namespace) -> int:
    ...
    auth_result = asyncio.run(authenticate(args.api_key))
    print(f"[DEBUG] auth_result type: {type(auth_result)}, value: {auth_result}")  # DEBUG
    if isinstance(auth_result, HackbotAuthError):
        log.error(f"❌ Error: {auth_result.message}")
        return 1
