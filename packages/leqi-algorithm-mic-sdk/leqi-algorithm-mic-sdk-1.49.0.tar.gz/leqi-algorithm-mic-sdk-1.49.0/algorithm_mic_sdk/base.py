...
class Base(object):
    ...
    def __init__(self, auth_info: AuthInfo):
        ...
        self._use_accelerate = auth_info.use_accelerate
    ...
    def get_put_url(self, oss_name, extranet, headers, timeout, use_accelerate=None):
    ...
    @lru_cache(maxsize=100)
    def _get_file_url(self, oss_name, extranet, watermark, timeout, api_oss_url, use_accelerate=None):
        if use_accelerate is None:
            use_accelerate = self._use_accelerate
        params = {'filename': oss_name, 'extranet': extranet, 'watermark': watermark, 'timeout': timeout, 'use_accelerate': use_accelerate}
        resp = Response.request('GET', api_oss_url, params=params)
        return resp.json['preview_url']

    def send_file(self, file_bytes, oss_name=None, cover=False, extranet=None, random_name=None, prefix='', use_accelerate=None):
        """
        上传文件到算法的oss中
        ...
        :param prefix: 文件名前缀
        :param use_accelerate: 是否使用传输加速域名
        :return: 文件在oss上面的文件名
        """
        if extranet is None:
        ...
        put_url, exist_file, oss_name = self.get_put_url(oss_name, extranet, headers=None, timeout=600, use_accelerate=use_accelerate)
        if not exist_file or cover:
            if callable(file_bytes):
                file_bytes = file_bytes()
            resp = Response.request('PUT', put_url, data=file_bytes, timeout=600)
        return oss_name

    def get_file(self, oss_name=None, extranet=None, watermark=None, use_accelerate=None):
        """
        下载文件数据
        :param oss_name:文件在阿里云oss上的名称
        :param extranet: 是否使用外网传输, 为None时将使用auth_info中的参数
        :param watermark: 是否需要添加水印
        :param use_accelerate: 是否使用传输加速
        :return: 图片二进制数据
        """
        if extranet is None:
            extranet = self._extranet
        if use_accelerate is None:
            use_accelerate = self._use_accelerate
        get_url = self._get_file_url(oss_name, extranet, watermark=watermark, timeout=600, api_oss_url=self.api_oss_url, use_accelerate=use_accelerate)
        resp = Response.request('GET', get_url, timeout=600)
        return resp.content

    @lru_cache(maxsize=100)
    def get_file_url(self, oss_name, extranet=None, watermark=None, timeout=3600, use_accelerate=None):
        """
        生成文件的预览地址
        :param oss_name: 文件在阿里云oss上的名称
        ...
        if use_accelerate is None:
            use_accelerate = self._use_accelerate
        get_url = self._get_file_url(oss_name, extranet, watermark, timeout, api_oss_url=self.api_oss_url, use_accelerate=use_accelerate)
        return get_url

    def get_classic_file(self, oss_name=None, extranet=None, watermark=None, use_accelerate=None):
        """
        下载经典算法下文件数据
        :param oss_name:文件在阿里云oss上的名称
        :param extranet: 是否使用外网传输, 为None时将使用auth_info中的参数
        :param watermark: 是否需要添加水印
        :param use_accelerate: 是否需要启用加速
        :return: 图片二进制数据
        """
        if extranet is None:
            extranet = self._extranet
        ...
        get_url = self._get_file_url(oss_name, extranet, watermark=watermark, timeout=600,
                                     api_oss_url=self.api_classic_oss_url, use_accelerate=use_accelerate)
        resp = Response.request('GET', get_url, timeout=10)
        return resp.content

    @lru_cache(maxsize=100)
    def get_classic_file_url(self, oss_name, extranet=None, watermark=None, timeout=3600, use_accelerate=None):
        """
        生成经典算法下文件的预览地址
        :param oss_name: 文件在阿里云oss上的名称
        :param extranet: 是否使用内网传输, 为None时将使用auth_info中的参数
        ...
        if use_accelerate is None:
            use_accelerate = self._use_accelerate
        get_url = self._get_file_url(oss_name, extranet, watermark, timeout, api_oss_url=self.api_classic_oss_url, use_accelerate=use_accelerate)
        return get_url
