...
class FileInfo(object):
    ...
    @classmethod
    def for_url(cls, url, forced_transmission=False):
        """
        文件来源于url,若为阿里云华东二区的url,直接交于算法服务器下载,否则由程序先下载,再上传
        @param url:
        @param forced_transmission: 是否强制传输,如果为False则会自动判断是否为华东2区的url,为True则不判断,直接交由算法处理
        @return:
        """
        if forced_transmission or 'oss-cn-shanghai.aliyuncs.com' in url or 'oss-cn-shanghai-internal.aliyuncs.com' in url:
