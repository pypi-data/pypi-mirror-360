...
class VirtualHostService:
    ...
    async def tool_deploy_virtual_host(
        ...
        await ensure_authenticated(credentials=get_credentials(ctx))
    ...
    async def tool_edit_virtual_host(
        ...
        await devopness.virtual_hosts.update_virtual_host(
            virtual_host_id,
            {
                "id": virtual_host_id,
                "name": vh_name,
        ...
        virtual_host = (
            await devopness.virtual_hosts.get_virtual_host(virtual_host_id)
        ).data
