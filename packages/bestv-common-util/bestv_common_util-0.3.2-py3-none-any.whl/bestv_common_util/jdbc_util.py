...
def pandas_upsert_to_postgres(engine, df, schema, table_name, conflict_columns):
    ...
    try:
        ...
        with engine.connect() as conn:
            conn.execute(text(f"CREATE TEMP TABLE IF NOT EXISTS {schema}.{temp_table} (LIKE {schema}.{table_name})"))
            conn.commit()
        ...
        logging.info("成功执行 Upsert 操作，影响行数: {}", len(df))

    except SQLAlchemyError as e:
        logging.error("数据库操作出错: {}", e)
        raise
    finally:
        logging.info("clear temp table: {}.{}", schema, temp_table)
        # 清理临时表
        try:
            with engine.connect() as conn:
                conn.execute(text(f"TRUNCATE TABLE {schema}.{temp_table}"))
