...
import numpy as np
import pygame
import cmo_gymnasium.utils.registration as sa_gym
...
class CMOFourRoom(gym.Env, EzPickle):
    """
    ## Description
    ...
    def __init__(self, render_mode: Optional[str] = None, maze=MAZE):
        ...
        self.height, self.width = maze.shape # Default = 14 x 14 
        self.maze = maze
        shape_types = ["1", "2", "3"]
        ...
        for c in range(self.width):
            for r in range(self.height):
                if maze[r, c] == "G":
                    ...
                    self.shape_ids[(r, c)] = len(self.shape_ids) # store shape each with unique id
        ...
        # Constraints 
        # Enter hazard state -> c1 += 1 
        self.constraint_dim = 1
        self.constraints = Box(low=0, high=1, shape=(1,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        self.constraints = np.array([0.0], dtype=np.float32)  # reset hazard constraint

        # perform the movement
        if action == CMOFourRoom.LEFT:
            col -= 1
        elif action == CMOFourRoom.UP:
            row -= 1
        elif action == CMOFourRoom.RIGHT:
            col += 1
        elif action == CMOFourRoom.DOWN:
            row += 1
        else:
        ...
        if s1 == self.goal:
            ...
            terminated = True
            return self.state_to_array(self.state), phi, self.constraints, terminated, False, {}
        
        # into a hazard cell
        if s1 in self.hazards:
            terminated = False
            self.constraints = np.array([1.0], dtype=np.float32)  # hazard encountered -> set c1 = 1 
        ...
        if s1 in self.shape_ids:
            ...
            if collected[shape_id] == 1:
                ...
                collected = list(collected) # collected stores shapes so far ; tuple -> list 
    ...
    def render(self):
        ...
        canvas.blit(img, (np.array(self.initial[0])[::-1] + 0.15) * pix_square_size)
            ...
            # Draw hazards first to ensure they are always visible
        for i in range(self.maze.shape[0]):
            for j in range(self.maze.shape[1]):
                if self.maze[i, j] == "H":
                    ...
                    pygame.draw.rect(
                        canvas,
                        (169, 169, 169),  # Dark gray for hazards
                        pygame.Rect(
...
if __name__ == "__main__":
    env = sa_gym.make("cmo-four-room-v0", render_mode="human")
    terminated = False
    env.reset()
    while True:
