...
class CMOMinecart(gym.Env, EzPickle):
    """
    ## Description
    ...
    def __init__(
        ...
        config=str(Path(__file__).parent / "mine_config.json")
    ):
        ...
        # added time step to the env to track ore carriage cost
        self.time_step_ore1 = 0
        self.time_step_ore2 = 0
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip if self.incremental_frame_skip else 1):
            ...
            # added cost function where c1 = 1 if ore1 is carried for more than 100 timesteps
            # and c2 = 1 if ore2 is carried for more than 150 timesteps, placing more emphasis on ore1
            cost = np.zeros(2, dtype=np.float32)
            if self.cart.content[0] > 0:
                self.time_step_ore1 += 1
                if self.time_step_ore1 > 100:
                    cost[0] = 1.0
            if self.cart.content[1] > 0:
                self.time_step_ore2 += 1
                if self.time_step_ore2 > 150:
                    cost[1] = 1.0
            ...
            if distanceFromBase < BASE_RADIUS * BASE_SCALE:
                if self.cart.departed:
                    ...
                    # proposed addition: put a greater reward for ore1 to complicate task
                    reward[0] += self.cart.content[0] * 1.1
        ...
        # added cost function in return
        return self.get_state(change), reward, cost, self.end, False, {}
...
if __name__ == "__main__":
    env = cmo_gymnasium.make('cmo-minecart-deterministic-v0', render_mode='human')
    obs, _ = env.reset()
    done = False
    ...
    while not done:
        ...
        obs, reward, cost, done, truncated, info = env.step(action)
        env.render()

        print(f"reward: {reward}, cost: {cost}, obs: {obs}, done?: {done}\n")
