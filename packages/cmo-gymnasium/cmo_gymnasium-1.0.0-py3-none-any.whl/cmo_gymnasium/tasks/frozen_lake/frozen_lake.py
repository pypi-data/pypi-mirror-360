...
def generate_random_map(
    size: int = 8, p: float = 0.8, seed: Union[int, None] = None
) -> List[str]:
...
class CMOFrozenLakeEnv(Env):
    ...
    * v0: Initial version release
    ...
    def __init__(
        ...
        self.reward_dim = 2
        self.reward_space = spaces.Box(low=self.reward_range[0], high=self.reward_range[1], 
                                        shape=(self.reward_dim,), dtype=np.float32)
        
        self.visit_counts = {} # make visits dict
        self.cost = np.zeros(1, dtype=np.float64)  # cost for the current step
        self.costs_dim = 1
    ...
    def step(self, a):
        ...
        # Since each state is a unique position in the grid, we use a dictionary
        # to map the number of visits per state and increment
        self.visit_counts[s] = self.visit_counts.get(s, 0) + 1

        # Reward[0] is 1 for reaching the goal, while Reward[1] is -1 for each time step
        reward = np.zeros(self.reward_dim, dtype=np.float32)
        reward[0] = r
        reward[1] = -1

        # If the visits to a state > 2 then cost = 1
        costs = np.zeros(self.costs_dim, dtype=np.float32)
        if self.visit_counts[s] > 2:
            costs[0] = 1
        self.s = s
        self.lastaction = a
        ...
        return int(s), reward, costs, t, False, {"prob": p}
    ...
    def reset(
        ...
        *,
        ...
        self.visit_counts.clear() # reset visit dict
