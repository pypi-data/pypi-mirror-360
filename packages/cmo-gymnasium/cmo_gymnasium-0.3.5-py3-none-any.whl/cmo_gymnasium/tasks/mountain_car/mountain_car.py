...
class CMOMountainCar(MountainCarEnv, EzPickle):
    """
    ...
    ## Cost space:
    The cost space is a 3D vector containing:
    - Cost[1]: 1.0 if the car goes beyond speed limit, else 0.0
    - Cost[2]: 1.0 if the mountain car in proximity to left wall x = -1.2, else 0.0
    """
    ...
    def __init__(
        ...
        # Initialize the Constraint space
        # Cost[1] = 1.0 if car goes beyond speed limit, else 0.0
        # Cost[2] = 1.0 if near the left wall (inelastic (velociy=0) collision)
        self.costs_dim = 2
        self.costs_space = spaces.Box(
            low=np.array([0.0, 0.0]), 
            high=np.array([1.0, 1.0]), 
            shape=(self.costs_dim,), 
            dtype=np.float32
        )

        self.speed_limit = 0.07 * 0.8 # don't want to exceed the speed limit of 80% of max speed
    ...
    def step(self, action: int):
        ...
        # action constraints (not able to foward)
        if action == 0: # action 0 is foward
            action = 1  # force to do nothing
        ...
        # Cost of exceeding the speed limit (80% of max speed)
        print(f"Velocity: {velocity}, Speed Limit: {self.speed_limit}")
        costs[0] = 1.0 if abs(velocity) > self.speed_limit else 0.0

        # Cost of being near the left wall (want to avoid collision)
        if position > self.min_position and position < self.min_position + 0.1:
            costs[1] = 1.0
        ...
        if self.render_mode == "human":
            self.render()
        return np.array(self.state, dtype=np.float32), reward, costs, terminated, False, {}


if __name__ == "__main__":
    env = cmo_gymnasium.make("cmo-mountaincar-v0", render_mode="human")
    terminated = False
    env.reset()
    while True:
