 	...
 	#     'Endoderm_sub14': 74,
  	#   'Endoderm_sub15': 75
        ...
        try:
            ...
            tissue_map = sc_color_map.copy()
            for cell_id, tissue_type in zip(ids, data.obs[tissue_id]):
                if tissue_type in tissue_map:
                    self.tissue[cell_id] = tissue_map[tissue_type]
                else:
                    new_id = max(tissue_map.values()) + 1
                    tissue_map[tissue_type] = new_id#更新字典,防止当有细胞类型不在预定义字典里面导致的细胞个体被赋值为细胞类型的进程崩溃
                    self.tissue[cell_id] = new_id 
        ...
        self.all_tissues = sorted(self.cells_from_tissue.keys())
        if store_anndata:
            self.anndata = data
        if pos_reg_id in data.obsm and data.obsm[pos_reg_id].shape[1] == 3:
