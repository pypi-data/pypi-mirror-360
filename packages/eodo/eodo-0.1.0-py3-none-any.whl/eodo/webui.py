...
def main_ui():
    ...
    with tab0:
        ...
        st.write(f"上一次执行ID: {info.get("id", "无")}")
        st.write(f"上一次执行结果: {info.get("result", "无")}")
        st.write(f"上一次执行时间: {info.get("time", "无")}")
...
def main_entry():
    ...
    threading.Thread(target=start_scheduler, args=(args.interval,), daemon=True).start()
    st.set_page_config(page_title="EdgeOne管理")
    main_ui()
