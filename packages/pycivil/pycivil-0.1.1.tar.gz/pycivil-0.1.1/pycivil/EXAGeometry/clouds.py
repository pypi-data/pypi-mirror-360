...
from pycivil.EXAGeometry.geometry import Point2d, Vector2d, Seg2d, Point2dMdl
...
class PointCloud2d:
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            if len(args[0]) == 0:
                ...
                self.__points: List[Point2d] = [Point2d()] * len(args[0])
                self.__signs: List[float] = [0.0] * (len(args[0]) - 2)
                if isinstance(args[0][0], list):
                    self.__metaData: List[List[Any]] = [[]] * len(args[0])
                    for idx in range(len(self.__points)):
                        ...
                        if idx >= 2:
                            ...
                            vecEnd = Vector2d(self.__points[idx - 1], self.__points[idx])
    ...
    def __getitem__(self, key):
        print(f"self.__points = {self.__points}")
    ...
    def contains(
            ...
            if normro:
                ...
                if hasIntersection:
                    ...
                    for ii in pintersect:
                        ...
                        if ii.isEqualTo(pint, rox, roy, 1e-9):
                            found = True
                    if not found:
                        pintersect.append(pint)
                        pintersectFactors.append(ray.extensionFactor(pint))
        ...
        if normro:
            ...
            return contained, minLenght, minPoint, None

        if len(pintersect) != 2:
            print("WARNING: intersection are %1.2i not 2 !!!" % len(pintersect))
            for p in pintersect:
                print(p)
    ...
    def model(self) -> PointCloud2dMdl:
        points: List[Point2dMdl] = [Point2dMdl(x=i, y=i, null=True) for i in range(len(self.__points))]
