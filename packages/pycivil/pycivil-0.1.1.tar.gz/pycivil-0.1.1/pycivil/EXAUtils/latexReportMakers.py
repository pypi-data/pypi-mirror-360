...
from pycivil.EXAStructural.codes import Code, Codes, KNOWN_CODES
...
from pycivil.EXAGeometry.shapes import ShapeRect, ShapeArea, ShapesEnum
...
class ForcesOnSectionListFB(FragmentsBuilder):
    def __init__(self, latexTemplatePath: Path):
        super().__init__()
        self.__forces: List[ForcesOnSection] = []
...
class CodesFB(Codes, FragmentsBuilder):
    def __init__(self, latexTemplatePath: Path, codes: Union[List[Code], None] = None) -> None:
...
class ConcreteFB(FragmentsBuilder):
    ...
    def buildFragment(self) -> Fragment:
        ...
        placeHolders = {
            ...
            "sigmac_max_c": "{:.1f} {:s}".format(self.__concrete.get_sigmac_max_c(), r"\text{ mPa}"),
            "sigmac_max_q": "{:.1f} {:s}".format(self.__concrete.get_sigmac_max_q(), r"\text{ mPa}"),
            "ecu": f"{self.__concrete.get_ecu():.4f}", "ec2": f"{self.__concrete.get_ec2():.4f}",
            "lambda": f"{self.__concrete.get_lambda():.2f}", "eta": f"{self.__concrete.get_eta():.2f}",
            "alphacc": f"{self.__concrete.get_alphacc():.2f}",
            "alphacc_fire": f"{self.__concrete.get_alphacc_fire():.2f}",
            "byCode": self.__concrete.isSettedByCode(), "glossary": self.__opt_glossary}
...
class SteelConcreteFB(FragmentsBuilder):
    ...
    def buildFragment(self) -> Fragment:
        ...
        placeHolders = {
            "classe": f"{self.__steel.catStr():s}", "norma": f"{self.__steel.codeStr():s}",
...
class ConcreteSectionFB(FragmentsBuilder):
    ...
    def buildFragment(self) -> Fragment:
        ...
        # RECTANGULAR SHAPE
        #
        if self.__rcs.getStructConcretelItem().getShape().getType() == ShapesEnum.SHAPE_RECT:
            placeHolders["shape"] = "RECT"
            #
            rectShape = cast(
                ShapeRect, self.__rcs.getStructConcretelItem().getShape()
            )
            ...
            print("scale figure = ", scaleFigure)
        ...
        f.add(
            templateName="template-ita-rc-section.tex",
            templatePlaceholders=placeHolders,
        )
        self._setFragment(f)
        return f
