...
from pycivil.EXAStructural.materials import Concrete, Code
...
class Winkler:
    ...
    def estimateTunnelWallHSprings(self, E, ni, a, surf=""):
    ...
    def estimateTunnelWallVSprings(self, E, ni, B, surf=""):
        """
        (Bussinesque)
        """
        if surf == "square":
    ...
    def estimateTunnelTopArcSprings(self, E, ni, R):
        """
        (Galerkin)
        """
        k = E / ((1 + ni) * R)
    ...
    def estimateTunnelBotArcSprings(self, E, ni, B, surf=""):
        """
        (Bussinesque)
        """
        if surf == "square":
    ...
    def estimateWinklerOnPileHead(self, Es: float, Ep: float, D: float, L: float) -> Tuple[float, float, float, float]:
...
class SoilLayer:
    ...
    def setPhiDry(self, phi: float):
        self.__phi_d = phi

    def setPhiWet(self, phi: float):
        self.__phi_w = phi
