...
class BeamMinimumAreaInput(BaseModel):
    ...
    @field_validator("wEl", "hEl", "bt", "rebarD", "cover", "bMin", "stirrupD", "rebarDComp", "nbLegDirX")
...
class BeamMinimumAreaLogs(BaseModel):
    ...
    rebarAreaDisposed: float = 0
    minimumRebarAreaForElementLenght: float = 0
    minimumLegsForElementLenght: float = 0
    maxStepCrit1: float = 0
    maxStepCrit2: float = 0
    maxStepCrit3: float = 0
    maxStepCrit4: float = 0
...
class SolverConcrete(Solver):
    ...
    def run(self, opt: Union[Enum, None] = None, **kwargs) -> bool:
        if opt is None:
            mat = Concrete()
            mat.setByCode(codeObj=Code(self.__in.keyCode), catstr=self.__in.concreteClass)
...
class SolverBeamMinRebar(Solver):
    ...
    def run(self, opt: Union[Enum, None] = None, **kwargs) -> bool:
        ...
        self.__out.rebarNumber = rebarNumber(self.__logs.minimumRebarArea, self.__in.rebarD)[0]
        self.__logs.rebarAreaDisposed = self.__out.rebarNumber * rebarArea(self.__in.rebarD)

        # Geometry for stirrups
        self.__logs.minimumRebarAreaForElementLenght = rcSectionBeamShearAreaMin(self.__in.bMin)
        self.__logs.minimumLegsForElementLenght = rebarNumber(
            self.__logs.minimumRebarAreaForElementLenght,
            self.__in.stirrupD)[0]
        self.__logs.maxStepCrit1 = self.__in.nbLegDirX / self.__logs.minimumLegsForElementLenght * 1000
        self.__logs.maxStepCrit2, self.__logs.maxStepCrit3, self.__logs.maxStepCrit4 = rcSectionBeamShearStepsMax(
            self.__logs.heightUtil, self.__in.rebarDComp)
        self.__out.stirrupStepMin = min([
            self.__logs.maxStepCrit3,
            self.__logs.maxStepCrit1,
            self.__logs.maxStepCrit2])
        self.__out.stirrupCompStepMin = min([self.__out.stirrupStepMin, self.__logs.maxStepCrit4])
        return True
