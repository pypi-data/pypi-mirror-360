...
from pycivil.EXAStructural.templateRCRect import (
    RCTemplRectEC2, CrackParameters, SectionCrackedStates)
...
from pycivil.EXAStructural.codeEC212 import RMinutes

from pycivil.EXAStructural.checkable import Checkable
from pycivil.EXAStructural.codeNTC2018 import (
    crackMeasure, CrackOut, shearCheckWithRebar)

from pycivil.EXAStructural.codes import Code
from pycivil.EXAStructural.loads import ForcesOnSection, LimiteState_Enum, Frequency_Enum
...
class RcsRectangular(Checkable):
    ...
    def setLogLevel(self, ll: int) -> None:
        self.__ll = ll
    ...
    ###########################
    #
    def __check_SLE_NM_NTC2018(self, load: ForcesOnSection) -> dict:
        [sigmac, sigmas, xi] = self.__obj.solverSLS_NM(
            load.Fx, load.My
        )
        if sigmac >= 0:
            sigmac = 0.0
        ...
        if isinstance(load, ForcesOnSection):
            if (
                load.frequency == Frequency_Enum.CHARACTERISTIC or load.frequency == Frequency_Enum.QUASI_PERMANENT
    ...
    def __check_SLU_T_NTC2018(self, load: ForcesOnSection) -> dict:
        res = {
            "loadIndex": None,
            "loadId": None,
            "checkLog": None,
        ...
        if isinstance(load, ForcesOnSection):
            if load.limitState == LimiteState_Enum.ULTIMATE:
                ...
                if (
                    ...
                    sigmacp = (
                        load.Fx
                        / section.getStructConcretelItem().getArea()
                    )
                else:
                    sigmacp = 0.0
    ...
    def __check_SLE_F_NTC2018(self, load: ForcesOnSection) -> dict:
        res = {
            "loadIndex": -1,
            "loadId": -1,
            "checkLog": {},
            "check": {"crack": False, "severity": ""},
            "safetyFactor": {},
        }
        if isinstance(load, ForcesOnSection):
            ...
            if not load.frequency == Frequency_Enum.FREQUENT and not load.frequency == Frequency_Enum.QUASI_PERMANENT:
        ...
        [sigmac, sigmas, xi] = section.solverSLS_NM(
            load.Fx, load.My
        )
        ...
        def _crackMeasure(param: Tuple[CrackParameters, CrackParameters], idx: int):
            return crackMeasure(
        ...
        [sigmac_u, sigmas_u, xi_u] = section.solverSLS_NM(
            load.Fx, load.My, True
        )
        ...
        if case_1:
            ...
            print("CASE (5)")
            if isinstance(load, ForcesOnSection):
                if load.frequency == Frequency_Enum.FREQUENT or load.frequency == Frequency_Enum.QUASI_PERMANENT:
                    wkMax = 0.2
                    res["checkLog"]["solverCRACKLimit"]["wk"] = wkMax
                    if _cm.wk <= wkMax:
                        res["check"]["crack"] = True
    ...
    def __check_SLU_NM_NTC2018(self, load: ForcesOnSection) -> dict:
        res = {
            "loadIndex": -1,
            "loadId": -1,
            "checkLog": {},
        ...
        if len(section.getInteractionDomain()) == 0:
            section.interactionDomainBuild2d(
                nbPoints=100, SLS=False, bounding=True, negative_compression=True
            )
    ...
    def __check_SLU_NM_FIRE_NTC2018(self, load: ForcesOnSection) -> dict:
        res = {
            "loadIndex": -1,
            "loadId": -1,
            "checkLog": {},
