...
from xstrumodeler.xstrumodeler import (
    XStruCircle, XStruModeler, XStruNode2D,
    XStruTriangle, toPythonDict, toPythonList,
    VDouble, VVDouble)
...
class SectionModeler:
    ...
    def importFromRCSection(self, section: ConcreteSection) -> bool:
        ...
        for si in section.getStructSteelItems():
            ...
            exc = self.__modeler.addCircle(
                self.__modeler.buildIdForCircles(),
                idn, si.getShape().getDiameter()/2)
    ...
    def addTriangle(self,
                    ids: int, idn_1: int, idn_2: int, idn_3: int
                    ) -> bool:
    ...
    def getTrianglesCoords(self) -> Dict[int, Tuple[
    ...
    def addCircleAndNode(self, ids: int, idn_center: int, x: float, y: float, radius: float) -> bool:
    ...
    def getTrianglesCoordsAtMesh(self, idm: int) -> Dict[int, Tuple[
        float, float, float, float, float, float]]:
        d = {}
    ...
    def interactionPoints(self) -> Tuple[List[float], List[float], List[float]]:
        return list(self.__modeler.fzVector()), list(self.__modeler.mxVector()), list(self.__modeler.myVector())

    def buildDomain(self, degreeDivision: int, ratioDivision: int, rebuild: bool = False) -> bool:
    ...
    def sectionRotate(
            self, degree: float, x: float = 0, y: float = 0
    ) -> None:
    ...
    def elStressConcreteNodeAt(self, i) -> float:
        return self.__modeler.elStressConcreteNodeAt(i)

    def elStressSteelNodeAt(self, i) -> float:
        return self.__modeler.elStressSteelNodeAt(i)

    def elStressConcreteNodeId(self, i) -> float:
        return self.__modeler.elStressConcreteNodeId(i)

    def elStressSteelCircleId(self, i) -> float:
