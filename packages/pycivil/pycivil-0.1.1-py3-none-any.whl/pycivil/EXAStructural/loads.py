...
class ForcesOnSection(BaseModel):
    ...
    def switchToNamedRef(self, newRef: Literal['DEFAULT', 'DOMAIN', 'ELASTIC'] = 'DEFAULT'):
        if newRef == 'DEFAULT':
    ...
    def switchToRef(self, newRef: Ref_Enum):
        new_Fx = self.Fx
        new_Fy = self.Fy
        new_Fz = self.Fz
        new_Mx = self.Mx
    ...
    def isNull(self) -> bool:
        return all([self.Fx == 0, self.Fy == 0, self.Fz == 0, self.Mx == 0, self.My == 0, self.Mz == 0])

    def getFrequencyTr(self, lan="IT") -> str:
        if lan == "IT":
    ...
    def getLimitStateTr(self, lan="IT") -> str:
        if lan == "IT":
