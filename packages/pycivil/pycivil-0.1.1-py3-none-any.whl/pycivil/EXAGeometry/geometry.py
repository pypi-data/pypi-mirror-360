...
class Point2d:
    ...
    def isEqualTo(self, other, rox, roy, prec):
        if (
            math.sqrt(math.pow(self.__x - other.x, 2) / math.pow(rox, 2)) < prec
            and math.sqrt(math.pow(self.__y - other.y, 2) / math.pow(roy, 2)) < prec
        ):
            return True
        else:
            return False
...
class Seg2d:
    ...
    def boundingHasPoint(self, p: Point2d, toll: float = 1e-15):
    ...
    def intersectAndContaints(self, other: Seg2d) -> Tuple[bool, Point2d]:
        ...
        if not pint.isNull() and self.boundingHasPoint(pint):
            return True, pint
        else:
            return False, pint
...
class Vector2d:
    def __init__(self,
                 p0: Point2d = Point2d(make_null=True), p1: Point2d = Point2d(make_null=True),
...
def twoPointsDivide(p0: Point2d, p1: Point2d, nb: int, extremes: bool = True) -> List[Point2d]:
    if nb < 1:
...
def twoPointsOffset(p0: Point2d, p1: Point2d, offset: float = 0.0) -> Tuple[Point2d, Point2d]:
