...
def get_schema(api: "Penta", path_prefix: str = "") -> "OpenAPISchema":
    openapi = OpenAPISchema(api, path_prefix)
    return openapi


class OpenAPISchema(dict):
    def __init__(self, api: "Penta", path_prefix: str) -> None:
        self.api = api
    ...
    def operation_parameters(self, operation: Operation) -> List[DictStrAny]:
        ...
        for model in operation.models:
            if model.__penta_param_source__ not in BODY_CONTENT_TYPES:
    ...
    def _extract_parameters(self, model: TModel) -> List[DictStrAny]:
        ...
        for name, details in properties.items():
            ...
            for p_name, p_schema, p_required in flatten_properties(
                ...
                param = {
                    "in": model.__penta_param_source__,
                    "name": p_name,
                    "schema": p_schema,
                    "required": p_required,
                }
    ...
    def _create_schema_from_model(
        ...
        if hasattr(model, "__penta_flatten_map__"):
    ...
    def request_body(self, operation: Operation) -> DictStrAny:
        models = [
            ...
            if m.__penta_param_source__ in BODY_CONTENT_TYPES
        ]
        if not models:
            return {}
        ...
        if len(models) == 1:
            model = models[0]
            content_type = BODY_CONTENT_TYPES[model.__penta_param_source__]
            schema, required = self._create_schema_from_model(
                model, remove_level=model.__penta_param_source__ == "body"
            )
        else:
