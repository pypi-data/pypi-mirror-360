...
class ViewSignature:
    ...
    def __init__(self, path: str, view_func: Callable[..., Any]) -> None:
        ...
        if hasattr(view_func, "_penta_contribute_args"):
            ...
            for p_name, p_type, p_source in view_func._penta_contribute_args:
                self.params.append(
    ...
    def _validate_view_path_params(self) -> None:
        ...
        if self.path_params_names:
            path_model = next(
                (m for m in self.models if m.__penta_param_source__ == "path"), None
            )
            missing = tuple(
                sorted(
                    name
                    for name in self.path_params_names
                    if not (path_model and name in path_model.__penta_flatten_map__)
                )
            )
            if missing:
    ...
    def _create_models(self) -> TModels:
        ...
        for param_cls, args in params_by_source_cls.items():
            ...
            attrs["__penta_param_source__"] = param_cls._param_source()
            attrs["__penta_flatten_map_reverse__"] = {}

            if attrs["__penta_param_source__"] == "file":
                pass

            elif attrs["__penta_param_source__"] in {
                "form",
                "query",
                "header",
                "cookie",
                ...
                attrs["__penta_flatten_map__"] = flatten_map
                attrs["__penta_flatten_map_reverse__"] = {
                    v: (k,) for k, v in flatten_map.items()
                }

            else:
                assert attrs["__penta_param_source__"] == "body"
                if is_multipart_response_with_body:
                    attrs["__penta_body_params__"] = {
            ...
            # collection fields:
            attrs["__penta_collection_fields__"] = detect_collection_fields(
                args, attrs.get("__penta_flatten_map__", {})
            )
...
def detect_collection_fields(
    ...
    """
    Django QueryDict has values that are always lists, so we need to help django penta to understand
    better the input parameters if it's a list or a single value
    This method detects attributes that should be treated by penta as lists and returns this list as a result
    """
