...
class ParamModel(BaseModel, ABC):
    ...
    def _map_data_paths(cls, data: DictStrAny) -> DictStrAny:
        flatten_map = getattr(cls, "__penta_flatten_map__", None)
        if not flatten_map:
            return data
...
class QueryModel(ParamModel):
    ...
    def get_request_data(
        ...
        list_fields = getattr(cls, "__penta_collection_fields__", [])
...
class HeaderModel(ParamModel):
    __penta_flatten_map__: DictStrAny
    ...
    def get_request_data(
        ...
        for name in cls.__penta_flatten_map__:
            if name in headers:
...
class _MultiPartBodyModel(BodyModel):
    __penta_body_params__: DictStrAny
