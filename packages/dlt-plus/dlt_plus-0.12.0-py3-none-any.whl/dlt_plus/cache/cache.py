    ...
    def flush(self) -> None:
        ...
        for cache_output in self.config["outputs"]:
            ...
            for in_table, out_table in cache_output["tables"].items():
                data[out_table] = cache_dataset(f"SELECT * FROM {in_table}", normalize_query=False)
    ...
    def verify_outputs(self) -> None:
        ...
        for cache_output in self.config["outputs"]:
            for table in cache_output["tables"].keys():
                try:
                    cache_dataset(f"SELECT * FROM {table}", normalize_query=False).fetchone()
    ...
    def _get_table_scanner_sql_client(self, cache_input: CacheInputBinding) -> WithTableScanners:
        ...
        with destination_client:
            return sql_client_class(
                ...
                cache_db=cache_input_dataset._get_destination_client(
                    input_dataset.schema
                ).config.credentials,  # type: ignore[arg-type]
