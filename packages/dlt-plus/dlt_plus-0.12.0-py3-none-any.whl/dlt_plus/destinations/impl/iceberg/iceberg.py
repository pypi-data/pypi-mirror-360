...
from dlt_plus.destinations.impl.iceberg.iceberg_adapter import build_iceberg_partition_spec
...
try:
    ...
    from dlt_plus.destinations.impl.iceberg.iceberg_adapter import (
        PARTITION_HINT,
        iceberg_partition,
...
class IcebergLoadJob(RunnableLoadJob):
    ...
    def run(self) -> None:
        ...
        if self._table["write_disposition"] == "merge":
            ...
            with self._job_client._clock_iceberg(  # type: ignore[attr-defined]
                f"write_iceberg_table `{iceberg_table.name()}`"
            ):
                write_iceberg_table(
                    table=iceberg_table,
                    data=ds_.to_table(),
    ...
    def _run_upsert(self, iceberg_table: Any) -> None:
        ...
        if is_nested_table(self._load_table):
            # We check if a unique column exists in verify_schema()
            key_column = get_first_column_name_with_prop(self._load_table, "unique")
        else:
...
class PyIcebergJobClient(JobClientBase, WithStateSync, WithSqlClient, SupportsOpenTables):
    ...
    def _clock_iceberg(op: str) -> Iterator[None]:
        ...
        logger.debug(f"ICEBERG {op} took {clocked} on thread {threading.currentThread().native_id}")
