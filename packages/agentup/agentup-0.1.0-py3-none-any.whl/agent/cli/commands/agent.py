...
def test_middleware(url, timeout, tests, requests, concurrent, output, verbose, no_server_check):
    ...
    Examples:
        agentup agent test-middleware                     # Test all middleware
        agentup agent test-middleware --tests rate-limit # Test only rate limiting
        agentup agent test-middleware --requests 20      # Custom request count
        agentup agent test-middleware --output results.json  # Save results
    """
    # Import and call the test-middleware functionality
    from .test_middleware import test_middleware as test_middleware_cmd

    return test_middleware_cmd.callback(url, timeout, tests, requests, concurrent, output, verbose, no_server_check)
