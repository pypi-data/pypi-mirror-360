            ...
            for f in filters:
                if filters[f]["flg_ativo"] == "S":
                    if filters[f]["datatype"].upper() == "STRING":
                        ...
                        __f.append(f"""{origem["alias"]}.{origem["quoted"]}{f}{origem["quoted"]} = '{self.DataBaseConvertDateTime(conn.DATABASE_NAME, filters[f]["valor"], delta["type"])}'""")
