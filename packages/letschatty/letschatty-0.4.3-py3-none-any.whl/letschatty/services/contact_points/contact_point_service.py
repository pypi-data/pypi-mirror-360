...
highlights_logger = logging.getLogger("highlights")

class ContactPointService:

    @staticmethod
    def create_matched_from_message_and_source(source: Source, company_id: str,chat: Chat, message: ChattyMessage):
        """
    ...
    @staticmethod
    def create_unmatched_from_chatty_pixel_referer_info(referer_info: RefererInfo, company_id: str):
    ...
    @staticmethod
    def create_unmatched_from_enviame_whats_app_source(source: Source, company_id: str, timestamp: datetime):
        """ We received a ChattyPixel message request, but there's no chat nor message still.
        We create a contact point for it, but it's not matched to a source yet.
        This has the information of the message request such as fbclid, gclid, etc.
        """
    ...
    @staticmethod
    def match_existing_contact_point_with_template_source_for_new_chat(contact_point: ContactPoint, source: TemplateSource):
        """
