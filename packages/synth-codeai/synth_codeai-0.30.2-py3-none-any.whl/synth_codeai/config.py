...
def get_config_file_path(project_state_dir: Optional[str] = None) -> Path:
    ...
    Args:
        project_state_dir: Optional custom directory to use instead of .synth-codeai in current directory

    Returns:
        Path: The path to the configuration file
    """
    if project_state_dir:
        synth_codeai_dir = Path(project_state_dir)
    else:
        cwd = os.getcwd()
        synth_codeai_dir = Path(os.path.join(cwd, ".synth-codeai"))

    # Create the directory if it doesn't exist
    synth_codeai_dir.mkdir(parents=True, exist_ok=True)

    return synth_codeai_dir / "config.json"
