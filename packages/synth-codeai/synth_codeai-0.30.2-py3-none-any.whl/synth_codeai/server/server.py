...
from synth_codeai.server.api_v1_sessions import router as sessions_router
from synth_codeai.server.api_v1_spawn_agent import router as spawn_agent_router
from synth_codeai.server.connection_manager import ConnectionManager
from synth_codeai.server.broadcast_sender import set_broadcast_queue
...
app = FastAPI(
    ...
    description="API for synth.codeai - AI Programming Assistant",
    version="1.0.0",
    lifespan=lifespan,
)
...
def custom_openapi():
    ...
    openapi_schema = get_openapi(
        ...
        version="1.0.0",
        description="synth.codeai's API provides REST endpoints for managing sessions and agents, and a WebSocket endpoint (/v1/ws) for real-time communication of events like new trajectories. The root endpoint serves the static web UI if available.",
        routes=app.routes,
        license_info={
            "name": "Apache 2.0",
