...
def setup_logging(log_mode: str = "file", pretty: bool = False, log_level: Optional[str] = None, base_dir: Optional[str] = None) -> None:
    """
    Configure logging for synth_codeai.
    ...
    # Create and add file handler only when log_mode is "file"
        ...
        try:
            ...
            log_filename = os.path.join(logs_dir_str, f"synth_codeai_{timestamp}.log")
            ...
            # Create an synth_codeai logger for compatibility
            logger = logging.getLogger("synth_codeai")
...
def get_logger(name: Optional[str] = None) -> logging.Logger:
    return logging.getLogger(f"synth_codeai.{name}" if name else "synth_codeai")
