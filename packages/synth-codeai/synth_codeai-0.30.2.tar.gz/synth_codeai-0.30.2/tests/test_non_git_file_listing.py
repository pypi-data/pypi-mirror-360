...
from synth_codeai.file_listing import get_file_listing
...
def test_non_git_file_listing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        synth_codeai_dir = Path(temp_dir) / ".synth-codeai"
        venv_dir = Path(temp_dir) / ".venv"
        os.makedirs(synth_codeai_dir)
        os.makedirs(venv_dir)
        
        # Create files in excluded directories
        synth_codeai_file = synth_codeai_dir / "config.json"
        venv_file = venv_dir / "activate"
        
        # Write content to all files
        for file_path in [file1, file2, file3, file4, synth_codeai_file, venv_file]:
...
def test_non_git_directory_with_excluded_dirs():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        excluded_dirs = [".synth-codeai", ".venv", ".git", ".aider", "__pycache__"]
