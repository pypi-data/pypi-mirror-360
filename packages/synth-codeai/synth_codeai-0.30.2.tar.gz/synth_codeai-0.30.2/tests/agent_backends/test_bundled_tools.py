...
def test_execute_tool_bundled():
    ...
    with patch("synth_codeai.tools.reflection.get_function_info", return_value="mock function info"):
        agent = CiaynAgent(
            model=MagicMock(),
            tools=mock_tools,
        )
...
ask_expert("What does this mean?")'''
    ...
    with patch("synth_codeai.agent_backends.ciayn_agent.validate_function_call_pattern", return_value=False):
        # Execute
...
emit_key_snippet({"file": "example.py", "start_line": 10, "end_line": 20})'''
    ...
    with patch("synth_codeai.agent_backends.ciayn_agent.validate_function_call_pattern") as mock_validate:
