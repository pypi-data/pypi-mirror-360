...
from synth_codeai.project_state import (
...
@pytest.fixture
def synth_codeai_only_dir(tmp_path):
    """Create a directory with only a .synth-codeai directory."""
...
def mixed_allowed_dir(tmp_path):
    """Create a directory with all allowed items (.git, .gitignore, and .synth-codeai)."""
    git_dir = tmp_path / ".git"
    git_dir.mkdir(exist_ok=True)
    gitignore = tmp_path / ".gitignore"
    gitignore.write_text("*.pyc\n")
...
def test_synth_codeai_only_directory(synth_codeai_only_dir):
    """Test that a directory with only a .synth-codeai directory is considered a new project."""
    assert is_new_project(str(synth_codeai_only_dir)) is True
...
def test_verify_fix(tmp_path):
    """
    Verify fix: a project with only .synth-codeai directory is considered new,
    but adding other files makes it recognized as an existing project.
    """
    # Create a .synth-codeai directory inside the temporary directory
    synth_codeai_dir = tmp_path / ".synth-codeai"
    synth_codeai_dir.mkdir(exist_ok=True)

    # Check that is_new_project() returns True (only .synth-codeai directory)
