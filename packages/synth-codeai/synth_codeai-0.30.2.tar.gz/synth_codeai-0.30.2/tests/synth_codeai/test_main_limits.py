...
def mock_config_repository():
    ...
    with patch('synth_codeai.database.repositories.config_repository.config_repo_var') as mock_repo_var:
...
def test_limit_config_storage(mock_config_repository):
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.enter_context(patch('synth_codeai.database.connection.DatabaseManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.session_repository.SessionRepositoryManager.__enter__', return_value=mock_session))
        stack.enter_context(patch('synth_codeai.database.repositories.key_fact_repository.KeyFactRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.key_snippet_repository.KeySnippetRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.human_input_repository.HumanInputRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.research_note_repository.ResearchNoteRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.related_files_repository.RelatedFilesRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.trajectory_repository.TrajectoryRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.work_log_repository.WorkLogRepositoryManager.__enter__', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.database.repositories.config_repository.ConfigRepositoryManager.__enter__', return_value=mock_config_repository))
        stack.enter_context(patch('synth_codeai.env_inv_context.EnvInvManager.__enter__', return_value=MagicMock()))
        
        # Mock the repository getter functions that access contextvars
        stack.enter_context(patch('synth_codeai.__main__.get_trajectory_repository', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.__main__.get_human_input_repository', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.__main__.get_key_fact_repository', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.__main__.get_key_snippet_repository', return_value=MagicMock()))
        stack.enter_context(patch('synth_codeai.__main__.get_research_note_repository', return_value=MagicMock()))
        
        # Mock other dependencies
        stack.enter_context(patch('synth_codeai.__main__.check_dependencies'))
        stack.enter_context(patch('synth_codeai.__main__.validate_environment', return_value=(True, [], True, [])))
        stack.enter_context(patch('synth_codeai.__main__.run_research_agent'))
        stack.enter_context(patch('synth_codeai.__main__.ensure_migrations_applied', return_value=True))
        stack.enter_context(patch('synth_codeai.__main__.EnvDiscovery'))
...
def test_limit_config_storage_none_values(mock_config_repository):
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.enter_context(patch.object(sys, "argv", ["synth-codeai", "-m", "test"]))
