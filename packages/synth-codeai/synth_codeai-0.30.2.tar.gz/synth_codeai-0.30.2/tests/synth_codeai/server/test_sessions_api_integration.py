...
This module contains integration tests for the API endpoints defined in synth_codeai/server/api_v1_sessions.py.
...
from synth_codeai.server.server import app
...
def mock_sessions():
    return [
        SessionModel(
            ...
            command_line=f"synth-codeai test{i + 1}",
            program_version="1.0.0",
...
def mock_repo(mock_session, mock_sessions):
    ...
    def create_with_metadata(metadata=None):
        ...
        return SessionModel(
            ...
            updated_at=now,
            start_time=now,
            command_line="synth-codeai test-custom" if metadata else "synth-codeai test-null",
            program_version="1.0.0",
