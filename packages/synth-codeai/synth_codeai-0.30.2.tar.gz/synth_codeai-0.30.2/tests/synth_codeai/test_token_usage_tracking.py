...
def test_token_usage_storage():
    ...
    with patch('synth_codeai.agent_utils.get_config_repository') as mock_config:
        ...
        with patch('synth_codeai.callbacks.default_callback_handler.DefaultCallbackHandler', return_value=mock_cb):
                # Force completion to happen after one iteration
                with patch('synth_codeai.agent_utils.is_completed', return_value=True):
                    # Replace the trajectory repository with our mock
                    with patch('synth_codeai.agent_utils.get_trajectory_repository', return_value=mock_repo):
                        # Suppress check_interrupt and print_agent_output
                        with patch('synth_codeai.agent_utils.check_interrupt'), \
                             patch('synth_codeai.agent_utils.print_agent_output'), \
                             patch('synth_codeai.agent_utils.reset_completion_flags'):
                            ...
                            with patch('synth_codeai.agent_utils._run_agent_stream', side_effect=side_effect):
            ...
            with patch('synth_codeai.callbacks.default_callback_handler.DefaultCallbackHandler', return_value=mock_cb):
                    ...
                    with patch('synth_codeai.agent_utils.is_completed', return_value=True):
                        ...
                        with patch('synth_codeai.agent_utils.get_trajectory_repository', return_value=mock_repo):
                            ...
                            with patch('synth_codeai.agent_utils.logger.error') as mock_error_log:
