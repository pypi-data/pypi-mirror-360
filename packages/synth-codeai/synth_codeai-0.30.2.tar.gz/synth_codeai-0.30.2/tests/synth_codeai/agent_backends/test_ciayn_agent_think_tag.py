...
def mock_trajectory_repository():
    ...
    with patch("synth_codeai.callbacks.default_callback_handler.get_trajectory_repository") as mock:
        mock_repo = MagicMock()
...
def mock_session_repository():
    ...
    with patch("synth_codeai.callbacks.default_callback_handler.get_session_repository") as mock:
        mock_repo = MagicMock()
...
def test_stream_supports_think_tag():
    ...
    with patch("synth_codeai.console.formatting.print_warning"), \
         patch("synth_codeai.console.formatting.print_error"):
...
def test_stream_implicit_think_tag_support():
    ...
    with patch("synth_codeai.console.formatting.print_warning"), \
        ...
        with patch.object(agent, "_execute_tool") as mock_execute:
            ...
            # Patch where it's imported: synth_codeai.database.repositories.config_repository
            with patch("synth_codeai.database.repositories.config_repository.get_config_repository") as mock_get_repo, \
