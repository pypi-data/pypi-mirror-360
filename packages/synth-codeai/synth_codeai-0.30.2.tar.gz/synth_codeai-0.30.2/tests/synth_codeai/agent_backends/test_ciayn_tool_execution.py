...
from synth_codeai.file_listing import FileListerError
...
def test_fuzzy_find_project_files_none_args_execution():
    ...
    with patch('synth_codeai.tools.fuzzy_find.process.extract', return_value=[]):
...
def test_error_handling_with_nonexistent_path():
    ...
    with patch('synth_codeai.file_listing.get_all_project_files', side_effect=FileListerError("Directory not found")):
