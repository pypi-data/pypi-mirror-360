...
from synth_codeai.config import DEFAULT_RECURSION_LIMIT
from synth_codeai.database.repositories.work_log_repository import WorkLogEntry
...
def mock_dependencies(monkeypatch):
    ...
    monkeypatch.setattr("synth_codeai.__main__.check_dependencies", lambda: None)
    monkeypatch.setattr("synth_codeai.__main__.validate_environment", lambda args: (True, [], True, []))
    monkeypatch.setattr("synth_codeai.__main__.create_agent", lambda *args, **kwargs: None)
    monkeypatch.setattr("synth_codeai.__main__.run_agent_with_retry", lambda *args, **kwargs: None)
    monkeypatch.setattr("synth_codeai.__main__.run_research_agent", lambda *args, **kwargs: None)
    monkeypatch.setattr("synth_codeai.agents.planning_agent.run_planning_agent", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("synth_codeai.__main__.initialize_llm", mock_config_update)
...
def mock_work_log_repository():
    ...
    with patch('synth_codeai.database.repositories.work_log_repository.work_log_repo_var') as mock_repo_var:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def test_recursion_limit_in_global_config(mock_dependencies, mock_config_repository):
    ...
    with patch('synth_codeai.database.repositories.config_repository.ConfigRepositoryManager.__enter__', return_value=mock_config_repository):
        # Test default recursion limit
        with patch.object(sys, "argv", ["synth-codeai", "-m", "test message"]):
            main()
        ...
        with patch.object(sys, "argv", ["synth-codeai", "-m", "test message", "--recursion-limit", "50"]):
            main()
            # Check that the recursion_limit value was included in the update call
            mock_config_repository.update.assert_called()
            # Get the call arguments
...
def test_temperature_validation(mock_dependencies, mock_config_repository):
    ...
    with patch('synth_codeai.database.repositories.config_repository.ConfigRepositoryManager.__enter__', return_value=mock_config_repository):
        ...
        with patch("synth_codeai.__main__.initialize_llm", return_value=None) as mock_init_llm:
            # Also patch any calls that would actually use the mocked initialize_llm function
            with patch("synth_codeai.__main__.run_research_agent", return_value=None):
                with patch("synth_codeai.agents.planning_agent.run_planning_agent", return_value=None):
                    with patch.object(
                        sys, "argv", ["synth-codeai", "-m", "test", "--temperature", "0.7"]
                    ):
                        main()
    ...
    with pytest.raises(SystemExit):
        with patch.object(
            sys, "argv", ["synth-codeai", "-m", "test", "--temperature", "2.1"]
        ):
            main()
...
def test_planner_model_provider_args(mock_dependencies, mock_config_repository):
    ...
    with patch('synth_codeai.database.repositories.config_repository.ConfigRepositoryManager.__enter__', return_value=mock_config_repository):
        with patch.object(
            sys,
            "argv",
            ["synth-codeai", "-m", "test message", "--provider", "openai", "--model", "gpt-4"],
        ):
            main()
            # Verify the mock repo's set method was called with the expected values
            mock_config_repository.set.assert_any_call("planner_provider", "openai")
...
def test_use_aider_flag(mock_dependencies, mock_config_repository):
    ...
    from synth_codeai.__main__ import main
    from synth_codeai.tool_configs import MODIFICATION_TOOLS, set_modification_tools
    ...
    with patch('synth_codeai.database.repositories.config_repository.ConfigRepositoryManager.__enter__', return_value=mock_config_repository):
        ...
        with patch.object(
            sys,
            "argv",
            ["synth-codeai", "-m", "test message", "--use-aider"],
        ):
            main()
