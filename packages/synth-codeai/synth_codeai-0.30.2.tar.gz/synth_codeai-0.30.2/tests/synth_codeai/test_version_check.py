...
def test_newer_version_available(monkeypatch):
    ...
    monkeypatch.setattr('synth_codeai.version_check.current_version', '0.15.2')
    ...
    monkeypatch.setattr('synth_codeai.version_check.requests.get', mock_get)
    
    result = check_for_newer_version()
    
    # Check that the message contains the new version
    assert "0.16.0" in result
    assert "A new version of synth.codeai is available" in result
