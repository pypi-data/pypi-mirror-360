...
from synth_codeai.database.models import Session, BaseModel, HumanInput
from synth_codeai.database.repositories.session_repository import (
    SessionRepository,
    SessionRepositoryManager,
    get_session_repository,
    session_repo_var
)
from synth_codeai.database.pydantic_models import SessionModel
...
def sample_session(setup_db, test_metadata):
    ...
    with setup_db.atomic():
        return Session.create(
            ...
            updated_at=now,
            start_time=now,
            command_line="python -m synth_codeai.cli",
...
def session_with_long_command(setup_db):
    ...
    now = datetime.datetime.now()
    long_command = "python -m synth_codeai.cli " + "very_long_argument " * 20  # Will exceed 80 chars
    with setup_db.atomic():
        return Session.create(
            created_at=now,
            updated_at=now,
...
def test_session_repository_manager(setup_db, cleanup_repo):
    ...
    with patch('synth_codeai.database.repositories.session_repository.SessionRepositoryManager.__enter__', 
...
def test_display_name_from_command_line(setup_db, sample_session):
    ...
    assert session_model.display_name == "python -m synth_codeai.cli"
...
def test_display_name_from_long_command_line(setup_db, session_with_long_command):
    ...
    assert session_model.display_name.startswith("python -m synth_codeai.cli very_long_argument")
...
def test_display_name_in_get_all(setup_db):
    ...
    session1 = Session.create(
        ...
        command_line="python -m synth_codeai.cli command1",
        program_version="0.1.0",
    )
    
    session2 = Session.create(
        start_time=datetime.datetime.now() - datetime.timedelta(hours=1),
        command_line="python -m synth_codeai.cli command2",
        program_version="0.1.0",
    )
    ...
    assert session2_result.display_name == "python -m synth_codeai.cli command2"
...
def test_display_name_in_get_recent(setup_db):
    ...
    session1_result = next((s for s in sessions if s.id == session1.id), None)
    assert session1_result is not None
    assert session1_result.display_name == "python -m synth_codeai.cli command1"
