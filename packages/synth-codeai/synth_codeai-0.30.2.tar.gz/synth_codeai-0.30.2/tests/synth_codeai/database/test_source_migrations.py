...
def mock_logger():
    ...
    with patch("synth_codeai.database.migrations.logger") as mock:
        yield mock
...
class TestSourceMigrations:
    ...
    def test_migration_manager_uses_source_migrations_dir(self, temp_dir, mock_logger):
        ...
        with patch("synth_codeai.database.migrations.Router") as mock_router:
    ...
    def test_get_source_package_migrations_dir(self, temp_dir, mock_logger):
        ...
        mock_base_dir = os.path.join(temp_dir, "synth_codeai")
        ...
        with patch("synth_codeai.database.migrations.os.path.dirname") as mock_dirname:
            with patch("synth_codeai.database.migrations.os.path.abspath") as mock_abspath:
    ...
    def test_ensure_migrations_applied_creates_synth_codeai_dir(self, temp_dir, mock_logger):
        """Test that ensure_migrations_applied creates the .synth-codeai directory if it doesn't exist."""
        # Get a path to a directory that doesn't exist
        synth_codeai_dir = os.path.join(temp_dir, ".synth-codeai")
        
        # Mock getcwd to return our temp directory
        with patch("os.getcwd", return_value=temp_dir):
            # Mock DatabaseManager
            with patch("synth_codeai.database.migrations.DatabaseManager") as mock_db_manager:
                # Mock the context manager
                mock_db_manager.return_value.__enter__.return_value = MagicMock()
                mock_db_manager.return_value.__exit__.return_value = None
                
                # Mock synth_codeai package import
                with patch("synth_codeai.database.migrations.synth_codeai", create=True) as mock_synth_codeai:
                    # Set up the mock package directory path
                    mock_package_dir = os.path.join(temp_dir, "synth_codeai_package")
                    ...
                    # Configure the mock
                    mock_synth_codeai.__file__ = os.path.join(mock_package_dir, "__init__.py")
                    ...
                    with patch("synth_codeai.database.migrations.init_migrations", return_value=mock_migration_manager):
                        ...
                        # Verify .synth-codeai directory was created
                        assert os.path.exists(synth_codeai_dir)

    def test_ensure_migrations_applied_handles_directory_error(self, mock_logger):
        """Test that ensure_migrations_applied handles errors creating the .synth-codeai directory."""
        ...
        with patch("os.makedirs", side_effect=PermissionError("Permission denied")):
            ...
            mock_logger.exception.assert_called_with(
                "Failed to ensure .synth-codeai directory exists or initialize migrations: Permission denied"
            )
        ...
        with patch("os.getcwd", return_value=temp_dir):
            ...
            with patch("synth_codeai.database.migrations.DatabaseManager") as mock_db_manager:
                ...
                with patch("synth_codeai.database.migrations.synth_codeai", create=True) as mock_synth_codeai:
                    ...
                    with patch("synth_codeai.database.migrations.init_migrations", return_value=mock_init_migrations) as mock_init:
                        # Call ensure_migrations_applied
                        result = ensure_migrations_applied()
    ...
    def test_router_initialization_with_source_migrations(self, temp_dir, mock_logger):
        ...
        with patch("synth_codeai.database.migrations.Router") as mock_router_class:
