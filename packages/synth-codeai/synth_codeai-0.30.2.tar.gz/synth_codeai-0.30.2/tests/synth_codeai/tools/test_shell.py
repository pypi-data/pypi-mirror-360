...
from synth_codeai.tools.shell import run_shell_command


@pytest.fixture
def mock_console():
    with patch("synth_codeai.tools.shell.console") as mock:
        yield mock


@pytest.fixture
def mock_prompt():
    with patch("synth_codeai.tools.shell.Prompt") as mock:
        yield mock


@pytest.fixture
def mock_run_interactive():
    with patch("synth_codeai.tools.shell.run_interactive_command") as mock:
...
def test_shell_command_cowboy_message(mock_console, mock_prompt, mock_run_interactive, mock_config_repository):
    ...
    with patch("synth_codeai.tools.shell.get_cowboy_message") as mock_get_message:
