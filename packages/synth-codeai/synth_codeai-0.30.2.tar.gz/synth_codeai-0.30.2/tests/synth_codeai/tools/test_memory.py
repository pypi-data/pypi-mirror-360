...
from synth_codeai.agents.key_snippets_gc_agent import delete_key_snippets
from synth_codeai.tools.memory import (
...
from synth_codeai.utils.file_utils import is_binary_file, _is_binary_fallback
...
from synth_codeai.database.models import KeyFact
...
def mock_repository():
    ...
    with patch('synth_codeai.tools.memory.get_key_fact_repository') as mock_repo:
...
def mock_key_snippet_repository():
    ...
    with patch('synth_codeai.tools.memory.get_key_snippet_repository') as memory_mock_repo, \
         patch('synth_codeai.agents.key_snippets_gc_agent.get_key_snippet_repository') as agent_mock_repo:
...
def mock_related_files_repository():
    ...
    with patch('synth_codeai.tools.memory.get_related_files_repository') as mock_repo:
        # Setup the mock repository to behave like the original, but using memory
...
@patch('synth_codeai.agents.key_snippets_gc_agent.log_work_event')
def test_delete_key_snippets(mock_log_work_event, reset_memory, mock_key_snippet_repository):
    ...
    with patch('synth_codeai.agents.key_snippets_gc_agent.get_key_snippet_repository', mock_key_snippet_repository):
...
@patch('synth_codeai.tools.memory.is_binary_file')
...
def test_python_file_detection():
    ...
    try:
        ...
        if magic:
            ...
            with patch('synth_codeai.utils.file_utils.os.path.splitext', side_effect=mock_splitext):
                ...
                with patch('synth_codeai.utils.file_utils._is_binary_content', return_value=True):
                    ...
                    with patch('builtins.open') as mock_open:
                        ...
                        with patch('synth_codeai.utils.file_utils.magic') as mock_magic:
