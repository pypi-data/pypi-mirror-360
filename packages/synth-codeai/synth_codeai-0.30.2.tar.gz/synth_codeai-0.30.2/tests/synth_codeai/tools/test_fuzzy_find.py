...
from synth_codeai.tools import fuzzy_find_project_files
...
def test_excluding_system_dirs(non_git_repo):
    ...
    (non_git_repo / ".synth-codeai").mkdir(exist_ok=True)
    (non_git_repo / ".synth-codeai/config.json").write_text('{"setting": "value"}')
    ...
    # Should find config.py but not .synth-codeai/config.json
    assert any("config.py" in match[0] for match in results)
    assert not any(".synth-codeai/config.json" in match[0] for match in results)
