...
from synth_codeai.tools.agent import (
    request_research,
    request_task_implementation,
    request_implementation,
    request_research_and_implementation,
)
from synth_codeai.database.repositories.related_files_repository import get_related_files_repository
from synth_codeai.database.repositories.work_log_repository import get_work_log_repository, WorkLogEntry
...
def mock_related_files_repository():
    ...
    with patch('synth_codeai.database.repositories.related_files_repository.related_files_repo_var') as mock_repo_var:
...
def mock_work_log_repository():
    ...
    with patch('synth_codeai.tools.memory.get_work_log_repository') as mock_repo:
...
def mock_trajectory_repository():
    ...
    with patch('synth_codeai.database.repositories.trajectory_repository.trajectory_repo_var') as mock_repo_var:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def mock_human_input_repository():
    ...
    with patch('synth_codeai.database.repositories.human_input_repository.human_input_repo_var') as mock_repo_var:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def mock_functions():
    ...
    with patch('synth_codeai.tools.agent.get_key_fact_repository', return_value=mock_fact_repo) as mock_get_fact_repo, \
         patch('synth_codeai.tools.agent.format_key_facts_dict') as mock_fact_formatter, \
         patch('synth_codeai.tools.agent.get_key_snippet_repository', return_value=mock_snippet_repo) as mock_get_snippet_repo, \
         patch('synth_codeai.tools.agent.format_key_snippets_dict') as mock_snippet_formatter, \
         patch('synth_codeai.tools.agent.initialize_llm') as mock_llm, \
         patch('synth_codeai.tools.agent.get_related_files') as mock_get_files, \
         patch('synth_codeai.tools.agent.get_work_log') as mock_get_work_log, \
         patch('synth_codeai.tools.agent.reset_completion_flags') as mock_reset, \
         patch('synth_codeai.tools.agent.get_completion_message') as mock_get_completion, \
         patch('synth_codeai.tools.agent.get_trajectory_repository') as mock_get_trajectory_repo, \
         patch('synth_codeai.tools.agent.get_human_input_repository') as mock_get_human_input_repo:
...
def test_request_research_uses_key_fact_repository(reset_memory, mock_functions):
    ...
    with patch('synth_codeai.agents.research_agent.run_research_agent'):
        # Call the function
...
def test_request_research_max_depth(reset_memory, mock_functions):
    ...
    with patch('synth_codeai.tools.agent.get_depth') as mock_get_depth:
...
def test_request_implementation_uses_key_fact_repository(reset_memory, mock_functions):
    ...
    with patch('synth_codeai.agents.planning_agent.run_planning_agent'):
        # Call the function
...
def test_request_task_implementation_uses_key_fact_repository(reset_memory, mock_functions):
    ...
    with patch('synth_codeai.agents.implementation_agent.run_task_implementation_agent'):
        # Call the function
