"""
Global pytest fixtures for synth-codeai tests.
...
def mock_trajectory_repository():
    ...
    with patch(
        "synth_codeai.database.repositories.trajectory_repository.TrajectoryRepository"
    ) as mock:
...
def mock_human_input_repository():
    ...
    with patch(
        "synth_codeai.database.repositories.human_input_repository.HumanInputRepository"
    ) as mock:
        # Setup a mock repository
        mock_repo = MagicMock()
...
def mock_session_repository():
    ...
    with patch(
        "synth_codeai.database.repositories.session_repository.SessionRepository"
    ) as mock:
        # Setup a mock repository
        mock_repo = MagicMock()
        ...
        # Set the contextvar
        from synth_codeai.database.repositories.session_repository import session_repo_var
...
def mock_repository_access(
    ...
    with patch(
        "synth_codeai.database.repositories.trajectory_repository.get_trajectory_repository",
        return_value=mock_trajectory_repository,
    ):
        with patch(
            "synth_codeai.database.repositories.human_input_repository.get_human_input_repository",
            return_value=mock_human_input_repository,
        ):
            with patch(
                "synth_codeai.database.repositories.session_repository.get_session_repository",
...
def isolated_db_environment(tmp_path, monkeypatch):
    ...
    actual .synth-codeai directory in the current working directory.
    ...
    # Create the .synth-codeai directory in the temporary path
    synth_codeai_dir = tmp_path / ".synth-codeai"
    synth_codeai_dir.mkdir(exist_ok=True)
