...
/**
 * WebviewViewProvider implementation for the synth.codeai panel
 */
class RAWebviewViewProvider implements vscode.WebviewViewProvider {
  ...
  private _getHtmlForWebview(webview: vscode.Webview): string {
    ...
    return `<!DOCTYPE html>
      ...
      <body>
        <div class="container">
          <img src="${logoUri}" alt="synth.codeai Logo" class="logo">
          <h1>synth.codeai</h1>
...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log('Congratulations, your extension "synth-codeai" is now active!');

  // Register the WebviewViewProvider
  const provider = new RAWebviewViewProvider(context.extensionUri);
  const viewRegistration = vscode.window.registerWebviewViewProvider(
    'synth-codeai.view', // Must match the view id in package.json
    provider
  );
  ...
  const disposable = vscode.commands.registerCommand('synth-codeai.helloWorld', () => {
    // The code you place here will be executed every time your command is executed
    // Display a message box to the user
    vscode.window.showInformationMessage('Hello World from synth.codeai!');
  });
