    ...
    def index(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        if ('path' in kwargs and 'server/status' in kwargs['path'] ):
            ...
            if is_auth == 'no_reports_view_permission':
                logger.info("OAP DASH Access denied: You are authenticated but lack the required reports permissions.")
                print("OAP DASH Access denied: You are authenticated but lack the required reports permissions.")
                return flask.Response('OAP DASH OapDash Access denied: You are authenticated but lack the required reports permissions.', status=403)
