...
from .inspect import (
    is_hero_event, is_local_only, is_force_remote, is_hero_method,
    local_only, force_remote, mark_hero_method
)
from heros import serdes
...
class HEROPeer:
    ...
    def _query_selector(self, *args, **kwargs) -> list:
        ...
        for reply in replies:
            try:
                if not reply.err:
                    log.debug(f"Received ('{reply.ok.key_expr}': '{reply.ok.payload.to_bytes()}') of kind"
    ...
    def _get_object_info(self, object_name: str, timeout: float = 10.0) -> dict:
        ...
        """
        ro_descriptors = self._query_selector(f"{self._ns_objects}/{self._realm}/{object_name}/{self._ep_discover}",
                                              target=zenoh.QueryTarget.ALL, timeout=timeout)
...
class RemoteHERO(HERO):
    ...
    def __init__(self, name: str, realm: str = "heros", *args, **kwargs):
        ...
        if self._name not in object_info:
            raise NameError(f"Remote Object with name {self._name} not found. Available objects are"
                            f"{list(object_info.keys())}")
        ...
        self._liveliness_subscription = self._session.liveliness().declare_subscriber(self._endpoints["health"],
                                                                                      self._liveliness_changed)
    ...
    def _get_capabilities(self):
        ...
        """
        return [Capability.from_dict(cap_dict)for cap_dict in self._query_endpoint(self._endpoints["capabilities"])]
...
class LocalHERO(HERO):
    ...
    """
    def __init__(
            self, name: str, *args, realm: str = "heros",
            implements: list[str] | None = None, tags: list[str] | None = None,
            **kwargs):
        ...
        def capabilities_callback(query):
            query.reply(self._endpoints["capabilities"], self._serialize([cap.to_dict() for cap in self.capabilities]),
            ...
            if callable(getattr(self, member_name)):
                ...
                if is_hero_event(getattr(self, member_name)):
                    ...
                    def wrapper(_cap, query):
                        ...
                        try:
                            ...
                            query.reply(f"{self._endpoint_base_path}/{_cap.name}", self._serialize(return_value),
                                        encoding=self._default_encoding)
                        except Exception as e:
                            query.reply_err(self._serialize(str(e) + "\n\n" +
                ...
                def wrapper(_cap, query):
                    if query.payload:
                        ...
                        # send back the result
                        query.reply(f"{self._endpoint_base_path}/{_cap.name}",
...
class HEROObserver(HEROPeer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._session.liveliness().declare_subscriber(f"{self._ns_objects}/{self._realm}/*/{self._ep_health}",
