...
def add_test_files(doctest_namespace):
    ...
    doctest_namespace["param_file"] = os.path.join(TEST_DATA, "nepal.param")
    doctest_namespace["quakeml_file"] = os.path.join(TEST_DATA, "quake.xml")
...
def repack_databases():
    ...
    try:
        ...
        return {"root_folder": None, "databases": {}}
    ...
    repack_file(
        input_filename=px_tr,
        output_filename=px_tr_and_back,
        contiguous=False,
        compression_level=4,
        quiet=True,
        transpose=True,
    ...
    repack_file(
        input_filename=pz_tr,
        output_filename=pz_tr_and_back,
        contiguous=False,
        compression_level=4,
        quiet=True,
        transpose=True,
    ...
    merged_bw_db = os.path.join(root_folder, "merged_100s_db_bwd_displ_only")
    os.makedirs(merged_bw_db)
    print("Creating a merged test database ...")
    merge_files(
        filenames=[px, pz],
        output_folder=merged_bw_db,
        contiguous=True,
        compression_level=None,
        quiet=True,
    )
    ...
    merged_fwd_db = os.path.join(root_folder, "merged_100s_db_fwd")
    ...
    with h5py.File(
        os.path.join(merged_transposed_bw_db, "merged_output.nc4"), "r"
    ) as f:
        merged_tr_shape = f["MergedSnapshots"].shape
    with h5py.File(
        os.path.join(horizontal_only_merged_db, "merged_output.nc4"), "r"
    ) as f:
        horizontal_only_merged_tr_shape = f["MergedSnapshots"].shape
    with h5py.File(
        os.path.join(vertical_only_merged_db, "merged_output.nc4"), "r"
    ) as f:
        vertical_only_merged_tr_shape = f["MergedSnapshots"].shape
    with h5py.File(os.path.join(merged_fwd_db, "merged_output.nc4"), "r") as f:
    ...
    assert horizontal_only_merged_tr_shape == (192, 3, 5, 5, 73), str(
        horizontal_only_merged_tr_shape
    )
    assert vertical_only_merged_tr_shape == (192, 2, 5, 5, 73), str(
        vertical_only_merged_tr_shape
    )
    assert merged_fwd_shape == (192, 10, 5, 5, 73), str(merged_fwd_shape)
    ...
    return {"root_folder": root_folder, "databases": dbs}
...
try:
    ...
    raise Exception(
        "pytest-xdist is required to run the tests. Please "
