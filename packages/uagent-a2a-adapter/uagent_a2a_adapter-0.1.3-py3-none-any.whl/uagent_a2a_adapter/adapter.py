...
class A2AAdapter:
                ...
                if isinstance(item, TextContent):
                    ...
                    try:
                        ...
                        if not best_agent:
                            ...
                            ctx.logger.info(f"🤖 A2A Response from {best_agent.get('name', 'unknown agent')}: "
                                          f"{response[:100]}...")
            ...
            for config in self.agent_configs:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if ctx:
                            ctx.logger.warning(f"❌ Agent {config.name} health check failed: HTTP {response.status_code}")
                        else:
                            print(f"❌ Agent {config.name} health check failed: HTTP {response.status_code}")
    ...
    async def _route_by_keywords(self, query: str, agents: List[Dict], ctx: Context) -> Optional[Dict[str, Any]]:
        ...
        for agent in agents:
            ...
            for specialty in specialties:
                ...
                if common_words:
                    ...
                    score += word_score
                    ctx.logger.info(f"   🎯 {agent_name}: specialty words {common_words} matched (+{word_score})")
            ...
            for skill in skills:
                ...
                if common_words:
                    ...
                    score += word_score
                    ctx.logger.info(f"   🎯 {agent_name}: skill words {common_words} matched (+{word_score})")
    ...
    async def _llm_route_query(self, query: str, agents: List[Dict], ctx: Context) -> Optional[Dict[str, Any]]:
        """Use LLM to intelligently route the query to the most suitable agent."""
        try:
            # Create agent descriptions for the LLM
            agent_descriptions = []
            for i, agent in enumerate(agents):
                agent_desc = f"{i+1}. {agent.get('name', 'Unknown')}: {agent.get('description', 'No description')} - Specializes in: {', '.join(agent.get('specialties', []))}"
...
Response format: Just return the number (e.g., "2")"""
            ...
            headers = {
                ...
                'Authorization': 'Bearer sk_5822be6c948d4a25830382b308f6f51ee13629ffe6fc4dc7a064cc26e486df84'
            }
                ...
                if 'choices' in result and len(result['choices']) > 0:
                    ...
                    try:
                        ...
                        if 0 <= agent_index < len(agents):
                            ...
                            ctx.logger.info(f"🎯 LLM selected agent: {selected_agent.get('name')} (index: {agent_index + 1})")
                            return selected_agent
                        else:
                            ctx.logger.warning(f"🚨 LLM returned invalid agent index: {agent_index + 1}")
                    except ValueError:
                        ctx.logger.warning(f"🚨 LLM returned non-numeric response: '{llm_response}'")
                else:
                    ctx.logger.warning("🚨 LLM response missing choices")
            else:
                ctx.logger.warning(f"🚨 LLM API call failed: {response.status_code} - {response.text}")
    ...
    async def _route_round_robin(self, agents: List[Dict], ctx: Context) -> Optional[Dict[str, Any]]:
