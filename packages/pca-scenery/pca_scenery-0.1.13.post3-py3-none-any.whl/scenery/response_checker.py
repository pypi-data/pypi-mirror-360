...
class Checker:
    ...
    # NOTE mad: the first functions takes a Take 
    ...
    def check_redirect_url(
        ...
        if config.framework == Framework.DJANGO.value and isinstance(response, django.http.HttpResponseRedirect):
            testcase.assertEqual(
                response.url,
                args,
            ...
            testcase.fail(f"Expected response of type HttpResponseRedirect or requests.Response but got {type(response)}")
    ...
    def check_json(
        ...
        if config.framework == Framework.DJANGO.value :
            ...
            # print(args)
            # print(response_json)
            # testcase.assertEqual(dir(response), "lala")
