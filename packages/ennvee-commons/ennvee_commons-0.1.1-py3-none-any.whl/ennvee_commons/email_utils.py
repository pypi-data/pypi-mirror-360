...
This module provides a utility function to send emails via Microsoft Graph API,
supporting features like multiple recipients (To, CC, BCC), attachments, and
content formatting (HTML or plain text).

Key functionality:
- Authenticate using Azure AD credentials (client_id, tenant_id, client_secret)
- Compose email with subject, body, and optional attachments
- Send email from a registered user mailbox
- Optionally save sent message to Sent Items folder
