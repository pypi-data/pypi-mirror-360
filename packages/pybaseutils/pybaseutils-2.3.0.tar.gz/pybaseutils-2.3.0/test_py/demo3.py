...
def process_gray_image(image, scale=(0.96, 0.96), vis=True):
    """
    :param image: BGR原始图像
    :param scale: 缩放大小
    :param vis: 可视化效果
    :return: dest_： 返回修复后的原图
             gray2： 返回修复后的灰度图
    """
    h, w = image.shape[:2]
    gray1 = 255 - cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    mask1 = image_utils.get_image_mask(gray1, inv=False)
    point = image_utils.find_mask_contours(mask1, mode=cv2.RETR_EXTERNAL)
    boxes = image_utils.points2boxes(point)
    boxes = image_utils.extend_xyxy(boxes, scale=scale)
    box2 = boxes[0]
    # box2 = [50, 50, 200, 200]
    crop = image_utils.get_box_crop(image, box2)
    dest_ = image_utils.get_box_crop_recover(crop, box2, size=(w, h), borderType=cv2.BORDER_REPLICATE)
...
def test_image_dir(image_dir, vis=True):
    ...
    for image_file in image_list:
        ...
        dest, gray = process_gray_image(image, vis=vis)
