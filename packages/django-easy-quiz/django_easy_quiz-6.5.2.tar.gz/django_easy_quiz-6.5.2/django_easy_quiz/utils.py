...
def get_quiz_and_formset(QuizType, FormType, pk):
    ...
    try:
        ...
        for question in questions:
            answers = []
            if question.answer_1 not in empty_answers:
                answers.append({"label": question.answer_1, "id": 1, "symbol": "■"})
            if question.answer_2 not in empty_answers:
                answers.append({"label": question.answer_2, "id": 2, "symbol": "▲"})
            if question.answer_3 not in empty_answers:
                answers.append({"label": question.answer_3, "id": 3, "symbol": "◆"})
            if question.answer_4 not in empty_answers:
        ...
        for question in questions:
            ...
            if question.answer_5 not in empty_answers:
                answers.append({"label": question.answer_5, "id": 5})
            if question.answer_6 not in empty_answers:
                answers.append({"label": question.answer_6, "id": 6})
            if question.answer_7 not in empty_answers:
                answers.append({"label": question.answer_7, "id": 7})
            if question.answer_8 not in empty_answers:
                answers.append({"label": question.answer_8, "id": 8})
            if question.answer_9 not in empty_answers:
                answers.append({"label": question.answer_9, "id": 9})
            if question.answer_10 not in empty_answers:
                answers.append({"label": question.answer_10, "id": 10})
            if quiz.answers_random:
                shuffle(answers)
            initial_data.append({"question": question, "answers": answers})
