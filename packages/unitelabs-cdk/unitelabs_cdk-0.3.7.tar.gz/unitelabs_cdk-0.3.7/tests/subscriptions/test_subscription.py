...
def bg_cancel(subscription: Subscription, timeout: float = ONE_OP_TIME * 5):
    """Cancel the subscription after a delay, default ~5 operations."""

    asyncio.get_running_loop().call_later(timeout, subscription.cancel)
...
class TestCancel:
    ...
    async def test_should_stop_iterating_after_cancel_in_bg_task(self):
        ...
        bg_cancel(sub, ONE_OP_TIME * N_OPS)
        await asyncio.sleep(0)
