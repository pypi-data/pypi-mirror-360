...
class LiteLLMEmbeddingModel(BaseEmbeddingModel):
    ...
    def __init__(self, model_key: str):
        ...
        logger.info(
            f"Initialized LiteLLM embedding model with model_key={model_key}, "
    ...
    async def embed_objects(self, texts: List[str]) -> List[List[float]]:
        """
        Generate embeddings for a list of objects using LiteLLM or direct Ollama client.
    ...
    async def embed_query(self, text: str) -> List[float]:
        """
        Generate an embedding for a single query using LiteLLM or direct Ollama client.
