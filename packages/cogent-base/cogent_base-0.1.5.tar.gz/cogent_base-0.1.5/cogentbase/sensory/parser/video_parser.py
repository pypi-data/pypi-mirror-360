...
from cogentbase.config.main import get_cogent_config
from cogentbase.models.video import ParseVideoResult, TimeSeriesData
...
class VisionModelClient:
    def __init__(self):
        ...
        if (
            not hasattr(self.settings.llm, "registered_models")
            or self.model_key not in self.settings.llm.registered_models
        ):
            raise ValueError(f"Model '{self.model_key}' not found in registered_models configuration")

        self.model_config = self.settings.llm.registered_models[self.model_key]
    ...
    async def get_frame_descriptions(self) -> TimeSeriesData:
        ...
        while True:
            ...
            if frame_count % self.frame_sample_rate == 0:
                ...
                context = f"""Describe this frame from a video. Focus on the main elements, actions,
                and any notable details. Here is the transcript around the time of the frame:
                ---
                ...
                ---
                {last_description if last_description else
                    'No previous frame description available, this is the first frame.'}
                ---
