...
def executeTemplate(data, searchPath="", useAdaptiveSize=False, useAdaptiveDuration=False, useHardwareEncode=True, printLog=True, oneVideoTimeout=240, useHardwareDecode=True):
...
def generateTemplate(config, output, searchPath, useHardwareEncode=True, printLog=True, useHardwareDecode=True): 
    binary_dir, binary_file = getBinary(searchPath, useHardwareEncode)
    if len(binary_dir) <= 0:
        raise Exception("binary not found")
