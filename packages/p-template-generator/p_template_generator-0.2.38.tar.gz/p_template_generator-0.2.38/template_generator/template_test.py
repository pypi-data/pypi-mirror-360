...
def test(searchPath):
    ...
    template.executeTemplate(config, searchPath, useAdaptiveDuration=False, useAdaptiveSize=False, printLog=True)
    img_success = False
    ...
    template.generateTemplate(os.path.join(test_template_dir, "param.config"), tp_dir, searchPath, printLog=True)
