...
class ANKPredictor:
    ...
    def __init__(self, model_type: str = 'tall', config_file: str = None):
        """初始化预测器
        ...
        self.config = ANKMLConfig()  # 使用默认配置
    ...
    def _load_model(self):
        ...
        model_path = self.loader.get_model_path(self.model_type)
        
        if not model_path:
            # 尝试下载模型
            try:
                model_path = self.loader.download_model(self.model_type)
    ...
    def get_model_version(self) -> str:
        """获取当前模型版本"""
        model_config = self.config.get_model_config(self.model_type)
        return model_config.get('version', 'unknown')
