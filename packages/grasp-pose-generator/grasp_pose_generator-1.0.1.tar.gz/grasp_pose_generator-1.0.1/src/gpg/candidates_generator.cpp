...
std::vector<Grasp> CandidatesGenerator::generateGraspCandidates(const CloudCamera& cloud_cam)
{
  ...
  std::vector<Grasp> candidates_select;
  ...
  if (candidates.size() <= params_.max_grasp_num_ or params_.max_grasp_num_ == 0)
  {
    candidates_select = candidates;
  }
  else
  {
    for (int i = 0; i < params_.max_grasp_num_; i++)
    {
        candidates_select.push_back(candidates[i]);
    }
  }
  std::cout << "Selected " << candidates_select.size() << " grasp candidates.\n";
  if (params_.plot_grasps_)
  {
    const HandSearch::Parameters& params = hand_search_->getParams();
    plotter_.plotFingers3D(candidates_select, cloud_cam.getCloudOriginal(), "Grasp Candidates", params.hand_outer_diameter_,
      params.finger_width_, params.hand_depth_, params.hand_height_);
  }

  return candidates_select;
}
