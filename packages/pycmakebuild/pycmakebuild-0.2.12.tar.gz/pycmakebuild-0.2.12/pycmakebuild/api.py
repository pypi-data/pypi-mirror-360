...
# 环境变量初始化函数，需在主程序入口调用，支持自定义env_path
def init_env(env_path: str = ".env"):
    from dotenv import load_dotenv
    abs_env_path = env_path
    if not os.path.isabs(env_path):
        abs_env_path = os.path.join(os.getcwd(), env_path)
    if not load_dotenv(abs_env_path):
        raise Exception(
            f"未找到.env文件({abs_env_path})或加载环境变量失败.\n"
            "使用pycmakebuild --init进行初始化(初始化或修改.env后，需要重启命令行才能生效)\n"
...
def load_env_globals():
    ...
    # 判断当前操作系统是否为Windows，影响CMake参数拼接和命令行行为。
    print(f"操作系统({sys.platform}):IS_WINDOWS={IS_WINDOWS}")

    INSTALL_PATH = os.getenv("INSTALL_PATH")
    # 检查并获取安装路径环境变量，未设置时报错。
    ...
    ARCH = os.getenv("ARCH")
    # 检查并获取架构环境变量，未设置时报错。自动推断CMAKE_ARCH参数。
