...
def clean_git_source(src_dir: str, install_dir: str, other_cleans: list = []):
...
# 供外部调用
__all__ = ["build_and_install", "clean_git_source", "BuildType"]

env_path = f"{os.getcwd()}/.env"
if not load_dotenv(env_path):
    raise Exception(
        f"未找到{env_path}文件或加载环境变量失败, 请使用pycmakebuild --init进行初始化。初始化或修改.env后，需要重启命令行才能生效。"
    )
else:
