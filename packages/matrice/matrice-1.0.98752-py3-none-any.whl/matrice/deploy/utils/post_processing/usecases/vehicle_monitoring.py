        ...
        try:
            ...
            if config.enable_tracking:
                try:
                    ...
                    self.logger.debug(f"Tracker input: {vehicle_processed_data}")
                    vehicle_processed_data = self.tracker.update(vehicle_processed_data)
                    self.logger.debug(f"Tracker output: {vehicle_processed_data}")
                except Exception as e:
                    self.logger.warning(f"AdvancedTracker failed: {e}. Setting track_id to None.")
