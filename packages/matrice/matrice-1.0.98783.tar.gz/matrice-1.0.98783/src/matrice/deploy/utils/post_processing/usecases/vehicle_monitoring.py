...
class VehicleMonitoringUseCase(BaseProcessor):
    ...
    @staticmethod
    def _deduplicate_vehicles(detections, iou_thresh=0.5):
    ...
    def __init__(self):
        ...
        self._track_merge_iou_threshold: float = 0.3  # IoU ≥ 0.05 → same vehicle
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None, stream_info: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        ...
        processed_data = self._deduplicate_vehicles(processed_data, iou_thresh=0.5)
