...
class Field(BaseField):
    ...
    def _evaluateconditions(self, computed: t.Dict[str, t.Any], data: t.Any = None) -> t.Dict[str, t.Any]:
        ...
        # get fields own source value if selfdependent
        fieldvalue = None
        if (
            self.selfdependent and
            self.source is not None and
            data is not None
        ):
            fieldvalue = self._getsourcevalue(data)


        depvals = [] # dependency values
        if self.selfdependent:
            depvals.append(fieldvalue)
        ...
        if self.conditional:
            if computed is None:
                ...
                if self.required:
                    ...
                    return self.default
            evaluated = self._evaluateconditions(computed, data)
