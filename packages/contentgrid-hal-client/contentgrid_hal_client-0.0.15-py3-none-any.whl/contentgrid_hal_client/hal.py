...
class HALLink:
    def __init__(self, uri: str, name: Optional[str] = None, title: Optional[str] = None, link_relation: Optional[str] = None, templated : bool = False) -> None:
...
class HALResponse:
    def __init__(self, data: dict, curie_registry: CurieRegistry = None) -> None:
        ...
        if "_templates" in data.keys():
            self.templates: dict[str, HALFormsTemplate] = {template_name : HALFormsTemplate.deserialize(value) for template_name, value in data["_templates"].items()}
        else:
            self.templates = {}
