"""
Adapter for Littercoder FLUX Kontext Pro API service provider.
...
class LittercoderFluxKontextProTextToImageAdapter(ILittercoderAdapter):
    """Adapter for FLUX Kontext Pro text-to-image generation using Littercoder API."""
    
    def convert_input(self, data) -> dict:
        """Convert Sunra TextToImageInput to Littercoder LittercoderFluxKontextProInput format."""
        input_model = TextToImageInput.model_validate(data)
        
        littercoder_input = LittercoderFluxKontextProInput(
    ...
    def get_request_endpoint(self) -> str:
        """Get the request endpoint for FLUX Kontext Pro."""
        return "/flux/v1/flux-kontext-pro"
    
    def get_status_endpoint(self, task_id: str) -> str:
        """Get the status endpoint for FLUX Kontext Pro."""
...
class LittercoderFluxKontextProImageToImageAdapter(ILittercoderAdapter):
    """Adapter for FLUX Kontext Pro image-to-image generation using Littercoder API."""
    
    def convert_input(self, data) -> dict:
        """Convert Sunra ImageToImageInput to Littercoder LittercoderFluxKontextProInput format."""
