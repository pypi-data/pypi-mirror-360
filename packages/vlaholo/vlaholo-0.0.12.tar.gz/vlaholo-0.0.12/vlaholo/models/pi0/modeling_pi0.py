...
from vlaholo.constants import ACTION, OBS_STATE
from vlaholo.models.normalize import Normalize, Unnormalize
...
from vlaholo.utils.utils import get_safe_dtype
from loguru import logger
import re
from vlaholo.utils.policies_utils import normalize_batch_key
...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        if os.path.exists('checkpoints/paligemma-3b-pt-224'):
            pretrained_model_p = 'checkpoints/paligemma-3b-pt-224'
        else:
            logger.info(f'checkpoints/paligemma-3b-pt-224 not found! download from google/paligemma-3b-pt-224')
            pretrained_model_p = 'google/paligemma-3b-pt-224'
        self.language_tokenizer = AutoTokenizer.from_pretrained(pretrained_model_p)
        self.model = PI0FlowMatching(config)
        logger.info(f'image features order: {self.config.image_features}')
    ...
    def _transform_state_dict_keys(self, state_dict: dict) -> dict:
        ...
        transformed_dict = {}
        logger.info(f'transforming state dict for transformers>=4.52.0 in paligemma changes model names...')
    ...
    @torch.no_grad
    def select_action(
        self, batch: dict[str, Tensor], noise: Tensor | None = None, n_steps_out=None, verbose=False
    ) -> Tensor:
        ...
        """
        self.eval()
        batch = normalize_batch_key(batch)
        ...
        if len(self._action_queue) == 0:
            ...
            if verbose:
                logger.info(f'batch: {batch} state: {state}')
            ...
            if verbose:
                logger.info(f'actions: {actions.shape}, original_action_dim: {original_action_dim}')
            ...
            if verbose:
                logger.info(f'final actions: {actions.shape}')
            if n_steps_out is not None and n_steps_out > 0:
                return actions
    ...
    def prepare_language(self, batch) -> tuple[Tensor, Tensor]:
        ...
        # print(f'tasks: {tasks} {self.config.tokenizer_max_length}')
        # assert all(isinstance(task, str) for task in tasks), f"Some task is not a string: {tasks}"
    ...
    def prepare_action(self, batch):
        """Pad action"""
        # s101 before action: torch.Size([12, 50, 6]) 32
        # koch: before action: torch.Size([12, 50, 6]) 32
        # logger.info(f'before action: {batch[ACTION].shape} {self.config.max_action_dim}')
        actions = pad_vector(batch[ACTION], self.config.max_action_dim)
        # logger.info(f'after actions: {actions.shape}')
        return actions
...
class PI0FlowMatching(nn.Module):
    ...
    def sample_noise(self, shape, device):
        noise = torch.normal(
            ...
            size=shape,
            # dtype=torch.float32,
            dtype=self.action_in_proj.dtype,
            device=device,
        )
        return noise
    ...
    def sample_actions(
        ...
        # print(f'images: {images}, img_masks: {img_masks}')
