...
from quarkupy import Quark, AsyncQuark, DefaultAioHttpClient
from quarkupy._utils import is_dict
...
logging.getLogger("quarkupy").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Quark]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Quark(base_url=base_url, api_key=api_key, cookie=cookie, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncQuark]:
    ...
    async with AsyncQuark(
        base_url=base_url, api_key=api_key, cookie=cookie, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
