...
class CrystalStructureGenerator:
    ...
    def __init__(self, config: dict, out_dir: Path):
        ...
        # Ruta al dump de defecto
        self.path_defect = Path(config["defect"])

        # Leer tipo y parámetros de la red
    ...
    def _read_defect_box(self):
        ...
        """
        if not self.path_defect.is_file():
            raise FileNotFoundError(f"No se encontró: {self.path_defect}")
        with open(self.path_defect, 'r') as f:
            lines = f.readlines()
        # Busca línea 'ITEM: BOX BOUNDS'
        ...
        # Parsear tres líneas siguientes
        bounds = []
        for line in lines[idx+1:idx+4]:
            lo, hi = map(float, line.split()[0:2])
            bounds.extend([lo, hi])
    ...
    def generate(self) -> Path:
        ...
        """
        # Generar coords y dimensiones de la réplica en su propio sistema
        coords, dims = self._build_replica(self.reps)
        # Centrar réplica en origen: centro local (dims/2)
        coords_centered = coords - dims/2
        # Luego trasladar para alinear su origen al box_center
        coords_aligned = coords_centered + self.box_center
        # Definir nueva caja centrada en box_center con semidimensiones dims/2
        half = dims/2
        box = (
    ...
    def _build_replica(self, reps: Tuple[int,int,int]) -> Tuple[np.ndarray, np.ndarray]:
        """
        Crea coordenadas únicas de la red y retorna (coords, dims),
        donde dims = reps * lattice.
        """
        # Base de celda según tipo
    ...
    def _write_dump(self, coords: np.ndarray, box: Tuple[float], out_file: Path):
        """
        Escribe el archivo LAMMPS dump con coords y box bounds.
        """
