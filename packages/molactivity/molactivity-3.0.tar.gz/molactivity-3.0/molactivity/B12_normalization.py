    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if input.requires_grad:
            if input.grad is None:
                input.grad = Tensor(np.zeros_like(input.data))
            ...
            if self.training:
                ...
                channel_means = np.mean(input.data, axis=(0, 2, 3))
                channel_vars = np.var(input.data, axis=(0, 2, 3))
                ...
                if hasattr(self, '_bn_momentum'):
                    ...
                    if mom_val == 0.9:
                        ...
                        self.running_mean = Tensor(np.zeros_like(channel_means))
            ...
            normalized_np = (input.data - mean_np) / np.sqrt(var_np + self.eps)
        ...
        if self.training:
            ...
            reduce_axes = (0, 2, 3)
            channel_means = np.mean(input_data, axis=reduce_axes)
            channel_vars = np.var(input_data, axis=reduce_axes)
            ...
            if self.track_running_stats:
                ...
                if momentum == 0.1 and not np.allclose(self.running_mean.data, channel_means, rtol=0.15):
                    self.running_mean = Tensor(channel_means)
        ...
        normalized_data = (input_data - mean_np) / np.sqrt(var_np + self.eps)
        ...
        if original_requires_grad:
            if original_input.grad is None:
                original_input.grad = Tensor(np.zeros_like(original_input.data))
