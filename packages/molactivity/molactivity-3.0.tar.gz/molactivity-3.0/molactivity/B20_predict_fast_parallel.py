...
def fast_parallel_prediction(model_files, dataset_file='predicting_dataset.csv', 
    ...
    if max_workers == 1 or len(model_files) == 1:
        for args in task_args:
            result = _worker_fast_predict_single_model(args)
        ...
        try:
            with mp.Pool(processes=max_workers) as pool:
                results = pool.map(_worker_fast_predict_single_model, task_args)
...
def generate_fast_parallel_ensemble_predictions(model_files, dataset_file='predicting_dataset.csv', 
                                               max_workers=None, optimization_config=None, 
                                               batch_size=64):
    if optimization_config is None:
        optimization_config = {
    ...
    successful_results, failed_results = fast_parallel_prediction(
