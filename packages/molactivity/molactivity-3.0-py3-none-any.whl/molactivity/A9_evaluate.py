...
def save_predictions_to_csv(predictions, output_file='evaluating_dataset_with_predictions.csv', std_devs=None, individual_predictions=None, input_file=None):

    try:
        if input_file is None:
            input_file = 'evaluating_dataset.csv'
...
def analyze_prediction_quality(output_file='evaluating_dataset_with_predictions.csv'):
    print("\nResults analysis:")
    
    try:
        data = dp.read_csv(output_file)
        
        prediction_column = 'ENSEMBLE_PREDICTION' if 'ENSEMBLE_PREDICTION' in data.columns else 'PREDICTION'
        
        if prediction_column not in data.columns or 'ACTIVITY' not in data.columns:
            print(f"missing {prediction_column} or ACTIVITY")
            return
            
        predictions = [data[prediction_column][i] for i in range(len(data))]
...
def main(device='cpu', model_file='model_1.dict', models='auto', ensemble=True):
    ...
    try:
        ...
        config_parser.add_argument('--parallel', action='store_true',
                                 help='using parallel evaluation (default: one by one)')
        config_parser.add_argument('--max_workers', type=int, default=None,
                                 help='max_workers for parallel evaluation (default: auto)')
        ...
        if len(sys.argv) > 1: 
            ...
            if parallel:
                print(f"parallel evaluation: enabled")
                if max_workers:
    ...
    input_dataset_file = 'evaluating_dataset.csv'  
