...
def parallel_evaluation(model_files, dataset_file='predicting_dataset.csv', max_workers=None, use_gpu=False):
    ...
    if max_workers == 1 or len(model_files) == 1:
        for args in task_args:
            result = _worker_evaluate_single_model(args)
        ...
        with mp.Pool(processes=max_workers) as pool:
            results = pool.map(_worker_evaluate_single_model, task_args)
...
def generate_parallel_ensemble_predictions(model_files, dataset_file='predicting_dataset.csv', 
    ...
    successful_results, failed_results = parallel_evaluation(
