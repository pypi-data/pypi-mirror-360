from .B2_arrays import Array, array, zeros, maximum, sqrt
    ...
    def state_dict(self) -> Dict:
        ...
        for param in self.params:
            ...
            if param.id in self.state:
                serializable_state[pid] = {k: (v.copy() if hasattr(v, 'copy') else v) for k, v in self.state[param.id].items() if isinstance(v, (np.ndarray, float, int, type(None)))}
        return {
...
class Adam:
    def __init__(self, params: List['Tensor'], lr: float = 1e-3, 
        ...
        for p in self.params:
            self.state[p] = {
                ...
                'max_exp_avg_sq': np.zeros_like(p.data) if amsgrad else None
            }
    ...
    def state_dict(self) -> Dict:
        ...
        for param in self.params:
            ...
            if param in self.state:
                serializable_state[pid] = {k: (v.copy() if hasattr(v, 'copy') else v) for k, v in self.state[param].items() if isinstance(v, (np.ndarray, float, int, type(None)))}
        return {
            'state': serializable_state,
            't': self.t,
            'lr': self.lr,
    ...
    def load_state_dict(self, state_dict: Dict) -> None:
        ...
        self.l1_weight = state_dict.get('l1_weight', 0.0)
    ...
    def step(self) -> None:
        ...
        for param in self.params:
            ...
            if hasattr(param.grad, 'data'):
                numpy_grad = np.array(param.grad.data, dtype=np.float32)
            else:
                numpy_grad = np.array(param.grad, dtype=np.float32)
            
            if hasattr(param.data, 'data'):
                numpy_param = np.array(param.data.data, dtype=np.float32)
            elif hasattr(param, 'data'):
                numpy_param = np.array(param.data, dtype=np.float32)
            else:
                numpy_param = np.array(param, dtype=np.float32)
            
            original_shape = numpy_param.shape
            
            if param not in self.state:
                self.state[param] = {
                    'exp_avg': np.zeros(original_shape, dtype=np.float32),
                    'exp_avg_sq': np.zeros(original_shape, dtype=np.float32),
                    'max_exp_avg_sq': np.zeros(original_shape, dtype=np.float32) if self.amsgrad else None
                }
            
            state = self.state[param]
            
            if self.l1_weight != 0:
                l1_grad = np.sign(numpy_param) * self.l1_weight
            ...
            if self.amsgrad:
                state['max_exp_avg_sq'] = np.maximum(state['max_exp_avg_sq'], state['exp_avg_sq'])
                denom = np.sqrt(state['max_exp_avg_sq']) / np.sqrt(bias_corr2) + self.eps
            else:
                denom = np.sqrt(state['exp_avg_sq']) / np.sqrt(bias_corr2) + self.eps
    ...
    def _clip_gradients(self) -> None:
        ...
        for p in self.params:
            if p.grad is not None:
                ...
                if isinstance(grad_data.data, list):
                    if isinstance(grad_data.data[0], list): 
    ...
    def step(self) -> None:
        ...
        for param in self.params:
            ...
            if self.amsgrad:
                ...
                denom = sqrt(state['max_exp_avg_sq']) / sqrt(bias_corr2_arr) + self.eps
            else:
                denom = sqrt(state['exp_avg_sq']) / sqrt(bias_corr2_arr) + self.eps
