...
async def _refresh_token() -> bool:
    ...
    global USER_ACCESS_TOKEN, TOKEN_EXPIRES_AT, REFRESH_TOKEN
...
async def _auth_flow() -> str:
    ...
    global USER_ACCESS_TOKEN, REFRESH_TOKEN
    
    async with token_lock:
        if USER_ACCESS_TOKEN and not await _check_token_expired():
            return USER_ACCESS_TOKEN
    
    # 如果token过期但有refresh_token，尝试刷新token
    if USER_ACCESS_TOKEN and REFRESH_TOKEN:
    ...
    # 如果刷新失败或没有refresh_token，启动OAuth流程
