...
async def convert_document(
    ...
    format: Annotated[Literal["pdf", "docx", "xlsx", "pptx", "jpg", "jpeg", "png", "webp", "svg", "html", "txt", "csv", "md", "tex", "rtf", "tiff", "eps", "azw3"], Field(..., description="Target output format")],
    ...
    image_quality: Annotated[Optional[int], Field(description="Image quality setting (0-200). Higher values = better quality but larger file size. Only effective when converting PDF to image formats (jpg, jpeg, png, tiff, svg). If not specified, uses default value 100", ge=0, le=200)] = None
    ...
    **FROM PDF - Supported output formats when converting from PDF:**
    - Documents: DOCX, XLSX, PPTX, HTML, TXT, CSV, MD (Markdown), RTF, TEX (LaTeX), AZW3 (Kindle eBook)
    - Images: JPG, JPEG, PNG, TIFF, SVG
    
    **TO PDF - Supported input formats that can be converted to PDF:**
    - Documents: DOCX, XLSX, PPTX, HTML, TXT, MD, RTF, ODT, TEX (LaTeX), AZW3 (Kindle eBook)
    - Images: JPG (includes .jpeg files), PNG, HEIC, SVG, TIFF (includes .tif files), WEBP, EPS (Encapsulated PostScript)
...
async def rotate_pdf(
    ...
    rotates: Annotated[List[dict], Field(..., description="List of rotation operations. Each dict must contain: {'angle': rotation_angle, 'pages': page_range}. Angle: 90, 180, 270, or -90. Pages: 'all', '1-3', '1,3,5', etc. Example: [{'angle': 90, 'pages': '1-3'}, {'angle': 180, 'pages': 'all'}]", min_length=1)]
) -> Annotated[str, "JSON formatted result report with rotated PDF files"]:
    """
    Rotate PDF pages by specified angles to correct orientation or adjust viewing angle. This tool allows different rotation angles for different page ranges within the same document.
    ...
    Example: [{"angle": 90, "pages": "1-3"}, {"angle": 180, "pages": "5,7"}]
    """
    logger = Logger(ctx, collect_info=False)
