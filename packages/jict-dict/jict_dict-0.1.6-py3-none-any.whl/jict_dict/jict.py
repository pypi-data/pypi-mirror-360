class ELJIX:
    ...
    def Convert(obj):
        if isinstance(obj, (int, float, str, bool, type(None))):
            ...
            return tuple(ELJIX.Convert(e) for e in obj)
        elif isinstance(obj, list):
            return tuple(ELJIX.Convert(e) for e in obj)
        elif isinstance(obj, set):
            return frozenset(ELJIX.Convert(e) for e in obj)
        elif isinstance(obj, dict):
            return frozenset((ELJIX.Convert(k), ELJIX.Convert(v)) for k, v in obj.items())
        # Only attempt items() if callable attribute 'items' exists (like dicts)
    ...
    def Unconvert(obj):
        if isinstance(obj, frozenset):
            try:
                return {ELJIX.Unconvert(k): ELJIX.Unconvert(v) for k, v in obj}
            except Exception:
                return set(ELJIX.Unconvert(e) for e in obj)
        elif isinstance(obj, tuple):
            return [ELJIX.Unconvert(e) for e in obj]
        else:
            return obj
