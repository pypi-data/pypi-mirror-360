...
class AeroSpotReportGenerator:
    ...
    def _process_uav_data(
        ...
        if not processed_merged_data:
            ...
            logger.info(f'无人机反演结果保存至：{self.path_manager.get_file_path("uav_data", "uav.csv")}')
        ...
        if len(processed_merged_data["processed_data"]) != len(ref_data):
            ...
            logger.info(f'无人机光谱数据保存至：{self.path_manager.get_file_path("uav_data", "ref_data.csv")}')
