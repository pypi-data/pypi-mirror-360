...
from .matcher import find_common_indicators, match_nearest_points, handle_invalid_values
...
class DataProcessor:
    ...
    def process_data(self, merged_data: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 提取指标列
            indicator_columns = [col for col in merged_data.columns 
                                if col not in ['index', 'latitude', 'longitude']]
            logger.info(f"找到 {len(indicator_columns)} 个指标: {', '.join(indicator_columns)}")
            
            # 标准化指标名称
            merged_data, indicator_columns = standardize_indicator_names(merged_data, indicator_columns)
            ...
            result['statistics'] = calculate_statistics(
                merged_data, 
                indicator_columns,
                self._get_indicator_unit
            )
            
            # 生成数据摘要
            result['data_summary'] = generate_data_summary(merged_data, indicator_columns)
    ...
    def match_and_analyze_data(self, merged_data: pd.DataFrame, measure_data: pd.DataFrame, 
                              ref_data: pd.DataFrame, output_file: Optional[str] = None) -> Dict[str, Any]:
        """
        匹配和分析航测数据与人工采样数据
        ...
        try:
            ...
            # 获取共同的指标列
            common_indicators = find_common_indicators(merged_data, measure_data)
            ...
            # 进行建模，获取新的反演值
            model_func, pred_data, all_pred_data = main(ref_data, merged_data[common_indicators], measure_data[common_indicators], matched_idx)
            ...
            # 分析误差
            return analyze_errors(matched_measure_df=matched_measure_df, 
                                matched_merged_df=matched_merged_df, 
                                pred_data=pred_data,
                                output_file=output_file), pred_data, model_func, all_pred_data
