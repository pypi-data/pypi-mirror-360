...
from .exceptions import (
    AeroSpotError, ResourceError, DataProcessingError, 
    ConfigError, ReportGenerationError
)
...
class AeroSpotReportGenerator:
    ...
    def __init__(self, output_dir: Optional[str] = None, config: Optional[Dict[str, Any]] = None, cache_enabled: Optional[bool] = None):
        """初始化报告生成器
        ...
        if output_dir is None:
            # 使用默认的输出目录
            self.output_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "AutoReportResults")
        else:
        ...
        if cache_enabled is not None:
            self.resource_manager = ResourceManager(self.path_manager, cache_enabled=cache_enabled)
        else:
    ...
    def create_report_data(self) -> None:
        ...
        self.report_data = {
            ...
            'company_info': {
                ...
                'watermark_enabled': company_info.get('watermark_enabled', DEFAULT_COMPANY_INFO['watermark_enabled']),
                'watermark_text': company_info.get('watermark_text', DEFAULT_COMPANY_INFO['watermark_text']),
                'watermark_size': company_info.get('watermark_size', DEFAULT_COMPANY_INFO['watermark_size']),
                'watermark_color': company_info.get('watermark_color', DEFAULT_COMPANY_INFO['watermark_color']),
                'watermark_diagonal': company_info.get('watermark_diagonal', DEFAULT_COMPANY_INFO['watermark_diagonal']),
                'watermark_use_spire': company_info.get('watermark_use_spire', DEFAULT_COMPANY_INFO['watermark_use_spire'])
            },
            # 资源路径，将在下载和处理后更新
            'image_resources': {
                'logo_path': '',
    ...
    def _log_config_info(self) -> None:
        ...
        logger.info(f"  航线图: {wayline_img[:30]}..." if wayline_img else "  航线图: 未指定")
        
        satellite_img = company_info.get('satellite_img', '')
        logger.info(f"  卫星图: {satellite_img[:30]}..." if satellite_img else "  卫星图: 未指定")
        
        measure_data = company_info.get('measure_data', '')
        logger.info(f"  测量数据: {measure_data[:30]}..." if isinstance(measure_data, str) and measure_data else "  测量数据: 未指定")
        
        # 数据文件
        file_url = company_info.get('file_url', '')
        logger.info(f"  数据文件: {file_url[:30]}..." if file_url else "  数据文件: 未指定")
        
        # 污染源信息
        pollution_source = self.config.get('pollution_source', {})
        logger.info(f"污染源信息: {', '.join(pollution_source.keys()) if pollution_source else '未指定'}")
    ...
    def _match_analyze_data(self):
        ...
        if all(key in self.report_data for key in required_keys):
            ...
            comparison_data, pred_data, model_func, all_pred_data = self.data_processor.match_and_analyze_data(merged_data, measure_data, ref_data)
    ...
    def _process_uav_data(self, indices_file: str, pos_file: str, ref_files: str) -> None:
        """处理UAV数据
        ...
        if not processed_merged_data:
            ...
            logger.info(f"无人机反演数据样本收集完成，样本量：{len(processed_merged_data['processed_data'])}")
        ...
        if ref_files:
            ...
            for ref_file in ref_files:
                single_ref_data = pd.read_csv(ref_file, encoding='utf-8', header=0, index_col=0).T
                single_ref_data.index = [os.path.basename(ref_file).split('_')[-1].split('.')[0]]
                ref_data = pd.concat([ref_data, single_ref_data], axis=0)

            logger.info(f"反射率数据包含 {len(ref_data)} 行和 {len(ref_data.columns)} 列")


        if len(processed_merged_data['processed_data']) != len(ref_data):
            logger.error(f"无人机反演数据样本：{len(processed_merged_data['processed_data'])},光谱样本：{len(ref_data)},不匹配，请分析原因。")
        else:
    ...
    def _process_measure_data(self) -> None:
        ...
        try:
            # 读取人工采样数据
            measure_data = pd.read_csv(measure_data_path, encoding='utf-8', header=0, index_col=0)
    ...
    def generate_config(self) -> Optional[str]:
        ...
        structure_path = create_updated_config(
            ...
            report_structure_file=self.path_manager.get_file_path('reports', 'report_structure.json')
        )
        ...
        try:
            with open(structure_path, 'r', encoding='utf-8') as f:
                ...
                for chapter in structure['chapters']:
                    logger.info(f"  - 第{chapter.get('chapter_num', '?')}章：{chapter.get('title', '未命名')}")
        except Exception as e:
    ...
    def generate_report(self, config_path: str) -> bool:
        ...
        self.print_info.append(os.path.abspath(output_file))
        return True
    ...
    def save_processed_config(self) -> Optional[str]:
        ...
        """
        output_path = self.path_manager.get_file_path('reports', 'processed_config.json')
    ...
    def generate_indicator_maps(self):
        ...
        # 初始化建图参数
        self.maps_processor.init_maps(geo_info, satellite_img, pred_data, uav_data)
        # 遍历生成每个指标反演结果分布图
    ...
    def save_model_func(self):
        ...
        if result:
            try:
                ...
                if encrypted_path:
                    # 打印output_path的绝对路径
                    self.print_info.append(os.path.abspath(encrypted_path))
            except Exception as e:
...
def main() -> int:
    ...
    try:
        # 解析命令行参数
        import argparse
        parser = argparse.ArgumentParser(description='AeroSpot自动化报告生成工具V2 (优化版)')
        parser.add_argument('config', nargs='?', help='配置文件路径')
        parser.add_argument('-o', '--output', help='输出目录')
        parser.add_argument('--cache-enabled', type=str, default='false', 
        ...
        # 初始化报告生成器
        generator = AeroSpotReportGenerator(output_dir, config, cache_enabled=cache_enabled)
