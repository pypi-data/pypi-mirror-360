...
class PathManager:
    ...
    def __init__(self, base_dir: str = None):
        ...
        # 完全使用v1版本的目录结构
        self.dirs = {
            'downloads': os.path.join(self.base_dir, 'downloads'), # 通过链接下载的所有文件，包括压缩包、csv和图片
    ...
    def get_path(self, path_type: str) -> str:
        ...
        if path_type == 'image' and 'images' in self.dirs:
            logger.debug(f"使用'images'目录代替'image'")
    ...
    def get_image_path(self, image_type: str, filename: str) -> str:
        ...
        """
        # 构建图片路径 (允许嵌套路径)
        image_path = self.get_path(f"images/{image_type}" if '/' not in image_type else image_type)
