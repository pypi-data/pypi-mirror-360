...
def generate_interpolation_indicator_map(data, indicator, satellite_info, save_path, satellite_geo_bounds, data_geo_bounds, all_points_outside, water_mask):
    ...
    if all_points_outside:
        ...
        main_ax = fig.add_axes([0.07, 0.07, 0.89, 0.88])
    ...
    # 添加轴标签
    main_ax.set_xlabel('经度', fontsize=48, labelpad=20)
    main_ax.set_ylabel('纬度', fontsize=48, labelpad=20)
    main_ax.tick_params(axis='both', which='major', labelsize=40, direction='out', pad=12)
    
    # 计算原始数据范围，确保与散点图colorbar范围一致
    original_values = data[indicator].values
    vmin, vmax = np.min(original_values), np.max(original_values)
    
    # 绘制插值热力图，使用统一的colorbar范围
    ...
    cbar.set_label(indicator, fontsize=48)
    ...
    if not all_points_outside:
        title += " - 卫星底图"
    main_ax.set_title(title, fontsize=56, pad=30)
...
def generate_distribution_indicator_map(data, indicator, satellite_info, save_path, satellite_geo_bounds, data_geo_bounds, all_points_outside):
    ...
    # 添加轴标签
    main_ax.set_xlabel('经度', fontsize=48, labelpad=20)  # 进一步增大字体
    main_ax.set_ylabel('纬度', fontsize=48, labelpad=20)  # 进一步增大字体
    
    # 调整刻度标签
    main_ax.tick_params(axis='both', which='major', labelsize=40, direction='out', pad=12)  # 进一步增大刻度标签
    ...
    cbar.set_label(indicator, fontsize=48)  # 进一步增大字体
    cbar.ax.tick_params(labelsize=40)  # 进一步增大刻度字体
    
    title = f"水质监测数据 - {indicator} (散点图)"

    if not all_points_outside:
        title += " - 卫星底图"
    main_ax.set_title(title, fontsize=56, pad=30)  # 进一步增大标题字体
...
def generate_level_indicator_map(indicator, satellite_info, save_path, satellite_geo_bounds, data_geo_bounds, all_points_outside, Z):
    ...
    if all_points_outside:
        ...
        figsize = (img_width / dpi, img_height / dpi)
        fig = plt.figure(figsize=figsize, dpi=dpi, frameon=True)
        main_ax = fig.add_axes([0.07, 0.07, 0.83, 0.88])
    ...
    legend = main_ax.legend(handles=patches, bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=36,
