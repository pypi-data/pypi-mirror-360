...
{str(additional_args)}."""
    ...
    async def _astep_stream(self, memory_step: ActionStep) -> AsyncGenerator[ChatMessageStreamDelta | ActionOutput | ToolOutput]:
        """
        Perform one step in the ReAct framework: the agent thinks, acts, and observes the result.
        Yields ChatMessageStreamDelta during the run if streaming is enabled.
        At the end, yields either None if the step is not final, or the final answer.
...
class CodeAgent(MultiStepAgent):
    ...
    async def _astep_stream(self, memory_step: ActionStep) -> AsyncGenerator[ChatMessageStreamDelta | ActionOutput]:
        """
        Perform one step in the ReAct framework: the agent thinks, acts, and observes the result.
        Yields ChatMessageStreamDelta during the run if streaming is enabled.
        At the end, yields either None if the step is not final, or the final answer.
        ...
        is_final_answer = False
        try:
            output, execution_logs, is_final_answer = await self.python_executor.acall(code_action)
            execution_outputs_console = []
            if len(execution_logs) > 0:
                execution_outputs_console += [
                    Text("Execution logs:", style="bold"),
