    ...
    def _load_install_state(self) -> dict:
        ...
        if self.state_path.exists():
            try:
                ...
                logger.warning(f"Failed to load install state: {e}")
        return {}
    ...
    def _clear_install_state(self):
        ...
        if self.state_path.exists():
            try:
                ...
                logger.warning(f"Failed to clear install state: {e}")
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            if self._is_first_time():
                print_header(
                    f"{Colors.CYAN}Primera vez detected in this project. Full preparation will be performed.{Colors.ENDC}"
                )
                print_info(
            ...
            print_info(
                f"Adaptive timeouts calculated: {adaptive_timeout}s for heavy operations based on your hardware."
            )

            # Secuencia de pasos con guardado de estado
            steps = [
                ("config_collected", self._collect_configuration),
                ("config_saved", lambda config: self._save_configuration(config)),
                ("docker_generated", lambda config: self._generate_docker_files(config)),
                ("modelfile_generated", lambda config: self._generate_modelfile(config)),
                ("database_initialized", lambda config: self._initialize_database(config)),
                (
                    "models_downloaded",
                ...
                (
                    "embeddings_warmed",
                    lambda config: asyncio.wait_for(
                        self._warmup_embeddings(config), timeout=adaptive_timeout
                    ),
                ),
            ]
            config = None
            for step_name, step_func in steps:
                ...
                try:
                    ...
                    logger.error(f"Error in step '{step_name}'", error=str(e), include_trace=True)
                    return False
...
"""
    ...
    async def _download_models(self, config: Dict[str, Any]):
        ...
        # 1. Download UniXcoder embeddings model
        check_disk_space(3, "UniXcoder model")
        print_info("Downloading UniXcoder embeddings model (~350MB)...")
        print_info("This is a one-time download that may take a few minutes.")
        try:
            # Import and initialize embeddings to trigger download
            from acolyte.embeddings import get_embeddings

            # Show progress while downloading
            start_time = time.time()
            print_info("Initializing embeddings service...")
            # Force download in thread to avoid blocking
            await asyncio.to_thread(get_embeddings)
            # Test encode to ensure model is fully loaded
            print_info("Testing embeddings...")
            await asyncio.to_thread(get_embeddings.encode, "test code snippet")
            elapsed = time.time() - start_time
            print_success(f"✓ UniXcoder model ready (took {elapsed:.1f}s)")
        except Exception as e:
            print_error(f"Failed to download embeddings model: {e}")
            print_warning("You may need to check your internet connection.")
            raise ConfigurationError(f"Embeddings download failed: {e}")
        ...
        print_info(f"\nDownloading Ollama model {base_model} (~2GB)...")
        print_info("This download shows real-time progress from Ollama.")
        try:
            # Check if ollama is available
            check_ollama = subprocess.run(["ollama", "--version"], capture_output=True, text=True)
            if check_ollama.returncode != 0:
                print_warning("Ollama CLI not found. It will be downloaded when Docker starts.")
                print_info("The model will be pulled automatically on first start.")
                return
            # Pull the model
            print_info(f"\nPulling {base_model} (this may take 5-10 minutes)...")
            # Run without capture_output to show Ollama's progress
            result = subprocess.run(["ollama", "pull", base_model], text=True)
            if result.returncode == 0:
                print_success(f"✓ Ollama model {base_model} downloaded successfully")
            else:
                print_warning(
    ...
    async def _prepare_weaviate(self, config: Dict[str, Any]):
        ...
        if not check_docker_ready():
            print_error(
                "Docker is not available or not running. Please ensure Docker Desktop is installed and running. If you just started Docker, wait a few seconds and try again. For help, run: acolyte doctor"
            )
        ...
        print_info("Starting Weaviate temporarily to create collections...")
        ...
        try:
            ...
            result = subprocess.run(
                docker_cmd + ["up", "-d", "weaviate"], cwd=infra_dir, capture_output=True, text=True
            )
            ...
            if not self._wait_for_service(
                "Weaviate", weaviate_port, "/v1/.well-known/ready", timeout=120
            ):
                raise ConfigurationError("Weaviate failed to become ready")

            # 3. Create collections
            print_info("Creating vector collections...")
            ...
            print_success(f"✓ Created {created} collections")
            ...
            if not health['healthy']:
                ...
                if missing:
                    print_error(f"Missing collections after creation: {missing}")
            ...
            for collection, info in stats['collections'].items():
                print_info(f"  • {collection}: {info['properties']} properties")

            print_success("✓ All vector collections ready")

        finally:
            # 4. Stop Weaviate
            print_info("\nStopping temporary services...")
            subprocess.run(docker_cmd + ["down"], cwd=infra_dir, capture_output=True)
            print_info("Temporary services stopped")

    async def _warmup_embeddings(self, config: Dict[str, Any]):
        """Pre-warm the embeddings system."""
        print_header("🧠 Initializing Embeddings System")

        try:
            # Create cache directory
            cache_dir = self.project_global_dir / "data" / "embeddings_cache"
            cache_dir.mkdir(parents=True, exist_ok=True)
            print_info(f"Created embeddings cache at: {cache_dir}")
            ...
            # Test texts covering different code patterns
            test_texts = [
            ...
            print_info("Warming up embeddings cache with sample code...")

            # Generate embeddings for warmup
            for i, text in enumerate(test_texts, 1):
                print_info(f"  Processing sample {i}/{len(test_texts)}...")
                await asyncio.to_thread(embeddings.encode, text)

            # Show cache stats
            cache_size = embeddings.cache.size if hasattr(embeddings.cache, 'size') else 0
            print_success(f"✓ Embeddings cache warmed up (cache size: {cache_size})")

            # Show device info
            device = str(embeddings.device) if hasattr(embeddings, 'device') else "unknown"
            print_info(f"Using device: {device}")

        except Exception as e:
            print_warning(f"Could not warm up embeddings: {e}")
            print_info("Embeddings will initialize on first use.")
            # Not critical, don't fail installation
