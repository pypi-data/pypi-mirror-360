...
async def health_check(response: Response) -> Dict[str, Any]:
    ...
    checks = [
        ...
        ("ollama", _check_ollama, False, 5),  # Not critical, 5s timeout
    ...
    try:
        ...
        try:
            ...
            test_response = await asyncio.wait_for(
                ollama_client.generate(prompt="Hello", max_tokens=5), timeout=60
            )
            generation_works = bool(test_response)
        except asyncio.TimeoutError:
            logger.error("Ollama test generation timed out (60s)", exc_info=True)
