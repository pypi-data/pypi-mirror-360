...
class DatabaseManager:
    ...
    def _get_default_path(self) -> str:
        ...
        # Check DATA_DIR environment variable (for Docker)
        ...
        async with self._lock:
            ...
            try:
                ...
                result = await retry_async(
                    lambda: asyncio.wait_for(loop.run_in_executor(None, _execute), timeout=30.0),
                    max_attempts=4,
                    retry_on=(SQLiteBusyError, DatabaseError, Exception),
                    logger=logger,
                )
                return result
