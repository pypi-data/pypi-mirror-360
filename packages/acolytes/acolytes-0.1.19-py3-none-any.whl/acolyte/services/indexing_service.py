            ...
            try:
                try:
                    ...
                    try:
                        ...
                        if result['confidence'] > 0.7 and result['encoding']:
                            ...
                            logger.debug(
                                f"Detected encoding {encoding_detected} with confidence {result['confidence']:.2f}"
                            )
                    except Exception as e:
                ...
                if ADAPTIVE_CHUNKER_AVAILABLE:
                    try:
                        ...
                        logger.warning(f"AdaptiveChunker failed for {file_path}: {e}")
                ...
                try:
                    ...
                    chunks.extend(file_chunks)
                    logger.info(f"DefaultChunker fallback succeeded for {file_path}")
                except Exception as e2:
                    logger.error(f"Both chunkers failed for {file_path}: {e2}")
