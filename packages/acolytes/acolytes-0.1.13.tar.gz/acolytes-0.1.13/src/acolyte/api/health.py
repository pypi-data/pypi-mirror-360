...
async def _check_ollama() -> Dict[str, Any]:
    ...
    try:
        ...
        # Create Ollama client - use the default initialization which respects env vars
        ...
        try:
            test_response = await asyncio.wait_for(
                ollama_client.generate(prompt="Hello", max_tokens=5), timeout=5
            )
            generation_works = bool(test_response)
        except asyncio.TimeoutError:
            logger.error("Ollama test generation timed out")
...
async def _check_database() -> Dict[str, Any]:
    ...
    """
    logger.info("[HEALTH] Starting database check...")
    try:
        db_start = time.time()
        db_manager = get_db_manager()

        # Timeout for table query
        try:
            logger.info("[HEALTH] Querying tables in the database (timeout=15s)...")
            tables_result = await asyncio.wait_for(
                db_manager.execute_async(
                    "SELECT name FROM sqlite_master WHERE type='table'", fetch=FetchType.ALL
                ),
                timeout=15,
            )
            logger.info("[HEALTH] Table query completed successfully.")
        except asyncio.TimeoutError:
            logger.error("[HEALTH] Timeout querying tables in the database (timeout=15s)")
            return {
                "status": "unhealthy",
                "error": "Timeout querying tables in the database",
            }
        except Exception as e:
            logger.error(f"[HEALTH] Exception querying tables in the database: {e}")
            return {
                "status": "unhealthy",
        ...
        # Timeout for counting records
        counts = {}
        for table in ["sessions", "tasks", "technical_decisions"]:
            if table in tables:
                try:
                    logger.info(f"[HEALTH] Counting records in table {table} (timeout=15s)...")
                    ...
                    logger.info(
                        f"[HEALTH] Count for table {table}: {count_result.data['count'] if isinstance(count_result.data, dict) and 'count' in count_result.data else 'N/A'}"
                    )
                    if count_result.data and isinstance(count_result.data, dict):
                        counts[table] = count_result.data.get("count", 0)
                except asyncio.TimeoutError:
                    logger.error(
                        f"[HEALTH] Timeout counting records in table {table} (timeout=15s)"
                    )
                    counts[table] = "timeout"
                except Exception as e:
                    logger.error(f"[HEALTH] Exception counting records in table {table}: {e}")
    ...
    try:
        ...
        try:
            embedding = await asyncio.wait_for(
                asyncio.to_thread(embeddings_service.encode, test_text), timeout=5
            )
        except asyncio.TimeoutError:
            logger.error("Embeddings test encoding timed out")
            return {
                "status": "unhealthy",
                "error": "Timeout encoding test embedding",
                "suggestion": _get_embeddings_suggestion("unknown"),
            }
...
async def _check_weaviate() -> Dict[str, Any]:
    ...
    """
    logger.info("[HEALTH] Starting Weaviate check...")
    ...
    try:
        ...
        logger.error("[HEALTH] Failed to create Weaviate client", error=str(e))
        return {
            "status": "unhealthy",
    ...
    try:
        is_ready = await asyncio.wait_for(asyncio.to_thread(client.is_ready), timeout=5)
        if not is_ready:
            logger.error("[HEALTH] Weaviate is not ready")
            return {
                "status": "unhealthy",
        ...
        logger.error("[HEALTH] Timeout waiting for Weaviate response")
        return {
            "status": "unhealthy",
            "error": "timeout_waiting_for_weaviate",
            "url": weaviate_url,
            "suggestion": "Check Weaviate service responsiveness",
            "response_time_ms": int((time.time() - weaviate_start) * 1000),
        }
    except WeaviateBaseError as e:
        logger.error("[HEALTH] Weaviate connection error", error=str(e))
        return {
            "status": "unhealthy",
