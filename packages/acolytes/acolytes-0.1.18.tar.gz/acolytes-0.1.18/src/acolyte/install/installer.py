...
def safe_input(prompt: str, default: str = "") -> str:
    """Cross-platform safe input. Handles EOFError and avoids unnecessary flushes on Windows."""
    try:
        if sys.platform == "win32":
            try:
                ...
                pass
            # Avoid flushing sys.stdin (can cause issues)
        return input(prompt)
    except EOFError:
...
class AdvancedConfiguration:
    ...
    def configure_model(self) -> Dict[str, Any]:
        ...
        # After hardware detection and before model selection
        expected_gpu = True  # Set this based on your logic or config
...
class ProjectInstaller:
    ...
    def _calculate_adaptive_timeout(self, ram_gb: int, cpu_cores: int, is_ssd: bool) -> int:
        ...
        base = 300  # 5 minutos base
        if ram_gb < 8:
            base += 60  # 1 minuto extra para sistemas con poca RAM
        if not is_ssd:
            base += 60  # 1 minuto extra para discos lentos
        if cpu_cores < 4:
            base += 30  # 30 segundos extra para CPUs lentos
        return base
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            steps = [
                ...
                ("database_initialized", lambda config: await self._initialize_database(config)),
                (
                    "models_downloaded",
...
"""
    ...
    async def _download_models(self, config: Dict[str, Any]):
        ...
        if model_dir.exists() and any(model_dir.iterdir()):
            console.print('[green]✓ UniXcoder ya está descargado.[/green]')
        else:
            console.print(
                '[yellow]El modelo UniXcoder se descargará automáticamente la primera vez que lo uses.[/yellow]'
            )
            console.print('[dim]Puedes continuar con la instalación.[/dim]')
        # (No forzar descarga ni warmup)
    ...
    async def _prepare_weaviate(self, config: Dict[str, Any]):
        ...
        print_info("Actualizando imagen de Ollama a la última versión...")
        subprocess.run(
