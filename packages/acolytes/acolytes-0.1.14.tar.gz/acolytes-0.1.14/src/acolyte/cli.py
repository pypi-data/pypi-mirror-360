            ...
            if export_result.returncode == 0:
                ...
                if copy_result.returncode == 0:
                    ...
                    if import_result.returncode == 0:
                        console.print("[green]✓ Model acolyte imported into container successfully")
                        return True
                    else:
                        console.print(
...
@cli.command()
@click.option('--path', default=".", help='Project path')
@click.option('--repair', is_flag=True, help='Reparar o reanudar una instalación interrumpida')
def install(path: str, repair: bool):
    ...
    click.echo(click.style("\n⚠️  IMPORTANTE SOBRE EL NOMBRE DEL PAQUETE", fg="yellow", bold=True))
    click.echo("El paquete en PyPI se llama 'acolytes' (con 's'), pero el comando es 'acolyte'.")
    click.echo("Si ejecutaste 'pip install acolyte' y falló, usa 'pip install acolytes'.\n")
    ...
    if repair:
        click.echo(
            click.style(
                "[Modo reparación] Se intentará reanudar o reparar la instalación sin perder el progreso previo.",
                fg="yellow",
            )
        )
    else:
        click.echo(
            click.style(
                "Si la instalación se interrumpe, puedes reanudarla con 'acolyte install --repair'",
                fg="yellow",
            )
        )
...
def doctor(fix: bool, project: str):
    ...
    run_doctor(fix=fix, project=project)
...
async def monitor_via_polling(backend_port: int, task_id: str, total_files: int, console):
    ...
    console.print("[dim]Usando HTTP polling para actualizaciones de progreso...[/dim]")
    ...
    with Progress(
        ...
        task = progress.add_task("[cyan]Indexando archivos...", total=total_files or 100)
    ...
    console.print("\n[bold green]✓ Indexado completado (polling HTTP)[/bold green]")
