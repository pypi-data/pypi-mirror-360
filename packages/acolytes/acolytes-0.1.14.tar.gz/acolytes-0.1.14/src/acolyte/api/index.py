...
async def index_project(
    ...
    try:
        ...
        files_to_index = await _collect_files_to_index(project_root, request.patterns)
...
async def _collect_files_to_index(root: Path, patterns: List[str]) -> List[str]:
    """
    Collect all files to index based on patterns.

    This is the SINGLE place where file scanning happens.
