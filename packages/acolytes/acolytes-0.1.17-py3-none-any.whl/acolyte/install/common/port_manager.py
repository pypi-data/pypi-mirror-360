...
class PortManager:
    ...
    def suggest_ports(cls, preferred_ports: dict) -> dict:
        ...
        if cls.is_port_available(pref_ollama):
            ...
            next_port = cls.find_next_available(pref_ollama)
            if next_port is None:
                raise RuntimeError(f"Cannot find available Ollama port starting from {pref_ollama}")
            result["ollama"] = next_port
        ...
        if cls.is_port_available(pref_backend):
            ...
            next_port = cls.find_next_available(pref_backend)
            if next_port is None:
                raise RuntimeError(
                    f"Cannot find available Backend port starting from {pref_backend}"
                )
            result["backend"] = next_port
