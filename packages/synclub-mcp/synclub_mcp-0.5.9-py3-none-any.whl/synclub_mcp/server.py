...
async def gbu_generate_comic_story(
    ...
    try:
        ...
        response_data = await make_unified_request(
            method="POST",
            path="/pulsar/mcp/inner/comic/generateScript",
            data=payload,
            stream=True  # 启用流式响应
        )
...
@mcp.tool(description="""
    ...
    Args:
        ...
        chapters_num (int): The number of chapters to generate, default is 10, max is 15.
...
async def gbu_generate_comic_chapters(
    ...
    try:
        ...
        if chapters_num < 10 or chapters_num > 15:
            raise Exception("chapters_num must be between 10 and 15")
        ...
        response_data = await make_unified_request(
            method="POST",
            path="/pulsar/mcp/inner/comic/generateStoryboards",
            data=payload,
            stream=True
        )
...
async def gbu_generate_comic_image_prompts(
    ...
    try:
        ...
        response_data = await make_unified_request(
            method="POST",
            path="/pulsar/mcp/inner/comic/promptFormat",
            data=payload,
            stream=True
        )
