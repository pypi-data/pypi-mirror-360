...
class Segment(BaseModel):
    ...
    size: Literal["short", "medium", "long"] = Field(..., description="Size of the segment")
    ...
    class ValidatedDialogue(BaseModel):
        ...
        def validate_speaker_name(cls, v):
            ...
            if cleaned_name not in valid_speaker_names:
                raise ValueError(f"Invalid speaker name '{cleaned_name}'. Must be one of: {', '.join(valid_speaker_names)}")
...
async def combine_audio_files(audio_dir: Union[Path, str], final_filename: str, final_output_dir: Union[Path, str]):
    """
    ...
    for i, file_path in enumerate(list_of_audio_paths):
        if not isinstance(file_path, Path):
            logger.warning(f"combine_audio_files: Item {i} in audio_segments_data is not a string path: {file_path}. Skipping.")
            continue
        ...
        try:
            if file_path.exists() and file_path.is_file():
                ...
                logger.error(f"combine_audio_files: File not found or not a file: {file_path}")
        except Exception as e:
            logger.error(f"combine_audio_files: Error loading audio clip {file_path}: {e}")
