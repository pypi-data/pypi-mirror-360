...
def main():
    ...
    try:
        ...
        with SeisData(sgy_path, mode='w+') as sgy_writer:
            ...
            headers[TraceField.SampleRate.name] = 2000  # 2ms in trace header
    ...
    print(f"--- 步骤 2: 正在加载刚才创建的 SGY 文件 '{Path(sgy_path).name}' ---")
    try:
        sgy = SeisData(sgy_path).load()
        print("SGY 文件加载成功。")
        print(f"  - 数据维度: {sgy.shape}")
        print(f"  - Inline范围: {sgy.arrInlines[0]} - {sgy.arrInlines[-1]}")
        print(f"  - Xline范围: {sgy.arrXlines[0]} - {sgy.arrXlines[-1]}")
        ...
        print(f"错误: 加载或处理 SGY 文件时失败: {e}")
        return

    # --- 步骤 3: 数据切片与可视化 ---
    print("--- 步骤 3: 演示数据切片与可视化 ---")
