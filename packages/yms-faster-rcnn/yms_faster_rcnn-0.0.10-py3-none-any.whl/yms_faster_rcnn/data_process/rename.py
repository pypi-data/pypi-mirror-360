...
if __name__ == "__main__":
    # 直接在这里指定参数
    folder_path = r"D:\Code\0-data\1-齿轮检测数据集\青山数据集\VOCdevkit\VOC2012\Annotations"  # 替换为你的文件夹路径
    old_xxx = "SE210179-2()-"  # 替换为需要替换的旧类别名
    new_xxx = "SE210179-2()"  # 替换为新的类别名
    # 可以指定具体后缀，如 'bmp'、'jpg'、'png'，设置为None则处理所有类型
    target_extension = None  # 例如: 'bmp' 或 None
    ...
    # 确认操作
    # confirm = input(
    #     f"\n即将重命名上述文件夹中所有以 {old_xxx}_ 开头的{'所有类型' if target_extension is None else target_extension}文件为 {new_xxx}_\n"
    #     f"是否继续? (y/n): ")
