    ...
    def forward(self, images, targets=None):
        ...
        if self.training:
            ...
            for target in targets:
                # 允许空目标（负样本），仅检查非空目标的格式
                if "boxes" in target and target["boxes"].numel() > 0:
                    boxes = target["boxes"]
                    if isinstance(boxes, torch.Tensor):
                        if len(boxes.shape) != 2 or boxes.shape[-1] != 4:
                            raise ValueError(f"Expected target boxes to be [N,4], got {boxes.shape}.")
                    else:
                        raise ValueError(f"Expected target boxes to be Tensor, got {type(boxes)}.")
                # 为空目标自动补充默认值
                if "boxes" not in target:
                    target["boxes"] = torch.zeros((0, 4), dtype=torch.float32, device=images.tensors.device)
                if "labels" not in target:
                    target["labels"] = torch.zeros(0, dtype=torch.int64, device=images.tensors.device)
