...
def plot_loss_and_lr(train_loss, learning_rate,
                     path='./loss_and_lr{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")),
                     upload=None
                     ):
    try:
        ...
        if upload:
            upload.log({'loss_and_lr': upload.Image(plt)})
        fig.savefig(path)
        plt.close()
...
def plot_map(mAP, path='./save_weights/mAP.png', upload=None):
    try:
        ...
        plt.legend(loc='best')
        if upload:
            upload.log({'mAP': upload.Image(plt)})
        plt.savefig(path)
        plt.close()
...
def plot_single(met, name, path, upload=None):
    try:
        ...
        plt.legend(loc='best')
        if upload:
            upload.log({name: upload.Image(plt)})
        plt.savefig(path)
        plt.close()
...
def plot_metrics(metrics_dict,
    ...
    try:
        ...
        plt.tight_layout()
        # wandb.log({'metrics': wandb.Image(plt)})
        plt.savefig(path)
        plt.close()
