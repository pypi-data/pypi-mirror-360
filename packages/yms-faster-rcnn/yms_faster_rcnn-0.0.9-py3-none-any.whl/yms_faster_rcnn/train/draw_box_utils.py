...
def draw_text(draw,
    ...
    display_str_heights = [font.getbbox(ds)[1] for ds in display_str]
    ...
    for ds in display_str:
        # text_width, text_height = font.getbbox(ds)
...
def draw_objs(image: Image,
    ...
    colors = [
        ...
        for cls in classes
        for r, g, b in [ImageColor.getrgb(STANDARD_COLORS[cls % len(STANDARD_COLORS)])]
    ]
...
def my_draw_objs(image: Image,
    ...
    if draw_masks_on_image and (masks is not None):
        # Draw all mask onto image.
        image = draw_masks(image, masks, colors, mask_thresh)
    fonts = ImageFont.truetype(font, 40)  # 创建字体对象
    # 先绘制图片名称，这里选择在图片左上角绘制，坐标和样式可根据需求调整
    # draw.text((10, 20), image_name, fill=(255, 255, 255), font=fonts, size=40)
    draw.text((10, 20), image_name, fill=0x00FF00, font=fonts, size=40)
    return image
