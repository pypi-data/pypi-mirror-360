...
def create_model(num_classes):
    ...
    # backbone = KAN(width=[3*224*224, 64], auto_save=False)
    # print(backbone)
    # backbone = create_feature_extractor(backbone, return_nodes={"features.42": "0"})
    # resnet50 backbone
...
def main(args):
    ...
    if len(train_loss) != 0 and len(learning_rate) != 0:
        ...
        plot_loss_and_lr(train_loss, learning_rate, "./save_weights/loos.png")
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(VOCdevkit)
    parser.add_argument('--data-path', default=r"E:\360Downloads\home", help='dataset')
    # 检测目标类别数(不包含背景)
