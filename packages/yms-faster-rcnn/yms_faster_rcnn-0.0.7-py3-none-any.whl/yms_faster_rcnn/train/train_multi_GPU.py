...
from yms_faster_rcnn.network_files import FasterRCNN, FastRCNNPredictor
from yms_faster_rcnn import train_utils as utils
from yms_faster_rcnn.train_utils import GroupedBatchSampler, create_aspect_ratio_groups, init_distributed_mode, save_on_master, mkdir, \
    transforms
...
def main(args):
    ...
    if args.rank in [-1, 0]:
        ...
        if len(train_loss) != 0 and len(learning_rate) != 0:
            from yms_faster_rcnn.train_utils.plot_curve import plot_loss_and_lr
            plot_loss_and_lr(train_loss, learning_rate)

        # plot mAP curve
        if len(val_map) != 0:
            from yms_faster_rcnn.train_utils.plot_curve import plot_map
            plot_map(val_map)
