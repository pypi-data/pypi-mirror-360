...
from .utils import unit_vector, normalize_angles, normalize_distance, normalize_coords, normalize_speed
...
def node_features(
    ...
    max_dist_to_player = np.sqrt(pitch_dimensions.pitch_length ** 2 +pitch_dimensions.pitch_width ** 2)
    max_dist_to_goal = np.sqrt(pitch_dimensions.pitch_length ** 2 +pitch_dimensions.pitch_width ** 2)
    
    def player_features(p, team, potential_receiver=None):
        ball_angle = math.atan2(p.y1 - ball.y1, p.x1 - ball.x1)
        goal_angle = math.atan2(p.y1 - goal_mouth_position[0], p.x1 - goal_mouth_position[1])
        
        player_node_features = [
            0.0 if np.isnan(p.x1) else normalize_coords(p.x1, pitch_dimensions.x_dim.max),
            0.0 if np.isnan(p.x1) else normalize_coords(p.y1, pitch_dimensions.y_dim.max),
            0.0 if np.isnan(p.x1) else unit_vector(p.velocity)[0],
            0.0 if np.isnan(p.x1) else unit_vector(p.velocity)[1],
            0.0 if np.isnan(p.x1) else round(normalize_speed(p.speed, max_speed=max_player_speed), 3),
            0.0 if np.isnan(p.x1) else normalize_angles(np.arctan2(p.velocity[1], p.velocity[0])),
            0.0 if np.isnan(p.x1) else normalize_distance(np.linalg.norm(p.position - goal_mouth_position), max_distance=max_dist_to_goal),  # distance to the goal mouth
            0.0 if np.isnan(p.x1) else normalize_angles(goal_angle),
            0.0 if np.isnan(p.x1) else normalize_distance(np.linalg.norm(p.position - ball.position), max_distance=max_dist_to_player),  # distance to the ball
            ...
            0.0 if np.isnan(p.x1) else 1. if potential_receiver else non_potential_receiver_node_value
        ]
        return player_node_features

    def ball_features(ball):
        goal_angle = math.atan2(ball.y1 - goal_mouth_position[1],
                                ball.x1 - goal_mouth_position[0])
        ball_node_features = [
            ...
            normalize_distance(np.linalg.norm(ball.position - goal_mouth_position), max_distance=max_dist_to_goal),  # distance to the goal mouth
    ...
    ap_features = np.asarray([
        player_features(
            p, team=1,
            potential_receiver=(i != ball_carrier_idx)
    ...
    dp_features = np.asarray([
        player_features(p, team=defending_team_node_value) for i, p in enumerate(defending_players)
    ])
