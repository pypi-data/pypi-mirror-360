"""CLI interface for cflabs-serverless."""
...
app = typer.Typer(
    ...
    help="Deploy Flask apps to AWS Lambda + API Gateway with zero code changes",
...
def generate(
    ...
    image_tag: str = typer.Option("latest", "--image-tag", "-i", help="Docker image tag to use for deployment (use unique tags to avoid caching issues)"),
...
def deploy(
    ...
    force_rebuild: bool = typer.Option(False, "--force-rebuild", help="Force rebuild Docker image without cache"),
):
    ...
    try:
        ...
        # Generate unique image tag if using "latest" to prevent caching issues
        original_image_tag = image_tag
        if image_tag == "latest":
            image_tag = utils.generate_unique_image_tag()
            console.print(f"[yellow]⚠️  Using 'latest' tag detected. Generated unique tag: {image_tag}[/yellow]")
            console.print("[yellow]💡 Tip: Use --image-tag to specify a custom tag for better control[/yellow]")
        elif debug:
            console.print(f"[blue]Debug: Using custom image tag: {image_tag}[/blue]")
        ...
        if sam_config_path.exists():
            ...
            # Add --no-cache flag if force_rebuild is specified
            if force_rebuild:
                build_cmd.extend(["--no-cache"])
                if debug:
                    console.print("[blue]Debug: Using --no-cache for force rebuild[/blue]")
            ...
            validate_cmd = ["sam", "validate", "--template-file", "template.yaml"]
        ...
        if "FlaskApi" in outputs:
            ...
            console.print(Panel(
                ...
                f"Image Tag: [blue]{image_tag}[/blue]\n\n"
                f"Your Flask app is now running on AWS Lambda + API Gateway!\n\n"
                f"Next steps:\n"
                f"• [yellow]cflabs-serverless logs[/yellow] - View logs\n"
                f"• [yellow]cflabs-serverless status[/yellow] - Check status\n"
                f"• [yellow]cflabs-serverless delete[/yellow] - Clean up",
...
def troubleshoot():
    ...
    console.print(Panel(
        ...
        "• [red]ImageNotFound[/red] → Run [yellow]cflabs-serverless build[/yellow] first\n\n"
        ...
        "• Run: [yellow]cflabs-serverless status[/yellow] to check deployment\n"
        "• Check AWS CloudFormation console for stack errors\n"
        "• Review CloudWatch logs: [yellow]cflabs-serverless logs[/yellow]\n"
...
def image_info(
    ...
    """Show information about the deployed image and help with image tag issues."""
    try:
        # Get stack name and region from config or parameters
        if stack_name is None or region is None:
            config = utils.load_config(config_path)
        ...
        console.print(f"[bold blue]📋 Image information for stack: {stack_name}[/bold blue]")
        ...
        if 'error' not in status:
            ...
            # Try to get Lambda function information
            try:
                cloudformation = utils.boto3.client('cloudformation', region_name=region)
                lambda_client = utils.boto3.client('lambda', region_name=region)
                
                # Get stack outputs to find Lambda function
                for output in status.get('outputs', []):
                    if 'Function' in output['OutputKey']:
                        function_name = output['OutputValue'].split('/')[-1]
                        ...
                        image_uri = function_config.get('ImageConfigResponse', {}).get('ImageConfig', {}).get('ImageUri', 'Not found')
                        
                        console.print(f"\n[bold]Lambda Function Details:[/bold]")
                        console.print(f"Function Name: [blue]{function_name}[/blue]")
                        console.print(f"Image URI: [blue]{image_uri}[/blue]")
                        console.print(f"Runtime: [blue]{function_config.get('Runtime', 'N/A')}[/blue]")
                        console.print(f"Last Modified: [blue]{function_config.get('LastModified', 'N/A')}[/blue]")
                        
                        # Check if image URI contains a tag
                        if ':' in image_uri:
                            tag = image_uri.split(':')[-1]
                            console.print(f"Image Tag: [blue]{tag}[/blue]")
                            
                            if tag == 'latest':
                                console.print(f"\n[yellow]⚠️  Warning: Using 'latest' tag detected![/yellow]")
                                console.print(f"[yellow]This can cause caching issues. Consider using a unique tag.[/yellow]")
                            else:
                                console.print(f"\n[green]✅ Using unique tag: {tag}[/green]")
                        else:
                            console.print(f"\n[yellow]⚠️  No image tag found in URI[/yellow]")
                        
                        break
                else:
                    console.print("[yellow]No Lambda function found in stack outputs[/yellow]")
                    
            except Exception as e:
                console.print(f"[yellow]Could not get Lambda function details: {e}[/yellow]")
        
        # Show image tag guidance
        console.print(Panel(
            "[bold blue]Image Tag Best Practices:[/bold blue]\n\n"
            "[bold]Why unique tags matter:[/bold]\n"
            "• AWS Lambda caches images by tag\n"
            "• Using 'latest' can cause old images to be used\n"
            "• Unique tags ensure new code is deployed\n\n"
            "[bold]Recommended approaches:[/bold]\n"
            "• Use timestamps: [yellow]--image-tag $(date +%s)[/yellow]\n"
            "• Use git commit: [yellow]--image-tag $(git rev-parse --short HEAD)[/yellow]\n"
            "• Use version numbers: [yellow]--image-tag v1.0.1[/yellow]\n"
            "• Let CLI auto-generate: [yellow]cflabs-serverless deploy[/yellow] (auto-generates unique tag)\n\n"
            "[bold]Troubleshooting:[/bold]\n"
            "• If changes aren't reflected, check the image tag\n"
            "• Verify the image was pushed to ECR\n"
            "• Check CloudWatch logs for the new code\n"
            "• Use [yellow]cflabs-serverless logs[/yellow] to see if new code is running",
            title="Image Tag Guidance",
            border_style="blue"
        ))
        
    except Exception as e:
        console.print(f"[red]❌ Failed to get image information: {e}[/red]")
        raise typer.Exit(1)
