"""Utility functions for cflabs-serverless."""
...
def generate_unique_image_tag() -> str:
    """Generate a unique image tag to prevent caching issues."""
    # Use timestamp + hash of current directory for uniqueness
    timestamp = int(time.time())
    project_hash = hashlib.md5(str(Path.cwd()).encode()).hexdigest()[:8]
    return f"{timestamp}-{project_hash}"
...
def get_aws_error_guidance(error: Exception) -> str:
    ...
    if "accessdenied" in error_str or "unauthorized" in error_str:
        return (
            ...
            "1. [yellow]cflabs-serverless doctor[/yellow] - Check your setup\n"
            "2. [yellow]cflabs-serverless troubleshoot[/yellow] - View troubleshooting guide\n"
        ...
        return (
            ...
            "1. [yellow]cflabs-serverless build[/yellow] - Build the container first\n"
        ...
        return (
            ...
            "1. Update the library: [yellow]pip install --upgrade cflabs-serverless[/yellow]\n"
        ...
        return (
            ...
            "1. [yellow]cflabs-serverless doctor[/yellow] - Run diagnostics\n"
            "2. [yellow]cflabs-serverless troubleshoot[/yellow] - View troubleshooting guide\n"
