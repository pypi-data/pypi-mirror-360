...
def generate(
    ...
    environment: str = typer.Option("dev", "--env", "-e", help="Environment (dev, stage, prod)"),
):
...
def deploy(
    ...
    try:
        ...
        with Progress(
            ...
            # Create parameter file to avoid command line parsing issues with ECR URIs
            param_file_path = project_root / "sam-params.json"
            param_content = {
                "ImageUri": image_uri,
            ...
            with open(param_file_path, 'w') as f:
                json.dump(param_content, f)
            
            # Use parameter file to avoid command line parsing issues
            deploy_cmd = [
                "sam", "deploy",
                ...
                "--parameter-overrides", f"ImageUri={image_uri}, DeployEnvironment={environment}",
    ...
    try:
        ...
        if not force:
            confirmed = Confirm.ask(
                f"Are you sure you want to delete stack '{stack_name}' in region '{region}'?"
            )
            if not confirmed:
        ...
        delete_cmd = ["sam", "delete", "--stack-name", stack_name, "--region", region, "--no-prompts"]
        ...
        try:
            account_id = utils.get_aws_account_id()
            s3_bucket_name = f"{config['deployment']['lambda_name']}-deployment-{region}-{account_id}".lower().replace('_', '-')
            # Ensure bucket name is valid (3-63 characters, lowercase, no underscores)
            s3_bucket_name = ''.join(c for c in s3_bucket_name if c.isalnum() or c == '-')
            if len(s3_bucket_name) > 63:
                s3_bucket_name = s3_bucket_name[:63]
            ...
            console.print(f"[blue]Cleaning up S3 bucket: {s3_bucket_name}[/blue]")
            ...
            try:
                ...
                console.print(f"[green]✅ S3 bucket {s3_bucket_name} deleted successfully[/green]")
            except Exception as e:
                if "NoSuchBucket" in str(e):
                    console.print(f"[yellow]S3 bucket {s3_bucket_name} doesn't exist (already deleted)[/yellow]")
                else:
                    console.print(f"[yellow]Warning: Could not delete S3 bucket {s3_bucket_name}: {e}[/yellow]")
        except Exception as e:
            console.print(f"[yellow]Warning: Could not clean up S3 bucket: {e}[/yellow]")
