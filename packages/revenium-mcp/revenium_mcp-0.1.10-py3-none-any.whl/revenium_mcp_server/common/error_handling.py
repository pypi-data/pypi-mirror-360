...
from ..standard_errors import (
    StandardErrorBuilder, StandardErrorFormatter,
...
class IntrospectionError(ToolExecutionError):
    ...
    def __init__(self, message: str, tool_name: str = "", action: str = "", original_error: Optional[Exception] = None):
...
class UCMError(ToolExecutionError):
    ...
    def __init__(self, message: str, tool_name: str = "", action: str = "", ucm_available: bool = False):
        super().__init__(message, tool_name, action)
...
class UCMToolMixin:
    ...
    def __init__(self, ucm_helper=None, **kwargs):
        ...
        if self.ucm_helper:
            ...
            from loguru import logger
            logger.warning(f"⚠️ {self.__class__.__name__}: No UCM integration (using static capabilities)")
...
def format_structured_error(
    error: Union[ToolError, ResourceError],
...
__all__ = [
    ...
    "create_missing_parameter_error", "create_invalid_value_error",
    "create_validation_error", "create_api_error", "create_unknown_action_error",
    "handle_alert_tool_errors", "ValidationError", "InvalidInputError",
    "format_error_response", "ToolExecutionError", "IntrospectionError",
    "UCMError", "ToolNotFoundError", "UCMToolMixin",
    # New structured error classes and functions
    "ErrorCodes", "ToolError", "ResourceError",
    "format_structured_error", "create_structured_validation_error",
    "create_structured_missing_parameter_error", "create_resource_not_found_error"
]


def format_error_response(error: Exception, context: str = "") -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    if isinstance(error, ToolExecutionError):
        ...
        if "Direct error:" in error_message and any(pattern in error_message for pattern in api_error_patterns):
...
def _format_api_validation_error(api_error: "ReveniumAPIError", context: str = "") -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
...
def _extract_validation_context(error_message: str, status_code: int) -> dict:
    ...
    elif status_code == 400 and ("required" in error_message.lower() or "missing" in error_message.lower()):
        context.update({
