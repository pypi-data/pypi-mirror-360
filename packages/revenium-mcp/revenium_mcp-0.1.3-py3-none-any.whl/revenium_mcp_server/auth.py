...
class AuthConfig(BaseModel):
    ...
    tenant_id: Optional[str] = Field(None, description="Revenium tenant ID (for endpoints that require tenantId instead of teamId)")
    base_url: str = Field(
        default="https://api.revenium.io",
        description="Base URL for Revenium API"
    )
    timeout: float = Field(
        default=30.0,
