...
enabling profile-based tool loading (starter/business/enterprise).
"""
...
class ToolConfigurationRegistry:
    ...
    async def register_tools_conditionally(self, mcp: FastMCP) -> None:
        """Register tools based on configuration profile.
        
        Args:
            mcp: FastMCP server instance for tool registration
        """
        enabled_tools = self.tool_config.get_enabled_tools()
        logger.info(f"Registering {len(enabled_tools)} tools for profile '{self.tool_config.profile}'")
        ...
        logger.info(f"Successfully registered {len(self._registered_tools)} tools")
    ...
    async def _register_single_tool(self, mcp: FastMCP, tool_name: str) -> None:
        ...
        try:
            ...
            if tool_name == "business_analytics_management":
                ...
                await self._register_system_monitoring(mcp)
    ...
    async def _register_system_monitoring(self, mcp: FastMCP) -> None:
        """Register consolidated system monitoring tool."""
        @mcp.tool()
        @dynamic_mcp_tool("system_monitoring")
        async def system_monitoring(
            action: str = "performance_dashboard"
        ) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
            ...
            # Import consolidated tool class
            from ..tools_decomposed.system_monitoring import SystemMonitoring
            from ..common.tool_execution import standardized_tool_execution

            # Use standardized execution path
            result = await standardized_tool_execution(
                tool_name="system_monitoring",
                action=action,
                arguments=arguments,
                tool_class=SystemMonitoring
            )
            return result
    ...
    async def _register_tool_introspection(self, mcp: FastMCP) -> None:
        ...
        # Note: tool_introspection is already registered in enhanced_server.py
        # This method exists for profile consistency but doesn't need to re-register
        logger.debug("tool_introspection already registered via introspection integration")
