...
class AutoDiscoveryService:
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None):
        ...
        self.base_url = base_url or "https://api.revenium.io/meter"
    ...
    def _extract_configuration(self, user_data: Dict) -> Dict[str, str]:
        ...
        if not owner_id:
            raise AutoDiscoveryError(
                "User ID not found in response. Please contact support."
            )
        ...
        if not team_id:
            raise AutoDiscoveryError(
                "Invalid team data found. Please contact support."
            )
    ...
    def _log_discovery_results(self, user_data: Dict, config: Dict[str, str]) -> None:
        ...
        if self.base_url != "https://api.revenium.io/meter":
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                response = await client.get(
                    ...
                    params={
                        "teamId": config["REVENIUM_TEAM_ID"],
                        "page": 0,
                        "size": 1
                    }
                )
