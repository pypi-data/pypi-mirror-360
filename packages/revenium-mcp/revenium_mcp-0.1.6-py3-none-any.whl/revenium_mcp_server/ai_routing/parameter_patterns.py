...
class ParameterPatterns:
    ...
    def _build_name_patterns(self) -> List[Pattern]:
        ...
        return [
            ...
            re.compile(r'(?:called|named)\s+([A-Za-z][A-Za-z0-9\s]+?)(?:\s+with|\s+type|\s*$)', re.IGNORECASE),
            re.compile(r'product\s+"([^"]+)"', re.IGNORECASE),
            re.compile(r'product\s+([A-Za-z][A-Za-z0-9\s]+?)(?:\s+with|\s+type|\s*$)', re.IGNORECASE),
            re.compile(r'customer\s+"([^"]+)"', re.IGNORECASE),
            re.compile(r'customer\s+([A-Za-z][A-Za-z0-9\s]+?)(?:\s+with|\s+type|\s*$)', re.IGNORECASE),
            re.compile(r'workflow\s+"([^"]+)"', re.IGNORECASE),
            re.compile(r'workflow\s+([A-Za-z][A-Za-z0-9\s]+?)(?:\s+with|\s+type|\s*$)', re.IGNORECASE),
    ...
    def _build_product_type_patterns(self) -> List[Pattern]:
        ...
        return [
            re.compile(r'\btype\s*[:\-]?\s*(api|usage|subscription|metering|[a-zA-Z_]+)', re.IGNORECASE),
    ...
    def _build_workflow_type_patterns(self) -> List[Pattern]:
        ...
        return [
            ...
            re.compile(r'\b(subscription_setup|customer_onboarding|product_creation)\b', re.IGNORECASE),
        ]
    ...
    def _build_status_patterns(self) -> List[Pattern]:
        ...
        return [
            re.compile(r'\bstatus\s*[:\-]?\s*(active|inactive|pending|completed|failed)', re.IGNORECASE),
