...
from ..introspection.integration import IntrospectionIntegration
...
class ConditionalToolRegistry:
    ...
    async def register_tools_conditionally(
        self, 
        mcp_server, 
        introspection_engine: Optional[ToolIntrospectionEngine] = None
    ) -> Dict[str, bool]:
        ...
        for tool_name, should_register in tools_to_register.items():
            if should_register:
                ...
                logger.debug(f"⏭️ Skipping tool registration: {tool_name} (not needed for current state)")
                registration_results[tool_name] = False
        ...
        logger.info(f"Conditional registration complete: {registered_count}/{len(tools_to_register)} tools registered")
    ...
    async def _determine_tools_to_register(self, onboarding_state) -> Dict[str, bool]:
        ...
        if onboarding_state.is_first_time:
            ...
            logger.debug("Returning user detected: registering all onboarding tools (useful for configuration management)")
    ...
    def get_registered_tools(self) -> List[str]:
        ...
        """
        return [tool_name for tool_name, registered in self._registration_cache.items() if registered]
    ...
    def get_registration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_state_check": self._last_state_check.isoformat() if self._last_state_check else None,
