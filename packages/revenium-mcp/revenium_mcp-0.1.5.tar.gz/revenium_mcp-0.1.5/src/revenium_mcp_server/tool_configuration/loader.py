...
5. Default: business (complete AI analytics and usage-based billing)
"""
...
class ConfigurationLoader:
    ...
    def load_environment_config(self) -> Tuple[str, Dict[str, bool]]:
        ...
        profile = os.getenv("TOOL_PROFILE", "business")
        if profile not in PROFILE_DEFINITIONS:
            logger.warning(f"Invalid TOOL_PROFILE '{profile}', using 'business'")
            profile = "business"
    ...
    def save_json_config(
        self,
        profile: str,
        custom_overrides: Dict[str, bool],
        version: int = 1
    ) -> bool:
    ...
    def validate_configuration(
        ...
        # Validate profile
        if profile not in PROFILE_DEFINITIONS:
            logger.warning(f"Invalid profile '{profile}', using 'business'")
            profile = "business"
