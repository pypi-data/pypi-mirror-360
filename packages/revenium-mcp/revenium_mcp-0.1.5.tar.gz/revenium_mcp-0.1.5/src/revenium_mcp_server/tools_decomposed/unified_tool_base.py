...
from ..introspection.metadata import (
    MetadataProvider, ToolMetadata, PerformanceMetrics, ToolType,
    ToolCapability, UsagePattern, ToolDependency, ResourceRelationship
)
...
class ToolBase(ABC, MetadataProvider):
    ...
    async def update_performance_metrics(self, execution_time: float, success: bool):
        ...
        if self._performance_metrics.total_executions == 1:
            ...
            # Update average
            total_time = (self._performance_metrics.avg_response_time_ms *
                         (self._performance_metrics.total_executions - 1) + execution_time)
            self._performance_metrics.avg_response_time_ms = total_time / self._performance_metrics.total_executions
        ...
        try:
            ...
            if hasattr(self, 'ucm_helper') and self.ucm_helper:
                try:
                    ...
                    resource_type = getattr(self, 'resource_type', self.tool_name.replace('_management', ''))
