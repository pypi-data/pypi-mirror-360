...
from .response_cache import response_cache, ResponseCacheManager
...
class CacheEntry:
    ...
    def should_warm(self, warming_threshold: float = 0.8) -> bool:
        ...
        if self.ttl_seconds is None:
            return False
        age_ratio = (datetime.now(timezone.utc) - self.created_at).total_seconds() / self.ttl_seconds
...
class RedisLikeCache:
    ...
    async def get_stats(self) -> CacheStats:
        ...
        async with self._lock:
            ...
            hot_keys = [key for key, _ in sorted(key_access_counts, key=lambda x: x[1], reverse=True)[:10]]
    ...
    def _update_access_time(self, start_time: float):
        ...
        if self._stats.avg_access_time_ms == 0:
            ...
            self._stats.avg_access_time_ms = (self._stats.avg_access_time_ms * 0.9) + (access_time_ms * 0.1)
