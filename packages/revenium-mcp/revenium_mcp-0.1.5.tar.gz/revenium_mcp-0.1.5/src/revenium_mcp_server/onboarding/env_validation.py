...
class EnvironmentVariableValidator:
    ...
    async def test_auth_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if api_key:
                ...
                if not team_id:
                    auth_result["warnings"].append("TEAM_ID not set - team-specific features will be limited")
                if not tenant_id:
                    auth_result["warnings"].append("TENANT_ID not set - some advanced features may be limited")
    ...
    async def validate_all_debug_auto_discovery_format(self) -> ValidationResult:
        ...
        auto_discovery_works = discovered_config.get('status') == 'success' and discovered_count >= 1
