...
from .models import (
    PlanType, Currency, BillingPeriod, TrialPeriod, 
    AggregationType, RatingAggregationType
)
...
class ProductValidator:
    ...
    @staticmethod
    def validate_email_list(emails: List[str], field_name: str = "notification_addresses") -> List[str]:
...
class PlanValidator:
    ...
    def validate_plan_structure(plan_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Validate currency
        try:
            currency = ProductValidator.validate_enum_value(
        ...
        has_rating_agg_tiers = any(
            ra.get("tiers") and len(ra.get("tiers", [])) > 0
            for ra in rating_aggregations
        )
...
class TierValidator:
    ...
    def validate_tiers_list(tiers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, tier in enumerate(tiers):
            try:
                ...
                name = ProductValidator.validate_product_name(tier.get("name", f"Tier {i+1}"))

                # Validate up_to value
                up_to_val = TierValidator.validate_tier_range(tier.get("up_to"))
    ...
    @staticmethod
    def _ranges_overlap(start1: Decimal, end1: Optional[Decimal],
                       start2: Decimal, end2: Optional[Decimal]) -> bool:
...
class ElementValidator:
    ...
    def validate_element_structure(element_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Validate name
        name = ProductValidator.validate_product_name(element_data["name"])
    ...
    def validate_elements_list(elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, element in enumerate(elements):
            try:
                ...
                # Create new ValidationError with updated field path
                original_field = e.details.get("field", "")
                new_field = f"elements[{i}].{original_field}" if original_field else f"elements[{i}]"
                raise ValidationError(
                    message=e.message,
                    field=new_field,
                    value=e.details.get("provided_value"),
...
class SetupFeeValidator:
    ...
    def validate_setup_fee_structure(setup_fee_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not setup_fee_data.get("name"):
            raise ValidationError(
                message="Setup fee name is required",
                field="name",
                expected="Non-empty string"
            )
    ...
    def validate_setup_fees_list(setup_fees: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, fee in enumerate(setup_fees):
            try:
                ...
                # Create new ValidationError with updated field path
                original_field = e.details.get("field", "")
                new_field = f"setup_fees[{i}].{original_field}" if original_field else f"setup_fees[{i}]"
                raise ValidationError(
                    message=e.message,
                    field=new_field,
                    value=e.details.get("provided_value"),
...
class RatingAggregationValidator:
    ...
    @staticmethod
    def validate_rating_aggregations_list(rating_aggs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, agg in enumerate(rating_aggs):
            try:
                validated_agg = RatingAggregationValidator.validate_rating_aggregation_structure(agg)
                ...
                # Create new ValidationError with updated field path
                original_field = e.details.get("field", "")
                new_field = f"rating_aggregations[{i}].{original_field}" if original_field else f"rating_aggregations[{i}]"
                raise ValidationError(
                    message=e.message,
                    field=new_field,
                    value=e.details.get("provided_value"),
