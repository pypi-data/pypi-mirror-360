...
class CapabilityVerifier:
    ...
    def __init__(self, client: ReveniumClient):
        ...
        self._l1_cache: Dict[str, Dict[str, Any]] = {}  # In-memory, 2-minute TTL (reduced for performance)
        self._l2_cache: Dict[str, Dict[str, Any]] = {}  # Persistent, 15-minute TTL (reduced for performance)
        self._l3_cache: Dict[str, Dict[str, Any]] = {}  # Historical, 1-hour TTL (reduced for performance)
    ...
    def _record_api_failure(self):
        ...
        if self._api_failure_count >= self._max_failures:
            ...
            logger.warning(f"Circuit breaker opened due to {self._api_failure_count} failures. "
    ...
    def _get_cached_capabilities(self, capability_type: str, resource_type: str = "default") -> Optional[Set[str]]:
    ...
    def _cache_capabilities(self, capability_type: str, values: Set[str], resource_type: str = "default") -> None:
    ...
    async def _discover_capabilities_from_api(self, capability_type: str, endpoint: str,
                                            field_path: str, resource_type: str = "default") -> Set[str]:
        ...
        try:
            ...
            if discovered_values:
                logger.info(f"Discovered {len(discovered_values)} {capability_type} values from API: {discovered_values}")
    ...
    async def verify_capabilities(self, resource_type: str, capabilities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _verify_capability_values(self, resource_type: str, capability_name: str, values: Any) -> Any:
        ...
        if not strategy:
            logger.debug(f"No verification strategy for {capability_name}, returning original values")
            return values
    ...
    async def _verify_schema_values(self, resource_type: str, capability_name: str, schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _verify_currency_capability(self, resource_type: str, currency: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                raise ValueError(f"Currency verification unavailable - circuit breaker open and no cached data for {currency}")

            # Try to get from cache first (L1/L2)
            cached_currencies = self._get_cached_capabilities("currencies", resource_type)
            if cached_currencies:
                logger.debug(f"Using cached currencies for verification: {len(cached_currencies)} currencies")
                return currency in cached_currencies
            ...
            if discovered_currencies:
                ...
                logger.debug(f"Currency {currency} verification result from API discovery: {result}")
                return result
            ...
            if alternative_currencies:
                ...
                logger.debug(f"Currency {currency} verification result from alternative discovery: {result}")
                return result
            ...
            raise ValueError(f"Currency verification failed - no API discovery methods successful for {currency}")
    ...
    async def _discover_currencies_alternative(self) -> Set[str]:
        ...
        try:
            ...
            if currencies:
                logger.info(f"Discovered {len(currencies)} currencies from subscriptions: {currencies}")
                return currencies
    ...
    async def _verify_plan_type_capability(self, resource_type: str, plan_type: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Plan type verification unavailable - circuit breaker open and no cached data for {plan_type}")

            # Try to get from cache first (L1/L2)
            cached_plan_types = self._get_cached_capabilities("plan_types", resource_type)
            if cached_plan_types:
                logger.debug(f"Using cached plan types for verification: {len(cached_plan_types)} types")
                return plan_type in cached_plan_types
            ...
            if discovered_plan_types:
                ...
                logger.debug(f"Plan type {plan_type} verification result from API discovery: {result}")
                return result
            ...
            if alternative_plan_types:
                ...
                logger.debug(f"Plan type {plan_type} verification result from alternative discovery: {result}")
                return result

            # If all discovery methods fail, raise error to force proper API integration
            logger.error(f"All plan type discovery methods failed for {plan_type}")
            raise ValueError(f"Plan type verification failed - no API discovery methods successful for {plan_type}")
    ...
    async def _discover_plan_types_alternative(self) -> Set[str]:
        ...
        try:
            ...
            if plan_types:
                logger.info(f"Discovered {len(plan_types)} plan types from subscriptions: {plan_types}")
                return plan_types
    ...
    async def _verify_billing_period_capability(self, resource_type: str, period: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Billing period verification unavailable - circuit breaker open and no cached data for {period}")
            ...
            raise ValueError(f"Billing period verification failed - no API discovery methods successful for {period}")
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                if cached_periods:
                    return period in cached_periods
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Trial period verification unavailable - circuit breaker open and no cached data for {period}")
            ...
            raise ValueError(f"Trial period verification failed - no API discovery methods successful for {period}")
    ...
    async def _verify_user_role_capability(self, resource_type: str, role: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"User role verification unavailable - circuit breaker open and no cached data for {role}")
            ...
            raise ValueError(f"User role verification failed - no API discovery methods successful for {role}")
    ...
    async def _verify_organization_type_capability(self, resource_type: str, org_type: str) -> bool:
        ...
        try:
            # Check circuit breaker
            if self._is_circuit_open():
                logger.debug(f"Circuit breaker open, using cached data for organization type {org_type}")
                cached_org_types = self._get_cached_capabilities("organization_types", resource_type)
                if cached_org_types:
                    return org_type in cached_org_types
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Organization type verification unavailable - circuit breaker open and no cached data for {org_type}")
            ...
            if discovered_org_types:
                ...
                logger.debug(f"Organization type {org_type} verification from API discovery: {result}")
                return result
            ...
            raise ValueError(f"Organization type verification failed - no API discovery methods successful for {org_type}")
    ...
    async def _verify_alert_type_capability(self, resource_type: str, alert_type: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Alert type verification unavailable - circuit breaker open and no cached data for {alert_type}")
            ...
            if discovered_alert_types:
                ...
                logger.debug(f"Alert type {alert_type} verification from discovery service: {result}")
                return result

            # If all discovery methods fail, raise error to force proper API integration
            logger.error(f"All alert type discovery methods failed for {alert_type}")
            raise ValueError(f"Alert type verification failed - no API discovery methods successful for {alert_type}")
    ...
    async def _verify_metric_capability(self, resource_type: str, metric: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Metric verification unavailable - circuit breaker open and no cached data for {metric}")
            ...
            if resource_type == "alerts":
                ...
                raise ValueError(f"Metric verification not supported for resource type: {resource_type}")
            ...
            raise ValueError(f"Metric verification failed - no API discovery methods successful for {metric}")
    ...
    async def _verify_operator_capability(self, resource_type: str, operator: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Operator verification unavailable - circuit breaker open and no cached data for {operator}")
            ...
            raise ValueError(f"Operator verification failed - no API discovery methods successful for {operator}")
    ...
    async def _verify_source_type_capability(self, resource_type: str, source_type: str) -> bool:
        ...
        try:
            # Check circuit breaker first
            if self._is_circuit_open():
                logger.debug(f"Circuit breaker open, using cached data for source type {source_type}")
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Source type verification unavailable - circuit breaker open and no cached data for {source_type}")
            ...
            raise ValueError(f"Source type verification failed - no API discovery methods successful for {source_type}")
    ...
    async def _verify_element_type_capability(self, resource_type: str, element_type: str) -> bool:
        ...
        try:
            # Check circuit breaker first
            if self._is_circuit_open():
                logger.debug(f"Circuit breaker open, using cached data for element type {element_type}")
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Element type verification unavailable - circuit breaker open and no cached data for {element_type}")
            ...
            if discovered_element_types:
                ...
                logger.debug(f"Element type {element_type} verification from API discovery: {result}")
                return result
            ...
            raise ValueError(f"Element type verification failed - no API discovery methods successful for {element_type}")
    ...
    async def _verify_provider_capability(self, resource_type: str, provider: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Provider verification unavailable - circuit breaker open and no cached data for {provider}")
            ...
            try:
                ...
                if "_embedded" in models_response and "aIModelResourceList" in models_response["_embedded"]:
                ...
                logger.info(f"Discovered {len(discovered_providers)} providers from AI models endpoint: {sorted(list(discovered_providers))}")
                ...
                if discovered_providers:
                    ...
                    logger.debug(f"Provider {provider} verification from AI models endpoint: {result}")
                    return result

            except Exception as api_error:
                self._record_api_failure()
                logger.warning(f"AI models endpoint call failed for provider verification: {api_error}")
                # No conservative fallback - raise error to force proper API integration
                raise ValueError(f"Provider verification failed - AI models endpoint unavailable: {str(api_error)}")
            ...
            raise ValueError(f"Provider verification failed - no API discovery methods successful for {provider}")
    ...
    async def _verify_model_capability(self, resource_type: str, model: str) -> bool:
        ...
        try:
            ...
            if self._is_circuit_open():
                ...
                # No hardcoded fallback - raise error to force proper API integration
                raise ValueError(f"Model verification unavailable - circuit breaker open and no cached data for {model}")
            ...
            try:
                ...
                self._record_api_failure()
                logger.warning(f"AI models endpoint call failed for model verification: {api_error}")
                # No conservative fallback - raise error to force proper API integration
                raise ValueError(f"Model verification failed - AI models endpoint unavailable: {str(api_error)}")

            # If no models discovered, raise error to force proper API integration
            logger.error(f"No models discovered for {model}")
            raise ValueError(f"Model verification failed - no API discovery methods successful for {model}")
