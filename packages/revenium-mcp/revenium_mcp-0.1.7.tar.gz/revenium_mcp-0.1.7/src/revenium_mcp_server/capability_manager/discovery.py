...
from ..models import (
    Currency, BillingPeriod, TrialPeriod, PlanType, AggregationType,
    RatingAggregationType, UserRole, OrganizationType, PaymentSource,
    MetricType
)
...
class CapabilityDiscovery:
    ...
    async def _discover_product_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "payment_sources": [payment.value for payment in PaymentSource],  # DOCUMENTED_OPERATIONAL_ENUM
            "aggregation_types": [agg_type.value for agg_type in AggregationType],
            "rating_aggregation_types": [rating_type.value for rating_type in RatingAggregationType],
            "schema": {
                "product_data": {
                    "required": ["name", "version", "plan"],
                    "optional": [
                        "description", "source_ids", "sla_ids", "custom_pricing_rule_ids",
                        "notification_addresses_on_invoice", "tags", "terms", "coming_soon",
                        "setup_fees", "rating_aggregations", "elements"
                    ],
                    "plan_schema": {
    ...
    async def _discover_subscription_capabilities(self) -> Dict[str, Any]:
        """Discover subscription-related capabilities from API."""
        # Discover capabilities from actual API endpoints instead of hardcoded arrays
    ...
    async def _discover_customer_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "schemas": {
                "users": {
                    "required": ["email", "firstName", "lastName"],  # VERIFIED: All present in API responses
                    "optional": ["phoneNumber", "homepagePreference", "roles", "primaryUser", "subscriberId"]  # VERIFIED: From API
                },
                "organizations": {
                    "required": ["name"],  # VERIFIED: Only name is required by API
                    "optional": [
                        "externalId", "logo", "address1", "address2", "city", "state", "country",
                        "postalCode", "billingPhoneNumber", "phoneNumber", "emailAddress",
                        "remittanceInstructions", "url", "currency", "metadata", "domainAliases",
                        "types", "billingEmail", "sourceAutoDiscoveryEnabled",
                ...
                "subscribers": {
                    ...
                    "optional": ["firstName", "lastName", "metadata", "organization_id"]  # VERIFIED: From API patterns
                }
            },
            "validation_rules": {
                ...
                "name": {"type": "string", "min_length": 1, "max_length": 255},
                "roles": {"enum": ["ROLE_TENANT_ADMIN", "ROLE_API_CONSUMER"]},  # VERIFIED: From actual API
    ...
    async def _discover_alert_capabilities(self) -> Dict[str, Any]:
        ...
        performance_metrics = [m for m in all_metrics if any(perf in m for perf in ["PER_MINUTE", "RATE"])]
    ...
    async def _discover_source_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "schema": {
                "source_data": {
                    "required": ["name", "description", "version", "type"],  # VERIFIED: Required by API
                    "optional": [
                        "sourceType", "syncedWithApiGateway", "autoDiscoveryEnabled",
                        "tags", "sourceClassifications", "metadata", "logoURL",
                        "devPortalLink", "externalId", "externalUsagePlanId"
            ...
            "validation_rules": {
                ...
                "version": {"type": "string", "pattern": r"^\d+\.\d+\.\d+$"},  # VERIFIED: Required format
    ...
    async def _discover_metering_element_capabilities(self) -> Dict[str, Any]:
        """Discover metering element capabilities."""
        return {
            "element_types": ["NUMBER", "STRING"],
            "aggregation_types": [agg_type.value for agg_type in AggregationType],
            "units_of_measure": [
                "count", "bytes", "seconds", "requests", "tokens", 
                "dollars", "percentage", "ratio"
            ],
            "schema": {
                "element_data": {
                    "required": ["name", "type"],
                    "optional": [
                        "description", "unit_of_measure", "default_value",
                        "aggregation_type", "metadata"
                    ]
                }
            },
            "validation_rules": {
                ...
                "aggregation_type": {"enum": [agg_type.value for agg_type in AggregationType]}
            },
            "templates": {
                "ai_cost_tracking": {
                    "elements": ["input_tokens", "output_tokens", "total_cost"],
                    "description": "Standard AI cost tracking elements"
                },
                "api_monitoring": {
                    "elements": ["request_count", "response_time", "error_count"],
                    "description": "API performance monitoring elements"
                }
            }
        }
    ...
    async def _discover_metering_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Discovered AI metering capabilities: {len(transaction_fields.get('required', []))} required fields, "
            ...
            return {
                ...
                "validation_requirements": {
                    ...
                    "lookup_tools": ["list_ai_models", "search_ai_models", "get_supported_providers"]
                },

                "schema": transaction_fields.get("schema", {
                    "transaction_data": {
                        "required": ["model", "provider", "input_tokens", "output_tokens", "duration_ms"],
                        "optional": [
                            "organization_id", "subscriber_email", "task_type", "agent", "trace_id",
                            "product_id", "subscription_id", "subscriber_credential",
                            "subscriber_credential_name", "task_id", "response_quality_score",
                            "stop_reason", "is_streamed"
                        ]
                    }
                }),
                "validation_rules": transaction_fields.get("validation_rules", {
    ...
    async def _discover_traditional_metrics(self) -> List[str]:
        ...
        try:
            ...
            if not metrics:
                metrics = {
                    "TOTAL_COST", "TOKEN_COUNT", "INPUT_TOKEN_COUNT", "OUTPUT_TOKEN_COUNT",
                    "TOKENS_PER_MINUTE", "REQUESTS_PER_MINUTE", "ERROR_RATE", "ERROR_COUNT",
                    "LATENCY", "THROUGHPUT", "USAGE_COUNT"
                }
            ...
            return ["TOTAL_COST", "TOKEN_COUNT", "ERROR_RATE", "TOKENS_PER_MINUTE", "REQUESTS_PER_MINUTE"]
    ...
    async def _get_models_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Models summary: {total_models} total models, {len(provider_list)} providers sampled")
    ...
    async def _discover_subscription_types_from_api(self) -> List[str]:
        ...
        try:
            ...
            if types:
                ...
                raise ValueError("No subscription types discovered from API - UCM integration required")
