...
class InputValidator:
    ...
    def validate_numeric_range(value: Any, field_name: str, min_value: Optional[float] = None, 
                             max_value: Optional[float] = None, allow_zero: bool = True) -> Union[int, float]:
    ...
    def validate_detection_rule(rule: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if rule_type_upper not in ["THRESHOLD", "STATISTICAL", "PATTERN", "ANOMALY", "TREND", "CUMULATIVE_USAGE"]:
            raise ValidationError(
    ...
    def convert_to_api_format(user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not user_data.get("detection_rules"):
            raise ValidationError(
                message="Detection rules are required",
        ...
        api_data = {
            ...
            "notificationAddresses": (
                user_data.get("notificationAddresses") or
                user_data.get("notification_addresses") or
                []
            ),
            "slackConfigurations": user_data.get("slack_configurations", []),
            "webhookConfigurations": user_data.get("webhook_configurations", []),  # Correct field name
        ...
        logger.debug(f"Converting period for {alert_type}: rule.time_window='{period}', user_data keys: {list(user_data.keys())}")

        if alert_type == "CUMULATIVE_USAGE":
            # For CUMULATIVE_USAGE, if time_window is missing, check user_data for period
            if "time_window" not in rule:
                logger.info(f"CUMULATIVE_USAGE alert missing time_window, checking user_data for period")
                period = (
                    user_data.get("period") or
                    user_data.get("trackingPeriod") or
                    user_data.get("periodDuration") or
                    "daily"  # Final fallback to daily (valid for CUMULATIVE_USAGE)
                )
                logger.debug(f"CUMULATIVE_USAGE: Using period from user_data or fallback: '{period}'")
            ...
            if period_lower in cumulative_period_mapping:
                ...
                logger.debug(f"CUMULATIVE_USAGE: Mapped '{period}' to '{cumulative_period_mapping[period_lower]}'")
            else:
                # Default to DAILY for CUMULATIVE_USAGE if period not recognized (changed from MONTHLY)
                api_data["periodDuration"] = "DAILY"
                logger.warning(f"CUMULATIVE_USAGE: Unrecognized period '{period}', defaulting to DAILY")
        else:
...
class AdvancedConfigurationBuilder:
    ...
    @staticmethod
    def create_filter(field: str, operator: str, value: Union[str, int, float, bool]) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_slack_config(webhook_url: str, channel: Optional[str] = None,
                          username: Optional[str] = None, icon_emoji: Optional[str] = None) -> str:
    ...
    @staticmethod
    def create_webhook_config(url: str, method: str = "POST",
...
ANOMALY_VALIDATION_RULES = {
    ...
    "detection_rules": lambda rules: [InputValidator.validate_detection_rule(rule) for rule in rules] if rules else [],
...
PAGINATION_VALIDATION_RULES = {
    "page": lambda x: InputValidator.validate_numeric_range(x, "page", min_value=0, allow_zero=True),
    "size": lambda x: InputValidator.validate_numeric_range(x, "size", min_value=1, max_value=1000, allow_zero=False),
}
