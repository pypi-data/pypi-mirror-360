...
class NamingStandards:
    ...
    STANDARD_ID_FIELDS = {
        "product_id", "subscription_id", "source_id", "user_id", 
        "subscriber_id", "organization_id", "team_id", "anomaly_id", 
        "alert_id", "workflow_id"
    }
    
    STANDARD_DATA_FIELDS = {
        "product_data", "subscription_data", "source_data", "user_data",
        "subscriber_data", "organization_data", "team_data", "anomaly_data"
    }
    
    STANDARD_COMMON_FIELDS = {
        "action", "resource_type", "page", "size", "filters", "query",
        "dry_run", "example_type", "step_result", "context"
    }
    
    # Standard action names
    STANDARD_ACTIONS = {
        "list", "get", "create", "update", "delete", "cancel",
        "get_capabilities", "get_examples", "validate", "get_agent_summary",
        "analyze", "clear_all", "get_metrics", "query"
    }
    
    # Standard resource types
    STANDARD_RESOURCE_TYPES = {
        "products", "subscriptions", "sources", "users", "subscribers",
        "organizations", "teams", "anomalies", "alerts", "workflows"
    }
    ...
    def suggest_standard_name(current_name: str, context: str = "") -> str:
        ...
        if context == "id_field" and not snake_name.endswith("_id"):
            ...
            snake_name += "_data"
        elif context == "boolean" and not any(snake_name.startswith(p) for p in NamingStandards.BOOLEAN_PREFIXES):
...
def validate_new_parameter(param_name: str, context: str = "") -> Dict[str, Any]:
    ...
    if not result["compliant"]:
        result["suggestions"].append(f"Use snake_case: {naming_standards.convert_to_snake_case(param_name)}")
