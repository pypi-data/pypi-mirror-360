...
class ComparativeAnalyticsProcessor:
    ...
    async def compare_periods(
        ...
        """
        logger.info(f"Comparing periods: {current_period} vs {previous_period} for {metric_type} by {breakdown_by}")
        
        try:
            # Validate inputs
            await self._validate_comparison_inputs(metric_type, breakdown_by, current_period, previous_period, group)
            
            # Fetch data for both periods concurrently
            results = await asyncio.gather(
                self._fetch_period_data(client, team_id, current_period, metric_type, breakdown_by, group),
                self._fetch_period_data(client, team_id, previous_period, metric_type, breakdown_by, group),
            ...
            percentage_changes = self._calculate_percentage_changes(current_data, comparison_data, breakdown_by)
            
            # Generate insights and recommendations
            insights = self._generate_comparison_insights(percentage_changes, metric_type, breakdown_by)
            recommendations = self._generate_comparison_recommendations(percentage_changes, metric_type)
    ...
    async def compare_models(
        ...
        """
        logger.info(f"Comparing models: {model_a} vs {model_b} for {metric_type} over {time_period}")
        ...
        try:
            ...
            percentage_changes = self._calculate_model_comparison_changes(model_a_data, model_b_data, metric_type, model_a, model_b)
            
            # Generate insights
            insights = self._generate_model_comparison_insights(model_a, model_b, percentage_changes, metric_type)
            recommendations = self._generate_model_recommendations(model_a, model_b, percentage_changes)
    ...
    async def compare_providers(
        ...
        """
        logger.info(f"Comparing providers: {provider_a} vs {provider_b} for {metric_type} over {time_period}")
        ...
        try:
            ...
            # Calculate comparison metrics
            percentage_changes = self._calculate_provider_comparison_changes(provider_a_data, provider_b_data, metric_type, provider_a, provider_b)

            # Generate insights
            insights = self._generate_provider_comparison_insights(provider_a, provider_b, percentage_changes, metric_type)
            recommendations = self._generate_provider_recommendations(provider_a, provider_b, percentage_changes)
    ...
    async def benchmark_customers(
        ...
        """
        logger.info(f"Benchmarking customer {customer_id} against {benchmark_type} for {time_period}")
        ...
        try:
            ...
            performance_vs_benchmark = self._calculate_benchmark_performance(customer_data, benchmark_data)
    ...
    async def _fetch_period_data(
        ...
        if "cost-metric-by-provider-over-time" in endpoint or "performance-metric-by-provider" in endpoint:
    ...
    def _calculate_percentage_changes(
        ...
        for item in comparison_items:
            if isinstance(item, dict):
                key = item.get(breakdown_by) or item.get("provider") or item.get("model") or item.get("organization")
                if key:
        ...
        for current_item in current_items:
            ...
            key = current_item.get(breakdown_by) or current_item.get("provider") or current_item.get("model") or current_item.get("organization")
            if not key:
                continue
            ...
            current_value = float(current_item.get("cost", 0) or current_item.get("revenue", 0) or current_item.get("performance", 0))

            comparison_item = comparison_lookup.get(normalized_key, {})
            previous_value = float(comparison_item.get("cost", 0) or comparison_item.get("revenue", 0) or comparison_item.get("performance", 0))
            ...
            percentage_changes.append(PercentageChange(
    ...
    def _generate_comparison_insights(
        ...
        significant_increases = [pc for pc in percentage_changes if pc.significance == "significant" and pc.trend_direction == "increasing"]
        significant_decreases = [pc for pc in percentage_changes if pc.significance == "significant" and pc.trend_direction == "decreasing"]

        if significant_increases:
            avg_increase = sum(pc.percentage_change for pc in significant_increases) / len(significant_increases)
            insights.append(f"Significant {metric_type} increases detected: {len(significant_increases)} {breakdown_by}(s) with average increase of {avg_increase:.1f}%")

        if significant_decreases:
            avg_decrease = sum(abs(pc.percentage_change) for pc in significant_decreases) / len(significant_decreases)
            insights.append(f"Significant {metric_type} decreases detected: {len(significant_decreases)} {breakdown_by}(s) with average decrease of {avg_decrease:.1f}%")

        # Overall trend analysis
        total_changes = len(percentage_changes)
        increasing_count = len([pc for pc in percentage_changes if pc.trend_direction == "increasing"])
        decreasing_count = len([pc for pc in percentage_changes if pc.trend_direction == "decreasing"])

        if increasing_count > decreasing_count:
            insights.append(f"Overall trend: {metric_type} is increasing across most {breakdown_by}s ({increasing_count}/{total_changes})")
        elif decreasing_count > increasing_count:
            insights.append(f"Overall trend: {metric_type} is decreasing across most {breakdown_by}s ({decreasing_count}/{total_changes})")
        else:
            insights.append(f"Overall trend: {metric_type} changes are mixed across {breakdown_by}s")
    ...
    def _generate_comparison_recommendations(
        ...
        if not percentage_changes:
            recommendations.append(f"Gather more {metric_type} data to enable meaningful comparisons")
            return recommendations

        # Enhanced recommendations based on significant changes with quantitative thresholds
        significant_increases = [pc for pc in percentage_changes if pc.significance == "significant" and pc.trend_direction == "increasing"]
        significant_decreases = [pc for pc in percentage_changes if pc.significance == "significant" and pc.trend_direction == "decreasing"]
        major_increases = [pc for pc in percentage_changes if abs(pc.percentage_change) > 50 and pc.trend_direction == "increasing"]
        ...
        if significant_increases and metric_type == "cost":
            ...
            recommendations.append(f"🚨 **Immediate Action Required**: {len(significant_increases)} entities show significant cost increases (avg: {avg_increase:.1f}%, max: {max_increase:.1f}%)")
            recommendations.append(f"🔍 **Investigation Priority**: Focus on entities with >50% increases - review usage patterns and implement cost controls within 48 hours")

            if major_increases:
                top_increase = max(major_increases, key=lambda x: x.percentage_change)
                recommendations.append(f"⚠️ **Critical Review**: {top_increase.entity_name} increased by {top_increase.percentage_change:.1f}% - requires immediate cost analysis and potential usage caps")

        if significant_decreases and metric_type == "cost":
            # Calculate average decrease for specific insights
            avg_decrease = abs(sum(pc.percentage_change for pc in significant_decreases) / len(significant_decreases))
            total_savings = sum(pc.current_value - pc.previous_value for pc in significant_decreases)

            recommendations.append(f"✅ **Cost Optimization Success**: {len(significant_decreases)} entities achieved significant cost reductions (avg: {avg_decrease:.1f}% decrease)")
            recommendations.append(f"💰 **Estimated Savings**: ${abs(total_savings):,.2f} - document and replicate successful optimization strategies")
            recommendations.append(f"📊 **Best Practice**: Analyze top-performing cost reductions to create optimization playbook for other entities")

        if significant_increases and metric_type == "revenue":
            recommendations.append("Capitalize on revenue growth: Scale successful revenue-generating activities")

        if significant_decreases and metric_type == "revenue":
            recommendations.append("Address revenue decline: Investigate causes and implement recovery strategies")

        # General recommendations
        if len(significant_increases) + len(significant_decreases) > len(percentage_changes) * 0.5:
            recommendations.append("High volatility detected: Consider implementing more consistent operational practices")
    ...
    def _calculate_model_comparison_changes(
        ...
        if value_b > 0:
            ...
            changes.append(PercentageChange(
    ...
    def _generate_model_comparison_insights(
        ...
        if change.trend_direction == "increasing":
            insights.append(f"{model_a} has {change.percentage_change:.1f}% higher {metric_type} than {model_b}")
        elif change.trend_direction == "decreasing":
            insights.append(f"{model_a} has {abs(change.percentage_change):.1f}% lower {metric_type} than {model_b}")
        else:
    ...
    def _generate_model_recommendations(
        ...
        if not percentage_changes:
            recommendations.append("📊 **Data Collection**: Gather at least 7 days of usage data for both models to enable meaningful cost comparisons")
            recommendations.append("🎯 **Testing Strategy**: Run parallel A/B tests with 20% traffic split to compare model performance and costs")
            return recommendations
        ...
        if change.significance == "significant":
            if change.trend_direction == "increasing":
                if percentage > 50:
                    recommendations.append(f"🚨 **Critical Cost Optimization**: {model_a} costs {percentage:.1f}% more than {model_b} (${cost_diff:,.2f} difference)")
                    recommendations.append(f"⚡ **Immediate Action**: Migrate 80% of non-critical workloads to {model_b} within 2 weeks to reduce costs")
                    recommendations.append(f"📋 **Implementation Plan**: Start with batch processing, then move real-time inference if quality metrics remain acceptable")
                else:
                    recommendations.append(f"💡 **Cost Optimization Opportunity**: {model_a} costs {percentage:.1f}% more than {model_b} - consider gradual migration")
                    recommendations.append(f"🧪 **Testing Recommendation**: Run 30-day pilot with {model_b} on 25% of traffic to validate quality and cost savings")
            else:
                recommendations.append(f"✅ **Cost Efficiency Confirmed**: {model_a} is {percentage:.1f}% more cost-effective than {model_b}")
                recommendations.append(f"📈 **Scaling Recommendation**: Increase {model_a} usage to 80% of workload to maximize cost savings")

        # Performance monitoring recommendations
        if cost_diff > 100:  # Significant absolute cost difference
            recommendations.append(f"📊 **Enhanced Monitoring**: Set up daily cost tracking alerts when {model_a} vs {model_b} cost difference exceeds ${cost_diff * 0.1:.2f}")

        recommendations.append(f"🔄 **Regular Review**: Schedule monthly model cost analysis to identify optimization opportunities")
    ...
    def _extract_provider_data(self, provider_data: Dict[str, Any], provider_name: str) -> Dict[str, Any]:
    ...
    def _generate_provider_comparison_insights(
        ...
        if not percentage_changes:
            insights.append(f"No {metric_type} data available for comparing {provider_a} vs {provider_b}")
            return insights

        change = percentage_changes[0]

        if change.trend_direction == "increasing":
            insights.append(f"{provider_a} has {change.percentage_change:.1f}% higher {metric_type} than {provider_b}")
        elif change.trend_direction == "decreasing":
            insights.append(f"{provider_a} has {abs(change.percentage_change):.1f}% lower {metric_type} than {provider_b}")
        else:
    ...
    def _generate_provider_recommendations(
        ...
        if not percentage_changes:
            recommendations.append("📊 **Multi-Provider Strategy**: Collect 14+ days of usage data across providers to enable strategic cost comparisons")
            recommendations.append("🔍 **Baseline Establishment**: Track cost per token, latency, and quality metrics for each provider")
            return recommendations
        ...
        if change.significance == "significant":
            if change.trend_direction == "increasing":
                if percentage > 30:
                    recommendations.append(f"🚨 **Strategic Cost Review**: {provider_a} costs {percentage:.1f}% more than {provider_b} (${cost_diff:,.2f}/day, ~${monthly_impact:,.2f}/month)")
                    recommendations.append(f"📋 **Migration Strategy**: Develop 90-day plan to shift 60-80% of workload to {provider_b} while maintaining quality standards")
                    recommendations.append(f"💼 **Contract Negotiation**: Use {provider_b} pricing as leverage to renegotiate {provider_a} rates - target 15-25% reduction")
                else:
                    recommendations.append(f"💡 **Cost Optimization**: {provider_a} costs {percentage:.1f}% more than {provider_b} - evaluate workload distribution")
                    recommendations.append(f"🎯 **Selective Migration**: Move batch processing and non-critical workloads to {provider_b} first")
            else:
                recommendations.append(f"✅ **Provider Efficiency**: {provider_a} is {percentage:.1f}% more cost-effective than {provider_b}")
                recommendations.append(f"📈 **Strategic Advantage**: Increase {provider_a} allocation to 70-80% of total workload to maximize savings")

        # Risk management and monitoring
        if monthly_impact > 1000:  # Significant monthly cost difference
            recommendations.append(f"⚠️ **Risk Management**: Maintain 20-30% workload on secondary provider to avoid vendor lock-in")
            recommendations.append(f"📊 **Cost Monitoring**: Set up weekly alerts when provider cost difference exceeds ${cost_diff * 0.2:.2f}/day")

        # Performance and quality considerations
        recommendations.append(f"🔄 **Quality Assurance**: Monitor output quality metrics during any provider shifts - maintain >95% quality threshold")
        recommendations.append(f"📈 **Performance Tracking**: Track latency, throughput, and error rates across providers for comprehensive optimization")
        ...
        try:
            ...
            if not isinstance(revenue_response, Exception):
                ...
                if isinstance(revenue_response_data, list):
                    processed_revenue_data = self._process_api_response(revenue_response_data, "customer")
            ...
            if "cost" in customer_metrics and "revenue" in customer_metrics:
                if customer_metrics["revenue"] > 0:
                    customer_metrics["profit_margin"] = ((customer_metrics["revenue"] - customer_metrics["cost"]) / customer_metrics["revenue"]) * 100
                else:
    ...
    async def _fetch_benchmark_metrics(
        ...
        try:
            ...
            if not isinstance(cost_response, Exception):
                ...
                if cost_data:
                    total_cost = sum(float(item.get("cost", 0)) for item in cost_data if isinstance(item, dict))
            ...
            if not isinstance(revenue_response, Exception):
                ...
                if revenue_data:
                    total_revenue = sum(float(item.get("revenue", 0)) for item in revenue_data if isinstance(item, dict))
            ...
            if "cost" in benchmark_metrics and "revenue" in benchmark_metrics:
                if benchmark_metrics["revenue"] > 0:
                    benchmark_metrics["profit_margin"] = ((benchmark_metrics["revenue"] - benchmark_metrics["cost"]) / benchmark_metrics["revenue"]) * 100
                else:
            ...
            if isinstance(response, dict):
                ...
                for j, group_data in enumerate(groups):
                    if not isinstance(group_data, dict):
                        logger.warning(f"Expected dict at groups[{j}], got {type(group_data).__name__}")
                        continue

                    group_name = self._normalize_entity_name(group_data.get("groupName", "Unknown"), breakdown_by)
                    ...
                    for k, metric in enumerate(metrics):
                        if not isinstance(metric, dict):
                            logger.warning(f"Expected dict at metrics[{k}], got {type(metric).__name__}")
                            continue
            ...
            # Handle legacy list format for backward compatibility
                ...
                for i, time_period in enumerate(response):
                    ...
                    if not isinstance(time_period, dict):
                        logger.warning(f"Expected dict at response[{i}], got {type(time_period).__name__}")
                        continue
