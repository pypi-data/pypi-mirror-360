...
class AIRoutingConfig:
    ...
    supported_tools: Set[str] = field(default_factory=lambda: {
        'products', 'alerts', 'subscriptions', 'customers', 'workflows'
    })
        ...
        if 'tool_overrides' in updates:
            for tool in updates['tool_overrides']:
                if tool not in self.supported_tools:
                    errors.append(f"Unsupported tool: {tool}, supported: {list(self.supported_tools)}")
    ...
    def get_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'version': self.version,
            'active_tools': [
                tool for tool, enabled in self.tool_overrides.items()
