    ...
    def format_log_response(
        ...
        try:
            ...
            response_text = self._build_response_header(
                log_type, total_elements, current_page, total_pages, 
                len(log_entries), applied_filters
            )
...
"""
    ...
    def _generate_diagnostic_insights(
        ...
        if ai_processing > 0:
            ai_info_count = sum(1 for entry in log_entries
                              if entry.get("operation") == "AI_METRIC_PROCESSING"
                              and entry.get("status") == "INFO")
            if ai_info_count > 0:
                insights.append(f"{ai_info_count} AI_METRIC_PROCESSING entries with INFO status (new products/organizations created)")

        # Check for large datasets - USE ACTUAL TOTAL FROM API
        if total_elements > 100:
            if total_pages > 1:
                insights.append(f"Large dataset available: {total_elements:,} total entries across {total_pages} pages")
            else:
                insights.append(f"Large dataset available: {total_elements:,} total entries")

        # Check for email dispatch issues
        email_failures = sum(1 for entry in log_entries
                           if "EMAIL_DISPATCH" in entry.get("operation", "")
    ...
    def format_multi_page_response(
        ...
        try:
            ...
            insights = self._generate_diagnostic_insights(
                all_entries, operation_counts, status_counts, total_entries_retrieved, pages_retrieved
            )
