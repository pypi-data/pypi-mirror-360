...
class ManageCapabilities(ToolBase):
    ...
    tool_description: ClassVar[str] = "Manage unified capabilities across all MCP tools. Key actions: get_capabilities, verify_capability, refresh_capabilities, get_health_status. Use get_examples() for usage guidance and get_capabilities() for system status."
    ...
    async def handle_action(
        ...
        try:
            ...
            elif request.action == "set_capability":
                result = await self._handle_set_capability(request)
...
"""
    ...
    async def _handle_get_capabilities(self, request: CapabilityRequest) -> str:
        ...
        if not request.resource_type:
            ...
            return f"""# **UCM Capabilities Overview**
...
"""
    ...
    async def _handle_manage_capabilities(self, request: CapabilityRequest) -> str:
        ...
        return (
            ...
            f"**Available Actions**: get_capabilities, set_capability, get_capability, verify_capability"
        )
    ...
    async def _handle_set_capability(self, request: CapabilityRequest) -> str:
        """Handle set_capability action.
        
        Args:
            request: Capability request with parameters
            
        Returns:
            Formatted set capability response
        """
        if not request.capability_name or not request.value:
            missing_params = []
            if not request.capability_name:
                missing_params.append("capability_name")
            if not request.value:
                missing_params.append("value")
            return create_missing_verify_params_error(missing_params)
            
        # Set the capability using UCM service
        success = await ucm_integration_service.set_capability(
            request.resource_type or "system", 
            request.capability_name, 
            request.value
        )
        
        status = "✅ Successfully set" if success else "❌ Failed to set"
        return (
            f"# **Capability Set Operation**\n\n"
    ...
    async def _get_input_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "value": {
                    "type": "string",
                    "description": "Value to verify against capability"
                }
            },
