...
class SetupChecklist(ToolBase):
    ...
    tool_description: ClassVar[str] = "Review setup status, check all required environment variables"
    ...
    async def _handle_show_checklist(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_check_requirements(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_check_optional(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_check_system_status(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_get_recommendations(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
...
"""
    ...
    def _build_complete_checklist(
        ...
        checklist += f"{'✅' if api_key_set else '❌'} **API Key (REVENIUM_API_KEY)**\n"
        if api_key_set:
        ...
        checklist += f"{'✅' if team_id_set else '❌'} **Team ID (REVENIUM_TEAM_ID)**\n"
        if team_id_set:
        ...
        checklist += f"{'✅' if email_set else '📧'} **Email Notifications**\n"
        if email_set:
        ...
        checklist += f"{'✅' if slack_configured else '📱'} **Slack Integration**\n"
        if slack_configured:
        ...
        checklist += f"{'✅' if api_works else '❌'} **API Connectivity**\n"
        if api_works:
        ...
        checklist += f"{'✅' if auto_discovery_works else '⚠️'} **Auto-Discovery**\n"
    ...
    def _build_requirements_checklist(self, validation_result) -> str:
        ...
        for var_name, description, is_critical in required_vars:
            ...
            display_value = "SET (hidden)" if var_name == "REVENIUM_API_KEY" and is_set else (get_config_value(var_name) or "NOT SET")
            ...
            if not is_set:
                requirements += f"**Action Required**: Set this environment variable\n"
                ...
                requirements += "**Status**: ✅ Properly configured\n"

            requirements += "\n"

        # Overall status
        if all_required_set:
            requirements += "## ✅ **Requirements Status: COMPLETE**\n\n"
    ...
    def _build_optional_checklist(self, validation_result, slack_status) -> str:
        ...
        optional += f"## {'✅' if email_set else '❌'} **Email Notifications**\n"
        optional += "**Purpose**: Receive alert notifications via email\n"
        optional += f"**Status**: {email_value}\n"
        if email_set:
            optional += "**Benefit**: ✅ You'll receive email notifications for alerts\n"
        else:
        ...
        optional += f"## {'✅' if slack_configured else '❌'} **Slack Integration**\n"
        optional += "**Purpose**: Real-time notifications and team collaboration\n"
        optional += f"**Status**: {slack_config_id}\n"
        if slack_configured:
            optional += "**Benefit**: ✅ Real-time Slack notifications enabled\n"
        else:
        ...
        optional += f"## {'✅' if auto_discovery_works else '❌'} **Auto-Discovery**\n"
        ...
        if auto_discovery_works:
            optional += f"**Benefit**: ✅ {discovered_count} values auto-discovered\n"
        else:
        ...
        optional += f"## 📊 **Optional Configuration Summary**\n\n"
    ...
    def _build_system_status(self, validation_result) -> str:
        ...
        if api_status == 'success':
            system += "✅ **Status**: Connected and working\n"
        ...
        if discovered_status == 'success':
            ...
            system += f"✅ **Status**: Working - {discovered_count} values discovered\n"
        ...
        overall_healthy = (
            api_status == 'success' and
            auth_status == 'success' and
            discovered_status == 'success'
        )
    ...
    def _build_recommendations(self, onboarding_state) -> str:
        ...
        if not high_priority and not medium_priority and not low_priority:
            ...
            recommendations += "- Use welcome_and_setup(action='complete_setup') to finish onboarding\n"
