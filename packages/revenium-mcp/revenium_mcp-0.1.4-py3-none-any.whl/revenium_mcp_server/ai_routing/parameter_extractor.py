...
EXTRACTION_ORDER = [
    'name', 'email', 'product_type', 'workflow_type', 'time_period', 
    'status', 'priority', 'amount', 'date', 'id'  # ID last to avoid conflicts
]
...
class ParameterExtractor:
    ...
    def _find_missing_parameters(
        ...
        expected_parameters: Optional[List[str]]
    ) -> List[str]:
        ...
        return [
            param for param in expected_parameters 
    ...
    def _calculate_extraction_quality(
        ...
        required_coverage = sum(
            ...
            if param in extracted_params.parameters and extracted_params.parameters[param] is not None
        ) / len(required_params)
    ...
    def _generate_parameter_recommendations(
        ...
        missing_required = [
            ...
            if param not in extracted_params.parameters or extracted_params.parameters[param] is None
        ]

        if missing_required:
            recommendations.append(f"Add missing required parameters: {', '.join(missing_required)}")

        # Operation-specific recommendations
        recommendations.extend(self._get_operation_specific_recommendations(
            operation_context, extracted_params.parameters
        ))

        # General quality recommendations
        if extracted_params.confidence < 0.7:
            recommendations.append("Consider using more specific language for better parameter extraction")
    ...
    def _get_operation_specific_recommendations(
        ...
        if operation_context == "products.create":
            ...
            if 'product_type' not in parameters:
                recommendations.append("Consider specifying product type: 'with type api/usage/subscription/metering'")
            ...
            if 'workflow_type' not in parameters:
                recommendations.append("Try: 'start workflow [subscription_setup/customer_onboarding/product_creation]'")
