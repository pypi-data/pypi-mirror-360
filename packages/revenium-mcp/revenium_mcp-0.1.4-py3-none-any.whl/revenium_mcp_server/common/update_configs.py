...
from .partial_update_handler import UpdateConfig, FieldTransformers
...
class UpdateConfigs:
    ...
    def create_sources_config(client: ReveniumClient) -> UpdateConfig:
        ...
        return UpdateConfig(
            ...
            field_mappings={
                "externalUsagePlanId": "externalPlanId"
            }
        )

    @staticmethod
    def create_customers_config(client: ReveniumClient, resource_type: str = "user") -> UpdateConfig:
        ...
        if resource_type == "user":
            ...
            preserve_fields.extend(["organization_id", "team_id", "teamIds", "permissions", "role", "tenant"])
            field_transformations = {
                "teams": {"teamIds": FieldTransformers.extract_team_ids}
            }
            ...
            # Note: "roles" removed from preserve_fields to allow role updates via partial updates
            preserve_fields.extend(["subscription_ids", "organization_id", "user_id", "billing_address", "payment_method", "subscriberId", "tenant"])
            field_transformations = {
            ...
            preserve_fields.extend(["organization_id", "parent_team_id", "owner_id", "members", "permissions"])
    ...
    def get_config(self, resource_type: str, **kwargs) -> UpdateConfig:
        ...
        if cache_key not in self._configs:
            if resource_type == "subscriber_credentials":
                self._configs[cache_key] = UpdateConfigs.create_subscriber_credentials_config(self.client)
                ...
                self._configs[cache_key] = UpdateConfigs.create_customers_config(self.client, customer_type)
            elif resource_type == "metering_elements":
                self._configs[cache_key] = UpdateConfigs.create_metering_elements_config(self.client)
            else:
