...
class SubscriberMigrationValidator:
    ...
    def convert_old_to_new_format(self, old_data: Dict[str, Any], remove_old_fields: bool = True) -> Dict[str, Any]:
    ...
    def validate_migration(self, old_data: Dict[str, Any], new_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_deprecation_warning(self, current_date: Optional[str] = None) -> Dict[str, Any]:
        ...
        if phase == "warning":
            warning_data.update({
                ...
                "time_remaining": self._calculate_time_remaining(current_date, "deprecation_start")
            })
...
class MigrationTracker:
    ...
    def get_migration_analytics(self) -> Dict[str, Any]:
        ...
        """
        total_usage = self.migration_stats["old_format_usage"] + self.migration_stats["new_format_usage"]
        ...
        if self.migration_stats["migration_attempts"] > 0:
            success_rate = ((self.migration_stats["migration_attempts"] - self.migration_stats["validation_failures"]) / 
                          self.migration_stats["migration_attempts"]) * 100
    ...
    def _generate_recommendations(self, old_percentage: float, success_rate: float) -> List[str]:
        ...
        if old_percentage > 0:
            recommendations.append("Consider implementing automatic conversion for remaining old format usage")
        
        if success_rate < 90:
            recommendations.append("Migration validation failures detected - review migration logic")
