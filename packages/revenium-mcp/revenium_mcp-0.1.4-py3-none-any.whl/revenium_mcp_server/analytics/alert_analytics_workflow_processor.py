...
class AlertAnalyticsWorkflowProcessor:
    ...
    def _is_transaction_related_alert(self, alert_context: AlertContext) -> bool:
        ...
        transaction_indicators = [
            "transaction", "request", "call", "api", "agent", "task",
            "response_time", "latency", "duration", "throughput", "performance"
        ]
    ...
    async def _generate_recommendations(
        ...
        for factor in contributing_factors:
            if factor["type"] == "cost_spike":
                ...
                recommendations.append("🛠️ Consider task optimization or resource scaling")
    ...
    def _calculate_confidence_score(
        ...
        if contributing_factors:
            factor_confidence = sum(factor.get("confidence", 0.5) for factor in contributing_factors)
    ...
    def format_root_cause_analysis(self, analysis: RootCauseAnalysis) -> str:
        ...
        lines.append(f"**Triggered**: {analysis.alert_context.trigger_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        lines.append("")

        # Contributing Factors
        lines.append("## 🎯 **Contributing Factors**")
        for i, factor in enumerate(analysis.contributing_factors, 1):
            confidence_emoji = "🟢" if factor["confidence"] > 0.8 else "🟡" if factor["confidence"] > 0.6 else "🔴"
        ...
        lines.append(f"**Analysis Completed**: {analysis.analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}")
