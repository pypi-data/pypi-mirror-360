...
class CostSpikeFormatter(AnalyticsResponseFormatter):
    ...
    def _format_cost_spike_content(self, data: Dict[str, Any], threshold: float, period: str) -> str:
...
"""
        ...
        response += self.utilities.add_insights_footer(
            "cost spike investigation", period, f"threshold: {self.utilities.format_currency(threshold)}"
        )
        
        return response
    
    def _format_spike_detected_section(self, contributors: List[Dict[str, Any]], threshold: float) -> str:
