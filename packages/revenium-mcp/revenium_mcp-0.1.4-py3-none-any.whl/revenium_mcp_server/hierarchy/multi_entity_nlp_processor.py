...
from .cross_tier_validator import CrossTierValidator, ValidationResult
...
class MultiEntityNLPProcessor:
    ...
    def __init__(self, 
        ...
        self.validator = validator or CrossTierValidator(self.client, self.navigation_service, self.lookup_service)
        ...
        self.excluded_words = {
            'for', 'with', 'and', 'the', 'a', 'an', 'to', 'from', 'in', 'on', 'at',
            'by', 'of', 'as', 'is', 'are', 'was', 'were', 'be', 'been', 'being',
            'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could',
            'should', 'may', 'might', 'can', 'must', 'shall', 'this', 'that',
            'these', 'those', 'all', 'any', 'some', 'each', 'every', 'no', 'none'
        }
        
        # Action patterns
        self.action_patterns = {
            ActionType.CREATE: [
                r"create", r"add", r"make", r"set up", r"establish", r"build"
            ],
            ActionType.FIND: [
                r"find", r"get", r"show", r"list", r"retrieve", r"search"
            ],
            ActionType.ASSOCIATE: [
                r"associate", r"link", r"connect", r"attach", r"bind", r"join"
            ],
            ActionType.NAVIGATE: [
                r"show hierarchy", r"navigate", r"traverse", r"explore"
            ],
            ActionType.UPDATE: [
                r"update", r"modify", r"change", r"edit"
            ],
            ActionType.DELETE: [
    ...
    async def process_query(self, query: str, context: Optional[Dict[str, Any]] = None) -> NLPResult:
        ...
        try:
            ...
            return NLPResult(
                ...
                suggestions=["Please try rephrasing your query", "Check for typos or unclear references"]
            )
        ...
        for entity_type, patterns in self.entity_patterns.items():
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    if (identifier.lower() in self.excluded_words or
                        len(identifier) < 2 or
                        identifier.lower() in ['id', 'name', 'email']):
                        continue
        ...
        for action_type, patterns in self.action_patterns.items():
            for pattern in patterns:
                if re.search(pattern, query, re.IGNORECASE):
                    ...
                    target_entity_type = self._infer_target_entity_type(query, action_type, entities)
    ...
    def _infer_target_entity_type(self, query: str, action_type: ActionType, entities: List[EntityMention]) -> EntityType:
    ...
    def _associate_entities_with_action(self, action: ParsedAction, entities: List[EntityMention], query: str) -> None:
    ...
    def _calculate_confidence(self, entities: List[EntityMention], actions: List[ParsedAction], query_type: QueryType) -> float:
            ...
            for entity in parsed_query.entities:
                try:
                    ...
                    if entity.entity_type == EntityType.PRODUCT:
                        resolved = await self.lookup_service.resolve_product(entity.identifier, strategy)
                    elif entity.entity_type == EntityType.SUBSCRIPTION:
                        resolved = await self.lookup_service.resolve_subscription(entity.identifier, strategy)
                    elif entity.entity_type == EntityType.CREDENTIAL:
                        resolved = await self.lookup_service.resolve_credential(entity.identifier, strategy)
                    elif entity.entity_type == EntityType.SUBSCRIBER:
                        resolved = await self.lookup_service.resolve_subscriber(entity.identifier, strategy)
                    elif entity.entity_type == EntityType.ORGANIZATION:
                        resolved = await self.lookup_service.resolve_organization(entity.identifier, strategy)
                    else:
                        resolved = None

                    if resolved:
                        entity.resolved_entity = resolved
                        entity.confidence = min(entity.confidence + 0.2, 1.0)  # Boost confidence for resolved entities
                    else:
