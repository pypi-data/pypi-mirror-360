...
from .error_handling import (
    MCPError, JSONRPCErrorCode, create_internal_error
)
from .protocol_handler import protocol_handler
from .capability_operations_helpers import (
    get_default_capabilities, register_capability_in_dict, update_capability_in_dict,
    enable_capability_in_dict, disable_capability_in_dict, get_capabilities_mcp_format,
    update_server_info_fields, add_change_listener_to_list, remove_change_listener_from_list,
    notify_capability_changed, send_notification, build_capability_stats
)
...
class ServerInfo:
    ...
    vendor: str = "Revenium"
    homepage: str = "https://github.com/revenium/revenium-platformapi-mcp"
...
class MCPCapabilityManager:
    ...
    async def _notify_capability_changed(self, capability_name: str) -> None:
        ...
        if hasattr(protocol_handler, 'client_supports_capability'):
            if protocol_handler.client_supports_capability("tools", "listChanged") and capability_name == "tools":
                await self._send_tools_list_changed_notification()
            elif protocol_handler.client_supports_capability("resources", "listChanged") and capability_name == "resources":
                await self._send_resources_list_changed_notification()
            elif protocol_handler.client_supports_capability("prompts", "listChanged") and capability_name == "prompts":
    ...
    async def _send_resources_list_changed_notification(self) -> None:
        ...
        await send_notification("resources", protocol_handler.send_resources_list_changed_notification)
    ...
    def get_capability_stats(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            "last_updated": max(
                (cap.last_updated for cap in self.capabilities.values()),
