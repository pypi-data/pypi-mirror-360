...
class SlackSetupAssistant(ToolBase):
    ...
    tool_description: ClassVar[str] = "Intelligent Slack setup and configuration assistant. Key actions: guided_setup, quick_setup, setup_status, detect_and_recommend. Use get_examples() for setup guidance and get_capabilities() for complete action list."
    ...
    async def handle_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_guided_setup(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Complete guided setup flow with intelligent detection."""
        try:
            skip_prompts = arguments.get("skip_prompts", False)
            ...
            if total_elements > 0:
                ...
                if current_default:
                    ...
                    default_config = next((c for c in configurations if c.get("id") == current_default), None)
                    if default_config:
                        ...
                        result_text += "Your Slack integration is already set up and ready to use!\n\n"
                ...
                result_text += "**Quick Start:** Use the quick setup action to streamline this process:\n"
            ...
            error = ValidationError(
                message="Failed to perform guided setup",
                details={"error": str(e)}
            )
            return [TextContent(type="text", text=error.format_user_message())]

    async def _handle_detect_and_recommend(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            result_text = f"# Slack Configuration Detection & Recommendations\n\n"
            ...
            if current_default:
                ...
                if default_config:
                    ...
                    result_text += f"## Current Default Configuration\n"
                    result_text += f"**{name}**\n"
                    result_text += f"- Workspace: {workspace}\n"
                    result_text += f"- Channel: #{channel}\n"
                    result_text += f"- ID: `{current_default}`\n\n"
                else:
                    result_text += f"## Default Configuration Issue\n"
                    result_text += f"Default ID `{current_default}` not found in available configurations.\n\n"
            ...
            for i, config in enumerate(configurations, 1):
                ...
                if is_default:
                    result_text += f"- **Status:** Current Default\n"
                result_text += f"\n"
            ...
            if not current_default:
                ...
                result_text += f"Choose a configuration to use automatically for new alerts:\n"
                for i, config in enumerate(configurations, 1):
                    config_id = config.get("id", "Unknown")
                    name = config.get("name", "Unnamed Configuration")
            ...
            result_text += f"**Management Actions**\n"
            result_text += f"- View detailed configuration: `slack_configuration_management(action='get_configuration', config_id='CONFIG_ID')`\n"
            result_text += f"- Add new configuration: `slack_oauth_workflow(action='initiate_oauth')`\n"
            result_text += f"- Check setup status: `slack_setup_assistant(action='setup_status')`\n"
            
            return [TextContent(type="text", text=result_text)]
            
        except Exception as e:
            error = ValidationError(
                message="Failed to detect and recommend configurations",
                details={"error": str(e)}
            )
            return [TextContent(type="text", text=error.format_user_message())]

    async def _handle_select_default_configuration(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Set default configuration with confirmation."""
        config_id = arguments.get("config_id")
        if not config_id:
            error = ValidationError(
                message="Configuration ID is required",
                details={"action": "select_default_configuration", "missing_parameter": "config_id"}
            )
            return [TextContent(type="text", text=error.format_user_message())]
        ...
        try:
            ...
            result_text = f"# Default Slack Configuration Set Successfully\n\n"
            result_text += f"## New Default Configuration\n"
            result_text += f"**{name}**\n"
            result_text += f"- **Workspace:** {workspace}\n"
            result_text += f"- **Channel:** #{channel}\n"
            result_text += f"- **ID:** `{config_id}`\n\n"

            result_text += f"## Setup Complete!\n\n"
            result_text += f"Your Slack integration is now ready to use. This configuration will be automatically used for:\n"
            result_text += f"- New alert notifications\n"
            result_text += f"- Any alert creation without a specific Slack configuration\n\n"

            result_text += f"## Next Steps\n\n"
            result_text += f"**Create your first alert with Slack notifications:**\n"
            result_text += f"```\nmanage_alerts(\n"
            result_text += f"    action='create_cumulative_usage',\n"
            result_text += f"    name='Monthly Cost Alert',\n"
            result_text += f"    threshold=1000,\n"
            result_text += f"    period='monthly'\n"
            result_text += f"    # Slack notifications will be added automatically!\n"
            result_text += f")\n```\n\n"
            
            result_text += f"**Make this setting persistent:**\n"
            result_text += f"Add this to your environment variables:\n"
            result_text += f"```bash\nexport REVENIUM_DEFAULT_SLACK_CONFIG_ID=\"{config_id}\"\n```\n\n"
            
            result_text += f"**Other useful commands:**\n"
            result_text += f"- Check setup status: `slack_setup_assistant(action='setup_status')`\n"
            result_text += f"- View all configurations: `slack_configuration_management(action='list_configurations')`\n"
    ...
    async def _handle_setup_status(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            result_text = f"# Slack Integration Setup Status\n\n"

            # Overall status
            if total_elements > 0 and current_default:
                result_text += f"## Setup Status: **COMPLETE**\n\n"
                result_text += f"Your Slack integration is fully configured and ready to use!\n\n"
            elif total_elements > 0:
                result_text += f"## Setup Status: **PARTIAL**\n\n"
                result_text += f"You have Slack configurations but no default is set.\n\n"
            else:
                result_text += f"## Setup Status: **NOT CONFIGURED**\n\n"
                result_text += f"No Slack configurations found. Setup required.\n\n"

            # Configuration details
            result_text += f"## Configuration Summary\n\n"
            result_text += f"- **Total Configurations:** {total_elements}\n"
            result_text += f"- **Default Configuration:** {'Set' if current_default else 'Not Set'}\n"
            ...
            if current_default:
                try:
                    ...
                    result_text += f"- **Default Config ID:** `{current_default}` (Configuration not found)\n"
                result_text += f"\n"
            ...
            if total_elements == 0:
                result_text += f"**Get Started:**\n"
                result_text += f"```\nslack_setup_assistant(action='quick_setup')\n```\n\n"
            elif not current_default:
                result_text += f"**Set Default Configuration:**\n"
                result_text += f"```\nslack_setup_assistant(action='detect_and_recommend')\n```\n\n"
            else:
                result_text += f"**You're all set!** Try creating an alert:\n"
                result_text += f"```\nmanage_alerts(action='create_simple_alert', threshold=100)\n```\n\n"

            result_text += f"**Other Actions:**\n"
            result_text += f"- Complete guided setup: `slack_setup_assistant(action='guided_setup')`\n"
            result_text += f"- Add new configuration: `slack_oauth_workflow(action='initiate_oauth')`\n"
            result_text += f"- View all configurations: `slack_configuration_management(action='list_configurations')`\n"
    ...
    async def _handle_quick_setup(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Streamlined setup process for new users."""
        try:
            result_text = f"# Quick Slack Setup\n\n"
            result_text += f"Let's get your Slack notifications set up in 4 easy steps!\n\n"

            result_text += f"## Step 1: Ensure User is logged into Revenium application\n\n"
            result_text += f"- Remind user to ensure they are logged in before clicking oauth workflow link as otherwise it will not work\n\n"

            result_text += f"## Step 2: Start OAuth Process\n\n"
            result_text += f"Start OAuth Process\n"
            result_text += f"Click this command to begin:\n"
            result_text += f"```\nslack_oauth_workflow(action='initiate_oauth')\n```\n\n"

            result_text += f"## Step 3: User Completes in Browser\n"
            result_text += f"- Follow the link that appears\n"
            result_text += f"- Authorize Slack integration\n"
            result_text += f"- Choose your workspace and channel\n"
            result_text += f"- Wait for success confirmation\n\n"

            result_text += f"## Step 4: Finish Setup\n"
            result_text += f"Return here and run:\n"
            result_text += f"```\nslack_setup_assistant(action='guided_setup')\n```\n\n"
            
            result_text += f"## Why Slack?\n\n"
            result_text += f"- **Instant notifications** delivered to your team\n"
            result_text += f"- **Better than email** for urgent alerts\n"
            result_text += f"- **Mobile-friendly** for on-the-go monitoring\n"
            result_text += f"- **Team collaboration** on alert responses\n\n"
            
            result_text += f"**Need help?** Use `slack_setup_assistant(action='setup_status')` to check your progress.\n"
            ...
            error = ValidationError(
                message="Failed to perform quick setup",
                details={"error": str(e)}
            )
            return [TextContent(type="text", text=error.format_user_message())]

    async def _handle_onboarding_setup(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            if total_elements == 0:
                ...
                if is_first_time:
                    ...
                    result_text += "- Create your first alert with automatic Slack notifications\n\n"

            elif not current_default:
                result_text += "## ✅ **Configurations Found - Set Default**\n\n"
                result_text += f"Great! You have {total_elements} Slack configuration(s) available.\n\n"
                ...
                if is_first_time:
                    ...
                    result_text += "- Use `welcome_and_setup(action='complete_setup')` when ready\n\n"

            else:
                result_text += "## 🎉 **Slack Setup Complete!**\n\n"
                result_text += f"✅ **Default Configuration**: Set and ready\n"
                result_text += f"✅ **Notifications**: Enabled for new alerts\n"
                result_text += f"✅ **Integration**: Fully functional\n\n"
                ...
                if is_first_time:
                    ...
                    result_text += "- Try `manage_alerts(action='create_simple_alert')` to test Slack\n"
                    result_text += "- Use `welcome_and_setup(action='complete_setup')` when ready\n\n"
                else:
    ...
    async def _handle_first_time_guidance(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            if total_elements == 0:
                ...
                result_text += "- Choose a channel for notifications (e.g., #alerts, #monitoring)\n\n"
                ...
                result_text += "**Test Your Setup**: Create a simple alert to see Slack in action:\n"
                result_text += "```\nmanage_alerts(action='create_simple_alert', threshold=100)\n```\n\n"
            ...
            result_text += "- Use `slack_setup_assistant(action='setup_status')` to check current state\n"
            ...
            result_text += "- Use `slack_setup_assistant(action='setup_status')` for current status\n"
    ...
    async def _get_tool_capabilities(self) -> List[ToolCapability]:
        ...
        return [
            ...
            ToolCapability(
                ...
                parameters={"action": "select_default_configuration", "config_id": "slack_config_id"}
            )
        ]
...
"""
    ...
    async def _handle_get_capabilities(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
