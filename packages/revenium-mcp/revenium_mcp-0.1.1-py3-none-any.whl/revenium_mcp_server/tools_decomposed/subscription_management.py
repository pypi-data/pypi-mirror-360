...
from ..common.error_handling import (
    ErrorCodes, ToolError, ResourceError,
...
from ..introspection.metadata import (
    ...
    ResourceRelationship, DependencyType
)
...
class SubscriptionManager:
        ...
        try:
            ...
            for product in products:
                formatted_product = {
                    ...
                    "status": "Available for subscriptions" if product.get('published') else "Not available (unpublished)",
    ...
    async def create_subscription(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not subscription_data and name:
            ...
            try:
                ...
                logger.info(f"Context7 auto-generation: Created subscription_data for '{name}' with product {product_id}")
            ...
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "example_data": {"product_id": "prod_123", "clientEmailAddress": "customer@company.com", "billing_cycle": "monthly"},
    ...
    def _classify_subscription_intent(self, query: str) -> str:
        ...
        if any(pattern in query_lower for pattern in [
            "list", "show", "display", "view all", "get all", "see all"
        ]):
        ...
        # Search patterns
        if any(pattern in query_lower for pattern in [
            "search", "find", "look for", "filter", "where"
        ]):
            return "search_subscriptions"

        # Get specific subscription patterns
        if any(pattern in query_lower for pattern in [
            "get subscription", "subscription details", "show subscription", "subscription info"
        ]):
            return "get_subscription"

        # Supporting data patterns
        if any(pattern in query_lower for pattern in [
            "products", "customers", "organizations", "subscribers", "credentials"
        ]):
            return "get_supporting_data"

        # Create patterns
        if any(pattern in query_lower for pattern in [
            "create", "add", "new subscription", "subscribe"
        ]):
    ...
    def _extract_subscription_entities(self, query: str) -> Dict[str, Any]:
        ...
        customer_patterns = [
            ...
            r"for\s+([a-zA-Z0-9\-_]+)",
            r"globaltech", r"nexus-research", r"innovate", r"streamflow"
        ]
        ...
        product_patterns = [
            ...
            r"automated billing", r"default api product"
        ]
        ...
        for remove_word in ["list", "show", "display", "search", "find", "get", "subscription", "subscriptions"]:
    ...
    async def create_simple(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product_id, client_email, name, billing_frequency_hint = self._extract_simple_parameters(arguments)
    ...
    def _extract_simple_parameters(self, arguments: Dict[str, Any]) -> tuple:
        ...
        client_email = arguments.get("clientEmailAddress") or subscription_data.get("clientEmailAddress")
        # Extract billing frequency hint for description purposes only
        billing_frequency_hint = (arguments.get("billing_frequency") or
                                 arguments.get("type") or  # Legacy support
                                 subscription_data.get("billing_frequency") or
                                 "monthly")  # Default for description generation only
        ...
        if not client_email:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "example_emails": ["user@company.com", "customer@domain.com", "billing@organization.com"],
    ...
    def _build_simple_subscription_config(self, product_id: str, client_email: str, name: str, billing_frequency_hint: str) -> Dict[str, Any]:
    ...
    async def create_from_text(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product_id, client_email = self._extract_and_validate_parameters(
            arguments, extracted_info
        )
    ...
    def _extract_and_validate_parameters(self, arguments: Dict[str, Any], extracted_info: Dict[str, Any]) -> tuple:
    ...
    def _build_text_subscription_config(self, product_id: str, client_email: str, text: str, extracted_info: Dict[str, Any]) -> Dict[str, Any]:
        """Build subscription configuration from text analysis."""
        subscription_name = extracted_info.get("subscription_name") or f"Subscription: {text[:50]}..."
    ...
    async def _create_subscription_with_safety_log(self, subscription_config: Dict[str, Any], product_id: str, client_email: str) -> Dict[str, Any]:
        """Create subscription with safety logging."""
        logger.info(f"SAFE SUBSCRIPTION CREATION: product_id={product_id}, client_email={client_email}")
    ...
    async def _analyze_subscription_text(self, text: str) -> Dict[str, Any]:
        ...
        if analysis["features_mentioned"]:
            ...
            analysis["subscription_name"] = f"{feature_str} {analysis['billing_frequency_hint'].title()} Subscription"
        else:
            analysis["subscription_name"] = f"{analysis['billing_frequency_hint'].title()} Subscription"
...
class SubscriptionValidator:
    ...
    def get_examples(self, example_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.schema_discovery:
            try:
                ...
                if schema_examples and schema_examples.get("examples") and len(schema_examples["examples"]) > 0:
    ...
    async def validate_configuration(self, subscription_data: Dict[str, Any], dry_run: bool = True) -> Dict[str, Any]:
        ...
        if not self.schema_discovery:
            ...
            raise ToolError(
                ...
                suggestions=[
                    ...
                    "Use UCM integration for real-time validation capabilities"
                ],
                examples={
                    "initialization": "SubscriptionManagement should be initialized with schema discovery",
                    "ucm_integration": "UCM provides real-time validation capabilities",
...
class SubscriptionAnalytics:
    ...
    async def get_metrics(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        monthly_subscriptions = len([s for s in subscriptions if s.get('billing_period') == 'monthly'])
        yearly_subscriptions = len([s for s in subscriptions if s.get('billing_period') == 'yearly'])
        ...
        metrics = {
            ...
            "trial_rate": (trial_subscriptions / total_subscriptions * 100) if total_subscriptions > 0 else 0,
            ...
            "products_represented": len(set(s.get('product_id') for s in subscriptions if s.get('product_id')))
        }
...
class SubscriptionHierarchyManager:
    ...
    async def get_product_details(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        navigation_result = await self.navigation_service.get_product_for_subscription(subscription_id)
        ...
        return {
            ...
            "data": navigation_result.related_entities[0] if navigation_result.related_entities else {},
    ...
    async def get_credentials(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        navigation_result = await self.navigation_service.get_credentials_for_subscription(subscription_id)
    ...
    async def create_with_credentials(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self._build_credentials_response(
            start_time, subscription_result, credentials_result
        )
    ...
    async def _validate_credentials_hierarchy(self, subscription_data: Dict[str, Any], credentials_data: Dict[str, Any]):
    ...
    def _prepare_credentials_subscription_data(self, subscription_data: Dict[str, Any], arguments: Dict[str, Any]):
    ...
    async def _create_subscription_and_credentials(self, subscription_data: Dict[str, Any], credentials_data: Dict[str, Any]) -> tuple:
        """Create subscription and credentials in coordinated sequence."""
        # Create subscription first
        logger.info(f"Creating subscription for customer: {subscription_data.get('clientEmailAddress')}")
    ...
    def _build_credentials_response(self, start_time: float, subscription_result: Dict[str, Any], credentials_result: Dict[str, Any]) -> Dict[str, Any]:
...
class SubscriptionManagement(ToolBase):
    ...
    async def handle_action(
        ...
        try:
            ...
            elif action in ["get_metrics", "get_supporting_data", "search_subscriptions", "subscription_nlp"]:
                return await self._handle_analytics_actions(action, arguments, subscription_manager, analytics_processor)
    ...
    async def _handle_crud_actions(self, action: str, arguments: Dict[str, Any], subscription_manager) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
            ...
            if auto_generate:
                ...
                if not name:
                    return [TextContent(
                        ...
                        text=f"**Missing Required Field**\n\n"
                             f"**Field**: `name` (required for all subscription creation)\n\n"
                             f"**Example**: `{{\"action\":\"create\",\"subscription_data\":{{\"name\":\"Monthly Subscription\"}}}}`\n\n"
                             f"**Auto-Generation**: Enabled (will auto-generate product_id from available products, clientEmailAddress from defaults)"
                    )]
                
                # Apply smart auto-generation logic
                enhanced_subscription_data = {
                    "name": name,
                    "description": subscription_data.get("description") or f"Subscription for {name}",
                }
                
                # Auto-generate product_id if not provided
                if not subscription_data.get("product_id") and not subscription_data.get("productId"):
                    ...
                    enhanced_subscription_data["product_id"] = subscription_data.get("product_id") or subscription_data.get("productId")
                ...
                if not subscription_data.get("clientEmailAddress"):
                    ...
                    if default_email:
                        ...
                        logger.info(f"Auto-generated clientEmailAddress from REVENIUM_DEFAULT_EMAIL: {default_email}")
                    else:
                        enhanced_subscription_data["clientEmailAddress"] = f"user-{int(time.time())}@example.com"
                        logger.info(f"Auto-generated test clientEmailAddress: {enhanced_subscription_data['clientEmailAddress']}")
                else:
                    enhanced_subscription_data["clientEmailAddress"] = subscription_data["clientEmailAddress"]
    ...
    async def _handle_creation_actions(self, action: str, arguments: Dict[str, Any], subscription_manager) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Handle subscription creation actions."""
        if action == "create_simple":
            result = await subscription_manager.create_simple(arguments)
            return [TextContent(
                type="text",
                text=f"Simple subscription created successfully:\n\n" +
                     json.dumps(result, indent=2)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"🔒 **Safe Subscription Created from Text**\n\n" +
                     json.dumps(result, indent=2)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"🔍 **Product Discovery Results**\n\n" +
                     json.dumps(result, indent=2)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"✅ **Product Validation Results**\n\n" +
                     json.dumps(result, indent=2)
            )]

    async def _handle_analytics_actions(self, action: str, arguments: Dict[str, Any], subscription_manager, analytics_processor) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Handle analytics and search actions."""
        if action == "get_metrics":
            result = await analytics_processor.get_metrics(arguments)
            return [TextContent(
                type="text",
                text=f"📊 **Subscription Metrics**\n\n" +
                     json.dumps(result, indent=2)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"**Subscription Search Results**\n\n" +
                     json.dumps(result, indent=2)
            )]
            ...
            return [TextContent(
                type="text",
                text=f"**Natural Language Processing Results**\n\n" +
                     json.dumps(result, indent=2)
            )]

    async def _handle_discovery_actions(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_hierarchy_actions(self, action: str, arguments: Dict[str, Any], hierarchy_manager) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if action == "get_product_details":
            ...
            return [TextContent(
                type="text",
                text=f"🔗 **Subscription and Credentials Created Successfully**\n\n" +
                     json.dumps(result, indent=2)
            )]

    async def _handle_validate_action(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _handle_unknown_action(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        raise ToolError(
            ...
            examples={
                ...
                "discovery_actions": ["get_capabilities", "get_examples", "get_agent_summary", "get_supporting_data"],
                "search_actions": ["search_subscriptions", "subscription_nlp"],
                "validation_actions": ["validate", "discover_products", "validate_product_for_subscription"],
                "analytics_actions": ["get_metrics", "get_tool_metadata"],
                "hierarchy_actions": ["get_product_details", "get_credentials", "create_with_credentials"],
    ...
    async def _handle_get_capabilities(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if self.ucm_helper:
            try:
                logger.info("Subscription Management: UCM helper available, fetching capabilities...")
                ucm_capabilities = await self.ucm_helper.ucm.get_capabilities("subscriptions")
                logger.info(f"Subscription Management: Got UCM capabilities with {len(ucm_capabilities.get('billing_periods', []))} billing periods")
            except ToolError:
                # Re-raise ToolError exceptions without modification
                # This preserves helpful error messages with specific suggestions
                raise
            except Exception as e:
                logger.warning(f"Failed to get UCM subscription capabilities, using static data: {e}")
        else:
            logger.info("⚠️ Subscription Management: No UCM helper available, using static capabilities")
...
```"""
        ...
        text += "\n\n## **Optional Fields**\n"
        optional_fields = subscription_schema.get("optional", ["description", "start_date", "end_date", "billing_address", "payment_method", "trial_end_date", "metadata", "tags"])
...
3. Use `create(subscription_data={...})` to create subscriptions"""
    ...
    def _format_capabilities_response(self, capabilities: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        result_text += f"### Creation States\n"
        for state in lifecycle.get("creation", []):
            result_text += f"- `{state}`\n"
        result_text += f"### Active States\n"
        for state in lifecycle.get("active_states", []):
            result_text += f"- `{state}`\n"
        result_text += f"### Terminal States\n"
    ...
    def _format_examples_response(self, examples: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if "error" in examples:
            ...
            raise create_structured_validation_error(
                ...
                suggestions=[
                    f"Available types: {', '.join(available_types)}" if available_types else "Check input parameters",
        ...
        if validation_result.get("suggestions"):
            ...
            for suggestion in validation_result["suggestions"]:
                if isinstance(suggestion, dict):
                    result_text += f"- **{suggestion.get('type', 'info')}**: {suggestion.get('message', '')}\n"
    ...
    async def _get_tool_capabilities(self) -> List[ToolCapability]:
        ...
        return [
            ...
            ToolCapability(
                ...
                parameters={
                    "get_metrics": {"filters": "dict", "date_range": "dict"}
                },
                examples=[
            ...
            ToolCapability(
                ...
                parameters={
                    "create_simple": {"product_id": "str", "clientEmailAddress": "str", "type": "str"},
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "list", "get", "create", "update", "cancel", "delete",
            "create_simple", "create_from_text", "get_metrics",
            "discover_products", "validate_product_for_subscription",
            "get_supporting_data", "search_subscriptions", "subscription_nlp",
            "get_capabilities", "get_examples", "validate",
            "get_agent_summary", "get_tool_metadata",
            # Hierarchy actions
            "get_product_details", "get_credentials", "create_with_credentials"
        ]
    ...
    async def _get_usage_patterns(self) -> List[UsagePattern]:
        ...
        return [
            UsagePattern(
                ...
                success_indicators=["Subscriptions listed successfully", "Subscription details retrieved"]
            ),
            UsagePattern(
