...
class TimeSeriesProcessor:
        ...
        for bucket_start, bucket_points in buckets.items():
            if bucket_points:
                aggregated_value = self._aggregate_values(
                    [p.value for p in bucket_points], 
                    aggregation
                )
    ...
    def compare_periods(
        ...
        current_value = self._aggregate_values(
            [p.value for p in current_data], 
            aggregation
        )
        previous_value = self._aggregate_values(
            [p.value for p in previous_data], 
            aggregation
        )
