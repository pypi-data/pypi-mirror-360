...
class IntelligentClarificationEngine:
        ...
        for pattern_idx, pattern in enumerate(patterns):
            for match in re.finditer(pattern, text, re.IGNORECASE):
                try:
                    ...
                    print(f"🔍 REGEX DEBUG: Pattern {pattern_idx} '{pattern}' matched '{match.group(0)}' -> amount_str='{amount_str}' -> amount={amount}")
                    ...
                    print(f"🔍 DETECTED VALUE: amount={detected_value.amount}, raw_text='{detected_value.raw_text}'")
    ...
    def _parse_multi_tier_structure(self, text: str) -> List[DetectedValue]:
        ...
        if first_match and first_rate_match and up_to_match and second_rate_match:
            ...
            print(f"🔍 MULTI-TIER DEBUG: SUCCESS! Found all 4 values - threshold1={threshold1}, rate1={rate1}, threshold2={threshold2}, rate2={rate2}")
            ...
            values_data = [
                ...
                (rate2, f"{rate2} dollars", "unit_rate", up_to_match.end() + second_rate_match.start())
            ]
            ...
            print(f"🔍 MULTI-TIER DEBUG: FAILED - Missing components: first={bool(first_match)}, first_rate={bool(first_rate_match)}, up_to={bool(up_to_match)}, second_rate={bool(second_rate_match)}")

        # If no complex pattern found, return empty list to fall back to regular parsing
        return []

    def _determine_possible_types(self, amount: float, context_before: str,
                                context_after: str, raw_text: str) -> List[PricingComponentType]:
        ...
        setup_fee_patterns = [
            ...
            r'setup\s+fee', r'initial\s+fee', r'onboarding\s+fee', r'activation\s+fee',
            r'implementation\s+fee', r'deployment\s+fee', r'installation\s+fee',
            r'one-?time\s+fee', r'startup\s+fee', r'initiation\s+fee',
        ...
        if any(re.search(pattern, full_context) for pattern in setup_fee_patterns):
            ...
            base_patterns = [
                r'monthly\s+(?:fee|charge|cost)', r'per\s+month', r'base\s+charge',
                r'recurring\s+(?:fee|charge)', r'billing', r'plan\s+(?:fee|cost)',
        ...
        unit_patterns = [
            r'per\s+(?:request|call|unit|item|transaction)', r'each', r'rate',
            r'cost\s+per', r'price\s+per'
        ]
        if (any(re.search(pattern, full_context) for pattern in unit_patterns) or 
            amount < 1.0):  # Small amounts likely unit rates
            possible_types.append(PricingComponentType.UNIT_RATE)
        
        # Tier threshold patterns (typically larger numbers)
        threshold_patterns = [
            r'up\s+to', r'first\s+\d+', r'after\s+\d+', r'tier', r'limit',
            r'included', r'free'
        ]
        if (any(re.search(pattern, full_context) for pattern in threshold_patterns) or
            amount > 100 and amount == int(amount)):  # Round numbers likely thresholds
            possible_types.append(PricingComponentType.TIER_THRESHOLD)
        
        # Flat amount patterns
        flat_patterns = [
            r'flat\s+(?:fee|rate|amount)', r'fixed\s+(?:fee|rate|amount)',
            r'one\s+time', r'lump\s+sum'
        ]
    ...
    def _calculate_confidence(self, possible_types: List[PricingComponentType], 
    ...
    def _build_pricing_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'setup_fee': [
                r'setup\s+fee', r'initial\s+(?:fee|cost|charge)',
                r'onboarding\s+(?:fee|cost)', r'activation\s+(?:fee|cost)',
                r'per\s+subscription\s+setup', r'per\s+customer\s+setup'
            ],
            'base_charge': [
                r'monthly\s+(?:fee|charge|cost)', r'per\s+month',
                r'subscription\s+(?:fee|cost)', r'base\s+(?:fee|charge)',
                r'recurring\s+(?:fee|charge)'
            ],
            'unit_rate': [
                r'per\s+(?:request|call|unit|item|transaction)',
                r'cost\s+per', r'price\s+per', r'rate\s+of'
            ]
        }
    ...
    def _build_context_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            'subscription_context': [
                r'subscription', r'monthly', r'recurring', r'plan'
            ],
            'usage_context': [
                r'usage', r'per\s+(?:request|call|unit)', r'tier', r'graduated'
            ],
            'setup_context': [
                r'setup', r'initial', r'onboarding', r'activation'
            ]
        }

    def _generate_clarification_options(self, user_input: str,
                                      detected_values: List[DetectedValue]) -> List[ClarificationOption]:
    ...
    def _generate_two_value_options(self, user_input: str,
                                  values: List[DetectedValue]) -> List[ClarificationOption]:
        ...
        if (PricingComponentType.SETUP_FEE in val1.possible_types and
            PricingComponentType.BASE_CHARGE in val2.possible_types):
        ...
        if (PricingComponentType.BASE_CHARGE in val1.possible_types and
            PricingComponentType.UNIT_RATE in val2.possible_types):
        ...
        if (PricingComponentType.TIER_THRESHOLD in val1.possible_types and
            PricingComponentType.UNIT_RATE in val2.possible_types):
    ...
    def _generate_three_value_options(self, user_input: str,
                                    values: List[DetectedValue]) -> List[ClarificationOption]:
    ...
    def _generate_multi_value_options(self, user_input: str,
                                    values: List[DetectedValue]) -> List[ClarificationOption]:
        ...
        if (len(values) == 4 and
            values[0].pattern_type == "multi_tier" and
            PricingComponentType.TIER_THRESHOLD in values[0].possible_types and
            PricingComponentType.UNIT_RATE in values[1].possible_types):
                    ...
                    "plan": {
                        ...
                        "ratingAggregations": [{
                            ...
                            "tiers": [
                                {
                                    "name": f"Tier {i+1}",
                                    "upTo": str(int(val.amount)) if i < len(values)-1 else None,
    ...
    def _determine_setup_fee_type(self, context_before: str, context_after: str) -> SetupFeeType:
        ...
        print(f"🔍 SETUP FEE TYPE DEBUG: context_before='{context_before}', context_after='{context_after}', full_context='{full_context}'")
        ...
        org_patterns = [
            ...
            (r'(?:customer|organization|client|company)\s+(?:setup\s+fee|onboarding|implementation|initial\s+fee)', 0.7),
            (r'(?:setup\s+fee|onboarding|implementation|initial\s+fee).*?(?:customer|organization|client|company)', 0.6),
        ...
        for pattern, confidence in org_patterns:
            if re.search(pattern, full_context):
                ...
                print(f"🔍 SETUP FEE TYPE DEBUG: Matched ORG pattern '{pattern}' with confidence {confidence}")

        for pattern, confidence in subscription_patterns:
            if re.search(pattern, full_context):
                subscription_confidence = max(subscription_confidence, confidence)
                print(f"🔍 SETUP FEE TYPE DEBUG: Matched SUB pattern '{pattern}' with confidence {confidence}")
        ...
        if org_confidence > subscription_confidence and org_confidence > 0.5:
            ...
            print(f"🔍 SETUP FEE TYPE DEBUG: SUBSCRIPTION (default - org: {org_confidence}, sub: {subscription_confidence})")
            return SetupFeeType.SUBSCRIPTION

    def _create_simple_clarification(self, user_input: str,
                                   detected_values: List[DetectedValue]) -> ClarificationRequest:
    ...
    def _generate_ambiguity_explanation(self, detected_values: List[DetectedValue]) -> str:
        ...
        return (f"I detected {len(detected_values)} pricing values: {'; '.join(value_descriptions)}. "
    ...
    def format_clarification_response(self, clarification: ClarificationRequest) -> str:
        ...
        response = f"🤔 **Pricing Clarification Needed**\n\n"
        ...
        for i, option in enumerate(clarification.clarification_options, 1):
            confidence_emoji = "🎯" if option.option_id == clarification.recommended_option else "💡"
    ...
    def analyze_setup_fee_ambiguity(self, user_input: str) -> Dict[str, Any]:
        ...
        setup_fee_mentioned = any(term in text_lower for term in [
            'setup fee', 'initial fee', 'onboarding fee', 'activation fee',
            'implementation fee', 'deployment fee', 'one-time fee', 'startup fee'
        ])
        ...
        for fee in setup_fee_candidates:
            ...
            has_explicit_customer = any(term in full_context for term in [
                'per customer', 'per client', 'per organization', 'per company',
                'each customer', 'each client', 'each organization'
            ])

            has_explicit_subscription = any(term in full_context for term in [
                'per subscription', 'each subscription', 'subscription setup'
            ])
    ...
    def _generate_setup_fee_clarification_guidance(self, ambiguous_fees: List[Dict[str, Any]]) -> str:
