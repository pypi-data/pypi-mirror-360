...
class ConfigurationStatus(ToolBase):
    ...
    def _build_system_health_summary(self, validation_result, onboarding_state) -> str:
        ...
        health_checks = [
            ...
            ("Authentication", validation_result.summary.get('auth_config_works', False)),
            ("Auto-Discovery", validation_result.summary.get('auto_discovery_works', False)),
            ("Required Configuration", validation_result.summary.get('api_key_available', False)),
            ("Cache System", onboarding_state.cache_valid)
        ]

        healthy_count = 0
        for check_name, is_healthy in health_checks:
            status_icon = "✅" if is_healthy else "❌"
            summary += f"- {status_icon} **{check_name}**: {'Healthy' if is_healthy else 'Issue detected'}\n"
            if is_healthy:
                healthy_count += 1
        ...
        summary += f"\n**Health Score**: {healthy_count}/{total_checks} ({health_percentage:.1f}%)\n\n"
