...
class ToolIntrospection(ToolBase):
    ...
    tool_description: ClassVar[str] = "Tool introspection providing detailed description of each tool's function"
    ...
    async def handle_action(
        ...
        try:
            ...
            if action == "get_capabilities":
                ...
                if action in ["list_tools", "get_tool_metadata", "get_all_metadata"]:
                    return await self.introspection_service.handle_introspection_action(action, arguments)
                else:
    ...
    async def _handle_get_capabilities(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        action_list = "\n".join([f"- `{action}` - {self._get_action_description(action)}" for action in actions])
...
- **Agent-Friendly**: Perfect experience for agents with zero prior knowledge

## **Integration**
- Integrates with UCM capability management system
- Provides agent-friendly formatted responses
...
"""
    ...
    def _get_action_description(self, action: str) -> str:
        ...
        descriptions = {
            ...
            "get_all_metadata": "Get comprehensive metadata for all tools",
...
```json
{
  "action": "get_all_metadata"
}
```

## **Usage Tips**
1. **Start with `list_tools`** to see all available tools
2. **Use `get_tool_metadata`** for detailed information about specific tools
3. **Use `get_all_metadata`** for comprehensive tool information
    ...
    def _handle_unknown_action(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        raise ToolError(
            ...
            suggestions=[
                ...
                "Valid actions: list_tools, get_tool_metadata, get_all_metadata, get_capabilities, get_examples"
            ],
            examples={
