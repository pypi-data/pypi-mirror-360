...
from ..introspection.metadata import (
    ToolType, ToolCapability, UsagePattern, ToolDependency,
...
class ProductManager:
    ...
    async def create_product(self, arguments: Dict[str, Any], enhancement_processor=None) -> Dict[str, Any]:
        ...
        if description:
            ...
            if "plan" in parsed_data and (not parsed_data["plan"].get("name") or parsed_data["plan"]["name"].strip() == ""):
            ...
            if "setupFees" in product_data and product_data["setupFees"]:
                ...
                validation_result = enhancement_processor._validate_setup_fee_configuration(setup_fee_data) if enhancement_processor else None

                if validation_result and not validation_result["valid"]:
                    suggestions = enhancement_processor._generate_setup_fee_suggestions(description) if enhancement_processor else []
                    error_message = f"Setup fee validation failed: {'; '.join(validation_result['errors'])}"
                    ...
                    raise create_structured_validation_error(
                        ...
                        examples={
                            "valid_setup_fees": [
                                ...
                                {"name": "Customer Setup", "type": "ORGANIZATION", "flatAmount": 100.00}
                            ],
                            "setup_fee_types": ["SUBSCRIPTION (per subscription)", "ORGANIZATION (per customer)"],
                            ...
                            "migration_notice": "⚠️ UPDATED: Use 'flatAmount' instead of 'amount', 'currency' field no longer used"
                        }
                    )
        ...
        # Assign default source to make product subscription-ready (from create_from_description)
        if description:  # Only for NLP-created products
            try:
                ...
                if sources:
                    ...
                    logger.info(f"Assigned default source {default_source_id} to product from description")
                else:
        ...
        if not product_data:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "updatable_fields": ["name", "description", "version", "plan", "paymentSource", "published"],
...
class ProductValidator:
        ...
        if self.ucm_helper:
            try:
                ...
                raise ToolError(
                    ...
                    examples={
                        "troubleshooting": ["Check UCM service status", "Verify authentication", "Test connectivity"],
    ...
    def get_examples(self, example_type: Optional[str] = None) -> Dict[str, Any]:  # noqa: ARG002
        ...
        validation_summary = ProductValidationEngine.get_validation_summary()
                ...
                "troubleshooting_example": {
                    ...
                    "common_errors": {
                        ...
                        "list_is_empty": {
                            ...
                            "cause": "Missing required subscription fields or invalid product structure",
                            "solution": "This is automatically handled by the implementation",
                            "note": "If you see this error, it indicates a bug in the implementation"
                        },
                        "validation_failed": {
    ...
    def validate_configuration(self, product_data: Dict[str, Any], dry_run: bool = True) -> Dict[str, Any]:
        ...
        if validation_response.get("isError"):
            return {
                "valid": False,
                "errors": [{"field": "validation", "error": validation_response["content"][0]["text"]}],
                "warnings": [],
                "dry_run": dry_run
            }
        else:
...
class ProductEnhancementProcessor:
    ...
    async def create_simple(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
            ...
            product_config["plan"]["tiers"] = [
                {
                    "name": "Usage Tier",
                    "up_to": None,
                    "unit_amount": str(per_unit_price)
                }
            ]
        ...
        if setup_fee is not None:
            ...
            api_setup_fee_type = "SUBSCRIPTION" if setup_fee_type == "per_subscription" else "ORGANIZATION"
        ...
        try:
            ...
            if (pricing_model == "usage_based" and
                "elementDefinitionId" in str(e) and
                "Missing required parameter" in str(e)):
                            ...
                            "example": {
                                ...
                                "plan": {
                                    ...
                                    "ratingAggregations": [{
                                        ...
                                        "aggregationType": "SUM",
                                        "tiers": [{"up_to": None, "unit_amount": str(per_unit_price)}]
                                    }]
                                }
                            }
                        },
    ...
    async def create_from_description(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not parsed_result.get("name") or parsed_result.get("name").strip() == "":
        ...
        if "plan" in parsed_result and (not parsed_result["plan"].get("name") or parsed_result["plan"]["name"].strip() == ""):
        ...
        if "setupFees" in parsed_result and parsed_result["setupFees"]:
            ...
            if validation_result["warnings"]:
                ...
                parsed_result["_parsing_guidance"]["setup_fee_warnings"] = validation_result["warnings"]
        ...
        try:
            ...
            if sources:
                default_source_id = sources[0]["id"]
                product_data["sourceIds"] = [default_source_id]
                logger.info(f"Assigned default source {default_source_id} to product from create_from_description")
            else:
                logger.warning("No sources available - product may not be subscription-ready")
        except Exception as e:
            logger.warning(f"Failed to assign default source: {e}")
    ...
    async def clarify_pricing(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(detected_values) >= 2:
                ...
                return await self._handle_single_value_pricing(text, detected_values[0], ucm_capabilities)
            else:
    ...
    async def _fallback_pricing_clarification(self, text: str, error: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _validate_setup_fee_configuration(self, setup_fee_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fee_type not in ["SUBSCRIPTION", "ORGANIZATION"]:
            validation_result["errors"].append(f"Invalid setup fee type: {fee_type}. Must be SUBSCRIPTION or ORGANIZATION")
        ...
        try:
            ...
            if amount_value <= 0:
                ...
                validation_result["warnings"].append("Setup fee amount is very high (>$100,000). Please verify this is correct")
            elif amount_value < 1:
                validation_result["warnings"].append("Setup fee amount is very low (<$1). Consider if this should be a usage charge instead")
        except (ValueError, TypeError):
            validation_result["errors"].append(f"Invalid setup fee amount: {flat_amount}. Must be a valid number")
            validation_result["valid"] = False
        ...
        if fee_type == "ORGANIZATION":
            ...
            validation_result["enhanced_data"]["business_rule"] = "One-time fee per customer organization"
        elif fee_type == "SUBSCRIPTION":
            validation_result["enhanced_data"]["description"] = (
                validation_result["enhanced_data"].get("description", "") +
                " (Charged per subscription)"
            ).strip()
            validation_result["enhanced_data"]["business_rule"] = "Fee charged for each subscription"

        # Add validation metadata
        validation_result["enhanced_data"]["validation_timestamp"] = datetime.now().isoformat()
        validation_result["enhanced_data"]["validation_status"] = "valid" if validation_result["valid"] else "invalid"

        return validation_result

    def _generate_setup_fee_suggestions(self, text: str, detected_amount: Optional[float] = None) -> List[str]:
        """Generate helpful suggestions for setup fee configuration."""
        suggestions = []

        if detected_amount:
            if detected_amount > 1000:
                suggestions.append(f"High setup fee (${detected_amount:.0f}) detected - consider if this should be 'per customer' (ORGANIZATION type)")
            elif detected_amount < 50:
                suggestions.append(f"Low setup fee (${detected_amount:.0f}) detected - consider if this should be 'per subscription' (SUBSCRIPTION type)")

        # Analyze text for context clues
        text_lower = text.lower()
        if any(word in text_lower for word in ["customer", "organization", "client", "company"]):
            suggestions.append("Text mentions customers/organizations - consider ORGANIZATION type setup fee")
        elif any(word in text_lower for word in ["subscription", "plan", "service"]):
            suggestions.append("Text mentions subscriptions/plans - consider SUBSCRIPTION type setup fee")
    ...
    async def _handle_ambiguous_pricing(self, text: str, detected_values: List[Any], ucm_capabilities: Dict[str, Any]) -> Dict[str, Any]:  # noqa: ARG002
        ...
        return {
            ...
            "setup_fee_options": {
                "per_subscription": {
                    ...
                    "api_format": {"name": "Setup Fee", "type": "SUBSCRIPTION", "flatAmount": 100.00}
                },
                "per_customer": {
                    ...
                    "api_format": {"name": "Customer Setup", "type": "ORGANIZATION", "flatAmount": 500.00}
                }
            },
    ...
    async def _handle_single_value_pricing(self, text: str, detected_value: Any, ucm_capabilities: Dict[str, Any]) -> Dict[str, Any]:  # noqa: ARG002
        """Handle cases with a single pricing value."""
        # Check if setup fee is mentioned in the text
        text_lower = text.lower()
        setup_mentioned = any(term in text_lower for term in [
            'setup', 'initial', 'onboarding', 'activation', 'implementation', 'one-time'
        ])
        ...
        if setup_mentioned:
            ...
            return {
                ...
                "setup_fee_format": {
                    "new_structure": {"name": "Setup Fee", "type": "SUBSCRIPTION", "flatAmount": 100.00},
    ...
    async def _handle_no_values_detected(self, text: str, ucm_capabilities: Dict[str, Any]) -> Dict[str, Any]:
...
class ProductHierarchyManager:
    ...
    async def get_related_credentials(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        hierarchy_data = navigation_result.related_entities[0] if navigation_result.related_entities else {}
    ...
    def _validate_required_fields(self, product_data: Dict[str, Any], subscription_data: Dict[str, Any]):
        ...
        for field, description in product_required_fields.items():
            if field not in product_data or not product_data[field]:
                raise create_structured_missing_parameter_error(
                    parameter_name=f"product_data.{field}", action="create product with subscription",
                    examples={"usage": f"product_data={{\"{field}\": \"example_value\", ...}}", "description": description,
                             "complete_example": {"name": "AI Analytics Platform", "version": "1.0.0",
                                                "plan": {"type": "SUBSCRIPTION", "name": "Plan Name", "currency": "USD"}},
        ...
        if "name" not in product_data.get("plan", {}):
            raise create_structured_missing_parameter_error(
                parameter_name="product_data.plan.name", action="create product with subscription",
                examples={"usage": "product_data={\"plan\": {\"name\": \"Plan Name\", \"type\": \"SUBSCRIPTION\", ...}}",
        ...
        for field, description in subscription_required_fields.items():
            if field not in subscription_data or not subscription_data[field]:
                raise create_structured_missing_parameter_error(
                    parameter_name=f"subscription_data.{field}", action="create product with subscription",
                    examples={"usage": f"subscription_data={{\"{field}\": \"example_value\", ...}}", "description": description,
                             "complete_example": {"name": "Customer Subscription", "clientEmailAddress": "customer@company.com"},
    ...
    async def create_with_subscription(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product_data, subscription_data = self._validate_create_with_subscription_parameters(arguments)
        ...
        logger.info(f"Enhancing product data for subscription compatibility: {product_data.get('name')}")
        ...
        if "plan" in product_data and ("ratingAggregations" not in product_data["plan"] or not product_data["plan"]["ratingAggregations"]):
            # Get available metering elements dynamically
            try:
                elements_response = await self.client.get_metering_elements(page=0, size=1)
            ...
            if "plan" in product_data and "tiers" in product_data["plan"]:
                for tier in product_data["plan"]["tiers"]:
                    if tier.get("up_to") is None:
                        ...
                        logger.info("Modified plan tier to be limited to avoid unlimited tier conflict")
        ...
        if "ownerId" not in subscription_data:
            ...
            if owner_id:
                ...
                logger.warning("REVENIUM_OWNER_ID not available from configuration store, API will use default owner")
        ...
        if "organizationId" not in subscription_data:
            ...
            try:
                ...
                logger.error(f"Failed to auto-resolve organizationId for subscription creation: {e}")
                raise create_structured_missing_parameter_error(
                    parameter_name="organizationId",
                    action="create subscription",
                    examples={
...
class ProductManagement(ToolBase):
    ...
    async def _setup_managers(self):
        ...
        return (
            ...
            ProductEnhancementProcessor(client),
            ProductHierarchyManager(client)
        )

    async def _handle_introspection_actions(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_standard_crud_actions(
        ...
        if action == "list":
            ...
            enhancement_processor = ProductEnhancementProcessor(self.client)
            return await self._handle_crud_with_dry_run(action, arguments, product_manager, enhancement_processor)
    ...
    async def _handle_create_dry_run(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if validation_result["valid"]:
            ...
            errors_text = "\n".join([f"• {error.get('error', 'Unknown error')}" for error in validation_result.get("errors", [])])
            return [TextContent(
                type="text",
    ...
    async def _handle_update_dry_run(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_delete_dry_run(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Handle dry run for delete operations."""
        product_id = arguments.get("product_id")
        return [TextContent(
            type="text",
            text=f"🧪 **DRY RUN MODE - Product Deletion**\n\n"
                 f"⚠️ **Would delete product:** {product_id}\n\n"
                 f"**Dry Run:** True (no actual deletion performed)\n\n"
                 f"💡 **Warning:** This action cannot be undone in real mode"
        )]

    def _format_create_success_response(self, result: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _format_update_success_response(self, result: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _format_delete_success_response(self, result: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_create_from_description(
        ...
        if dry_run:
            return await self._handle_create_from_description_dry_run(arguments, enhancement_processor)
    ...
    async def _handle_create_from_description_dry_run(
        ...
        if enhancement_processor.nlp_processor:
            try:
                parsed_result = enhancement_processor.nlp_processor.parse_product_request(description)
            ...
            return [TextContent(
                type="text",
                text=f"🧪 **DRY RUN MODE - Create from Description**\n\n"
                     f"⚠️ **NLP Processor Unavailable**: Would use fallback creation\n\n"
                     f"**Original Description:** {description}\n\n"
                     f"**Dry Run:** True (no actual creation performed)"
            )]
    ...
    def _generate_educational_feedback(self, parsed_result: Dict[str, Any]) -> str:
        ...
        if "setupFees" in parsed_result and parsed_result["setupFees"]:
            ...
            if setup_fee.get("type") == "SUBSCRIPTION":
                educational_notes.append("✅ **Setup Fee Structure**: Using new SUBSCRIPTION type (charged per subscription)")
            elif setup_fee.get("type") == "ORGANIZATION":
                educational_notes.append("✅ **Setup Fee Structure**: Using new ORGANIZATION type (charged per customer)")

            if "flatAmount" in setup_fee:
                educational_notes.append("✅ **Setup Fee Format**: Using new 'flatAmount' field structure")
            else:
                educational_notes.append("⚠️ **Setup Fee Migration**: Consider using 'flatAmount' instead of 'amount'")

        if "paymentSource" in parsed_result:
            payment_source = parsed_result["paymentSource"]
            if payment_source == "INVOICE_ONLY_NO_PAYMENT":
                educational_notes.append("✅ **Payment Source**: Manual invoice payment (customers pay outside system)")
            elif payment_source == "EXTERNAL_PAYMENT_NOTIFICATION":
                educational_notes.append("✅ **Payment Source**: Tracked invoice payment (external system confirms payment)")
    ...
    async def _handle_hierarchy_actions(
        ...
        action: str,
        arguments: Dict[str, Any],
        hierarchy_manager: ProductHierarchyManager
    ) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _format_create_with_subscription_response(
        ...
        if "plan" in product and "ratingAggregations" in product["plan"] and product["plan"]["ratingAggregations"]:
            rating_agg = product["plan"]["ratingAggregations"][0]
            element_id = rating_agg.get("elementDefinitionId", "unknown")
            auto_configured.append(f"Usage billing: {element_id} metering element (dynamically resolved)")
        ...
        auto_config_text = ""
        if auto_configured:
            auto_config_text = "\n\n**🔧 Auto-Configured:**\n" + "\n".join([f"• {config}" for config in auto_configured])

        return [TextContent(
            type="text",
            text=f"🎉 **Product and Subscription Created Successfully**\n\n" +
                 ...
                 f"{auto_config_text}\n\n" +
                 f"**Next Steps:**\n" +
                 f"• Use manage_subscriber_credentials to add API keys for the subscription\n" +
                 f"• Use 'get_subscriptions' to see all subscriptions for this product\n" +
                 f"• Use manage_subscriber_credentials to add credentials to the subscription"
        )]

    async def _handle_fallback_actions(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _handle_unknown_action(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        raise ToolError(
            ...
            examples={
                ...
                "hierarchy_actions": ["get_subscriptions", "get_related_credentials", "create_with_subscription"],
                "example_usage": {
    ...
    async def handle_action(
        ...
        try:
                    ...
                    if description:
                        try:
                            ...
                            if "setupFees" in merged_data and merged_data["setupFees"]:
                                ...
                                if setup_fee.get("type") == "SUBSCRIPTION":
                                    educational_notes.append("✅ **Setup Fee Structure**: Using SUBSCRIPTION type (charged per subscription)")
                                elif setup_fee.get("type") == "ORGANIZATION":
                                    educational_notes.append("✅ **Setup Fee Structure**: Using ORGANIZATION type (charged per customer)")
                                if "flatAmount" in setup_fee:
                                    educational_notes.append("✅ **Setup Fee Format**: Using new 'flatAmount' field structure")
            ...
            # Handle hierarchy actions
                ...
                auto_configured.append("System fields: ownerId, teamId (auto-populated from authenticated context)")
                auto_configured.append("Required subscription fields: 15+ fields with proper defaults")

                # Build enhanced success message
                enhanced_message = f"✅ Product and subscription created successfully with hierarchy linking\n\n"
                enhanced_message += f"**🔗 Created Entities:**\n"
                enhanced_message += f"• **Product**: {product['name']} (ID: {product_id})\n"
                enhanced_message += f"• **Subscription**: {subscription['name']} (ID: {subscription_id})\n"
                enhanced_message += f"• **Client**: {subscription['client']['label']} (auto-created)\n\n"

                enhanced_message += f"**🤖 Automatic Enhancements Applied:**\n"
                for config in auto_configured:
                    enhanced_message += f"• {config}\n"
                enhanced_message += "\n"

                enhanced_message += f"**📊 Product Structure:**\n"
                enhanced_message += f"• **Subscription-ready**: Product has sources and rating aggregations\n"
                enhanced_message += f"• **Billing period**: {product['plan']['period']}\n"
                enhanced_message += f"• **Currency**: {product['plan']['currency']}\n"
                enhanced_message += f"• **Tier structure**: Optimized (single unlimited tier)\n\n"

                enhanced_message += f"**💡 Account-Agnostic Design:**\n"
                enhanced_message += f"• All resource IDs dynamically resolved from your account\n"
                enhanced_message += f"• No hardcoded values - works across different Revenium accounts\n"
                enhanced_message += f"• Automatic prerequisite validation and error handling\n"

                return [TextContent(
                    type="text",
                    text=enhanced_message + f"\n**Next Steps:**\n" +
    ...
    async def _handle_get_capabilities(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        return [TextContent(
            type="text",
            text=await self._build_enhanced_capabilities_text(ucm_capabilities)
        )]
...
# ⚠️ IMPORTANT: Copy the actual elementDefinitionId from Step 1 results - IDs are unique per account
...
create(product_data={
  ...
  "plan": {
    "ratingAggregations": [{
      "elementDefinitionId": "COPY_FROM_STEP_1",  # ⚠️ Use actual ID from manage_metering_elements list
      "aggregationType": "SUM",
...
```"""
        ...
        if ucm_capabilities and "currencies" in ucm_capabilities:
            text += "\n\n## **Supported Currencies (API-Verified)**\n"
        ...
        text += "\n\n## **Supported Payment Sources (API-Verified)**\n"
        ...
        text += "\n**⚠️ Stripe Limitation**: Stripe-supported products must be configured manually through the Revenium UI and are not currently available via the MCP API.\n"
        ...
        text += "\n\n## **Optional Fields**\n"
        optional_fields = product_schema.get("optional", ["description", "source_ids", "sla_ids", "custom_pricing_rule_ids", "notification_addresses_on_invoice", "tags", "terms", "coming_soon", "setup_fees", "rating_aggregations", "elements"])
...
- **⚠️ CRITICAL**: Element IDs are unique per account - NEVER hardcode IDs, always lookup from list results
...
3. Use `create` to create the product"""
    ...
    def _format_capabilities_response(self, capabilities: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _format_examples_response(self, examples: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        result_text = "**Product Creation Examples** (Updated with Current Valid Values)\n\n"
    ...
    def _format_create_with_subscription_examples(self, examples: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if coordinated_example:
            ...
            result_text += f"**Type**: `{coordinated_example.get('type', 'coordinated_creation')}`\n"
            ...
            if prerequisites:
                ...
                for step_key, step in prerequisites.items():
                    result_text += f"**{step.get('action', 'Step')}**:\n"
                    result_text += f"```bash\n{step.get('tool_call', 'N/A')}\n```\n"
                    result_text += f"*Purpose*: {step.get('purpose', 'N/A')}\n"
                    result_text += f"*Required*: {'✅ Yes' if step.get('required') else '⚠️ Optional'}\n\n"

            # Template section
            template = coordinated_example.get("template", {})
            if template:
                result_text += "### **📋 Copy-Paste Ready Template**\n\n"
                result_text += "```json\n"
        ...
        result_text += "4. **Verify results**: Check the enhanced output for automatic configurations\n\n"

        result_text += "**💡 Pro Tip**: This workflow is account-agnostic and works across different Revenium accounts!\n"

        return [TextContent(type="text", text=result_text)]

    def _format_validation_response(self, validation_result: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if validation_result.get("valid"):
            ...
            if validation_response_text:
                ...
                result_text += validation_result.get("message", "Your product configuration passes all validation checks and is ready for creation!")
                result_text += "\n\n**✅ Next Steps**:\n"
        ...
        result_text += f"**Validation Engine**: Enhanced (prevents CHARGE vs SUBSCRIPTION errors)\n"
...
• Check API authentication if all operations fail"""
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            ...
            "get_capabilities", "get_examples", "validate",
            "get_agent_summary", "create_simple",
            "get_templates", "suggest_template", "clarify_pricing",
            "get_tool_metadata",
            # Hierarchy actions
            "get_subscriptions", "get_related_credentials", "create_with_subscription"
        ]
