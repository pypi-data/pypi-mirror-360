...
from .analytics_engine import AnomalyFrequencyData, AlertTrendData, TimeRange
...
class AnalyticsFormatter:
    ...
    def format_anomaly_frequency_report(data: AnomalyFrequencyData, time_range: TimeRange) -> str:
        ...
        report.append(f"📅 **Period**: {time_range.start.strftime('%Y-%m-%d')} to {time_range.end.strftime('%Y-%m-%d')} ({time_range.duration_days:.1f} days)")
        report.append("")
        
        # Overview
        report.append("## 📈 **Overview**")
        report.append(f"• **Total Anomalies**: {data.total_anomalies:,}")
        report.append(f"• **Active**: {data.active_anomalies:,} ({data.get_activation_rate():.1f}%)")
        report.append(f"• **Inactive**: {data.inactive_anomalies:,} ({100 - data.get_activation_rate():.1f}%)")
        report.append(f"• **Enabled**: {data.enabled_anomalies:,} ({data.get_enablement_rate():.1f}%)")
        report.append(f"• **Disabled**: {data.disabled_anomalies:,} ({100 - data.get_enablement_rate():.1f}%)")
        report.append("")
        ...
        if data.anomalies_by_status:
            ...
            for status, count in sorted(data.anomalies_by_status.items(), key=lambda x: x[1], reverse=True):
                ...
                report.append(f"• {status_emoji} **{status.title()}**: {count:,} ({percentage:.1f}%) {bar}")
            report.append("")
        ...
        if data.detection_rule_types:
            ...
            for rule_type, count in sorted(data.detection_rule_types.items(), key=lambda x: x[1], reverse=True):
                percentage = (count / total_rules * 100) if total_rules > 0 else 0
                rule_emoji = AnalyticsFormatter._get_rule_type_emoji(rule_type)
                report.append(f"• {rule_emoji} **{rule_type.title()}**: {count:,} rules ({percentage:.1f}%)")
            report.append("")
        ...
        if data.creation_trend:
            ...
            recent_days = data.creation_trend[-7:] if len(data.creation_trend) >= 7 else data.creation_trend
            if recent_days:
                ...
                for day_data in recent_days:
                    ...
                    bar = AnalyticsFormatter._create_mini_bar(count, max(d["count"] for d in recent_days) if recent_days else 1)
    ...
    @staticmethod
    def format_alert_trend_report(data: AlertTrendData, time_range: TimeRange, anomaly_id: Optional[str] = None) -> str:
        ...
        report.append(title)
        report.append(f"📅 **Period**: {time_range.start.strftime('%Y-%m-%d %H:%M')} to {time_range.end.strftime('%Y-%m-%d %H:%M')} ({time_range.duration_hours:.1f} hours)")
        report.append("")
        ...
        if data.resolution_times:
            ...
            report.append(f"• **Average Resolution Time**: {AnalyticsFormatter._format_duration(avg_resolution)}")
            report.append(f"• **Median Resolution Time**: {AnalyticsFormatter._format_duration(median_resolution)}")
        
        alert_rate = data.total_alerts / time_range.duration_hours if time_range.duration_hours > 0 else 0
        ...
        if data.alerts_by_severity:
            ...
            for severity in severity_order:
                if severity in data.alerts_by_severity:
                    ...
                    bar = AnalyticsFormatter._create_progress_bar(percentage, 20)
                    report.append(f"• {severity_emoji} **{severity.title()}**: {count:,} ({percentage:.1f}%) {bar}")
        ...
        if data.alerts_by_status:
            ...
            for status, count in sorted(data.alerts_by_status.items(), key=lambda x: x[1], reverse=True):
                percentage = (count / total * 100) if total > 0 else 0
        ...
        if data.top_triggering_anomalies and not anomaly_id:
            ...
            for i, anomaly in enumerate(data.top_triggering_anomalies, 1):
                medal = AnalyticsFormatter._get_ranking_emoji(i)
                percentage = (anomaly["alert_count"] / data.total_alerts * 100) if data.total_alerts > 0 else 0
        ...
        if data.alert_frequency:
            ...
            frequency_insights = AnalyticsFormatter._analyze_alert_frequency_pattern(data.alert_frequency)
    ...
    @staticmethod
    def format_top_alerting_anomalies_report(anomalies: List[Dict[str, Any]], time_range: TimeRange) -> str:
        ...
        for i, anomaly in enumerate(anomalies, 1):
            ...
            report.append(f"   • **Most Common Severity**: {AnalyticsFormatter._get_severity_emoji(anomaly['most_common_severity'])} {anomaly['most_common_severity'].title()}")
            
            if anomaly.get("latest_alert"):
                latest_alert_time = datetime.fromisoformat(anomaly["latest_alert"].replace("Z", "+00:00"))
                time_ago = datetime.now().replace(tzinfo=latest_alert_time.tzinfo) - latest_alert_time
                report.append(f"   • **Latest Alert**: {AnalyticsFormatter._format_time_ago(time_ago)} ago")
    ...
    def _analyze_alert_frequency_pattern(frequency_data: List[Dict[str, Any]]) -> List[str]:
        ...
        if len(counts) >= 24:  # At least 24 hours of data
            ...
            hourly_averages = {hour: sum(counts) / len(counts) for hour, counts in hourly_patterns.items()}
            busiest_hour = max(hourly_averages.keys(), key=lambda h: hourly_averages[h])
            insights.append(f"**Busiest Hour**: {busiest_hour:02d}:00 (avg {hourly_averages[busiest_hour]:.1f} alerts)")
    ...
    def _generate_anomaly_recommendations(data: AnomalyFrequencyData) -> List[str]:
        ...
        if data.get_enablement_rate() < 90:
            recommendations.append("Enable disabled anomalies or document reasons for keeping them disabled")

        # Rule type recommendations
        if "threshold" not in data.detection_rule_types:
            recommendations.append("Consider adding threshold-based rules for immediate value detection")
        ...
        if data.creation_trend:
            ...
            if recent_activity == 0:
                recommendations.append("No recent anomaly creation - consider reviewing monitoring gaps")
            elif recent_activity > 10:
                recommendations.append("High recent anomaly creation - ensure quality over quantity")
    ...
    def _generate_alert_insights(data: AlertTrendData, time_range: TimeRange) -> List[str]:
        ...
        if data.alerts_by_severity:
            critical_pct = (data.alerts_by_severity.get("critical", 0) / data.total_alerts * 100) if data.total_alerts > 0 else 0
            if critical_pct > 50:
    ...
    def _generate_alert_recommendations(data: AlertTrendData) -> List[str]:
        ...
        if data.total_alerts > 1000:
            recommendations.append("Consider alert consolidation or threshold adjustment to reduce noise")

        # Severity recommendations
        if data.alerts_by_severity:
            total = sum(data.alerts_by_severity.values())
            critical_pct = (data.alerts_by_severity.get("critical", 0) / total * 100) if total > 0 else 0

            if critical_pct > 30:
                recommendations.append("Review critical alert thresholds - too many critical alerts")
            elif critical_pct < 2:
        ...
        if data.resolution_times:
            avg_resolution = data.get_average_resolution_time()
            if avg_resolution > 4:
                recommendations.append("Implement automated response procedures to reduce resolution times")
            ...
            if len(data.resolution_times) > 1:
                ...
                if std_dev > avg_resolution:
                    recommendations.append("High variance in resolution times - standardize response procedures")
        ...
        if data.top_triggering_anomalies:
            ...
            if top_anomaly["alert_count"] > data.total_alerts * 0.3:
                recommendations.append(f"Review '{top_anomaly['anomaly_name']}' - generating too many alerts")
