...
class UCMIntegrationService:
    ...
    async def initialize(self, client: Optional[ReveniumClient] = None) -> None:
        ...
        try:
            ...
            if ("api_key" in error_msg or "revenium_api_key" in error_msg or
                "failed to create revenium client" in error_msg or
                "none was provided" in error_msg):
    ...
    async def _register_analytics_capabilities(self) -> None:
        ...
        try:
            ...
            await asyncio.wait_for(
                analytics_ucm.register_analytics_capabilities(),
                timeout=10.0  # 10 second timeout
            )
            logger.info("Analytics capabilities registered successfully")

        except asyncio.TimeoutError:
            logger.warning("Analytics capabilities registration timed out - using static configuration")
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        if not self._initialized:
            return {
                ...
                "error": "UCM integration service not initialized"
            }
    ...
    async def get_all_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            resource_types = ["system", "products", "customers", "subscriptions", "sources", "analytics"]
            all_capabilities = {}
    ...
    async def get_onboarding_tool_recommendations(
        ...
        if not setup_completion_status.get("api_key_configured") or not setup_completion_status.get("team_id_configured"):
    ...
    async def get_onboarding_enhanced_capabilities(
        ...
        if is_first_time_user:
            ...
            recommendations = await self.get_onboarding_tool_recommendations(setup_completion_status)
