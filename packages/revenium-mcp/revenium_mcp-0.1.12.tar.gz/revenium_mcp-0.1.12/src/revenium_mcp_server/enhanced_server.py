...
async def main() -> None:
    ...
    try:
        ...
        # Check if this is a missing API key issue (important for users to know)
        ...
        if ("api_key" in error_msg or "revenium_api_key" in error_msg or
            ...
            # Always show missing API key message - this is important user information
            import sys
            print("⚠️  REVENIUM_API_KEY not found - server will not work until this is set.", file=sys.stderr)
            print("   Set REVENIUM_API_KEY environment variable. It can be found within the Reveniub web console on the API Keys page.", file=sys.stderr)
    ...
    import sys
    print(f"Revenium MCP Server v{server_version} ready with {summary['registered_tools']} tools", file=sys.stderr)
