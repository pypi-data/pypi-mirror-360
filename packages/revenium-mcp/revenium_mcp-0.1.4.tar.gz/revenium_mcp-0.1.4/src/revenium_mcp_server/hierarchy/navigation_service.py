            ...
            while True:
                try:
                    ...
                    product_subscriptions = [
                        sub for sub in page_subscriptions
                        if (sub.get("product_id") == product_id or
                            sub.get("productId") == product_id or
                            (sub.get("product") and sub.get("product", {}).get("id") == product_id))
                    ]
    ...
    async def get_product_for_subscription(self, subscription_id: str) -> NavigationResult:
        ...
        try:
            ...
            product_id = (subscription.get("product_id") or
                         subscription.get("productId") or
                         (subscription.get("product") and subscription.get("product", {}).get("id")))
                        ...
                        if sub_result.success:
                            ...
                            for subscription in sub_result.related_entities:
                                ...
                                if sub_id:  # Only process if subscription has valid ID
                                    cred_result = await self.get_credentials_for_subscription(sub_id)
                                    if cred_result.success:
                                        hierarchy_data["credentials"].extend(cred_result.related_entities)
                except Exception as e:
