...
class DebugAutoDiscovery(ToolBase):
    ...
    tool_description: ClassVar[str] = "Diagnostic tool to debug auto-discovery configuration issues and validate environment setup. Key actions: debug, get_capabilities, get_examples. Use get_examples() for diagnostic guidance and get_capabilities() for feature details."
    ...
    async def _handle_debug(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            base_url = os.getenv("REVENIUM_BASE_URL", "https://api.revenium.io")
            ...
            result_text = f"""# **Auto-Discovery Diagnostic Report**
...
## **Recommendations**
"""
            if diagnostic_report['summary']['env_vars_missing'] > 0:
                result_text += "- Set missing environment variables before using Revenium tools\n"
            
            if not diagnostic_report['summary']['api_accessible']:
                result_text += "- Check API connectivity and credentials\n"
            
            if diagnostic_report['summary']['env_vars_set'] == len(revenium_vars) and diagnostic_report['summary']['api_accessible']:
                result_text += "- All systems operational! Auto-discovery should work correctly\n"
