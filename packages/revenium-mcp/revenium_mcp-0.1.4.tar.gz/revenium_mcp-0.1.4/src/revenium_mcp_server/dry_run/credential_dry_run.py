...
class CredentialDryRunValidator:
    ...
    def _initialize_validation_rules(self):
        ...
        self.validation_rules = {
            "required_fields": {
                "create": ["label", "subscriberId", "organizationId", "externalId", "externalSecret"],
            ...
            "business_rules": {
                ...
                "immutable_fields": ["subscriberId", "organizationId"]  # Cannot be changed after creation
            }
        }
    ...
    async def validate_create_operation(self, credential_data: Dict[str, Any]) -> DryRunResult:
        ...
        return DryRunResult(
            operation="create",
            valid=not any(issue.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL] for issue in validation_issues),
    ...
    async def validate_update_operation(self, credential_id: str, credential_data: Dict[str, Any]) -> DryRunResult:
        ...
        validation_issues.extend(await self._check_immutable_fields(current_credential, credential_data))

        # Check for conflicts
        validation_issues.extend(await self._check_update_conflicts(credential_id, credential_data))

        # Analyze billing impact
        billing_impact = await self._analyze_billing_impact("update", credential_data, current_credential)
    ...
    async def _validate_required_fields(self, operation: str, credential_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _validate_field_constraints(self, credential_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _check_create_conflicts(self, credential_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _check_update_conflicts(self, credential_id: str, credential_data: Dict[str, Any]) -> List[ValidationIssue]:
        ...
        if "externalId" in credential_data:
            try:
                ...
                for cred in credentials:
                    if cred.get("id") != credential_id and cred.get("externalId") == credential_data["externalId"]:
                        issues.append(ValidationIssue(
                            severity=ValidationSeverity.ERROR,
                            field="externalId",
    ...
    async def _check_immutable_fields(self, current_credential: Dict[str, Any], update_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _validate_business_relationships(self, credential_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _analyze_billing_impact(self, operation: str, credential_data: Dict[str, Any], current_credential: Optional[Dict[str, Any]] = None) -> BillingImpact:
        ...
        if operation == "create":
            ...
            metering_impact = "Secret changes may temporarily disrupt metering until systems are updated"

        # Cost implications
        cost_implications = "No direct cost impact"
        if affected_subscriptions:
            cost_implications = f"May affect billing for {len(affected_subscriptions)} subscription(s)"
    ...
    def _generate_next_steps(self, operation: str, validation_issues: List[ValidationIssue], billing_impact: BillingImpact) -> List[str]:
        """Generate actionable next steps based on validation results."""
        next_steps = []

        # Address critical and error issues first
        critical_issues = [issue for issue in validation_issues if issue.severity in [ValidationSeverity.CRITICAL, ValidationSeverity.ERROR]]
        if critical_issues:
        ...
        # Address warnings
        warning_issues = [issue for issue in validation_issues if issue.severity == ValidationSeverity.WARNING]
        if warning_issues:
        ...
        if not critical_issues:
            if operation == "create":
                next_steps.append("✅ Ready to create credential - use create() without dry_run parameter")
            elif operation == "update":
                next_steps.append("✅ Ready to update credential - use update() without dry_run parameter")
