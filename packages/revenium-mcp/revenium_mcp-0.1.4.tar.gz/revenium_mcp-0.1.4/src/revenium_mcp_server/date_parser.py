...
class DateRangeParser:
    ...
    def _parse_since_expression(self, text: str) -> Dict[str, Optional[str]]:
        ...
        if since_match:
            ...
            if start_date:
                return {
                    "start": start_date,
                    "end": None  # No end date means "until now"
                }
    ...
    def _parse_flexible_date(self, date_text: str) -> Optional[str]:
        ...
        if "yesterday" in date_text:
            ...
            return self.now.replace(hour=0, minute=0, second=0, microsecond=0).isoformat().replace('+00:00', 'Z')
        ...
        for pattern, format_str in common_patterns:
            ...
            if match:
                try:
                    if format_str == '%m/%d/%Y':
                        date = datetime.strptime(f"{match.group(1)}/{match.group(2)}/{match.group(3)}", format_str)
