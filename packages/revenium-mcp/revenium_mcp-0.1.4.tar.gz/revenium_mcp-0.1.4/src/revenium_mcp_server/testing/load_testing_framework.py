...
class LoadTestingFramework:
    ...
    async def run_load_test(
        ...
        spawn_interval = config.ramp_up_seconds / config.concurrent_users if config.concurrent_users > 0 else 0
        ...
        logger.info(f"Results: {result.successful_requests}/{result.total_requests} successful, "
    ...
    def _calculate_results(
        ...
        if request_latencies:
            ...
            if n >= 20:
                ...
                p95_latency_ms = max(avg_latency_ms, sorted_latencies[int(n * 0.9)] if n > 1 else sorted_latencies[0])
        ...
        target_compliance = {
            ...
            "latency_p99": p99_latency_ms <= config.target_latency_ms * 1.5,  # Allow 50% margin for P99
    ...
    def establish_baseline(self, test_name: str, results: List[LoadTestResult]) -> PerformanceBaseline:
        ...
        logger.info(f"Established baseline '{test_name}': "
    ...
    def check_regression(self, test_result: LoadTestResult, baseline_name: str) -> Dict[str, Any]:
        ...
        changes = {
            "avg_latency": ((test_result.avg_latency_ms - baseline.avg_latency_ms) / baseline.avg_latency_ms) * 100,
            "p95_latency": ((test_result.p95_latency_ms - baseline.p95_latency_ms) / baseline.p95_latency_ms) * 100,
            "p99_latency": ((test_result.p99_latency_ms - baseline.p99_latency_ms) / baseline.p99_latency_ms) * 100,
            "throughput": ((test_result.throughput_rps - baseline.throughput_rps) / baseline.throughput_rps) * 100,
            "error_rate": ((test_result.error_rate_percent - baseline.error_rate_percent) / max(baseline.error_rate_percent, 0.1)) * 100
        }
        ...
        for metric, change in changes.items():
            ...
            regressions[metric] = {
                ...
                "current_value": getattr(test_result, metric.replace("_", "_") + ("_ms" if "latency" in metric else "_rps" if metric == "throughput" else "_percent")),
                "baseline_value": getattr(baseline, metric.replace("_", "_") + ("_ms" if "latency" in metric else "_rps" if metric == "throughput" else "_percent"))
            }
...
async def run_comprehensive_load_test_suite() -> Dict[str, Any]:
    ...
    for config in load_testing_framework.default_configs:
        ...
        try:
            ...
            logger.info(f"  - P95 Latency: {result.p95_latency_ms:.2f}ms (target: {config.target_latency_ms}ms)")
            logger.info(f"  - Throughput: {result.throughput_rps:.2f} RPS (target: {config.target_throughput_rps} RPS)")
            logger.info(f"  - Error Rate: {result.error_rate_percent:.2f}% (target: {config.target_error_rate_percent}%)")
    ...
    suite_summary = {
        ...
        "success_rate_percent": (len(successful_tests) / len(suite_results) * 100) if suite_results else 0,
        "overall_performance": {
            "avg_p95_latency_ms": statistics.mean([r.p95_latency_ms for r in suite_results]) if suite_results else 0,
            "avg_throughput_rps": statistics.mean([r.throughput_rps for r in suite_results]) if suite_results else 0,
            "avg_error_rate_percent": statistics.mean([r.error_rate_percent for r in suite_results]) if suite_results else 0
        },
        "test_results": [asdict(r) for r in suite_results]
    }

    logger.info(f"Load test suite completed: {len(successful_tests)}/{len(suite_results)} tests passed")
