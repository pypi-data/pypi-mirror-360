...
class CommonErrors:
    ...
    @staticmethod
    def missing_required_parameter(parameter_name: str, valid_values: Optional[List[str]] = None) -> StandardError:
        ...
        return StandardErrorBuilder(f"Missing required parameter: {parameter_name}").\
    ...
    @staticmethod
    def invalid_parameter_value(parameter_name: str, provided_value: Any, 
                              valid_values: Optional[List[str]] = None,
                              expected_format: Optional[str] = None) -> StandardError:
        ...
        return StandardErrorBuilder(f"Invalid value for parameter: {parameter_name}").\
            field(parameter_name).\
            expected(expected).\
    ...
    @staticmethod
    def validation_failed(field_name: str, reason: str, example: Optional[Any] = None) -> StandardError:
        ...
        builder = StandardErrorBuilder(f"Validation failed for {field_name}: {reason}").\
            field(field_name).\
    ...
    def api_error(status_code: int, message: str, endpoint: str) -> StandardError:
        ...
        if status_code == 400:
            suggestions.extend([
                "Validate your request data format",
        ...
        return StandardErrorBuilder(f"API request failed: {message}").\
            field("api_request").\
    ...
    def unknown_action(action: str, valid_actions: List[str]) -> StandardError:
        ...
        return StandardErrorBuilder(f"Unknown action: {action}").\
            field("action").\
...
def create_missing_parameter_error(parameter_name: str, valid_values: Optional[List[str]] = None) -> TextContent:
...
def create_invalid_value_error(parameter_name: str, provided_value: Any, 
                             valid_values: Optional[List[str]] = None,
                             expected_format: Optional[str] = None) -> TextContent:
    """Quick function to create invalid value error."""
    error = CommonErrors.invalid_parameter_value(parameter_name, provided_value, valid_values, expected_format)
    return StandardErrorFormatter.format_for_mcp(error)


def create_validation_error(field_name: str, reason: str, example: Optional[Any] = None) -> TextContent:
