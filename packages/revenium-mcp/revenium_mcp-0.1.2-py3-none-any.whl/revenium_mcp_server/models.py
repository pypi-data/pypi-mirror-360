...
class Tier(BaseReveniumModel):
    ...
    up_to: Optional[Union[int, float, Decimal]] = Field(..., description="Upper bound of tier range (null for unlimited)")
    unit_amount: Optional[Union[int, float, Decimal]] = Field(None, description="Price per unit in this tier")
    flat_amount: Optional[Union[int, float, Decimal]] = Field(None, description="Flat fee for this tier")
...
class Plan(BaseReveniumModel):
    ...
    rating_aggregations: List[RatingAggregation] = Field(default_factory=list, description="Array of rating aggregations")
...
class Product(BaseReveniumModel):
    ...
    custom_pricing_rule_ids: List[str] = Field(default_factory=list, description="Custom pricing rule IDs")
    notification_addresses_on_invoice: List[str] = Field(default_factory=list, description="Invoice notification addresses")
...
class Source(BaseReveniumModel):
    ...
    sourceType: Optional[str] = Field(None, description="Source type classification (UNKNOWN, etc.)")
...
class AlertFilter(BaseReveniumModel):
    ...
    def validate_field(cls, v):
        ...
        allowed_fields = [
            'organization', 'credential', 'product', 'model',
            'provider', 'agent', 'subscriber', 'cost', 'usage'
        ]
...
class AdvancedAlertConfiguration(BaseReveniumModel):
    ...
    trigger_duration: Optional[TriggerDuration] = Field(None, description="Trigger persistence duration")
    ...
    def to_api_format(self) -> Dict[str, Any]:
        ...
        if self.slack_configs:
            api_data["slackConfigurations"] = [
                config.webhook_url for config in self.slack_configs
            ]
        else:
            api_data["slackConfigurations"] = []
        ...
        if self.webhook_configs:
            api_data["webhook_enabled"] = [
                config.url for config in self.webhook_configs
            ]
        else:
            api_data["webhook_enabled"] = []
...
class DetectionRule(BaseReveniumModel):
    ...
    rule_type: str = Field(..., description="Type of detection rule (e.g., 'threshold', 'pattern', 'statistical')")
    ...
    time_window: Optional[str] = Field(None, description="Time window for evaluation (e.g., '5m', '1h', '1d')")
    aggregation: Optional[str] = Field(None, description="Aggregation method (e.g., 'avg', 'sum', 'max', 'min')")
    conditions: Optional[List[str]] = Field(default_factory=list, description="Additional conditions")
...
class ThresholdViolation(BaseReveniumModel):
    ...
    actual_value: Union[float, int, str] = Field(..., description="Actual value that triggered violation")
...
class AIAnomalyRequest(BaseReveniumModel):
    ...
    operatorType: str = Field(..., description="The operator type used to make the comparison between the threshold value")
    threshold: Union[float, int] = Field(..., description="The number threshold that triggers the alert")
    ...
    periodDuration: str = Field(..., description="Value that determines when the frequency of checks made to this alert monitor")
    enabled: bool = Field(..., description="Disables the monitor without deleting it from db")
    filters: List[Dict[str, Any]] = Field(..., description="The filters applied to limit the anomalies scope")
    webhookConfigurations: List[str] = Field(..., description="Webhook configurations for the anomaly to dispatch to")

    # Optional fields per API specification
    isPercentage: Optional[bool] = Field(None, description="If the threshold represents a threshold")
    description: Optional[str] = Field(None, description="The description of the alert monitor")
    groupBy: Optional[str] = Field(None, description="The group by dimension used to group the anomalies")
    triggerAfterPersistsDuration: Optional[str] = Field(None, description="The duration after which the anomaly is triggered")
...
class AIAnomaly(BaseReveniumModel):
    ...
    operatorType: str = Field(..., description="The operator type used to make the comparison between the threshold value")
    threshold: Union[float, int] = Field(..., description="The number threshold that triggers the alert")
    notificationAddresses: List[str] = Field(default_factory=list, description="The email addresses of the alert")
    slackConfigurations: List[str] = Field(default_factory=list, description="The slack addresses of the alert")
    periodDuration: str = Field(..., description="Value that determines when the frequency of checks made to this alert monitor")
    enabled: bool = Field(True, description="Disables the monitor without deleting it from db")
    filters: List[Dict[str, Any]] = Field(default_factory=list, description="The filters applied to limit the anomalies scope")
    webhookConfigurations: List[str] = Field(default_factory=list, description="Webhook configurations for the anomaly to dispatch to")
...
class AIAnomalyLegacy(BaseReveniumModel):
    ...
    detection_rules: List[DetectionRule] = Field(..., min_length=1, description="Detection rules for the anomaly")
    thresholds: Dict[str, Union[float, int, str]] = Field(..., description="Threshold configurations")
    conditions: Optional[List[str]] = Field(default_factory=list, description="Additional conditions")
...
class AIAlert(BaseReveniumModel):
    ...
    threshold_violations: List[ThresholdViolation] = Field(default_factory=list, description="Threshold violations")
...
class FilterParams(BaseReveniumModel):
    ...
    logic: Literal["AND", "OR"] = Field("AND", description="Logic operator for combining conditions")
    ...
    created_after: Optional[datetime] = Field(None, description="Quick filter by creation date (after)")
    created_before: Optional[datetime] = Field(None, description="Quick filter by creation date (before)")
            ...
            for condition in self.conditions:
                if condition.operator == FilterOperator.EQUALS:
                    ...
                    if condition.values and len(condition.values) == 2:
                        filter_expressions.append(f"{condition.field}><:[{condition.values[0]},{condition.values[1]}]")
...
class PaginationMetadata(BaseReveniumModel):
    ...
    @classmethod
    def from_response(cls, page: int, size: int, total: int, items_count: int,
                     next_cursor: Optional[str] = None, prev_cursor: Optional[str] = None) -> "PaginationMetadata":
...
class PaginatedResponse(BaseReveniumModel):
    ...
    sort_applied: Optional[List[SortField]] = Field(default_factory=list, description="Sort configuration applied")

    @classmethod
    def create(cls, items: List[Dict[str, Any]], page: int, size: int, total: int,
               filters: Optional[FilterParams] = None, sort: Optional[List[SortField]] = None,
               next_cursor: Optional[str] = None, prev_cursor: Optional[str] = None) -> "PaginatedResponse":
...
__all__ = [
    ...
    "ProductStatus", "PlanType", "Currency", "BillingPeriod", "TrialPeriod",
    "AggregationType", "RatingAggregationType", "SetupFee", "Element",
    "RatingAggregation", "Tier", "Plan", "Product",
    # Subscription models
    ...
    # Customer models
    "UserStatus", "UserRole", "User", "SubscriberStatus", "Subscriber",
    "OrganizationType", "OrganizationStatus", "Organization",
    "TeamStatus", "TeamRole", "TeamMember", "Team",
    "CustomerAnalytics", "CustomerRelationship",
    # Alert models
