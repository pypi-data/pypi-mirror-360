...
from ..alerts import AnomalyManager, AlertManager, AlertSemanticProcessor, AnalyticsEngine
...
from ..common.ucm_config import log_ucm_status, get_integration_status_text
...
class AlertManagement(ToolBase, SlackPromptingMixin):
    ...
    tool_description = "AI spending alerts and anomaly monitoring with persistence-based triggering. Key actions: create_threshold_alert, create_cumulative_usage_alert, list, get, update, delete. Supports triggerAfterPersistsDuration for persistence-based alerting. Use get_examples(alert_type='update') for Slack setup examples. Use get_capabilities() for complete action list."
            ...
            if action == "list":
                ...
                raise ToolError(
                    ...
                    examples={
                        ...
                        "creation_actions": ["create_threshold_alert", "create_cumulative_usage_alert", "create_from_text"],
                        "management_actions": ["list", "get", "update", "delete", "clear_all"],
                        "analysis_actions": ["get_metrics", "query", "validate"],
                        "bulk_operations": ["enable_all", "disable_all", "enable_multiple", "disable_multiple"],
    ...
    async def _get_tool_capabilities(self) -> List[ToolCapability]:
        ...
        if self.ucm_helper:
            try:
                ...
                ucm_trigger_durations = ucm_capabilities.get("trigger_durations") if ucm_capabilities else None
            except Exception:
        ...
        return [
            ...
            ToolCapability(
                ...
                parameters={
                    ...
                    "create_from_text": {"text": "str (required) - Natural language description including metric type"},
                ...
                limitations=[
                    ...
                    "Valid metrics: TOTAL_COST, TOKEN_COUNT, INPUT_TOKEN_COUNT, OUTPUT_TOKEN_COUNT, TOKENS_PER_MINUTE, REQUESTS_PER_MINUTE, ERROR_RATE, ERROR_COUNT, COST_PER_TRANSACTION"
                ]
            )
        ]
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "list", "get", "create", "update", "delete", "clear_all",
            "get_metrics", "query", "validate", "get_capabilities", "get_examples",
            "get_agent_summary", "create_cumulative_usage_alert", "create_threshold_alert",
            "create_from_text", "create_simple", "enable", "disable", "enable_multiple",
            "disable_multiple", "enable_all", "disable_all", "toggle_status", "get_status"
        ]
...
• Integration with products, sources, and teams"""
    ...
    def _detect_semantic_intent(self, arguments: Dict[str, Any]) -> str:
        ...
        history_keywords = [
            "history", "historical", "fired", "triggered", "events", "incidents",
            "recent", "last", "past", "occurred", "happened", "when", "timeline",
            "resolved", "active", "ongoing", "duration", "frequency"
        ]

        # Keywords that suggest alert definitions (rules)
        definition_keywords = [
            "rules", "definitions", "configuration", "config", "setup", "create",
            "define", "configure", "manage", "enable", "disable", "threshold",
            "budget", "quota", "monitoring", "watch", "track"
        ]
    ...
    async def _handle_list(
        ...
        if resource_type == "anomalies":
            ...
            try:
                ...
                validation_result = validate_pagination_with_performance(page, size, "Alert Management")

                logger.info(f"Calling alert_manager.list_alerts with page={page}, size={size} (tier: {validation_result['performance_tier']})")
    ...
    async def _handle_create(
        ...
        if resource_type == "anomalies":
            ...
            if dry_run:
                ...
                if not anomaly_data:
                    return [TextContent(type="text", text="❌ **Dry Run Validation Failed**: Missing anomaly_data parameter")]
                ...
                return [TextContent(type="text", text=f"🧪 **DRY RUN MODE**\n\n{json.dumps(validation_result, indent=2)}")]
        ...
        if resource_type == "anomalies":
            ...
            if dry_run:
                ...
                if not anomaly_id or not anomaly_data:
                    return [TextContent(type="text", text="❌ **Dry Run Validation Failed**: Missing anomaly_id or anomaly_data parameter")]
        ...
        if action == "list":
            ...
            if not anomaly_data:
                error = create_structured_missing_parameter_error(
                    ...
                    action="create",
                    examples={
                        "usage": "create(anomaly_data={\"name\": \"High Usage Alert\", \"metricType\": \"TOTAL_COST\", \"threshold\": 100})",
            ...
            if not anomaly_data:
                error = create_structured_missing_parameter_error(
                    ...
                    examples={
                        ...
                        "updatable_fields": ["name", "threshold", "alertType", "enabled", "slackConfigurations", "notificationAddresses"],
                        "slack_config_example": {"slackConfigurations": ["5jpABv"]},
                        "email_example": {"notificationAddresses": ["user@example.com"]},
                        "combined_example": {"threshold": 200, "enabled": True, "slackConfigurations": ["5jpABv"]},
                        "note": "Use get_examples action for comprehensive update patterns"
                    }
                )
                return [TextContent(type="text", text=format_structured_error(error))]
            return await self.anomaly_manager.update_anomaly(client, anomaly_id, anomaly_data)
            ...
            if not confirm:
                return [TextContent(
                    type="text",
                    text="⚠️ **Confirmation Required**: This will delete ALL anomalies\n\n"
...
## **Semantic Guidance for Agents**

**When users say...**                    **They probably want...**
...
- Performance alerts: `create_threshold_alert(...)`"""
        ...
        # Add UCM-enhanced metrics if available
        if ucm_capabilities and "metrics" in ucm_capabilities:
            text += "\n\n## **Available Metrics (API-Verified)**\n"
            metrics = ucm_capabilities["metrics"]
...
## **Available Metrics**
- **TOTAL_COST** - Total spending amount
- **COST_PER_TRANSACTION** - Average cost per API call
- **TOKEN_COUNT** - Total tokens used
- **INPUT_TOKEN_COUNT** - Input tokens only
- **OUTPUT_TOKEN_COUNT** - Output tokens only
...
- **ERROR_COUNT** - Total number of errors"""
        ...
        if ucm_capabilities and "operators" in ucm_capabilities:
            text += "\n\n## **Available Operators (API-Verified)**\n"
...
- **Long-term Budget Overruns**: Only alert if monthly budget exceeded for 3 consecutive days"""
        ...
        if ucm_capabilities and "trigger_durations" in ucm_capabilities:
            text += "\n\n### **Valid Duration Values (API-Verified)**\n"
            ...
            if isinstance(trigger_durations, list):
                ...
                short_term = [d for d in trigger_durations if d in ["ONE_MINUTE", "FIVE_MINUTES", "FIFTEEN_MINUTES", "THIRTY_MINUTES"]]
                medium_term = [d for d in trigger_durations if d in ["ONE_HOUR", "TWELVE_HOURS", "TWENTY_FOUR_HOURS"]]
                long_term = [d for d in trigger_durations if d in ["SEVEN_DAYS", "THIRTY_DAYS"]]
                periodic = [d for d in trigger_durations if d in ["DAILY", "WEEKLY", "MONTHLY", "QUARTERLY"]]
...
**Pro Tip**: When in doubt, use `resource_type="alerts"` for history and `resource_type="anomalies"` for configuration!"""
        ...
        # Add UCM integration status for debugging
        text += "\n\n## **Integration Status**\n"
        text += get_integration_status_text(self.ucm_helper is not None, ucm_capabilities)

        return text

    async def _handle_test_ucm_integration(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if self.ucm_helper:
            ...
            try:
                ...
                if ucm_capabilities:
                    ...
                    result_text += f"**Alert Types**: {len(ucm_capabilities.get('alert_types', []))} types\n"
                    result_text += f"**Metrics**: {len(ucm_capabilities.get('metrics', {}))} categories\n"
                    result_text += f"**Operators**: {len(ucm_capabilities.get('operators', []))} operators\n"

                    # Show sample metrics
                    metrics = ucm_capabilities.get('metrics', {})
                    if metrics:
                        result_text += "\n**Sample UCM Metrics**:\n"
                        for category, metric_list in list(metrics.items())[:2]:  # Show first 2 categories
    ...
    async def _handle_get_examples(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
...
These examples show how to update existing alerts with new configurations, including adding Slack notifications.
...
- ✅ Basic alerts without advanced features
- ✅ Simple threshold or budget monitoring
- ✅ Quick setup with minimal configuration
- ✅ Threshold alerts with persistence (create_threshold_alert supports triggerAfterPersistsDuration)

### **Use create() Action For:**
- ✅ Budget alerts with persistence (create_cumulative_usage_alert doesn't support triggerAfterPersistsDuration)
- ✅ Advanced filtering by provider, model, customer
...
# Threshold alerts with persistence (NEW: triggerAfterPersistsDuration support)
...
**Integration**: Works seamlessly with products, sources, and team management for comprehensive monitoring."""
    ...
    def _resolve_notification_config(self, provided_email: Optional[str] = None,
                                   slack_config_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _post_alert_creation_prompting(self, alert_data: Dict[str, Any],  # noqa: ARG002
    ...
    async def _handle_create_cumulative_usage_alert(
        ...
        try:
            ...
            try:
                notification_config = self._resolve_notification_config(provided_email, slack_config_id)
            ...
            if not name:
                error = create_structured_missing_parameter_error(
                    ...
                    examples={
                        ...
                        "example_names": ["Monthly Usage Alert", "Cost Threshold Alert", "Token Limit Alert"]
                    }
                )
                return [TextContent(type="text", text=format_structured_error(error))]
            if not threshold:
        ...
        try:
            ...
            if not name:
                raise create_structured_missing_parameter_error(
                    ...
                    examples={
                        ...
                        "example_names": ["High API Usage", "Error Rate Alert", "Token Limit Warning"]
                    }
                )
            if not threshold:
                raise create_structured_missing_parameter_error(
            ...
            if trigger_after_persists_duration:
                ...
                valid_durations = [
                    ...
                    "ONE_HOUR", "TWELVE_HOURS", "TWENTY_FOUR_HOURS",
                ...
                if trigger_after_persists_duration not in valid_durations:
                    raise create_structured_validation_error(
                        ...
                        examples={
                            "short_term": ["ONE_MINUTE", "FIVE_MINUTES", "FIFTEEN_MINUTES", "THIRTY_MINUTES"],
            ...
            raise ToolError(
                ...
                examples={
                    ...
                    "troubleshooting": ["Check API credentials", "Verify alert parameters", "Test with basic alert"],
    ...
    async def _handle_create_from_text(
        ...
        try:
            ...
            raise ToolError(
                ...
                examples={
                    "alternative_methods": ["create_threshold_alert()", "create_cumulative_usage_alert()"],
                    "better_descriptions": [
    ...
    def _simple_parse_alert_text(self, text: str) -> Dict[str, Any]:
        ...
        cumulative_keywords = ["monthly", "weekly", "daily", "budget", "quota", "limit", "total", "cumulative", "period"]
        threshold_keywords = ["spike", "exceeds", "above", "real-time", "immediate", "instant", "per minute", "per hour"]
        ...
        name = f"Auto-generated Alert: {text[:50]}..." if len(text) > 50 else f"Auto-generated Alert: {text}"
    ...
    async def _handle_enable_anomaly(
        ...
        try:
            ...
            raise ToolError(
                ...
                examples={
                    "verification_steps": ["list()", "get_anomaly_status()", "enable(anomaly_id='valid_id')"],
                    "troubleshooting": ["Check anomaly ID spelling", "Verify alert exists", "Test API connectivity"],
    ...
    async def _handle_disable_anomaly(
        ...
        try:
            ...
            raise ToolError(
                ...
                examples={
                    "verification_steps": ["list()", "get_anomaly_status()", "disable(anomaly_id='valid_id')"],
                    "troubleshooting": ["Check anomaly ID spelling", "Verify alert exists", "Test API connectivity"],
        ...
        if not anomaly_ids:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "example_lists": [["alert-123", "alert-456"], ["threshold-789", "error-rate-101"]],
    ...
    async def _handle_enable_all_anomalies(
        ...
        if not confirm:
            return [TextContent(
                type="text",
                text="⚠️ **Confirmation Required**: This will enable ALL alerts in your system\n\n"
        ...
        try:
            ...
            result_text = (
                f"⚠️ **Enable All Alerts - Complete**\n\n"
    ...
    async def _handle_disable_all_anomalies(
        ...
        if not confirm:
            return [TextContent(
                type="text",
                text="⚠️ **Confirmation Required**: This will disable ALL alerts in your system\n\n"
        ...
        try:
            ...
            result_text = (
                f"⚠️ **Disable All Alerts - Complete**\n\n"
                f"**Total Alerts:** {len(anomalies)}\n"
            ...
            raise ToolError(
                ...
                examples={
                    ...
                    "verification_steps": ["list()", "disable_multiple(anomaly_ids=['id1', 'id2'])"],
    ...
    async def _handle_toggle_anomaly_status(
        ...
        try:
            ...
            raise ToolError(
                ...
                examples={
                    "alternative_methods": ["enable(anomaly_id='alert-123')", "disable(anomaly_id='alert-123')"],
                    "verification_steps": ["get_anomaly_status()", "list()", "toggle_status(anomaly_id='valid_id')"],
        ...
        try:
            ...
            raise ToolError(
                ...
                examples={
                    ...
                    "troubleshooting": ["Test API connectivity", "Verify authentication", "Check service status"],
    ...
    async def _get_input_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "required": ["action"],  # Context7: User-centric - only action required, other fields depend on action
