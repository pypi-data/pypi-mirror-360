...
try:
    from ..prometheus_metrics import record_cache_operation, update_cache_stats
    PROMETHEUS_METRICS_AVAILABLE = True
except ImportError:
    logger.debug("Prometheus metrics not available for cache monitoring")
    PROMETHEUS_METRICS_AVAILABLE = False

# Import Redis-like cache for enhanced performance
try:
    from .redis_like_cache import redis_like_cache, RedisLikeCache
...
if not DISKCACHE_AVAILABLE:
    class MockCache:
        ...
        def set(self, key, value, expire=None): self._data[key] = value; return True
        def delete(self, key): return self._data.pop(key, None) is not None
        def clear(self): self._data.clear(); return len(self._data)
        def memoize(self, expire=None): return lambda f: f
...
class ResponseCacheManager:
    ...
    async def get_cached_response(
        ...
        if REDIS_LIKE_CACHE_AVAILABLE and redis_like_cache and cache_level in ("auto", "redis"):
            ...
            if redis_result is not None:
                if PROMETHEUS_METRICS_AVAILABLE:
                    record_cache_operation(cache_type, "get", "hit", "redis")
        ...
        try:
            ...
            if cache_level in ("auto", "l1") and cache_key in self.l1_cache:
                ...
                if PROMETHEUS_METRICS_AVAILABLE:
                    record_cache_operation(cache_type, "get", "hit", "l1")
            ...
            if cache_level == "l1":
                ...
                if PROMETHEUS_METRICS_AVAILABLE:
                    record_cache_operation(cache_type, "get", "miss", "l1")
                return None
            ...
            if cache_level in ("auto", "l2"):
                ...
                if result is not None:
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "hit", "l2")
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "miss", "l2")
                    return None
            ...
            if cache_level in ("auto", "l3"):
                ...
                if result is not None:
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "hit", "l3")
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "miss", "l3")
                    return None
            ...
            if cache_level in ("auto", "l4"):
                ...
                if result is not None:
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "hit", "l4")
                    ...
                    if PROMETHEUS_METRICS_AVAILABLE:
                        record_cache_operation(cache_type, "get", "miss", "l4")
    ...
    async def set_cached_response(
        ...
        try:
            ...
            if REDIS_LIKE_CACHE_AVAILABLE and redis_like_cache:
                ...
                if PROMETHEUS_METRICS_AVAILABLE:
                    record_cache_operation(cache_type, "set", "success", "redis")

            # Store in L1 (immediate access)
            self.l1_cache[cache_key] = value
            if PROMETHEUS_METRICS_AVAILABLE:
                record_cache_operation(cache_type, "set", "success", "l1")
    ...
    async def _async_cache_set(self, cache: Any, key: str, value: Any, expire: Optional[int] = None) -> bool:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        if PROMETHEUS_METRICS_AVAILABLE:
            # Update hit rates for each cache level
            for level in ["l1", "l2", "l3", "l4"]:
                ...
                for cache_type in ["api", "validation", "ucm"]:
                    update_cache_stats(cache_type, level, level_hit_rate, 0)  # Size tracking would need more work
    ...
    async def warm_frequently_accessed_data(self) -> Dict[str, int]:
        ...
        async def load_ucm_data():
            return {
                "products_capabilities": {"currencies": ["USD", "EUR", "GBP"], "plan_types": ["BASIC", "PREMIUM"]},
                "alerts_capabilities": {"metrics": ["TOTAL_COST", "REQUEST_COUNT"], "periods": ["daily", "weekly"]},
                "subscriptions_capabilities": {"billing_periods": ["MONTHLY", "YEARLY"], "statuses": ["ACTIVE", "CANCELLED"]}
            }
        ...
        logger.info(f"Cache warming completed: {total_warmed} total items across {len(results)} cache types")
...
def cache_response(cache_type: str, ttl_seconds: Optional[int] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            await response_cache.set_cached_response(cache_type, cache_key_data, result, ttl_seconds)
            return result
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                loop.run_until_complete(
                    response_cache.set_cached_response(cache_type, cache_key_data, result, ttl_seconds)
                )
            except RuntimeError:
                # No event loop running, skip caching
                pass
