...
class NLPBusinessProcessor:
    ...
    async def process_natural_language_query(
        ...
        try:
            ...
            query_complexity, is_follow_up = self._analyze_query_complexity(normalized_text, session)
            processing_notes.append(f"Query complexity: {query_complexity}, Follow-up: {is_follow_up}")
            ...
            intent, intent_confidence = self._extract_intent_with_context(normalized_text, session, context_references)
            processing_notes.append(f"Detected intent: {intent.value} (confidence: {intent_confidence:.2f})")

            # Enhanced entity extraction with context resolution
            entities = self._extract_entities_with_context(normalized_text, session, context_references)
            ...
            time_frame, time_context = self._extract_time_frame_with_context(normalized_text, session, context_references)
            ...
            transaction_complexity = self._analyze_transaction_level_complexity(normalized_text, entities)
            processing_notes.append(f"Transaction complexity: {transaction_complexity['complexity_score']:.2f}")

            # Build structured query
            structured_query = self._build_structured_query(
                intent, entities, time_frame, aggregation, time_context, numerical_quantities, transaction_complexity
            )
            ...
            suggested_follow_ups = self._generate_follow_up_suggestions(intent, entities, time_frame)
            ...
            logger.info(f"Enhanced NLP processing complete. Intent: {intent.value}, Complexity: {query_complexity}, Confidence: {overall_confidence:.2f}")
            return result
    ...
    def _extract_numerical_quantities(self, text: str) -> List[ExtractedEntity]:
        ...
        try:
            ...
            for quantity in quantities:
                ...
                if key not in unique_quantities or quantity.confidence > unique_quantities[key].confidence:
    ...
    def _get_or_create_session(self, session_id: Optional[str] = None) -> QuerySession:
        ...
        try:
            if session_id and session_id in self.sessions:
                ...
                if (datetime.now(timezone.utc) - session.updated_at).total_seconds() < self.session_timeout_hours * 3600:
                    return session
                else:
    ...
    def _update_session(self, session: QuerySession, result: NLPQueryResult):
        ...
        try:
            ...
            for entity in result.entities:
                ...
                existing_values = [e.entity_value.lower() for e in session.entities_mentioned[entity_type]]
    ...
    def _analyze_query_complexity(self, text: str, session: QuerySession) -> Tuple[str, bool]:  # noqa: ARG002
    ...
    def _analyze_transaction_level_complexity(self, text: str, entities: List[ExtractedEntity]) -> Dict[str, Any]:
    ...
    def _extract_context_references(self, text: str, session: QuerySession) -> List[ContextReference]:
    ...
    def _extract_query_dimensions(self, text: str) -> List[QueryDimension]:
        ...
        for i, part in enumerate(parts):
            if len(part.strip()) > 5 and not re.match(r'\b(and|also|plus|additionally)\b', part.strip(), re.IGNORECASE):
                ...
                if intent != QueryIntent.UNKNOWN:
                    dimensions.append(QueryDimension(
                        intent=intent,
                        entities=entities,
                        time_frame=time_frame,
                        aggregation=aggregation,
    ...
    def _build_intent_patterns(self) -> Dict[QueryIntent, List[Tuple[str, float]]]:
        ...
        return {
            ...
            QueryIntent.BREAKDOWN: [
                ...
                (r'\bshow\s+me\s+cost\s+breakdown\b', 1.0),  # "show me cost breakdown" gets highest priority
                ...
                (r'\bshow\s+me\s+(the\s+)?(top\s+\d+\s+)?(products?|customers?|models?|providers?|agents?)', 0.8),
                (r'\blist\s+(the\s+)?(top\s+\d+\s+)?(products?|customers?|models?|providers?|agents?)', 0.8),
                (r'\bget\s+(the\s+)?(top\s+\d+\s+)?(products?|customers?|models?|providers?|agents?)', 0.7),
                (r'\b(products?|customers?|models?|providers?|agents?)\s+by\s+(cost|revenue|usage)', 0.8),
                ...
                (r'\bshow\s+me\s+(the\s+)?top\s+agents?\b', 1.0),  # "show me top agents" gets highest priority
                ...
                (r'\btop\s+providers?\b.*\bcost\b', 1.0),  # "top providers by cost" gets highest priority
                (r'\bshow\s+me\s+(the\s+)?top\s+providers?\b', 1.0),  # "show me top providers" gets highest priority
            ...
            QueryIntent.TRANSACTION_LEVEL: [
                ...
                (r'\bcost\s+per\s+\w+\s+transaction\b', 1.0),  # Matches "cost per [any word] transaction"
                ...
                (r'\bshow\s+me\s+agent\s+efficiency\b', 1.0),  # High priority for specific efficiency queries
                (r'\bshow\s+me\s+agent\s+metrics\b', 1.0),  # High priority for specific metrics queries
    ...
    def _build_time_patterns(self) -> Dict[TimeFrame, List[str]]:
        ...
        return {
            TimeFrame.YESTERDAY: [
                r'\byesterday\b', r'\blast\s+day\b', r'\btoday\b',
                r'\bfor\s+the\s+last\s+day\b', r'\bin\s+the\s+last\s+day\b',
                r'\blast\s+24\s+hours?\b', r'\bpast\s+24\s+hours?\b',
                r'\blast\s+twenty.?four\s+hours?\b',
                r'\bfor\s+the\s+last\s+24\s+hours?\b', r'\bin\s+the\s+last\s+24\s+hours?\b',
                r'\bfor\s+the\s+last\s+twenty.?four\s+hours?\b'
            ],
            TimeFrame.LAST_WEEK: [
                r'\blast\s+week\b', r'\bthis\s+week\b', r'\bpast\s+week\b',
                r'\blast\s+7\s+days?\b', r'\bpast\s+7\s+days?\b',
                r'\blast\s+seven\s+days?\b', r'\bpast\s+seven\s+days?\b'
            ],
            TimeFrame.LAST_THIRTY_DAYS: [
                r'\blast\s+30\s+days?\b', r'\bpast\s+30\s+days?\b',
                r'\blast\s+thirty\s+days?\b', r'\bpast\s+thirty\s+days?\b'
            ],
            TimeFrame.LAST_MONTH: [
                r'\blast\s+month\b', r'\bthis\s+month\b', r'\bpast\s+month\b'
            ],
            TimeFrame.LAST_THREE_MONTHS: [
                r'\blast\s+3\s+months?\b', r'\bpast\s+three\s+months?\b', r'\bquarter\b',
                r'\blast\s+three\s+months?\b'
            ],
            TimeFrame.LAST_SIX_MONTHS: [
                r'\blast\s+6\s+months?\b', r'\bpast\s+six\s+months?\b', r'\bhalf\s+year\b',
                r'\blast\s+six\s+months?\b'
            ],
            TimeFrame.LAST_YEAR: [
                r'\blast\s+year\b', r'\bthis\s+year\b', r'\bpast\s+year\b', r'\b12\s+months?\b',
                r'\blast\s+twelve\s+months?\b', r'\bpast\s+twelve\s+months?\b'
            ],
            TimeFrame.LAST_HOUR: [
                r'\blast\s+hour\b', r'\bthis\s+hour\b', r'\bpast\s+hour\b'
            ],
            TimeFrame.LAST_EIGHT_HOURS: [
                r'\blast\s+8\s+hours?\b', r'\bpast\s+eight\s+hours?\b',
    ...
    def _build_aggregation_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            "TOTAL": [
                r'\btotal\b', r'\bsum\b', r'\ball\b', r'\bcombined\b', r'\baggregate\b',
                r'\boverall\b', r'\bcumulative\b', r'\bentire\b'
            ],
            "MEAN": [
                r'\baverage\b', r'\bmean\b', r'\btypical\b', r'\bstandard\b',
                r'\bnormal\b', r'\bregular\b', r'\busual\b', r'\bavg\b'
            ],
            "MAXIMUM": [
                r'\bmax\b', r'\bmaximum\b', r'\bhighest\b', r'\bpeak\b', r'\btop\b',
                r'\bgreatest\b', r'\blargest\b', r'\bbest\b', r'\bmost\b',
                r'\bwith\s+max\b', r'\bwith\s+maximum\b', r'\bwith\s+highest\b'
            ],
            "MINIMUM": [
                r'\bmin\b', r'\bminimum\b', r'\blowest\b', r'\bbottom\b', r'\bsmallest\b',
                r'\bleast\b', r'\bworst\b', r'\bwith\s+min\b', r'\bwith\s+minimum\b',
                r'\bwith\s+lowest\b'
            ],
            "MEDIAN": [
                r'\bmedian\b', r'\bmiddle\b', r'\bmid\b', r'\bmidpoint\b',
                r'\bwith\s+median\b', r'\busing\s+median\b'
            ]
        }
