...
from .shared_parameters import (
    MeteringTransactionBuilder, AnalyticsQueryBuilder,
    MeteringTransaction, AnalyticsTimeRange, AnalyticsFilters
)
...
class AnalyticsRegistry(BaseToolRegistry):
    ...
    registry_description: ClassVar[str] = "Sophisticated analytics tools with Builder Pattern for complex parameter handling"
    ...
    async def _handle_registry_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _route_analytics_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        allowed_metering_actions = [
            ...
            "estimate_transaction_cost", "get_api_endpoints", "get_authentication_details",
            "get_response_formats", "get_integration_config", "get_rate_limits", 
            "get_integration_guide", "validate", "get_capabilities", "get_examples", "get_agent_summary"
        ]
        
        allowed_metering_elements_actions = [
            "list", "get", "create", "update", "delete", "get_templates",
            "create_from_template", "assign_to_source", "get_capabilities", "get_examples"
        ]
        
        allowed_business_analytics_actions = [
            "get_provider_costs", "get_model_costs", "get_customer_costs",
            "get_cost_summary", "get_capabilities", "get_examples"
        ]
        
        # Business analytics actions
        if action in allowed_business_analytics_actions and action in ["get_provider_costs", "get_model_costs", "get_customer_costs",
                     "get_cost_summary"]:
        ...
        elif action in allowed_metering_actions and action in ["submit_ai_transaction", "verify_transactions", "get_transaction_status",
                       "get_transaction_by_id", "generate_test_transactions", "list_ai_models"]:
        ...
        elif action in allowed_metering_elements_actions and action in ["list", "get", "create", "update", "delete", "get_templates",
                       "create_from_template", "assign_to_source"]:
            return await self._handle_metering_elements_action(action, arguments)
        
        else:
            return await self._handle_security_violation("unsupported_action", action, tool_name)
    
    async def _handle_business_analytics_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if action in ["get_provider_costs", "get_model_costs", "get_customer_costs", "get_cost_summary"]:
            query_params = builder.with_time_range(period).with_grouping(group).build_provider_costs_query()
        else:
            query_params = arguments
        
        # Execute via standardized tool execution
        return await self._standardized_tool_execution("business_analytics_management", action, query_params)
    
    async def _handle_metering_builder_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        excluded_fields = {'subscriber_email', 'subscriber_id', 'subscriber_credential_name', 
                          'subscriber_credential', 'task_id', 'task_type', 'agent', 'description'}
        transaction_args = {k: v for k, v in transaction.__dict__.items() 
                           if k not in excluded_fields and v is not None}
        return await self._standardized_tool_execution("manage_metering", "submit_ai_transaction", transaction_args)
    ...
    def _validate_parameter_security(self, arguments: Dict[str, Any]):
        ...
        elif len(model) > 100 or any(char in model for char in ['<', '>', '&', '"', "'", '/', '\\']):
        ...
        elif provider.upper() not in ["OPENAI", "ANTHROPIC", "GOOGLE", "AZURE", "COHERE", "MISTRAL", "TOGETHER", "GROQ"]:
            errors.append(f"provider: Invalid provider '{provider}'. Must be one of: OPENAI, ANTHROPIC, GOOGLE, AZURE, COHERE, MISTRAL, TOGETHER, GROQ")
            
        # Validate numeric parameters
        for param in ["input_tokens", "output_tokens", "duration_ms"]:
            value = arguments.get(param)
            if not isinstance(value, (int, float)) or value < 0:
                errors.append(f"{param}: Must be non-negative number, got {type(value).__name__}: {value}")
        ...
        for param in ["trace_id", "task_id", "task_type", "agent", "description", "organization_id", "subscription_id", "product_id"]:
            value = arguments.get(param)
            if value is not None:
                if not isinstance(value, str):
                    errors.append(f"{param}: Must be string if provided, got {type(value).__name__}")
                elif len(value) > 500 or any(char in value for char in ['<', '>', '&']):
                    errors.append(f"{param}: Contains invalid characters or exceeds 500 character limit")
    ...
    def _add_optional_context(self, builder: MeteringTransactionBuilder, arguments: Dict[str, Any]) -> MeteringTransactionBuilder:
    ...
    async def _handle_metering_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_metering_elements_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Handle metering elements actions."""
        return await self._standardized_tool_execution("manage_metering_elements", action, arguments)
    
    async def _handle_security_violation(self, violation_type: str, action: str, tool_name: str = None) -> List[TextContent]:
...
"""
    ...
    async def handle_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        """Handle analytics actions with sophisticated routing."""
        # Registry-level actions
        if action in ["get_capabilities", "get_examples"]:
            return await self._handle_registry_action(action, arguments)
