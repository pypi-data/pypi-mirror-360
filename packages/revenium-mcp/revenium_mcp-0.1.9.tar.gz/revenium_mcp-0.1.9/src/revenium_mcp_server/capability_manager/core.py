...
class UnifiedCapabilityManager:
    ...
    def __init__(self, client: ReveniumClient, cache_ttl: int = 3600):
        ...
        self.supported_resource_types = {
            "system", "products", "subscriptions", "customers", "alerts",
            "sources", "metering_elements", "metering"
        }
    ...
    async def verify_capability(self, resource_type: str, capability: str) -> bool:
        ...
        try:
            result = await self.verifier.verify_single_capability(
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_breakers_open": len([
                k for k, v in self._verification_failures.items() 
                if v >= self._max_failures
            ]),
    ...
    async def set_capability(self, resource_type: str, capability_name: str, value: str) -> None:
        ...
        async with self._lock:
            try:
                ...
                logger.info(f"Successfully set capability {capability_name}={value} for {resource_type}")
                ...
                logger.error(f"Failed to set capability {capability_name}={value} for {resource_type}: {e}")
                raise
