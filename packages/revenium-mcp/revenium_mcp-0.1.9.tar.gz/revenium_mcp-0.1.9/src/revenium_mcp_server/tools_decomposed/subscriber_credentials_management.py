...
from ..nlp.credential_nlp_processor import CredentialNLPProcessor, CredentialIntent
...
from ..hierarchy import hierarchy_navigation_service, entity_lookup_service, cross_tier_validator
...
class CredentialsHierarchyManager:
    ...
    async def get_subscription_details(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        navigation_result = await self.navigation_service.get_subscription_for_credential(credential_id)
    ...
    async def get_product_details(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        navigation_result = await self.navigation_service.get_full_hierarchy("credentials", credential_id)
...
class SubscriberCredentialsManagement(ToolBase):
    ...
    tool_description: ClassVar[str] = "Subscriber credential management for billing identity. Key actions: list, create, update, delete, validate. Use get_capabilities() for complete action list."
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            ...
            "parse_natural_language", "get_agent_summary",
            "get_business_guidance", "get_onboarding_checklist", 
            "get_troubleshooting_guide", "analyze_billing_impact",
            "get_subscription_details", "get_product_details"
        ]
    ...
    async def handle_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            if action == "list":
                ...
                # Convert dict response to TextContent for MCP compatibility
            ...
            # Handle hierarchy actions
                ...
                raise ToolError(
                    ...
                    examples={
                        "supported_actions": ["list", "get", "create", "update", "delete", "get_capabilities", "get_examples", "validate", "get_subscription_details", "get_product_details"],
                        "example_usage": {
    ...
    async def _create_credential(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not credential_data:
            ...
            individual_params = {}
            param_fields = ["label", "subscriberId", "organizationId", "externalId", "externalSecret", "name", "tags", "subscriptionIds"]
            ...
            if individual_params:
                ...
                logger.info(f"Context7: Built credential_data from individual parameters: {list(individual_params.keys())}")
        ...
        if not credential_data:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "required_fields": ["label", "subscriberId", "organizationId", "externalId", "externalSecret"],
                    "example_data": {
        ...
        if "subscriber_email" in credential_data:
            try:
                subscriber_id = await self.client.resolve_subscriber_email_to_id(credential_data["subscriber_email"])
        ...
        if "organization_name" in credential_data:
            try:
                org_id = await self.client.resolve_organization_name_to_id(credential_data["organization_name"])
        ...
        if dry_run:
            ...
            business_impact = self.business_context.get_billing_impact_explanation("create", credential_data)
        ...
        required_fields = ["label", "subscriberId", "organizationId", "externalId", "externalSecret"]
        missing_fields = [field for field in required_fields if field not in credential_data or not credential_data[field]]
        ...
        result["business_context"] = self.business_context.get_billing_impact_explanation("create", credential_data)
        ...
        if not credential_data:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "updatable_fields": ["label", "name", "externalId", "externalSecret", "tags", "subscriptionIds"],
                    "example_partial": {"label": "New Label"},
                    "example_multiple": {"label": "New Label", "externalId": "new_key_123"}
                }
            )

        logger.info(f"Updating credential: {credential_id} with partial data: {list(credential_data.keys())}")
        ...
        try:
            ...
            if "subscriptions" in current_credential and current_credential["subscriptions"]:
                ...
                if existing_subscriptions and "subscriptions" not in credential_data:
                    ...
                    subscription_ids = [sub.get("id", "unknown") for sub in existing_subscriptions if isinstance(sub, dict)]
                    logger.info(f"Preserving subscription associations: {subscription_ids}")
                else:
                    logger.info(f"Not preserving subscriptions - existing: {len(existing_subscriptions)}, subscriptions in credential_data: {'subscriptions' in credential_data}")
            ...
            fields_to_remove = [
                "id", "createdAt", "updatedAt", "version", "created", "updated",
                "team", "organization", "subscriber", "_links",
            ...
            required_fields = ["label", "name", "subscriberId", "organizationId", "externalId", "externalSecret", "teamId"]
            ...
            final_payload = {k: v for k, v in final_payload.items() if v is not None or k in preserve_fields}

            # Step 8: Call the API with complete merged data
            logger.info(f"Updating credential {credential_id} with merged payload keys: {list(final_payload.keys())}")
            ...
            result = {
                ...
                "business_context": self.business_context.get_billing_impact_explanation("update", credential_data)
            }
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "list", "get", "create", "update", "delete",
            "get_capabilities", "get_examples", "validate",
            "get_agent_summary", "parse_natural_language",
            "get_business_guidance", "get_onboarding_checklist",
            "get_troubleshooting_guide", "analyze_billing_impact",
            "get_subscription_details", "get_product_details"
        ]
        ...
        return [
            ToolCapability(
                ...
                parameters={
                    ...
                    "update": {"credential_id": "str", "credential_data": "dict", "dry_run": "bool"},
