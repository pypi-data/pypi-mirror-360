...
**💡 Quick Fix**: Replace the old fields with the subscriber object structure shown above."""
    ...
    def _process_batch_result(
        ...
        if isinstance(result, Exception):
            ...
            logger.error(f"❌ {operation_name.capitalize()} task failed for {transaction_id}: {result}")
            return {
                "transaction_id": transaction_id,
                "found": False,
                "error": str(result),
    ...
    async def _handle_retry_attempt(
        ...
        try:
            ...
            logger.info(f"🔍 Executing {operation_name} for {transaction_id} (attempt {attempt + 1}/{max_retries})")
            ...
            logger.warning(f"⚠️ Error in {operation_name} for {transaction_id} (attempt {attempt + 1}): {e}")
            return False, None, {}, e
    ...
    async def _execute_with_retry(
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                logger.info(f"⏳ {operation_name.capitalize()} failed for {transaction_id}, retrying in {retry_interval}s...")
    ...
    async def _process_session_results(self, transaction_ids: List[str]) -> Tuple[List[Dict[str, Any]], List[str]]:
        ...
        for tid in remaining_ids:
            ...
            async def search_operation():
                return await self._search_transaction_pages(
                    client, tid, params["search_page_range"],
                    params["page_size"], params["early_termination"]
                )

            success, transaction_data, search_metadata, error = await self._execute_with_retry(
                search_operation, tid, params["max_retries"],
                params["retry_interval"], "lookup"
            )
    ...
    async def lookup_transactions(self, client: ReveniumClient, arguments: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                for transaction in transactions_list:
                    ...
                    if self._transaction_ids_match(stored_transaction_id, transaction_id):
                        ...
                        logger.info(f"✅ Transaction {transaction_id} found on page {page} (stored as: {stored_transaction_id})")
...
class MeteringManagement(ToolBase):
    ...
    tool_description = "Submit AI transaction metering metadata and lookup existing AI transactions metered by Revenium. Receive guidance for writing new integrations to Revenium's AI metering API using python and typescript. Key actions: submit_ai_transaction, lookup_transactions, get_integration_guide, list_ai_models, validate. Supports Python and JavaScript integration examples. Use get_capabilities() for full guidance."
    ...
    async def handle_action(
        ...
        try:
            ...
            if action == "submit_ai_transaction":
                ...
                response_text = f"**Transaction Lookup Results**\n\n"
                response_text += f"**Summary**: {result['summary']['found_count']}/{result['summary']['total_requested']} transactions found\n"
                response_text += f"**Sources**: {result['summary']['sources']['session']} session, {result['summary']['sources']['api']} API\n\n"

                # Display individual results
                for i, transaction_result in enumerate(result['results'], 1):
                    response_text += f"**{i}. Transaction {transaction_result['transaction_id']}**\n"
                    if transaction_result['found']:
                        ...
                        if 'transaction_data' in transaction_result:
                            ...
                            response_text += f"- **Input Tokens**: {data.get('inputTokens', 'N/A')}\n"
                            response_text += f"- **Output Tokens**: {data.get('outputTokens', 'N/A')}\n"
                    else:
                        response_text += f"❌ **Not Found** (searched via {transaction_result['source']})\n"
            ...
            # Integration Support Actions
                ...
                raise ToolError(
                    ...
                    examples={
                        "transaction_actions": ["submit_ai_transaction", "lookup_transactions", "get_transaction_status"],
...
lookup_transactions(transaction_ids=["tx_abc", "tx_def"]) # Efficient batch processing
...
4. Review examples with `get_examples()`"""
            ...
            if "_embedded" in response and "aIModelResourceList" in response["_embedded"]:
                ...
                for model in all_models:
                    ...
                    if (query_lower in model_name or
                        query_lower in provider or
                        model_name.startswith(query_lower) or
                        provider.startswith(query_lower)):
    ...
    async def _handle_get_agent_summary(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        return [TextContent(
            ...
            text="# **Agent-Friendly Quick Start Guide**\n\n"
                 ...
                 "**Problem**: `lookup_transactions()` shows not found\n"
                 "**Solutions**:\n"
                 "• Increase retry attempts: `lookup_transactions(transaction_ids=[\"tx_123\"], max_retries=5)`\n"
                 "• Expand search range: `lookup_transactions(transaction_ids=[\"tx_123\"], search_page_range=100)`\n"
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "submit_ai_transaction", "lookup_transactions", "get_transaction_status",
    ...
    async def _get_input_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "wait_seconds": {
                    "type": "integer",
                    "description": "Seconds to wait for transaction processing (default: 30) - used with lookup_transactions"
                },
                "max_retries": {
                    "type": "integer",
                ...
                "search_page_range": {
                    "type": ["integer", "array"],
                    "description": "Pages to search: integer (0 to N-1) or array [start, end] (default: 50) - used with lookup_transactions"
                },
                "page_size": {
                    "type": "integer",
                    "description": "Transactions per API call, 1-1000 (default: 1000) - used with lookup_transactions"
                },
                "early_termination": {
                    "type": "boolean",
                    "description": "Stop searching when transaction found (default: true) - used with lookup_transactions"
                }
