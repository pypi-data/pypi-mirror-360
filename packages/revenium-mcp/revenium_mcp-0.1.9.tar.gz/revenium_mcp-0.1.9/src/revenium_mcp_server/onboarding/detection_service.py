...
class OnboardingDetectionService:
    ...
    async def get_onboarding_state(self) -> OnboardingState:
        ...
        recommendations = self._generate_recommendations(
            is_first_time, setup_completion
        )
        ...
        logger.debug(f"📋 Onboarding state: first_time={is_first_time}, cache_exists={cache_exists}")
    ...
    async def _get_setup_completion_status(self) -> Dict[str, bool]:
        ...
        try:
            ...
            status["auto_discovery_working"] = validation_result.summary.get('auto_discovery_works', False)
    ...
    def _generate_recommendations(
        ...
        try:
            ...
            notification_email = smart_defaults.env_defaults.get("notification_email", "admin@company.com")

            if is_first_time:
                recommendations.append("Welcome! Let's get you set up with the Revenium MCP server.")
            ...
            if not setup_completion.get("email_configured", False):
                ...
                if notification_email != "dummy@email.com":
                    recommendations.append(f"Configure your notification email (suggested: {notification_email}) for alerts.")
                else:
            ...
            if not is_config_cached():
                ...
                for attempt in range(max_retries):
                    ...
                    if config_store._discovered_config and config_store._discovered_config.has_required_fields():
                        logger.info(f"✅ Auto-discovery completed for cache creation on attempt {attempt + 1}")
                        break

                    if attempt < max_retries - 1:
                        logger.warning(f"⚠️ Auto-discovery incomplete for cache creation on attempt {attempt + 1}, retrying...")
                        await asyncio.sleep(0.5)
                    else:
                        logger.error("❌ Auto-discovery failed for cache creation after all retries")

                # Get current configuration values
                config = {}

                # Add any available configuration values
                for var in ["REVENIUM_API_KEY", "REVENIUM_TEAM_ID", "REVENIUM_TENANT_ID",
                           "REVENIUM_OWNER_ID", "REVENIUM_DEFAULT_EMAIL", "REVENIUM_BASE_URL"]:
                    value = get_config_value(var)
                    if value:
                        config[var] = value
                    logger.debug(f"Cache creation - {var}: {'SET (hidden)' if 'API_KEY' in var and value else value or 'NONE'}")
                ...
                if missing_required:
                    logger.warning(f"⚠️ Cannot mark onboarding complete - missing required fields: {missing_required}")
                    logger.info("💡 Auto-discovery should provide these values. Check API connectivity and authentication.")
                    return False

                logger.info(f"✅ All required fields present, saving cache with {len(config)} values")

                # Save to cache with all required fields
                await _default_cache.save_config(config)
                logger.info(f"✅ Onboarding completion marked - cache created with {len(config)} configuration values")
