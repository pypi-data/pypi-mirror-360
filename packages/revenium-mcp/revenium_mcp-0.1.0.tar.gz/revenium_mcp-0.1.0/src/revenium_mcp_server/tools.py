...
# REFACTORED: Using consolidated management tools instead of dual-layer architecture
from .models import Product
from .product_validators import ProductValidationEngine
from .exceptions import ValidationError
from .schema_discovery import ProductSchemaDiscovery, SourceSchemaDiscovery, SubscriptionSchemaDiscovery
from .workflow_engine import WorkflowEngine
from .smart_defaults_integration import smart_defaults_integration, smart_defaults_reporter
...
class ReveniumTools:
    ...
    async def handle_manage_products(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_subscriptions(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_sources(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_customers(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_alerts(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            if not action:
                ...
                return [create_missing_parameter_error("action", [
                    "list", "get", "create", "update", "delete", "clear_all",
                    "get_metrics", "query", "get_capabilities", "get_examples",
                    "validate", "get_agent_summary", "acknowledge"
                ])]
            ...
            error = StandardErrorBuilder("Failed to process alert management request").\
                field("system_error").\
    ...
    async def handle_manage_workflows(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            if action == "get_agent_summary":
                ...
                for workflow_id, workflow_info in workflows.items():
                    ...
                    result_text += f"**Tools Involved**: {', '.join(workflow_info['tools_involved'])}\n\n"
                ...
                if not workflow_id:
                    return [TextContent(
                        type="text",
                        text="❌ **Error**: workflow_id is required for get action"
                    )]

                workflow_details = self.workflow_engine.get_workflow_details(workflow_id)
                if not workflow_details:
                    available_workflows = list(self.workflow_engine.get_available_workflows().keys())
                    return [TextContent(
                        type="text",
                ...
                result_text += f"Use `start` action with workflow_id='{workflow_id}' to begin execution\n"
...
💡 **Pro Tip**: Copy any example above and modify the context data for your specific use case!"""
    ...
    def _handle_validate(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if not has_errors:
            ...
            result_text += f"**Next Step**: Use `action=\"start\"` with the same parameters to begin execution."
        else:
            result_text += f"\n## **🔧 Required Fixes**\n"
            result_text += f"Please address the validation errors above before starting the workflow.\n\n"
            result_text += f"**Help**: Use `get_examples` to see valid workflow configurations."

        return [TextContent(type="text", text=result_text)]

    def _handle_create_simple(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _create_simple_customer_onboarding(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _create_simple_product_launch(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _create_simple_alert_setup(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    def _create_simple_data_pipeline(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_metering(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def handle_manage_metering_elements(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
