...
5. Default: enterprise (all tools enabled)
"""
...
class ConfigurationLoader:
    ...
    def load_environment_config(self) -> Tuple[str, Dict[str, bool]]:
        ...
        profile = os.getenv("TOOL_PROFILE", "enterprise")
        if profile not in PROFILE_DEFINITIONS:
            logger.warning(f"Invalid TOOL_PROFILE '{profile}', using 'enterprise'")
            profile = "enterprise"
    ...
    def validate_configuration(
        ...
        # Validate profile
        if profile not in PROFILE_DEFINITIONS:
            logger.warning(f"Invalid profile '{profile}', using 'enterprise'")
            profile = "enterprise"
