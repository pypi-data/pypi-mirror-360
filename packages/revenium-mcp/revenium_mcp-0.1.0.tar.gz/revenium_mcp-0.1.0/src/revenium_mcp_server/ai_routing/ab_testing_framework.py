...
class ABTestingFramework:
    ...
    def __init__(
        ...
        logger.info(
            f"A/B Testing Framework initialized with session: {self.session_id}"
        )
    ...
    async def _test_scenario(self, scenario: TestScenario) -> ABTestResult:
        ...
        rule_result, rule_time, rule_success, rule_error = (
            await self._test_rule_routing(scenario.query, scenario.expected_tool)
        )

        # Compare results
        accuracy_match = self._check_accuracy_match(
            ai_result, rule_result, scenario
        )
    ...
    def _check_accuracy_match(
        ...
        expected_tool_match = (
            ai_result.tool_name == scenario.expected_tool and
            rule_result.tool_name == scenario.expected_tool
        )
        expected_action_match = (
            ai_result.action == scenario.expected_action and
            rule_result.action == scenario.expected_action
        )

        return (
            tool_match and action_match and
            expected_tool_match and expected_action_match
        )
    ...
    def _generate_summary(self, results: List[ABTestResult]) -> ABTestSummary:
        ...
        successful_comparisons = sum(
            1 for r in results if r.ai_success and r.rule_success
        )
        ...
        if rule_avg_time > 0:
            ai_performance_advantage = (
                ((rule_avg_time - ai_avg_time) / rule_avg_time) * 100
            )
        ...
        ai_accurate = sum(
            1 for r in results
            if r.ai_success and self._is_result_accurate(r, "ai")
        )
        rule_accurate = sum(
            1 for r in results
            if r.rule_success and self._is_result_accurate(r, "rule")
        )

        ai_accuracy_rate = ai_accurate / ai_successes if ai_successes > 0 else 0.0
        rule_accuracy_rate = (
            rule_accurate / rule_successes if rule_successes > 0 else 0.0
        )

        # Generate recommendations and confidence level
        recommendations = self._generate_recommendations(
            ai_success_rate, rule_success_rate, ai_performance_advantage,
            accuracy_match_rate, successful_comparisons
        )
    ...
    def _generate_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "A/B testing results look good! Both routing methods are "
                "performing well."
            )
