    ...
    async def validate_workflow(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        raise ToolError(
            ...
            examples={
                ...
                "ucm_validation": "UCM provides real-time validation capabilities"
            }
        )
    ...
    async def _handle_get_capabilities(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if self.ucm_helper:
            try:
                ...
                logger.info(f"Workflow Management: Got UCM capabilities with {len(ucm_capabilities.get('workflow_types', []))} workflow types")
            except ToolError:
...
```"""
        ...
        if ucm_capabilities and "workflow_types" in ucm_capabilities:
            text += "\n\n## **Supported Workflow Types (API-Verified)**\n"
            for workflow_type in ucm_capabilities["workflow_types"]:
                description = ucm_capabilities.get("workflow_descriptions", {}).get(workflow_type, "")
                text += f"- **{workflow_type}** - {description}\n"
        else:
            text += "\n\n## **Supported Workflow Types**\n"
            text += "- Use UCM integration to see API-verified workflow types\n"
        ...
        if ucm_capabilities and "templates" in ucm_capabilities:
            ...
            for template_name, template_info in templates.items():
                text += f"- **{template_name}**: {template_info.get('description', 'No description')}\n"
        else:
            text += "\n\n## **Available Templates**\n"
            text += "- Use UCM integration to see API-verified workflow templates\n"
...
4. Complete steps with `complete_step(workflow_id="...", step_result={...})`"""
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "list", "get", "start", "next_step", "complete_step",
            "get_workflow_templates", "get_capabilities",
            "get_examples", "get_agent_summary"
        ]
