...
class EmailVerification(ToolBase):
    ...
    tool_description: ClassVar[str] = "Email configuration and verification for notification setup. Key actions: check_status, update_email, validate_email, setup_guidance. Use get_examples() for email setup guidance and get_capabilities() for complete action list."
...
"""
            ...
            error = ToolError(
                message=f"Failed to update email configuration",
...
"""
    ...
    def _build_email_status(self, current_email: Optional[str]) -> str:
        ...
        if current_email:
            ...
            status += f"**Status**: Active and ready for notifications\n"
            status += f"**Source**: {'Environment variable' if current_email else 'Auto-discovered'}\n\n"
    ...
    def _build_setup_guidance(self, current_email: Optional[str]) -> str:
        ...
        if current_email:
            guidance += f"## ✅ **Current Configuration**\n\n"
            guidance += f"**Email**: {current_email}\n"
            guidance += f"**Status**: Configured and ready\n\n"
        ...
        guidance += "- **Check setup**: Use `setup_checklist()` to verify complete configuration\n\n"
    ...
    def _build_configuration_test(self, current_email: Optional[str]) -> str:
        ...
        if current_email:
            ...
            try:
                ...
                test += f"**Format Validation**: ✅ Valid\n"
            ...
            if "@" in current_email:
                ...
                if "." in domain:
                    test += f"**Domain Format**: ✅ Contains TLD\n"
                else:
                    test += f"**Domain Format**: ⚠️ Missing TLD\n"
            else:
                test += f"**Format**: ❌ Missing @ symbol\n"
