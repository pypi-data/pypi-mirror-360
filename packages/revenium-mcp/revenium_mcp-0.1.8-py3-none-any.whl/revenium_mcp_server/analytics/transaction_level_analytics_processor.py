...
class TransactionLevelAnalyticsProcessor:
    ...
    async def analyze_summary_metrics(
        ...
        try:
            ...
            logger.info(f"Summary transaction analysis complete. Total cost: ${transaction_data.total_cost:.2f}, Providers: {len(transaction_data.cost_by_provider)}")
    ...
    async def analyze_customer_transactions(
        ...
        """
        logger.info(f"Analyzing customer transaction metrics for team {team_id}, period: {period}, group: {group}")

        # Validate parameters using existing validator
        self.validator.validate_transaction_level_query("customer_analytics", {
            "teamId": team_id,
            "period": period,
            "group": group
        })
        ...
        try:
            ...
            logger.info(f"Customer transaction analysis complete. Organizations analyzed: {len(customer_analysis.get('organizations', {}))}")
    ...
    async def analyze_customer_profitability(
        ...
        try:
            ...
            logger.info(f"Customer profitability analysis complete. Customers analyzed: {len(customer_profitability)}")
    ...
    async def analyze_product_transactions(
        ...
        """
        logger.info(f"Analyzing product transaction metrics for team {team_id}, period: {period}, group: {group}")

        # Validate parameters using existing validator
        self.validator.validate_transaction_level_query("product_analytics", {
            "teamId": team_id,
            "period": period,
            "group": group
        })
        ...
        try:
            ...
            logger.info(f"Product transaction analysis complete. Products analyzed: {len(product_analysis.get('products', {}))}")
    ...
    async def analyze_product_profitability(
        ...
        try:
            ...
            logger.info(f"Product profitability analysis complete. Products analyzed: {len(product_profitability)}")
    ...
    async def analyze_agent_transactions(
        ...
        """
        logger.info(f"Analyzing agent transaction metrics for team {team_id}, period: {period}, group: {group}")

        # Validate parameters using existing validator
        self.validator.validate_transaction_level_query("agent_analytics", {
            "teamId": team_id,
            "period": period,
            "group": group
        })
        ...
        try:
            ...
            logger.info(f"Agent transaction analysis complete. Agents analyzed: {len(agent_analysis.get('agents', {}))}")
    ...
    async def analyze_agent_performance(
        ...
        try:
            ...
            agent_performance = self._process_agent_performance(
                agent_data, top_n
            )

            logger.info(f"Agent performance analysis complete. Agents analyzed: {len(agent_performance)}")
    ...
    async def analyze_task_metrics(
        ...
        # Validate parameters using existing validator
        self.validator.validate_transaction_level_query("task_analytics", {
            "teamId": team_id,
            "period": period,
            "group": group
        })
        ...
        try:
            ...
            logger.info(f"Task analytics analysis complete. Providers/Models analyzed: {len(task_analysis.get('providers', {})) + len(task_analysis.get('models', {}))}")
    ...
    async def analyze_task_performance(
        ...
        try:
            ...
            task_performance = self._process_task_performance(
                task_data, top_n
            )

            logger.info(f"Task performance analysis complete. Items analyzed: {len(task_performance)}")
    ...
    async def _fetch_summary_data(
        ...
        tasks = {
            ...
            "tokens_per_minute_by_provider": client._request_with_retry(
                ...
                params={**params, "tokenType": "TOTAL"}  # Add required tokenType parameter to base params
            )
        }
    ...
    def _process_summary_data(
            ...
            if isinstance(provider_data, list) and provider_data:
                for i, time_period in enumerate(provider_data):
                    if not isinstance(time_period, dict):
                        logger.warning(f"Expected dict at provider_data[{i}], got {type(time_period).__name__}")
                        continue
                    ...
                    for j, group_data in enumerate(groups):
                        ...
                        if provider_cost > 0:
                            cost_by_provider[provider_name] = cost_by_provider.get(provider_name, 0) + provider_cost
                ...
                for time_period in avg_provider_data:
                    ...
                    for group_data in groups:
                        ...
                        if avg_cost > 0:
                            ...
                            performance_metrics[provider_name]["avg_cost_per_transaction"] = avg_cost
        except Exception as e:
        ...
        try:
            ...
            if isinstance(model_data, list) and model_data:
                for i, time_period in enumerate(model_data):
                    if not isinstance(time_period, dict):
                        logger.warning(f"Expected dict at model_data[{i}], got {type(time_period).__name__}")
                        continue
                    ...
                    for j, group_data in enumerate(groups):
                        ...
                        if model_cost > 0:
                            cost_by_model[model_name] = cost_by_model.get(model_name, 0) + model_cost
        except Exception as e:
                ...
                for time_period in subscriber_data:
                    ...
                    for group_data in groups:
                        ...
                        if subscriber_cost > 0:
                            cost_by_subscriber[subscriber_name] = cost_by_subscriber.get(subscriber_name, 0) + subscriber_cost
        except Exception as e:
                ...
                for time_period in token_data:
                    ...
                    for group_data in groups:
                        ...
                        if tokens_per_minute > 0:
                            if provider_name not in performance_metrics:
                                performance_metrics[provider_name] = {}
                            performance_metrics[provider_name]["tokens_per_minute"] = tokens_per_minute
        except Exception as e:
        ...
        average_cost_per_transaction = total_cost / max(total_transactions, 1) if total_transactions > 0 else 0.0
    ...
    async def _fetch_customer_data(self, client: ReveniumClient, team_id: str, period: str, group: str) -> Dict[str, Any]:
        ...
        tasks = {
            "cost_metric_by_organization": client._request_with_retry(
                "GET",
                self.customer_endpoints["cost_metric_by_organization"],
                params=params
            ),
            "revenue_metric_by_organization": client._request_with_retry(
                "GET",
                self.customer_endpoints["revenue_metric_by_organization"],
                params=params
            ),
    ...
    def _process_customer_profitability(
            ...
            for time_period in cost_data:
                ...
                for group_data in groups:
                    ...
                    if org_name not in organization_data:
                        organization_data[org_name] = {"cost": 0, "revenue": 0, "percentage_revenue": 0, "transactions": 0}
        ...
        for org_name, data in organization_data.items():
            ...
            transaction_count = max(1, int(cost / 0.01)) if cost > 0 else 1  # Assume $0.01 per transaction
            cost_per_transaction = cost / transaction_count if transaction_count > 0 else 0.0

            customers.append(CustomerTransactionData(
    ...
    def _process_product_profitability(
            ...
            for time_period in cost_data:
                ...
                for group_data in groups:
                    ...
                    if product_name not in product_data_dict:
                        product_data_dict[product_name] = {"cost": 0, "revenue": 0, "percentage_revenue": 0, "transactions": 0}
        ...
        for product_name, data in product_data_dict.items():
            ...
            products.append(ProductTransactionData(
    ...
    def _process_customer_data(self, customer_data: Dict[str, Any], period: str, group: str) -> Dict[str, Any]:
                    ...
                    for group_data in groups:
                        ...
                        if org_cost > 0:
                            if org_name not in organizations:
                                organizations[org_name] = {"cost": 0.0, "revenue": 0.0, "percentage_revenue": 0.0}
        ...
        try:
            percentage_revenue_data = customer_data.get("percentage_revenue_metric_by_organization", {})
    ...
    async def _fetch_product_data(self, client: ReveniumClient, team_id: str, period: str, group: str) -> Dict[str, Any]:
        ...
        tasks = {
            "cost_metric_by_product": client._request_with_retry(
                "GET",
                self.product_endpoints["cost_metric_by_product"],
                params=params
            ),
            "revenue_metric_by_product": client._request_with_retry(
                "GET",
                self.product_endpoints["revenue_metric_by_product"],
                params=params
            ),
            "percentage_revenue_metric_by_product": client._request_with_retry(
                "GET",
                self.product_endpoints["percentage_revenue_metric_by_product"],
                params=params
            )
        }
    ...
    def _process_product_data(self, product_data: Dict[str, Any], period: str, group: str) -> Dict[str, Any]:
                    ...
                    for group_data in groups:
                        ...
                        if product_cost > 0:
                            if product_name not in products:
                                products[product_name] = {"cost": 0.0, "revenue": 0.0, "percentage_revenue": 0.0}
    ...
    async def _fetch_agent_data(self, client: ReveniumClient, team_id: str, period: str, group: str) -> Dict[str, Any]:
        ...
        tasks = {
            "cost_metrics_by_agents_over_time": client._request_with_retry(
                "GET",
                self.agent_endpoints["cost_metrics_by_agents_over_time"],
                params=params
            ),
            "call_count_metrics_by_agents": client._request_with_retry(
                "GET",
                self.agent_endpoints["call_count_metrics_by_agents"],
                params=params
            ),
            "performance_metrics_by_agents": client._request_with_retry(
                "GET",
                self.agent_endpoints["performance_metrics_by_agents"],
                params=params
            )
        }
    ...
    def _process_agent_data(self, agent_data: Dict[str, Any], period: str, group: str) -> Dict[str, Any]:
    ...
    def _process_agent_performance(
        ...
        for agent_name, data in agent_data_dict.items():
            ...
            agents.append(AgentAnalyticsData(
    ...
    async def _fetch_task_data(self, client: ReveniumClient, team_id: str, period: str, group: str) -> Dict[str, Any]:
        ...
        tasks = {
            "cost_metric_by_provider": client._request_with_retry(
                "GET",
                self.task_endpoints["cost_metric_by_provider"],
                params=params
            ),
            "cost_metric_by_model": client._request_with_retry(
                "GET",
                self.task_endpoints["cost_metric_by_model"],
                params=params
            ),
            "performance_metric_by_provider": client._request_with_retry(
                "GET",
                self.task_endpoints["performance_metric_by_provider"],
                params=params
            ),
            "performance_metric_by_model": client._request_with_retry(
                "GET",
                self.task_endpoints["performance_metric_by_model"],
                params=params
            )
        }
    ...
    def _process_task_data(self, task_data: Dict[str, Any], period: str, group: str) -> Dict[str, Any]:
    ...
    def _process_task_performance(
        ...
        for provider_name, data in provider_data_dict.items():
            ...
            task_analytics.append(TaskAnalyticsData(
        ...
        task_analytics.sort(key=lambda x: max(x.completion_rates.values()) if x.completion_rates else 0, reverse=True)
