...
class AlertSemanticProcessor:
    ...
    def parse_alert_request(self, text: str) -> Dict[str, Any]:
        ...
        notification_email = env_email if env_email and env_email != "dummy@email.com" else "admin@example.com"
    ...
    def _extract_threshold(self, text: str) -> Tuple[Optional[str], Optional[float], bool]:
        ...
        operator = None
        sorted_operators = sorted(self.operator_mappings.items(), key=lambda x: len(x[0]), reverse=True)
    ...
    def _extract_time_period(self, text: str) -> Optional[str]:
        ...
        # Try exact matches first (longer phrases first)
        sorted_periods = sorted(self.time_period_mappings.items(), key=lambda x: len(x[0]), reverse=True)
    ...
    def _extract_alert_type(self, text: str) -> Optional[str]:
        ...
        cumulative_strong_indicators = [
            ...
            "budget", "spending limit", "cost limit", "usage limit", "quota",
            "budget alert", "budget monitoring", "spending cap", "cost cap",
            "monthly budget", "weekly budget", "daily budget", "quarterly budget",

            # Calendar period language (key indicator)
            "monthly", "weekly", "daily", "quarterly",
            "per month", "per week", "per day", "per quarter",
            "in a month", "in a week", "in a day", "in a quarter",
            "each month", "each week", "each day", "each quarter",
            "every month", "every week", "every day", "every quarter",

            # Cumulative/total language
            "cumulative", "total usage", "period usage", "total spending",
            "total cost", "aggregate", "sum", "accumulate",

            # Tracking period language
            "tracking period", "budget period", "billing period", "reporting period",

            # Reset/period language
            "resets", "period", "calendar", "billing cycle"
        ]
        ...
        for pattern in product_patterns:
            ...
            for match in matches:
                ...
                if value:
                    ...
                    is_exclusion = "exclude" in match.group(0) or "except" in match.group(0) or "but not" in match.group(0) or "ignore" in match.group(0)
            ...
            for filter_item in enhanced["filters"]:
                ...
                if "dimension" in filter_item:
                    ...
                    if dimension_lower in self.filter_dimension_mappings:
                        filter_item["dimension"] = self.filter_dimension_mappings[dimension_lower].upper()
    ...
    def _analyze_conceptual_intent(self, text_lower: str) -> Dict[str, Any]:
        ...
        create_alert_patterns = [
            "create alert", "create an alert", "make alert", "make an alert",
            "set up alert", "set up an alert", "add alert", "add an alert"
        ]

        show_alerts_patterns = [
            "show alerts", "show me alerts", "list alerts", "get alerts",
            "see alerts", "view alerts", "display alerts", "find alerts"
        ]
        ...
        monitoring_patterns = [
            "monitor", "track", "watch", "observe", "keep an eye on"
        ]
        ...
        budget_patterns = [
            "budget", "spending", "usage", "quota", "limit", "monthly", "weekly", "daily", "quarterly"
        ]
        ...
        realtime_patterns = [
            "real-time", "immediate", "instant", "right now", "currently", "at the moment",
            "per minute", "per hour", "rate", "speed", "performance"
        ]
