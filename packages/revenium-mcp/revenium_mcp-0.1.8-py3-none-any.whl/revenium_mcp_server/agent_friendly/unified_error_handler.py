    ...
    def create_invalid_value_error(
        ...
        field_errors = {
            parameter_name: f"Invalid value '{provided_value}'. " + 
                           (f"Valid values: {', '.join(valid_values)}" if valid_values else 
                            f"Expected format: {expected_format}" if expected_format else "Invalid value")
        }
        
        if not examples and valid_values:
            examples = {
                "valid_request": {
                    parameter_name: valid_values[0]
                }
            }
