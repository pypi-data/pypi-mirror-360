...
class AnalyticsCapabilityDiscovery:
    ...
    async def discover_analytics_capabilities(self) -> Dict[str, Any]:
        ...
        capabilities = {
            ...
            "chart_types": ["line", "bar", "pie", "area", "stacked_bar", "dual_axis", "scatter", "donut"],
            "query_types": ["cost_analysis", "profitability", "trend", "comparison", "breakdown", "transaction_level"],
            "entity_types": ["providers", "models", "customers", "organizations", "products", "agents", "transactions", "tasks", "performance_metrics", "cost_metrics"]
        }

        # Use static endpoint configuration to avoid timeout during startup
        await self._configure_static_endpoints(capabilities)

        logger.info(f"Configured analytics capabilities: {len(capabilities['analytics_endpoints'])} endpoints")
        return capabilities
    ...
    async def _discover_endpoint_availability(self, capabilities: Dict[str, Any]) -> None:
        ...
        for endpoint_name, endpoint_path in self.analytics_endpoints.items():
            try:
                ...
                response = await self.client._request_with_retry("GET", endpoint_path, params=params)
    ...
    async def _discover_supported_periods(self, capabilities: Dict[str, Any]) -> None:
        ...
        test_periods = [
            ...
            "ONE_WEEK", "TWO_WEEKS", "FOUR_WEEKS", "TWELVE_MONTHS"
        ]
    ...
    async def _discover_chart_types(self, capabilities: Dict[str, Any]) -> None:
        ...
        if capabilities.get("analytics_endpoints"):
            ...
            cost_endpoints = [ep for ep in capabilities["analytics_endpoints"]
            ...
            time_endpoints = [ep for ep in capabilities["analytics_endpoints"]
    ...
    async def _discover_query_types(self, capabilities: Dict[str, Any]) -> None:
        ...
        revenue_endpoints = [ep for ep in endpoints if "revenue" in ep and endpoints[ep].get("available")]
        if revenue_endpoints:
            query_types.append("profitability")

        # Trend analysis if we have time-based endpoints
        time_endpoints = [ep for ep in endpoints if "over-time" in ep and endpoints[ep].get("available")]
        if time_endpoints:
        ...
        breakdown_endpoints = [ep for ep in endpoints
                             if any(cat in ep for cat in ["by-provider", "by-model", "by-organization", "by-product"])
        ...
        transaction_endpoints = [ep for ep in endpoints
                               if any(cat in ep for cat in ["by-agents", "by-subscriber-credential", "tokens-per-minute"])
                               and endpoints[ep].get("available")]
