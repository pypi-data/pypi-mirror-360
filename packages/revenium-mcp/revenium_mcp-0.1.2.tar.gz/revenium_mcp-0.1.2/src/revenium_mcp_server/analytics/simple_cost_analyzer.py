...
class SimpleCostAnalyzer:
    ...
    async def get_provider_costs(self, period: str, aggregation: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            self.logger.info(f"Getting provider costs for period={period}, aggregation={aggregation}")
    ...
    async def get_model_costs(self, period: str, aggregation: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(response, (list, dict)):
                self.logger.info(f"DEBUG: Response length/keys: {len(response) if isinstance(response, list) else list(response.keys())}")
            ...
            if isinstance(response, dict) and 'groups' in response:
                ...
                data = response
                self.logger.info(f"DEBUG: Response is dict with groups, found {len(response.get('groups', []))} groups")
            ...
            if isinstance(data, dict) and 'groups' in data:
                ...
                self.logger.info(f"DEBUG: Processing {len(groups)} groups from total endpoint response")
            ...
            elif isinstance(data, list) and data and isinstance(data[0], dict) and 'groups' in data[0]:
                model_totals = {}
            ...
            # Try direct format (simple list of model objects)
                ...
                for i, item in enumerate(data):
                    ...
                    model_name = (item.get('model') or item.get('modelName') or
                                item.get('name') or item.get('groupName') or 'Unknown Model')

                    # Extract cost from nested metrics structure or direct fields
                    cost = 0.0
                    if 'metrics' in item and isinstance(item['metrics'], list) and len(item['metrics']) > 0:
                        ...
                        cost = float(item.get('metricResult', 0) or item.get('cost', 0) or
                                   item.get('totalCost', 0) or item.get('amount', 0) or 0)
            ...
            self.logger.info(f"Successfully processed {len(processed_data)} model cost records, total: ${total_cost:.2f}")
            return processed_data
    ...
    async def get_customer_costs(self, period: str, aggregation: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            self.logger.info(f"Getting customer costs for period={period}, aggregation={aggregation}")
            ...
            # Try direct format (simple list of customer objects)
                ...
                for i, item in enumerate(data):
                    ...
                    customer_name = (item.get('organization') or item.get('customer') or
                                   item.get('name') or item.get('groupName') or 'Unknown Customer')
                    cost = float(item.get('metricResult', 0) or item.get('cost', 0) or
                               item.get('totalCost', 0) or item.get('amount', 0) or 0)
            ...
            self.logger.info(f"Successfully processed {len(processed_data)} customer cost records, total: ${total_cost:.2f}")
            return processed_data
    ...
    async def investigate_cost_spike(self, threshold: float, period: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Cost spike investigation completed: spike_detected={investigation_result['spike_detected']}, contributors={len(spike_contributors)}, total_spike_cost=${total_spike_cost:.2f}")
    ...
    async def get_api_key_costs(self, period: str, aggregation: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            self.logger.info(f"Getting API key costs for period={period}, aggregation={aggregation}")

            # Use the API key costs endpoint (from PRD specifications)
            endpoint = "/profitstream/v2/api/sources/metrics/ai/cost-metrics-by-subscriber-credential"
    ...
    async def get_cost_summary(self, period: str, aggregation: str) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"Getting enhanced cost summary for period={period}, aggregation={aggregation}")
            ...
            self.logger.info(f"Enhanced cost summary completed: total_cost=${total_cost:,.2f}, dimensions=5")
            return summary
