...
class LogFilter:
    ...
    def apply_filters(
        ...
        try:
            ...
            if operation_filter:
                filtered_entries = self._filter_by_operation(
                    filtered_entries, operation_filter
                )
                applied_filters["operation_filter"] = operation_filter
            
            # Apply status filter
            if status_filter:
                filtered_entries = self._filter_by_status(
                    filtered_entries, status_filter
                )
                applied_filters["status_filter"] = status_filter
            
            # Apply search term filter
            if search_term:
                filtered_entries = self._filter_by_search_term(
                    filtered_entries, search_term
                )
    ...
    def validate_filter_parameters(
        ...
        if status_filter:
            ...
            if status_filter.upper() not in valid_statuses:
                warnings.append(f"Status filter '{status_filter}' may not match any entries. Valid statuses: {', '.join(valid_statuses)}")
