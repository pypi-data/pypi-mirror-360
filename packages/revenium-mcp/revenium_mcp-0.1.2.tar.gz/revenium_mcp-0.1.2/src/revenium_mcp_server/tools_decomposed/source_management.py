        ...
        if not source_data:
            raise create_structured_missing_parameter_error(
                ...
                examples={
                    ...
                    "example_data": {"name": "API Source", "type": "api", "url": "https://api.example.com"},
...
class SourceValidator:
    ...
    async def get_capabilities(self) -> Dict[str, Any]:
        ...
        supported_actions = [
            ...
            "create_from_text", "validate",
            "get_capabilities", "get_examples", "get_agent_summary", "get_tool_metadata"
        ]
    ...
    def validate_configuration(self, source_data: Dict[str, Any], dry_run: bool = True) -> Dict[str, Any]:
...
class SourceEnhancementProcessor:
    ...
    async def create_source(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if source_type == "api":
            ...
            stream_url = arguments.get("stream_url") or source_data.get("stream_url") or arguments.get("url") or source_data.get("url")
            ...
            model_endpoint = arguments.get("model_endpoint") or source_data.get("model_endpoint") or arguments.get("url") or source_data.get("url")
        ...
        logger.info(f"CREATE_SIMPLE DEBUG: Final source config: {json.dumps(source_config, indent=2)}")
...
class SourceManagement(ToolBase):
            ...
            "properties": {
                ...
                "source_data": {
                    ...
                    "properties": {
                        ...
                        "url": {
                            "type": "string",
                            "description": "Source URL/endpoint (optional)"
                        },
    ...
    async def _get_examples(self, example_type: Optional[str] = None) -> str:
        ...
        examples_text += "## **Benefits**\n\n"
        examples_text += "✅ **Simple**: Users only specify what they know (name, type, URL)\n"
        examples_text += "✅ **Smart**: Auto-generation of technical fields\n"
        examples_text += "✅ **Automatic**: System fields handled behind the scenes\n"
        examples_text += "✅ **Consistent**: Same pattern across all source types\n\n"
    ...
    async def handle_action(
        ...
        try:
            ...
            if action == "list":
                ...
                if auto_generate:
                    ...
                    if not name:
                        return [TextContent(
                            ...
                            text=f"**Missing Required Field**\n\n"
                                 f"**Field**: `name` (required for all source creation)\n\n"
                                 f"**Example**: `{{\"action\":\"create\",\"source_data\":{{\"name\":\"My API\"}}}}`\n\n"
                                 f"**Auto-Generation**: Enabled (will auto-generate description, version, type)"
                        )]
                    ...
                    enhanced_source_data = {
                        ...
                        "type": "API" if source_type == "api" else source_type.upper(),
                        "sourceType": source_type.upper() if source_type in ["api", "stream", "ai"] else "UNKNOWN",
                        "syncedWithApiGateway": False,
                        "autoDiscoveryEnabled": False,
                        "tags": [],
                        "sourceClassifications": []
                    ...
                    if source_type == "api":
                        enhanced_source_data.update({
                            "description": source_data.get("description") or f"REST API source for {name}",
                            "type": "API",
                            "sourceType": "API"
                        })
                        if url:
                        ...
                        enhanced_source_data.update({
                            "description": source_data.get("description") or f"Stream source for {name}",
                            "type": "STREAM",
                            "sourceType": "STREAM"
                        })
                        if url:
                        ...
                        enhanced_source_data.update({
                            "description": source_data.get("description") or f"AI source for {name}",
                            "type": "AI",
                            "sourceType": "AI"
                        })
                        if url:
                ...
                if not auto_generate:
                    validation_result = self.validator.validate_configuration(source_data, dry_run=False)
            ...
            # Enhanced operations
                ...
                return [TextContent(
                    type="text",
                    text=f"Source created from text successfully:\n\n" +
                         json.dumps(result, indent=2)
                )]
            ...
            # Validation and discovery operations
                ...
                if auto_generate:
                    ...
                    if not name:
                        return [TextContent(
                            type="text",
                            text=f"**Validation Failed**\n\n"
                                 f"**Field**: `name` (required for validation)\n\n"
                                 f"**Example**: `{{\"action\":\"validate\",\"source_data\":{{\"name\":\"My API\"}},\"auto_generate\":true}}`\n\n"
                                 f"**Auto-Generation**: Enabled (will auto-generate description, version, type)"
                        )]
                ...
                debug_info = {
                    ...
                    "ucm_helper_type": type(self.validator.ucm_helper).__name__ if self.validator.ucm_helper else None,
                ...
                return [TextContent(type="text", text=f"**UCM Debug Info**:\n\n{json.dumps(debug_info, indent=2)}")]
    ...
    def _format_capabilities_response(self, capabilities: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        result_text += "## **Source Types (API-Verified)**\n"
    ...
    def _format_examples_response(self, examples: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        result_text += "## **Usage**\n"
        result_text += "Copy any template above and modify it for your needs, then use the `create` action.\n"
    ...
    async def _get_tool_capabilities(self) -> List[ToolCapability]:
        ...
        return [
            ...
            ToolCapability(
                ...
                parameters={
                    "validate": {"source_data": "dict", "dry_run": "bool"}
                },
                examples=[
                    "validate(source_data={...}, dry_run=True)"
                ]
            ),
            ToolCapability(
