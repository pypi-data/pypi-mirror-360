...
class CredentialNLPProcessor:
    ...
    def _initialize_business_rules(self):
        ...
        self.business_rules = {
            ...
            "subscription_linking_keywords": [
                "billing", "charge", "meter", "usage", "cost", "payment", "invoice"
            ],
            "security_keywords": [
                "production", "live", "prod", "secure", "encrypted", "protected"
            ]
        }
    ...
    def _extract_intent(self, text: str) -> Tuple[CredentialIntent, float]:
        ...
        for intent, patterns in self.intent_patterns.items():
            ...
            if intent == CredentialIntent.CREATE and any(word in text for word in ["create", "new", "add"]):
                score += 0.4
            elif intent == CredentialIntent.UPDATE and any(word in text for word in ["update", "modify", "change"]):
                score += 0.4
            elif intent == CredentialIntent.LINK_SUBSCRIPTION and any(word in text for word in ["link", "subscription", "billing"]):
                score += 0.5
    ...
    def _generate_business_context(self, intent: CredentialIntent, entities: Dict[str, ExtractedEntity], text: str) -> Dict[str, Any]:
        ...
        if intent in [CredentialIntent.CREATE, CredentialIntent.UPDATE, CredentialIntent.LINK_SUBSCRIPTION]:
            context["billing_implications"].append(
                "Credential changes may affect billing automation and usage tracking"
            )

        if "subscription" in text or any(keyword in text for keyword in self.business_rules["subscription_linking_keywords"]):
            context["billing_implications"].append(
                "Subscription associations are critical for accurate billing and metering"
            )

        # Security considerations
        if "external_secret" in entities or any(keyword in text for keyword in self.business_rules["security_keywords"]):
    ...
    def _generate_suggestions(self, intent: CredentialIntent, entities: Dict[str, ExtractedEntity], context: Dict[str, Any]) -> List[str]:
        """Generate actionable suggestions based on extracted information."""
        suggestions = []

        # Entity-based suggestions
        if "email" in entities:
            suggestions.append(f"Resolve subscriber email '{entities['email'].value}' to ID using resolve_subscriber_email_to_id()")

        if "organization" in entities:
            suggestions.append(f"Resolve organization name '{entities['organization'].value}' to ID using resolve_organization_name_to_id()")
        ...
        if intent == CredentialIntent.CREATE:
            ...
            suggestions.append("Use dry_run=true to preview billing impact before linking subscription")
    ...
    def _generate_warnings(self, intent: CredentialIntent, entities: Dict[str, ExtractedEntity], context: Dict[str, Any]) -> List[str]:
        ...
        if "external_secret" in entities:
            ...
            if len(secret_value) < 12:
                warnings.append("⚠️ External secret appears short - consider using a stronger secret")

        # Billing warnings
        if intent == CredentialIntent.DELETE:
            warnings.append("🔒 BILLING WARNING: Credential deletion may disrupt billing automation")
