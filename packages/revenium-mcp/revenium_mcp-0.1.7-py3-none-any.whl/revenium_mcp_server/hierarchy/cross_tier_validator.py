...
class CrossTierValidator:
    ...
    async def validate_hierarchy_operation(self, operation: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if operation_type == OperationType.CREATE:
                ...
                if entity_id:
                    validation_issues = await self._validate_update_operation(entity_type, entity_id, entity_data)
                    issues.extend(validation_issues)
                else:
                ...
                if entity_id:
                    validation_issues = await self._validate_delete_operation(entity_type, entity_id)
                    issues.extend(validation_issues)
                else:
                    issues.append(ValidationIssue(
                        severity=ValidationSeverity.ERROR,
                ...
                if entity_id:
                    validation_issues = await self._validate_link_operation(operation_type, entity_type, entity_id, entity_data)
                    issues.extend(validation_issues)
                else:
                    issues.append(ValidationIssue(
                        severity=ValidationSeverity.ERROR,
    ...
    async def validate_entity_relationships(self, entity_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if entity_type == "credentials" and "subscriberId" in entity_data:
                ...
                if subscriber_id:
                    subscriber_ref = await self.lookup_service.resolve_subscriber(subscriber_id, "id")
            ...
            return ValidationResult(
                ...
                metadata={
                    ...
                    "relationships_checked": ["product_id", "subscriptionIds", "subscriberId", "organizationId"]
                }
            )
        ...
        try:
            ...
            if entity_type == "products":
                ...
                if nav_result.success and nav_result.related_entities:
                    ...
                    recommendations.append("Delete or reassign all subscriptions before deleting the product")
                    ...
                    for subscription in nav_result.related_entities:
                        ...
                        if sub_id:
                            cred_result = await self.navigation_service.get_credentials_for_subscription(sub_id)
                    ...
                    if all_credentials:
                        ...
                        recommendations.append("Delete or reassign all credentials before deleting subscriptions")
            
            elif entity_type == "subscriptions":
                # Check for dependent credentials
                nav_result = await self.navigation_service.get_credentials_for_subscription(entity_id)
                if nav_result.success and nav_result.related_entities:
                    affected_entities["credentials"] = nav_result.related_entities
                    impact_severity = ValidationSeverity.WARNING  # Credentials can exist without subscriptions
                    recommendations.append("Consider reassigning credentials to other subscriptions")
            ...
            return ImpactAnalysis(
                ...
                metadata={
                    "total_affected_entities": sum(len(entities) for entities in affected_entities.values()),
    ...
    async def _validate_create_operation(self, entity_type: str, entity_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _validate_update_operation(self, entity_type: str, entity_id: str, entity_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def _validate_delete_operation(self, entity_type: str, entity_id: str) -> List[ValidationIssue]:
            ...
            if entity_type == "products" and dependent_type == "subscriptions":
                ...
                if nav_result.success and nav_result.related_entities:
                    issues.append(ValidationIssue(
                        ...
                        metadata={"dependent_count": len(nav_result.related_entities), "dependent_type": dependent_type}
                    ))
    ...
    async def _validate_link_operation(self, operation_type: OperationType, entity_type: str, entity_id: str, entity_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    async def check_referential_integrity(self, entity_type: str, entity_id: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                metadata={
                    ...
                    "entity_exists": entity_ref is not None
                }
            )
