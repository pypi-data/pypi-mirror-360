...
class UCMFactory:
    ...
    async def create_ucm(
        ...
        cache_ttl = cache_ttl or int(os.getenv("UCM_CACHE_TTL", "900"))  # Reduced from 3600 to 900 (15 min)
        cleanup_interval = cleanup_interval or int(os.getenv("UCM_CLEANUP_INTERVAL", "180"))  # Reduced from 300 to 180 (3 min)
        ...
        logger.info(f"Created UCM with cache_ttl={cache_ttl}s, cleanup_interval={cleanup_interval}s")
        return ucm
    ...
    async def _create_default_client() -> Optional[ReveniumClient]:
        ...
        if not api_key:
            logger.warning("REVENIUM_API_KEY environment variable not found - UCM will run in offline mode")
            return None
...
class UCMIntegrationHelper:
    ...
    async def replace_hardcoded_capabilities(self, tool_instance, resource_type: str) -> None:
        ...
        if not hasattr(tool_instance, 'get_capabilities'):
            logger.warning(f"Tool {type(tool_instance).__name__} does not have get_capabilities method")
            return
    ...
    async def validate_capability_value(self, resource_type: str, capability_name: str, value: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Capability validation failed for {resource_type}.{capability_name}={value}: {e}")
            return False
