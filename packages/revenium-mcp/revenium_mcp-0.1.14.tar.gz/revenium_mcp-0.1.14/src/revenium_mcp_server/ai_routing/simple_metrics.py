...
class SimpleMetricsCollector:
    ...
    def __init__(self, session_id: Optional[str] = None):
        ...
        logger.info(
            f"Simple metrics collector initialized with session: "
            f"{self.session_id}"
        )
    ...
    def _log_recorded_metrics(
        ...
        logger.debug(
            f"Recorded {method_name} routing metrics for query: "
            f"{query_preview}..."
        )
    ...
    def _check_performance_recommendations(self) -> List[str]:
        ...
        if avg_time > 50:
            recommendations.append(
                f"Average response time ({avg_time:.2f}ms) exceeds "
                f"target (<50ms)"
            )
        return recommendations
    ...
    def _check_fallback_rate_recommendations(self) -> List[str]:
        ...
        if "ai" in method_counts and "fallback" in method_counts:
            ...
            if fallback_rate > 0.1:
                recommendations.append(
                    f"High fallback rate ({fallback_rate:.1%}) indicates "
                    f"AI reliability issues"
                )
        return recommendations
    ...
    def get_comparison_report(self) -> Dict[str, Any]:
        ...
        return {
            "session_id": self.session_id,
            "collection_period": self._get_collection_period(),
            "summary": self._get_routing_summary(
    ...
    def _filter_metrics_by_method(self, method: str) -> List[RoutingMetrics]:
        ...
        return [
            ...
            if m.routing_method.value.lower() == method.lower()
        ]
    ...
    def _generate_comparative_analysis(
        ...
        rule_success_rate = (
            sum(1 for m in rule_metrics if m.success) / len(rule_metrics)
        )
    ...
    def _create_comparison_metrics(
        ...
        if rule_success_rate > 0:
            success_diff_pct = (
                (ai_success_rate - rule_success_rate) / rule_success_rate * 100
            )
    ...
    def _generate_comprehensive_recommendations(
        ...
        fallback_rate = (
            len(fallback_metrics) / total_metrics if total_metrics > 0 else 0
        )

        recommendations.extend(self._check_fallback_rate_issues(fallback_rate))
        recommendations.extend(
            self._check_performance_comparison(ai_metrics, rule_metrics)
        )
    ...
    def _export_csv_format(self) -> str:
        ...
        if self.metrics:
            fieldnames = [
                'query', 'tool_selected', 'action_selected', 'routing_method',
                'response_time_ms', 'success', 'confidence_score', 'timestamp',
                'session_id'
            ]
