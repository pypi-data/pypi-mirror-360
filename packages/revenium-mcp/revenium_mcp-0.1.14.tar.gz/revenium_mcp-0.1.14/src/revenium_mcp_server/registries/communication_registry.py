...
class CommunicationRegistry(BaseToolRegistry):
    ...
    def _initialize_tools(self) -> None:
        ...
        self.logger.info("Communication registry initialized with OAuth, integration, and workflow tools")
    ...
    async def slack_oauth_workflow(
        ...
        return await self._standardized_tool_execution(
            tool_name="slack_oauth_workflow",
        ...
        if hasattr(request, 'action'):
            ...
            if isinstance(request, dict):
                ...
                if tool_name == "slack_oauth_workflow":
                    parameters = OAuthWorkflowParameters(action=action, **{k: v for k, v in request.items() if k != 'action'})
                elif tool_name == "slack_setup_assistant":
                    parameters = IntegrationSetupParameters(action=action, **{k: v for k, v in request.items() if k != 'action'})
                elif tool_name == "verify_email_setup":
                    parameters = EmailVerificationParameters(action=action, **{k: v for k, v in request.items() if k != 'action'})
                elif tool_name == "manage_workflows":
                    parameters = WorkflowManagementParameters(action=action, **{k: v for k, v in request.items() if k != 'action'})
                else:
                    parameters = request
            else:
                parameters = request
...
def create_communication_registry(mcp: FastMCP, logger, ucm_integration_service) -> CommunicationRegistry:
