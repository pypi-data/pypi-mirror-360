...
from .error_handling import (
    ToolError, ResourceError, ErrorCodes,
...
class PartialUpdateHandler:
    ...
    async def update_with_merge(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                config.update_method(resource_id, final_payload),
                timeout=self._operation_timeout
            )
            ...
            if source_field in data:
                ...
                for target_field, transformer_func in transformations.items():
                    try:
                        ...
                        logger.warning(f"Field transformation failed for {source_field} -> {target_field}: {e}")
    ...
    def _prepare_final_payload(
        ...
        fields_to_remove = [
            'resourceType', 'created', 'updated', 'createdAt', 'updatedAt',
            '_links', 'meteringElementDefinitions', 'products', 'contracts',
            'meteringId', 'metadata', 'descriptionAsHTML'
        ]
