...
from .base import (
    BaseReveniumModel, TimestampMixin, IdentifierMixin, MetadataMixin,
...
class MetricType(str, Enum):
    """Metric type enumeration for Revenium API."""
...
class AdvancedAlertConfiguration(BaseReveniumModel):
    ...
    slack_configs: List[SlackConfiguration] = Field(default_factory=list, description="Slack configurations")
    webhook_configs: List[WebhookConfiguration] = Field(default_factory=list, description="Webhook configurations")
...
class ThresholdViolation(BaseReveniumModel):
    ...
    actual_value: Union[int, float] = Field(..., description="Actual value that triggered violation")
...
class AIAnomalyRequest(BaseReveniumModel):
    ...
    notification_addresses: Optional[List[str]] = Field(None, description="Email addresses for notifications")
    filters: Optional[List[AlertFilter]] = Field(None, description="Additional filters")
    advanced_config: Optional[AdvancedAlertConfiguration] = Field(None, description="Advanced configuration")
...
class AIAlert(BaseReveniumModel, TimestampMixin, IdentifierMixin, MetadataMixin):
    ...
    threshold_violation: Optional[ThresholdViolation] = Field(None, description="Threshold violation details")
