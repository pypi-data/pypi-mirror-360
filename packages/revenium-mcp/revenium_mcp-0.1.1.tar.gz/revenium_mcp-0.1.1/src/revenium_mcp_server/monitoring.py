...
class HealthMonitor:
    ...
    def check_alert_conditions(self, metrics: SystemMetrics, performance_summary: Dict[str, Any]) -> List[str]:
    ...
    async def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        if (system_metrics.cpu_percent > 70 or 
            system_metrics.memory_percent > 75 or 
            system_metrics.disk_usage_percent > 80):
    ...
    def update_performance_baseline(self, metric_name: str, value: float):
        ...
        if len(self.baseline_samples[metric_name]) > self.baseline_window_size:
            self.baseline_samples[metric_name] = self.baseline_samples[metric_name][-self.baseline_window_size:]
    ...
    def create_alert(self, alert_type: str, severity: str, message: str, details: Optional[Dict[str, Any]] = None) -> str:
        ...
        getattr(logger, log_level)(f"Alert created: {message}", alert_id=alert_id, alert_type=alert_type)
