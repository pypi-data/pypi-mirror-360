...
from .exceptions import (
    ...
    APIConnectionError, PermissionError, ConfigurationError, OperationTimeoutError,
    DataIntegrityError, InvalidInputError
)
...
def handle_alert_tool_errors(operation_name: str):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
            try:
                ...
                return [TextContent(
                    ...
                    text=(
                        f"🔒 **Permission Denied**\n\n"
                        f"You don't have permission to perform this operation.\n\n"
                        f"**Suggestions:**\n"
                        f"• Check your API key and team permissions\n"
                        f"• Verify you're using the correct team ID\n"
                ...
                return [TextContent(
                    ...
                    text=(
                        f"🌐 **Connection Error**\n\n"
                        f"Unable to connect to the API service.\n\n"
                        f"**Suggestions:**\n"
                        f"• Check your internet connection\n"
                        f"• Verify the API endpoint is accessible\n"
                        f"• Try again in a few moments\n"
                        f"• Check if there are any service outages"
                    )
                )]
                ...
                return [TextContent(
                    type="text",
                    text=(
                        f"⏱️ **Operation Timeout**\n\n"
                        f"The operation took too long to complete.\n\n"
                        f"**Suggestions:**\n"
                        f"• Try the operation again\n"
                        f"• Check your network connection\n"
                        f"• Consider breaking large operations into smaller chunks\n"
...
def translate_http_error(status_code: int, response_text: str, endpoint: str, error_data: Optional[Dict[str, Any]] = None) -> AlertToolsError:
    ...
    if status_code == 400:
        ...
        friendly_error = UserFriendlyErrorTranslator.format_user_friendly_error(error_message, error_data)
        ...
        return OperationTimeoutError(
            operation=f"API call to {endpoint}",
...
def validate_anomaly_id(anomaly_id: Any) -> str:
    ...
    if not anomaly_id:
        raise InvalidInputError(
            ...
            value=anomaly_id,
            reason="Anomaly ID cannot be empty"
        )
...
def validate_alert_id(alert_id: Any) -> str:
    ...
    if not alert_id:
        raise InvalidInputError(
            parameter="alert_id",
            value=alert_id,
            reason="Alert ID cannot be empty"
        )
    ...
    if not alert_id_str:
        raise InvalidInputError(
            parameter="alert_id",
            value=alert_id,
            reason="Alert ID cannot be empty or whitespace"
        )
    ...
    if len(alert_id_str) > 100:
        raise InvalidInputError(
            parameter="alert_id",
            value=alert_id,
            reason="Alert ID is too long (max 100 characters)"
        )
...
def validate_pagination_params(page: Any, size: Any) -> tuple[int, int]:
    ...
    if page_int < 0:
        raise InvalidInputError(
            parameter="page",
            value=page_int,
            reason="Page number cannot be negative"
        )
    ...
    if size_int > 1000:
        raise InvalidInputError(
            parameter="size",
            value=size_int,
            reason="Page size cannot exceed 1000"
        )
...
class UserFriendlyErrorTranslator:
    ...
    FIELD_HELP = {
        ...
        "metricType": {
            ...
            "examples": ["COST_PER_TRANSACTION", "TOTAL_COST", "TRANSACTION_COUNT", "LATENCY", "ERROR_RATE"]
        },
        "operator": {
            "description": "How to compare the metric value against the threshold",
            "requirements": "Must be a valid comparison operator",
            "examples": ["GREATER_THAN", "LESS_THAN", "GREATER_THAN_OR_EQUAL", "LESS_THAN_OR_EQUAL", "EQUAL"]
        },
        "threshold": {
    ...
    @classmethod
    def translate_error(cls, error_message: str, error_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    @classmethod
    def format_user_friendly_error(cls, error_message: str, error_data: Optional[Dict[str, Any]] = None) -> str:
