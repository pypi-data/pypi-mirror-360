...
class UnifiedResponseFormatter:
    ...
    def _generate_educational_feedback(self, data: Optional[Dict[str, Any]], action: str) -> List[str]:
        ...
        if action in ["create", "create_from_description", "update"]:
            ...
            if "setupFees" in result_data or ("plan" in result_data and "setupFees" in result_data.get("plan", {})):
                ...
                if setup_fees:
                    for setup_fee in setup_fees:
                        if setup_fee.get("type") == "SUBSCRIPTION":
                            feedback.append("📚 **Setup Fee Education**: SUBSCRIPTION type charges per subscription created")
                        elif setup_fee.get("type") == "ORGANIZATION":
                            feedback.append("📚 **Setup Fee Education**: ORGANIZATION type charges once per customer organization")

                        if "flatAmount" in setup_fee:
                            feedback.append("✅ **Structure Update**: Using new 'flatAmount' field format")
            ...
            if "paymentSource" in result_data:
                ...
                if payment_source == "INVOICE_ONLY_NO_PAYMENT":
                    feedback.append("📚 **Payment Source**: Manual invoice payment - customers pay outside system")
                elif payment_source == "EXTERNAL_PAYMENT_NOTIFICATION":
                    feedback.append("📚 **Payment Source**: Tracked payment - external system confirms when paid")
            ...
            if "_setup_fee_validation" in result_data:
                ...
                if validation_info.get("warnings"):
                    feedback.append("⚠️ **Migration Notice**: Old setup fee format detected and converted")
                    feedback.extend([f"💡 {suggestion}" for suggestion in validation_info.get("suggestions", [])[:2]])
