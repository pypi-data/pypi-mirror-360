...
class NLPProcessor:
    ...
    def _build_parameters(self, query: str, intent: Intent, entities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_list_response(self, data: Dict[str, Any], context: Optional[ParsedQuery]) -> str:
        ...
        resource_name = "anomalies" if context and context.resource_type == ResourceType.ANOMALIES else "alerts"
        ...
        # Header
        if total > count:
            response_parts.append(f"I found {total} {resource_name} total. Here are the first {count}:")
        else:
...
class ProductNLPProcessor:
    ...
    def parse_product_request(self, text: str) -> Dict[str, Any]:
        ...
        # Extract product type and pricing model
        product_type = self._extract_product_type(normalized_text)
        ...
        if pricing_info:
            ...
            if "setup_fee" in pricing_info:
                result["plan"]["setupFees"] = [pricing_info["setup_fee"]]  # FIXED: Plan-level setupFees (API requirement)
        ...
        if domain_context:
            ...
            result["_parsing_guidance"]["suggested_metering"] = self._suggest_metering_for_domain(domain_context)

        # Add parsing confidence and suggestions
        result["_parsing_guidance"]["confidence"] = self._calculate_parsing_confidence(result, normalized_text)
        result["_parsing_guidance"]["suggestions"] = self._generate_improvement_suggestions(result, normalized_text)
    ...
    def _extract_payment_source(self, text: str) -> Optional[str]:
        ...
        payment_patterns = {
            "INVOICE_ONLY_NO_PAYMENT": [
                r'\bmanual\s+payment\b', r'\bpay\s+outside\s+system\b', r'\binvoice\s+only\b',
                r'\bno\s+payment\s+tracking\b', r'\bmanual\s+invoice\b', r'\boffline\s+payment\b',
                r'\bexternal\s+payment\b(?!\s+notification)', r'\bpay\s+separately\b'
            ],
            "EXTERNAL_PAYMENT_NOTIFICATION": [
                r'\btrack\s+payments?\b', r'\bpayment\s+confirmation\b', r'\bexternal\s+payment\s+notification\b',
                r'\bpayment\s+tracking\b', r'\bconfirm\s+payment\b', r'\bnotify\s+payment\b',
                r'\bpayment\s+status\s+update\b', r'\bexternal\s+system\s+confirms?\b'
            ]
        }
    ...
    def _extract_pricing_info(self, text: str) -> Dict[str, Any]:
        ...
        setup_fee_patterns = [
            ...
            (r'(?:per\s+subscription|each\s+subscription|subscription)\s+(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)', 'SUBSCRIPTION'),
            (r'(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+subscription|each\s+subscription)', 'SUBSCRIPTION'),
            (r'(?:setup\s+fee|initial\s+cost|onboarding\s+cost|activation\s+charge)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+sub|each\s+sub)', 'SUBSCRIPTION'),

            # Per organization/customer patterns (ORGANIZATION type)
            (r'(?:per\s+(?:customer|organization|client|company|account|tenant)|each\s+(?:customer|organization|client|company|account|tenant))\s+(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)', 'ORGANIZATION'),
            (r'(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+(?:customer|organization|client|company|account|tenant)|each\s+(?:customer|organization|client|company|account|tenant))', 'ORGANIZATION'),
            (r'(?:setup\s+fee|initial\s+cost|onboarding\s+cost|activation\s+charge)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+(?:org|customer|client)|each\s+(?:org|customer|client))', 'ORGANIZATION'),

            # Enhanced patterns for edge cases
            (r'(?:one-time|onetime)\s+(?:setup\s+fee|initial\s+fee|onboarding\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+(?:customer|organization|client|company))', 'ORGANIZATION'),
            (r'(?:one-time|onetime)\s+(?:setup\s+fee|initial\s+fee|onboarding\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+subscription)', 'SUBSCRIPTION'),

            # Implicit setup fee patterns
            (r'(?:implementation|deployment|installation)\s+(?:fee|cost|charge)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+(?:customer|organization|client))', 'ORGANIZATION'),
            (r'(?:implementation|deployment|installation)\s+(?:fee|cost|charge)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)\s+(?:per\s+subscription)', 'SUBSCRIPTION'),

            # Generic setup fee patterns (default to SUBSCRIPTION)
            (r'(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee|implementation\s+fee)\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)', 'SUBSCRIPTION'),
            (r'\$(\d+(?:\.\d{2})?)\s+(?:setup\s+fee|initial\s+fee|onboarding\s+fee|activation\s+fee|implementation\s+fee)', 'SUBSCRIPTION'),
            (r'one[- ]time\s+(?:per\s+(?:customer|organization|client))\s+setup\s+fee\s+(?:of\s+)?\$(\d+(?:\.\d{2})?)', 'ORGANIZATION'),
    ...
    def check_for_old_setup_fee_format(self, text: str) -> Optional[str]:
        ...
        if re.search(r'\bPRODUCT_LICENSE\b', text, re.IGNORECASE):
            ...
            migration_guidance.append("PRODUCT_LICENSE → SUBSCRIPTION (setup fee charged per subscription)")

        if re.search(r'\bCUSTOMER\b.*setup.*fee', text, re.IGNORECASE):
            old_patterns.append("CUSTOMER")
            migration_guidance.append("CUSTOMER → ORGANIZATION (setup fee charged once per customer organization)")
...
**🔧 Your request will be processed with automatic conversion, but please update to the new format for future requests.**"""
    ...
    def _calculate_parsing_confidence(self, result: Dict[str, Any], text: str) -> float:
        ...
        if any(keyword in text.lower() for keyword in ["subscription", "monthly", "usage", "per", "based on"]):
    ...
    def _generate_improvement_suggestions(self, result: Dict[str, Any], text: str) -> List[str]:
        ...
        if not result.get("name"):
            suggestions.append("Consider providing a specific product name for better identification")
        ...
        if result.get("plan", {}).get("rating_aggregations"):
            suggestions.append("This appears to be usage-based billing. You can create the product now, but you'll need to set up metering elements later to track actual usage")
        elif result.get("plan", {}).get("type") == "SUBSCRIPTION" and not result.get("plan", {}).get("rating_aggregations"):
            suggestions.append("This looks like a simple subscription product - no additional setup needed after creation")

        if result.get("plan", {}).get("type") == "SUBSCRIPTION" and not result.get("plan", {}).get("period"):
            suggestions.append("For subscription products, specify the billing period (monthly, yearly, etc.)")
        elif result.get("plan", {}).get("type") == "SUBSCRIPTION":
            suggestions.append("This subscription product will work immediately after creation - no metering elements needed")

        if not result.get("_parsing_guidance", {}).get("business_domain"):
            suggestions.append("Mention the business domain or industry to get better default configurations")
    ...
    def _build_product_type_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "simple": [
                r'\bsimple\b', r'\bbasic\b', r'\bstandard\b', r'\bdefault\b'
            ],
            "subscription": [
                r'\bsubscription\b', r'\brecurring\b', r'\bmonthly\b', r'\byearly\b',
                r'\bperiodic\b', r'\bregular\b'
            ],
            "usage_based": [
                r'\busage[- ]?based\b', r'\bpay[- ]?as[- ]?you[- ]?go\b', r'\bmetered\b',
                r'\bper[- ]?use\b', r'\bvariable\b', r'\bconsumption\b'
            ],
            "hybrid": [
                r'\bhybrid\b', r'\bcombined\b', r'\bmixed\b', r'\bbase.*usage\b'
            ]
        }
    ...
    def _build_pricing_model_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "simple": [
                r'\bflat[- ]?rate\b', r'\bfixed[- ]?price\b', r'\bone[- ]?time\b'
            ],
            "subscription": [
                r'\bsubscription\b', r'\bmonthly\b', r'\byearly\b', r'\brecurring\b',
                r'\bperiodic\b', r'\bregular[- ]?billing\b'
            ],
            "usage_based": [
                r'\bper\s+\w+', r'\bbased\s+on\b', r'\busage\b', r'\bmetered\b',
                r'\bpay[- ]?per[- ]?use\b', r'\bvariable[- ]?pricing\b'
            ],
            "tiered": [
                r'\btiered\b', r'\bvolume[- ]?discount\b', r'\bbulk[- ]?pricing\b',
                r'\bgraduated\b', r'\bscaled\b'
            ]
        }
    ...
    def _build_billing_period_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "MONTH": [
                r'\bmonthly\b', r'\bper[- ]?month\b', r'\bevery[- ]?month\b',
                r'\bmonth\b', r'\b30[- ]?days?\b'
            ],
            "YEAR": [
                r'\byearly\b', r'\bper[- ]?year\b', r'\bannually\b', r'\banual\b',
                r'\bevery[- ]?year\b', r'\byear\b'
            ],
            "WEEK": [
                r'\bweekly\b', r'\bper[- ]?week\b', r'\bevery[- ]?week\b',
                r'\bweek\b', r'\b7[- ]?days?\b'
            ],
            "DAY": [
                r'\bdaily\b', r'\bper[- ]?day\b', r'\bevery[- ]?day\b',
                r'\bday\b', r'\b24[- ]?hours?\b'
            ]
        }
    ...
    def _build_currency_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "USD": [
                r'\$', r'\busd\b', r'\bdollars?\b', r'\bus[- ]?dollars?\b'
            ],
            "EUR": [
                r'€', r'\beur\b', r'\beuros?\b', r'\beuropean?\b'
            ],
            "GBP": [
                r'£', r'\bgbp\b', r'\bpounds?\b', r'\bsterling\b', r'\bbritish\b'
            ],
            "CAD": [
                r'\bcad\b', r'\bcanadian[- ]?dollars?\b'
            ],
            "AUD": [
                r'\baud\b', r'\baustralian[- ]?dollars?\b'
            ],
            "CNY": [
                r'¥', r'\bcny\b', r'\byuan\b', r'\bchinese[- ]?yuan\b', r'\brmb\b'
            ],
            "MXN": [
                r'\bmxn\b', r'\bpesos?\b', r'\bmexican[- ]?pesos?\b'
            ],
            "COP": [
                r'\bcop\b', r'\bcolombian[- ]?pesos?\b'
            ],
            "ARS": [
                r'\bars\b', r'\bargentine[- ]?pesos?\b', r'\bargentinian[- ]?pesos?\b'
            ],
            "ZMW": [
                r'\bzmw\b', r'\bkwacha\b', r'\bzambian[- ]?kwacha\b'
            ]
        }
    ...
    def _build_aggregation_type_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "COUNT": [
                r'\bcount\b', r'\bnumber[- ]?of\b', r'\btotal\b', r'\bquantity\b',
                r'\beach\b', r'\bper[- ]?item\b', r'\bper[- ]?unit\b'
            ],
            "SUM": [
                r'\bsum\b', r'\btotal[- ]?amount\b', r'\bvolume\b', r'\bsize\b',
                r'\bweight\b', r'\bmass\b', r'\bcumulative\b'
            ],
            "AVERAGE": [
                r'\baverage\b', r'\bmean\b', r'\btypical\b', r'\bavg\b'
            ],
            "MAXIMUM": [
                r'\bmaximum\b', r'\bmax\b', r'\bhighest\b', r'\bpeak\b'
            ],
            "MINIMUM": [
                r'\bminimum\b', r'\bmin\b', r'\blowest\b', r'\bbase\b'
            ]
        }
    ...
    def _build_business_domain_mappings(self) -> Dict[str, List[str]]:
        ...
        return {
            "shipping": [
                r'\bshipping\b', r'\bdelivery\b', r'\blogistics\b', r'\bpackage\b',
                r'\bfreight\b', r'\bcourier\b', r'\btransport\b'
            ],
            "api": [
                r'\bapi\b', r'\brest\b', r'\bwebservice\b', r'\bendpoint\b',
                r'\bmicroservice\b', r'\bintegration\b'
            ],
            "saas": [
                r'\bsaas\b', r'\bsoftware[- ]?as[- ]?a[- ]?service\b', r'\bcloud[- ]?software\b',
                r'\bweb[- ]?app\b', r'\bplatform\b'
            ],
            "ecommerce": [
                r'\becommerce\b', r'\be[- ]?commerce\b', r'\bonline[- ]?store\b',
                r'\bshopping\b', r'\bretail\b', r'\bmarketplace\b'
            ],
            "fintech": [
                r'\bfintech\b', r'\bfinancial\b', r'\bbanking\b', r'\bpayment\b',
                r'\bbilling\b', r'\binvoicing\b', r'\baccounting\b'
            ],
            "healthcare": [
                r'\bhealthcare\b', r'\bmedical\b', r'\bhospital\b', r'\bclinic\b',
                r'\bpatient\b', r'\bhealth\b'
            ],
            "education": [
                r'\beducation\b', r'\blearning\b', r'\bschool\b', r'\buniversity\b',
                r'\bcourse\b', r'\btraining\b', r'\bacademic\b'
            ]
        }
