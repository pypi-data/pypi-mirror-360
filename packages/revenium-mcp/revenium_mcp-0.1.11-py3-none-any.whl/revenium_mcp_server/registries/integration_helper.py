...
async def manage_products_via_registry(request: ProductRequest) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
...
async def manage_customers_via_registry(request: CustomerRequest) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    """Manage customers via business registry (≤25 lines, ≤3 params)."""
    registry = get_business_registry()
    return await registry.manage_customers(request)


async def manage_subscriptions_via_registry(request: SubscriptionRequest) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    """Manage subscriptions via business registry (≤25 lines, ≤3 params)."""
    registry = get_business_registry()
    return await registry.manage_subscriptions(request)


async def manage_sources_via_registry(request: SourceRequest) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
