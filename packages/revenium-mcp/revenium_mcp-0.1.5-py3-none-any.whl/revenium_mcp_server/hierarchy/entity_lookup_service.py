...
class IDValidator:
    ...
    NAME_INDICATORS = {
        ...
        "common_words": re.compile(r'\b(api|platform|service|suite|analytics|monitor|enterprise|pro|basic|premium|free|tier|plan)\b', re.IGNORECASE),
...
class EntityLookupService:
    ...
    async def resolve_product(self, identifier: str, strategy: str = "auto") -> Optional[EntityReference]:
    ...
    async def resolve_subscription(self, identifier: str, strategy: str = "auto") -> Optional[EntityReference]:
    ...
    async def resolve_credential(self, identifier: str, strategy: str = "auto") -> Optional[EntityReference]:
    ...
    async def resolve_subscriber(self, identifier: str, strategy: str = "auto") -> Optional[EntityReference]:
    ...
    async def resolve_organization(self, identifier: str, strategy: str = "auto") -> Optional[EntityReference]:
    ...
    async def bulk_resolve(self, entities: List[Tuple[str, str]]) -> Dict[str, Optional[EntityReference]]:
    ...
    async def fuzzy_search(self, entity_type: str, query: str, limit: int = 10) -> List[EntityReference]:
        ...
        try:
            ...
            if result.success and result.matches:
                ...
                for i, match in enumerate(result.matches[:limit]):
                    confidence = result.confidence_scores[i] if i < len(result.confidence_scores) else 0.5
    ...
    async def _resolve_entity(self, entity_type: str, identifier: str, strategy: str) -> LookupResult:
        ...
        try:
            ...
            logger.error(f"Error resolving {entity_type} {identifier} with strategy {strategy}: {e}")
            return LookupResult(
                success=False,
                entity_type=entity_type,
                query=identifier,
        ...
        try:
            ...
            if not IDValidator.is_valid_object_id(identifier):
                ...
                if classification == "name":
                    ...
                    logger.info(f"Identifier '{identifier}' appears to be a name, redirecting to name search")
                    return await self._lookup_by_name(entity_type, identifier)
                elif classification == "email":
                    # Redirect to email-based search
                    logger.info(f"Identifier '{identifier}' appears to be an email, redirecting to email search")
        ...
        try:
            ...
            for entity in entities:
                ...
                for field in entity_config["search_fields"]:
                    ...
                    if field_value:
                        score = SequenceMatcher(None, identifier.lower(), str(field_value).lower()).ratio()
                        if score > max_score:
                            max_score = score
            ...
            if matches:
                sorted_pairs = sorted(zip(matches, confidence_scores), key=lambda x: x[1], reverse=True)
    ...
    async def _lookup_auto_strategy(self, entity_type: str, identifier: str) -> LookupResult:
        ...
        try:
            ...
            logger.info(f"Auto strategy for '{identifier}': classified as '{classification}', trying strategies: {strategies_to_try}")
            ...
            for strategy in strategies_to_try:
                try:
                    ...
                    logger.warning(f"Auto strategy {strategy} failed for {entity_type} {identifier}: {e}")
                    continue
    ...
    async def _lookup_by_search(self, entity_type: str, identifier: str, field_type: str) -> LookupResult:
        ...
        try:
            ...
            logger.error(f"Error in search lookup for {entity_type} {identifier} by {field_type}: {e}")
            return LookupResult(
                success=False,
                entity_type=entity_type,
                query=identifier,
