...
from .models import (
    PaginationParams, FilterParams, SortField, PaginatedResponse,
    PaginationMetadata, FilterCondition, FilterOperator
)
...
class PaginationHelper:
    ...
    def build_query_params(self, pagination: PaginationParams, filters: Optional[FilterParams] = None) -> Dict[str, Any]:
    ...
    def create_filter_from_dict(self, filter_dict: Dict[str, Any]) -> FilterParams:
        ...
        if "tags" in filter_dict:
            filters.tags = filter_dict["tags"] if isinstance(filter_dict["tags"], list) else [filter_dict["tags"]]
        
        # Handle date filters
        if "created_after" in filter_dict:
            if isinstance(filter_dict["created_after"], str):
                filters.created_after = datetime.fromisoformat(filter_dict["created_after"].replace("Z", "+00:00"))
        ...
        if "created_before" in filter_dict:
            if isinstance(filter_dict["created_before"], str):
                filters.created_before = datetime.fromisoformat(filter_dict["created_before"].replace("Z", "+00:00"))
    ...
    def format_paginated_response_text(self, response: PaginatedResponse, item_formatter: callable) -> str:
