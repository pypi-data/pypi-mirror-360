...
class ConfigurationStatus(ToolBase):
    ...
    tool_description: ClassVar[str] = "Comprehensive configuration status and diagnostic analysis. Key actions: environment_variables, auto_discovery, system_health. Use get_examples() for diagnostic guidance and get_capabilities() for complete action list."
    ...
    async def _handle_onboarding_status(self, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        onboarding_text = self._build_onboarding_status_analysis(onboarding_state, validation_result)
    ...
    def _build_environment_variables_analysis(self, validation_result) -> str:
        ...
        for category, vars_list in variables_by_category.items():
            ...
            for var_name, var_data in vars_list:
                ...
                if var_name == "REVENIUM_API_KEY":
                    analysis += f"- Importance: 🔴 Critical - Required for all API operations\n"
                elif var_name == "REVENIUM_TEAM_ID":
                    analysis += f"- Importance: 🔴 Critical - Required for team access\n"
                elif var_name == "REVENIUM_DEFAULT_EMAIL":
                    analysis += f"- Importance: 🟡 Recommended - Used for alert notifications\n"
                elif var_name == "REVENIUM_DEFAULT_SLACK_CONFIG_ID":
                    analysis += f"- Importance: 🟡 Recommended - Used for Slack notifications\n"
                else:
                    analysis += f"- Importance: 🟢 Optional - Enhances functionality\n"

                analysis += "\n"

        # Summary statistics using effective configuration values
        config_vars = ["REVENIUM_API_KEY", "REVENIUM_TEAM_ID", "REVENIUM_TENANT_ID",
        ...
        if set_vars < total_vars:
            ...
            analysis += f"Use `welcome_and_setup(action='environment_status')` for detailed guidance.\n"
    ...
    def _build_auto_discovery_analysis(self, validation_result) -> str:
        ...
        if discovered_status == 'success':
            ...
            analysis += f"## ✅ **Auto-Discovery Status: WORKING**\n\n"
            ...
            effectiveness = (actually_discovered / total_discoverable * 100) if total_discoverable > 0 else 0

            analysis += f"## 📊 **Discovery Effectiveness**\n\n"
            ...
            if effectiveness >= 80:
                ...
                analysis += "⚠️ **Limited**: Auto-discovery has limited effectiveness.\n"

        else:
            analysis += f"## ❌ **Auto-Discovery Status: FAILED**\n\n"
        ...
        if discovered_status != 'success':
            analysis += "## 🛠️ **Manual Configuration Alternative**\n\n"
    ...
    def _build_onboarding_status_analysis(self, onboarding_state, validation_result) -> str:
        ...
        analysis += f"- **Cache Exists**: {'✅ Yes' if onboarding_state.cache_exists else '❌ No'}\n"
        ...
        for item_name, is_complete in completion_items:
            ...
            analysis += f"- {status_icon} **{item_name}**: {'Complete' if is_complete else 'Incomplete'}\n"
            if is_complete:
                completed_count += 1
    ...
    def _build_system_health_summary(self, validation_result, onboarding_state) -> str:
        ...
        overall_status = validation_result.summary.get('overall_status', False)
        health_icon = "💚" if overall_status else "⚠️"
        ...
        for check_name, is_healthy in critical_checks:
            ...
            summary += f"- {status_icon} **{check_name}**: {'Operational' if is_healthy else 'Failed'}\n"
            if is_healthy:
                critical_healthy += 1
        ...
        weighted_health_percentage = (critical_score * critical_weight + optional_score * optional_weight) * 100
        ...
        summary += f"- Critical: {critical_healthy}/{len(critical_checks)} ({'✅' if critical_healthy == len(critical_checks) else '❌'})\n"
        summary += f"- Optional: {optional_healthy}/{len(optional_checks)} ({'✅' if optional_healthy == len(optional_checks) else '⚠️'})\n\n"
        ...
        if weighted_health_percentage >= 90:
            ...
            summary += "✅ **Functional**: Core system is operational, optional features may be limited.\n"
        elif weighted_health_percentage >= 50:
            summary += "⚠️ **Fair**: System has some health issues that should be addressed.\n"
        else:
            summary += "🚨 **Poor**: System has significant health issues requiring immediate attention.\n"
        ...
        if not validation_result.summary.get('direct_api_works'):
            summary += "🔴 **Critical**: API connectivity failed - check network and authentication\n"
        ...
        if not validation_result.summary.get('auto_discovery_works'):
            summary += "🟡 **Warning**: Auto-discovery not working - manual configuration required\n"
        if not onboarding_state.cache_valid:
            if onboarding_state.is_first_time:
                summary += "🟢 **Info**: Cache system not initialized - normal for first-time users\n"
            else:
        ...
        if not overall_status:
            ...
            summary += "1. Use `configuration_status(action='full_diagnostic')` for detailed analysis\n"
        ...
        if overall_status:
            ...
            summary += "⚠️ **Not Ready**: System requires configuration before use\n"
