...
class ProductErrorHandler:
    ...
    def _create_enhanced_error(self, error_info: Dict[str, Any], api_error: str, product_data: Dict[str, Any]) -> ProductError:
    ...
    def _create_generic_error(self, api_error: str, product_data: Dict[str, Any]) -> ProductError:
        ...
        return ProductError(
            ...
            common_causes=["Missing required fields", "Invalid field values", "API connectivity issues"]
        )
    ...
    def _validate_product_name_business(self, name: Optional[str]) -> List[ProductError]:
        ...
        if not name:
            ...
            errors.append(ProductError(
                ...
                examples=[
                    {"current": name, "suggested": f"{name} Service"}
                ],
    ...
    def _validate_pricing_consistency(self, product_data: Dict[str, Any]) -> List[ProductError]:
        ...
        if "tiers" in plan:
            all_zero = all(
                float(tier.get("unit_amount", 0)) == 0
                for tier in plan["tiers"]
            )

            if all_zero and not any(keyword in product_data.get("name", "").lower()
    ...
    def _build_error_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            r"invalid.*currency": {
                ...
                "examples": [
                    {"valid": "USD"}, {"valid": "EUR"}, {"invalid": "US"}
                ]
            }
        }
