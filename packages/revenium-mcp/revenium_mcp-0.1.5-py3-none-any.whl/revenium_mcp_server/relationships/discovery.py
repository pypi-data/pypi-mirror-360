...
from .graph import ResourceGraph, ResourceNode, RelationshipEdge, RelationshipType, RelationshipStrength
...
class ResourceRelationshipDiscovery:
    ...
    def _create_node_from_data(self, resource_type: str, resource_data: Dict[str, Any]) -> ResourceNode:
    ...
    def _determine_relationship_strength(self, relationship_type: RelationshipType) -> RelationshipStrength:
    ...
    def add_resource(self, resource_type: str, resource_data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Added resource {resource_type}:{node.resource_id} with {len(relationships)} relationships")
    ...
    def get_resource_relationships(
        ...
        return {
            ...
            "related_resources": {
                "owns": [edge.target_node.to_dict() for edge in outgoing_edges 
                        if edge.relationship_type == RelationshipType.OWNS],
                "belongs_to": [edge.target_node.to_dict() for edge in outgoing_edges 
                              if edge.relationship_type == RelationshipType.BELONGS_TO],
                "references": [edge.target_node.to_dict() for edge in outgoing_edges 
                              if edge.relationship_type == RelationshipType.REFERENCES],
                "manages": [edge.target_node.to_dict() for edge in outgoing_edges 
