...
class ProfitabilityAnalyticsProcessor:
    ...
    async def analyze_profitability(
        ...
        """
        logger.info(f"Analyzing profitability for team {team_id}, period: {period}, entity: {entity_type}")
        ...
        try:
            ...
            logger.info(f"Profitability analysis complete. Net profit: ${profitability_data.net_profit:.2f}")
    ...
    async def _fetch_revenue_data(
        ...
        if entity_type in ["customers", "both"]:
            tasks["revenue_by_organization"] = client._request_with_retry(
                "GET",
                self.revenue_endpoints["revenue_metric_by_organization"],
                params=params
            )
        ...
        if entity_type in ["products", "both"]:
            tasks["revenue_by_product"] = client._request_with_retry(
                "GET",
                self.revenue_endpoints["revenue_metric_by_product"],
                params=params
            )
            tasks["percentage_revenue_by_product"] = client._request_with_retry(
                "GET",
                self.revenue_endpoints["percentage_revenue_metric_by_product"],
                params=params
            )
    ...
    def _process_profitability_data(
        ...
        if total_revenue == 0 and total_cost > 0:
            logger.info(f"Revenue tracking not enabled (total_revenue: ${total_revenue:.2f}, total_cost: ${total_cost:.2f})")
            logger.info("Revenue analysis will be available once usage-based billing is enabled - cost analysis is fully functional")

        # Calculate customer profitability
        for customer, data in profitability_by_customer.items():
            data["profit"] = data["revenue"] - data["cost"]
            data["margin"] = (data["profit"] / data["revenue"] * 100) if data["revenue"] > 0 else 0.0
    ...
    def _get_top_performers(
        self,
        profitability_data: Dict[str, Dict[str, float]],
        entity_type: str,
        top_n: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    def _process_customer_profitability(
        ...
        for customer_id, data in customer_data.items():
            ...
            customers.append(CustomerProfitability(
    ...
    def _process_product_profitability(
        ...
        for product_id, data in product_data.items():
            ...
            products.append(ProductProfitability(
