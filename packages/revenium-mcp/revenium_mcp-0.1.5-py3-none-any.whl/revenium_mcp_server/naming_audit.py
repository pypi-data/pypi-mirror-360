...
from .naming_standards import naming_standards, parameter_validator
...
class NamingAudit:
    ...
    def audit_all_tools(self) -> Dict[str, Any]:
        ...
        self.audit_results = {
            ...
            "standardization_recommendations": self._generate_standardization_recommendations(audit_results)
        }
    ...
    def _calculate_overall_metrics(self, audit_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tools_with_standard_patterns = sum(
            1 for tool in audit_results.values() 
            if all(tool["follows_standard_patterns"].values())
        )
        ...
        return {
            ...
            "overall_compliance_percentage": (total_compliant / total_params) * 100 if total_params > 0 else 0,
            "tools_with_standard_patterns": tools_with_standard_patterns,
            "total_tools": len(audit_results),
            "pattern_compliance_percentage": (tools_with_standard_patterns / len(audit_results)) * 100
        }
    
    def _generate_standardization_recommendations(self, audit_results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def generate_compliance_report(self) -> str:
        ...
        if self.audit_results["standardization_recommendations"]:
            ...
            for i, rec in enumerate(self.audit_results["standardization_recommendations"], 1):
                report += f"{i}. **{rec['type']}** ({rec['priority']} priority): {rec['description']}\n"
