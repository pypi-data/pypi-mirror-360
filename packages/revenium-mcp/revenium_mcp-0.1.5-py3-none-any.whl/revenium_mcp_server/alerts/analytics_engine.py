...
from ..models import PaginationParams, FilterParams
...
class AnalyticsEngine:
    ...
    async def get_anomaly_frequency_analysis(
        ...
        filters = FilterParams(
            created_after=time_range.start,
            created_before=time_range.end
        )
    ...
    async def get_alert_trend_analysis(
        ...
        for alert in all_alerts:
            if alert.get("resolved_at") and alert.get("trigger_timestamp"):
                try:
                    trigger_time = datetime.fromisoformat(alert["trigger_timestamp"].replace("Z", "+00:00"))
    ...
    def _analyze_creation_trend(self, anomalies: List[Dict[str, Any]], time_range: TimeRange) -> List[Dict[str, Any]]:
    ...
    def _analyze_alert_frequency(self, alerts: List[Dict[str, Any]], time_range: TimeRange) -> List[Dict[str, Any]]:
