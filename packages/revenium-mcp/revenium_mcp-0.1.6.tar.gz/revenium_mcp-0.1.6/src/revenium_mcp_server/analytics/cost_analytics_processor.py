...
class CostAnalyticsProcessor:
    ...
    async def analyze_cost_trends(
        ...
        """
        logger.info(f"Analyzing cost trends for team {team_id}, period: {period}, intent: {query_intent}")
    ...
    async def analyze_cost_spike(
        ...
        try:
            ...
            spike_analysis = self._identify_spike_contributors(
                spike_data, baseline_data, threshold
            )
            ...
            if spike_cost_data:
                ...
                spike_trend_data = self._process_cost_trend_data(spike_cost_data, spike_period, "spike_investigation")

                # Add the unified percentage calculation to the spike analysis
                spike_analysis["unified_percentage_change"] = spike_trend_data.period_over_period_change
                ...
                logger.info(f"SPIKE_CONSISTENCY_FIX: Unified percentage change: {spike_trend_data.period_over_period_change:.2f}%")
    ...
    async def get_cost_breakdown(
        ...
        endpoint_map = {
            "provider": self.cost_endpoints["total_cost_by_provider_over_time"],  # Use verified endpoint
    ...
    async def _fetch_cost_data(
        ...
        tasks = {
            ...
            "cost_by_product": client._request_with_retry(
                ...
                params={"teamId": team_id, "period": period, "group": "TOTAL"}  # Add group parameter to match UI
            )
        }
    ...
    def _process_cost_trend_data(
        ...
        try:
            ...
            provider_responses = [provider_data] if isinstance(provider_data, dict) else provider_data if isinstance(provider_data, list) else []
            ...
            for i, time_period in enumerate(provider_responses):
                ...
                for j, group_data in enumerate(groups):
                    ...
                    provider = self._normalize_entity_name(group_data.get("groupName", "Unknown"), "provider")
                ...
                if start_timestamp and period_total > 0:
                    period_totals[start_timestamp] = period_totals.get(start_timestamp, 0) + period_total
        except Exception as e:
        ...
        try:
            # Handle both single dict and list of dicts response formats
            model_responses = [model_data] if isinstance(model_data, dict) else model_data if isinstance(model_data, list) else []
            ...
            for i, time_period in enumerate(model_responses):
                ...
                for j, group_data in enumerate(groups):
                    ...
                    model = self._normalize_entity_name(group_data.get("groupName", "Unknown"), "model")
                    metrics = group_data.get("metrics", [])
        ...
        try:
            # Handle both single dict and list of dicts response formats
            customer_responses = [customer_data] if isinstance(customer_data, dict) else customer_data if isinstance(customer_data, list) else []

            for i, time_period in enumerate(customer_responses):
                if not isinstance(time_period, dict):
                    logger.warning(f"Expected dict at customer_data[{i}], got {type(time_period).__name__}")
                    continue
                ...
                for j, group_data in enumerate(groups):
                    ...
                    customer = self._normalize_entity_name(group_data.get("groupName", "Unknown"), "customer")
                    metrics = group_data.get("metrics", [])
                    ...
                    for k, metric in enumerate(metrics):
                        ...
                        metric_result = metric.get("metricResult", 0)
                        if isinstance(metric_result, (int, float)):
                            cost_by_customer[customer] = cost_by_customer.get(customer, 0) + metric_result
        except Exception as e:
        ...
        try:
            # Handle both single dict and list of dicts response formats
            product_responses = [product_data] if isinstance(product_data, dict) else product_data if isinstance(product_data, list) else []

            for i, time_period in enumerate(product_responses):
                if not isinstance(time_period, dict):
                    logger.warning(f"Expected dict at product_data[{i}], got {type(time_period).__name__}")
                    continue
                ...
                for j, group_data in enumerate(groups):
                    ...
                    product = self._normalize_entity_name(group_data.get("groupName", "Unknown"), "product")
                    metrics = group_data.get("metrics", [])
                    ...
                    for k, metric in enumerate(metrics):
                        ...
                        metric_result = metric.get("metricResult", 0)
                        if isinstance(metric_result, (int, float)):
                            cost_by_product[product] = cost_by_product.get(product, 0) + metric_result
        except Exception as e:
        ...
        logger.info(f"PERCENTAGE_FIX_DEBUG: Provider data type: {type(cost_data.get('cost_by_provider_over_time', 'MISSING'))}")

        # Calculate metrics based on period
        average_cost_per_request = self._calculate_average_cost_per_request(cost_data)
        period_over_period_change = self._calculate_period_change(cost_by_period, period, query_intent)
    ...
    def _calculate_period_change(self, cost_by_period: List[Dict[str, Any]], period: str, query_intent: str = "cost_analysis") -> float:
        ...
        """
        logger.debug(f"Calculating period change for {period} with {len(cost_by_period)} data points, intent: {query_intent}")
    ...
    def _calculate_trend_change(self, sorted_periods: List[Dict[str, Any]]) -> float:
        ...
        if len(sorted_periods) <= 3:
            ...
            logger.info(f"PERCENTAGE_FIX_DEBUG: Small dataset - first_cost={first_cost}, last_cost={last_cost}")

            if first_cost > 0:
                percentage_change = ((last_cost - first_cost) / first_cost) * 100
                logger.info(f"PERCENTAGE_FIX_DEBUG: Calculated percentage change: {percentage_change:.2f}%")
        ...
        logger.info(f"PERCENTAGE_FIX_DEBUG: Robust calculation - baseline_median={baseline_median}, recent_median={recent_median}, periods={len(sorted_periods)}")

        if baseline_median > 0:
            percentage_change = ((recent_median - baseline_median) / baseline_median) * 100
            logger.info(f"PERCENTAGE_FIX_DEBUG: Calculated robust percentage change: {percentage_change:.2f}%")
            return percentage_change
    ...
    def _determine_trend_direction(self, cost_by_period: List[Dict[str, Any]]) -> str:
        ...
        if len(sorted_periods) >= 2:
            ...
            change_percentage = abs(((last_cost - first_cost) / first_cost) * 100) if first_cost > 0 else 0
    ...
    def _identify_spike_contributors(
        ...
        logger.info(f"Using adaptive detection threshold: ${detection_threshold:.2f} (total cost: ${total_cost:.2f})")
        ...
        if isinstance(provider_data, list):
            ...
            for time_period in provider_data:
                ...
                for group in time_period["groups"]:
                    ...
                    provider_name = self._normalize_entity_name(group.get("groupName", "Unknown"), "provider")
                    metrics = group.get("metrics", [])
        ...
        if isinstance(customer_data, dict) and "groups" in customer_data:
            for group in customer_data["groups"]:
                ...
                customer_name = self._normalize_entity_name(group.get("groupName", "Unknown"), "customer")
                metrics = group.get("metrics", [])
        ...
        if isinstance(product_data, dict) and "groups" in product_data:
            for group in product_data["groups"]:
                ...
                product_name = self._normalize_entity_name(group.get("groupName", "Unknown"), "product")
                metrics = group.get("metrics", [])
    ...
    def _process_breakdown_data(
        ...
        try:
            ...
            if isinstance(response, dict) and "groups" in response:
                ...
                if not isinstance(groups, list):
                    ...
                    for j, group_data in enumerate(groups):
                        ...
                        group_name = self._normalize_entity_name(group_data.get("groupName", "Unknown"), breakdown_type)
                        metrics = group_data.get("metrics", [])
            ...
            # Handle legacy API response structure: array with groups → metrics → metricResult
                ...
                logger.warning(f"Unexpected response structure, got {type(response).__name__}: {str(response)[:200]}...")
    ...
    def _calculate_spike_trends(self, time_based_analysis: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for entry in time_based_analysis:
            ...
            entity_trends[entity_key].append({
                "timestamp": timestamp,
                "cost": cost
            })
        ...
        if len(total_cost_by_hour) >= 2:
            ...
            first_half_avg = sum(total_cost_by_hour[t] for t in timestamps[:len(timestamps)//2]) / max(1, len(timestamps)//2)
            second_half_avg = sum(total_cost_by_hour[t] for t in timestamps[len(timestamps)//2:]) / max(1, len(timestamps) - len(timestamps)//2)
    ...
    def _generate_baseline_comparison_summary(self, contributors: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        increases = [c.get("percentage_increase", 0) for c in contributors if c.get("increase", 0) > 0]
        decreases = [c.get("percentage_increase", 0) for c in contributors if c.get("increase", 0) < 0]
        ...
        return {
            ...
            "baseline_comparison_available": any(c.get("baseline_cost", 0) > 0 for c in contributors)
        }
