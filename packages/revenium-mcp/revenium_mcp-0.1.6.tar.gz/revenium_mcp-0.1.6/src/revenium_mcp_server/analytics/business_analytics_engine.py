...
class BusinessAnalyticsEngine:
    ...
    def __init__(self, ucm_integration: Optional['AnalyticsUCMIntegration'] = None):
        ...
        if ucm_integration:
            ...
            logger.warning("BusinessAnalyticsEngine initialized without UCM integration (using static capabilities)")
    ...
    async def _validate_query(self, query: AnalyticsQuery) -> None:
        ...
        # Validate query type
        supported_query_types = capabilities.get("query_types", [])
        logger.debug(f"Validating query type '{query.query_type}' against supported types: {supported_query_types}")

        if query.query_type not in supported_query_types:
            logger.error(f"Query type '{query.query_type}' not in supported types: {supported_query_types}")
    ...
    async def _get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            "query_types": ["cost_analysis", "profitability", "comparison", "trend", "breakdown", "spike_investigation", "transaction_level"],
            "entity_types": ["products", "customers", "models", "providers", "agents", "transactions", "tasks", "performance_metrics", "cost_metrics", "numerical_quantity", "numerical_quantities"],
            "supported_periods": ["SEVEN_DAYS", "THIRTY_DAYS", "TWELVE_MONTHS", "HOUR", "EIGHT_HOURS", "TWENTY_FOUR_HOURS"],
            "supported_aggregations": ["TOTAL", "MEAN", "MAXIMUM", "MINIMUM", "MEDIAN"],
            "chart_types": ["line", "bar", "pie", "area"],
            "data_sources": ["cost_trends", "cost_breakdown", "profitability", "period_comparison", "custom"]
        }
    ...
    async def _process_profitability_analysis(
        ...
        try:
            ...
            if entity_type in ["customers", "both"]:
                customer_profitability = await profitability_processor.analyze_customer_profitability(
            ...
            recommendations = await self._generate_profitability_recommendations(profitability_data, period)
        ...
        try:
            ...
            if comparison_type == "period":
                ...
                comparison_result = await comparative_processor.compare_periods(
                    client, client.team_id, current_period, previous_period, metric_type, breakdown_by
                )
                ...
                return {
                    ...
                    "api_calls_count": getattr(comparison_result, 'metadata', {}).get("api_calls_made", 2),
                    "status": "completed"
                }
                ...
                return {
                    ...
                    "api_calls_count": getattr(comparison_result, 'metadata', {}).get("api_calls_made", 1),
                    "status": "completed"
                }
    ...
    async def _process_breakdown_analysis(
        ...
        try:
            ...
            if has_agent_intent:
                ...
                from .cost_analytics_processor import CostAnalyticsProcessor
                cost_processor = CostAnalyticsProcessor()
                cost_trends = await cost_processor.analyze_cost_trends(client, client.team_id, period, aggregation, "breakdown")
            ...
            recommendations = await self._generate_breakdown_recommendations(breakdown_results, period)
    ...
    async def _process_transaction_level_analysis(
        ...
        """
        logger.info(f"Processing transaction-level analysis query for {len(query.entities)} entities")
        ...
        try:
            ...
            if query.context and "intent" in query.context:
                ...
                if isinstance(intent_value, str):
                    has_customer_intent = has_customer_intent or "customer" in intent_value or "organization" in intent_value
                    ...
                    logger.warning(f"Intent value in context is not a string: {type(intent_value).__name__}")
            ...
            if has_customer_intent:
                ...
                has_profitability_intent = any(entity in query.entities for entity in profitability_entities)
                if has_profitability_intent or (query.context and "profitability" in str(query.context)):
                    customer_profitability = await transaction_processor.analyze_customer_profitability(
                        client, client.team_id, period, top_n=10
                    )
                ...
                has_profitability_intent = any(entity in query.entities for entity in profitability_entities)
                if has_profitability_intent or (query.context and "profitability" in str(query.context)):
                    product_profitability = await transaction_processor.analyze_product_profitability(
                        client, client.team_id, period, top_n=10
                    )
                ...
                has_performance_intent = any(entity in query.entities for entity in performance_entities)
                if has_performance_intent or (query.context and "performance" in str(query.context)):
            ...
            if "customer_analytics" in analysis_results:
                insights = await self._generate_customer_analytics_insights(analysis_results["customer_analytics"], period)
                recommendations = await self._generate_customer_analytics_recommendations(analysis_results["customer_analytics"], period)
            elif "product_analytics" in analysis_results:
                insights = await self._generate_product_analytics_insights(analysis_results["product_analytics"], period)
                recommendations = await self._generate_product_analytics_recommendations(analysis_results["product_analytics"], period)
            elif "agent_analytics" in analysis_results:
                insights = await self._generate_agent_analytics_insights(analysis_results["agent_analytics"], period)
                recommendations = await self._generate_agent_analytics_recommendations(analysis_results["agent_analytics"], period)
            elif "task_analytics" in analysis_results:
                insights = await self._generate_task_analytics_insights(analysis_results["task_analytics"], period)
                recommendations = await self._generate_task_analytics_recommendations(analysis_results["task_analytics"], period)
            else:
                insights = await self._generate_transaction_level_insights(analysis_results["summary_analytics"], period)
                recommendations = await self._generate_transaction_level_recommendations(analysis_results["summary_analytics"], period)
            ...
            for key, value in analysis_results.items():
                ...
                if hasattr(value, '__dict__') and hasattr(value, 'total_cost'):
                    ...
                    processed_results[key] = {
                        ...
                        "average_cost_per_transaction": getattr(value, 'average_cost_per_transaction', 0.0),
    ...
    async def _generate_cost_insights(self, cost_trends: Any, period: str) -> List[str]:
        ...
        if total_cost > 0:
            insights.append(f"💰 Total cost for {period.replace('_', ' ').lower()}: ${total_cost:.2f}")
        ...
        if trend_direction == "increasing" and period_change > 10:
            insights.append(f"📈 Significant cost increase detected: {period_change:+.1f}% change within analysis period")
        elif trend_direction == "decreasing" and period_change < -10:
            insights.append(f"📉 Notable cost reduction achieved: {period_change:+.1f}% change within analysis period")
        elif trend_direction == "stable":
            insights.append(f"📊 Cost trends are stable with {period_change:+.1f}% change within analysis period")

        # Add clarification about what the percentage represents
        if abs(period_change) > 50:
            insights.append(f"ℹ️ Note: {period_change:+.1f}% represents change from beginning to end of the {period.replace('_', ' ').lower()} analysis window")
        ...
        if cost_by_provider:
            ...
            insights.append(f"🏢 Top cost driver: {top_provider[0]} (${top_provider[1]:.2f}, {provider_percentage:.1f}%)")
        ...
        if cost_by_model:
            ...
            insights.append(f"🤖 Most expensive model: {top_model[0]} (${top_model[1]:.2f}, {model_percentage:.1f}%)")
    ...
    async def _generate_cost_recommendations(self, cost_trends: Any, period: str) -> List[str]:
        ...
        if trend_direction == "increasing" and period_change > 20:
            recommendations.append("🚨 Consider implementing cost controls - costs increased significantly")
        ...
        if cost_by_provider:
            ...
            if len(sorted_providers) > 1:
                ...
                if top_provider_cost > second_provider_cost * 2:
                    recommendations.append(f"💡 Consider diversifying from {sorted_providers[0][0]} to reduce cost concentration")

        # Model-based recommendations
        if cost_by_model:
            sorted_models = sorted(cost_by_model.items(), key=lambda x: x[1], reverse=True)
            if sorted_models:
                expensive_models = [model for model, cost in sorted_models if cost > 100]  # $100+ models
                if expensive_models:
                    recommendations.append("🔍 Review usage of high-cost models for optimization opportunities")
    ...
    async def _generate_spike_insights(self, spike_analysis: Dict[str, Any], period: str) -> List[str]:
        ...
        try:
            ...
            if contributors:
                ...
                if unified_percentage is not None and debug_marker:
                    ...
                    insights.append(f"🏆 Primary spike driver: {top_contributor.get('name', 'Unknown')} ({top_contributor.get('type', 'unknown')})")
                    insights.append(f"💰 Spike cost: ${spike_cost:.2f} (baseline: ${baseline_cost:.2f})")
                    insights.append(f"📈 Cost increase: ${increase:.2f} ({percentage_display:+.1f}%) [UNIFIED CALCULATION]")
                else:
                    # Fallback to old logic if unified calculation not available
                    percentage_increase = top_contributor.get("percentage_increase", 0)
                    insights.append(f"🏆 Primary spike driver: {top_contributor.get('name', 'Unknown')} ({top_contributor.get('type', 'unknown')})")
                    insights.append(f"💰 Spike cost: ${spike_cost:.2f} (baseline: ${baseline_cost:.2f})")
                    insights.append(f"📈 Cost increase: ${increase:.2f} ({percentage_increase:+.1f}%) [OLD LOGIC - FIX NOT APPLIED]")

                # Provider-specific insights
                provider_contributors = [c for c in contributors if c.get("type") == "provider"]
                if provider_contributors:
                    insights.append(f"🏢 {len(provider_contributors)} provider(s) contributed to spike")
                ...
                if customer_contributors:
                    insights.append(f"👥 {len(customer_contributors)} customer(s) contributed to spike")
    ...
    async def _generate_spike_recommendations(self, spike_analysis: Dict[str, Any], period: str) -> List[str]:
        ...
        try:
            ...
            if contributors:
                ...
                if contributor_type == "provider":
                    recommendations.append(f"🏢 Investigate {contributor_name} provider usage patterns")
                ...
                recommendations.append("✅ No immediate action required - spike within normal variance")
            ...
            recommendations.append("🔍 Review spike patterns and implement monitoring best practices")

        return recommendations

    async def _generate_profitability_insights(
        self,
        profitability_data,
        period: str
    ) -> List[str]:
        ...
        if profitability_by_customer:
            profitable_customers = [c for c, data in profitability_by_customer.items() if data.get('profit', 0) > 0]
            unprofitable_customers = [c for c, data in profitability_by_customer.items() if data.get('profit', 0) <= 0]

            insights.append(f"👥 {len(profitable_customers)} profitable customers, {len(unprofitable_customers)} unprofitable")

            if profitable_customers:
                top_customer = max(profitability_by_customer.items(), key=lambda x: x[1].get('profit', 0))
                insights.append(f"🏆 Top customer: {top_customer[0]} (${top_customer[1].get('profit', 0):,.2f} profit)")

        # Product profitability insights
        profitability_by_product = getattr(profitability_data, 'profitability_by_product', {})
        if profitability_by_product:
            profitable_products = [p for p, data in profitability_by_product.items() if data.get('profit', 0) > 0]
            unprofitable_products = [p for p, data in profitability_by_product.items() if data.get('profit', 0) <= 0]

            insights.append(f"📦 {len(profitable_products)} profitable products, {len(unprofitable_products)} unprofitable")

            if profitable_products:
                top_product = max(profitability_by_product.items(), key=lambda x: x[1].get('profit', 0))
                insights.append(f"🥇 Top product: {top_product[0]} (${top_product[1].get('profit', 0):,.2f} profit)")
    ...
    async def _generate_profitability_recommendations(
        ...
        if profit_margin < 5:
            ...
            recommendations.append("📊 Analyze unprofitable customers and consider pricing adjustments")
            ...
            recommendations.append("✅ Strong profitability - consider scaling successful strategies")
        ...
        if profitability_by_customer:
            ...
            if unprofitable_customers:
                recommendations.append(f"👥 Review pricing strategy for {len(unprofitable_customers)} unprofitable customers")
                recommendations.append("💰 Consider implementing usage-based pricing or minimum commitments")
        ...
        if profitability_by_product:
            ...
            if unprofitable_products:
                recommendations.append(f"📦 Evaluate {len(unprofitable_products)} unprofitable products for discontinuation or repricing")
                recommendations.append("🔄 Consider bundling unprofitable products with high-margin offerings")
        ...
        try:
            ...
            if "cost_by_model" in breakdown_data:
                ...
                if cost_by_model:
                    ...
                    top_model_percentage = (top_model[1] / total_cost * 100) if total_cost > 0 else 0

                    insights.append(f"🤖 Model breakdown: {total_models} models analyzed")
                    insights.append(f"🏆 Top model: {top_model[0]} (${top_model[1]:.2f}, {top_model_percentage:.1f}%)")

                    # Identify expensive models
                    expensive_models = [(model, cost) for model, cost in cost_by_model.items() if cost > 100]
                    if expensive_models:
            ...
            if "cost_by_provider" in breakdown_data:
                ...
                if cost_by_provider:
                    ...
                    top_provider_percentage = (top_provider[1] / total_cost * 100) if total_cost > 0 else 0

                    insights.append(f"🏢 Provider breakdown: {total_providers} providers analyzed")
                    insights.append(f"🏆 Top provider: {top_provider[0]} (${top_provider[1]:.2f}, {top_provider_percentage:.1f}%)")
            ...
            if "cost_by_agent" in breakdown_data:
                ...
                if cost_by_agent and isinstance(cost_by_agent, list):
                    ...
                    if total_agents > 0:
                        ...
                        insights.append(f"🏆 Top agent: {top_agent.get('name', 'Unknown')} (${top_agent.get('cost', 0):.2f})")

                        # Identify high-cost agents
                        high_cost_agents = [agent for agent in cost_by_agent if agent.get("cost", 0) > 10]
                        if high_cost_agents:
            ...
            if "cost_by_model" in breakdown_data:
                ...
                if cost_by_model:
                    ...
                    if sorted_models and sorted_models[0][1] > total_cost * 0.5:
                        recommendations.append(f"🎯 {sorted_models[0][0]} dominates costs - consider optimization or alternatives")

                    # Identify expensive models
                    expensive_models = [model for model, cost in sorted_models if cost > 100]
                    if expensive_models:
                        recommendations.append(f"💡 Review usage patterns for {len(expensive_models)} high-cost models")
            ...
            if "cost_by_provider" in breakdown_data:
                ...
                if cost_by_provider:
                    ...
                    if len(sorted_providers) > 1 and sorted_providers[0][1] > total_cost * 0.8:
                        recommendations.append(f"🔄 Consider diversifying from {sorted_providers[0][0]} to reduce risk")

                    # Cost optimization opportunities
                    if len(sorted_providers) > 2:
                        recommendations.append("📊 Compare provider rates for potential cost savings")

            # Customer-based recommendations
            if "cost_by_customer" in breakdown_data:
                cost_by_customer = breakdown_data["cost_by_customer"]
                if cost_by_customer:
                    sorted_customers = sorted(cost_by_customer.items(), key=lambda x: x[1], reverse=True)

                    # High-usage customers
                    if sorted_customers:
                        top_customer_cost = sorted_customers[0][1]
                        if top_customer_cost > 100:
                            recommendations.append(f"💰 Consider volume discounts for high-usage customer: {sorted_customers[0][0]}")

                    # Customer optimization
                    recommendations.append("👥 Analyze customer usage patterns for optimization opportunities")

            # Product-based recommendations
            if "cost_by_product" in breakdown_data:
                cost_by_product = breakdown_data["cost_by_product"]
                if cost_by_product:
                    sorted_products = sorted(cost_by_product.items(), key=lambda x: x[1], reverse=True)

                    # High-cost products
                    if sorted_products:
                        top_product_cost = sorted_products[0][1]
                        if top_product_cost > 200:
                            recommendations.append(f"📦 Review cost structure for high-cost product: {sorted_products[0][0]}")

                    # Product optimization
                    recommendations.append("🔍 Evaluate product cost efficiency and pricing strategies")
            ...
            if "cost_by_task" in breakdown_data:
                ...
                if cost_by_task:
                    ...
                    if sorted_tasks:
                        ...
                        if top_task_cost > 50:
                            recommendations.append(f"📋 Optimize high-cost task type: {sorted_tasks[0][0]}")

                    # Task optimization
                    recommendations.append("⚡ Consider task batching or optimization for cost efficiency")

            if not recommendations:
                recommendations.append("✅ Breakdown analysis shows balanced distribution - continue monitoring")

        except Exception as e:
            logger.error(f"Error generating breakdown recommendations: {e}")
            recommendations.append("📊 Continue monitoring breakdown patterns for optimization opportunities")
    ...
    async def _generate_transaction_level_insights(
        ...
        try:
            ...
            avg_cost_per_transaction = getattr(transaction_data, 'average_cost_per_transaction', 0.0)
            ...
            if total_cost > 0:
                ...
                if avg_cost_per_transaction > 0:
                    insights.append(f"📊 Average cost per transaction: ${avg_cost_per_transaction:.4f}")

            # Provider insights
            if cost_by_provider:
                top_provider = max(cost_by_provider.items(), key=lambda x: x[1])
                insights.append(f"🏆 Top provider by cost: {top_provider[0]} (${top_provider[1]:.2f})")
            ...
            if performance_metrics:
                for provider, metrics in performance_metrics.items():
                    if 'tokens_per_minute' in metrics:
                        insights.append(f"⚡ {provider} throughput: {metrics['tokens_per_minute']:.0f} tokens/min")
                    if 'avg_cost_per_transaction' in metrics:
                        insights.append(f"💡 {provider} avg cost: ${metrics['avg_cost_per_transaction']:.4f}/transaction")
    ...
    async def _generate_transaction_level_recommendations(
        ...
        try:
            ...
            if cost_by_provider and len(cost_by_provider) > 1:
                providers_sorted = sorted(cost_by_provider.items(), key=lambda x: x[1], reverse=True)
    ...
    async def _generate_customer_analytics_insights(
        ...
        try:
            ...
            if organizations:
                insights.append(f"📊 Analyzed {len(organizations)} customer organizations for {period}")
                ...
                if total_revenue > 0:
                    ...
                    insights.append(f"📊 Average revenue per customer: ${avg_revenue_per_customer:.2f}")

                # Profitability insights
                if customer_profitability:
                    profitable_customers = sum(1 for data in customer_profitability.values() if data.get('profit', 0) > 0)
                    insights.append(f"🎯 {profitable_customers}/{len(customer_profitability)} customers are profitable")

                    # Top customer insights
                    top_customer = max(customer_profitability.items(), key=lambda x: x[1].get('profit', 0))
                    if top_customer[1].get('profit', 0) > 0:
                        insights.append(f"🏆 Top customer: {top_customer[0]} (${top_customer[1]['profit']:.2f} profit)")
    ...
    async def _generate_customer_analytics_recommendations(
        ...
        try:
            ...
            if customer_profitability:
                ...
                unprofitable_customers = [name for name, data in customer_profitability.items()
                ...
                high_margin_customers = [name for name, data in customer_profitability.items()
                ...
                if len(customer_profitability) > 1:
                    sorted_customers = sorted(customer_profitability.items(),
                                            key=lambda x: x[1].get('revenue', 0), reverse=True)
                    top_customer_revenue = sorted_customers[0][1].get('revenue', 0)
                    total_revenue = sum(data.get('revenue', 0) for data in customer_profitability.values())
            ...
            recommendations.append("📊 Review customer profitability patterns for optimization opportunities")
        ...
        try:
            ...
            if products:
                ...
                if total_revenue > 0:
                    ...
                    insights.append(f"📊 Average revenue per product: ${avg_revenue_per_product:.2f}")

                # Profitability insights
                if product_profitability:
                    profitable_products = sum(1 for data in product_profitability.values() if data.get('profit', 0) > 0)
                    insights.append(f"🎯 {profitable_products}/{len(product_profitability)} products are profitable")

                    # Top product insights
                    top_product = max(product_profitability.items(), key=lambda x: x[1].get('profit', 0))
                    if top_product[1].get('profit', 0) > 0:
                        insights.append(f"🏆 Top product: {top_product[0]} (${top_product[1]['profit']:.2f} profit)")
    ...
    async def _generate_product_analytics_recommendations(
        ...
        try:
            ...
            if product_profitability:
                ...
                unprofitable_products = [name for name, data in product_profitability.items()
                                       if data.get('profit', 0) < 0]
                ...
                high_margin_products = [name for name, data in product_profitability.items()
                                      if data.get('margin', 0) > 20]
                ...
                if len(product_profitability) > 1:
                    ...
                    total_revenue = sum(data.get('revenue', 0) for data in product_profitability.values())
            ...
            recommendations.append("📦 Review product profitability patterns for optimization opportunities")
        ...
        try:
            ...
            if agents:
                ...
                if agent_performance:
                    ...
                    top_agent = max(agent_performance.items(), key=lambda x: x[1].get('efficiency_score', 0))
                    if top_agent[1].get('efficiency_score', 0) > 0:
                        insights.append(f"🏆 Top agent: {top_agent[0]} (efficiency: {top_agent[1]['efficiency_score']:.2f})")

                    # Cost efficiency insights
                    efficient_agents = sum(1 for data in agent_performance.values() if data.get('efficiency_score', 0) > 1.0)
                    insights.append(f"⚡ {efficient_agents}/{len(agent_performance)} agents are highly efficient")
    ...
    async def _generate_agent_analytics_recommendations(
        ...
        try:
            ...
            if agent_performance:
                ...
                low_efficiency_agents = [name for name, data in agent_performance.items()
                ...
                # Identify high-cost agents
                high_cost_agents = [name for name, data in agent_performance.items()
                                  if data.get('cost_per_call', 0) > 0.01]  # $0.01 threshold
            ...
            recommendations.append("🤖 Review agent performance patterns for optimization opportunities")
    ...
    async def _generate_task_analytics_insights(
        ...
        try:
            ...
            if providers or models:
                ...
                insights.append(f"🔧 Analyzed {provider_count} providers and {model_count} models for task-level metrics")
                ...
                if total_cost > 0:
                    ...
                    if provider_count > 0:
                        ...
                        insights.append(f"📈 Average cost per provider: ${avg_cost_per_provider:.2f}")
                ...
                if total_performance > 0:
                    ...
                    if provider_count > 0:
                        ...
                        insights.append(f"📊 Average performance per provider: {avg_performance_per_provider:.2f}")

                # Provider performance insights
                if provider_performance:
                    # Find top performing provider
                    top_provider = max(provider_performance.items(), key=lambda x: x[1].get('efficiency', 0))
                    if top_provider[1].get('efficiency', 0) > 0:
                        insights.append(f"🏆 Top provider: {top_provider[0]} (efficiency: {top_provider[1]['efficiency']:.2f})")

                    # Cost efficiency insights
                    efficient_providers = sum(1 for data in provider_performance.values() if data.get('efficiency', 0) > 1.0)
                    insights.append(f"⚡ {efficient_providers}/{len(provider_performance)} providers are highly efficient")

                # Model performance insights
                if model_performance:
                    # Find top performing model
                    top_model = max(model_performance.items(), key=lambda x: x[1].get('efficiency', 0))
                    if top_model[1].get('efficiency', 0) > 0:
                        insights.append(f"🎯 Top model: {top_model[0]} (efficiency: {top_model[1]['efficiency']:.2f})")

                    # Model efficiency insights
                    efficient_models = sum(1 for data in model_performance.values() if data.get('efficiency', 0) > 1.0)
                    insights.append(f"🚀 {efficient_models}/{len(model_performance)} models are highly efficient")

                # Task-level correlation insights
                if total_cost > 0 and total_performance > 0:
                    overall_efficiency = total_performance / total_cost
                    insights.append(f"📈 Overall task efficiency: {overall_efficiency:.2f} performance per dollar")
    ...
    async def _generate_task_analytics_recommendations(
        ...
        try:
            ...
            if provider_performance:
                ...
                low_efficiency_providers = [name for name, data in provider_performance.items()
                ...
                high_cost_providers = [name for name, data in provider_performance.items()
                                     if data.get('cost', 0) > total_cost * 0.4]  # >40% of total cost
            ...
            if model_performance:
                ...
                low_efficiency_models = [name for name, data in model_performance.items()
                                       if data.get('efficiency', 0) < 0.5]
                ...
                high_cost_models = [name for name, data in model_performance.items()
                                  if data.get('cost', 0) > total_cost * 0.3]  # >30% of total cost
            ...
            recommendations.append("🔧 Review task performance patterns for optimization opportunities")
