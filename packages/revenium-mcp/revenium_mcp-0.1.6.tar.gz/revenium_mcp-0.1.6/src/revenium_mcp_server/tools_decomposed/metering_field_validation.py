...
class TestDataGenerator:
    ...
    def generate_batch(self, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        test_data = []
        industry_pattern = self.industry_patterns.get(industry, self.industry_patterns["financial_services"])
    ...
    def _generate_base_transaction(self, index: int, industry_pattern: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _generate_enterprise_fields(self, index: int, industry_pattern: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _build_industry_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "financial_services": {
                "organization_ids": ["goldman-sachs", "jp-morgan", "wells-fargo", "blackrock", "vanguard"],
                "task_types": ["risk_analysis", "fraud_detection", "market_sentiment", "portfolio_optimization"],
                "agents": ["RiskBot_v2.1", "FraudDetector_AI", "MarketIntel_v3.0", "PortfolioAI"]
            },
            "healthcare": {
                "organization_ids": ["mayo-clinic", "johns-hopkins", "cleveland-clinic", "kaiser-permanente"],
                "task_types": ["diagnosis_support", "treatment_planning", "medical_research", "patient_monitoring"],
                "agents": ["DiagnosisAI_v1.5", "TreatmentBot", "ResearchAssistant", "PatientMonitor"]
            },
            "legal": {
                "organization_ids": ["baker-mckenzie", "latham-watkins", "skadden", "kirkland-ellis"],
                "task_types": ["contract_analysis", "legal_research", "compliance_check", "document_review"],
                "agents": ["LegalBot_v2.0", "ContractAI", "ComplianceChecker", "DocReviewer"]
            },
            "technology": {
                "organization_ids": ["microsoft", "google", "amazon", "meta", "apple"],
                "task_types": ["code_generation", "bug_analysis", "system_design", "performance_optimization"],
    ...
    def _build_edge_case_patterns(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"task_type": "test_with_special_chars_!@#$%", "agent": "Agent-With-Dashes_And_Underscores"},
...
class FieldMappingAnalyzer:
    ...
    async def analyze_field_presence(self, client: ReveniumClient, verification_result: Dict[str, Any], submitted_transactions: Optional[Dict[str, Dict[str, Any]]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if "transaction_data" in verification_result:
                ...
                logger.info(f"📊 Using {len(recent_transactions)} transactions from verification result (100% code reuse)")
            else:
                # Fallback for backward compatibility
                recent_transactions = await self._fetch_recent_transactions(client, limit=50)
                logger.warning("⚠️ Using fallback transaction fetch - consider enabling return_transaction_data for better performance")
            ...
            if not recent_transactions:
                return {
                    ...
                    "recommendations": ["Submit test transactions first", "Check API permissions", "Ensure verification found transactions"]
                }
            ...
            if submitted_transactions:
                logger.info(f"🔬 Performing data integrity validation with {len(submitted_transactions)} submitted transactions")
                integrity_analysis = await self._analyze_data_integrity(recent_transactions, submitted_transactions)
            ...
            if integrity_analysis:
                integrity_recommendations = self._generate_integrity_recommendations(integrity_analysis)
                recommendations.extend(integrity_recommendations)

            return {
                "analysis_type": "field_mapping_with_integrity" if submitted_transactions else "field_mapping",
            ...
            return {
                ...
                "status": "error",
                "error": str(e),
                "recommendations": ["Check API connectivity", "Verify authentication", "Try again later"]
            }

    async def _fetch_recent_transactions(self, client: ReveniumClient, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "_embedded" in response and "aICompletionMetricResourceList" in response["_embedded"]:
                ...
                logger.info(f"📊 Retrieved {len(transactions)} transactions for analysis (new structure)")
                return transactions
            elif "content" in response:
                # Legacy API structure with content
                transactions = response["content"]
                logger.info(f"📊 Retrieved {len(transactions)} transactions for analysis (legacy structure)")
                return transactions
            else:
                logger.warning("No 'content' or '_embedded.aICompletionMetricResourceList' field in reporting API response")
                return []
    ...
    async def _analyze_transaction_fields(self, transactions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _build_critical_fields(self) -> List[str]:
        ...
        return [
            ...
            "duration_ms", "subscriber"  # Use subscriber object instead of legacy fields
        ]
    ...
    def _analyze_critical_fields(self, field_percentages: Dict[str, float]) -> Dict[str, Any]:
        ...
        for field in self.critical_fields:
            ...
            status = "excellent" if percentage >= 95 else "good" if percentage >= 80 else "poor" if percentage >= 50 else "critical"
    ...
    def _generate_field_recommendations(self, field_analysis: Dict[str, Any]) -> List[str]:
        ...
        if total_with_subscriber > 0:
            ...
            if email_percentage < 90:
                recommendations.append(f"📧 Subscriber email mapping issue: {email_percentage:.1f}% presence")
    ...
    async def _analyze_data_integrity(self, retrieved_transactions: List[Dict[str, Any]], submitted_transactions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"🔬 Starting data integrity validation: {len(retrieved_transactions)} retrieved vs {len(submitted_transactions)} submitted")

        # Step 1: Correlate transactions by transactionId
        correlation_results = self._correlate_transactions(retrieved_transactions, submitted_transactions)
        ...
        return {
            ...
            "transactions_with_mismatches": integrity_results.get("transactions_with_mismatches", 0),
    ...
    def _generate_integrity_recommendations(self, integrity_analysis: Dict[str, Any]) -> List[str]:
        ...
        if missing_transactions:
            recommendations.append(f"⚠️ {len(missing_transactions)} transactions missing from reporting API")
        ...
        if transactions_with_mismatches > 0:
            recommendations.append(f"🔍 {transactions_with_mismatches} transactions have field mismatches")
    ...
    def _correlate_transactions(self, retrieved_transactions: List[Dict[str, Any]], submitted_transactions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"🔗 Correlating {len(retrieved_transactions)} retrieved vs {len(submitted_transactions)} submitted transactions")
        ...
        correlation_summary = {
            ...
            "correlation_rate": len(matched_pairs) / len(submitted_transactions) if submitted_transactions else 0.0
        }

        logger.info(f"🔗 Correlation complete: {len(matched_pairs)} matched, {len(missing_transactions)} missing")
    ...
    def _validate_field_integrity(self, correlation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"🔍 Validating field integrity for {len(matched_pairs)} matched transaction pairs")
        ...
        for pair in matched_pairs:
            ...
            if not mismatches:
                ...
                logger.warning(f"⚠️ {len(mismatches)} mismatches found for transaction {transaction_id}")
    ...
    def _compare_transaction_values(self, submitted_payload: Dict[str, Any], retrieved_transaction: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for submitted_field, api_field in self.expected_field_mappings.items():
            ...
            if submitted_field in submitted_payload and api_field in retrieved_transaction:
                ...
                if not self._values_match(submitted_value, retrieved_value):
                    ...
                    logger.debug(f"🔍 Mismatch in {submitted_field}: {submitted_value} → {retrieved_value} ({mismatch_type})")
    ...
    def _classify_mismatch(self, submitted_value: Any, retrieved_value: Any) -> str:
        ...
        if isinstance(submitted_value, str) and isinstance(retrieved_value, str):
            ...
            if len(submitted_value) > len(retrieved_value) and submitted_value.startswith(retrieved_value):
                return "truncation"
        ...
        elif isinstance(submitted_value, (int, float)) and isinstance(retrieved_value, (int, float)):
...
class ValidationReporter:
    ...
    def enhance_validation_report(self, validation_result: Dict[str, Any], arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def generate_field_mapping_report(self, field_analysis: Dict[str, Any], format_type: str = "markdown") -> str:
    ...
    def _generate_markdown_report(self, field_analysis: Dict[str, Any]) -> str:
        ...
        if field_percentages:
            ...
            for field, percentage in sorted(field_percentages.items()):
                status = "✅ Excellent" if percentage >= 95 else "⚠️ Good" if percentage >= 80 else "❌ Poor"
        ...
        if subscriber_analysis.get("total_with_subscriber", 0) > 0:
            ...
            email_pct = (subscriber_analysis.get("email_present", 0) / total * 100) if total > 0 else 0
        ...
        if integrity_analysis:
            ...
            if field_mismatch_summary:
                ...
                for field, count in sorted(field_mismatch_summary.items(), key=lambda x: x[1], reverse=True):
    ...
    def _generate_batch_recommendations(self, success_rate: float, failure_patterns: Dict[str, int]) -> List[str]:
        ...
        if success_rate < 50:
            ...
            recommendations.append("⚠️ Warning: Success rate below 80% - investigate common failures")
        ...
        if failure_patterns:
            ...
            if "validation" in most_common_error[0].lower():
                recommendations.append("💡 Consider using validate_test_data() before batch submission")
...
class MeteringFieldValidationManagement(ToolBase):
    ...
    async def _handle_generate_test_data(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_submit_test_batch(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        if not test_data:
            return self.format_error_response(
                "No test data provided for batch submission",
                "submit_test_batch"
            )
    ...
    async def _handle_analyze_field_mapping(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_validate_test_data(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_run_validation_suite(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
...
"""
    ...
    async def _get_supported_actions(self) -> List[str]:
        ...
        return [
            "generate_test_data", "submit_test_batch", "analyze_field_mapping",
            "validate_test_data", "run_validation_suite",
