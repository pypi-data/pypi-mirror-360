...
from .resources import resource_manager, ResourceType, MCPResource
from .error_handling import (
    MCPError, JSONRPCErrorCode, create_invalid_params_error,
    create_resource_not_found_error, create_internal_error
)
...
class MCPResourceDiscoveryEngine:
    ...
    def _register_mcp_handlers(self):
        ...
        protocol_handler.register_request_handler("resources/subscribe", self._handle_resources_subscribe)
        protocol_handler.register_request_handler("resources/unsubscribe", self._handle_resources_unsubscribe)
    ...
    async def _handle_resources_read(
        ...
        try:
            ...
            raise create_internal_error(
                message=f"Resource reading failed: {str(e)}",
                context={"params": params}
            )
    ...
    async def _handle_resources_subscribe(
        ...
        try:
            ...
            raise create_internal_error(
                message=f"Resource subscription failed: {str(e)}",
                context={"params": params}
            )
    ...
    async def _handle_resources_unsubscribe(
        ...
        try:
            ...
            raise create_internal_error(
                message=f"Resource unsubscription failed: {str(e)}",
                context={"params": params}
            )
