...
from .log_analysis_constants import (
    CAPABILITIES_TEXT, EXAMPLES_TEXT, UNSUPPORTED_ACTION_TEMPLATE,
    ERROR_MESSAGES, SUGGESTIONS, DEFAULT_VALUES, LOG_ENDPOINTS
)
...
class ReveniumLogAnalysis(ToolBase):
    ...
    tool_description: ClassVar[str] = "Revenium log analysis for system troubleshooting and diagnostic investigation. Key actions: get_internal_logs, get_integration_logs, get_recent_logs, search_logs, analyze_operations. Default size: 200 records (max: 1000). Use get_examples() for usage guidance and get_capabilities() for status."
    ...
    async def _handle_unsupported_action(self, action: str) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _search_all_pages(
        ...
        logger.info(f"Search complete: {len(all_matches)} total matches across {total_pages_searched} pages")
    ...
    def _analyze_operation_patterns(self, all_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        failure_rate = status_counts.get("FAILURE", 0) / total_operations if total_operations > 0 else 0
        ...
        return {
            ...
            "problematic_operations": [op for op, errors in error_patterns.items() if len(errors) > 1]
        }
...
"""
        ...
        response_text += "\n## Status Distribution\n"
        for status, count in sorted(analysis['status_counts'].items(), key=lambda x: x[1], reverse=True):
            percentage = (count / analysis['total_operations']) * 100
        ...
        if analysis['failure_rate'] > 0.1:
            response_text += f"- ⚠️ High failure rate ({analysis['failure_rate']:.1%}) requires investigation\n"
        if analysis['problematic_operations']:
    ...
    async def _route_action(self, action: str, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_get_internal_logs(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_get_integration_logs(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
    ...
    async def _handle_get_recent_logs(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            ...
            return self.response_formatter.format_multi_page_response(
                all_entries, log_type, pages
            )
    ...
    async def _handle_search_logs(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        try:
            if search_all_pages:
                ...
                all_matches, applied_filters, pages_searched = await self._search_all_pages(
                    log_type, arguments.get("operation_filter"),
                    arguments.get("status_filter"), arguments.get("search_term")
                )
                ...
                response, applied_filters = await self._get_filtered_logs(
                    log_type, page, size, arguments.get("operation_filter"),
                    arguments.get("status_filter"), arguments.get("search_term")
                )
    ...
    async def _handle_analyze_operations(self, arguments: Dict[str, Any]) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
