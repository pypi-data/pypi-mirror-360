...
class SlackConfigurationManagement(ToolBase):
    ...
    tool_description: ClassVar[str] = "Slack configuration management for alert notifications. Key actions: list_configurations, get_configuration, set_default_configuration, get_default_configuration. Use get_examples() for configuration examples and get_capabilities() for complete action list."
    ...
    def _handle_unknown_action(self, action: str):
        ...
        raise ToolError(
            ...
            examples={
                "basic_actions": ["list_configurations", "get_configuration", "set_default_configuration"],
    ...
    async def _handle_list_configurations(
        ...
        if self._is_dry_run(arguments):
            return self._format_dry_run_response(f"Would list Slack configurations with page={page}, size={size}.")
    ...
    async def _handle_get_configuration(
        ...
        if self._is_dry_run(arguments):
            return self._format_dry_run_response(f"Would get configuration details for: {config_id}")
    ...
    async def _handle_set_default_configuration(
        ...
        if self._is_dry_run(arguments):
            return self._format_dry_run_response(f"Would set {config_id} as default Slack configuration.")
    ...
    async def _get_tool_capabilities(self) -> List[ToolCapability]:
        ...
        return [
            ...
            ToolCapability(
                ...
                examples=["set_default_configuration(config_id='slack-config-123')", "get_default_configuration()"],
    ...
    async def get_agent_summary(self) -> List[Union[TextContent, ImageContent, EmbeddedResource]]:
        ...
        return [TextContent(
            type="text",
            text="""# **Agent Summary: Slack Configuration Management**

## **What This Tool Does**
Manages Slack configurations for alert notifications with comprehensive CRUD operations and default configuration management.

## **Key Capabilities**
• **Configuration Management**: List, get, and manage Slack configurations
• **Default Settings**: Set and retrieve default configurations for alerts
• **OAuth Integration**: Get OAuth URLs for new Slack workspace setup
• **Pagination Support**: Handle large numbers of configurations efficiently
• **Validation Mode**: Dry-run capabilities for testing changes

## **Primary Use Cases**
• **Alert Setup**: Configure Slack channels for alert notifications
• **Default Management**: Set default Slack configurations for all alert-creation from MCP integration
• **Configuration Discovery**: List and inspect existing Slack integrations
• **OAuth Workflow**: Initiate new Slack workspace connections

## **Integration Benefits**
• **Alert Integration**: Seamlessly works with alert management tools
• **OAuth Workflow**: Integrates with slack_oauth_workflow for setup
• **Environment Persistence**: Maintains default settings across sessions
• **Error Handling**: Provides actionable error messages and suggestions
...
## **Next Steps**
• Use `get_capabilities()` for detailed action documentation
• Use `get_examples()` for copy-paste ready examples
• Start with listing configurations to understand current setup
"""
        )]
