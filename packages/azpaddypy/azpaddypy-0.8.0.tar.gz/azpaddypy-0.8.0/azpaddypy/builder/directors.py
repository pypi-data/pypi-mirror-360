...
class ConfigurationSetupDirector:
    ...
    def build_default_config():
        ...
        return (ConfigurationSetupBuilder()
                ...
                .with_environment_variables(configuration_config, in_docker=False, in_machine=True)  # Development storage only for local machine
...
class AzureManagementDirector:
    ...
    def build_default_config(configuration_config) -> AzureManagementConfiguration:
        ...
        return (AzureManagementBuilder(configuration_config)
                .with_logger()
                .with_identity()
                .with_keyvault()
                .build())
...
class AzureResourceDirector:
    ...
    def build_default_config(configuration_config, management_config) -> AzureConfiguration:
        ...
        resource_config = AzureResourceBuilder(management_config=management_config, env_config=configuration_config).with_storage().build()
        return AzureConfiguration(management=management_config, resources=resource_config)
    
    @staticmethod
    def build_scripts_cosmos_db_config() -> AzureConfiguration:
        """Build configuration specifically for scripts that need CosmosDB access.
        
        This method creates a complete Azure configuration with:
        - Environment setup with local development support
        - Management services (identity, logger)
        - CosmosDB resource configuration
        
        Used by scripts like upload_prompt.py that need CosmosDB access.
        
        Returns:
            AzureConfiguration: Complete configuration with CosmosDB client
        """
        # Step 1: Build environment configuration
        env_config = ConfigurationSetupDirector.build_default_config()
        
        # Step 2: Build management configuration (identity, logger, etc.)
        management_config = AzureManagementDirector.build_default_config(env_config)
        
        # Step 3: Build resource configuration with CosmosDB
        resource_config = (AzureResourceBuilder(management_config=management_config, env_config=env_config)
                          .with_cosmosdb()  # Add CosmosDB with default settings
                          .build())
        
        # Step 4: Return combined configuration
        return AzureConfiguration(management=management_config, resources=resource_config)
