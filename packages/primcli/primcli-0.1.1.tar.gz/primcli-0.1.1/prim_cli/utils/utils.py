...
def get_agent(session: requests.Session, name_or_id: str):
    ...
    try:
        ...
        agent = next((a for a in agents if a.get("name", "").lower() == name), None)
        if not agent:
...
def get_function(session: requests.Session, agent_id: str, function_name_or_id: str):
    ...
    try:
        ...
        function = next(
            (f for f in functions if f.get("name") == function_name_or_id), None
        )
        if not function:
            print_error(f"Function with name or ID '{function_name_or_id}' not found.")
            return None
        return function
    except requests.HTTPError as e:
        handle_http_error(e)
...
def get_environment(session: requests.Session, agent_id: str, env_name_or_id: str):
    ...
    try:
        ...
        env = next((e for e in envs if e.get("name") == env_name_or_id), None)
        if not env:
            print_error(f"Environment with name or ID '{env_name_or_id}' not found.")
            return None
        return env
    except requests.HTTPError as e:
        handle_http_error(e)
...
def manage_variables(
    session: requests.Session, agent_id: str, env_id: str, is_update: bool
):
    ...
    if is_update:
        ...
        for var in variables:
            if confirm(f"Update variable '{var['name']}'?", default=False):
                ...
                value = prompt(
                    "New value", default="*****" if var.get("masked") else var["value"]
                )
