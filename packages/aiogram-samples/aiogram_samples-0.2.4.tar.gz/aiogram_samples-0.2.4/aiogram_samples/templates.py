...
async def admin(message: Message, state: FSMContext):
    ...
    admin_dir = target_dir / "bot" / "admin"
    admin_dir.mkdir(parents=True, exist_ok=True)  # Ключевое исправление

    (admin_dir / "admin.py").write_text(content)
...
admin_buttons = InlineKeyboardMarkup(
    ...
    (target_dir /"bot" / "admin" / "admin_keyboard.py").write_text(content)
...
class Admin(StatesGroup):
    ...
    (target_dir /"bot" / "admin" / "admin_states.py").write_text(content)
...
"""
    ...
    commands_dir = target_dir /"bot" / "commands"
    commands_dir.mkdir(parents=True, exist_ok=True)  # Ключевое исправление

    (commands_dir / "commands.py").write_text(content)
...
from aiogram.utils.keyboard import InlineKeyboardBuilder"""
    ...
    (target_dir /"bot" / "commands" / "commands_keyboard.py").write_text(content)
...
class States(StatesGroup):
    ...
    (target_dir /"bot" / "commands" / "commands_states.py").write_text(content)
...
async def async_main():
    ...
    database_dir = target_dir / "api" / "database"
    database_dir.mkdir(parents=True, exist_ok=True)  # Ключевое исправление

    (database_dir / "models.py").write_text(content)
...
async def get_admins():
    ...
    (target_dir / "api" / "database" / "requests.py").write_text(content)
