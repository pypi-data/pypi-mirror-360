    ...
    def _check_proxy_ready(self, timeout=15):
        ...
        while time.time() - start_time < timeout:
            ...
            if self.v2ray_process.poll() is not None:
                ...
                error_msg = f"V2Ray process terminated early. Exit code: {self.v2ray_process.returncode}\nStdout: {stdout}\nStderr: {stderr}"
...
class V2RayPool:
    ...
    def get_fastest_proxy(self, count=3):
        """
        Measure latencies and return the fastest proxy.
        
        Args:
            count (int): Number of measurement samples to take.
            
        Returns:
            V2RayProxy: The proxy with the lowest average latency.
        """
        avg_latencies = {}
        ...
        for _ in range(count):
            latencies = self.measure_latencies()
            for proxy_id, latency in latencies.items():
                if proxy_id not in avg_latencies:
                    avg_latencies[proxy_id] = []
                avg_latencies[proxy_id].append(latency)
        
        # Calculate averages
        for proxy_id in list(avg_latencies.keys()):
            if avg_latencies[proxy_id]:  # Check if we have any successful measurements
                avg_latencies[proxy_id] = sum(avg_latencies[proxy_id]) / len(avg_latencies[proxy_id])
            else:
                del avg_latencies[proxy_id]  # Remove proxies with no successful measurements
        
        if not avg_latencies:
            return self.get_proxy(strategy="round-robin")
        
        fastest_id = min(avg_latencies, key=avg_latencies.get)
