...
from maleo_metadata.types.results.client.user_type import (
    MaleoMetadataUserTypeClientResultsTypes,
)
...
class MaleoMetadataUserTypeClientService(MaleoClientService):
    ...
    async def get_user_types(
        ...
        @BaseExceptions.service_exception_handler(
            ...
            logger=self._logger,
            fail_result_class=MaleoMetadataUserTypeClientResultsTransfers.Fail,  # type: ignore
        )
        async def _impl(
            parameters: MaleoMetadataUserTypeClientParametersTransfers.GetMultiple,
            controller_type: MaleoMetadataGeneralEnums.ClientControllerType = MaleoMetadataGeneralEnums.ClientControllerType.HTTP,
            ...
            if controller_type == MaleoMetadataGeneralEnums.ClientControllerType.HTTP:
                controller_result = await self._controllers.http.get_user_types(
            ...
            if not controller_result.success:
                return MaleoMetadataUserTypeClientResultsTransfers.Fail.model_validate(
                    controller_result.content
                )
            else:
                if controller_result.content["data"] is None:
                    return MaleoMetadataUserTypeClientResultsTransfers.NoData.model_validate(
                        controller_result.content
                    )
                else:
                    return MaleoMetadataUserTypeClientResultsTransfers.MultipleData.model_validate(
                        controller_result.content
                    )
    ...
    async def get_user_type(
        ...
        async def _impl(
            ...
            if controller_type == MaleoMetadataGeneralEnums.ClientControllerType.HTTP:
                controller_result = await self._controllers.http.get_user_type(
                    parameters=parameters, authorization=authorization, headers=headers
                )
            else:
                message = "Invalid controller type"
            ...
            if not controller_result.success:
                ...
                return MaleoMetadataUserTypeClientResultsTransfers.SingleData.model_validate(
                    controller_result.content
                )
