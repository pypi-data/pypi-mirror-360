...
def _write_chapter(file_handle, start_time: float, end_time: float, title: str) -> None:
    ...
    file_handle.write(CHAPTER_FORMAT.format(
        _float_to_milliseconds(start_time),
        _float_to_milliseconds(end_time),
        title
    ))
...
def build_flags(anime_id: str, episode: int, chapters_file: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            logging.info("Failed to fetch AniSkip data (Status: %d)",
                         response.status_code)
            return ""
...
def aniskip(title: str, episode: int, season: int, aniworld_episodes: int) -> str:
    ...
    try:
        ...
        if mal_episodes != aniworld_episodes:
            logging.warning("Episode count mismatch: MAL=%s, AniWorld=%s",
                            mal_episodes, aniworld_episodes)
            return ""
