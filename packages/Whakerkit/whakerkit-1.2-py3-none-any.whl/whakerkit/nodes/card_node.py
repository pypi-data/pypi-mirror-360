...
MSG_WHO = _("From: ")
MSG_DATE = _("On: ")
MSG_DOWNLOAD = _("Download /<span name=\"download-count\">{nb}</span>")
...
class WhakerKitDocumentCardNode(HTMLNode):
    ...
    def __init__(self, parent, doc: ImmutableDocument = None, doc_file_path: str = "#"):
        ...
        WhakerKitDocumentCardNode.__add_info(main_card, " ".join(["<i>", MSG_NAME, "</i><b>", doc.filename, "</b>"]))
        WhakerKitDocumentCardNode.__add_info(main_card, "<i>" + MSG_WHO + "</i>" + author)
        WhakerKitDocumentCardNode.__add_info(main_card, "<i>" + MSG_DATE + "</i>" + doc.date.strftime('%Y-%m-%d'))
        WhakerKitDocumentCardNode.__add_info(main_card, "<i>" + MSG_TYPE + "</i>" + doc.filetype)
        ...
        btn = WhakerKitDownloadNode(footer_card.identifier,
                                    ...
                                    text=MSG_DOWNLOAD.format(nb=doc.downloads))
