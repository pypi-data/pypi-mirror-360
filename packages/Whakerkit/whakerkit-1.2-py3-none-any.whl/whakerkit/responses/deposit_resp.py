...
MSG_TITLE_TAB = _("Deposits")
MSG_TITLE_DEPOSIT = _("My deposits")
MSG_NOT_AUTH = _("The content of this page is access-restricted [...]")
MSG_LOGIN = _("Log in")
MSG_LOGOUT = _("Log out")
MSG_CONNECTED = _("You are logged in.")

MSG_UPLOAD_DOC = _("Upload a document")
MSG_MANAGE_DOC = _("Manage uploaded documents of {author}")
MSG_NO_DOCS = _("You have not deposited any documents.")
MSG_NB_DOCS = _("You have deposited {nb_docs} documents.")
MSG_SEND_FILE = _("Send")
MSG_PLEASE_WAIT = _("Uploading... Please wait.")
MSG_VALIDATE = _("Validate")
MSG_FILENAME_INFO = _("The uploaded document will be visible to all users [...]")
MSG_FILENAME_RECOMMANDATION = _("Consider depositing your file with a name that [...]")

MSG_UPLOAD_FAILED = _("Error saving the document on the server")
MSG_NO_DESCRIPTION = _("No description provided.")
MSG_NEW_DESCR = _("New description for the referenced document:")
MSG_MAX_LEN_DESCR = _("/ 160 characters")
MSG_UNRESPONDING = _("The server did not respond to the request.")
MSG_EMPTY_FILE = _("The file upload was not completed: the file is empty.")
MSG_NON_AUTHORIZED = _("You are not authorized to make changes to this document.")
MSG_SERVER_ERROR = _("The following error occurred: {error}")
...
BODY_SCRIPT = f"""
    ...
    docManager.errorMessage = "{MSG_UNRESPONDING}";
...
class DepositResponse(WhakerkitAuthResponse):
    ...
    def __init__(self, name: str = None):
        super(DepositResponse, self).__init__(name, title=MSG_TITLE_TAB)
    ...
    def __upload_callback(self, upload_event: dict) -> bool:
        ...
        if len(content) == 0:
            ...
            self._data = {"error": MSG_EMPTY_FILE}
            ...
            try:
                ...
                self._data = {"error": MSG_UPLOAD_FAILED + " " + str(e)}
    ...
    def __describe_callback(self, describe_event: dict) -> bool:
        ...
        if len(description) == 0:
            self._data = {"error": MSG_NO_DESCRIPTION}
        else:
            try:
                ...
                self._data = {"error": MSG_SERVER_ERROR + " " + str(e)}
    ...
    def __bake_anonymous(self):
        ...
        me = HTMLNode(section.identifier, None, "p", value=MSG_NOT_AUTH)
        ...
        if logging.getLogger().getEffectiveLevel() > 1:
            action = "open_dialog('" + login_node.identifier + "', true)"
        else:
            action = "authManager.submitLogin('anonymous', '********');"
        btn = create_action_button(section.identifier, action, MSG_LOGIN, whakerkit.sg.path + "statics/icons/authentication.png")
    ...
    def __bake_authenticated(self):
        ...
        me = HTMLNode(section.identifier, None, "p", value=MSG_CONNECTED)
        section.append_child(me)
        ...
        if token is not None:
            ...
            action = ""
        btn = create_action_button(section.identifier, action, MSG_LOGOUT, whakerkit.sg.path + "statics/icons/log-out.png")
        btn.add_attribute("type", "submit")
        if token is None:
            btn.add_attribute("disabled", None)
        section.append_child(btn)

        stitle = HTMLNode(section.identifier, None, "h2", value=MSG_UPLOAD_DOC)
        ...
        right = HTMLNode(ssection.identifier, None, "dialog", value=MSG_FILENAME_RECOMMANDATION)
    ...
    def __append_title(self):
        ...
        h1.set_value(MSG_TITLE_DEPOSIT)
    ...
    def __append_file_sending(self, parent: HTMLNode):
        ...
        """
        fn_info = HTMLNode(parent.identifier, None, "p", value="<br>"+MSG_FILENAME_INFO)
        ...
        btn = create_action_button(parent.identifier, "docManager.sendDocument();",
                                   MSG_SEND_FILE, whakerkit.sg.path + "statics/icons/upload.png")
        btn.add_attribute("type", "submit")
    ...
    def __append_all_docs(self, parent: HTMLNode, documents: list):
        ...
        """
        stitle = HTMLNode(parent.identifier, None, "h2", value=MSG_MANAGE_DOC.format(author=self._author))
        ...
        if documents is not None:
            ...
            if len(documents) == 0:
                no = HTMLNode(parent.identifier, None, "p", value=MSG_NO_DOCS)
                parent.append_child(no)
                # Display no document in a table
                file_node = DocumentsNode(parent.identifier, dm, self._author)
            else:
                no = HTMLNode(parent.identifier, None, "p", value=MSG_NB_DOCS.format(nb_docs=len(documents)))
                parent.append_child(no)
                ...
                descr_dlg.set_value(
                    f"""
                    <form method="dialog">
                        <p>{MSG_NEW_DESCR}<br><i><span id="folder_name_span"></span></i></p>
                        <textarea id="new_description_field" rows="3" cols="55" maxlength="160"
                                  onkeyup="docManager.currentDescriptionLength();"
                        ></textarea>
                        <p><small><span id="new_description_span">--</span>{MSG_MAX_LEN_DESCR}</small></p>
                        <menu>
                            <button id="confirm" type="submit">{MSG_VALIDATE}</button>
                        </menu>
                    </form>
                    """
                )
    ...
    def describe_document(self, folder_name: str, description: str):
        ...
        if doc.author != WhakerKitDocsManager.format_author(self._author):
            raise Exception(MSG_NON_AUTHORIZED)
