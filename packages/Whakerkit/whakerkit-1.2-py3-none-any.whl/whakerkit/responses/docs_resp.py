...
MSG_TITLE_TAB = _("Documents")
MSG_TITLE_DOCS = _("Available documents")
MSG_UNRESPONDING = _("The server did not respond to the request.")
MSG_NB_DOCS = _("There are {nb} documents")
MSG_NO_DOC = _("No documents match the filters.")
MSG_ALL_DOCS = _("All documents match the filters.")
MSG_NO_FILETYPE = _("At least one file type must be checked.")
MSG_NO_AUTHOR = _("At least one author must be checked.")
MSG_NO_FILTER = _("No filter has been applied.")
MSG_ARE_FILTERED = _(" that match the filters")
...
""".format(MSG_UNRESPONDING, whakerkit.sg.whakerexa+"icons", whakerkit.sg.whakerexa+"/icons")
...
class DocsResponse(get_base_response_class()):
    ...
    def __init__(self):
        super(DocsResponse, self).__init__(name=None, title=MSG_TITLE_TAB)
    ...
    def __append_documents(parent: HTMLNode, doc_manager: WhakerKitDocsManager, filtered=False):
        ...
        """
        p_value = MSG_NB_DOCS.format(nb=len(doc_manager))
        if filtered is True:
            p_value += MSG_ARE_FILTERED
    ...
    def __check_filters(self, filetypes: list, authors: list) -> bool:
        if len(filetypes) == 0:
            self._data = {"info": MSG_NO_FILETYPE}
            return False
        if len(authors) == 0:
            self._data = {"info": MSG_NO_AUTHOR}
            return False
        return True
