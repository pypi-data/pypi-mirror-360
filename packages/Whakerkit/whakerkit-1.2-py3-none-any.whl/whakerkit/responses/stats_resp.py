...
MSG_TITLE_INDEX = _("Consultations")
MSG_TITLE_HOME = _("Website visit frequency")
MSG_TITLE_SEE_LOGS = _("View logs")
MSG_TITLE_SEE_FREQ = _("Number of accesses to HTML pages")
MSG_FREQ_OVERALL = _("Overall view")
MSG_FREQ_DETAILS = _("Monthly visits per page")
MSG_TITLE_SEE_FILTERS = _("Document filters")
MSG_FILTERS_VALID = _("Filters that found documents")
MSG_FILTERS_INVALID = _("Filters that did not find documents")
...
class StatsResponse(get_base_response_class()):
    ...
    def __init__(self):
        ...
        """
        super(StatsResponse, self).__init__(name=None, title=MSG_TITLE_INDEX)
        self.__logs = list()
    ...
    def load_and_view_logs(self):
        ...
        h2_logs.set_value(MSG_TITLE_SEE_LOGS)
            ...
            if entry.endswith(".log") is True:
                with open(whakerkit.sg.base_dir + entry, "r", encoding="utf-8") as f:
                    ...
                    summary_logs = HTMLNode(details_logs.identifier, None, "summary",
                                            value=MSG_EXPAND_FOR_CONTENT.format(entry=entry, content_len=len(content)))
    ...
    def applied_filters(self):
        ...
        h2_logs = self._htree.element("h2")
        h2_logs.set_value(MSG_TITLE_SEE_FILTERS)

        h3_logs = self._htree.element("h3")
        h3_logs.set_value(MSG_FILTERS_INVALID + f" ({len(not_found)})")
        ...
        h3_logs = self._htree.element("h3")
        h3_logs.set_value(MSG_FILTERS_VALID + f" ({len(found)})")
