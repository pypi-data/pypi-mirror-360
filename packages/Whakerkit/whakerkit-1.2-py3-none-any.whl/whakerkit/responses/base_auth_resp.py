...
MSG_BASE_SUCCESS = _("Operation successful")
MSG_AUTHENTICATED = _("User authenticated")

MSG_BASE_FAILED = _("An error occurred: ")
MSG_NOT_AUTHENTICATED = _("User not authenticated: ")
MSG_NON_AUTHORIZED = _("User not authorized: ")
MSG_LOGIN_FAILED = _("Incorrect username or password.")

MSG_LOGOUT_SUCCESS = _("Logout successful")
MSG_LOGOUT_FAILED = _("Logout failed")

MSG_MISSING_JWT = _("PyJWT must be installed on the server to enable login. "
...
class WhakerkitAuthResponse(get_base_response_class()):
    ...
    def login(self, username: str, password: str):
        ...
        self._data = {"error": MSG_LOGIN_FAILED}
        ...
        if len(username)*len(password) > 0:
            ...
            if self._is_authenticated is True:
                ...
                try:
                    ...
                    self._data = {"error": MSG_MISSING_JWT}
            else:
                self._data = {"error": MSG_NOT_AUTHENTICATED + msg}
        else:
            self._data = {"error": MSG_LOGIN_FAILED}
