...
class RunzePump(SyringePump, SerialDevice):
    ...
    def __init__(self,
        ...
        :param pump_model: model of the pump to define the max fine step. 'SY01B' (default) or 'SY01C'
        ...
        if pump_model == 'SY01B':
            self.max_fine_step = 96000
        elif pump_model == 'SY01C':
            self.max_fine_step = 24000
        else:
            raise ValueError(f"Pump model {pump_model} not recognized! Please select 'SY01B' or 'SY01C'")
    ...
    def _set_fine_position(self):
        ...
        print(self.fine_pos)
        if self.fine_pos:
            self._max_step = self.max_fine_step
            self.home_pos *= 8
            self.backlash_pos *= 8
            self._execute_pump(command='N1')
        else:
            self._max_step = 12000
