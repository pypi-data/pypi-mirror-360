...
class LongerPump(SyringePump, SerialDevice):
    ...
    def _set_fine_position(self):
        ...
        """
        self.fine_pos=False
        print("Fine position not available for Longer, set to False ")
        if self.fine_pos:
    ...
    def report_dead_volume(self):
        print(f"Dead volume in steps is {self._query_pump(command='24')}")

    def report_pump_configuration(self):
        print("Pump configuration query not available with Longer")
