...
COM_PORT = "COM5"
def test_init():
    pump = TecanXCPump(com_port=COM_PORT, address=0, syringe_volume=1e-3, num_valve_port=12)
    assert isinstance(pump, TecanXCPump)
...
def test_draw_dispense():
    pump = TecanXCPump(com_port=COM_PORT, address=0, syringe_volume=1e-3, num_valve_port=12)
    pump.draw(0.25, 1, speed=0.5)
    assert pump.volume == 1.0
    assert pump.port == 1
    assert 0.499 <= pump.top_speed_ml <= 0.501
    pump.dispense(0.25, 1)
