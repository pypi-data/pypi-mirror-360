...
class TecanCentrisPump(SyringePump, SerialDevice):
    category="SyringePump"
    def __init__(self,
                 com_port: str,
    ...
    def connect(self):
        ...
        self.set_top_speed(top_speed=self.top_speed, auto_start_stop_speed=True)
        self.set_speed_slope(speed_slope=self.speed_slope)
    ...
    def _query_pump(self, command: str) -> str:
        ...
        """
        rtn = self.query(write_command=f"/{self.address}{command}\r",
                         remove_from_start=4, remove_from_end=3, read_delay= 0.05)
        return rtn
    ...
    def _busy_report(self):
        ...
        #print(rtn)
        if len(rtn) < 4:  # no meaningful return means communication failure, pump is not ready
            self._ready = False
            #print('not meaningful')
            return
        if rtn[3] == 0x60:  # 3rd byte is 0x60 (int 96) means pump is ready
            self._ready = True
            #print('ready')
            return
        else:
            ...
            #print('not ready')
        if rtn[3] != 0x40:  # 3rd byte is 0x40 (int 64) means pump is simply not ready
            error_num = rtn[3] % 16  # not 0x40 means hardware error, raise error message
    ...
    def _set_fine_position(self):
        """
        set the max step of the pump
        use this name so same as _set_fine_position in other pump drivers
        Returns
        -------

        """
        self._max_step = 181490
    ...
    def report_home_position(self):
        """
        report home pos by subtract absolute pos from hardstop with current pos from home
        :return:
        """
        # rtn = self.home_pos
        rtn = int(self._query_pump(command='?0')) - int(self._query_pump(command='?1'))
    ...
    def report_start_speed(self):
        rtn = int(float(self._query_pump(command='?6')))
    ...
    def report_top_speed(self):
        rtn = int(float(self._query_pump(command='?7')))
    ...
    def set_top_speed(self, top_speed: int = 1000, auto_start_stop_speed: bool = True):
        ...
        """
        self._execute_pump(command=f'V64001')
        if top_speed < 50:
            top_speed = 50
        if top_speed > 200000:
            top_speed = 200000
    ...
    def report_stop_speed(self):
        rtn = int(float(self._query_pump(command='?8')))
    ...
    def set_speed_slope(self, speed_slope: int = 8):
        """
        ...
        """
        assert 1 <= speed_slope < 41, "Speed slope out of range"
        self._execute_pump(command=f'L{speed_slope}')

    def set_default_speeds(self):
        self.set_start_speed(self.start_speed)
        self.set_stop_speed(self.stop_speed)
        self.set_speed_slope(self.speed_slope)
        self.set_top_speed(self.top_speed)

    def report_buffer_status(self):
        self._buffer_status = bool(int(float(self._query_pump(command='?67'))))
        #print(f"Buffer status is {str(self._buffer_status)}.")

    def report_total_operating_time(self):
        self._total_operating_time = int(float(self._query_pump(command='?40')))
        print(f"Total_operating_time is {self._total_operating_time}.")

    def report_number_device_power_up(self):
        self._number_device_power_up = int(self._query_pump(command='?41'))
        print(f"Number_device_power_up is {self._number_device_power_up}")

    def report_number_initialization(self):
        self._number_initialization = int(self._query_pump(command='?42'))
        print(f"Number_initialization is {self._number_initialization}.")

    def report_number_initialization_since_last(self):
        self._number_initialization_since_last = int(self._query_pump(command='?43'))
        print(f"Number_initialization is {self._number_initialization_since_last}.")

    def report_plunger_move_distance(self):
        self._plunger_move_distance = int(float(self._query_pump(command='?44')))
        print(f"Plunger_move is {self._plunger_move_distance} meters.")

    def report_plunger_move_counting(self):
        self._plunger_move = int(self._query_pump(command='?45'))
        print(f"Plunger_move is {self._plunger_move}.")

    def report_plunger_move_counting_since_last(self):
        self._plunger_move_since_last = int(self._query_pump(command='?46'))
        print(f"Plunger_move is {self._plunger_move_since_last}.")

    def report_valve_move_counting(self):
        self._valve_move = int(self._query_pump(command='?47'))
        print(f"Valve_move is {self._valve_move}.")

    def report_valve_move_counting_since_last(self):
        self._valve_move_since_last = int(self._query_pump(command='?48'))
        print(f"Valve_move is {self._valve_move_since_last}.")

    def report_current_voltage(self):
        self._current_voltage = float(self._query_pump(command='?50'))
        print(f"Current_voltage is {self._current_voltage}")

    def report_current_temperature(self):
        self._current_temperature = float(self._query_pump(command='?53'))
        print(f"Current_temperature_F is {self._current_temp}")

    def report_firmware_version(self):
        print(f"Firmware version is {self._query_pump(command='?63')}")
    ...
    def report_valve_number(self) -> int:
        """
        with configuration of Eugenia's pump, it does not report 1-12 but o,i,e,b
        firmware CENTRIS-V2.010-04/2016
        :return:
        """
        rtn = int(self._query_pump(command='?20'))
    ...
    def initialize_pump(self, init_valve: int = None, out_valve: int = None):
        ...
        for ready_count in range(0, 30):
            if self._ensure_ready(wait_time=30):
                ...
                self._update_valve_number(out_valve)
    ...
    def move_plunger(self, plunger_pos: int,
        ...
        if ready_after_execute:
            self._ensure_ready()
        if confirmation:
            if plunger_pos -5000 <= self.report_plunger_absolute_position() <= plunger_pos + 5000:
    ...
    def volume(self) -> float:
        ...
        """
        plunger_pos = self.report_plunger_relative_position()
    ...
    def top_speed_ml(self) -> float:
        ...
        """
        return self.report_top_speed() * 5e-5 * self.syringe_volume
    ...
    def top_speed_ml(self, top_speed: float):
        ...
        """
        top_speed_int = int(top_speed / (5e-3 * self.syringe_volume))
