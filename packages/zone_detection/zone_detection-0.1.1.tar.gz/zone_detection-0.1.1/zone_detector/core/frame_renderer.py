...
class FrameRenderer: 
    ...
    def _draw_info_panel(self, frame, current_people_in_zone, tracker, frame_count, start_time):
        ...
        if tracker.warned_people:
            warning_text = f"DANGER: {len(tracker.warned_people)} person exceeded {tracker.warning_time}s"
            (text_width, text_height), _ = cv2.getTextSize(warning_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
            cv2.rectangle(frame, (10, 100), (20 + text_width, 130), (0, 0, 0), -1)  # Nền hộp
            cv2.putText(frame, warning_text, (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)  # Văn bản cảnh báo
