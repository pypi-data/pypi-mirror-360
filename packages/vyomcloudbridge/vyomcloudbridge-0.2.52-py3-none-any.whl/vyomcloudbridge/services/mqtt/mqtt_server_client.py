...
class MqttServerClient:
    ...
    def _on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            logger.info(f"Server client connected successfully to {self.broker_host}:{self.broker_port}")
    ...
    def connect(self):
        ...
        try:
            for attempt in range(self.max_reconnect_attempts):
                try:
                    delay = min(self.base_reconnect_delay * (2 ** attempt), self.reconnect_delay_max)
...
def main():
    try:
        ...
        client = MqttServerClient(
            broker_host="3.110.9.183",  # Change to your EC2 instance IP for remote access
            broker_port=8883,
