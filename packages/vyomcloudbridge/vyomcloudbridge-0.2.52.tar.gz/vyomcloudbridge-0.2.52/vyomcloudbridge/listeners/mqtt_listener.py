...
class MqttListener(AbcListener):
    def __init__(self, daemon: bool = False):
        try:
            super().__init__(
                multi_thread=False, daemon=daemon
            ...
            if self.daemon:
                ...
                self.logger.info(
                    "MqttListener initialization in background thread started..."
                )
            else:
    ...
    def message_callback(self, topic: str, payload, **kwargs):
        ...
        try:
            ...
            if len(topic_parts) == 5:
                self.logger.debug(
                    f"topic_parts[3] {topic_parts[3]} topic_parts[1] {topic_parts[1]} topic_parts[2]: {topic_parts[2]}"
                )
