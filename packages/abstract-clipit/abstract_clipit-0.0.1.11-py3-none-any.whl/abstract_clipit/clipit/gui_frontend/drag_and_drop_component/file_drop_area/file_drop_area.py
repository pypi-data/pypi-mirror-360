...
class FileDropArea(QtWidgets.QWidget):
    ...
    # ────────────────────────────────────────────────────────────────────────
    # FileDropArea.filter_paths
    def filter_paths(self, paths: list[str]) -> list[str]:
        """
        Return every valid file path – whether it came from a directory walk
        or was dropped as an individual file.
        """
        filtered: list[str] = []

        for p in paths:
            if os.path.isdir(p):
                # walk the directory, honouring your exclude rules
                filtered.extend(
                    collect_filepaths(
                        [p],
        ...
        # apply extension allow/deny lists ------------------------------
        allowed = {
            f for f in filtered
            if (ext := os.path.splitext(f)[1].lower()) in self.allowed_extensions
            and ext not in self.unallowed_extentions
        }

        self._log(f"filter_paths → {len(allowed)} file(s): {allowed!r}")
    ...
    def get_contents_text(self, file_path: str, idx: int = 0, filtered_paths: list[str] = []):
        ...
        if ext not in self.unallowed_extentions:
            header = f"=== {file_path} ===\n"
            footer = "\n\n――――――――――――――――――\n\n"
            info   = {
                'path':    file_path,
    ...
    def populate_python_view(self) -> None:
        ...
        all_paths   = [info for info in self._last_raw_paths if info.endswith('.py')]
    ...
    def _populate_text_view(self) -> None:
        ...
        for path, info in self.combined_text_lines.items():
            ...
            lines = info['text']  # this is your full string with headers/footers
    ...
    def _toggle_populate_text_view(self, view_toggle=None) -> None:
        ...
        # rebuild the text view under the new mode
        self._populate_text_view()
