...
from approck_sqlalchemy_utils.types.encrypted.padding import PADDING_MECHANISM
    ...
    def _set_padding_mechanism(self, padding_mechanism=None):
        ...
        if isinstance(padding_mechanism, str):
            if padding_mechanism not in PADDING_MECHANISM.keys():
                raise ImproperlyConfigured("There is not padding mechanism with name {}".format(padding_mechanism))
...
class AesGcmEngine(EncryptionDecryptionBaseEngine):
    ...
    def decrypt(self, value):
        ...
        tag = decrypted[self.IV_BYTES_NEEDED : self.IV_BYTES_NEEDED + self.TAG_SIZE_BYTES]
...
class StringEncryptedType(TypeDecorator, ScalarCoercible):
    ...
    def process_result_value(self, value, dialect):
        ...
        if value is not None:
            ...
            try:
                return self.underlying_type.process_result_value(decrypted_value, dialect)
