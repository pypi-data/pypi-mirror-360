...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  cc_warmup                              # Default: watch mode with auto-warmup
  cc_warmup --no-watch                   # Run once and exit (legacy mode)
  cc_warmup --no-warmup                  # Watch mode without sending warmups
  cc_warmup --dry-run --no-watch         # Show warmup analysis without sending
  cc_warmup --test-warmup                # Test warmup message sending
  cc_warmup --lookback 24                # Analyze last 24 hours only
  cc_warmup --refresh-interval 60        # Watch mode with 60s refresh
  cc_warmup --categories-only            # Show detailed category analytics
  cc_warmup --project "my-app"            # Analyze specific project
        """
    )
