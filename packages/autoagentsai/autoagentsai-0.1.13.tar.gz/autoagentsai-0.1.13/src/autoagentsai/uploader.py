...
def create_file_like_from_path(file_path: str):
    """
    将文件路径转换为文件对象
    
    这是一个辅助函数，用于将本地文件路径转换为可以传递给 ChatClient.invoke() 的文件对象。
    该函数会读取文件内容并创建一个 BytesIO 对象，方便在内存中处理和传输。
    
    Args:
        file_path (str): 本地文件的路径
            - 必填参数，指向要读取的本地文件
            - 支持相对路径和绝对路径
            - 示例: "./document.pdf", "/Users/username/data.xlsx"
            
    Returns:
        BytesIO: 包含文件内容的 BytesIO 对象
            - 带有 name 属性，包含文件名（不含路径）
            - 可以直接传递给 ChatClient.invoke() 的 files 参数
            - 文件内容存储在内存中，适合网络传输
            
    示例:
        >>> # 基本用法
        >>> file_obj = create_file_like_from_path("./document.pdf")
        >>> client.invoke("分析这个文档", files=[file_obj])
        
        >>> # 处理多个文件
        >>> files = [
        ...     create_file_like_from_path("./report1.pdf"),
        ...     create_file_like_from_path("./report2.xlsx")
        ... ]
        >>> client.invoke("对比这两个文件", files=files)
        
    注意:
        - 该函数会将整个文件读入内存，大文件时注意内存使用
        - 确保文件路径存在且可读
        - 建议在 with 语句外使用，避免文件句柄管理问题
    """
...
def create_file_like(file_input, filename: Optional[str] = None):
    """
    将多种类型的文件输入转换为标准的文件对象
    
    这是一个通用的文件处理函数，可以处理本地文件路径、前端上传的文件对象、
    字节数据等多种输入类型，并统一转换为 BytesIO 对象。
    
    Args:
        file_input: 文件输入，支持多种类型：
            - str: 本地文件路径
                示例: "./document.pdf", "/path/to/file.xlsx"
            - bytes: 原始字节数据
                示例: b"file content here"
            - BytesIO: 字节流对象
                示例: BytesIO(b"content")
            - 文件对象: 包含 read() 方法的对象
                示例: open("file.txt", "rb")
            - 前端文件对象: Flask FileStorage、Django UploadedFile、FastAPI UploadFile 等
                
        filename (str, optional): 指定文件名
            - 可选参数，如果不提供会尝试从 file_input 中获取
            - 优先级：filename 参数 > file_input.filename > file_input.name > "uploaded_file"
    ...
    示例:
        ...
        >>> file_obj = create_file_like(content, filename="hello.txt")
        
        >>> # 处理前端上传的文件 (Flask 示例)
        >>> from flask import request
        >>> uploaded_file = request.files['file']
        >>> file_obj = create_file_like(uploaded_file)
        >>> client.invoke("分析这个文档", files=[file_obj])
        ...
        ...     return client.invoke("分析文档", files=[file_obj])
    ...
    Web框架集成示例:
        ...
        ...     file_obj = create_file_like(uploaded_file)
        ...     response = ""
        ...     for chunk in client.invoke("分析这个文件", files=[file_obj]):
        ...         response += chunk
    ...
    注意:
        - 对于大文件，该函数会将整个文件读入内存
        - 确保输入数据有效且可读
        - 对于流式文件对象，调用后文件指针会移动到末尾
    """
    # 处理不同类型的输入
    if isinstance(file_input, str):
        # 文件路径
        return create_file_like_from_path(file_input)
