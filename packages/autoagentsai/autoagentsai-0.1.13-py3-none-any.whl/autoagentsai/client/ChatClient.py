...
class ChatClient:
    def __init__(self, agent_id: str, auth_key: str, auth_secret: str, base_url: str = "https://uat.agentspro.cn"):
        """
        AutoAgents AI 对话客户端
        ...
        示例:
            ...
            ...     auth_key="abcd1234567890", 
            ...     auth_secret="abcd1234567890"
            ... )
        """
        self.agent_id = agent_id
        self.jwt_token = get_jwt_token_api(auth_key, auth_secret, base_url) 
    ...
    def invoke(
        ...
        示例:
            ...
            上传本地文件并分析:
            >>> from src.autoagentsai.uploader import create_file_like_from_path
            >>> file_obj = create_file_like_from_path("./document.pdf")
            >>> for chunk in client.invoke("请分析这个文档", files=[file_obj]):
            ...     print(chunk, end="", flush=True)
            
            处理前端上传的文件（通用方法）:
            >>> from src.autoagentsai.uploader import create_file_like
            >>> # Flask 示例
            >>> uploaded_file = request.files['file']  # 前端上传的文件
            >>> file_obj = create_file_like(uploaded_file)
            >>> for chunk in client.invoke("分析上传的文件", files=[file_obj]):
            ...     print(chunk, end="", flush=True)
    ...
    def history(self):
        ...
        示例:
            >>> client = ChatClient(agent_id="xxx", auth_key="xxx", auth_secret="xxx")
            >>> # 先进行一次对话
            >>> for chunk in client.invoke("你好"):
            ...     print(chunk, end="")
            >>> 
