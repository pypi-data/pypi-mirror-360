...
from netket.vqs.mc.mc_state import MCState
from netket.vqs.mc.mc_state.expect import (
...
@dispatch
def get_local_kernel_arguments(vstate: MCState, op: "ContinuousOperator"):
    """Return samples and arguments for local value evaluation."""

    check_hilbert(vstate.hilbert, op.hilbert)
    sigma = vstate.samples
    args = op._pack_arguments()
    return sigma, args


@dispatch
def get_local_kernel(vstate: MCState, op: "ContinuousOperator"):
    """Return the jit-able kernel computing local values."""

    return op._expect_kernel
