...
from .models import EvaluateMessageResponse, AnalysisContext, CustomField, Actions
from .utils import retry_with_exponential_backoff, async_retry_with_exponential_backoff
import logging
...
class ActiveFenceClient:
    ...
    def __init__(
        ...
        logger.debug("ActiveFenceClient initialized successfully with base_url: %s, app_name: %s", self.base_url,
                     self.app_name)
    ...
    def validate_init_parameters(self) -> None:
        """
    ...
    def handle_api_response(self, detection: dict) -> EvaluateMessageResponse:
    ...
    def sync_internal_call(
        ...
        body = self.create_request_body(
            model, platform, provider, session_id, text, type, user_id, version, custom_fields
        )
        ...
        return self.handle_api_response(detection_response.json())
    ...
    async def async_internal_call(
        ...
        async with self.http_client.post(
            ...
            return self.handle_api_response(await detection_response.json())

    def evaluate_prompt_sync(self, prompt: str, context: AnalysisContext,
                             custom_fields: Optional[set[CustomField]] = None) -> EvaluateMessageResponse:
        """
    ...
    def evaluate_response_sync(self, response: any, context: AnalysisContext,
                               custom_fields: Optional[set[CustomField]] = None) -> EvaluateMessageResponse:
        """
    ...
    async def evaluate_prompt(self, prompt: str, context: AnalysisContext,
                              custom_fields: Optional[set[CustomField]] = None) -> EvaluateMessageResponse:
        """
    ...
    async def evaluate_response(self, response: any, context: AnalysisContext,
                                custom_fields: Optional[set[CustomField]] = None) -> EvaluateMessageResponse:
        """
