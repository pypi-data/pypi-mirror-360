...
class WKPage {
  ...
  async _onTargetCreated(event) {
    ...
    if (!targetInfo.isProvisional) {
      ...
      this._page.reportAsNew(this._opener?._page, pageOrError instanceof _page.Page ? undefined : pageOrError);
    } else {
  ...
  _toolbarHeight() {
    if (this._page._browserContext._browser?.options.headful) return _hostPlatform.hostPlatform === 'mac10.15' ? 55 : 59;
    return 0;
  }
  ...
  _onLoadingFinished(event) {
    ...
    if (response) {
      var _event$metrics$respon, _event$metrics$respon2;
      ...
      if (event.metrics?.protocol) response._setHttpVersion(event.metrics.protocol);
      response.setEncodedBodySize((_event$metrics$respon = event.metrics?.responseBodyBytesReceived) !== null && _event$metrics$respon !== void 0 ? _event$metrics$respon : null);
      response.setResponseHeadersSize((_event$metrics$respon2 = event.metrics?.responseHeaderBytesReceived) !== null && _event$metrics$respon2 !== void 0 ? _event$metrics$respon2 : null);
