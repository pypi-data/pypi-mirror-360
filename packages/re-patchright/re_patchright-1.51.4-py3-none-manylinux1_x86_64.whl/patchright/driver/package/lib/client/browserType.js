...
class BrowserType extends _channelOwner.ChannelOwner {
  ...
  async launch(options = {}) {
    ...
    return await this._wrapApiCall(async () => {
      const browser = _browser.Browser.from((await this._channel.launch(launchOptions)).browser);
  ...
  async _connect(params) {
    ...
    return await this._wrapApiCall(async () => {
      ...
      const result = await (0, _timeoutRunner.raceAgainstDeadline)(async () => {
        ...
        browser = _browser.Browser.from(playwright._initializer.preLaunchedBrowser);
