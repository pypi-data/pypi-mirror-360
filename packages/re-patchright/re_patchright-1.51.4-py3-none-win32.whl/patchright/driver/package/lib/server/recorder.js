...
class Recorder {
  ...
  async _scopeHighlightedSelectorToFrame(frame) {
    ...
    try {
      ...
      if (resolved?.frame === mainFrame) return (0, _selectorParser.stringifySelector)(resolved.info.parsed);

      // selector points to this frame, highlight it
      if (resolved?.frame === frame) return (0, _selectorParser.stringifySelector)(resolved.info.parsed);
  ...
  _updateUserSources() {
    ...
    this._pushAllSources();
    if (fileToSelect) this._recorderApp?.setRunningFile(fileToSelect);
  }
  _pushAllSources() {
