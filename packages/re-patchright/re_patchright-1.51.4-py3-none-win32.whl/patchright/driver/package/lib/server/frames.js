...
class FrameManager {
  ...
  requestStarted(request, route) {
    ...
    if (route) {
      ...
      if (this._page._serverRequestInterceptor?.(r, request)) return;
      if (this._page._clientRequestInterceptor?.(r, request)) return;
      if (this._page._browserContext._requestInterceptor?.(r, request)) return;
      r.continue({
        isFallback: true
      }).catch(() => {});
    }
  ...
  async querySelectorAll(selector) {
    ...
    return await controller.run(async progress => {
      const handles = await this._retryWithProgressIfNotConnected(progress, selector, false, false, async result => {
        if (!result || !result[0]) {
          // No elements found, or result itself is null from _retry...
          return [];
        }
        ...
        for (const scope of currentScopingElements) {
          ...
          if (!isUsingXPath) {
            ...
            for (var shadowRoot of shadowRoots) {
              ...
              const describedHostNode = await client.send('DOM.describeNode', {
                objectId: hostOfShadowRootHandle._objectId
              });
