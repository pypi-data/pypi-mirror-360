...
class BrowserContext extends _instrumentation.SdkObject {
  ...
  async resetForReuse(metadata, params) {
    ...
    await page?.mainFrame().goto(metadata, 'about:blank', {
      timeout: 0
    });
  ...
  async addInitScript(source, name) {
    const initScript = new _page.InitScript(source, false /* internal */, name);
  ...
  async storageState(indexedDB = false) {
    ...
    for (const page of this.pages()) {
      ...
      try {
        ...
        if (storage.localStorage.length || storage.indexedDB?.length) result.origins.push({
          origin,
  ...
  async _resetCookies() {
    await this.doClearCookies();
    if (this._options.storageState?.cookies) await this.addCookies(this._options.storageState?.cookies);
  }
