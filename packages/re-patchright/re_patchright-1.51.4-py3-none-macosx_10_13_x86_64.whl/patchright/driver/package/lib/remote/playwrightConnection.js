...
class PlaywrightConnection {
  ...
  async _initLaunchBrowserMode(scope, options) {
    ...
    if ('bidi' === browserName) {
      if (this._options.launchOptions?.channel?.toLocaleLowerCase().includes('firefox')) browserName = 'bidiFirefox';else browserName = 'bidiChromium';
    }
  ...
  async close(reason) {
    ...
    _debugLogger.debugLogger.log('server', `[${this._id}] force closing connection: ${reason?.reason || ''} (${reason?.code || 0})`);
    try {
