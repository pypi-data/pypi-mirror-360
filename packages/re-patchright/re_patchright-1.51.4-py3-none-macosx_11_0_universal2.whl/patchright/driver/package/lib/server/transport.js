...
class WebSocketTransport {
    ...
    const result = await new Promise((fulfill, reject) => {
      ...
      transport._ws.on('unexpected-response', (request, response) => {
        ...
        for (let i = 0; i < response.rawHeaders.length; i += 2) {
          if (debugLogHeader && response.rawHeaders[i] === debugLogHeader) progress?.log(response.rawHeaders[i + 1]);
        }
        const chunks = [];
  ...
  constructor(progress, url, logUrl, headers, followRedirects, debugLogHeader) {
    ...
    this._ws = new _utilsBundle.ws(url, [], {
      ...
      handshakeTimeout: Math.max((_progress$timeUntilDe = progress?.timeUntilDeadline()) !== null && _progress$timeUntilDe !== void 0 ? _progress$timeUntilDe : 30_000, 1),
      headers,
      followRedirects,
