  ...
  async _customFindElementsByParsed(resolved, client, context, documentScope, progress, parsed) {
    ...
    while (parsed.parts.length > 0) {
      ...
      if (part.name == "nth") {
        ...
        if ((currentScopingElements.length || partNth) && (partNth >= currentScopingElements.length || partNth < -currentScopingElements.length)) {
        ...
        for (const scope of currentScopingElements) {
          ...
          const describedScope = await client.send("DOM.describeNode", {
            ...
            depth: -1,
            pierce: true
            // This 'true' really pierces through closed shadowRoots but not iframes.
          });
