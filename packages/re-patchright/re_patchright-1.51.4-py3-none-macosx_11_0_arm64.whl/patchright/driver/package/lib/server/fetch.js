...
class APIRequestContext extends _instrumentation.SdkObject {
  ...
  _getHttpCredentials(url) {
    if (!this._defaultOptions().httpCredentials?.origin || url.origin.toLowerCase() === this._defaultOptions().httpCredentials?.origin?.toLowerCase()) return this._defaultOptions().httpCredentials;
    return undefined;
  }
}
...
class GlobalAPIRequestContext extends APIRequestContext {
  constructor(playwright, options) {
    ...
    if (options.storageState) {
      this._origins = options.storageState.origins?.map(origin => ({
        indexedDB: [],
        ...origin
      }));
