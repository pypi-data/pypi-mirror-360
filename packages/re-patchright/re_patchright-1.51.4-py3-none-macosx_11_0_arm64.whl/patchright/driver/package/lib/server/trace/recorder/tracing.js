...
class Tracing extends _instrumentation.SdkObject {
  ...
  async startChunk(options = {}) {
    ...
    if (this._state.options.snapshots) await this._snapshotter?.start();
    return {
  ...
  async stopChunk(params) {
    ...
    if (this._state.options.snapshots) await this._snapshotter?.stop();
    this.flushHarEntries();
  ...
  onBeforeInputAction(sdkObject, metadata) {
    if (!this._state?.callIds.has(metadata.id)) return Promise.resolve();
    // IMPORTANT: no awaits before this._appendTraceEvent in this method.
  ...
  async onAfterCall(sdkObject, metadata) {
    if (!this._state?.callIds.has(metadata.id)) return;
