...
class HarTracer {
  ...
  _onAPIRequestFinished(event) {
    ...
    if (!this._options.omitSizes) harEntry.response.bodySize = (_event$body$length = event.body?.length) !== null && _event$body$length !== void 0 ? _event$body$length : 0;
  ...
  async _onRequestFinished(request, response) {
    ...
    if (!this._options.omitServerIP) {
      this._addBarrier(page || request.serviceWorker(), response.serverAddr().then(server => {
        if (server?.ipAddress) harEntry.serverIPAddress = server.ipAddress;
        if (server?.port) harEntry._serverPort = server.port;
      }));
    }
