...
class Frame extends _channelOwner.ChannelOwner {
  ...
  async waitForNavigation(options = {}) {
    return await this._page._wrapApiCall(async () => {
      ...
      const navigatedEvent = await waiter.waitForEvent(this._eventEmitter, 'navigated', event => {
        ...
        return (0, _urlMatch.urlMatches)(this._page?.context()._options.baseURL, event.url, options.url);
      });
  ...
  async waitForURL(url, options = {}) {
    if ((0, _urlMatch.urlMatches)(this._page?.context()._options.baseURL, this.url(), url)) return await this.waitForLoadState(options.waitUntil, options);
