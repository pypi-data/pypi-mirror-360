...
class Request extends _channelOwner.ChannelOwner {
  ...
  _safePage() {
    return _frame.Frame.fromNullable(this._initializer.frame)?._page || null;
  }
  serviceWorker() {
  ...
  _targetClosedScope() {
    return this.serviceWorker()?._closedScope || this._safePage()?._closedOrCrashedScope || new _manualPromise.LongStandingScope();
  }
}
