  ...
  async fulfill(response) {
    ...
    if (isTextHtml && allInjections.length) {
      ...
      allInjections.forEach((script) => {
        ...
        injectionHTML += `<script class="${this._page._delegate.initScriptTag}"${useNonce ? ` nonce="${scriptNonce}"` : ""} type="text/javascript">document.currentScript.remove();${scriptSource}</script>`;
      });
