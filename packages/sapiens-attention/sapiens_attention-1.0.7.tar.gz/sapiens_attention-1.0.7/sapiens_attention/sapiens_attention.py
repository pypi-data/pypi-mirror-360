...
class SapiensAttention:
    ...
    def get_attention_words(self, text='', maximum_length=10):
        try:
            ...
            if keyword_length < 1: keywords = [_clean_text(text=keyword[0]).lower().strip() for keyword in self.__Counter(self.__findall(r'\b\w{5,}\b', text.lower())).most_common(maximum_length)]
