...
class Paylix:
    ...
    class PaylixException(Exception):
    ...
    def __init__(self, api_key, merchant=""):
        ...
        self.api_endpoint = 'https://dev.paylix.gg/v1'

    def handle_response(self, response, key=None):
        if response["status"] != 200:
            raise self.PaylixException(response["error"], response["status"])
    ...
    def request(self, component, action="get", payload={}):
        ...
        if self.merchant:
            headers["X-Paylix-Merchant"] = self.merchant
