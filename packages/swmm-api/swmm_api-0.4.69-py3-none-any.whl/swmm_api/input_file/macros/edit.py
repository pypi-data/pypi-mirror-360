...
def delete_link(inp: SwmmInput, link: str):
    ...
    delete the link objects and all relevant objects.
    XSECTIONS, LOSSES, VERTICES, REPORT, TAG, CONTROLS, INLET_USAGE

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        link (str): label of link to delete.
...
def delete_subcatchment(inp: SwmmInput, subcatchment: str):
    """
    Delete subcatchment from model.

    Delete subcatchment objects and all relevant objects.
    SUBCATCHMENTS, SUBAREAS, INFILTRATION, POLYGONS, LOADINGS, COVERAGES, REPORT, TAG, GWF, GROUNDWATER, LID_USAGE

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        subcatchment (str): subcatchment label.
...
def combine_vertices(inp: SwmmInput, label1, label2):
    """
    Combine the vertice of two links.

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        label1 (str): label of the first link.
        label2 (str): label of the second link.
...
def rename_subcatchment(inp: SwmmInput, old_label: str, new_label: str):
    """
    Rename subcatchment object and update all relevant mentions.

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        old_label (str): previous subcatchment label.
        new_label (str): new subcatchment label.
...
def flip_link_direction(inp, link_label):
    ...
    Exchange the "from_node" with the "to_node".

    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        link_label (str): link label.
...
def copy_link(inp_from: SwmmInput, inp_to: SwmmInput, link_label):
    """
    Copy a link (with its additional sections) from one input file to another.

    Args:
        inp_from (swmm_api.SwmmInput): base SWMM input-file data.
        inp_to (swmm_api.SwmmInput): target SWMM input-file data.
        link_label: label of the link to copy.
...
def copy_node(inp_from: SwmmInput, inp_to: SwmmInput, node_label):
    """
    Copy a node (with its additional sections) from one input file to another.

    Args:
        inp_from (swmm_api.SwmmInput): base SWMM input-file data.
        inp_to (swmm_api.SwmmInput): target SWMM input-file data.
        node_label (str): label of the node to copy.
