...
from .macros import subcatchments_data_frame, nodes_data_frame, links_data_frame
...
def convert_inp_to_geo_package(inp_fn, gpkg_fn=None, driver='GPKG', label_sep='.', crs=None):
    """
...
def write_geo_package(inp, gpkg_fn, driver='GPKG', label_sep='.', crs=None,
    ...
    if COORDINATES in inp:
        ...
        simplify_vertices(inp, distance=1., min_length=simplify_link_min_length)
...
def links_geo_data_frame(inp, label_sep='.'):
    """
    Combine all information of the links from the input file into a single geo-data-frame.
    ...
    data = links_data_frame(inp, label_sep=label_sep, add_section_label=True, add_vertices=False)
    complete_vertices(inp)
    return gpd.GeoDataFrame(data, geometry=inp[VERTICES].geo_series)


def nodes_geo_data_frame(inp, label_sep='.'):
    """
    Combine all information of the nodes from the input file into a single geo-data-frame.
    ...
    data = nodes_data_frame(inp, label_sep=label_sep, add_section_label=True, add_coordinates=False)
    return gpd.GeoDataFrame(data, geometry=inp[COORDINATES].geo_series)


def subcatchment_geo_data_frame(inp: SwmmInput, label_sep='.'):
    """
    Combine all information of the subcatchments from the input file into a single geo-data-frame.
    ...
    Returns:
        geopandas.GeoDataFrame: subcatchments as geo-data-frame
    """
    import geopandas as gpd

    data = subcatchments_data_frame(inp, label_sep=label_sep, add_section_label=True)
    return gpd.GeoDataFrame(data, geometry=inp[POLYGONS].geo_series)


def gpkg_to_swmm(fn, label_sep='.', infiltration_class=None, custom_section_handler=None, simplify=None):
    """
    ...
    Args:
        ...
        label_sep (str):  separator for attribute label between section header and object attribute.
            I.e. "JUNCTIONS.elevation" with label_sep='.'
        infiltration_class (type): InfiltrationGreenAmpt or InfiltrationHorton or InfiltrationCurveNumber
        custom_section_handler (dict[str, type]): mapping of section label as key to section class (child class of InpSectionGeneric or BaseSectionObject) as value. Default: :py:const:`SECTION_TYPES`.
        simplify (float or int): Distance of unneeded vertices to be removed with the Ramer-Douglas simplification.
    Returns:
        SwmmInput: inp data
    """
    import geopandas as gpd
    ...
    if simplify is not None and isinstance(simplify, (float, int)):
        simplify_vertices(inp, distance=simplify)
    reduce_vertices(inp)
...
def update_length(inp):
    ...
    .. Important::
        Works inplace.
        Only tested for metric system (updating the length in meters)
    """
    if OPTIONS in inp and inp.OPTIONS.is_metric():
        for c in inp.CONDUITS.values():
            c.length = inp.VERTICES[c.name].geo.length
    else:
        raise NotImplementedError('update_length is not implemented for imperial unit system. '
                                  'Please consider contributing to the swmm_api project to make it more universal.')
...
def update_area(inp, decimals=4):
    ...
    Args:
        inp (swmm_api.SwmmInput): SWMM input-file data.
        decimals (int): decimal places to round the new value (default=4).
                Higher precision doesn't influence the simulation results but makes the input file messy and bigger.

    .. Important::
        Works inplace.
        Only tested for metric system (updating the area in ha)
        And multiplies 10^4 to the area calculated from the polygon to convert m² to ha.
    """
    if OPTIONS in inp and inp.OPTIONS.is_metric():
        for sc in inp.SUBCATCHMENTS.values():
            sc.area = round(inp.POLYGONS[sc.name].geo.area, decimals-4) * 1e-4  # m² to ha
    else:
        raise NotImplementedError('update_area is not implemented for imperial unit system. '
                                  'Please consider contributing to the swmm_api project to make it more universal.')
