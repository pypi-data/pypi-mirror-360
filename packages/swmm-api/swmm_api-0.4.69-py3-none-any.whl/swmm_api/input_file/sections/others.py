...
class RainGage(BaseSectionObject):
    ...
    Attributes:
        ...
        interval (str | pandas.Timedelta): time interval between gage readings in decimal hours or hours:minutes format (e.g., 0:15 for 15-minute readings).
    ...
    def __init__(self, name, form, interval, SCF, source: Literal['TIMESERIES', 'FILE'] or str, *args, timeseries=np.nan, filename=np.nan, station=np.nan, units: str or Literal['MM', 'IN'] = np.nan):
        ...
        Args:
            name (str): Name assigned to rain gage.
            form (str): Form of recorded rainfall, either ``INTENSITY``, ``VOLUME`` or ``CUMULATIVE`` (:attr:`RainGage.FORMATS`).
            interval (str or pandas.Timedelta): time interval between gage readings in decimal hours or hours:minutes format (e.g., 0:15 for 15-minute readings).
            SCF (float): Snow catch deficiency correction factor (use 1.0 for no adjustment).
...
class Curve(BaseSectionObject):
    ...
    Remarks:
        ...
        user’s choice of flow units set in the [``OPTIONS``] section (:class:`OptionSection`)):
            ...
            - ``PUMP1``: pump outflow v. increment of inlet node volume in ft3 (m3) | Consists of a series of constant flow rates that apply over a series of volume intervals at the pump’s inlet node.
            - ``PUMP2``: pump outflow v. increment of inlet node depth in ft (m) | Similar to a Type1 pump except that the fixed flow rate levels vary over a set of depth intervals at the pump’s inlet node.
            - ``PUMP3``: pump outflow v. head difference between outlet and inlet nodes in ft (m) | Uses a pump characteristic curve at some nominal impeller speed to relate flow rate and delivered head.
            - ``PUMP4``: pump outflow v. continuous depth in ft (m) | A variable speed pump where flow varies continuously with inlet node water depth.
...
class SnowPack(BaseSectionObject):
    ...
    def to_inp_line(self):
        ...
        for pack in self.PARTS._possible:
            if (pack in self.parts) and (self.parts[pack] is not None):
