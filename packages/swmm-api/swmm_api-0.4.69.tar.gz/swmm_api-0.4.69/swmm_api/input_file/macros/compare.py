    ...
    def get_diff_string(self):
        ...
        if self.dict_not_equal:
            ...
            if len(unique_changes) < len(self.dict_not_equal)*0.5:
                dict_not_equal_r = _reverse_dict(self.dict_not_equal)
...
def compare_sections(sec1, sec2, precision=3):
    ...
    Args:
        sec1 (InpSection): first inp file section.
        sec2 (InpSection): second inp file section.
        precision (int): number of relevant decimal places.
...
def compare_inp_files(fn1, fn2, precision=2, skip_section=None, sep='\n' + '#' * 100, show_progressbar=True, skip_equal=False):
    ...
    return s + compare_inp_objects(inp1, inp2, precision=precision, skip_section=skip_section, sep=sep,
                                   show_progressbar=show_progressbar, skip_equal=skip_equal)


def compare_inp_objects(inp1, inp2, precision=2, skip_section=None, sep='\n' + '#' * 100, show_progressbar=True, skip_equal=False):
    """
    Compare inp objects and get the differences as string output.

    Args:
        inp1 (SwmmInput): first inp data.
        inp2 (SwmmInput): second inp data.
        precision (int): number of relevant decimal places.
        skip_section (list): skip sections if you don't care for specific changes.
        sep (str): separator between sections for the output string.
        show_progressbar (bool): show progress bar.
    ...
    Returns:
        str: differences of the inp data objects.
    """
    s = ''
    ...
    s = f'Following sections are equal sections:\n {equal_sections}\n{s}'
...
def _inp_version_control(inp_v1, inp_v2):  # Work in progress
