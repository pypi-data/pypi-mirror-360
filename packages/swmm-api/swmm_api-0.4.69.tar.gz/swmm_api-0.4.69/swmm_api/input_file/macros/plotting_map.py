...
def init_empty_map_plot(ax=None) -> (plt.Figure, plt.Axes):
    """
    Initialize empty map plot.

    Args:
        ax (plt.Axes): Axes for the map plot.
...
def add_link_map(ax: plt.Axes, inp: SwmmInput,
    ...
    for link, vertices in inp[VERTICES].items():
        ...
        if colorbar_kwargs is None and add_kind_legend and section_label not in _types_done:
            style['label'] = section_label
            _types_done.add(section_label)
...
def add_subcatchment_map(ax: plt.Axes, inp: SwmmInput,
    ...
    for label, poly in inp[POLYGONS].items():  # type: Polygon
        ...
        # ---
        if ('label' in kwargs) and (kwargs['label'] == SUBCATCHMENTS) and _label_add_for_legend:
            del kwargs['label']
        if not colorbar_kwargs and not _label_add_for_legend:
            kwargs.setdefault('label', SUBCATCHMENTS)
            _label_add_for_legend = True
        ...
        if add_center_point:
            ...
            _center_points = ax.plot(center.x, center.y, lw=0, **center_point_kwargs)
...
class PlottingMap:
    ...
    def __init__(self, inp: SwmmInput, ax: plt.Axes = None):
        ...
        self.fig, self.ax = init_empty_map_plot(ax=ax)
