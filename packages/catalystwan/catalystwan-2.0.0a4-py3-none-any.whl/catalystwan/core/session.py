...
def determine_session_type(
    ...
    modes_map = {
        (
            TenancyMode.SINGLE_TENANT,
            UserMode.TENANT,
            ViewMode.TENANT,
        ): SessionType.SINGLE_TENANT,
        (
            TenancyMode.MULTI_TENANT,
            UserMode.PROVIDER,
            ViewMode.PROVIDER,
        ...
        (
            ...
            UserMode.PROVIDER,
            ViewMode.TENANT,
        ): SessionType.PROVIDER_AS_TENANT,
        (
            TenancyMode.MULTI_TENANT,
            UserMode.TENANT,
            ViewMode.TENANT,
    ...
    try:
        return modes_map.get(
            (TenancyMode(tenancy_mode), UserMode(user_mode), ViewMode(view_mode)),
            SessionType.NOT_DEFINED,
        )
    except ValueError:
...
class ManagerSession(ManagerResponseAdapter, SessionInterface):
    ...
    def _fetch_server_info(self) -> dict:
        try:
            server_info = self.get("dataservice/client/server").json()["data"]
    ...
    def _finalize_login(self, server_info: dict) -> None:
        ...
        view_mode = server_info.get("viewMode")
    ...
    def request(self, method, url, *args, **kwargs) -> ManagerResponse:
        ...
        ):  # do not modify user provided kwargs unless property is set
        ...
        try:
            ...
            raise ManagerRequestException(
                *exception.args, request=exception.request, response=exception.response
            )
        ...
        if response.jsessionid_expired and self.state in [
            ...
            if self.state == ManagerSessionState.LOGIN_IN_PROGRESS:
                ...
                self.logger.warning(
                    "Logging to session. Reason: expired JSESSIONID detected in response headers"
                )
                self.state = ManagerSessionState.LOGIN
            return self.request(method, url, *args, **_kwargs)
        ...
        if response.api_gw_unauthorized and self.state in [
            ...
            if self.state == ManagerSessionState.LOGIN_IN_PROGRESS:
                ...
                self.logger.warning(
                    "Logging to API GW session. Reason: unauthorized detected in response headers"
                )
                self.state = ManagerSessionState.LOGIN
            return self.request(method, url, *args, **_kwargs)
