...
class NS_Common_Keywords(object):
    ...
    def wacht_op_laden_element(self, xpath, eindstatus='visible', wachttijd='10s'):
        ...
        logger.debug(f"Type selector: {type(xpath)} | waarde: {xpath}")
        logger.debug(f"Type eindstatus: {type(eindstatus)} | waarde: {eindstatus}")
        logger.debug(f"Type wachttijd: {type(wachttijd)} | waarde: {wachttijd}")

        geldige_statussen = {'attached', 'detached', 'visible', 'hidden', 'enabled', 'disabled', 'editable'}
        if eindstatus not in geldige_statussen:
            raise ValueError(f"'{eindstatus}' is geen geldige eindstatus. Kies uit: {', '.join(geldige_statussen)}")

        browser_lib.wait_for_elements_state(locator=xpath, statue=eindstatus, timeout=wachttijd)
