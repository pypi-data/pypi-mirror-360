...
class NS_Common_Keywords(object):
    ...
    def wacht_op_laden_element(self, xpath, eindstatus='visible', wachttijd='10s'):
        ...
        # Verwijder 'xpath=' prefix indien aanwezig
        if isinstance(xpath, str) and xpath.startswith('xpath='):
            selector = xpath[len('xpath=')]
        else:
            selector = xpath

        logger.info(f"Type selector: {type(xpath)} | waarde: {selector}")
        ...
        browser_lib.wait_for_elements_state(selector=selector, state=eindstatus, timeout=wachttijd)
