...
def analyze_walmart_access():
    ...
    walmart_locations = [
        {
            ...
            "lat": 37.7761,  # Approximate coordinates
            "lon": -100.0203
        },
        {
            "name": "Great Bend", 
            "address": "3503 10th St, Great Bend, KS 67530",
            "lat": 38.3639,  # Approximate coordinates
            "lon": -98.8034
        },
        {
            "name": "Pratt",
            "address": "2003 E 1st St, Pratt, KS 67124", 
            "lat": 37.6439,  # Approximate coordinates
            "lon": -98.7115
        }
    ]
    ...
    for i, walmart in enumerate(track(walmart_locations, description="Analyzing locations...")):
        ...
        try:
            with SocialMapperClient() as client:
                # Build analysis configuration using location-based approach
                # We'll search for Walmart near the coordinates
                analysis = (
                    client.create_analysis()
                    .with_location(walmart['address'], "KS")
                    .with_osm_pois("shop", "supermarket", {"brand": "Walmart"})
                    .with_travel_time(120)  # 2 hours = 120 minutes
                    ...
                    .limit_pois(1)  # Only analyze the nearest Walmart
                    .build()
                )
                ...
                if result.is_ok():
                    ...
                    all_results.append({
                        ...
                        "coordinates": {"lat": walmart['lat'], "lon": walmart['lon']},
    ...
    console.print("The 2-hour isochrones from these three Walmart Supercenters cover the following areas:")
    ...
    console.print("\nTo determine if anyone lives beyond 2 hours from a Walmart, you would need to:")
    console.print("1. Overlay these isochrones on a map of Kansas")
    console.print("2. Identify populated areas outside the isochrone boundaries")
    console.print("3. Check census data for those uncovered areas")
    

def create_poi_file(walmart, output_dir):
    """Create a POI file for a single Walmart location."""
    poi_file = output_dir / "walmart_poi.json"
    poi_data = {
        "type": "FeatureCollection",
        "features": [{
            "type": "Feature",
            "properties": {
                "name": f"Walmart Supercenter - {walmart['name']}",
                "type": "shop",
                "shop": "supermarket"
            },
            "geometry": {
                "type": "Point",
                "coordinates": [walmart['lon'], walmart['lat']]
            }
        }]
    }
