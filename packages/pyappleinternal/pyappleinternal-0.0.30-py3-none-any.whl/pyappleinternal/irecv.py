...
from pyappleinternal import get_libusb_path
...
class IRecv:
    def __init__(self, ecid=None, timeout=3, is_recovery=None):
        self.libusb_path=get_libusb_path()
    ...
    def serial_number(self) -> int:
        ...
        except:return self._device_info['ECID']
    ...
    def _find(self, ecid=None, timeout=0xffffffff, is_recovery=None):
        ...
        while (self._device is None) and (time.time() < end):
            try:
                result=find(find_all=True,backend=usb.backend.libusb1.get_backend(find_library=lambda x: self.libusb_path))
