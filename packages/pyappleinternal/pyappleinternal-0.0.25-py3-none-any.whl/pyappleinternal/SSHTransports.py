...
class SSHTransports():
    ...
    def command(self, command, timeout=3, callback=None):
        try:
            self.connect()
    ...
    def invoke_shell(self, command,timeout=5):
        try:
            self.connect()
    ...
    def delete(self, *remote_path):
        ...
        try:
            ...
            self.command(f"rm -rf {remote_list}")
        except Exception as e:
            print(e)
            status=  False
        finally:
    ...
    def find_file(self,remote_path,filename):
        ...
        try:
            ...
            for file_attr in files:
                if filename.lower() in file_attr.filename.lower():
                    file_reslut.append(f"{remote_path}/{file_attr.filename}")
        except Exception as e:
            print(e)
        finally:
            if sftp!=None:sftp.close()
    ...
    def decompress(self, remote_path):
        try:
            ...
            self.command(f"cd '{os.path.dirname(remote_path)}';mkdir '{base_name}';bsdtar -xf '{remote_path}' -C ./'{base_name}'",timeout=None)
            return True
        except Exception as e:
            print(e)
            return False
    ...
    def showimage(self, path,internal=False):
        try:
            if internal:
                ...
                self.command(f"diagstool lcdmura --imagepath '{path}'")
            else:
    ...
    def unlock_ssh(self):
        try:
            self.command(f"OSDToolbox display -s 1 & diagstool bootargs --a rdar102068001=yes --a rdar102068389=yes --a rdar102001044=yes ; reboot")
    ...
    def appswitch(self,mode,add=None):
        try:
            ...
            self.command(f"OSDToolbox display -s 1 &>/dev/null & {OS_command};OSDToolbox appswitch -b")
            return self.get_bootargs() 
