...
from s1_cns_cli.cli.registry import CodeTypeSubParser, HttpMethod, TIME_FORMAT, SUPPORTED_FRAMEWORKS, \
    IacFramework, IacConfigData, MissingConfig, BASELINE_FILE, LogColors, MissingDependencies, \
    DOWNLOAD_CACHE_URL, PUBLISH_ISSUES_API, InvalidInput, MissingRequiredFlags, SUPPORTED_GIT_PROVIDERS, \
    APP_URL, OutputFormat, SEVERITY_MAP, RegoException
from s1_cns_cli.cli.utils import make_request, read_json_file, write_json_to_file, check_if_paths_exist, wrap_text, \
...
def iac_post_evaluation(args, results, cache_directory):
    ...
    if len(filtered_results) > 0:
        ...
        send_issues_to_sentinelone(args, filtered_results, global_config_data)
