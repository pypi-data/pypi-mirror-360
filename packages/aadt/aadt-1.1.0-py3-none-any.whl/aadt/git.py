...
class Git:
    ...
    def check_git_availability() -> None:
        ...
        if not Git.is_git_available():
            raise GitAvailabilityError(
                "Git is not available or current directory is not a Git repository"
            )
    ...
    def get_latest_tag(self) -> str:
        ...
        try:
            version = call_shell(
                ["git", "describe", "--tags", "--abbrev=0"], error_exit=False
            )
    ...
    def get_current_commit(self) -> str:
        ...
        try:
            commit = call_shell(
                ["git", "rev-parse", "--short", "HEAD"], error_exit=False
            )
    ...
    def parse_version(self, vstring: str | None = None) -> str:
        ...
        match vstring:
            ...
            case "dev":
                ...
                try:
                    git_status = call_shell(
                        ["git", "status", "--porcelain"], error_exit=False
                    )
                    if git_status.strip() == "":
                        return self.get_current_commit()
                    return "dev"
...
class FileSystemArchiver:
    ...
    def archive(self, version: str, outpath: Path) -> bool:
        ...
        logging.info(
            f"Archiving current directory (version: {version}) to {outpath}..."
        )
        ...
        self._copy_directory_selective(
            self.project_root, src_path, self.exclude_patterns
        )
    ...
    def _copy_directory_selective(
        self, src_dir: Path, dst_dir: Path, exclude_patterns: set[str]
    ) -> None:
        """
...
class VersionManager:
    ...
    def __init__(self, project_root: Path, exclude_patterns: list[str] | None = None):
        if Git.is_git_available():
            ...
            logging.warning(
                "Git not found or not in a Git repository. "
                "Using fallback for versioning."
            )
    ...
    def parse_version(self, vstring: str | None = None) -> str:
        ...
        try:
            ...
            if isinstance(self.strategy, Git):
                logging.warning(
                    f"Git version parsing failed: {e}, falling back to filesystem"
                )
    ...
    def archive(self, version: str, outpath: Path) -> bool:
        ...
        try:
            ...
            if isinstance(self.strategy, Git):
                logging.warning(
                    f"Git archive failed: {e}, falling back to filesystem copy"
                )
                fallback = FileSystemArchiver(self.project_root, self.exclude_patterns)
            ...
            if isinstance(self.strategy, Git):
                logging.warning(
                    f"Git archive failed with unexpected error: {e}, "
                    "falling back to filesystem copy"
                )
                fallback = FileSystemArchiver(self.project_root, self.exclude_patterns)
                return fallback.archive(version, outpath)
            else:
