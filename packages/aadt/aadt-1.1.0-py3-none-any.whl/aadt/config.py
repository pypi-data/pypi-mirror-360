...
class BuildConfig:
    ...
    output_dir: str = "dist"
    trash_patterns: list[str] = field(
        default_factory=lambda: ["*.pyc", "*.pyo", "__pycache__"]
    )
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "BuildConfig":
        ...
        init_data = {k: v for k, v in data.items() if hasattr(cls, k)}
...
class Config(UserDict[str, Any]):
    ...
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        try:
            ...
            raise ConfigError(
                f"Config validation failed for '{self._path}': {e.message}"
            ) from e
    ...
    def _write(self, data: dict[str, Any]) -> None:
        try:
            ...
            raise ConfigError(
                f"Could not write to config file '{self._path}': {e}"
            ) from e
