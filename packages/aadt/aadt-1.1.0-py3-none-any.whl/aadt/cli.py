...
from . import COPYRIGHT_MSG, DIST_TYPES, PATH_PACKAGE, PATH_PROJECT_ROOT
...
def test(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        try:
            ...
            raise CLIError(
                "Anki (aqt) not found. Make sure it's installed: uv sync --group dev"
            ) from e
        except Exception as e:
    ...
    try:
        ...
        if not validate_cwd():
            raise CLIError(
                ...
                "Please run this from the add-on project root."
            )
...
def _copy_file(source_path, output_path, force, file_description):
    ...
    try:
        ...
        if output_path.exists() and not force:
            print(f"ℹ️  {file_description} already exists, skipping (use --force to overwrite)")
            return False
...
def construct_parser() -> argparse.ArgumentParser:
    ...
    link_group = subparsers.add_parser(
        "link", help="Link add-on to Anki's addon directory for development"
    )
    ...
    test_group = subparsers.add_parser(
        "test", help="Link add-on and launch Anki for testing"
    )
    test_group.set_defaults(func=test)

    claude_group = subparsers.add_parser(
        "claude", 
        help="Generate Claude AI assistant memory file for better assistance"
    )
