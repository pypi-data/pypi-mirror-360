...
class UIBuilder:
    ...
    def _build(
        ...
        if not tool_path:
            ...
            logging.error(
                f"Could not find any of the required UI compilation tools: {tools_list}"
            )
            logging.error(
                "PyQt6 should be available through AADT. Try: uv sync --group dev"
            )
            return False
        ...
        for file in ui_files:
            ...
            try:
                ...
                ui_file_str = (
                    self._relative_to_cwd(e.ui_file) if e.ui_file else "unknown file"
                )
                logging.error(
        ...
        try:
            ...
            if class_match:
                ...
                for pattern, qt_type in type_mappings.items():
                    if pattern in ui_class_suffix:
                        # Verify the type exists in aqt.qt
                        if hasattr(aqt.qt, qt_type):
                            return qt_type
