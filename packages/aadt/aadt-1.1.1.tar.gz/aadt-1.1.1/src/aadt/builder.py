...
from . import DIST_TYPES, PATH_DIST, PATH_PROJECT_ROOT
from .config import Config
from .git import VersionManager
from .manifest import DistType, ManifestUtils
...
class AddonBuilder:
    def __init__(
        ...
        self._path_optional_icons = (
            PATH_PROJECT_ROOT / "ui" / "resources" / "icons" / "optional"
        )
        self._path_changelog = PATH_DIST / "CHANGELOG.md"

        # License paths from config
        self._paths_licenses = [
            PATH_DIST if path == "." else PATH_DIST / path
            for path in self._build_config.license_paths
        ]
