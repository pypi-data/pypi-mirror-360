...
export function initPythonBridge(url: string = "ws://127.0.0.1:8777/ws"): Promise<void> {
  ...
  */
  ...
  connectionPromise = new Promise<void>((resolve, reject) => {
    // Resolve the URL for JupyterHub environment
    let resolvedURL = resolveWebSocketURL(url);

    // Check if we need to add bonnieUrl as query parameter
    const shouldAddBonnieParam = url.startsWith('/') && (window as any).escobarCurrentSettings?.bonnieUrl;

    if (shouldAddBonnieParam) {
      const bonnieUrl = (window as any).escobarCurrentSettings.bonnieUrl;
      console.log(`>>> Adding bonnieUrl as query parameter: ${bonnieUrl}`);

      // Add bonnieUrl as query parameter
      const separator = resolvedURL.includes('?') ? '&' : '?';
      resolvedURL += `${separator}bonnie_url=${encodeURIComponent(bonnieUrl)}`;
  ...
  // If not, try to get it from localStorage
    ...
    try {
      ...
      if (savedSettings) {
        ...
        user_id = settings.username || 'VoittaDefaultUser';
      }
    } catch (error) {
