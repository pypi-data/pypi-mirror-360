...
export function initPythonBridge(url: string = "ws://127.0.0.1:8777/ws"): Promise<void> {
  ...
  */
  ...
  connectionPromise = new Promise<void>((resolve, reject) => {
    ...
    // Check if bonnieUrl is set in settings and use it directly
    const currentSettings = (window as any).escobarCurrentSettings;
    if (currentSettings?.bonnieUrl) {
      finalURL = currentSettings.bonnieUrl;
      console.log(`>>> Using bonnieUrl from settings: ${finalURL}`);
    } else {
      // Resolve the URL for JupyterHub environment only if no bonnieUrl is set
      finalURL = resolveWebSocketURL(url);
