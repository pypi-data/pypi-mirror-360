...
- Supports custom input and output directories
- Supports Tags column with automatic sanitization for Anki compatibility
"""
        ...
        for index, row in df.iterrows():
            try:
                ...
                if has_tags and not pd.isna(row['Tags']):
                    ...
                    if tags_str:
                        # Split tags by comma, clean them, and sanitize for Anki
                        raw_tags = [tag.strip() for tag in tags_str.split(',') if tag.strip()]
                        tags = [sanitize_tag(tag) for tag in raw_tags if sanitize_tag(tag)]
...
CSV Format:
  ...
  HTML formatting is supported in both Front and Back columns.
  Tags should be comma-separated (e.g., "tag1,tag2,tag3").
  German characters in tags will be automatically converted (ü→ue, ä→ae, ö→oe).
        """
    )
