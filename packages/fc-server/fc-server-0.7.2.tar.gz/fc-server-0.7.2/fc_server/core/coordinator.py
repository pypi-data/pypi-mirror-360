...
class Coordinator:
    ...
    async def __managed_issue_resources_connect(self):
        for resource in self.__managed_issue_disconnect_resources:
            if await self.__default_framework_plugin.default_framework_connect(
                ...
                self.update_to_shelve(
                    "managed_issue_disconnect_resources",
                    self.__managed_issue_disconnect_resources,
                )
    ...
    async def kick_booked_resource(self, resource):
        target_framework = None
        status = self.managed_resources_status.get(resource)
        for framework in self.framework_instances:
            if framework.__module__.split(".")[-1] in status:
                target_framework = framework
                break

        if not target_framework:
            return 1, "no need to kick"

        if not Config.frameworks_config[target_framework.__module__.split(".")[-1]].get(
            "allow_booking_kick", True
        ):
            return 1, f"{target_framework.__module__.split('.')[-1]} refused be kicked"

        await target_framework.force_kick_off(resource)
    ...
    def __set_resource_status(self, resource, status):
        ...
        asyncio.create_task(self.publisher_queue.put(resource + ":" + status))
    ...
    async def return_resource(self, resource):
        ...
        if Config.default_framework and resource in self.__managed_disconnect_resources:
            ...
            self.update_to_shelve(
                "managed_disconnect_resources", self.__managed_disconnect_resources
            )
    ...
    def reconstruct_from_shelve(self):
        with shelve.open(Config.db_file) as shelf:
            self.__managed_disconnect_resources = shelf.get(
                "managed_disconnect_resources", []
            )
            self.logger.info("Managed disconnect resources:")
            self.logger.info(self.__managed_disconnect_resources)

            self.__managed_issue_disconnect_resources = shelf.get(
                "managed_issue_disconnect_resources", []
            )
            self.logger.info("Managed issue disconnect resources:")
            self.logger.info(self.__managed_issue_disconnect_resources)

    def update_to_shelve(self, key, value):
        with shelve.open(Config.db_file, writeback=True) as shelf:
            shelf[key] = value
