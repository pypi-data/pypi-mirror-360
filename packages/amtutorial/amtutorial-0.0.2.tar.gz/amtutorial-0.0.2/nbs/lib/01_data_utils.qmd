# data_utils
> A collection of useful functions and classes for downloading and processing the data used in this tutorial.
...
def get_cache_dir(
    ...
    if cache_dir := os.environ.get(f'{APP_NAME}_CACHE_DIR'):
        ...
        cache_base = pathlib.Path(os.environ.get('XDG_CACHE_HOME', '~/.cache')).expanduser() # Linux, macOS, Colab
...
def prepare_bipolar_mnist(force=False):
    ...
    if not force and all((MNIST_DIR / file).exists() for file in required_files): return

    mnist = datasets.load_dataset("mnist").with_format("jax")
    Xtrain = mnist['train']['image']
    Ytrain = mnist['train']['label']
    Xtest = mnist['test']['image']
    Ytest = mnist['test']['label']

    # Bipolarize
    Xtrain = Xtrain / 255.
    Xtest = Xtest / 255.
    Xtrain = (Xtrain > 0.5).astype(np.float32) * 2 - 1
    Xtest = (Xtest > 0.5).astype(np.float32) * 2 - 1

    # Save
    np.save(MNIST_DIR / "Xtrain.npy", Xtrain)
    np.save(MNIST_DIR / "Xtest.npy", Xtest)
    np.save(MNIST_DIR / "Ytrain.npy", Ytrain)
    np.save(MNIST_DIR / "Ytest.npy", Ytest)
...
def load_bipolar_mnist():
    ...
    Xtrain = np.load(MNIST_DIR / "Xtrain.npy")
    Xtest = np.load(MNIST_DIR / "Xtest.npy")
    Ytrain = np.load(MNIST_DIR / "Ytrain.npy")
...
#| echo: false
nrows = ncols = 5
idxs = np.random.choice(np.arange(len(Xtest)), nrows * ncols, replace=False)
fig, axs = plt.subplots(nrows, ncols, figsize=(nrows*1.2, ncols*1.2))
for i, ax in enumerate(axs.flat):
    ax.imshow(Xtest[idxs[i]], cmap='gray')
    ax.axis('off')
plt.tight_layout()
plt.show()
```
...
For cutesies and funsies, let's build our own black and white pokemon dataset from the [Pokemon Database](https://pokemondb.net/sprites).
...
def download_pokemon_sprites(
    ...
    output_dir = get_cache_dir("data/pokemon/raw")
...
def process_pokemon_sprites(
    ...
    """
    input_dir = get_cache_dir("data/pokemon/raw")
    output_dir = get_cache_dir(f"data/pokemon/{ds_name}")
...
def datafy_pokemon_sprites(ds_name:str):
    ...
    input_dir = get_cache_dir(f"data/pokemon/{ds_name}")
