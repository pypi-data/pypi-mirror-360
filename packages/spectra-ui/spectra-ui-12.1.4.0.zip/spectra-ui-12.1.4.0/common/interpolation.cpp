...
int Spline::IntegrateGtEiwtOld(int nini, double x[], double w, double *Gr, double *Gi, const char *debug)
{
	...
	*Gr = *Gi = 0.0;
	...
	for(int n = nini; n <= nfin; n++){
		if(n == nfin){
			...
			IntegrateGtEiwtSingle(w, m_x[n], dx, gderiv, &Grs, &Gis);
		}
		else{
		...
		if(n == nini){
			...
			IntegrateGtEiwtSingle(w, m_x[n], dx, gderiv, &Hrs, &His);
			Grs -= Hrs;
			Gis -= His;
		}
		...
		*Gi += Gis;
		...
		if(debug != nullptr){
			...
			PrintDebugItems(debug_out, m_x[n], values);
		}
#endif
	}
	return nfin;
	...
	double w, double xini, double dx, vector<double> &gderiv, double *Gr, double *Gi)
{
	...
	if(fabs(Omega) < 0.1){
		...
		for(int j = 1; j <= 3; j++){
			facto *= (double)j;
			F[j][1] = (-F[j-1][2]+sn/facto)/Omega;
			F[j][2] = (F[j-1][1]-cs/facto)/Omega;
		}
	}
