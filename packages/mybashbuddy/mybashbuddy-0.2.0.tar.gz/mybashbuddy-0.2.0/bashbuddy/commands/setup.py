...
def set_key():
    ...
    typer.echo("💡 For permanent setup, run: bashbuddy setup")
    
    # Test the API key
    test_api_key(api_key)
    typer.echo("\n🎉 Ready to use BashBuddy!")
...
@app.command()
def status():
    """Check the current setup status of BashBuddy"""
    typer.echo("🔍 BashBuddy Setup Status")
    typer.echo("=" * 40)
    ...
    if api_key:
        ...
        typer.echo("💡 Run 'bashbuddy setup' to configure")
        typer.echo("💡 Or run 'bashbuddy set-key' for current session only")
...
def setup():
    ...
    typer.echo("• bashbuddy generate 'your command'")
    typer.echo("• bashbuddy explain 'your command'")
    typer.echo("• bashbuddy fix 'your command'")
    typer.echo("• bashbuddy ask 'your question'")
