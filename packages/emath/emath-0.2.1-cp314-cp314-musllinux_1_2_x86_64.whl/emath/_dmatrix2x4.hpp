...
#ifndef E_MATH_DMATRIX2X4_HPP
#define E_MATH_DMATRIX2X4_HPP
...
DMatrix2x4_getbufferproc(DMatrix2x4 *self, Py_buffer *view, int flags)
{
    ...
    {
        PyErr_SetString(PyExc_BufferError, "DMatrix2x4 cannot be made Fortran contiguous");
        view->obj = 0;
        return -1;
    }
...
static DVector2 *
DMatrix2x4_get_row(DMatrix2x4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1)
    {
...
static PyObject *
DMatrix2x4_get_size(DMatrix2x4 *cls, void *)
{
...
static PyObject *
DMatrix2x4_from_buffer(PyTypeObject *cls, PyObject *buffer)
{
    ...
    auto *result = (DMatrix2x4 *)cls->tp_alloc(cls, 0);
    if (!result)
    {
        PyBuffer_Release(&view);
        return 0;
    ...
    static FMatrix2x4 *
    DMatrix2x4_to_fmatrix(DMatrix2x4 *self, void*)
    {
        auto module_state = get_module_state();
        if (!module_state){ return 0; }
...
static PyObject *
DMatrix2x4_get_array_type(PyTypeObject *cls, void*)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto array_type = module_state->DMatrix2x4Array_PyTypeObject;
...
static PyMethodDef DMatrix2x4_PyMethodDef[] = {
        ...
        {"to_fmatrix", (PyCFunction)DMatrix2x4_to_fmatrix, METH_NOARGS, 0},


    {"get_row", (PyCFunction)DMatrix2x4_get_row, METH_FASTCALL, 0},
    ...
    {"get_size", (PyCFunction)DMatrix2x4_get_size, METH_NOARGS | METH_STATIC, 0},
    {"get_array_type", (PyCFunction)DMatrix2x4_get_array_type, METH_NOARGS | METH_STATIC, 0},
    {"from_buffer", (PyCFunction)DMatrix2x4_from_buffer, METH_O | METH_CLASS, 0},
    {0, 0, 0, 0}
};
...
static PyObject *
DMatrix2x4Array__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    ...
    if (arg_count == 0)
    {
        auto self = (DMatrix2x4Array *)cls->tp_alloc(cls, 0);
        if (!self){ return 0; }
        self->length = 0;
        self->glm = 0;
        return (PyObject *)self;
    ...
    self->glm = new DMatrix2x4Glm[arg_count];
...
static void
DMatrix2x4Array__dealloc__(DMatrix2x4Array *self)
{
    if (self->weakreflist)
    {
        PyObject_ClearWeakRefs((PyObject *)self);
...
static Py_hash_t
DMatrix2x4Array__hash__(DMatrix2x4Array *self)
{
...
static PyObject *
DMatrix2x4Array__repr__(DMatrix2x4Array *self)
{
    return PyUnicode_FromFormat("DMatrix2x4Array[%zu]", self->length);
}


static Py_ssize_t
DMatrix2x4Array__len__(DMatrix2x4Array *self)
{
    return self->length;
}
...
static PyObject *
DMatrix2x4Array__sq_getitem__(DMatrix2x4Array *self, Py_ssize_t index)
{
    ...
    DMatrix2x4 *result = (DMatrix2x4 *)element_cls->tp_alloc(element_cls, 0);
    if (!result){ return 0; }
    result->glm = new DMatrix2x4Glm(self->glm[index]);
...
static PyObject *
DMatrix2x4Array__mp_getitem__(DMatrix2x4Array *self, PyObject *key)
{
    if (PySlice_Check(key))
    {
        ...
        else
        {
            ...
            result->glm = new DMatrix2x4Glm[length];
            for (DMatrix2x4Glm::length_type i = 0; i < length; i++)
            {
...
static PyObject *
DMatrix2x4Array__richcmp__(
    DMatrix2x4Array *self,
    DMatrix2x4Array *other,
    int op
)
{
    if (Py_TYPE(self) != Py_TYPE(other))
...
static int
DMatrix2x4Array__bool__(DMatrix2x4Array *self)
{
...
static int
DMatrix2x4Array_getbufferproc(DMatrix2x4Array *self, Py_buffer *view, int flags)
{
    if (flags & PyBUF_WRITABLE)
    {
        PyErr_SetString(PyExc_TypeError, "DMatrix2x4 is read only");
...
static void
DMatrix2x4Array_releasebufferproc(DMatrix2x4Array *self, Py_buffer *view)
{
    delete view->shape;
}


static PyMemberDef DMatrix2x4Array_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DMatrix2x4Array, weakreflist), READONLY},
    {0}
};
...
static PyObject *
DMatrix2x4Array_size(DMatrix2x4Array *self, void *)
{
...
static PyGetSetDef DMatrix2x4Array_PyGetSetDef[] = {
    ...
    {"size", (getter)DMatrix2x4Array_size, 0, 0, 0},
    {0, 0, 0, 0, 0}
};


static PyObject *
DMatrix2x4Array_from_buffer(PyTypeObject *cls, PyObject *buffer)
{
    static Py_ssize_t expected_size = sizeof(double);
    Py_buffer view;
    if (PyObject_GetBuffer(buffer, &view, PyBUF_SIMPLE) == -1){ return 0; }
    auto view_length = view.len;
    if (view_length % (sizeof(double) * 8))
    {
        PyBuffer_Release(&view);
        PyErr_Format(PyExc_BufferError, "expected buffer evenly divisible by %zd, got %zd", sizeof(double), view_length);
        return 0;
    }
    auto array_length = view_length / (sizeof(double) * 8);
    ...
    if (array_length > 0)
    {
        result->glm = new DMatrix2x4Glm[array_length];
...
static PyObject *
DMatrix2x4Array_get_component_type(PyTypeObject *cls, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 0)
    {
    ...
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto component_type = module_state->DMatrix2x4_PyTypeObject;
...
static PyType_Slot DMatrix2x4Array_PyType_Slots [] = {
    ...
    {Py_tp_hash, (void*)DMatrix2x4Array__hash__},
    {Py_tp_repr, (void*)DMatrix2x4Array__repr__},
    {Py_sq_length, (void*)DMatrix2x4Array__len__},
    {Py_sq_item, (void*)DMatrix2x4Array__sq_getitem__},
    {Py_mp_subscript, (void*)DMatrix2x4Array__mp_getitem__},
    {Py_tp_richcompare, (void*)DMatrix2x4Array__richcmp__},
    ...
    {Py_tp_methods, (void*)DMatrix2x4Array_PyMethodDef},
    {0, 0},
};


static PyType_Spec DMatrix2x4Array_PyTypeSpec = {
    "emath.DMatrix2x4Array",
    sizeof(DMatrix2x4Array),
    0,
    Py_TPFLAGS_DEFAULT,
    DMatrix2x4Array_PyType_Slots
};


static PyTypeObject *
define_DMatrix2x4Array_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DMatrix2x4Array_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    // Unlike other functions that steal references, PyModule_AddObject() only
    // decrements the reference count of value on success.
    if (PyModule_AddObject(module, "DMatrix2x4Array", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }
...
get_DMatrix2x4_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->DMatrix2x4_PyTypeObject;
}


static PyTypeObject *
get_DMatrix2x4Array_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->DMatrix2x4Array_PyTypeObject;
}
...
create_DMatrix2x4(const double *value)
{
    ...
    auto result = (DMatrix2x4 *)cls->tp_alloc(cls, 0);
    if (!result){ return 0; }
    result->glm = new DMatrix2x4Glm(*(DMatrix2x4Glm *)value);
    return (PyObject *)result;
}
...
create_DMatrix2x4Array(size_t length, const double *value)
{
    ...
    if (length > 0)
    {
        ...
        {
            result->glm[i] = ((DMatrix2x4Glm *)value)[i];
        }
    }
    else
    {
...
get_DMatrix2x4_value_ptr(const PyObject *self)
{
    ...
    {
        PyErr_Format(PyExc_TypeError, "expected DMatrix2x4, got %R", self);
        return 0;
    }
    return (double *)((DMatrix2x4 *)self)->glm;
}
...
get_DMatrix2x4Array_value_ptr(const PyObject *self)
{
    if (Py_TYPE(self) != get_DMatrix2x4Array_type())
    {
        PyErr_Format(
            PyExc_TypeError,
            "expected DMatrix2x4Array, got %R",
            self
        );
        return 0;
    }
    return (double *)((DMatrix2x4Array *)self)->glm;
}
...
get_DMatrix2x4Array_length(const PyObject *self)
{
    ...
    return ((DMatrix2x4Array *)self)->length;
}
