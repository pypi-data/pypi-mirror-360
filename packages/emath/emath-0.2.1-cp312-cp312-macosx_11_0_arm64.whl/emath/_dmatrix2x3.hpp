...
#ifndef E_MATH_DMATRIX2X3_HPP
#define E_MATH_DMATRIX2X3_HPP
...
static DVector2 *
DMatrix2x3_get_row(DMatrix2x3 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 1)
    {
...
static PyObject *
DMatrix2x3_get_array_type(PyTypeObject *cls, void*)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto array_type = module_state->DMatrix2x3Array_PyTypeObject;
...
static PyMethodDef DMatrix2x3_PyMethodDef[] = {
    ...
    {"get_array_type", (PyCFunction)DMatrix2x3_get_array_type, METH_NOARGS | METH_STATIC, 0},
...
static PyObject *
DMatrix2x3Array__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto element_cls = module_state->DMatrix2x3_PyTypeObject;
    ...
    if (arg_count == 0)
    {
        auto self = (DMatrix2x3Array *)cls->tp_alloc(cls, 0);
        if (!self){ return 0; }
        self->length = 0;
        self->glm = 0;
        return (PyObject *)self;
    ...
    self->glm = new DMatrix2x3Glm[arg_count];
...
static void
DMatrix2x3Array__dealloc__(DMatrix2x3Array *self)
{
    if (self->weakreflist)
    {
        PyObject_ClearWeakRefs((PyObject *)self);
...
static Py_hash_t
DMatrix2x3Array__hash__(DMatrix2x3Array *self)
{
...
static PyObject *
DMatrix2x3Array__repr__(DMatrix2x3Array *self)
{
    return PyUnicode_FromFormat("DMatrix2x3Array[%zu]", self->length);
}


static Py_ssize_t
DMatrix2x3Array__len__(DMatrix2x3Array *self)
{
    return self->length;
}
...
static PyObject *
DMatrix2x3Array__sq_getitem__(DMatrix2x3Array *self, Py_ssize_t index)
{
    ...
    DMatrix2x3 *result = (DMatrix2x3 *)element_cls->tp_alloc(element_cls, 0);
    if (!result){ return 0; }
    result->glm = new DMatrix2x3Glm(self->glm[index]);
...
static PyObject *
DMatrix2x3Array__mp_getitem__(DMatrix2x3Array *self, PyObject *key)
{
    if (PySlice_Check(key))
    {
        ...
        else
        {
            ...
            result->glm = new DMatrix2x3Glm[length];
            for (DMatrix2x3Glm::length_type i = 0; i < length; i++)
            {
...
static PyObject *
DMatrix2x3Array__richcmp__(
    DMatrix2x3Array *self,
    DMatrix2x3Array *other,
    int op
)
{
    if (Py_TYPE(self) != Py_TYPE(other))
...
static int
DMatrix2x3Array__bool__(DMatrix2x3Array *self)
{
...
static int
DMatrix2x3Array_getbufferproc(DMatrix2x3Array *self, Py_buffer *view, int flags)
{
    if (flags & PyBUF_WRITABLE)
    {
        PyErr_SetString(PyExc_TypeError, "DMatrix2x3 is read only");
...
static void
DMatrix2x3Array_releasebufferproc(DMatrix2x3Array *self, Py_buffer *view)
{
    delete view->shape;
}


static PyMemberDef DMatrix2x3Array_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DMatrix2x3Array, weakreflist), READONLY},
    {0}
};
...
static PyObject *
DMatrix2x3Array_get_component_type(PyTypeObject *cls, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 0)
    {
    ...
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto component_type = module_state->DMatrix2x3_PyTypeObject;
...
static PyMethodDef DMatrix2x3Array_PyMethodDef[] = {
    ...
    {"get_component_type", (PyCFunction)DMatrix2x3Array_get_component_type, METH_FASTCALL | METH_CLASS, 0},
    {0, 0, 0, 0}
};


static PyType_Slot DMatrix2x3Array_PyType_Slots [] = {
    {Py_tp_new, (void*)DMatrix2x3Array__new__},
    {Py_tp_dealloc, (void*)DMatrix2x3Array__dealloc__},
    {Py_tp_hash, (void*)DMatrix2x3Array__hash__},
    {Py_tp_repr, (void*)DMatrix2x3Array__repr__},
    {Py_sq_length, (void*)DMatrix2x3Array__len__},
    {Py_sq_item, (void*)DMatrix2x3Array__sq_getitem__},
    {Py_mp_subscript, (void*)DMatrix2x3Array__mp_getitem__},
    {Py_tp_richcompare, (void*)DMatrix2x3Array__richcmp__},
    {Py_nb_bool, (void*)DMatrix2x3Array__bool__},
    {Py_bf_getbuffer, (void*)DMatrix2x3Array_getbufferproc},
...
static PyType_Spec DMatrix2x3Array_PyTypeSpec = {
    "emath.DMatrix2x3Array",
    sizeof(DMatrix2x3Array),
    0,
    Py_TPFLAGS_DEFAULT,
    DMatrix2x3Array_PyType_Slots
};


static PyTypeObject *
define_DMatrix2x3Array_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DMatrix2x3Array_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    // Unlike other functions that steal references, PyModule_AddObject() only
    // decrements the reference count of value on success.
    if (PyModule_AddObject(module, "DMatrix2x3Array", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }
...
get_DMatrix2x3_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->DMatrix2x3_PyTypeObject;
}


static PyTypeObject *
get_DMatrix2x3Array_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->DMatrix2x3Array_PyTypeObject;
}


static PyObject *
create_DMatrix2x3(const double *value)
{

    auto cls = get_DMatrix2x3_type();
    auto result = (DMatrix2x3 *)cls->tp_alloc(cls, 0);
    if (!result){ return 0; }
    result->glm = new DMatrix2x3Glm(*(DMatrix2x3Glm *)value);
    return (PyObject *)result;
}


static PyObject *
create_DMatrix2x3Array(size_t length, const double *value)
{
    auto cls = get_DMatrix2x3Array_type();
    auto result = (DMatrix2x3Array *)cls->tp_alloc(cls, 0);
    if (!result){ return 0; }
    result->length = length;
    if (length > 0)
    {
        ...
        {
            result->glm[i] = ((DMatrix2x3Glm *)value)[i];
        }
    }
    else
    {
...
static double *
get_DMatrix2x3_value_ptr(const PyObject *self)
{
    if (Py_TYPE(self) != get_DMatrix2x3_type())
    {
        PyErr_Format(PyExc_TypeError, "expected DMatrix2x3, got %R", self);
        return 0;
    }
    return (double *)((DMatrix2x3 *)self)->glm;
}


static double *
get_DMatrix2x3Array_value_ptr(const PyObject *self)
{
    if (Py_TYPE(self) != get_DMatrix2x3Array_type())
    {
        PyErr_Format(
            PyExc_TypeError,
            "expected DMatrix2x3Array, got %R",
            self
        );
        return 0;
    }
    return (double *)((DMatrix2x3Array *)self)->glm;
}


static size_t
get_DMatrix2x3Array_length(const PyObject *self)
{
    if (Py_TYPE(self) != get_DMatrix2x3Array_type())
    {
        PyErr_Format(
    ...
    return ((DMatrix2x3Array *)self)->length;
}
