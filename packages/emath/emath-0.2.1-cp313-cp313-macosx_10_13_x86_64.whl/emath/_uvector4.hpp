...
#ifndef E_MATH_UVECTOR4_HPP
#define E_MATH_UVECTOR4_HPP
...
UVector4__richcmp__(UVector4 *self, UVector4 *other, int op)
{
    ...
    switch(op)
    {
        case Py_LT:
        {
            for (UVector4Glm::length_type i = 0; i < 4; i++)
            {
...
static PyObject *
UVector4__add__(PyObject *left, PyObject *right)
{
    ...
    {
        vector = ((UVector4 *)left)->glm + ((UVector4 *)right)->glm;
    }
    else
    {
        if (Py_TYPE(left) == cls)
        {
            ...
            vector = ((UVector4 *)left)->glm + c_right;
        }
        else
        {
            auto c_left = pyobject_to_c_unsigned_int(left);
            if (PyErr_Occurred()){ PyErr_Clear(); Py_RETURN_NOTIMPLEMENTED; }
            vector = c_left + ((UVector4 *)right)->glm;
        }
    }

    UVector4 *result = (UVector4 *)cls->tp_alloc(cls, 0);
    if (!result){ return 0; }
    result->glm = UVector4Glm(
...
static PyObject *
UVector4__sub__(PyObject *left, PyObject *right)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto cls = module_state->UVector4_PyTypeObject;
...
static PyObject *
UVector4__mul__(PyObject *left, PyObject *right)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto cls = module_state->UVector4_PyTypeObject;
    ...
    static PyObject *
    UVector4__truediv__(PyObject *left, PyObject *right)
    {
        auto module_state = get_module_state();
        if (!module_state){ return 0; }
        auto cls = module_state->UVector4_PyTypeObject;
        ...
        if (Py_TYPE(left) == Py_TYPE(right))
        {
                ...
                if (
                        ...
                        ((UVector4 *)right)->glm[0] == 0 ||

                        ((UVector4 *)right)->glm[1] == 0 ||

                        ((UVector4 *)right)->glm[2] == 0 ||

                        ((UVector4 *)right)->glm[3] == 0
...
UVector4_getbufferproc(UVector4 *self, Py_buffer *view, int flags)
{
    ...
    {
        PyErr_SetString(PyExc_TypeError, "UVector4 is read only");
        view->obj = 0;
        return -1;
    }
    view->buf = &self->glm;
...
static PyObject *
UVector4_pointer(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyGetSetDef UVector4_PyGetSetDef[] = {
    ...
    {"pointer", (getter)UVector4_pointer, 0, 0, 0},
    {0, 0, 0, 0, 0}
};
...
static PyObject *
UVector4_min(UVector4 *self, PyObject *min)
{
    ...
    UVector4 *result = (UVector4 *)cls->tp_alloc(cls, 0);
    if (!result){ return 0; }
    result->glm = UVector4Glm(vector);
    return (PyObject *)result;
}


static PyObject *
UVector4_max(UVector4 *self, PyObject *max)
{
...
static PyObject *
UVector4_clamp(UVector4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
...
static PyObject *
UVector4_get_size(UVector4 *cls, void *)
{
    return PyLong_FromSize_t(sizeof(unsigned int) * 4);
}


static PyObject *
UVector4_get_limits(UVector4 *cls, void *)
{
...
static PyObject *
UVector4_from_buffer(PyTypeObject *cls, PyObject *buffer)
{
    static Py_ssize_t expected_size = sizeof(unsigned int) * 4;
    Py_buffer view;
...
static PyObject *
UVector4_get_array_type(PyTypeObject *cls, void*)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto array_type = module_state->UVector4Array_PyTypeObject;
...
static PyObject *
UVector4_to_b(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_d(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_f(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_i8(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_u8(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_i16(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_u16(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_i32(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_u32(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_i(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_i64(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyObject *
UVector4_to_u64(UVector4 *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
...
static PyMethodDef UVector4_PyMethodDef[] = {
    ...
    {"min", (PyCFunction)UVector4_min, METH_O, 0},
    {"max", (PyCFunction)UVector4_max, METH_O, 0},
    {"clamp", (PyCFunction)UVector4_clamp, METH_FASTCALL, 0},
    {"get_limits", (PyCFunction)UVector4_get_limits, METH_NOARGS | METH_STATIC, 0},
    {"get_size", (PyCFunction)UVector4_get_size, METH_NOARGS | METH_STATIC, 0},
    {"get_array_type", (PyCFunction)UVector4_get_array_type, METH_NOARGS | METH_STATIC, 0},
    {"from_buffer", (PyCFunction)UVector4_from_buffer, METH_O | METH_CLASS, 0},

        {"to_b", (PyCFunction)UVector4_to_b, METH_NOARGS, 0},

        {"to_d", (PyCFunction)UVector4_to_d, METH_NOARGS, 0},

        {"to_f", (PyCFunction)UVector4_to_f, METH_NOARGS, 0},

        {"to_i8", (PyCFunction)UVector4_to_i8, METH_NOARGS, 0},

        {"to_u8", (PyCFunction)UVector4_to_u8, METH_NOARGS, 0},

        {"to_i16", (PyCFunction)UVector4_to_i16, METH_NOARGS, 0},

        {"to_u16", (PyCFunction)UVector4_to_u16, METH_NOARGS, 0},

        {"to_i32", (PyCFunction)UVector4_to_i32, METH_NOARGS, 0},

        {"to_u32", (PyCFunction)UVector4_to_u32, METH_NOARGS, 0},

        {"to_i", (PyCFunction)UVector4_to_i, METH_NOARGS, 0},

        {"to_i64", (PyCFunction)UVector4_to_i64, METH_NOARGS, 0},

        {"to_u64", (PyCFunction)UVector4_to_u64, METH_NOARGS, 0},
...
static PyObject *
UVector4Array__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto element_cls = module_state->UVector4_PyTypeObject;
    ...
    if (arg_count == 0)
    {
        auto self = (UVector4Array *)cls->tp_alloc(cls, 0);
        if (!self){ return 0; }
        self->length = 0;
        self->glm = 0;
        return (PyObject *)self;
    ...
    self->glm = new UVector4Glm[arg_count];
...
static void
UVector4Array__dealloc__(UVector4Array *self)
{
    if (self->weakreflist)
    {
        PyObject_ClearWeakRefs((PyObject *)self);
...
static Py_hash_t
UVector4Array__hash__(UVector4Array *self)
{
    Py_ssize_t len = self->length * 4;
    Py_uhash_t acc = _HASH_XXPRIME_5;
    for (Py_ssize_t i = 0; i < (Py_ssize_t)self->length; i++)
    {
        for (UVector4Glm::length_type j = 0; j < 4; j++)
        {
...
static PyObject *
UVector4Array__repr__(UVector4Array *self)
{
    return PyUnicode_FromFormat("UVector4Array[%zu]", self->length);
}


static Py_ssize_t
UVector4Array__len__(UVector4Array *self)
{
    return self->length;
}
...
static PyObject *
UVector4Array__sq_getitem__(UVector4Array *self, Py_ssize_t index)
{
    ...
    UVector4 *result = (UVector4 *)element_cls->tp_alloc(element_cls, 0);
    if (!result){ return 0; }
    result->glm = UVector4Glm(self->glm[index]);
...
static PyObject *
UVector4Array__mp_getitem__(UVector4Array *self, PyObject *key)
{
    if (PySlice_Check(key))
    {
        ...
        else
        {
            ...
            result->glm = new UVector4Glm[length];
            for (UVector4Glm::length_type i = 0; i < length; i++)
            {
...
static int
UVector4Array__bool__(UVector4Array *self)
{
...
static int
UVector4Array_getbufferproc(UVector4Array *self, Py_buffer *view, int flags)
{
    if (flags & PyBUF_WRITABLE)
    {
        PyErr_SetString(PyExc_BufferError, "UVector4 is read only");
        view->obj = 0;
        return -1;
    }
        ...
        {
            PyErr_SetString(PyExc_BufferError, "UVector4 cannot be made Fortran contiguous");
            view->obj = 0;
            return -1;
        }
    ...
    view->buf = self->glm;
    view->obj = (PyObject *)self;
    view->len = sizeof(unsigned int) * 4 * self->length;
    view->readonly = 1;
    view->itemsize = sizeof(unsigned int);
    view->ndim = 2;
    if (flags & PyBUF_FORMAT)
...
static void
UVector4Array_releasebufferproc(UVector4Array *self, Py_buffer *view)
{
    delete view->shape;
}


static PyMemberDef UVector4Array_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(UVector4Array, weakreflist), READONLY},
    {0}
};


static PyObject *
UVector4Array_pointer(UVector4Array *self, void *)
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto c_p = module_state->ctypes_c_unsigned_int_p;
...
static PyObject *
UVector4Array_size(UVector4Array *self, void *)
{
    return PyLong_FromSize_t(sizeof(unsigned int) * 4 * self->length);
}


static PyGetSetDef UVector4Array_PyGetSetDef[] = {
    {"pointer", (getter)UVector4Array_pointer, 0, 0, 0},
    {"size", (getter)UVector4Array_size, 0, 0, 0},
    {0, 0, 0, 0, 0}
};


static PyObject *
UVector4Array_from_buffer(PyTypeObject *cls, PyObject *buffer)
{
    static Py_ssize_t expected_size = sizeof(unsigned int);
    Py_buffer view;
    if (PyObject_GetBuffer(buffer, &view, PyBUF_SIMPLE) == -1){ return 0; }
    auto view_length = view.len;
    if (view_length % (sizeof(unsigned int) * 4))
    {
        PyBuffer_Release(&view);
        PyErr_Format(PyExc_BufferError, "expected buffer evenly divisible by %zd, got %zd", sizeof(unsigned int), view_length);
        return 0;
    }
    auto array_length = view_length / (sizeof(unsigned int) * 4);
    ...
    if (array_length > 0)
    {
        result->glm = new UVector4Glm[array_length];
...
static PyObject *
UVector4Array_get_component_type(PyTypeObject *cls, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 0)
    {
    ...
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    auto component_type = module_state->UVector4_PyTypeObject;
    Py_INCREF(component_type);
    return (PyObject *)component_type;
}


static PyMethodDef UVector4Array_PyMethodDef[] = {
    {"from_buffer", (PyCFunction)UVector4Array_from_buffer, METH_O | METH_CLASS, 0},
    {"get_component_type", (PyCFunction)UVector4Array_get_component_type, METH_FASTCALL | METH_CLASS, 0},
    {0, 0, 0, 0}
};


static PyType_Slot UVector4Array_PyType_Slots [] = {
    {Py_tp_new, (void*)UVector4Array__new__},
    {Py_tp_dealloc, (void*)UVector4Array__dealloc__},
    {Py_tp_hash, (void*)UVector4Array__hash__},
    {Py_tp_repr, (void*)UVector4Array__repr__},
    {Py_sq_length, (void*)UVector4Array__len__},
    {Py_sq_item, (void*)UVector4Array__sq_getitem__},
    {Py_mp_subscript, (void*)UVector4Array__mp_getitem__},
    {Py_tp_richcompare, (void*)UVector4Array__richcmp__},
    {Py_nb_bool, (void*)UVector4Array__bool__},
    {Py_bf_getbuffer, (void*)UVector4Array_getbufferproc},
    {Py_bf_releasebuffer, (void*)UVector4Array_releasebufferproc},
    {Py_tp_getset, (void*)UVector4Array_PyGetSetDef},
    {Py_tp_members, (void*)UVector4Array_PyMemberDef},
    {Py_tp_methods, (void*)UVector4Array_PyMethodDef},
    {0, 0},
};


static PyType_Spec UVector4Array_PyTypeSpec = {
    "emath.UVector4Array",
    sizeof(UVector4Array),
    0,
    Py_TPFLAGS_DEFAULT,
    UVector4Array_PyType_Slots
};


static PyTypeObject *
define_UVector4Array_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &UVector4Array_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    // Unlike other functions that steal references, PyModule_AddObject() only
    // decrements the reference count of value on success.
    if (PyModule_AddObject(module, "UVector4Array", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }
...
get_UVector4_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->UVector4_PyTypeObject;
}


static PyTypeObject *
get_UVector4Array_type()
{
    auto module_state = get_module_state();
    if (!module_state){ return 0; }
    return module_state->UVector4Array_PyTypeObject;
}
...
create_UVector4Array(size_t length, const unsigned int *value)
{
    ...
    if (length > 0)
    {
        ...
        {
            result->glm[i] = ((UVector4Glm *)value)[i];
        }
    }
    else
    {
...
get_UVector4_value_ptr(const PyObject *self)
{
    ...
    {
        PyErr_Format(PyExc_TypeError, "expected UVector4, got %R", self);
        return 0;
    }
    return (unsigned int *)&((UVector4 *)self)->glm;
}
...
get_UVector4Array_value_ptr(const PyObject *self)
{
    if (Py_TYPE(self) != get_UVector4Array_type())
    {
        PyErr_Format(
            PyExc_TypeError,
            "expected UVector4Array, got %R",
            self
        );
        return 0;
    }
    return (unsigned int *)((UVector4Array *)self)->glm;
}
...
get_UVector4Array_length(const PyObject *self)
{
    ...
    return ((UVector4Array *)self)->length;
}
