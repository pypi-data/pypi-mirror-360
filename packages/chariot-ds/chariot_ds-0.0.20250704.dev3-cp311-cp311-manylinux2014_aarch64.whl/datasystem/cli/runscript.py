...
"""Chariot datasystem CLI runscript command."""
...
class Command(BaseCommand, ParallelMixin):
    ...
    def process_node(self, worker_node):
        ...
        """
        remote_script_path = f"~/{os.path.basename(self._script_path)}"
        try:
            ...
            util.ssh_execute(
                worker_node, user_name, private_key, f"bash {remote_script_path}"
            )
            self.logger.info(f"Script executed successfully on {worker_node}.")

            util.ssh_execute(
                worker_node, user_name, private_key, f"rm -f {remote_script_path}"
            )
        except Exception as e:
