...
class RBLNModelRunner(ModelRunnerBase[ModelInputForRBLN]):
    ...
    def _prepare_prompt(
        ...
        seq_group_metadata_list: List[SequenceGroupMetadata],
               ...
               BatchedTensorInputs, torch.Tensor]:
        ...
        block_tables = make_tensor_with_pad(
            ...
            dtype=torch.int16,
            device=self.device,
        ) if len(block_tables) > 0 else None
    ...
    def prepare_model_input(
        ...
        if is_prompt:
            (input_tokens, input_positions, seq_lens, multi_modal_kwargs,
             block_tables,
             type_token_ids) = self._prepare_prompt(seq_group_metadata_list)
            pooling_metadata = self._prepare_pooling(seq_group_metadata_list,
                                                     seq_lens)

        else:
            (input_tokens, input_positions,
             block_tables) = self._prepare_decode(seq_group_metadata_list)
            seq_lens = None
            type_token_ids = None
