...
class RBLNOptimumForEncoderModel(RBLNOptimumForCausalLM):
    ...
    def preprocess(
        ...
        return pad_if_needed(input_ids), pad_if_needed(type_token_ids)
    ...
    def forward(self, model_input: ModelInputForRBLN) -> torch.Tensor:
        input_ids, token_type_ids = self.preprocess(model_input.input_tokens,
                                                    model_input.token_type_ids)
        compiled_max_seq_len = self.rbln_model_config.max_seq_len
        positions = model_input.input_positions[:, :compiled_max_seq_len]
...
class RBLNOptimumIdefics3ConditionalGeneration(RBLNOptimumForCausalLM):
...
def get_rbln_model(
    ...
    if is_multi_modal(model_config.hf_config):
        ...
        if architectures[0] in ["Qwen2_5_VLForConditionalGeneration"]:
            ...
            rbln_model = RBLNOptimumIdefics3ConditionalGeneration(
                model_config=model_config, scheduler_config=scheduler_config)
        else:
