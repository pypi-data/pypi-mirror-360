...
class EngineArgs:
    ...
    block_size: int = 16 if not current_platform.is_rbln() else 4096
    ...
    compiled_model_dir: Optional[str] = None  # RBLN
    device_id: Optional[str] = None  # RBLN
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument('--compiled-model-dir',
                            type=nullable_str,
                            default=EngineArgs.compiled_model_dir,
                            help='Absolute path to compiled model directory '
                            'for RBLN')

        parser.add_argument(
            '--device-id',
            type=nullable_str,
            default=EngineArgs.device_id,
            help='Specifies the target device(s) for RBLN. '
            'Accepts a single integer for one device (e.g., "0") or a '
            'comma-separated list of integers for multiple '
            'devices (e.g., "0,1,2").  If omitted, defaults to the '
            'value defined in rebel-compiler.')
    ...
    def create_model_config(self) -> ModelConfig:
        return ModelConfig(
            ...
            compiled_model_dir=self.compiled_model_dir,
