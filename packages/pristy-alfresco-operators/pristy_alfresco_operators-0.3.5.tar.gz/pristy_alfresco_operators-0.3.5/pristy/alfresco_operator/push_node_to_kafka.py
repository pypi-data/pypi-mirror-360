    ...
    def execute(self, context):
        ...
        for c in nodes_array:
            ...
            try:
                ...
                if dag_param is not None:
                    follow.append(dag_param['local_db_id'])
                    update_state_db(dag_param['local_db_id'], "sending", self.table_name_local_db, self.source_key)
                else:
                    follow.append(c['source']['uuid'])
                    update_state_db(c['source']['uuid'], "sending", self.table_name_local_db, self.source_key)
            except BufferError:
                self.log.warning(
