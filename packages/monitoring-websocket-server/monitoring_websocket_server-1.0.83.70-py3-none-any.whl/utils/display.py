...
class DisplayManager:
    ...
    @staticmethod
    def print_header(title: str = "SERVICE DE MONITORING SYSTÈME TEMPS RÉEL") -> None:
    ...
    def print_compact_header(iteration: int, timestamp: str) -> None:
        ...
        """
        print(f"🖥️ MONITORING #{iteration:,} | {timestamp} | Ctrl+C pour arrêter")
        print("=" * 80)
    ...
    def print_detailed_metrics(self, data: Dict[str, Any]) -> None:
        ...
        mem_percent = data['memory']['usage_percent']
        mem_emoji = "🟢" if mem_percent < MEMORY_WARNING_THRESHOLD else "🟡" if mem_percent < MEMORY_CRITICAL_THRESHOLD else "🔴"
        print(f"{mem_emoji} Mémoire:    {mem_percent:6.1f}% "
    ...
    def print_statistics_section(self, stats: Dict[str, Any]) -> None:
        ...
        print("📈 STATISTIQUES:")
        self.print_separator()
        print(f"🚨 Alertes générées:     {stats.get('alerts_count', 0):,}")
        ...
        if start_time:
            ...
            print(f"⏱️  Temps de fonctionnement: {uptime_str}")
