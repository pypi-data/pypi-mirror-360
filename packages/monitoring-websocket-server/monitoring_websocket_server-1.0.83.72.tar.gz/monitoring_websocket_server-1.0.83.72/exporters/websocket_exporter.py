...
class WebSocketExporter:
    ...
    def initialize(self) -> None:
        """Initialise l'exporteur WebSocket."""
        self._logger.info(f"Initialisation de l'exporteur WebSocket sur {self.host}:{self.port}")
    ...
    def _do_export(self, snapshots: List[MonitoringSnapshot]) -> None:
        ...
        try:
            ...
            self._logger.error(f"Erreur lors de la diffusion WebSocket: {e}")
            
    def start_server(self) -> None:
        """Démarre le serveur WebSocket dans un thread séparé."""
        if self._server_thread and self._server_thread.is_alive():
            self._logger.warning("Le serveur WebSocket est déjà en cours d'exécution")
            return
        ...
        def run_server() -> None:
            ...
            try:
                ...
                self._logger.error(f"Erreur dans le serveur WebSocket: {e}")
            finally:
                if self._loop:
                    self._loop.close()
                    self._loop = None
    ...
    def stop_server(self) -> None:
        ...
        if self._loop and self.websocket_server.is_running:
            ...
            try:
                ...
                self._logger.error(f"Erreur lors de l'arrêt du serveur: {e}")
