...
class ExporterFactory:
    ...
    def create_exporter(cls, exporter_type: str, output_dir: Union[str, Path], 
        ...
        if exporter_type not in cls._EXPORTER_CLASSES:
            raise ExportError(
                f"Type d'exporteur non supporté: {exporter_type}. "
                f"Types disponibles: {list(cls._EXPORTER_CLASSES.keys())}"
            )
        ...
        try:
            ...
            if exporter_type == "json":
                ...
                if kwargs:
                    cls._logger.warning(
                        f"Paramètres ignorés pour {exporter_type}: {list(kwargs.keys())}"
                    )
                return exporter_class(output_path)
        except Exception as e:
            raise ExportError(
                f"Erreur lors de la création de l'exporteur {exporter_type}: {e}"
            )
    ...
    def validate_exporter_config(cls, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if exporter_type not in cls._EXPORTER_CLASSES:
            raise ValueError(
                f"Type d'exporteur invalide: {exporter_type}. "
                f"Types disponibles: {list(cls._EXPORTER_CLASSES.keys())}"
            )
        ...
        if not isinstance(output_dir, (str, Path)):
            raise ValueError("output_dir doit être une chaîne ou un Path")
        ...
        if exporter_type == "json":
            ...
            if not isinstance(date_in_filename, bool):
                raise ValueError("date_in_filename doit être un booléen")
    ...
    def get_preset_descriptions(cls) -> Dict[str, str]:
        ...
        """
        return {
            "default": "Configuration par défaut avec JSON formaté",
            "compressed": "Fichiers compressés pour économiser l'espace",
            "development": "Optimisé pour le développement avec JSON lisible",
            "production": "Optimisé pour la production avec compression",
            "minimal": "Configuration minimale avec compression maximale"
        }
    ...
    def estimate_file_size(cls, exporter_type: str, data_points: int, 
        ...
        if exporter_type not in base_sizes:
            raise ValueError(f"Type d'exporteur non supporté: {exporter_type}")
