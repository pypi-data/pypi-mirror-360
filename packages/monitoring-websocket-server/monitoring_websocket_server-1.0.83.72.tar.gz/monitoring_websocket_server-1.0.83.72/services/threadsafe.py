...
from ..config import (
    MONITOR_INTERVAL, EXPORT_INTERVAL, DATA_QUEUE_SIZE,
    ...
    RESPONSE_GET_TIMEOUT, EXPORT_RESPONSE_TIMEOUT, MAIN_LOOP_DELAY,
    ERROR_LOOP_DELAY, CPU_USAGE_CHANGE_THRESHOLD, MEMORY_USAGE_CHANGE_THRESHOLD,
...
class ThreadSafeMonitoringService:
    ...
    def start(self, timeout: float = SERVICE_START_TIMEOUT) -> bool:
        ...
        """
        if self._is_running:
            self._logger.warning("Le service est déjà en cours d'exécution")
            return True
        ...
        try:
            ...
            if self._ready_event.wait(timeout):
                ...
                self._logger.info("Service de monitoring démarré avec succès")
                return True
            else:
                self._logger.error(f"Timeout lors du démarrage du service ({timeout}s)")
                self.stop()
                raise ServiceStartupError(f"Timeout de démarrage ({timeout}s)")

        except Exception as e:
            self._logger.error(f"Erreur lors du démarrage: {e}")
            raise ServiceStartupError(f"Impossible de démarrer le service: {e}")
    ...
    def stop(self, timeout: float = SERVICE_STOP_TIMEOUT) -> bool:
        ...
        try:
            self._logger.info("Arrêt du service de monitoring...")
            ...
            if self._thread and self._thread.is_alive():
                ...
                if self._thread.is_alive():
                    self._logger.warning("Le thread ne s'est pas arrêté dans les temps")
                    raise ServiceShutdownError(f"Timeout d'arrêt ({timeout}s)")

            self._is_running = False
            self._thread = None
            self._logger.info("Service arrêté avec succès")
            return True

        except Exception as e:
            self._logger.error(f"Erreur lors de l'arrêt: {e}")
            raise ServiceShutdownError(f"Erreur d'arrêt: {e}")
    ...
    def set_alert_threshold(self, component: str, level: str, value: float) -> bool:
        ...
        try:
            ...
            self._logger.error("Timeout lors de la configuration du seuil")
            return False
        except Exception as e:
            self._logger.error(f"Erreur lors de la configuration: {e}")
            return False
    ...
    def configure_export(self, compress: bool = False, pretty_print: bool = True) -> bool:
        ...
        try:
            ...
            self._logger.error("Timeout lors de la configuration d'export")
            return False
        except Exception as e:
            self._logger.error(f"Erreur lors de la configuration d'export: {e}")
            return False
    ...
    def force_export(self) -> bool:
        ...
        try:
            ...
            self._logger.error("Timeout lors de l'export forcé")
            return False
        except Exception as e:
            self._logger.error(f"Erreur lors de l'export forcé: {e}")
            return False
    ...
    def _run_async_service(self) -> None:
        ...
        try:
            ...
            self._logger.error(f"Erreur dans le thread asynchrone: {e}")
        finally:
            # Nettoyage
            try:
        ...
        try:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    self._logger.error(f"Erreur dans la boucle principale: {e}")
                    await asyncio.sleep(0.5)

        except Exception as e:
            self._logger.error(f"Erreur lors de l'initialisation du service: {e}")
        finally:
            ...
            if service:
                try:
                    ...
                    self._logger.error(f"Erreur lors de l'arrêt du service: {e}")
            ...
            while True:
                ...
                if command["action"] == "set_threshold":
                    ...
                    try:
                        ...
                        self._logger.error(f"Erreur configuration export: {e}")
                        self._response_queue.put({"success": False, "error": str(e)})
                    ...
                    try:
                        ...
                        self._logger.error(f"Erreur export forcé: {e}")
                        self._response_queue.put({"success": False, "error": str(e)})
    ...
    async def _update_data_cache(self, service: RealtimeMonitoringService) -> None:
        ...
        try:
                ...
                for alert in new_alerts:
                    for callback in self._alert_callbacks:
                        try:
                            ...
                            self._logger.error(f"Erreur dans callback d'alerte: {e}")
                ...
                if old_summary and ThreadSafeMonitoringService._has_significant_change(old_summary, summary):
                    for callback in self._data_callbacks:
                        try:
                            ...
                            self._logger.error(f"Erreur dans callback de données: {e}")
            ...
            self._logger.error(f"Erreur lors de la mise à jour du cache: {e}")
