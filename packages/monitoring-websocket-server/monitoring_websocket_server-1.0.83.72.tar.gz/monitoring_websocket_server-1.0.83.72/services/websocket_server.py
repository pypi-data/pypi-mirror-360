...
class WebSocketMonitoringServer:
    ...
    async def register_client(self, websocket: Any) -> None:
        ...
        if len(self.clients) >= self.max_clients:
            self._logger.warning(f"Limite de clients atteinte ({self.max_clients}). Refus de la connexion de {websocket.remote_address}")
            await websocket.close(code=1008, reason="Server at capacity")
            return
            
        self.clients.add(websocket)
        self._logger.info(f"Nouveau client connecté: {websocket.remote_address}. Total: {len(self.clients)}")
        ...
        await websocket.send(json.dumps({
            ...
            "message": "Connecté au serveur de monitoring"
        }))
    ...
    async def unregister_client(self, websocket: Any) -> None:
        ...
        if websocket in self.clients:
            ...
            self._logger.info(f"Client déconnecté: {websocket.remote_address}")
            ...
            async def send_with_semaphore(client: Any) -> None:
                async with self._broadcast_semaphore:
                    try:
                        ...
                        self._logger.error(f"Erreur lors de l'envoi au client {getattr(client, 'remote_address', 'unknown')}: {e}")
                        disconnected_clients.add(client)
    ...
    async def start_server(self, monitoring_service: RealtimeMonitoringService) -> None:
        ...
        self._logger.info(f"Serveur WebSocket démarré sur {self.host}:{self.port}")
    ...
    async def stop_server(self) -> None:
        ...
        self._logger.info("Serveur WebSocket arrêté")
    ...
    def run(self, monitoring_service: RealtimeMonitoringService) -> None:
        ...
        try:
            ...
            self._logger.info("Arrêt du serveur WebSocket...")