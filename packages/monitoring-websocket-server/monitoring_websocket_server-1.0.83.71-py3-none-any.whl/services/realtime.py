...
from ..config import (
    ...
    THREAD_POOL_WORKERS, DEFAULT_EXPORT_DIR, HISTORY_TTL,
    MAX_MONITORING_INTERVAL, MAX_ERROR_COUNT, ERROR_RETRY_DELAY,
...
class RealtimeMonitoringService:
    ...
    def _validate_interval(interval: float) -> float:
        ...
        if interval > 3600:  # Max 1 heure
            raise ValueError("L'intervalle ne doit pas dépasser 1 heure")
        return interval
    ...
    async def start(self) -> None:
        ...
        if self._status == MonitoringStatus.STARTING:
            self._logger.warning("Le service est déjà en cours de démarrage")
            return

        try:
            self._status = MonitoringStatus.STARTING
            self._logger.info("Démarrage du service de monitoring temps réel")
    ...
    async def stop(self) -> None:
        ...
        if self._status == MonitoringStatus.STOPPING:
            self._logger.warning("Le service est déjà en cours d'arrêt")
            return
        ...
        try:
            ...
            raise ServiceShutdownError(f"Erreur lors de l'arrêt du service: {e}")
    ...
    async def _cleanup_tasks(self) -> None:
        ...
        try:
            ...
            self._logger.error(f"Erreur lors de la fermeture de l'executor: {e}")
        finally:
            self._executor = None
    ...
    async def _monitoring_loop(self) -> None:
        ...
        while self._status in (MonitoringStatus.STARTING, MonitoringStatus.RUNNING):
            try:
                ...
                self._logger.error(f"Erreur dans la boucle de monitoring: {e}")
    ...
    async def _collect_system_data(self) -> None:
        ...
        try:
            ...
            if self._executor is None:
                raise DataCollectionError("ThreadPoolExecutor non initialisé")
            ...
            self._logger.error(f"Erreur lors de la collecte des données: {e}")
            # Limite errors_count pour éviter overflow après très longue exécution
            if self._stats["errors_count"] < 1000000:  # 1 million max
                self._stats["errors_count"] += 1
            raise DataCollectionError(f"Erreur de collecte: {e}")
    ...
    async def force_export(self) -> None:
        ...
        if self._current_snapshot:
            ...
            self._logger.info("Export forcé effectué")
    ...
    def _cleanup_old_snapshots(self) -> None:
        ...
        try:
            ...
            self._logger.error(f"Erreur lors du nettoyage de l'historique: {e}")
