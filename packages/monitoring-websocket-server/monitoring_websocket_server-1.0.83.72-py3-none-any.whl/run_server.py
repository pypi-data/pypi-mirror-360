...
class StandaloneWebSocketServer:
    ...
    async def register_client(self, websocket: Any) -> None:
        ...
        if len(self.clients) >= self.max_clients:
            logger.warning(f"Limite de clients atteinte ({self.max_clients}). Refus de {websocket.remote_address}")
            await websocket.close(code=WEBSOCKET_CLOSE_CODE_CAPACITY, reason="Server at capacity")
            return
            
        self.clients.add(websocket)
        logger.info(f"Nouveau client connecté: {websocket.remote_address}. Total: {len(self.clients)}")
                ...
                if self.clients:
                    ...
                    async def send_to_client(client):
                        try:
                            ...
                            logger.error(f"Erreur envoi à {getattr(client, 'remote_address', 'unknown')}: {exc}")
                            disconnected.add(client)
    ...
    async def start(self) -> None:
        ...
        logger.info(f"Démarrage du serveur WebSocket sur {self.host}:{self.port}")
...
def main() -> None:
    ...
    logger.info("=" * 50)
    logger.info("Serveur WebSocket de monitoring autonome")
    logger.info("=" * 50)
    logger.info(f"Écoute sur ws://{WEBSOCKET_HOST}:{WEBSOCKET_PORT}")

    server = StandaloneWebSocketServer()
