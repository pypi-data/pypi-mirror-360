...
from r00system import CMDResult, run, exists_process, run_background, run_stream
...
class ADBase:
    ...
    def exec(
        ...
        if stream:
            ...
            if not background:
                result = run(f'adb {command}', timeout=timeout, ignore_errors=ignore_errors, disable_log=log_off, verbose=verbose)
            else:
