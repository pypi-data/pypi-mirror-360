...
class ErioonClient:
    ...
    def _session_login(self, db_id):
        sas_info = self.sas_tokens.get(db_id)
        if not sas_info:
            raise RuntimeError(f"[ErioonClient] No login certificate info for database: {db_id}")

        now = datetime.now(timezone.utc)
        expiry = sas_info["expiry"]
        if now >= expiry - timedelta(minutes=1):
            print("[ErioonClient] Login session has expired. Refreshing login...")
            self._perform_login()
