...
class InMemoryInsertOne:
    ...
    def __init__(self, user_id, db, collection, container_url):
        ...
        # Use .msgpack instead of .json for index and logs
    ...
    def _load_index(self):
        ...
        try:
            index_bytes = blob_client.download_blob().readall()
            self.index_data = msgpack.unpackb(index_bytes, raw=False)
        except Exception:
            self.index_data = []
    ...
    def _load_logs(self):
        ...
        try:
            logs_bytes = blob_client.download_blob().readall()
            self.logs_data = msgpack.unpackb(logs_bytes, raw=False)
        except Exception:
            self.logs_data = {}
