...
def save_logs(user_id_cont, database, collection, method, log_type, log_message, count, container_url):
    """
    Saves an individual log entry into logs.msgpack inside the container.
    ...
    # Upload updated logs
    with BytesIO() as out_file:
        out_file.write(msgpack.packb(logs_data, use_bin_type=True))
        out_file.seek(0)
...
def generate_next_id(last_id, shard_number):
    ...
    if not last_id:
        ...
        try:
            ...
            last_dt = decode_datetime_from_hex(datetime_hex)
    ...
    return f"{datetime_hex}-{shard_hex}-{record_hex}"
