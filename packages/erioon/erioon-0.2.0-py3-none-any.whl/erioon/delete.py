...
def handle_delete_with_id(user_id, db_id, coll_id, record_id, container_url):
    ...
    - Load the index.msgpack file which maps shards to record IDs.
    ...
    - Update index.msgpack to reflect deletion.
    ...
    index_blob_client = container_client.get_blob_client(f"{db_id}/{coll_id}/index.msgpack")

    if not index_blob_client.exists():
        return {"error": "Index file does not exist"}, 404

    index_data_bytes = index_blob_client.download_blob().readall()
    try:
        index_data = msgpack.unpackb(index_data_bytes, raw=False)
    except Exception:
        return {"error": "Failed to read index.msgpack file"}, 500
