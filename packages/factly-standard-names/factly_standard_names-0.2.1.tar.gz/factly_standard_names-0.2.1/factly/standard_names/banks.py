...
def bank_names(dfObj, column_name, thresh=70, manual_changes={}, identifier="None"):
    """
    find all improper bank names from a given dataframe
    and replaces it with standard names proved.
    dfObj : DataFrame object on which bank names should be standardize
    column_name : name of column which has entries as bank names
    manual_changes = Dict , default : null dict , changes in names done manually.
    """

    proper_name = get_std_names(filter="bank")  # gets the list of standard bank names
