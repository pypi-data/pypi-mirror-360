    ...
    if workspace_dir.exists():
        if lock_path.exists():
            ...
            if existing_scaffold == lock_data["scaffold"]:
                typer.echo("✅ Scaffold matches existing scaffold.lock. Skipping workspace generation.", fg=typer.colors.BLUE)
                return False
            else:
                typer.confirm(
...
def _init_workspace(target_dir: Path, name: str, lock_data: dict, set_default: bool) -> WorkspaceFactory:
    ...
    typer.secho(f"Workspace '{name}' initialized at {wf.workspace_dir}",fg=typer.colors.GREEN)
