...
from bestsms.api.addressbook.contacts.responses.contact_api_result import ContactApiResult

class ContactDetail(Common):
    ...
    def __PostMessage(self):
        try:
            r = requests.get(f"{self.APIURL}/addressbook/contact/{self.ContactID}", headers=self.APIHeaders)
            r.raise_for_status()
    ...
    def __str__(self):
        ...
        return 'ContactDetail(AuthToken='+self.AuthToken+ ')'