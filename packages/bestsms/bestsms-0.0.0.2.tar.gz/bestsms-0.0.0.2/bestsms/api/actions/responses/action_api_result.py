from bestsms.api.actions.dtos.action_api_result_dto import ActionApiResultDTO
from bestsms.core.dtos.error_with_message_id_dto import ErrorWithMessagIDDTO
from bestsms.helpers.functions import Functions

class ActionApiResult(object):

    def __init__(self, **kwargs):
        
        for key, value in kwargs.items():
            
            if key == "response":
                self.ParseResponse(value)
            
            if key == "error":
                self.Data = ActionApiResultDTO(
                    Result = "Error",
                    ErrorMessage = [value]
                )

    def ParseResponse(self, r):

        if r.text:

            self.__response_string__ = r.text

            response = Functions.__parsejson__(self,r.text)

            data = ActionApiResultDTO()

            for key, value in response.items():
                                      
                setattr(data, key, value)

            self.Data = data

    """ Data """
    @property
    def Data(self) -> ActionApiResultDTO:
        
        if self.__data.Result != "Success":
            
            return ErrorWithMessagIDDTO(
                Result=self.__data.Result,
                MessageID=self.__data.MessageID,
                ErrorMessage=self.__data.ErrorMessage
            )
        
        return self.__data
    
    @Data.setter
    def Data(self, val):
        self.__data = val

    def __repr__(self):
        return Functions.__pretty_class__(self, self.Data)

    def __str__(self):
        return 'ActionApiResult('+ self.__response_string__ +')'

    