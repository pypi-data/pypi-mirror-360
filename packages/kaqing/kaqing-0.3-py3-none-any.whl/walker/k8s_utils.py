...
def get_user_pass(ss_name: str, namespace: str, secret_path: str = 'cql.secret'):
    ...
    secret_name = Config().get(f'{secret_path}.name', '{cluster}-superuser').replace('{cluster}', groups[1], 1)
    v1 = client.CoreV1Api()
    try:
...
def cassandra_nodes_exec(statefulset: str, namespace: str, command: str, action: str, max_workers=0):
    ...
    if max_workers > 0:
        ...
        for pod_name in pods:
            try:
                result = cassandra_pod_exec(pod_name, namespace, command, throw_err=True)
...
def pod_exec(pod_name: str, container: str, namespace: str, command: str, show_out = True, throw_err = False, interaction: Callable[[any, list[str]]] = None):
