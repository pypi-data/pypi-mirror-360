...
class ResultFile:
    def __init__(
        ...
        if self.type == ResultFileType.BYTE_ARRAY and file_name is not None and byte_array is not None:
            self.name = file_name
            self.__bytes__ = byte_array
            self.__path__ = None

        elif self.type == ResultFileType.FILE_PATH and file_path is not None:
            ...
            raise ValueError(
                "Invalid parameters provided. For byte array result files, both 'file_name' and 'byte_array' must be provided. For file path result files, only 'file_path' is required."
            )
