...
from ecmind_blue_client.const import FieldSchema, ImportActions, Jobs, MainTypeId, StoreInCacheByIdConversion, SystemFields
...
class Client(ABC):
    ...
    def xml_import(
        ...
        - `search_fields` -- Dict of internal field names and values.
    ...
    def lol_query(
        ...
        - `include_file_info` -- (Optional) bool indicating,
        if file size, file extension and mime type should be added to document results, default = False.
        - `follow_doc_links` -- (Optional) bool indicating,
        the include_file_info returns the data of referenced files ("green arrows"), default = False.
        - `include_status` -- (Optional) bool indicating, if status field should be added to the result, default = False.
        - `garbage_mode` -- (Optional) bool indicating,
        if the query searches the recycle bin instead of non-deleted objects, default = False.
        - `context_user` -- (Optional) string to change the job context to another user name.
        - `field_schema -- (Optional) defines if the api will return all fields or only the the fields defined in `result_fields`.
        By default, only the defined fields will be return if `result_fields` is not empty resp. only the defined fields if `result_fields`
        is not empty. https://ecm.community/t/lol-query-systemfields-auch-mit-schema-all/375
        """
