...
class UniversalPatternExtractor:
    ...
    def _analyze_html_structure(soup: BeautifulSoup, base_url: str) -> dict[str, Any]:
        ...
        for link in all_links:
            ...
            if isinstance(parent, Tag):
                parent_classes = parent.get("class") or []  # type: ignore[arg-type]
                parent_class = " ".join(parent_classes) if parent_classes else ""
        ...
        parent_patterns: Counter[str] = Counter()
                ...
                if isinstance(classes, list):
                    ...
                    for keyword in layout_keywords:
                        if keyword in class_str.lower():
                            css_layout_patterns[f"{element.name}.{class_str}"] += 1
        ...
        return {
            ...
            "avg_text_length": (
                sum(ctx["text_length"] for ctx in link_contexts) / len(link_contexts)
                if link_contexts
                else 0
            ),
        }
    ...
    def _extract_links_using_pattern(
        ...
        for selector in selectors_to_try:
            ...
            # We stop only if the number of *unique* links has reached (or exceeded)
            # the model's estimated_items value (when >0). This lets us capture
            # additional lists when the first selector returns just a subset.
            if (
