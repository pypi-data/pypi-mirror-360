...
class UniversalPatternExtractor:
    ...
    def _analyze_html_structure(soup: BeautifulSoup, base_url: str) -> dict[str, Any]:
        ...
        for element in all_links:
            ...
            if isinstance(parent, Tag):
                parent = cast("Tag", parent)
                parent_tag = parent.name
                parent_classes = parent.get("class")
                if parent_classes and isinstance(parent_classes, list):
                    parent_class = " ".join(str(cls) for cls in parent_classes)
                else:
                    parent_class = ""
                parent_pattern = (
                ...
                if parent_classes and isinstance(parent_classes, list):
                    for cls in parent_classes:
                        ...
                        if any(
                            ...
                            css_layout_patterns[cls_str] += 1

            # Analyze URL patterns
            href_str = str(href) if href else ""
            parsed_url = urlparse(urljoin(base_url, href_str))
            ...
            if isinstance(parent, Tag):
                parent_classes = parent.get("class")
                if parent_classes and isinstance(parent_classes, list):
                    parent_class_str = " ".join(str(cls) for cls in parent_classes)
    ...
    def _fallback_pattern(self) -> PatternAnalysis:
        """Return a fallback pattern when LLM is unavailable."""
