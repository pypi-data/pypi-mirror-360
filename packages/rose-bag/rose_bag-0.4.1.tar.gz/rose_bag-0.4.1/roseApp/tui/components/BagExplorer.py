...
from roseApp.core.util import get_logger, get_preferred_parser_type
...
class BagExplorer(DirectoryTree):
    ...
    # Use automatic parser selection instead of hardcoded PYTHON parser
    def _create_default_bag_manager(self):
        """Create BagManager with automatically selected parser"""
        preferred_type = get_preferred_parser_type()
        if preferred_type == 'rosbags':
            return BagManager(create_parser(ParserType.ROSBAGS))
        else:
            return BagManager(create_parser(ParserType.PYTHON))
    
    bags = reactive(None)  # Will be set in __init__
    ...
    def __init__(self, init_path: str = "."):
        ...
        # Auto-select best parser based on availability
        preferred_type = get_preferred_parser_type()
        if preferred_type == 'rosbags':
        ...
        # Override with config if CPP is explicitly requested
        if config.get('load_cpp_parser', False):
            parser_type = ParserType.CPP
