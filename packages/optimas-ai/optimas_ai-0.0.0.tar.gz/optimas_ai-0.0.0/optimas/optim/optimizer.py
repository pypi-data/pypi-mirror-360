...
class OptimasOptimizer:
    ...
    def evaluate_system(self, system, eval_set):
        """
        Evaluate the system on the test dataset and return average score
        """
        metrics, _ = eval_system(system=system, testset=eval_set, num_repeat=1)
        if "mean" in metrics:
            return metrics["mean"]
    ...
    def optimize(self):
        ...
        try:
            ...
            initial_score = self.evaluate_system(self.system, self.val_dataset)
        ...
        skip_online_datagen = self.optimas_args.skip_online_datagen
        enable_data_gen = False or self.preference_dataset is not None

        if skip_online_datagen and not self.preference_dataset:
            self.logger.error(
        ...
        for iteration in range(self.iterations):
            ...
            if skip_online_datagen or not enable_data_gen:
                self.logger.info(
                    "Skipping fresh training dataset generation due to skip_online_datagen flag"
                )
            ...
            if component_to_optimize in fresh_preference_dataset:
                ...
                try:
                    new_score = self.evaluate_system(
                        optimized_system, self.val_dataset
                    )
                    self.logger.info(
