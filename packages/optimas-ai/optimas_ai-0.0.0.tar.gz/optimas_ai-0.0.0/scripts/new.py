...
class OnPolicyArgs:
    ...
    iterations: int = 100
    per_iteration_input_size: int = 50
    per_iteration_rm_train_size: int = -1
    per_component_train_size: int = 300
    per_component_search_size: int = 300
    num_prompt_candidates: int = 3

    use_replay_buffer: bool = True
    replay_buffer_size: int = 200

    weight_optimizer: str = "ppo"
    ppo_epochs: int = 3
    ppo_train_steps: int = 3
...
if __name__ == "__main__":
    ...
    parser = HfArgumentParser(OnPolicyArgs)
    ...
    args.output_dir = osp.join(args.output_dir, args.dataset, args.system, args.run_name)
    ...
    if args.system == "amazon_next_item_selection_local_system":
        logger.info(
            f"Using Amazon system with adapters: {args.session_adapter=}, {args.profiler_adapter=}"
        )
