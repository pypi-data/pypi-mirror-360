...
class BaseComponent:
    ...
    def update(self, new_variable: Any) -> None:
        ...
        with self._lock:
            ...
            logger.info(f"Variable updated.")
    ...
    def __call__(self, **inputs: Any) -> Dict[str, Any]:
        ...
        if getattr(current_config, 'randomize_variable', False) and self.variable_search_space:
            ...
            logger.info(f"Sampled random variable for this call.")

        outputs = None
        exception = None
        logger.info(f"[CALL] {self.__class__.__name__}: Current variable: {self.variable}, config: {self.config}")
