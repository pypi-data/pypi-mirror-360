...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `trackdo`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in trackdo.__all__ or not hasattr(trackdo, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in trackdo.__all__:
