...
function writeRSS($blog, $items) {
    global $headerRSS;
    echo("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
    echo("<rss version=\"2.0\">\n");
    echo("  <channel>\n");
    foreach($headerRSS as $key => $value) {
        echo("    <".$key.">".$blog[$value]."</".$key.">\n");
    }
...
function writeAtom($blog, $items) {
    global $headerAtom;
    global $itemAtom;
    echo("<?xml version=\"1.0\" encoding=\"utf-8\"?>\n");
    echo("<feed xml:lang=\"".$blog["language"]."\" xmlns=\"http://www.w3.org/2005/Atom\">\n");
    echo("  <link href=\"".$blog["linkAtom"]."\" rel=\"self\"/>\n");
    foreach($headerAtom as $key => $value) {
        echo("  <".$key.">".$blog[$value]."</".$key.">\n");
    }
    echo("  <author>\n");
    echo("    <name>".$blog["authorName"]."</name>\n");
    echo("    <email>".$blog["authorEmail"]."</email>\n");
    echo("  </author>\n");
    foreach($items as $item) {
        echo("  <entry>\n");
        echo("    <link type=\"text/html\" href=\"".$item["link"]."\" rel=\"self\"/>\n");
        foreach($itemAtom as $key => $value) {
            echo("    <".$key.">".$item[$value]."</".$key.">\n");
        }
        echo("  </entry>\n");
    }
    echo("</feed>\n");  
...
foreach ($items as $entry) {
    ...
    $idate = date_create($entry->date);
    $rssItem = array(
        "title"=>$title,
        "link"=>$url,
        "description"=>$desc,
        "pubDate"=>$idate->format(DATE_RFC822),
        "guid"=>$url
        );
    $rssItems[$r] = $rssItem;
    $sortArray[$r] = $idate;
...
array_multisort($sortArray, SORT_DESC, $rssItems);

$type = params_get("type");
if($type==null || $type=="rss") {
    writeRSS($blogDefinition, $rssItems);
} else {
    writeAtom($blogDefinition, $rssItems);
}
?>