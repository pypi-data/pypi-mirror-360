...
def list_examples(args):
    ...
    console = Console()
    with console.status("[bold green]Searching for examples...[/bold green]"):
        examples_dir, is_package = find_examples_dir()

    if not examples_dir:
        console.print("[bold red]Error:[/bold red] Could not find the bundled 'examples' directory.", file=sys.stderr)
        sys.exit(1)
...
def fetch_example(args):
    ...
    console = Console()
    with console.status("[bold green]Fetching example...[/bold green]"):
        examples_dir, is_package = find_examples_dir()
    ...
    if not source_path.is_dir():
        console.print(f"[bold red]Error:[/bold red] Example '{args.name}' not found.", file=sys.stderr)
        console.print("Use 'fastworkflow examples list' to see available examples.")
        sys.exit(1)

    # If examples are only found locally (not in package), skip the fetch operation
    if not is_package:
        console.print(f"Note: Example '{args.name}' is already in the local examples directory.")
        return source_path
        ...
        with console.status("[bold green]Copying files...[/bold green]"):
            ...
            if env_file.exists():
                if not local_env_file.exists():
                    ...
                    console.print(f"✅ Copied environment file to '{local_env_file}'")
                else:
                    console.print(f"⚠️ Environment file already exists at '{local_env_file}', skipping copy")
                
                # Remind users to add their API keys
                console.print("\n[bold]NOTE:[/bold] You need to add your API keys to the passwords file before training.")
                console.print(f"      Edit '{local_passwords_file}' with your API keys.")
            else:
                console.print(f"⚠️ [yellow]Warning:[/yellow] Environment file not found at '{env_file}'")
            ...
            if passwords_file.exists():
                if not local_passwords_file.exists():
                    ...
                    console.print(f"✅ Copied passwords file to '{local_passwords_file}'")
                else:
                    console.print(f"⚠️ Passwords file already exists at '{local_passwords_file}', skipping copy")
            else:
                console.print(f"⚠️ [yellow]Warning:[/yellow] Passwords file not found at '{passwords_file}'")
                ...
                if not local_passwords_file.exists():
                    ...
                    console.print(f"✅ Created template passwords file at '{local_passwords_file}'")
                else:
                    console.print(f"⚠️ Using existing passwords file at '{local_passwords_file}'")
        
        console.print(f"\n✅ Example '{args.name}' copied to '{target_path}'")
        return target_path
    except Exception as e:
        console.print(f"[bold red]Error copying example:[/bold red] {e}", file=sys.stderr)
        import traceback
...
def train_example(args):
    ...
    console = Console()
    console.print("[bold green]Training example...[/bold green]\n")
    ...
    if not workflow_path.is_dir():
        console.print(f"[bold red]Error:[/bold red] Example '{args.name}' not found in '{local_examples_dir}'.", file=sys.stderr)
        console.print(f"Use 'fastworkflow examples fetch {args.name}' to fetch the example first.")
        console.print("Or use 'fastworkflow examples list' to see available examples.")
        sys.exit(1)

    console.print(f"Training example in '{workflow_path}'...")
    ...
    if not env_file.exists() or not passwords_file.exists():
        console.print(f"[bold red]Error:[/bold red] Required environment files not found:", file=sys.stderr)
        if not env_file.exists():
            console.print(f"  - {env_file} (not found)")
        if not passwords_file.exists():
            console.print(f"  - {passwords_file} (not found)")
        console.print("\nPlease run the following command to fetch the example and its environment files:")
        console.print(f"  fastworkflow examples fetch {args.name}")
        console.print("\nAfter fetching, edit the passwords file to add your API keys:")
        console.print(f"  {local_examples_dir}/fastworkflow.passwords.env")
        sys.exit(1)
    ...
    try:
        ...
        if result is None or result == 0:
            ...
            console.print(f"You can now run it with:\npython -m fastworkflow.run {workflow_path}")
        else:
            console.print(f"\n❌ Training failed.", file=sys.stderr)
            sys.exit(1)

    except Exception as e:
        console.print(f"[bold red]An unexpected error occurred during training:[/bold red] {e}", file=sys.stderr)
        import traceback
        traceback.print_exc(file=sys.stderr)
        sys.exit(1)
...
def run_example(args):
    ...
    console.print(f"[bold green]Running example[/bold green] '{args.name}'...")
    ...
    if not env_file.exists() or not passwords_file.exists():
        ...
        console.print(f"  fastworkflow examples train {args.name}")
        sys.exit(1)
    ...
    if not command_info_dir.exists() or not any(command_info_dir.iterdir()):
        console.print(f"[bold yellow]Warning:[/bold yellow] Example '{args.name}' does not appear to be trained yet.", file=sys.stderr)
        console.print(f"Please train the example first with:")
        console.print(f"  fastworkflow examples train {args.name}")
    ...
    try:
        console.print(f"[bold green]Starting interactive session...[/bold green]")
        # Replace the current process with the run command
        # This ensures that the interactive prompt works correctly
        os.execvp(sys.executable, cmd)
    except Exception as e:
        console.print(f"[bold red]An unexpected error occurred while running the example:[/bold red] {e}", file=sys.stderr)
        import traceback
        traceback.print_exc(file=sys.stderr)
        sys.exit(1)
