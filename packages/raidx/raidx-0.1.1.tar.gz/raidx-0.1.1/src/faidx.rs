...
impl Faidx {
    ...
    ///
    /// Efficiently processes large numbers of regions (e.g., 200,000+) using smart batching:
    /// - Sequential processing for small batches (< 1000 regions) to avoid overhead
    /// - Parallel processing for large batches to maximize throughput
    ///
    /// Args:
    ...
    pub fn fetch_many(&self, regions: Vec<(String, usize, usize)>) -> PyResult<Vec<Sequence>> {
        if regions.is_empty() {
            return Ok(Vec::new());
        }

        // Use sequential processing for small batches to avoid parallel overhead
        const PARALLEL_THRESHOLD: usize = 1000;

        if regions.len() < PARALLEL_THRESHOLD {
            // Sequential path - much faster for small batches
            let mut results = Vec::with_capacity(regions.len());
            for (name, start, end) in regions {
                match self.fetch(name, start, end) {
                    Ok(seq) => results.push(seq),
                    Err(e) => return Err(e),
                }
            }
            return Ok(results);
        ...
        // Create indexed regions to preserve order
        let indexed_regions: Vec<(usize, String, usize, usize)> = regions
            .into_iter()
            .enumerate()
            .map(|(idx, (name, start, end))| (idx, name, start, end))
            .collect();

        // Use Arc to share memory map across threads safely if available
        let mmap_arc = if let Some(ref mmap) = self.file_mmap {
            Some(Arc::new(mmap))
        } else {
            None
        };
        ...
        // Process in parallel
        let parallel_results: Vec<(usize, std::result::Result<Sequence, FaidxError>)> =
            indexed_regions
                .par_iter()
                .map(|(idx, name, start, end)| {
                    let result = self.fetch_single_optimized(name, *start, *end, mmap_arc.as_ref());
                    (*idx, result)
                })
                .collect();

        // Sort results back to original order and convert to final format
        let mut indexed_results = parallel_results;
        indexed_results.sort_by_key(|(idx, _)| *idx);

        let mut final_results = Vec::with_capacity(indexed_results.len());
        for (idx, result) in indexed_results {
            match result {
                Ok(seq) => final_results.push(seq),
                Err(e) => {
    ...
    ///
    /// Optimized for BED file processing where many regions come from the same chromosome.
    /// Uses smart batching like fetch_many but optimized for single chromosome access.
    ///
    /// Args:
    ...
    ///     >>> sequences = faidx.fetch_many_same_chr("chr1", regions)
    pub fn fetch_many_same_chr(
        &self,
        name: String,
        ...
        // Pre-validate that the sequence exists
        ...
        if regions.len() < PARALLEL_THRESHOLD {
            ...
            for (start, end) in regions {
                match self.fetch(name.clone(), start, end) {
                    Ok(seq) => results.push(seq),
                    Err(e) => return Err(e),
                }
            }
        ...
        // Process in parallel with preserved order
        let parallel_results: Vec<std::result::Result<Sequence, FaidxError>> = regions
            .par_iter()
            .map(|(start, end)| self.fetch_single_optimized(&name, *start, *end, mmap_arc.as_ref()))
            .collect();

        // Convert results, handling errors
        let mut final_results = Vec::with_capacity(parallel_results.len());
        for (idx, result) in parallel_results.into_iter().enumerate() {
            match result {
                Ok(seq) => final_results.push(seq),
                Err(e) => {
                    return Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                        "Error fetching region {} from {}: {}",
                        idx, name, e
                    )));
                }
            }
...
impl Faidx {
    /// Internal optimized fetch method for parallel processing
    fn fetch_single_optimized(
        &self,
        name: &str,
        start: usize,
        end: usize,
        mmap_arc: Option<&Arc<&Mmap>>,
    ) -> std::result::Result<Sequence, FaidxError> {
        use std::cmp::min;

        let Some(record) = self.index.get(name) else {
            return Err(FaidxError::FetchError(format!(
                "Sequence '{}' not found",
                name
            )));
        };
        ...
        let length = record.rlen;
        let (clamped_start, clamped_end) = if self.strict_bounds {
            if start > length || end > length || start == 0 || end == 0 || start > end {
                return Err(FaidxError::InvalidRegion(format!(
                    "Invalid region {}:{}-{} (sequence length: {})",
                    name, start, end, length
                )));
            }
            (start, end)
        } else {
            let clamped_start = min(start.max(1), length);
            let clamped_end = min(end.max(1), length);
            if clamped_start > clamped_end {
                return Ok(Sequence::new(
                    name.to_string(),
                    String::new(),
        ...
        // Extract sequence using the fastest available method
        let mut seq = if let Some(mmap_arc) = mmap_arc {
            // Use shared memory map for parallel access
            Self::extract_sequence_from_mmap(mmap_arc, record, clamped_start, seq_len)?
        } else if let Some(ref file) = self.file_handle {
            // Fall back to file I/O (not parallel-safe, but works)
            Self::extract_sequence_from_file_optimized(file, record, clamped_start, seq_len)?
        } else {
            return Err(FaidxError::IoError(std::io::Error::new(
                std::io::ErrorKind::Other,
