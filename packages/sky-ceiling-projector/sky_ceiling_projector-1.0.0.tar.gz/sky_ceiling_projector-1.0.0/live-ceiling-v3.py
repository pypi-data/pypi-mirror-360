...
SETTINGS = QUALITY_SETTINGS[args.preset]
...
class Satellite:
    ...
    def update(self, dt):
        ...
        # Check if off screen
        return not (self.x < -50 or self.x > SCREEN_WIDTH + 50 or 
...
class MilkyWay:
    ...
    def generate_milky_way(self):
        ...
        center_y = SCREEN_HEIGHT * random.uniform(0.3, 0.7)  # Height across sky
        
        # Create star clouds along the galactic plane
        num_clouds = 8
        for i in range(num_clouds):
            x = (i / num_clouds) * SCREEN_WIDTH
        ...
        for _ in range(3):
            lane_x = random.uniform(SCREEN_WIDTH * 0.2, SCREEN_WIDTH * 0.8)
    ...
    def draw(self, screen, star_visibility):
        ...
        dust_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
...
class SimpleLightning:
    ...
    def trigger(self):
        ...
        self.branches = []
        start_x = random.randint(SCREEN_WIDTH // 4, 3 * SCREEN_WIDTH // 4)
        start_y = 0
...
class SkySimulator:
    def __init__(self):
        ...
        self.clouds = []
        self.particles = []
        self.lightning = SimpleLightning()
    ...
    def create_planets(self):
        ...
        if random.random() < 0.7:  # 70% chance Venus is visible
            if random.random() < 0.5:  # Evening star
                venus_x = SCREEN_WIDTH * random.uniform(0.1, 0.4)
                venus_y = SCREEN_HEIGHT * random.uniform(0.6, 0.9)
            else:  # Morning star
                venus_x = SCREEN_WIDTH * random.uniform(0.6, 0.9)
                venus_y = SCREEN_HEIGHT * random.uniform(0.6, 0.9)
        ...
        if random.random() < 0.4:  # 40% chance Mars is visible
            mars_x = SCREEN_WIDTH * random.uniform(0.2, 0.8)
            mars_y = SCREEN_HEIGHT * random.uniform(0.3, 0.8)
        ...
        if random.random() < 0.5:  # 50% chance Jupiter is visible
            jupiter_x = SCREEN_WIDTH * random.uniform(0.3, 0.7)
            jupiter_y = SCREEN_HEIGHT * random.uniform(0.2, 0.7)
        ...
        if random.random() < 0.3:  # 30% chance Saturn is visible
            saturn_x = SCREEN_WIDTH * random.uniform(0.2, 0.8)
            saturn_y = SCREEN_HEIGHT * random.uniform(0.4, 0.8)
    ...
    def update_weather_effects(self):
        ...
        if weather == "Clear":
            ...
            num_clouds = SETTINGS["max_clouds"]
            for i in range(num_clouds):
                x = random.uniform(-200, SCREEN_WIDTH + 200)
                y = random.uniform(0, SCREEN_HEIGHT * 0.5)
                size = random.uniform(80, 200)
                speed = random.uniform(20, 60)
                opacity = random.uniform(0.6, 0.9)
                self.clouds.append(DetailedCloud(x, y, size, speed, opacity))
            ...
            for i in range(num_clouds):
                x = random.uniform(-200, SCREEN_WIDTH + 200)
                y = random.uniform(0, SCREEN_HEIGHT * 0.4)
            ...
            # Rain particles
            num_particles = SETTINGS["max_particles"]
            for i in range(num_particles):
                self.particles.append(SimpleParticle("rain"))
            ...
            num_particles = SETTINGS["max_particles"]
            for i in range(num_particles):
                self.particles.append(SimpleParticle("snow"))
            ...
            num_clouds = SETTINGS["max_clouds"] // 2
            for i in range(num_clouds):
                x = random.uniform(-200, SCREEN_WIDTH + 200)
    ...
    def update(self, dt):
        ...
        if current_time > self.next_shooting_star and star_visibility > 0.5:
            self.shooting_stars.append(EnhancedShootingStar())
        ...
        #    self.bright_flares.append(BrightFlare())
    ...
    def draw_stars(self):
        ...
        self.milky_way.draw(self.screen, star_visibility)
    ...
    def draw_clouds(self):
        ...
        for cloud in self.clouds:
            cloud.draw(self.screen, cloud_color, self.sun_position)
    ...
    def draw_info_overlay(self):
        ...
        y_offset = SCREEN_HEIGHT - len(text_lines) * 40 - 20
