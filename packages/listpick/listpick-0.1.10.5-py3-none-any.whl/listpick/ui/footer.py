...
class Footer:
    def __init__(self, stdscr, colours_start, get_state_callback):
        """
...
class StandardFooter(Footer):
    def draw(self, h, w):
        ...
        sort_method_info = f"{state['sort_method']}" if state['sort_column'] is not None else "NA"
        ...
        if state["paginate"]:
            cursor_disp_str = f" {state['cursor_pos']+1}/{state['total_items']}  Page {state['page_num']}/{state['page_count']}  Selected {selected_count}"
        else:
            cursor_disp_str = f" {state['cursor_pos']+1}/{state['total_items']}  |  Selected {selected_count}"
        self.stdscr.addstr(h - 3, w-35, f"{cursor_disp_str:>33} ", curses.color_pair(self.colours_start+20))
