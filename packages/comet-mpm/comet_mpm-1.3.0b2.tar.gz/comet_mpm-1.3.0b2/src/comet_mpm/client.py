...
from .connection_helpers import sanitize_url, get_root_url
...
def get_comet_base_url(api_key: str) -> str:
    ...
    if api_key_parsed is not None and api_key_parsed.base_url is not None:
        ...
        return get_root_url(get_config("comet.url_override") or os.environ.get("COMET_URL_OVERRIDE"))
...
class Client:
    ...
    def get_model(self, model_id: Optional[str] = None) -> Dict:
        """
        ...
        """
        if model_id is None:
            model_id = get_config("COMET_PANEL_OPTIONS")["modelId"]
