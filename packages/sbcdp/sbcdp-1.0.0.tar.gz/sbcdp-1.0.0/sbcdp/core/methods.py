    ...
    async def find_element_by_text(self, text, tag_name=None, timeout=None):
        ...
        for element in elements:
            if element and not tag_name:
                element = self.__add_element_methods(element)
                return self.__add_element_methods(element)
            elif (
                element
                ...
                element = self.__add_element_methods(element.parent)
                return self.__add_element_methods(element)
            elif (
                element
                and element.parent
                ...
                element = self.__add_element_methods(element.parent.parent)
                return self.__add_element_methods(element)
        plural = "s"
        if timeout == 1:
            plural = ""
    ...
    async def select(self, selector, timeout=None):
        ...
        if ":contains(" in selector:
            ...
            if not elements:
                ...
                raise Exception(message)
            element = self.__add_element_methods(elements[0])
            return element
        failure = False
        try:
