...
class DatabaseQueue(OurQueue, GenericCRUD, metaclass=MetaLogger, object=queue_logger_object):
    ...
    def push_back(self, queue_item: dict = None, queue_dict: dict = None) -> None:
        ...
        queue_dict = {"queue_status_id": QueueStatusEnum.NEW.value, "process_id": None}
        self.update_by_column_and_value(column_value=queue_item[self.queue_item_id_column_name], data_dict=queue_dict)
    ...
    def get(self, *, action_ids: tuple = (), custom_condition: str = "",
        ...
        self.update_by_where(where=update_where, order_by=self.queue_item_id_column_name, limit=1, data_dict=queue_dict)
        ...
        if queue_item:
            ...
            self.logger.info("Entry retrieved and updated from the queue database successfully",
                             object=queue_item)
        else:
    ...
    def peek(self, action_ids: tuple = ()) -> dict:
        ...
        queue_item = self.select_one_dict_by_where(where=where, order_by=self.queue_item_id_column_name)
        return queue_item
    ...
    def _fix_action_ids(self, action_ids):
        ...
        fix_action_ids_result = action_ids if len(action_ids) != 1 else f"({action_ids[0]})"
