...
from rusty_tags import Div, P, Span, HtmlString
...
def Uk_select(*c, **kwargs): return fh.CustomTag('uk-select', *c, **kwargs)
def Uk_input_tag(*c, **kwargs): return fh.CustomTag('uk-input-tag', *c, **kwargs)
def Uk_icon(*c, **kwargs): return fh.CustomTag('uk-icon', *c, **kwargs)
def Uk_input_range(*c, **kwargs): return fh.CustomTag('uk-input-range', *c, **kwargs)
def Uk_chart(*c, **kwargs): return fh.CustomTag('uk-chart', *c, **kwargs)
...
def CodeSpan(*c, # Contents of CodeSpan tag (inline text code snippets)
             ...
             **kwargs # Additional args for CodeSpan tag
             )->HtmlString: # Code(..., cls='uk-codespan')
    "A CodeSpan with Styling"
    return fh.Code(*c, cls=stringify(('uk-codespan', stringify(cls))), **kwargs)
...
def CodeBlock(*c: str, # Contents of Code tag (often text)
    ...
    return Div(
        Pre(Code(*c, cls=stringify(('uk-codeblock', stringify(code_cls))), **kwargs),
            cls=stringify((f'bg-gray-100 dark:bg-gray-800 {TextT.gray} p-0.4 rounded text-sm font-mono'))),
#             cls=('bg-gray-100 dark:bg-gray-800 dark:text-gray-200 p-0.4 rounded text-sm font-mono')),
        cls=stringify(('uk-block', stringify(cls))))

# %% ../nbs/02_franken.ipynb
def H1(*c:HtmlString|str, # Contents of H1 tag (often text)
       cls:Enum|str|tuple=(), # Classes in addition to H1 styling
       **kwargs # Additional args for H1 tag
       )->HtmlString: # H1(..., cls='uk-h1')
    "H1 with styling and appropriate size"
    return fh.H1(*c, cls=stringify(('uk-h1',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def H2(*c:HtmlString|str, # Contents of H2 tag (often text)
       cls:Enum|str|tuple=(), # Classes in addition to H2 styling
       **kwargs # Additional args for H2 tag
       )->HtmlString: # H2(..., cls='uk-h2')
    "H2 with styling and appropriate size"
    return fh.H2(*c, cls=stringify(('uk-h2',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def H3(*c:HtmlString|str, # Contents of H3 tag (often text)
       cls:Enum|str|tuple=(), # Classes in addition to H3 styling
       **kwargs # Additional args for H3 tag
       )->HtmlString: # H3(..., cls='uk-h3')
    "H3 with styling and appropriate size"
    return fh.H3(*c, cls=stringify(('uk-h3',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def H4(*c:HtmlString|str, # Contents of H4 tag (often text)
       cls:Enum|str|tuple=(), # Classes in addition to H4 styling
       **kwargs # Additional args for H4 tag
       )->HtmlString: # H4(..., cls='uk-h4')
    "H4 with styling and appropriate size"
    return fh.H4(*c, cls=stringify(('uk-h4',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def H5(*c:HtmlString|str, # Contents of H5 tag (often text)
    ...
    return fh.H5(*c, cls=stringify(('text-lg font-semibold',stringify(cls))), **kwargs)

def H6(*c:HtmlString|str, # Contents of H6 tag (often text)
    ...
    return fh.H6(*c, cls=stringify(('text-base font-semibold',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Subtitle(*c:HtmlString|str, # Contents of P tag (often text)
    ...
    return fh.P(*c, cls=stringify((TextPresets.muted_sm, stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Q(*c:HtmlString|str, # Contents of Q tag (quote)
      cls:Enum|str|tuple=(), # Additional classes
      **kwargs # Additional args for Q tag
      )->HtmlString:
    "Styled quotation mark"
    return fh.Q(*c, cls=stringify((TextT.italic,TextT.lg, stringify(cls))), **kwargs)

def Em(*c:HtmlString|str, # Contents of Em tag (emphasis)
       cls:Enum|str|tuple=(), # Additional classes 
       **kwargs # Additional args for Em tag
       )->HtmlString:
    "Styled emphasis text"
    return fh.Em(*c, cls=stringify((TextT.medium, stringify(cls))), **kwargs)

def Strong(*c:HtmlString|str, # Contents of Strong tag
          cls:Enum|str|tuple=(), # Additional classes
          **kwargs # Additional args for Strong tag
          )->HtmlString:
    "Styled strong text" 
    return fh.Strong(*c, cls=stringify((TextT.bold, stringify(cls))), **kwargs)

def I(*c:HtmlString|str, # Contents of I tag (italics)
      cls:Enum|str|tuple=(), # Additional classes
      **kwargs # Additional args for I tag
      )->HtmlString:
    "Styled italic text"
    return fh.I(*c, cls=stringify((TextT.italic, stringify(cls))), **kwargs)

def Small(*c:HtmlString|str, # Contents of Small tag
         cls:Enum|str|tuple=(), # Additional classes
         **kwargs # Additional args for Small tag
         )->HtmlString:
    "Styled small text"
    return fh.Small(*c, cls=stringify((TextT.sm, stringify(cls))), **kwargs)

def Mark(*c:HtmlString|str, # Contents of Mark tag (highlighted text)
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Mark tag
        )->HtmlString:
    "Styled highlighted text"
    return fh.Mark(*c, cls=stringify((TextT.highlight, stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Del(*c:HtmlString|str, # Contents of Del tag (deleted text)
       cls:Enum|str|tuple=(), # Additional classes
       **kwargs # Additional args for Del tag
       )->HtmlString:
    "Styled deleted text"
    return fh.Del(*c, cls=stringify(('line-through', TextT.gray, stringify(cls))), **kwargs)

def Ins(*c:HtmlString|str, # Contents of Ins tag (inserted text)
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Ins tag
        )->HtmlString:
    "Styled inserted text"
    return fh.Ins(*c, cls=stringify((TextT.underline+' text-green-600', stringify(cls))), **kwargs)

def Sub(*c:HtmlString|str, # Contents of Sub tag (subscript)
       cls:Enum|str|tuple=(), # Additional classes
       **kwargs # Additional args for Sub tag
       )->HtmlString:
    "Styled subscript text"
    return fh.Sub(*c, cls=stringify((TextT.sm+' -bottom-1 relative', stringify(cls))), **kwargs)

def Sup(*c:HtmlString|str, # Contents of Sup tag (superscript) 
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Sup tag
        )->HtmlString:
    "Styled superscript text"
    return fh.Sup(*c, cls=stringify((TextT.sm+' -top-1 relative', stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Blockquote(*c:HtmlString|str, # Contents of Blockquote tag (often text)
               cls:Enum|str|tuple=(), # Classes in addition to Blockquote styling
               **kwargs # Additional args for Blockquote tag
               )->HtmlString: # Blockquote(..., cls='uk-blockquote')
    "Blockquote with Styling"
    return fh.Blockquote(*c, cls=stringify(('uk-blockquote',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Caption(*c:HtmlString|str, cls:Enum|str|tuple=(), **kwargs)->HtmlString:
    "Styled caption text"
    return fh.Caption(
        Span(*c, cls=stringify((TextT.gray, TextT.sm, stringify(cls)))),
        **kwargs)

# %% ../nbs/02_franken.ipynb
def Cite(*c:HtmlString|str, # Contents of Cite tag
         cls:Enum|str|tuple=(), # Additional classes
         **kwargs # Additional args for Cite tag
         )->HtmlString:
    "Styled citation text"
    return fh.Cite(*c, cls=stringify((TextT.italic, TextT.gray, stringify(cls))), **kwargs)

def Time(*c:HtmlString|str, # Contents of Time tag
         cls:Enum|str|tuple=(), # Additional classes
         datetime:str|None=None, # datetime attribute
         **kwargs # Additional args for Time tag
         )->HtmlString:
    "Styled time element"
    if datetime: kwargs['datetime'] = datetime
    return fh.Time(*c, cls=stringify((TextT.gray, stringify(cls))), **kwargs)

def Address(*c:HtmlString|str, # Contents of Address tag
           cls:Enum|str|tuple=(), # Additional classes
           **kwargs # Additional args for Address tag
           )->HtmlString:
    "Styled address element"
    return fh.Address(*c, cls=stringify((TextT.italic, stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Abbr(*c:HtmlString|str, # Contents of Abbr tag
         cls:Enum|str|tuple=(), # Additional classes
         title:str|None=None, # Title attribute for abbreviation
         **kwargs # Additional args for Abbr tag
         )->HtmlString:
    "Styled abbreviation with dotted underline"
    if title: kwargs['title'] = title
    return fh.Abbr(*c, cls=stringify(('border-b border-dotted border-secondary hover:cursor-help', stringify(cls))), **kwargs)

def Dfn(*c:HtmlString|str, # Contents of Dfn tag (definition)
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Dfn tag
        )->HtmlString:
    "Styled definition term with italic and medium weight"
    return fh.Dfn(*c, cls=stringify((TextT.medium + TextT.italic + TextT.gray, stringify(cls))), **kwargs)

def Kbd(*c:HtmlString|str, # Contents of Kbd tag (keyboard input)
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Kbd tag
        )->HtmlString:
    "Styled keyboard input with subtle background"
    return fh.Kbd(*c, cls=stringify(('font-mono px-1.5 py-0.5 text-sm bg-secondary border border-gray-300 dark:border-gray-600 rounded shadow-sm', stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Samp(*c:HtmlString|str, # Contents of Samp tag (sample output)
         cls:Enum|str|tuple=(), # Additional classes
         **kwargs # Additional args for Samp tag
         )->HtmlString:
    "Styled sample output with subtle background"
    return fh.Samp(*c, cls=stringify(('font-mono bg-secondary px-1 rounded', TextT.gray, stringify(cls))), **kwargs)

def Var(*c:HtmlString|str, # Contents of Var tag (variable)
        cls:Enum|str|tuple=(), # Additional classes
        **kwargs # Additional args for Var tag
        )->HtmlString:
    "Styled variable with italic monospace"
    return fh.Var(*c, cls=stringify(('font-mono',TextT.italic + TextT.gray, stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Figure(*c:HtmlString|str, # Contents of Figure tag
          cls:Enum|str|tuple=(), # Additional classes 
          **kwargs # Additional args for Figure tag
          )->HtmlString:
    "Styled figure container with card-like appearance"
    return fh.Figure(*c, cls=stringify(('p-4 my-4 border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm bg-card', stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Details(*c:HtmlString|str, # Contents of Details tag
           cls:Enum|str|tuple=(), # Additional classes
           **kwargs # Additional args for Details tag
           )->HtmlString:
    "Styled details element"
    return fh.Details(*c, cls=stringify(('border border-secondary rounded-lg', stringify(cls))), **kwargs)

def Summary(*c:HtmlString|str, # Contents of Summary tag
           cls:Enum|str|tuple=(), # Additional classes
           **kwargs # Additional args for Summary tag
           )->HtmlString:
    "Styled summary element"
    return fh.Summary(*c, cls=stringify((TextT.medium + ' p-3 hover:bg-secondary cursor-pointer', stringify(cls))), **kwargs)

def Data(*c:HtmlString|str, # Contents of Data tag
         value:str|None=None, # Value attribute
         cls:Enum|str|tuple=(), # Additional classes
         **kwargs # Additional args for Data tag
         )->HtmlString:
    "Styled data element"
    if value: kwargs['value'] = value
    return fh.Data(*c, cls=stringify(('font-mono text-sm bg-secondary px-1 rounded', stringify(cls))), **kwargs)

def Meter(*c:HtmlString|str, # Contents of Meter tag
    ...
    return fh.Meter(*c, cls=stringify(('w-full h-2 bg-secondary rounded', stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def S(*c:HtmlString|str, # Contents of S tag (strikethrough)
      cls:Enum|str|tuple=(), # Additional classes
      **kwargs # Additional args for S tag
      )->HtmlString:
    "Styled strikethrough text (different semantic meaning from Del)"
    return fh.S(*c, cls=stringify(('line-through', TextT.gray, stringify(cls))), **kwargs)

def U(*c:HtmlString|str, # Contents of U tag (unarticulated annotation)
      cls:Enum|str|tuple=(), # Additional classes
      **kwargs # Additional args for U tag
      )->HtmlString:
    "Styled underline (for proper names in Chinese, proper spelling etc)"
    return fh.U(*c, cls=stringify((TextT.underline, stringify(cls))), **kwargs)

def Output(*c:HtmlString|str, # Contents of Output tag
          form:str|None=None, # ID of form this output belongs to
          for_:str|None=None, # IDs of elements this output is for
          cls:Enum|str|tuple=(), # Additional classes
    ...
    return fh.CustomTag('output', *c, cls=stringify(('font-mono bg-secondary px-2 py-1 rounded', 
                             stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def PicSumImg(h:int=200,           # Height in pixels
              w:int=200,           # Width in pixels
              id:int|None=None,        # Optional specific image ID to use
              grayscale:bool=False, # Whether to return grayscale version
              blur:int|None=None,       # Optional blur amount (1-10)
...
# %% ../nbs/02_franken.ipynb
def AccordionItem(title: Union[str, HtmlString], # Content for the accordion item title
                  *c: HtmlString,                # Content to display when the item is open
                  ...
                  ) -> HtmlString: # Li(A(title, Span(Icon, Icon)), Div(content))
    ...
    return fh.Li(
        ...
        Div(*c, cls=content_classes, **div_attrs),
        cls=final_li_cls,
        **li_attrs
    )

# %% ../nbs/02_franken.ipynb
def Accordion(*c: HtmlString, # One or more `AccordionItem` components
              ...
              **kwargs                            # Additional attributes for the container tag (e.g., id)
              ) -> HtmlString: # Ul(*items...) or Div(*items...)
    """
    ...
    container_tag = fh.Ul if tag.lower() == 'ul' else Div
...
# %% ../nbs/02_franken.ipynb
def Button(*c: Union[str, HtmlString], # Contents of `Button` tag (often text)
...
# %% ../nbs/02_franken.ipynb
def Container(*c, # Contents of Container tag (often other HtmlString Components)
    ...
    return Div(*c, cls=stringify(('uk-container',stringify(cls))), **kwargs)
...
def Divider(*c, # contents of Divider tag (often nothing)
    ...
    return container(*c, cls=stringify(cls), **kwargs)
...
def Article(*c, # contents of Article tag (often other tags)
    ...
    return fh.Article(*c, cls=stringify(('uk-article',stringify(cls))), **kwargs)
...
def ArticleTitle(*c, # contents of ArticleTitle tag (often other tags)
    ...
    return H1(*c, cls=stringify(('uk-article-title',stringify(cls))), **kwargs)
...
def ArticleMeta(*c, # contents of ArticleMeta tag (often other tags)
    ...
    return P(*c, cls=stringify(('uk-article-meta',stringify(cls))), **kwargs)
...
def Section(*c, # contents of Section tag (often other tags)
    ...
    return Div(*c, cls=stringify(('uk-section',stringify(cls))), **kwargs)
...
def Fieldset(*c, # contents of Fieldset tag (often other tags)
    ...
    return fh.Fieldset(*c, cls=stringify(('uk-fieldset',stringify(cls))), **kwargs)
...
def Legend(*c, # contents of Legend tag (often other tags)
    ...
    return fh.Legend(*c, cls=stringify(('uk-legend',stringify(cls))), **kwargs)
...
def Input(*c, # contents of Input tag (often nothing)
    ...
    return fh.Input(*c, cls=stringify(('uk-input',stringify(cls))), **kwargs)
...
def Radio(*c, # contents of Radio tag (often nothing)
    ...
    return fh.Input(*c, cls=stringify(('uk-radio',stringify(cls))), type='radio', **kwargs)
def CheckboxX(*c, # contents of CheckboxX tag (often nothing)
    ...
    return fh.Input(*c, cls=stringify(('uk-checkbox',stringify(cls))), type='checkbox', **kwargs)
...
def Range(*c, # contents of Range tag (often nothing)
    ...
    return Uk_input_range(*c, min=min, label=label, max=max, value=value, step=step, multiple=len(value.split(','))>1, cls=stringify(('uk-range',stringify(cls))), **kwargs)
...
def TextArea(*c, # contents of TextArea tag (often text)
    ...
    return fh.Textarea(*c, cls=stringify(('uk-textarea',stringify(cls))), **kwargs)
def Switch(*c, # contents of Switch tag (often nothing)
    ...
    return fh.Input(*c, cls=stringify(('uk-toggle-switch uk-toggle-switch-primary min-w-9',stringify(cls))), type='checkbox', **kwargs)
...
def Upload(*c, # Contents of Upload tag button (often text)
    ...
    return Div(
        ...
        cls=stringify(('w-full js-upload', stringify(cls))),
        uk_form_custom=True)
...
def UploadZone(*c, # Contents of UploadZone tag (often text or other tags)
    ...
    return Div(
        Div(fh.Input(**input_kwargs),
            ...
            uk_form_custom=True, 
            cls='w-full'),
        cls=stringify(('js-upload uk-placeholder uk-text-center', stringify(cls))),
        **kwargs)
...
def FormLabel(*c, # contents of FormLabel tag (often text)
    ...
    return fh.Label(*c, cls=stringify(('uk-form-label',stringify(cls))), **kwargs)
...
def Label(*c, # contents of Label tag (often text)
    ...
    return fh.Label(*c, cls=stringify(('uk-label',stringify(cls))), **kwargs)
...
def GenericLabelInput(
    ...
    if isinstance(label, str): 
        label = FormLabel(label, cls=stringify(lbl_cls), fr=id)
    inp = input_fn(id=id, cls=stringify(input_cls), **kwargs)        
    if container: return container(label, inp, cls=stringify(cls))
    return Div(label, inp)

# %% ../nbs/02_franken.ipynb
def LabelInput(label:str|HtmlString, # FormLabel content (often text)
               lbl_cls='', # Additional classes for `FormLabel`
...
# %% ../nbs/02_franken.ipynb
def LabelTextArea(label:str|HtmlString, # FormLabel content (often text)
...
# %% ../nbs/02_franken.ipynb
def LabelSwitch(label:str|HtmlString, # FormLabel content (often text)
               lbl_cls='', # Additional classes for `FormLabel`
...
# %% ../nbs/02_franken.ipynb
def LabelRadio(label:str|HtmlString, # FormLabel content (often text)
               lbl_cls='', # Additional classes for `FormLabel`
    ...
    return Div(inp, label)

# %% ../nbs/02_franken.ipynb
def LabelCheckboxX(label:str|HtmlString, # FormLabel content (often text)
               lbl_cls='', # Additional classes for `FormLabel`
...
def Option(*args, **kwargs):
    return fh.CustomTag('option', *args, **kwargs)
# %% ../nbs/02_franken.ipynb
def Options(*c,                    # Content for an `Option`
            selected_idx:int|None=None, # Index location of selected `Option`
            disabled_idxs:set|None=None # Idex locations of disabled `Options`
           ):
...
# %% ../nbs/02_franken.ipynb
def LabelRange(label:str|HtmlString, # FormLabel content (often text)
               lbl_cls='', # Additional classes for `FormLabel`
...
def ModalContainer(*c, # Components to put in the modal (often `ModalDialog`)
    ...
    return Div(*c, cls=stringify(('uk-modal uk-modal-container',stringify(cls))), data_uk_modal=True, **kwargs)
def ModalDialog(*c, # Components to put in the `ModalDialog` (often `ModalBody`, `ModalHeader`, etc)
    ...
    return Div(*c, cls=stringify(('uk-modal-dialog',   stringify(cls))),                **kwargs)
def ModalHeader(*c, # Components to put in the `ModalHeader`
    ...
    return Div(*c, cls=stringify(('uk-modal-header',   stringify(cls))),                **kwargs)
def ModalBody(*c, # Components to put in the `ModalBody` (often forms, sign in buttons, images, etc.)
    ...
    return Div(*c, cls=stringify(('uk-modal-body',     stringify(cls))),                **kwargs)
def ModalFooter(*c, # Components to put in the `ModalFooter` (often buttons)
    ...
    return Div(*c, cls=stringify(('uk-modal-footer',   stringify(cls))),                **kwargs)
def ModalTitle(*c, # Components to put in the `ModalTitle` (often text)
    ...
    return fh.H2(*c,  cls=stringify(('uk-modal-title',  stringify(cls))),  **kwargs)
def ModalCloseButton(*c, # Components to put in the button (often text and/or an icon)
                      ...
                      **kwargs # Additional args for `Button` tag
                      )->HtmlString: # Button(..., cls='uk-modal-close') + `hx_target` and `hx_swap` if htmx is True
...
def Modal(*c,                 # Components to put in the `ModalBody` (often forms, sign in buttons, images, etc.)
    ...
    res = []
    if header: res.append(ModalHeader(header,cls=header_cls))
    res.append(ModalBody(*c,cls=body_cls))
    if footer: res.append(ModalFooter(footer,cls=footer_cls))
...
def Placeholder(*c, # Components to put in the placeholder
    ...
    return Div(*c, cls=stringify(('uk-placeholder',stringify(cls))), **kwargs)
...
def Progress(*c, # Components to put in the progress bar (often nothing)
    ...
    return fh.Progress(*c, value=value, max=max, cls=stringify(('uk-progress',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def UkIcon(icon:str, # Icon name from [lucide icons](https://lucide.dev/icons/)
           height:int|None=None, 
           width:int|None=None, 
           stroke_width:int|None=None, # Thickness of lines
...
def UkIconLink(icon:str,  # Icon name from [lucide icons](https://lucide.dev/icons/)
    ...
    if button: 
        return fh.Button(UkIcon(icon=icon, height=height, width=width, stroke_width=stroke_width, cls=stringify((f"uk-icon-{'button'}", stringify(cls)))))
    else:
        return fh.A(UkIcon(icon=icon, height=height, width=width, stroke_width=stroke_width, cls=stringify(("uk-icon-link", stringify(cls)))))
...
def Center(*c, # Components to center
    ...
    return Div(*c, cls=stringify((stringify(classes), stringify(cls))), **kwargs)
...
def Grid(*div, # `Div` components to put in the grid
         ...
         cols_sm:int|None=None, # Number of columns on small screens
         cols_md:int|None=None, # Number of columns on medium screens
         cols_lg:int|None=None, # Number of columns on large screens
         cols_xl:int|None=None, # Number of columns on extra large screens
         cols:int|None=None, # Number of columns on all screens
    ...
    return Div(cls=stringify((f'grid grid-cols-{cols_min} sm:grid-cols-{cols_sm} md:grid-cols-{cols_md} lg:grid-cols-{cols_lg} xl:grid-cols-{cols_xl}', stringify(cls))), **kwargs)(*div)
...
def DivFullySpaced(*c,                # Components
    ...
    cls = stringify(cls)
    return Div(cls=stringify((FlexT.block,FlexT.between,FlexT.middle,cls)), **kwargs)(*c)
...
def DivCentered(*c,      # Components
    ...
    cls=stringify(cls)
    return Div(cls=stringify((FlexT.block,(FlexT.column if vstack else FlexT.row),FlexT.middle,FlexT.center,cls)),**kwargs)(*c)
...
def DivLAligned(*c, # Components
    ...
    cls=stringify(cls)
    return Div(cls=stringify((FlexT.block,FlexT.left,FlexT.middle,cls)), **kwargs)(*c)
...
def DivRAligned(*c, # Components
    ...
    cls=stringify(cls)
    return Div(cls=stringify((FlexT.block,FlexT.right,FlexT.middle,cls)), **kwargs)(*c)
...
def DivVStacked(*c, # Components
    ...
    cls=stringify(cls)
    return Div(cls=stringify((FlexT.block,FlexT.column,FlexT.middle,cls)), **kwargs)(*c)
...
def DivHStacked(*c, # Components
    ...
    cls=stringify(cls)
    return Div(cls=stringify((FlexT.block,FlexT.row,FlexT.middle,cls)), **kwargs)(*c)
...
def NavContainer(*li, # List items are navigation elements (Special `Li` such as `NavParentLi`, `NavDividerLi`, `NavHeaderLi`, `NavSubtitle`, `NavCloseLi` can also be used)
                 cls:str=NavT.primary.__str__(), # Additional classes on the nav
    ...
    return fh.Ul(*li, uk_nav=uk_nav, cls=stringify((f"uk-nav{'' if parent else '-sub'}", stringify(cls), _sticky)), uk_scrollspy_nav=_uk_scrollspy_nav, **kwargs)
...
def NavSubtitle(*c, # Components
    ...
    return Div(*c, cls=stringify(('uk-nav-subtitle', stringify(cls))),**kwargs)
def NavCloseLi(*c, # Components
    ...
    return fh.Li(*c, cls=stringify(('uk-drop-close', stringify(cls))),**kwargs)
...
def NavBar(*c, # Component for right side of navbar (Often A tag links)
    ...
    return Div(
        ...
        DivCentered(*c, 
                    cls=stringify((stringify(mobile_cls),stringify(cls), stringify(scrollspy_cls),
...
def SliderContainer(
    ...
    return Div(*c, cls=stringify(('relative', stringify(cls))), uk_slider=uk_slider, **kwargs)
...
def SliderItems(
    ...
    return Div(*c, cls=stringify(('uk-slider-items uk-grid', stringify(cls))), **kwargs)
...
def SliderNav(
    ...
    return Div(
        fh.A(cls=stringify((prev_cls, stringify(cls))), href='',
             uk_slidenav_previous=True, uk_slider_item='previous', **kwargs),
        fh.A(cls=stringify((next_cls, stringify(cls))), href='',
...
def DropDownNavContainer(*li, # Components
    ...
    return Div(NavContainer(*li, cls=stringify(('uk-dropdown-nav',stringify(cls)))),
                            cls='uk-drop uk-dropdown',uk_dropdown=uk_dropdown, uk_nav=uk_nav, parent=parent, **kwargs)
...
def TabContainer(*li, # Components
    ...
    cls = stringify(cls)
    return Ul(cls=stringify((f"uk-tab{'-alt' if alt else ''}",stringify(cls))),**kwargs)(*li)
...
def CardTitle(*c, # Components (often a string)
    ...
    return Div(*c, cls=stringify(('uk-card-title',stringify(cls))), **kwargs)
...
def CardHeader(*c, # Components that goes in the header (often a `ModalTitle` and description)
    ...
    return Div(*c, cls=stringify(('uk-card-header',stringify(cls))), **kwargs)
...
def CardBody(*c, # Components that go in the body (Main content of the card such as a form, and image, a signin form, etc.)
    ...
    "Creates a card body"
    return Div(*c, cls=stringify(('uk-card-body',stringify(cls))), **kwargs)
...
def CardFooter(*c, # Components that go in the footer (often a `ModalCloseButton`)
    ...
    return Div(*c, cls=stringify(('uk-card-footer',stringify(cls))), **kwargs)

def CardContainer(*c, # Components (typically `CardHeader`, `CardBody`, `CardFooter`)
                   cls:CardT|str|tuple=CardT.default, # Additional classes on the div
                   **kwargs # Additional args for the div
                  )->HtmlString: # Container for a card
    "Creates a card container"
    return Div(*c, cls=stringify(('uk-card',stringify(cls))), **kwargs)

# %% ../nbs/02_franken.ipynb
def Card(*c, # Components that go in the body (Main content of the card such as a form, and image, a signin form, etc.)
        header:HtmlString|Iterable[HtmlString]|None=None, # Component(s) that goes in the header (often a `ModalTitle` and a subtitle)
        footer:HtmlString|Iterable[HtmlString]|None=None,  # Component(s) that goes in the footer (often a `ModalCloseButton`)
    ...
    res = []
    if header: res.append(CardHeader(header, cls=header_cls))
    res.append(CardBody(*c, cls=body_cls))
    if footer: res.append(CardFooter(footer, cls=footer_cls))
    return CardContainer(*res, cls=stringify(cls), **kwargs)
...
def Table(*c, # Components (typically `Thead`, `Tbody`, `Tfoot`)
          cls:TableT|str|tuple=(TableT.middle, TableT.divider, TableT.hover, TableT.sm), # Additional classes on the table
          **kwargs # Additional args for the table
         )->HtmlString: # Table component
    "Creates a table"
    return fh.Table(cls=stringify(('uk-table', stringify(cls))), *c, **kwargs)

# %% ../nbs/02_franken.ipynb
def Td(*c, # Components that go in the cell
    ...
    return fh.Td(*c,cls=cls, **kwargs)


def Th(*c, # Components that go in the cell
       cls=(), # Additional classes on the cell container
       shrink=False, # Whether to shrink the cell
       expand=False, # Whether to expand the cell
       small=False, # Whether to use a small table
...
def TableFromDicts(header_data:Sequence, # List of header data
                   ...
                   footer_data=None, # List of footer data
                   header_cell_render=Th, # Function(content) -> HtmlString that renders header cells
                   body_cell_render=lambda k,v : Td(v), # Function(key, content) -> HtmlString that renders body cells
                   footer_cell_render=lambda k,v : Td(v), #  Function(key, content) -> HtmlString that renders footer cells
                   cls=(TableT.middle, TableT.divider, TableT.hover, TableT.sm), # Additional classes on the table
                   sortable=False, # Whether to use sortable table
                   **kwargs # Additional args for the table
...
def Uk_theme_switcher(*c, **kwargs):
    "Theme switcher component"
    return fh.CustomTag('uk-theme-switcher',*c, **kwargs)
...
def ThemePicker(color=True, radii=True, shadows=True, font=True, mode=True, cls='p-4', custom_themes=[]):
    ...
    def _optgrp(key, lbl, opts): return fh.Optgroup(*opts,data_key=key, label=lbl)
...
def LightboxContainer(*lightboxitem, # `LightBoxItem`s that will be inside lightbox
    ...
    return Div(*lightboxitem, data_uk_lightbox=data_uk_lightbox, **kwargs)
...
def LightboxItem(*c, # Component that when clicked will open the lightbox (often a button)
                 ...
                 data_alt: str|None=None, # Alt text for the lightbox item/image
                 data_caption: str|None=None, # Caption for the item that shows below it
    ...
    return fh.A(*c, href=href, cls=stringify(cls), **kwargs) # , data_alt=data_alt,
