...
class fragly:
    """
    Async client for interacting with FraglyFragment API service.
        ...
        try:
            async with self.session.post(url, json=payload) as resp:
                ...
                if resp.status == 400:
                    ...
                    raise FraglyFragmentAPIError(data.get("message", "Bad request"))
                if resp.status == 404:
                    raise InvalidUsernameError("User not found.")
                resp.raise_for_status()
                return data
        except aiohttp.ClientError as e:
            raise FraglyFragmentAPIError(f"Network error: {str(e)}") from e
