...
class Structure(SarcAsM):
    """
    Class for analyzing sarcomere morphology with xarray integration.
    ...
    **info : Any
    ...
    ----------
    data : xr.Dataset
        xarray Dataset containing all structure analysis results with 
        features accessible as attributes.
    """
    ...
    def __init__(self,
        ...
        # Initialize xarray Dataset for structure data
        if self._structure_dataset_exists():
            self._load_xarray_dataset()
        else:
            self._initialize_empty_dataset()

        # Keep backward compatibility with existing data dict
        if os.path.exists(self.__get_structure_data_file()):
            self._migrate_legacy_data()

    def _structure_dataset_exists(self) -> bool:
        """Check if xarray dataset already exists."""
        return os.path.exists(self.__get_structure_dataset_path())

    def __get_structure_dataset_path(self) -> str:
        """Get path to the compressed xarray dataset file."""
        return os.path.join(self.data_dir, "structure_data.zarr")

    def _initialize_empty_dataset(self) -> None:
        """Initialize an empty xarray Dataset with proper structure."""
        # Create basic coordinates based on image metadata
        coords = {}
        
        # Add spatial coordinates if available
        if hasattr(self.metadata, 'shape_orig') and self.metadata.shape_orig:
            shape = self.metadata.shape_orig
            axes = self.metadata.axes if hasattr(self.metadata, 'axes') else 'YX'
            
            if 'Y' in axes:
                y_idx = axes.index('Y')
                coords['y'] = np.arange(shape[y_idx])
            if 'X' in axes:
                x_idx = axes.index('X')
                coords['x'] = np.arange(shape[x_idx])
            if 'T' in axes:
                t_idx = axes.index('T')
                coords['t'] = np.arange(shape[t_idx])

        # Add physical coordinates if pixelsize is available
        if hasattr(self.metadata, 'pixelsize') and self.metadata.pixelsize:
            if 'y' in coords:
                coords['y_physical'] = coords['y'] * self.metadata.pixelsize
            if 'x' in coords:
                coords['x_physical'] = coords['x'] * self.metadata.pixelsize

        # Add temporal coordinates if frametime is available
        if hasattr(self.metadata, 'frametime') and self.metadata.frametime and 't' in coords:
            coords['t_physical'] = coords['t'] * self.metadata.frametime

        # Initialize empty dataset with metadata
        attrs = {
            'pixelsize': getattr(self.metadata, 'pixelsize', None),
            'frametime': getattr(self.metadata, 'frametime', None),
            'file_path': str(self.filepath),
            'created_with': 'SarcAsM',
            'version': getattr(self.metadata, 'version', None),
        }

        self.data = xr.Dataset(coords=coords, attrs=attrs)

    def _load_xarray_dataset(self) -> None:
        """Load existing xarray dataset from compressed storage."""
        try:
            self.data = xr.open_zarr(
                self.__get_structure_dataset_path(),
                chunks="auto"
            )
        except Exception as e:
            print(f"Warning: Could not load xarray dataset: {e}")
            self._initialize_empty_dataset()

    def _migrate_legacy_data(self) -> None:
        """Migrate data from legacy JSON format to xarray format."""
        try:
            legacy_data = IOUtils.json_deserialize(self.__get_structure_data_file())
            
            # Convert legacy data to xarray format
            for key, value in legacy_data.items():
                if isinstance(value, (list, np.ndarray)):
                    # Convert arrays to DataArrays with appropriate dimensions
                    if key.endswith('_vectors') or key.endswith('_points'):
                        # These are typically coordinate arrays
                        da = xr.DataArray(
                            np.array(value),
                            dims=['point', 'coord'] if np.array(value).ndim == 2 else ['point'],
                            name=key,
                            attrs={'source': 'legacy_migration'}
                        )
                        self.data[key] = da
                    else:
                        # Other numeric data
            ...
            print("Successfully migrated legacy data to xarray format")
            
        except Exception as e:
            print(f"Warning: Could not migrate legacy data: {e}")

    def store_structure_data(self, override: bool = True) -> None:
        """
        Store structure data in compressed zarr format with OME-ZARR compatibility.

        Parameters
        ----------
        override : bool, optional
            If True, override existing files.
        """
        if override or not self._structure_dataset_exists():
            # Configure compression for efficient storage
            encoding = {}
            for var in self.data.data_vars:
                if self.data[var].dtype.kind in ['f', 'i']:  # float or int
                    encoding[var] = {
            ...
            self.data.to_zarr(
                self.__get_structure_dataset_path(),
                mode='w' if override else 'w-',
                encoding=encoding,
                consolidated=True
            )

    def add_structure_feature(self, 
                            name: str, 
                            data: np.ndarray, 
        ...
        """
        Add a new structure feature to the dataset.
        ...
        data : np.ndarray
            Feature data
        dims : list, optional
            Dimension names for the data
        coords : dict, optional
            Coordinate arrays for the dimensions
        attrs : dict, optional
            Metadata attributes for this feature
        """
    ...
    def __getattr__(self, name: str) -> Any:
        """
        Access structure features as attributes through xarray.
        
        This enables accessing features like:
        - structure.sarcomere_length
        - structure.pos_vectors
        - structure.orientation_data
        """
        # First check if it's in the xarray dataset
        if hasattr(self, 'data') and name in self.data:
            return self.data[name]
        
        # Check if it's a dataset attribute
        if hasattr(self, 'data') and name in self.data.attrs:
            return self.data.attrs[name]
            
        # Fallback to legacy data dict for backward compatibility
    ...
    def prepare_for_omezarr_append(self) -> str:
        """
        Prepare structure data for appending to OME-ZARR store.
        
        Returns
        -------
        str
            Path to the prepared data in OME-ZARR compatible format
        """
        # Create a labels group in the main OME-ZARR store
        if not hasattr(self, 'root'):
            self._bootstrap_store()
        
        # Create labels group if it doesn't exist
        if 'labels' not in self.root:
            labels_group = self.root.create_group('labels')
            labels_group.attrs['labels'] = []
        else:
            labels_group = self.root['labels']

        # Add structure data as a label
        structure_path = 'structure_analysis'
        if structure_path not in labels_group:
            # Convert xarray dataset to zarr format compatible with OME-ZARR
            structure_group = labels_group.create_group(structure_path)
            
            # Store each data variable as a separate array
            for var_name, data_array in self.data.data_vars.items():
                structure_group[var_name] = data_array.values
                structure_group[var_name].attrs.update(data_array.attrs)
            
            # Store coordinates
            for coord_name, coord_array in self.data.coords.items():
                structure_group[f'coord_{coord_name}'] = coord_array.values
            
            # Store dataset attributes
            structure_group.attrs.update(self.data.attrs)
            
            # Update labels list
            labels_list = list(labels_group.attrs.get('labels', []))
            if structure_path not in labels_list:
                labels_list.append(structure_path)
                labels_group.attrs['labels'] = labels_list

        return f"{self.ome_path}/labels/{structure_path}"

    def get_structure_summary(self) -> Dict[str, Any]:
        """
        Get a summary of all structure features in the dataset.
        
        Returns
        -------
        Dict[str, Any]
            Summary information about the structure data
        """
        summary = {
            'n_features': len(self.data.data_vars),
            'features': list(self.data.data_vars.keys()),
            'coordinates': list(self.data.coords.keys()),
            'dimensions': list(self.data.dims.keys()),
            'total_size_mb': self.data.nbytes / 1024**2,
            'attrs': dict(self.data.attrs)
        }
        
        # Add per-feature information
        feature_info = {}
        for var_name, data_array in self.data.data_vars.items():
            feature_info[var_name] = {
                ...
                'size_mb': data_array.nbytes / 1024**2
            }
        summary['feature_details'] = feature_info
        
        return summary

    # Keep existing methods for backward compatibility
    def __get_structure_data_file(self, is_temp_file: bool = False) -> str:
        """Returns the path to the legacy structure data file."""
        file_name = "structure.temp.json" if is_temp_file else "structure.json"
        return os.path.join(self.data_dir, file_name)

    def commit(self) -> None:
        """Commit data by saving xarray dataset and legacy format."""
        # Save xarray dataset
        self.store_structure_data()
        
        # Keep legacy commit for backward compatibility
    ...
    def _load_structure_data(self) -> None:
        """Load structure data - prefer xarray, fallback to legacy."""
        if self._structure_dataset_exists():
            self._load_xarray_dataset()
        else:
            # Load legacy data
