...
class PlayerConfigTag(str, Enum):
    ...
    def get_yaml_file_path(self) -> path:
        ...
        if self is PlayerConfigTag.CHIPIRON:
            path_player = files("chipiron").joinpath("data/players/player_config/chipiron/chipiron.yaml")
        else:
            path_player = files("chipiron").joinpath(f"data/players/player_config/{self.value}.yaml")
