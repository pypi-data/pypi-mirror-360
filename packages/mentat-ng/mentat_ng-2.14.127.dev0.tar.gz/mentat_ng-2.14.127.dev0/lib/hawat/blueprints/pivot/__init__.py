...
"""
This file contains pluggable module for Hawat web interface that provides pivot
...
def _get_search_form(request_args: Any = None) -> SimplePivotSearchForm:
...
class AbstractSearchView(PsycopgMixin, BaseSearchView):
    """
    Base class for all views responsible for searching and displaying pivot results.
    """
    ...
    def get_view_title(cls, **kwargs: Any) -> str | LazyString:
        return lazy_gettext("Calculate event pivot tables")
    ...
    def get_menu_title(cls, **kwargs: Any) -> str | LazyString:
        return lazy_gettext("Pivot tables")

    @staticmethod
    def get_search_form(request_args: Any) -> SimplePivotSearchForm:
...
class SearchView(HTMLMixin, AbstractSearchView):
    """
    View responsible for searching and displaying pivot table search results
    ...
    def do_after_search(self, items: PivotItems) -> None:
        self.response_context.update(
            result_data=PivotTableChartData(items, PivotTableChartConfig(column_name="Value", value_name="Event Count"))
        )
...
class PivotBlueprint(HawatBlueprint):
    """
    Blueprint for the pivot module.
    """

    @classmethod
    def get_module_title(cls) -> str | LazyString:
        return lazy_gettext("Event pivot tables")
...
def get_blueprint() -> PivotBlueprint:
    """
