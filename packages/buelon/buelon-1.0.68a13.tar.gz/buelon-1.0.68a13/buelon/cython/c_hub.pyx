...
def display_text():
    ...
    text = (f'done: {len(done):,}'
            ...
            f', steps: {steps_len:,}'
            f', holds: {holds_len:,}'
            f', total: {steps_len + holds_len + len(done) + len(queued) + len(errors):,}')
    return text
...
def hold_promise(s):
    ...
    with s:
        ...
        if method == b'get':
            ...
            step_id = data.decode('utf-8')
            if step_id in ALL_STEPS:
                send(s, ALL_STEPS[step_id][0].encode('utf-8'))
            else:
                send(s, b'not-found')
        elif method == b'job-status-bulk':
            step_ids = orjson.loads(data)  # .decode('utf-8').split(',')
            statuses = []
            for step_id in step_ids:
                if step_id in ALL_STEPS:
                    statuses.append(ALL_STEPS[step_id][0])
                else:
