...
secure_logout_bp, logger = get_bp(
    "secure_logout_bp",
    __name__,
    static_folder=STATIC_FOLDER,  # if you ever serve a logout.html, but can be unused
    url_prefix=URL_PREFIX        # blueprint sits under /secure-files
)

@secure_logout_bp.route("/api/secure-logout", methods=["POST"])
@login_required
def logout():
    initialize_call_log()
