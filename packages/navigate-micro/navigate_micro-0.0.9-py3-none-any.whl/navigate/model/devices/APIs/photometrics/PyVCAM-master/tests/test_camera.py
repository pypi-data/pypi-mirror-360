...
class CameraConstructionTests(unittest.TestCase):
    ...
    def test_get_dd_version(self):
        self.test_cam.open()
        dd_ver = pvc.get_param(
            self.test_cam.handle, const.PARAM_DD_VERSION, const.ATTR_CURRENT
        )
        dd_ver = "{}.{}.{}".format(
            dd_ver & 0xFF00 >> 8, dd_ver & 0x00F0 >> 4, dd_ver & 0x000F
        )
    ...
    def test_get_bin_x(self):
        ...
        try:
            ...
            self.skipTest("test_get_bin_x: This camera does not " "support binning.")
    ...
    def test_get_bin_y(self):
        ...
        try:
            ...
            self.skipTest("test_get_bin_y: This camera does not " "support binning.")

    @pytest.mark.hardware
    def test_get_chip_name(self):
        self.test_cam.open()
        chip_name = pvc.get_param(
            self.test_cam.handle, const.PARAM_CHIP_NAME, const.ATTR_CURRENT
        )
    ...
    def test_get_serial_no(self):
        self.test_cam.open()
        ser_no = pvc.get_param(
            self.test_cam.handle, const.PARAM_HEAD_SER_NUM_ALPHA, const.ATTR_CURRENT
        )
    ...
    def test_get_speed_table_index(self):
        self.test_cam.open()
        spdtab_index = pvc.get_param(
            self.test_cam.handle, const.PARAM_SPDTAB_INDEX, const.ATTR_CURRENT
        )
    ...
    def test_set_speed_table_index(self):
        self.test_cam.open()
        num_entries = self.test_cam.get_param(
            const.PARAM_SPDTAB_INDEX, const.ATTR_COUNT
        )
    ...
    def test_get_readout_port(self):
        self.test_cam.open()
        readout_port = pvc.get_param(
            self.test_cam.handle, const.PARAM_READOUT_PORT, const.ATTR_CURRENT
        )
    ...
    def test_set_readout_port_index(self):
        self.test_cam.open()
        num_entries = self.test_cam.get_param(
            const.PARAM_READOUT_PORT, const.ATTR_COUNT
        )
        for i in range(num_entries):
    ...
    def test_get_bit_depth(self):
        self.test_cam.open()
        curr_bit_depth = pvc.get_param(
            self.test_cam.handle, const.PARAM_BIT_DEPTH, const.ATTR_CURRENT
        )
    ...
    def test_get_pix_time(self):
        self.test_cam.open()
        curr_pix_time = pvc.get_param(
            self.test_cam.handle, const.PARAM_PIX_TIME, const.ATTR_CURRENT
        )
    ...
    def test_get_gain(self):
        self.test_cam.open()
        curr_gain_index = pvc.get_param(
            self.test_cam.handle, const.PARAM_GAIN_INDEX, const.ATTR_CURRENT
        )
    ...
    def test_set_gain(self):
        self.test_cam.open()
        max_gain = pvc.get_param(
            self.test_cam.handle, const.PARAM_GAIN_INDEX, const.ATTR_MAX
        )
    ...
    def test_adc_offset(self):
        self.test_cam.open()
        curr_adc_offset = pvc.get_param(
            self.test_cam.handle, const.PARAM_ADC_OFFSET, const.ATTR_CURRENT
        )
    ...
    def test_get_clear_mode(self):
        self.test_cam.open()
        curr_clear_mode = pvc.get_param(
            self.test_cam.handle, const.PARAM_CLEAR_MODE, const.ATTR_CURRENT
        )
    ...
    def test_set_clear_mode_bad_name_fail(self):
        self.test_cam.open()
        self.assertRaises(ValueError, setattr, self.test_cam, "clear_mode", "")

    @pytest.mark.hardware
    def test_set_clear_mode_bad_value_fail(self):
        self.test_cam.open()
        self.assertRaises(ValueError, setattr, self.test_cam, "clear_mode", -1)

    @pytest.mark.hardware
    def test_set_clear_mode_no_open_fail(self):
        self.assertRaises(AttributeError, setattr, self.test_cam, "clear_mode", 0)

    # TODO: All setters should raise Runtime if not open
    @pytest.mark.hardware
    def test_set_clear_mode_no_open_bad_value_fail(self):
        self.assertRaises(AttributeError, setattr, self.test_cam, "clear_mode", -1)

    @pytest.mark.hardware
    def test_get_temp(self):
        self.test_cam.open()
        curr_temp = pvc.get_param(
            self.test_cam.handle, const.PARAM_TEMP, const.ATTR_CURRENT
        )
    ...
    def test_get_temp_setpoint(self):
        self.test_cam.open()
        curr_temp_setpoint = pvc.get_param(
            self.test_cam.handle, const.PARAM_TEMP_SETPOINT, const.ATTR_CURRENT
        )
    ...
    def test_set_temp_setpoint(self):
        self.test_cam.open()
        min_temp = self.test_cam.get_param(const.PARAM_TEMP_SETPOINT, const.ATTR_MIN)
        max_temp = self.test_cam.get_param(const.PARAM_TEMP_SETPOINT, const.ATTR_MAX)
    ...
    def test_set_temp_to_high_fail(self):
        self.test_cam.open()
        one_over_max_temp = (
            self.test_cam.get_param(const.PARAM_TEMP_SETPOINT, const.ATTR_MAX) + 1
        )
        self.assertRaises(
            ValueError, setattr, self.test_cam, "temp_setpoint", one_over_max_temp
        )
    ...
    def test_set_temp_to_low_fail(self):
        self.test_cam.open()
        one_below_max_temp = (
            self.test_cam.get_param(const.PARAM_TEMP_SETPOINT, const.ATTR_MIN) - 1
        )
        self.assertRaises(
            ValueError, setattr, self.test_cam, "temp_setpoint", one_below_max_temp
        )

    @pytest.mark.hardware
    def test_set_temp_no_open_fail(self):
        self.assertRaises(RuntimeError, setattr, self.test_cam, "temp_setpoint", 0)

    @pytest.mark.hardware
    def test_get_exp_res(self):
        self.test_cam.open()
        curr_exp_res_index = pvc.get_param(
            self.test_cam.handle, const.PARAM_EXP_RES_INDEX, const.ATTR_CURRENT
        )
    ...
    def test_set_exp_res_by_name(self):
        ...
        for i in range(self.test_cam.exp_res_index):
            ...
            curr_exp_res = pvc.get_param(
                self.test_cam.handle, const.PARAM_EXP_RES, const.ATTR_CURRENT
            )
    ...
    def test_set_exp_res_bad_name_fail(self):
        self.test_cam.open()
        self.assertRaises(ValueError, setattr, self.test_cam, "exp_res", "")

    @pytest.mark.hardware
    def test_set_exp_res_bad_value_fail(self):
        self.test_cam.open()
        self.assertRaises(ValueError, setattr, self.test_cam, "exp_res", -1)

    @pytest.mark.hardware
    def test_set_exp_res_no_open_fail(self):
        self.assertRaises(AttributeError, setattr, self.test_cam, "exp_res", 0)

    @pytest.mark.hardware
    def test_get_sensor_size(self):
        self.test_cam.open()
        rows = pvc.get_param(
            self.test_cam.handle, const.PARAM_SER_SIZE, const.ATTR_CURRENT
        )
        cols = pvc.get_param(
            self.test_cam.handle, const.PARAM_PAR_SIZE, const.ATTR_CURRENT
        )
    ...
    def test_get_exp_mode(self):
        self.test_cam.open()
        curr_exp_mode = pvc.get_param(
            self.test_cam.handle, const.PARAM_EXPOSURE_MODE, const.ATTR_CURRENT
        )
