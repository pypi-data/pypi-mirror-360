...
class AcousticField(ABC):
    ...
    def _downsample_field(self):
        """
        Downsample the acoustic field to reduce its size for faster processing.
        This method is useful for large datasets where full resolution is not necessary.
        """
        try:
            if self.field is None:
                raise ValueError("Acoustic field is not generated. Please generate the field first.")

            factor = ceil(self.params['f_AQ'] / self.params['f_saving'])
            if factor < 1:
                raise ValueError("Downsampling factor must be greater than or equal to 1.")
            # Downsample the field by taking every second element along each axis
            downsampled_field = self.field[::factor, ::factor, ::factor]

            # Update the field attribute with the downsampled field
            self.field = downsampled_field

            print(f"Field downsampled to shape: {self.field.shape}")
        except Exception as e:
            print(f"Error in _downsample_field method: {e}")
            raise
    ...
    def reshape_field(field,factor):
        ...
        try:
            if field is None:
                raise ValueError("Acoustic field is not generated. Please generate the field first.")
            if factor.dim == Dim.D2.value:
                return field[::factor[0], ::factor[1]]
            elif factor.dim == Dim.D3.value:
                return field[::factor[0], ::factor[1], ::factor[2]]
            else:
                raise ValueError("Invalid dimension for downsampling. Supported dimensions are: 2D, 3D.")
        except Exception as e:
            print(f"Error in _downsample_field method: {e}")
            raise
...
class StructuredWave(AcousticField):
    ...
    def _generate_2Dacoustic_field_KWAVE(self, isGPU=True if config.get_process() == 'gpu' else False, show_log=True):
        ...
        try:
            ...
            dx = self.params['dx']
            if dx < 2 * element_width_meters:
                dx = 2 * element_width_meters  # Ensure dx is at least twice the element width
                dz = dx # Assuming square grid spacing for simplicity
            ...
            data = sensor_data['p'].reshape(self.kgrid.Nt, Nz, Nx)
