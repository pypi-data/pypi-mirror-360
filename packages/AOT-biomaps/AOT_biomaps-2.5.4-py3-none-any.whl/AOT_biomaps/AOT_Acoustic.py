...
class StructuredWave(AcousticField):
    ...
    def _generate_2Dacoustic_field_KWAVE(self, isGPU=True if config.get_process() == 'gpu' else False, show_log=True):
        ...
        try:
            ...
            total_size_x = StructuredWave.next_power_of_2(self.params['Nx'])  # Assuming initial PML size is 20
            total_size_z = StructuredWave.next_power_of_2(self.params['Nz'])

            # Calculate the required PML size
            pml_x_size = (total_size_x - self.params['Nx'])//2
            pml_z_size = (total_size_z - self.params['Nz'])//2
