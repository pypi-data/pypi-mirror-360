...
class BowCar:
    ...
    def upload_firmware(self):
        ...
        try:
            # 1. 패키지 내의 .ino 파일을 가리키는 'Traversable' 객체를 가져옵니다.
            traversable_path = importlib.resources.files("bowcar.firmware").joinpath(
                f"bowCarForPython_V{firmware_version}",
                f"bowCarForPython_V{firmware_version}.ino"
            )

            # 2. 'as_file'을 사용해 'Traversable' 객체를 실제 파일 경로('concrete_path')로 변환합니다.
            with importlib.resources.as_file(traversable_path) as concrete_path:
                
                # 3. 임시 디렉토리를 만들고 그 안에 파일을 복사합니다.
                with tempfile.TemporaryDirectory() as temp_dir:
                    # 이제 shutil.copy는 일반 경로인 concrete_path를 이해할 수 있습니다.
                    shutil.copy(concrete_path, temp_dir)
                    
                    temp_sketch_path = Path(temp_dir) / concrete_path.name

                    # 4. 'arduino-cli' 명령어를 임시 경로로 실행합니다.
                    command = [
                        'arduino-cli', 'compile', '--upload', 
                        '--port', str(self.port),
                        '--fqbn', 'arduino:avr:uno',
                        str(temp_sketch_path)
                    ]
