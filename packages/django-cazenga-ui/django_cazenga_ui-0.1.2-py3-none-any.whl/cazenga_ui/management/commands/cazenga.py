        ...
        if self._has_tailwind_app():
            ...
            try:
                ...
                process = subprocess.run([
                    sys.executable, 'manage.py', 'tailwind', 'init', self.TAILWIND_APP_NAME
                ], cwd=base_dir, capture_output=True, text=True, timeout=60)
                
                if process.returncode != 0:
                    self.stdout.write(f'  ❌ Erro ao criar app {self.TAILWIND_APP_NAME}: {process.stderr}')
                    
                    # Se o erro indica que o comando tailwind não foi encontrado
                    if 'Unknown command' in process.stderr or 'tailwind' in process.stderr:
                        self.stdout.write('')
                ...
                self.stdout.write(f'  ❌ Timeout ao criar app {self.TAILWIND_APP_NAME}')
                return False
