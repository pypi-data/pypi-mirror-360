    ...
    def _setup_tailwind(self, skip_npm: bool = False) -> bool:
        ...
        if self._has_tailwind_app():
            ...
            try:
                ...
                try:
                    # Debug: mostrar o comando exato
                    cmd = [sys.executable, 'manage.py', 'tailwind', 'init']
                    self.stdout.write(f'  🔍 Debug: Executando comando: {" ".join(cmd)}')
                    self.stdout.write(f'  🔍 Debug: Diretório: {base_dir}')
                    self.stdout.write(f'  🔍 Debug: Input a ser enviado: "{self.TAILWIND_APP_NAME}"')
                    ...
                    self.stdout.write(f'  📤 Enviando input: {self.TAILWIND_APP_NAME}')
                    
                    # Tentar com timeout mais curto
                    try:
                    ...
                    # Debug: mostrar saídas
                    self.stdout.write(f'  🔍 Debug: Return code: {process.returncode}')
                    self.stdout.write(f'  🔍 Debug: STDOUT: {repr(stdout)}')
                    self.stdout.write(f'  🔍 Debug: STDERR: {repr(stderr)}')
                    ...
                    # Fallback para verificação manual já implementado acima
                    return False
