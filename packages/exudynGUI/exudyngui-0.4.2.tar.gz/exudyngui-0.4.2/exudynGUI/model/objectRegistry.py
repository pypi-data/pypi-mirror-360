...
def categorize(objType):
    ...
    # fallback
    debugLog(f"[categorize] ⚠️ Unknown category for '{objType}', defaulting to 'Other'", origin="objectRegistry.py")
    return "Other"
    ...
    for entry in dataList:
        if isinstance(entry, dict) and 'name' in entry and 'args' in entry:
            try:
                ...
                gd = convertRecursively(gd)  # 👈 ensures no np.float64 remains
                out.append(gd)
            except Exception as e:
                debugLog(f"[evaluateGraphicsDataList] ❌ Failed to evaluate {entry['name']}({entry['args']}): {e}")
    return out
...
def evaluateGraphicsEntry(entry):
    ...
    try:
        ...
        debugLog(f"[evaluateGraphicsEntry] ❌ Failed to evaluate entry: {entry} → {e}", origin="objectRegistry.py")
        return None
...
def genericBuilder(mbs, item):
    ...
    if isinstance(objType, str):
        ...
        if objType != original_objType:
            debugLog(f"[genericBuilder] ⚠️ Typo detected: '{original_objType}' corrected to '{objType}'", origin="objectRegistry.py")
    ...
    # ✅ SENSOR FIXES: Apply sensor-specific fixes IMMEDIATELY after kwargs construction
    if objType.startswith('Sensor'):
        debugLog(f"[genericBuilder] 🔧 Applying early sensor fixes for {objType}", origin="objectRegistry.py")
        
        # Fix 1: Ensure fileName is always a string (Exudyn requirement)
        if 'fileName' in kwargs and kwargs['fileName'] is None:
            kwargs['fileName'] = ""
            debugLog(f"[genericBuilder] 🔧 Fixed sensor fileName: None -> empty string", origin="objectRegistry.py")
        ...
        for field in gui_only_fields:
            if field in kwargs:
                del kwargs[field]
                debugLog(f"[genericBuilder] 🔧 Removed GUI-only field '{field}' for sensor", origin="objectRegistry.py")
        ...
        if kwargs.get('writeToFile', False) and (not kwargs.get('fileName') or kwargs.get('fileName').strip() == ''):
            ...
            debugLog(f"[genericBuilder] 🔧 Auto-generated fileName for sensor: {auto_filename}", origin="objectRegistry.py")
    ...
    # ───────────── If there is a "Connection" key, strip/translate here ─────────────
    #
    ...
    if conn is not None:
        if conn == "bodyNumbers":
            ...
            kwargs.pop("bodyNumbers",      None)
            kwargs.pop("bodyOrNodeList",   None)
            # Translate "bodyList" → "bodyNumbers"
        ...
        # Finally, drop the "Connection" key itself—it's not a real parameter for the C++ call
    ...
    if not success:
        debugLog(f"[genericBuilder] ❌ Validation failed for {objType}: missing={missingFields}, failed={failedFields}", origin="objectRegistry.py")
    ...
    debugLog(f"[genericBuilder] 🏷 Requested name: {data['name']} for {data['objectType']}", origin="objectRegistry.py")    # Only rename if this name was already added
    if isNameUsed(mbs, data['name']):
        ...
        debugLog(f"[genericBuilder] 🛑 Duplicate name '{data['name']}' detected, renaming to '{uniqueName}'", origin="objectRegistry.py")
    ...
    for k in list(kwargs):
        ...
        if hasattr(v, '__class__') and 'exu' in str(v.__class__):
            debugLog(f"[genericBuilder] ⚠️ Removing internal Exudyn object from field '{k}'", origin="objectRegistry.py")
            del kwargs[k]
    ...
    if objType.startswith("Create") and hasattr(mbs, objType):
        ...
        # ✅ CRITICAL FIX: Convert nodeType from string to actual enum value
        if 'nodeType' in kwargs and isinstance(kwargs['nodeType'], str):
            ...
            try:
                ...
                debugLog(f"[genericBuilder] ✅ Converted nodeType '{nodeType_str}' → {kwargs['nodeType']}", origin="objectRegistry.py")
            except AttributeError as e:
                debugLog(f"[genericBuilder] ❌ Invalid nodeType '{nodeType_str}': {e}", origin="objectRegistry.py")
        ...
        if not any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
            for key in list(kwargs.keys()):
                if key not in allowedParams:
                    debugLog(f"[genericBuilder] ⚠️ Stripping unsupported parameter '{key}' for {objType}", origin="objectRegistry.py")
                    kwargs.pop(key)
            ...
            if 'inertia' in kwargs and isinstance(kwargs['inertia'], str):
                ...
                try:
                    ...
                    debugLog(f"[genericBuilder] ✅ inertia = {kwargs['inertia']}", origin="objectRegistry.py")
                except Exception as e:
                    debugLog(f"[genericBuilder] ❌ Could not eval inertia: {e}", origin="objectRegistry.py")
            ...
            if 'graphicsDataList' in allowedParams:
                ...
                debugLog(f"[genericBuilder] ⚠️ Removed graphicsDataList (not in allowed params)", origin="objectRegistry.py")
        ...
        if 'returnDict' not in allowedParams:
            ...
            debugLog(f"[genericBuilder] ⚠️ Removed returnDict (not in allowed params)", origin="objectRegistry.py")
    ...
    try:
        ...
        debugLog(f"[genericBuilder] 📥 INPUT kwargs for {objType}:\n{pformat(kwargsForDebug)}", origin="objectRegistry.py")
        ...
        if not allowedParams:
            try:
                ...
                debugLog(f"[genericBuilder] ℹ️ Fallback to inspect.signature() for {objType}", origin="objectRegistry.py")
            except Exception as e:
                debugLog(f"[genericBuilder] ⚠️ Could not inspect signature for {objType}: {e}", origin="objectRegistry.py")
                allowedParams = set()
                
        for key in list(kwargs.keys()):
            if key not in allowedParams:
                debugLog(f"[genericBuilder] ⚠️ Removing unsupported param '{key}'", origin="objectRegistry.py")
                del kwargs[key]
        ...
        if 'graphicsDataUserFunction' in kwargs:
            ...
            if val in ['None', '', None]:
                kwargs['graphicsDataUserFunction'] = 0
                debugLog(f"[genericBuilder] 🧹 Cleaned graphicsDataUserFunction to 0", origin="objectRegistry.py")
        ...
        for matKey in ('referenceRotationMatrix', 'initialRotationMatrix'):
            if matKey in kwargs and isinstance(kwargs[matKey], list):
                try:
                    ...
                    debugLog(f"[genericBuilder] ⚠️ Could not convert {matKey} to np.array: {e}", origin="objectRegistry.py")
        
        
        # ─── unwrap inertia dict into a real RigidBodyInertia ───
        if "inertia" in kwargs and isinstance(kwargs["inertia"], dict):
            ...
            try:
                ...
                debugLog(f"[genericBuilder] ❌ Could not build inertia '{iname}({iargs})': {e}", origin="objectRegistry.py")
                kwargs["inertia"] = None
        ...
        try:            # Build a mapping from symbolic variable names to actual integer values
                ...
                if return_vals and obj_name:
                    for key, value in return_vals.items():
                        if isinstance(value, int):
                            ...
                            symbol_to_index[symbol_name] = value            # 1) Evaluate any single-string fields "m" → number, etc.
            ...
            for key, val in list(kwargs.items()):
                if isinstance(val, str):
                    ...
                    if val in symbol_to_index:
                        ...
                        debugLog(f"[genericBuilder] ✅ Resolved symbolic variable '{val}' → {symbol_to_index[val]} (field: {key})", origin="objectRegistry.py")
                    else:
                ...
                if isinstance(val, list):
                    ...
                    for list_item in val:  # Changed from 'item' to 'list_item' to avoid shadowing
                        if isinstance(list_item, str) and list_item in symbol_to_index:
                            ...
                            debugLog(f"[genericBuilder] ✅ Resolved symbolic variable in list '{list_item}' → {symbol_to_index[list_item]} (field: {key})", origin="objectRegistry.py")
                        else:
            ...
            debugLog(f"[genericBuilder] ℹ️ Skipping resolveSymbolicExpressions for object execution", origin="objectRegistry.py")
        except:
            pass
        ...
        if objType.startswith("Create") and hasattr(mbs, objType):
            ...
            # —————————————— Delete "distance" if it is None ——————————————
            if objType == "CreateDistanceConstraint":
                if "distance" in kwargs and kwargs["distance"] is None:
                    del kwargs["distance"]
            # ——————————————————————————————————————————————————————————————            
            
            debugLog(f"[genericBuilder] 🔧 Calling {objType} with kwargs: {kwargs}", origin="objectRegistry.py")
            result = method(**kwargs)
            debugLog(f"[genericBuilder] 📋 Raw result from {objType}: {result} (type: {type(result)})", origin="objectRegistry.py")
            ...
            if not isinstance(result, dict):
                ...
                # ✅ CRITICAL FIX: Use the correct return value key based on the Create function type
                ...
                debugLog(f"[genericBuilder] 🔄 Converted result to: {result} for {objType}", origin="objectRegistry.py")
            ...
            # ✅ CRITICAL FIX: Ensure all returnValues are integers, not strings or Exudyn index objects
            fixed_returnValues = {}
            ...
            debugLog(f"[objectRegistry] 📝 Stored actual returnValues in GUI (fixed to integers): {fixed_returnValues}", origin="objectRegistry.py")
            ...
            if objIndex is not None:
                try:
                    if objIndex == 0:
                        debugLog(f"[genericBuilder] ⚠️ objIndex=0 (special ground or invalid), skipping GetObject call", origin="objectRegistry.py")
                    else:
                        ...
                        if preferredKey == 'loadNumber':
                            ...
                            debugLog(f"[genericBuilder] ✅ Confirmed load exists: ID {objIndex}", origin="objectRegistry.py")
                        elif preferredKey == 'nodeNumber':
                            # For nodes, use GetNode instead of GetObject
                            mbs.GetNode(objIndex)
                            debugLog(f"[genericBuilder] ✅ Confirmed node exists: ID {objIndex}", origin="objectRegistry.py")
                        elif preferredKey == 'markerNumber':
                            # For markers, use GetMarker instead of GetObject
                            mbs.GetMarker(objIndex)
                            debugLog(f"[genericBuilder] ✅ Confirmed marker exists: ID {objIndex}", origin="objectRegistry.py")
                        elif preferredKey == 'sensorNumber':
                            # For sensors, use GetSensor instead of GetObject
                            mbs.GetSensor(objIndex)
                            debugLog(f"[genericBuilder] ✅ Confirmed sensor exists: ID {objIndex}", origin="objectRegistry.py")
                        else:
                            # For objects and bodies, use GetObject
                            mbs.GetObject(objIndex)
                            debugLog(f"[genericBuilder] ✅ Confirmed object exists: ID {objIndex}", origin="objectRegistry.py")
                except Exception as e:
                    debugLog(f"[genericBuilder] ⚠️ Entity with ID {objIndex} (type: {preferredKey}) not accessible: {e}", origin="objectRegistry.py")
        
            debugLog(f"[genericBuilder] ✅ Result registered: objIndex={objIndex}, returnValues={data.get('returnValues')}", origin="objectRegistry.py")
            return result
    
    except Exception as e:
        debugLog(f"[genericBuilder] ❌ Exception while calling {objType}: {e}", origin="objectRegistry.py")
        # --- Prevent legacy fallback for Create* types ---
        if isinstance(objType, str) and objType.startswith("Create"):
            raise RuntimeError(f"[genericBuilder] ❌ Could not create object of type '{objType}'. This is a Create* type and cannot be handled by the legacy builder. Error: {e}")
    ...
    try:  # === LEGACY BUILDER ===
        ...
        import re
        debugLog(f"[genericBuilder] 🌀 Entering fallback legacy builder for {objType}", origin="objectRegistry.py")
    
        match = re.match(r"^(Object|Node|Marker|Load|Sensor)(.+)", objType)
        if not match:
            raise ValueError(f"[genericBuilder] ❌ Unrecognized legacy objectType format: {objType}")
        ...
        if not addMethod or not getDefaults:
            raise ValueError(f"[genericBuilder] ❌ Add or GetDefaults method not found for prefix: {prefix}")
    
        # Clean optionals
        kwargs = normalizeKnownOptionals(kwargs)        # Filter unsupported fields
        try:
            defaultFields = getDefaults(shortType)
            debugLog(f"[legacyBuilder] 🔍 Default fields for {shortType}: {list(defaultFields.keys())}", origin="objectRegistry.py")
            for k in list(kwargs.keys()):
                if k not in defaultFields:
                    debugLog(f"[legacyBuilder] ⚠️ Removing unsupported param '{k}' for {prefix}{shortType}", origin="objectRegistry.py")
                    del kwargs[k]
        except Exception as e:
            debugLog(f"[legacyBuilder] ⚠️ Could not get default fields for {shortType}: {e}", origin="objectRegistry.py")
    
        # ✅ SENSOR FIXES: Apply sensor-specific fixes in legacy builder
        if prefix == 'Sensor':
            debugLog(f"[legacyBuilder] 🔧 Applying sensor fixes for {prefix}{shortType}", origin="objectRegistry.py")
            
            # Fix 1: Ensure fileName is always a string (Exudyn requirement)
            if 'fileName' in kwargs and kwargs['fileName'] is None:
                kwargs['fileName'] = ""
                debugLog(f"[legacyBuilder] 🔧 Fixed sensor fileName: None -> empty string", origin="objectRegistry.py")
            ...
            for field in gui_only_fields:
                if field in kwargs:
                    del kwargs[field]
                    debugLog(f"[legacyBuilder] 🔧 Removed GUI-only field '{field}' for sensor", origin="objectRegistry.py")
            ...
            if kwargs.get('writeToFile', False) and (not kwargs.get('fileName') or kwargs.get('fileName').strip() == ''):
                ...
                auto_filename = f"{sensor_name}_{output_type}.txt"
                kwargs['fileName'] = auto_filename
                debugLog(f"[legacyBuilder] 🔧 Auto-generated fileName for sensor: {auto_filename}", origin="objectRegistry.py")
        ...
        if isinstance(val, list) and val:
            ...
            debugLog(f"[legacyBuilder] 🧩 Evaluated graphicsData: {deepSummarize(gdList)}", origin="objectRegistry.py")
        ...
        # ✅ Set the correct type field based on prefix
        if prefix == 'Node':
        ...
        # ✅ Ensure name field is preserved from original data if not in kwargs
        if 'name' not in objDict or objDict['name'] is None:
            if 'name' in data and data['name'] is not None:
                objDict['name'] = data['name']
                debugLog(f"[legacyBuilder] 🏷️ Preserved name from data: '{data['name']}'", origin="objectRegistry.py")
        ...
        if 'graphicsData' in visDict and visDict['graphicsData']:
            ...
            if not isinstance(gd, dict) or 'type' not in gd:
                raise AssertionError(f"❌ Invalid graphicsData format: {gd}")
        
        # 🧼 Patch: convert graphicsDataUserFunction = 0 → None
        vis = objDict["visualization"]
        if vis.get("graphicsDataUserFunction", 0) == 0:
            vis["graphicsDataUserFunction"] = None
            debugLog(f"[legacyBuilder] 🧼 Patched graphicsDataUserFunction=0 → None", origin="objectRegistry.py")
          # ✅ Rebuild evaluated graphicsData from metadata
        if 'VgraphicsData' in data and data['VgraphicsData'] is not None:
            ...
            debugLog(f"[legacyBuilder] 🔁 Evaluated VgraphicsData → {len(graphicsData)} items", origin="objectRegistry.py")
          # ✅ Ensure show=True if graphics present but no explicit show flag
        ...
        if 'VgraphicsData' not in objDict and 'visualization' in objDict:
            ...
            if isinstance(vis.get('graphicsData'), list):
                ...
                debugLog(f"[legacyBuilder] 📦 Patched VgraphicsData = visualization['graphicsData']", origin="objectRegistry.py")
        ...
        if summary_dict:
            data['summaryPythonCode'] = summary_dict# ✅ Filter out GUI-only fields before calling addMethod
        GUI_ONLY_FIELDS = {'summaryPythonCode', 'isLegacy'}
        
        # ✅ Keep required type fields for Exudyn API calls
        if prefix != 'Object':
        ...
        # ✅ Ensure nodeType is a string for AddNode (remove GUI prefixes)
        if 'nodeType' in filtered_objDict and prefix == 'Node':
            ...
            if isinstance(nodeType_value, str):
                ...
                debugLog(f"[legacyBuilder] ✅ Using nodeType as string: '{filtered_objDict['nodeType']}'", origin="objectRegistry.py")
          # ✅ Ensure objectType is a string for AddObject (remove GUI prefixes)
        if 'objectType' in filtered_objDict and prefix == 'Object':
            ...
            if isinstance(objectType_value, str):
                ...
                debugLog(f"[legacyBuilder] ✅ Using objectType as string: '{filtered_objDict['objectType']}'", origin="objectRegistry.py")
        
        # ✅ Ensure markerType is a string for AddMarker (remove GUI prefixes)
        if 'markerType' in filtered_objDict and prefix == 'Marker':
            ...
            if isinstance(markerType_value, str):
                ...
                debugLog(f"[legacyBuilder] ✅ Using markerType as string: '{filtered_objDict['markerType']}'", origin="objectRegistry.py")
        
        # ✅ Ensure loadType is a string for AddLoad (remove GUI prefixes)
        if 'loadType' in filtered_objDict and prefix == 'Load':
            ...
            if isinstance(loadType_value, str):
                ...
                debugLog(f"[legacyBuilder] ✅ Using loadType as string: '{filtered_objDict['loadType']}'", origin="objectRegistry.py")
        
        # ✅ Ensure sensorType is a string for AddSensor (remove GUI prefixes)
        if 'sensorType' in filtered_objDict and prefix == 'Sensor':
            ...
            if isinstance(sensorType_value, str):
                ...
                debugLog(f"[legacyBuilder] ✅ Using sensorType as string: '{filtered_objDict['sensorType']}'", origin="objectRegistry.py")

        # ✅ Patch: convert outputVariableType string to enum for AddSensor
        if prefix == 'Sensor' and 'outputVariableType' in filtered_objDict:
            ...
            if isinstance(ovt, str):
                try:
                    ...
                    debugLog(f"[legacyBuilder] ✅ Converted outputVariableType string '{ovt}' to enum", origin="objectRegistry.py")
                except Exception as e:
                    debugLog(f"[legacyBuilder] ❌ Could not convert outputVariableType '{ovt}' to enum: {e}", origin="objectRegistry.py")

        # 🔍 Show full mbs.AddObject(...) line for debugging
        debugLog("[legacyBuilder] 🧾 Final creation line:\n"
                 + f"mbs.{addMethod.__name__}(\n{pformat(deepSummarize(filtered_objDict), indent=4)})",
                 origin="objectRegistry.py")
        
        for g in filtered_objDict['visualization'].get("graphicsData", []):            debugLog("✔ graphics entry type:", type(g), "→", str(g)[:100])
        
        debugLog("📦 [DEBUG] Final objDict passed to mbs.Add*():")
        debugLog(pformat(deepSummarize(filtered_objDict)))
        
        # ✅ DEBUG: Always log sensor processing
        if 'Sensor' in str(objType) or prefix == 'Sensor':
            debugLog(f"[objectRegistry] 🚨 SENSOR DEBUG: objType={objType}, prefix={prefix}", origin="objectRegistry.py")
            debugLog(f"[objectRegistry] 🚨 SENSOR DEBUG: filtered_objDict={filtered_objDict}", origin="objectRegistry.py")
        
        # ✅ General fix: Ensure fileName is always a string for sensors (Exudyn requirement)
        if prefix == 'Sensor' and 'fileName' in filtered_objDict:
            if filtered_objDict['fileName'] is None:
                filtered_objDict['fileName'] = ""
                debugLog(f"[objectRegistry] 🔧 Fixed sensor fileName: None -> empty string", origin="objectRegistry.py")
        
        # ✅ Validate sensor-specific logic: writeToFile requires a valid fileName
        debugLog(f"[objectRegistry] 🔍 About to check sensor validation - prefix={prefix}", origin="objectRegistry.py")
        debugLog(f"[objectRegistry] 🔍 filtered_objDict keys: {list(filtered_objDict.keys())}", origin="objectRegistry.py")
        debugLog(f"[objectRegistry] 🔍 writeToFile in filtered_objDict: {'writeToFile' in filtered_objDict}", origin="objectRegistry.py")
        debugLog(f"[objectRegistry] 🔍 fileName in filtered_objDict: {'fileName' in filtered_objDict}", origin="objectRegistry.py")
        ...
        if prefix == 'Sensor' and 'writeToFile' in filtered_objDict and 'fileName' in filtered_objDict:
            ...
            debugLog(f"[objectRegistry] 🔍 Sensor validation: writeToFile={write_to_file}, fileName={repr(file_name)}", origin="objectRegistry.py")
            
            # Fix: Ensure fileName is always a string for Exudyn compatibility
            if file_name is None:
                filtered_objDict['fileName'] = ""
                file_name = ""
                debugLog(f"[objectRegistry] 🔧 Fixed fileName None -> empty string", origin="objectRegistry.py")
            ...
            if write_to_file and (file_name == '' or str(file_name).strip() == ''):
                ...
                debugLog(f"[objectRegistry] 🔧 Auto-generated fileName for sensor: {auto_filename}", origin="objectRegistry.py")
                ...
                debugLog(f"[objectRegistry] 🔧 Set fileName to empty string since writeToFile=False", origin="objectRegistry.py")
          # ✅ Actually add object
        ...
        # ✅ CRITICAL FIX: Ensure returnValues are always integers, not strings or Exudyn index objects
        ...
        debugLog(f"[legacyBuilder] 🔢 FIXED: Converted returnValues to integers: {data['returnValues']}", origin="objectRegistry.py")
        # Don't add return values to data - they should only be in returnValues!
        
        debugLog(f"[legacyBuilder] ✅ Created {prefix}{shortType} with ID {objIndex}", origin="objectRegistry.py")
        debugLog(f"[genericBuilder] 📦 Final stored data:\n{pformat(deepSummarize(data))}", origin="objectRegistry.py")
        return data

    
    except Exception as e:
        debugLog(f"[legacyBuilder] ❌ Exception during legacy add: {repr(e)}", origin="objectRegistry.py")
        raise
...
# ─── Return key → component group mapping ─────────────────────────────────────
RETURN_KEY_TO_GROUP = {
    'nodeNumber': 'nodes',
    'nodeNumbers': 'nodes',
...
def returnKeyToGroup(returnKey):
    ...
    if not group:
        debugLog(f"[returnKeyToGroup] ⚠️ Unknown return key: {returnKey}", origin="objectRegistry.py")
    return group
