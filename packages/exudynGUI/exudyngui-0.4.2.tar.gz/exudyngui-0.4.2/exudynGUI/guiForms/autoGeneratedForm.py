...
from exudynGUI.model.modelData import existingNames  # ✅ this was missing
...
def _convertLatexLikeToHtml(text: str) -> str:
    """
    Convert simple LaTeX‐style '$…$' math into minimal HTML, always adding
    ...
    def _replace_math(match: re.Match) -> str:
        ...
        # 4) Drop any remaining backslashes (e.g. '\vw' → 'vw')
        inner = re.sub(r"\\([A-Za-z]+)", r"\1", inner)

        # 5) Wrap in <i>…</i> and add a trailing space (&nbsp; or regular space)
        #    Using &nbsp; prevents HTML from collapsing it away.
        return f"<i>{inner}</i>&nbsp;"

    # Replace all $…$ blocks with our italicized, subscripted HTML + a space
...
class AutoGeneratedForm(QDialog):
    def __init__(self, objectType: str, existingData=None, parent=None, structure=None, SC=None):
        ...
        debugLog(f"🚨 [AutoGeneratedForm.__init__] URGENT DEBUG: existingNames = {self.existingNames}")
        debugLog(f"🚨 [AutoGeneratedForm.__init__] URGENT DEBUG: parent type = {type(parent)}")
        debugLog(f"🚨 [AutoGeneratedForm.__init__] URGENT DEBUG: hasattr existingNames = {hasattr(parent, 'existingNames') if parent else 'No parent'}")
        ...
        self.SC = SC
        if SC:
            debugLog(f"✅ [AutoGeneratedForm.__init__] SC provided directly: {type(SC)}", origin="autoGeneratedForm.py")
        else:
            debugLog(f"⚠️ [AutoGeneratedForm.__init__] No SC provided directly", origin="autoGeneratedForm.py")


        # Preload PDF bookmarks once (for the entire form)
        # 1) Store the absolute PDF path
        self.pdfPath = str(theDocPdfPath)  # e.g. "C:/…/mainExudynGUI_V0.03/exudynGUI/theDocHelper/theDoc.pdf"
        ...
        # 🚀 Make this dialog NON-MODAL so users can interact with OpenGL window simultaneously
        self.setModal(False)
        self.setWindowFlags(self.windowFlags() | Qt.WindowStaysOnTopHint)
        debugLog(f"✅ [AutoGeneratedForm] Set as non-modal dialog for simultaneous OpenGL interaction", origin="autoGeneratedForm.py")
        ...
        self.exclusiveFieldWidgets = {}  # mapping of group → { fieldName: widget }
        ...
        try:
            self.fieldMetadata = builder.build(objectType)
              # 🔧 Filter out GUI-only metadata fields for legacy items (Object*, Node*, Marker*, etc.)
            ...
            if (objectType.startswith(('Object', 'Node', 'Marker', 'Load', 'Sensor')) and 
                ...
                for field in gui_only_fields:
                    if field in self.fieldMetadata:
                        debugLog(f"🔧 [AutoGeneratedForm] Filtering out '{field}' field for legacy item: {objectType}")
                        del self.fieldMetadata[field]
                        filtered_count += 1
                
                if filtered_count > 0:
                    debugLog(f"🔧 [AutoGeneratedForm] Filtered {filtered_count} GUI-only fields from {objectType}")
            
            self.exclusiveGroups = getExclusiveFieldGroups(objectType, list(self.fieldMetadata.keys()))
            debugLog(f"🚀 Building AutoGeneratedForm for: {objectType}", origin="AutoGeneratedForm.__init__")
            debugLog(f"🧩 Field metadata keys: {list(self.fieldMetadata.keys())}", origin="AutoGeneratedForm.__init__")
        except Exception as e:
            debugLog(f"❌ Failed to load field metadata for {objectType}: {e}", origin="AutoGeneratedForm.__init__")
            self.fieldMetadata = {}
        ...
        for group in self.exclusiveGroups:
            ...
            options     = group['options']       # e.g. ["bodyList", "nodeList", …]
            defaultOption = group.get('default', options[0])
        
            # ─── Build the combo‐box for selecting which option in this group is active ───
            combo = QComboBox()
            combo.addItems(options)
            combo.setCurrentText(defaultOption)
        
            # ─── Build a QStackedWidget, one page for each option in group[...] ──────────
            ...
            for opt in options:
                ...
                ) or self.createFieldWidget(opt, meta, self.userVariables)
            ...
            # ─── Place combo and stacked together in a single container ───
            ...
            groupHelpText = ""  # or fetch from metadata if you happen to have group‐level help
        ...
        if 'name' in self.fieldMetadata:
            ...
            if self.existingData.get('name'):
                ...
                debugLog(f"[AutoGeneratedForm] 🆕 Generated unique default name: '{baseName}' -> '{defaultName}'", origin="autoGeneratedForm.py")
            ...
            def validateNameOnChange():
                ...
                if currentName.lower() in existing_names_set:
                    ...
                    nameField.setToolTip(f"❌ Name '{currentName}' already exists! Please choose a different name.")
                    debugLog(f"[NameValidation] ❌ Collision detected: '{currentName}' already exists")
                else:
                    # Name is unique - clear any error styling
                    nameField.setStyleSheet("")
                    nameField.setToolTip("")
                    debugLog(f"[NameValidation] ✅ Name '{currentName}' is unique")
        ...
        for fieldName, meta in self.fieldMetadata.items():
            ...
            if widget is None:
                widget = self.createFieldWidget(fieldName, meta, self.userVariables)

            self.fields[fieldName] = widget
            self.inputFields[fieldName] = widget            # ── ENHANCED: Get help from Exudyn internal documentation ──
                                ...
                                try:
                                    ...
                                    if index is not None and group_type:
                                        ...
                                        if parent_window and hasattr(parent_window, 'mbs'):
                                            ...
                                            debugLog(f"✅ Resolved {selector_field}[{index}] → actual type: '{actual_type}'")
                                            ...
                                            if start_page and end_page:
                                                ...
                                                debugLog(f"⚠️ No PDF help found for '{type_name_normalized}'")
                                        else:
                                            debugLog("⚠️ Could not access MBS from AutoGeneratedForm")
                                    ...
                                    debugLog(f"❌ Error in help resolution: {e}")
                                    import traceback
                                    traceback.print_exc()
    ...
    def _wrapInHBox(self, widget):
        ...
        wrapper.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)  # ⬅ restore normal width
        return wrapper
    ...
    def onHelpClicked(self):
        ...
        # If it's a Create… function, first try to locate the literal definition page
        ...
        # … existing "Object…" logic below …        # For legacy items (Object*, Node*, Marker*, Load*, Sensor*), use the improved page range logic
    ...
    def _makeLabelWithHelp(self, text: str, helpText: str) -> QWidget:
        ...
        # 2) The tiny "ℹ" icon with enhanced tooltip
        infoIcon = QLabel("ℹ")
    ...
    def validateAndPrintCollectedData(self):
        ...
        if not hasattr(self, '_data'):
            debugLog("[validateAndPrint] ⚠️ Forcing accept() to populate data.")
        ...
        for fieldName, rawValue in data.items():
            ...
            if rawValue in ['', None]:
                ...
                if fallback is not None and fallback != '':
                    debugLog(f"[validateAndPrint] 🛠️ Fallback for {fieldName} = {fallback}")
                    rawValue = fallback
        ...
        debugLog("\n📦 Final collected data (including weak/invalid fields):")
    ...
    def buildBodyNumberWidget(fieldName, typeHint, default, parent=None, structure=None):
        ...
        def on_highlight_clicked():
            ...
            try:
                ...
                debugLog(f"⚠️ [Highlighting] Invalid body number: {selected_text}", origin="autoGeneratedForm")
    ...
    def createFieldWidget(self, fieldName, meta, userVariables=None):
        ...
        if self.objectType == "CreateRigidBodySpringDamper" and fieldName == "offset":
            value = self.existingData.get(fieldName, meta.get("default", [0,0,0,0,0,0]))
            return buildVec6Widget(fieldName, value=value, parent=self, meta=meta, default=meta.get("default", [0,0,0,0,0,0]), userVariables=userVariables or {})

        # --- Always use matrix widget for Matrix3D fields ---
        if fieldType in ("matrix3d", "matrix3x3[list]", "matrix"):  # support all naming variants
            value = self.existingData.get(fieldName, defaultValue)
            return buildMatrix3x3Widget(fieldName, value=value, parent=self, meta=meta, default=defaultValue, userVariables=userVariables or {})
        ...
        if lname == "referencelength" and self.objectType == "CreateSpringDamper":
            line = QLineEdit()
            line.setPlaceholderText("leave empty → computed automatically")
            return line
        ...
        if self.objectType == "CreateRigidBodySpringDamper" and fieldName in ("stiffness", "damping"):
            ...
            value = self.existingData.get(fieldName, meta.get("default", np.zeros((6,6)).tolist()))
            return buildMatrix6x6Widget(fieldName, value=value, parent=self, meta=meta, default=meta.get("default", np.zeros((6,6)).tolist()))
        ...
        # === Step 1: "vector" or "list" types get a QLineEdit that can accept [ … ] literals or variables ≔ "[m,0,0]" ===
        if any(substr in fieldType for substr in ("vector", "list")):
            line = QLineEdit()
            line.setPlaceholderText("e.g. [m, 0, 0] or [100, 100, 100]")

            # If there's a pure‐python default (like [100,100,100]), display it in gray:
            if defaultValue not in ("", None):
                line.setText(str(defaultValue))
                line.setStyleSheet("color: gray;")

                def on_focus():
                    # clear the gray default when user clicks into the box
                    if line.text() == str(defaultValue):
                        line.clear()
                    line.setStyleSheet("color: black;")

                # Override focusInEvent to call on_focus() first:
                orig_focus = line.focusInEvent
                line.focusInEvent = lambda ev, f=line: (on_focus(), orig_focus(ev))

            return line



        # === Step 2: Boolean fields → QCheckBox ===
        if fieldType == "bool":
            box = QCheckBox()
            box.setChecked(bool(defaultValue))
            return box

        # === Step 3: Everything else → plain QLineEdit ===
    ...
    def get_data(self):
        if hasattr(self, '_data'):
            # 🔁 Patch: ensure excluded fields have safe values
            for group in getattr(self, 'exclusiveGroups', []):
                ...
                for fieldName in group['options']:
                    if fieldName == selected:
                        continue
                    # 🧩 Ensure excluded fields are always present with safe default
        ...
        debugLog("[AutoGeneratedForm.get_data] ⚠️ Called before accept()", origin="AutoGeneratedForm")
        return {}
        ...
        for fieldName, widget in self.fields.items():
            ...
            try:
                ...
                if shouldPreserveVariable:
                    # 🧠 Preserve symbolic input like 'm' or 'k+3*m' for post-substitution
                    debugLog(f"[accept] 🧬 Preserving symbolic variable for {fieldName}: {rawValue}")
                    converted = rawValue
                else:
                    try:
                        converted = convertToType(rawValue, metaType)
                    except Exception as e:
                        debugLog(f"[accept] ❌ Failed to convert {fieldName}='{rawValue}' to {metaType}: {e}")
                        converted = rawValue
    ...
    def collectAndNormalizeData(self):
        ...
        for fieldName, widget in self.fields.items():
            ...
            if fieldName.lower() == "inertia":
                ...
                try:
                    ...
                    debugLog(f"[accept] ❌ Failed to evaluate inertia: {e}", origin="AutoGeneratedForm")
                    finalValue = None


            # ─────────────── STEP 1 (VECTOR3): if metaType contains "vector" & "3",
            ...
            if isinstance(metaType, str) and "vector" in metaType and "3" in metaType:
                ...
                # not a vector3 field → normal extract
                rawValue = extractValueFromWidget(widget)

            debugLog(f"[accept] 🔎 {fieldName} raw = {repr(rawValue)} "
                     f"(type: {type(rawValue).__name__})")

            # ─────────────── STEP 2: If rawValue is a string AND metaType says "vector" or "list",
            # attempt to parse … but do NOT call evaluateExpression(token) on every token.
            if isinstance(rawValue, str) and any(substr in metaType for substr in ("vector", "list")):
                # txt = rawValue.strip()
                # # 2.a) If it looks like "[ … ]", try literal_eval → Python list
                ...
                #             converted = py_list
                # else:
                    # 2.b) Maybe user typed "k" (no brackets) → we'll just store the string "k"
                    # (don't call evaluateExpression here, because we want to remain symbolic)
                converted = rawValue

            # ─────────────── STEP 3: If rawValue is some other string (e.g. "123" or "{…}" or "true"),
            # try a simple literal_eval
                ...
                if (
                    ...
                    try:
                        ...
                        debugLog(f"[accept] 🧠 Parsed simple literal for {fieldName}: {converted!r}")
                    except Exception:
                        converted = rawValue
                else:
                    converted = rawValue

            # ─────────────── STEP 4: Otherwise (rawValue is not a string), keep it as-is
            else:
                converted = rawValue

            # ─────────────── STEP 5: If conversion produced "empty," fall back to metadata default
            if converted in ("", None):
                fallback = meta.get("default")
                if fallback not in ("", None):
                    debugLog(f"[accept] 🛠️ Fallback for {fieldName}: {fallback}")
                    converted = fallback

            # ─────────────── STEP 6 (UserFunction fields): keep symbolic if user typed a name
            if fieldName.endswith("UserFunction"):
            ...
            # ─────────────── STEP 7 (graphicsDataList): strip down each entry to {name, args}
            ...
            # ─────────────── STEP 8: If it's a list that still contains a string,
            ...
            if isinstance(converted, list) and any(isinstance(x, str) for x in converted):
                ...
                try:
                    ...
                    debugLog(f"[accept] ❌ Failed to convert {fieldName}={converted!r} → {meta.get('type')}: {e}")
                    finalValue = converted
            ...
            # ─────────────── STEP 9: Now run the validator (and color red/yellow if needed)
            ...
            if not isValid:
                ...
                debugLog(f"[accept] ⚠️ {fieldName}: {msg}")
                ...
                hasWarnings = True
                warningFields.append(fieldName)
                debugLog(f"[accept] ⚠️ {fieldName} is weak: {finalValue!r}")

            debugLog(f"[accept] ✅ Stored: {fieldName} = {repr(finalValue)}")
            self.values[fieldName] = finalValue

        # ─────────────── STEP 10: Validate name for duplicates before final submission ───────────────
        ...
        if name.lower() in existing_names_set:
            ...
            nameWidget.setToolTip(f"❌ Name '{name}' already exists! Please choose a different name.")
            ...
            debugLog(f"[accept] ❌ BLOCKED: Form submission prevented due to duplicate name: '{name}'")
            return  # Prevent form submission
          # ─────────────── STEP 11: Filter out GUI-only fields for legacy items before final submission ───────────────
        ...
        if (self.objectType.startswith(('Object', 'Node', 'Marker', 'Load', 'Sensor')) and 
            ...
            if filtered_fields:
                debugLog(f"🔧 [AutoGeneratedForm.accept] Final filtering of GUI-only fields from {self.objectType}: {filtered_fields}")
        ...
        def parseGraphicsString(entry):
            ...
            if isinstance(entry, str) and entry.startswith('exu.graphics.') and '(' in entry:
                try:
                    ...
                    debugLog(f"[WARNING] Failed to parse graphics string: {entry} → {e}", origin="AutoGeneratedForm.parseGraphicsString")
        ...
        if 'graphicsDataList' in data and isinstance(data['graphicsDataList'], list):
            data['graphicsDataList'] = [parseGraphicsString(e) for e in data['graphicsDataList']]        # ─────────────── STEP 12: Finally save validated name and call super() ───────────────
        self.values["name"] = name
        self._data = self.values
        self.accepted = True

        debugLog(f"[accept] ✅ Final form data: {summarizeDict(self.values)}")
          # 🎯 SAVE VIEW STATE: Right after user clicks OK, before form closes
        try:
            ...
            if parent_window and saveViewStateWithAutoRestore(parent_window, "after_ok_clicked", duration=2.0):
                debugLog("[accept] 📌 Saved view state with auto-restore after OK clicked", origin="AutoGeneratedForm")
            else:
                debugLog("[accept] ⚠️ Failed to save view state with auto-restore after OK clicked", origin="AutoGeneratedForm")
        except Exception as e:
            debugLog(f"[accept] ⚠️ View state auto-restore setup failed: {e}", origin="AutoGeneratedForm")
    ...
    def _setupRendererHighlighting(self):
        ...
        """
        debugLog(f"🎯 [Highlighting] Setting up multi-color renderer highlighting for {self.objectType}", origin="AutoGeneratedForm")
        ...
        if hasattr(self, 'SC') and self.SC:
            SC = self.SC
            debugLog(f"✅ [Highlighting] Using directly provided SC: {type(SC)}", origin="autoGeneratedForm")
        elif hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'SC'):
            SC = self.parent.SC  # Very visible debug
            debugLog(f"✅ [Highlighting] Using SC from parent: {type(SC)}", origin="autoGeneratedForm")
        
        # Check if we actually found an SC
        if not SC: # Very visible debug
            debugLog("⚠️ [Highlighting] No SC available (neither direct nor from parent)", origin="autoGeneratedForm")
            if hasattr(self, 'parent') and self.parent:
                debugLog(f"🔍 [Highlighting] Parent type: {type(self.parent)}", origin="AutoGeneratedForm")
                debugLog(f"🔍 [Highlighting] Parent has SC? {hasattr(self.parent, 'SC')}", origin="AutoGeneratedForm")
                debugLog(f"🔍 [Highlighting] Parent attributes: {[attr for attr in dir(self.parent) if not attr.startswith('_')][:10]}", origin="AutoGeneratedForm")
            return
        # Very visible debug
        debugLog(f"🚀 [Highlighting] Proceeding with multi-color SC setup", origin="AutoGeneratedForm")
        ...
        debugLog(f"🔍 [Highlighting] Available fields: {list(self.fields.keys())}", origin="AutoGeneratedForm")
        ...
        debugLog(f"✅ [Highlighting] Connected {connected_count} fields for multi-color highlighting", origin="AutoGeneratedForm")
        ...
        for fieldName, widget in self.fields.items():
            ...
            if field_lower in dual_field_mappings:
                ...
                if hasattr(widget, 'combo0') and hasattr(widget, 'combo1'):
                    ...
                    debugLog(f"🔗 [Dual-Field] Found combo0/combo1 in {fieldName}", origin="AutoGeneratedForm")
                    ...
                    debugLog(f"🔗 [Dual-Field] Found combo1/combo2 in {fieldName}", origin="AutoGeneratedForm")
                    ...
                    if len(combos) >= 2:
                        combo0 = combos[0]
                        combo1 = combos[1]
                        debugLog(f"🔗 [Dual-Field] Found child combos in {fieldName}: {len(combos)} total", origin="AutoGeneratedForm")
                ...
                if combo0 and combo1:
                    ...
                    def create_highlight_function(widget_ref=widget, SC_ref=SC):
                        def do_highlight():
                            ...
                            try:
                                ...
                                debugLog(f"🎨 [Dual-Field] Highlighted {widget_ref.fieldName}: item0={item0}, item1={item1}", origin="AutoGeneratedForm")
                                
                            except Exception as e:
                                debugLog(f"❌ [Dual-Field] Failed to highlight {widget_ref.fieldName}: {e}", origin="AutoGeneratedForm")
                        ...
                        def clear_highlight():
                            ...
                            try:
                                ...
                                # Update the renderer
                                self._updateActiveHighlighting(SC_ref)
                                debugLog(f"🎨 [Dual-Field] Cleared highlighting for {widget_ref.fieldName}", origin="AutoGeneratedForm")
                                
                            except Exception as e:
                                debugLog(f"❌ [Dual-Field] Failed to clear highlighting for {widget_ref.fieldName}: {e}", origin="AutoGeneratedForm")
                    ...
                    try:
                        ...
                        debugLog(f"🔗 [Dual-Field] Connected combo change signals for {fieldName}", origin="AutoGeneratedForm")
                    except Exception as e:
                        debugLog(f"⚠️ [Dual-Field] Failed to connect combo change signals for {fieldName}: {e}", origin="AutoGeneratedForm")
                    ...
                    debugLog(f"🎨 [Dual-Field] Added highlight button for {fieldName} with colors: combo0={color0}, combo1={color1}", origin="AutoGeneratedForm")
                    connected_count += 1
                else:
                    debugLog(f"⚠️ [Dual-Field] Could not find dual combo boxes in {fieldName}", origin="AutoGeneratedForm")
    ...
    def _connectSingleFieldToHighlighting(self, fieldName, widget, SC, highlight_type):
        ...
        try:
            ...
            debugLog(f"🔗 [Single-Field] Connected {fieldName} with color {field_color}", origin="AutoGeneratedForm")
    ...
    def _highlightItemWithColor(self, SC, highlight_type, item_index, fieldName):
        ...
        debugLog(f"🎯 [Multi-Color] _highlightItemWithColor called: field={fieldName}, type={highlight_type}, index={item_index}", origin="AutoGeneratedForm")
        
        try:
            if item_index is None:
                # Clear highlighting for this field
                debugLog(f"🎯 [Multi-Color] Clearing highlighting for field {fieldName}", origin="AutoGeneratedForm")
            ...
            if isinstance(item_index, str):
                try:
                    ...
                    debugLog(f"🎯 [Multi-Color] Converted string '{item_index}' to int", origin="AutoGeneratedForm")
                    item_index = item_index_int
                except (ValueError, TypeError):
                    debugLog(f"⚠️ [Multi-Color] Failed to convert '{item_index}' to int", origin="AutoGeneratedForm")
                    return
            ...
            if not item_type:
                debugLog(f"❌ [Multi-Color] Unknown highlight type: {highlight_type}", origin="AutoGeneratedForm")
                return
                
            # Get field-specific color
            field_color = self._fieldColors.get(fieldName, self._predefinedColors['primary'])
            
            debugLog(f"🎯 [Multi-Color] Mapped {highlight_type} to {item_type} with color {field_color}", origin="AutoGeneratedForm")
            ...
            debugLog(f"❌ [Multi-Color] Failed to highlight item {item_index} of type {highlight_type}: {e}", origin="AutoGeneratedForm")
    ...
    def _updateActiveHighlighting(self, SC):
        ...
        try:
            if not hasattr(SC, 'visualizationSettings') or not hasattr(SC.visualizationSettings, 'interactive'):
                debugLog(f"⚠️ [Multi-Color] No visualizationSettings.interactive available", origin="AutoGeneratedForm")
                return
            ...
            if not active_highlights:                # No active highlights - clear everything
                ...
                debugLog(f"✅ [Multi-Color] Cleared all highlighting", origin="AutoGeneratedForm")
                
                # 🚀 Force OpenGL window refresh to show changes
                ...
                debugLog(f"✅ [Multi-Color] Single highlight: {highlight['type']} #{highlight['index']} with color {highlight['color']}", origin="AutoGeneratedForm")
                
                # 🚀 Force OpenGL window refresh to show highlighting
                self._refreshOpenGLRenderer()
                ...
                debugLog(f"🔄 [Multi-Color] Starting rotation for {len(active_highlights)} highlights", origin="AutoGeneratedForm")
                self._startHighlightRotation(SC, active_highlights)

        except Exception as e:
            debugLog(f"❌ [Multi-Color] Failed to update active highlighting: {e}", origin="AutoGeneratedForm")
    ...
    def _startHighlightRotation(self, SC, highlights):
        ...
        try:
            ...
            debugLog(f"🔄 [Multi-Color] Started highlight rotation with {len(highlights)} items", origin="AutoGeneratedForm")
        except Exception as e:
            debugLog(f"❌ [Multi-Color] Failed to start highlight rotation: {e}", origin="AutoGeneratedForm")
    ...
    def _rotateHighlight(self):
        ...
        try:
            ...
            debugLog(f"🔄 [Multi-Color] Rotated to highlight {self._rotationIndex}: {current_highlight['type']} #{current_highlight['index']} with color {current_highlight['color']}", origin="AutoGeneratedForm")
            
            # 🚀 Force OpenGL window refresh to show rotation
            self._refreshOpenGLRenderer()
            ...
            debugLog(f"❌ [Multi-Color] Failed to rotate highlight: {e}", origin="AutoGeneratedForm")
    ...
    def _getFieldColor(self, fieldName):
        ...
        self._fieldColors[fieldName] = color
        debugLog(f"🎨 [Highlighting] Assigned color {color} to field {fieldName}", origin="AutoGeneratedForm")
        return color
        ...
        try:
            ...
            if hasattr(self, 'parent') and self.parent:
                if hasattr(self.parent, 'rendererSettings') and hasattr(self.parent.rendererSettings, 'openGL'):
                    ...
                    debugLog(f"🚀 [Multi-Color] Forced OpenGL refresh via parent.rendererSettings", origin="AutoGeneratedForm")
                    return

                if hasattr(self.parent, 'forceOpenGLUpdate'):
                    self.parent.forceOpenGLUpdate()
                    debugLog(f"🚀 [Multi-Color] Forced OpenGL refresh via parent.forceOpenGLUpdate()", origin="AutoGeneratedForm")
                    return

                if hasattr(self.parent, 'update'):
                    self.parent.update()
                    debugLog(f"🚀 [Multi-Color] Forced refresh via parent.update()", origin="AutoGeneratedForm")
                    return
            ...
            try:
                ...
                activateOpenGL()
                debugLog(f"🚀 [Multi-Color] Forced OpenGL refresh via activateOpenGL()", origin="AutoGeneratedForm")
            except ImportError:
                debugLog(f"⚠️ [Multi-Color] Could not import activateOpenGL for refresh", origin="AutoGeneratedForm")

        except Exception as e:
            debugLog(f"❌ [Multi-Color] Failed to refresh OpenGL renderer: {e}", origin="AutoGeneratedForm")
    ...
    def _updateDualFieldHighlights(self):
        ...
        try:
            ...
            debugLog(f"❌ [Dual-Field] Failed to update dual field highlights: {e}", origin="AutoGeneratedForm")
    ...
    def _highlightDualFieldWidget(self, widget, SC):
        ...
        try:
            ...
            # Update the renderer
            self._updateActiveHighlighting(SC)
            debugLog(f"🎨 [Dual-Field] Updated highlight for {fieldName}: item0={item0}, item1={item1}", origin="AutoGeneratedForm")
            
        except Exception as e:
            debugLog(f"❌ [Dual-Field] Failed to highlight widget {getattr(widget, 'fieldName', repr(widget))}: {e}", origin="AutoGeneratedForm")
    ...
    def _replayHighlightState(self):
        ...
        try:
            ...
            if hasattr(self, '_activeHighlights') and self._activeHighlights:
                ...
                if SC:
                    self._updateActiveHighlighting(SC)
                    debugLog(f"🔄 [Highlight] Replayed highlight state with {len(self._activeHighlights)} active highlights", origin="AutoGeneratedForm")
            
        except Exception as e:
            debugLog(f"❌ [Highlight] Failed to replay highlight state: {e}", origin="AutoGeneratedForm")
