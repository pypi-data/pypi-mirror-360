...
import torch
from scarabs.nova.utils import ModelOutput, PretrainedConfig, PreTrainedModel
...
class CtrWithDeepFMConfig(PretrainedConfig):
    model_type = "CtrWithDeepFM"
...
class CtrWithDeepFM(PreTrainedModel):
    """
    模型：DeepFM
    结构：
    fea1     fea2      fea3     fea4
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.out = torch.nn.Linear(
            config.dnn_hidden_units[-1] + config.embedding_dim + 1, 1
        )
    ...
    def forward(self, **kwargs):
        ...
        _fm_out = torch.sum(_out, dim=1) ** 2 - torch.sum(_out**2, dim=1)

        _dnn_out = self.mlp_layer(_out.flatten(start_dim=1))

        _lr_out = self.logitistic_layer(**kwargs)

        logits = torch.concat([_fm_out, _dnn_out, _lr_out], dim=-1)
