...
class RawFeature(Feature):
    ...
    def handle(self, value):
        ...
        if len(new_value) > self.length:
            ...
            new_value = new_value + [self.default] * (self.length - len(new_value))
        return new_value
        ...
        if meta_dir is not None:
            ...
            with open(meta_dir, "r") as f:
                ...
                for item, value in feature2meta_json["features"].items():
                    ...
                    if value["target"] == "RawFeature":
                        self.feature2meta[item] = RawFeature(**value)

                assert len(self.feature2meta) == len(feature2meta_json["features"]), (
            ...
            assert len(self.feature2meta) == len(config.features), (
                ">>>>> feature2meta unmatched <<<<<"
            )
        else:
