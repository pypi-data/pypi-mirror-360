...
import cciplus_ado.utils.common.artifacttool_updater as updater
...
def test__get_artifacttool_override_url(monkeypatch, mock_uuid):
    ...
    with mock.patch("os.path.exists", return_value=False), mock.patch(
        "cciplus_ado.utils.common.artifacttool_updater._update_artifacttool"
    ) as upd:
...
def test__get_artifacttool_release_exists(monkeypatch, mock_uuid):
    ...
    atu = updater.ArtifactToolUpdater()
    with mock.patch(
        "cciplus_ado.utils.common.artifacttool_updater._get_current_release",
