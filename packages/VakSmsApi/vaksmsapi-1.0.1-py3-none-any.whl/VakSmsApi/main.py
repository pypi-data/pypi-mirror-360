...
class VakSmsApi:
    ...
    def get_number(self, service, country, operator):
        url=f"{self.base_url}getNumber/?apiKey={self.api_key}&service={service}&country={country}&operator={operator}&softId=1019"
        response = requests.get(url)
        if response.status_code == 200:
            return response.json()
        else:
    ...
    def prolongNumber(self, service, tel):
        url=f"{self.base_url}prolongNumber/?apiKey={self.api_key}&service={service}&tel={tel}&softId=1019"
        response = requests.get(url)
        if response.status_code == 200:
            return response.json()
        else:
