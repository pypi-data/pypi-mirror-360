...
class FeatrixSphereClient:
    ...
    def create_session(self, session_type: str = "sphere") -> SessionInfo:
        ...
        response_data = self._post_json("/session", {})
    ...
    def get_session_status(self, session_id: str) -> SessionInfo:
        ...
        """
        response_data = self._get_json(f"/session/{session_id}")
    ...
    def create_embedding_space(self, name: str, s3_training_dataset: str, s3_validation_dataset: str) -> SessionInfo:
        ...
        response_data = self._post_json("/create-embedding-space", data)
    ...
    def upload_file_and_create_session(self, file_path: Path) -> SessionInfo:
        ...
        with open(file_path, 'rb') as f:
            ...
            response = self._make_request("POST", "/upload_with_new_session/", files=files)
    ...
    def upload_df_and_create_session(self, df=None, filename: str = "data.csv", file_path: str = None, 
        ...
        response = self._make_request("POST", "/upload_with_new_session/", files=files, data=data)
    ...
    def predict(self, session_id: str, record: Dict[str, Any], max_retries: int = None) -> Dict[str, Any]:
        ...
        """
        response_data = self._post_json(f"/session/{session_id}/predict", record, max_retries=max_retries)
        return response_data
    ...
    def get_training_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        """
        response_data = self._get_json(f"/session/{session_id}/training_metrics")
        return response_data
    ...
    def predict_records(self, session_id: str, records: List[Dict[str, Any]], 
                       batch_size: int = 250, use_async: bool = None) -> Dict[str, Any]:
        """
        ...
        if (total_records >= ASYNC_THRESHOLD and use_async is not False) or use_async is True:
            ...
            try:
                ...
                if result.get('async') and result.get('job_id'):
                    ...
                    # Ask user if they want to watch the job
                    try:
                        watch_job = input("\n🔄 Watch job progress? [Y/n]: ").strip().lower()
                        if watch_job in ['', 'y', 'yes']:
                        ...
                        # Handle Ctrl+C or non-interactive environments
                        print(f"\n📋 Job submitted. Use client.watch_prediction_job('{session_id}', '{result['job_id']}') to monitor progress.")
                        return result
                else:
...
"""
    ...
    def update_prediction_label(self, prediction_id: str, user_label: str) -> Dict[str, Any]:
        ...
        response_data = self._post_json(f"/prediction/{prediction_id}/update_label", data)
        return response_data
